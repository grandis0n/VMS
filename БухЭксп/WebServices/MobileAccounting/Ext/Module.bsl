...
Функция ConfirmGettingFile(УзелОбменаКод, НомерСообщения, СообщениеОбОшибке)
	...
	УзелОбъект.ДатаСинхронизации               = ДатаСинхронизации;
	УзелОбъект.ПервичнаяИнициализацияВыполнена = Истина;
	УзелОбъект.Записать();
	
	//Установим дату последней активности абонента
	Если ОбщегоНазначенияБП.ЭтоБизнесСтарт() Тогда
		МобильнаяБухгалтерия.ЗапуститьДлительнуюОперациюУстановкиДатыПоследнейАктивности(ДатаСинхронизации);
	КонецЕсли;
