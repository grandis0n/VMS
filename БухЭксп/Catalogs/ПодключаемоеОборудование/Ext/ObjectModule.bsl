...
// ++ Локализация
	Если ОбщегоНазначенияБПО.ИспользуетсяТСПИоТ() Тогда 
		ТСПИоТОборудования = МенеджерОборудованияТСПИОТ.ПолучитьТСПИоТ(ЭтотОбъект.Ссылка);
		Если ЗначениеЗаполнено(ТСПИоТОборудования) Тогда
			Если ТСПИоТОборудования.Тип = Перечисления.ТипыТСПИоТ.Внутренний1С И Не ЭтотОбъект.ПометкаУдаления Тогда
			...
			ИначеЕсли ТСПИоТОборудования.Тип = Перечисления.ТипыТСПИоТ.Внутренний1С И ЭтотОбъект.ПометкаУдаления 
