...
Функция ЗаполнитьДанныеУстройства(Выборка, ТекущийПользовательИБ, КодЯзыка)
	...
	ТСПИоТИдентификатор = Неопределено;  
	ТСПИоТПлатформа  = Неопределено;     
	ТСПИоТПутьДанных = Неопределено;  
	// ++ НеМобильноеПриложение 
	...
	Если Выборка.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ 
		И ОбщегоНазначенияБПО.ИспользуетсяТСПИоТ() Тогда                   
			...
			Если ЗначениеЗаполнено(ТСПИоТ) Тогда 
				...
				ТСПИоТИдентификатор = ТСПИоТ.ИдентификаторЭкземпляра;  
				ТСПИоТПлатформа = ТСПИоТ.ТипПлатформыКлиента;    
				ТСПИоТПутьДанных = ТСПИоТ.ПутьДанныхЭкземпляра;
			КонецЕсли;
	КонецЕсли;
	// -- НеМобильноеПриложение
	...
	ДанныеУстройства.Вставить("ТСПИоТИдентификатор", ТСПИоТИдентификатор);   
	ДанныеУстройства.Вставить("ТСПИоТПлатформа"    , ТСПИоТПлатформа);  
	ДанныеУстройства.Вставить("ТСПИоТПутьДанных"   , ТСПИоТПутьДанных);
	// -- Локализация
