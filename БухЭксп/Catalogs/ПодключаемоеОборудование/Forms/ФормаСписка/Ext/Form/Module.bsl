...
Асинх Процедура СписокУстройствПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяТСПИоТ() Тогда  
		ПроверкаИнициализацияПИоТ = МенеджерОборудованияКлиент.ПроверкаИнициализацияПИоТПройдена(ВыбранноеОборудование);    
		Если ПроверкаИнициализацияПИоТ Тогда               
			ТекстПредупреждения = НСтр("ru = 'Для экземпляр ККТ запущен ТС ПИоТ.
							   |Пометка на удаление невозможна.
							   |Для продолжения необходимо перегрузить «1С:Предприятие».'");  
