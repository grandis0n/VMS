...
Процедура ПодключитьТСПИоТ_Продолжить(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда     
		...
		#Иначе     
			РабочийКаталог = "";
		#КонецЕсли
			ТипПлатформыКлиента = Строка(ОбщегоНазначенияБПОКлиент.ТипПлатформыКлиента());
			СоздатьНовыйТСПИоТ(РабочийКаталог, ТипПлатформыКлиента);       
...
&НаКлиенте
Процедура АутентификацияПользователяТСПИоТ(Команда)
// ++ НеМобильноеПриложение
// ++ Локализация
	Если ОбщегоНазначенияБПОклиент.ИспользуетсяТСПИоТ() Тогда 
		...
		МодульМенеджерОборудованияТСПИоТКлиент.ОткрытьАутентификацииПользователяПИоТ(Объект.Ссылка)
	КонецЕсли;                                              
// -- Локализация                          
// -- НеМобильноеПриложение 
КонецПроцедуры
...
&НаКлиенте
Процедура СобытияЭксплуатацииТСПИоТ(Команда)
// ++ НеМобильноеПриложение
// ++ Локализация
	Если ОбщегоНазначенияБПОклиент.ИспользуетсяТСПИоТ() Тогда 
		МодульМенеджерОборудованияТСПИоТКлиент = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("МенеджерОборудованияТСПИоТКлиент");
		МодульМенеджерОборудованияТСПИоТКлиент.ОткрытьСобытияЭксплуатацииТСПИоТ(Объект.Ссылка)
	КонецЕсли;                                              
// -- Локализация
// -- НеМобильноеПриложение 
КонецПроцедуры
...
// ++ Локализация 
	...
	Элементы.ДополнительноПИОТ.Видимость = ТипОборудованияККТ И ЗначениеЗаполнено(ТСПИоТ); 
...
&НаСервере
Процедура СоздатьНовыйТСПИоТ(РабочийКаталог, ТипПлатформыКлиента)
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяТСПИоТ() Тогда     
		...
		ПараметрыСоздания.ТипПлатформыКлиента = ТипПлатформыКлиента;
...
// ++ Локализация             
	Если ОбщегоНазначенияБПО.ИспользуетсяТСПИоТ() Тогда
		МодульМенеджерОборудованияТСПИоТ = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОборудованияТСПИоТ");      
		ТСПИоТ = МодульМенеджерОборудованияТСПИоТ.ПолучитьТСПИоТ(ПодключаемоеОборудование); 
		Если ЗначениеЗаполнено(ТСПИоТ) Тогда 
			СобытияЭксплуатации = МодульМенеджерОборудованияТСПИоТ.АрхивСобытияЭксплуатации(ПодключаемоеОборудование);
			Адрес = ПоместитьВоВременноеХранилище(СобытияЭксплуатации.ДанныеАрхива);
			ПередаваемыйФайл = Новый ОписаниеПередаваемогоФайла;
			ПередаваемыйФайл.Хранение = Адрес;
			ПередаваемыйФайл.Имя = СтрШаблон("events-%1.txt", Формат(СобытияЭксплуатации.ДатаАрхива, "ДФ=dd-MM-yyyy-hh-mm-ss"));
			ПередаваемыеФайлы.Добавить(ПередаваемыйФайл);  
		КонецЕсли;             
	КонецЕсли;             
// -- Локализация 
