...
Процедура ПередЗаписью(Отказ)
	...
	ИначеЕсли Не ЭтоГруппа И ТипОплаты = Перечисления.ТипыОплат.ЦифровойРубль Тогда
		ДанныеБанка = Справочники.ВидыОплатОрганизаций.ДанныеКонтрагентаПлатформаЦифровогоРубля(Истина);
		Если ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(Контрагент) Тогда
			Если Контрагент <> ДанныеБанка.Ссылка Тогда
				РеквизитыЦБ = ЦифровойРубль.РеквизитыОператораПлатформыЦифровогоРубля();
				ШаблонСообщения = СтрШаблон(
					НСтр("ru = 'Для способа оплаты ""Цифровой рубль"" контрагентом должен быть %1'"),
					РеквизитыЦБ.Наименование);
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Корректность",
					НСтр("ru = 'Контрагент'"),,, ШаблонСообщения);
			...
			Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
				ВидОплаты = Справочники.ВидыОплатОрганизаций.ВидОплатыПоДоговоруИТипуОплаты(
					Организация, Контрагент, ДоговорКонтрагента, Перечисления.ТипыОплат.ЦифровойРубль);
				Если ЗначениеЗаполнено(ВидОплаты) И (ЭтоНовый() Или ВидОплаты <> Ссылка) Тогда
					ШаблонСообщения = НСтр("ru = 'Вид оплаты с такими реквизитами уже существует'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Корректность",
						НСтр("ru = 'Контрагент'"),,, ШаблонСообщения);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Контрагент", "Объект", Отказ);
					Возврат;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Если ТипОплаты = Перечисления.ТипыОплат.ЦифровойРубль Тогда // будут созданы при записи
			БИКЦР = ЦифровойРубль.БИКПлатформыЦифровогоРубля();
			БанкЦР = РаботаСБанкамиБП.НайтиБанк(БИКЦР);
			Если ЗначениеЗаполнено(БанкЦР) Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Контрагент");
				МассивНепроверяемыхРеквизитов.Добавить("ДоговорКонтрагента");
			КонецЕсли;
		КонецЕсли;
