...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Элементы.ГруппаОтражениеВРасходах.Видимость Тогда
		ТекущийОбъект.УчитыватьНДСПоКомиссии = ОтражениеВРасходах = 1;
	Иначе
		ТекущийОбъект.УчитыватьНДСПоКомиссии = Ложь;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	ИначеЕсли Объект.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплат.ЦифровойРубль") Тогда
		Элементы.Контрагент.ПодсказкаВвода = НСтр("ru = 'БАНК РОССИИ (Оператор платформы цифрового рубля)'");
		ЭтоЦифровойРубль       = Истина;
	...
	Элементы.ГруппаОтражениеВРасходах.Видимость = ВидимостьКомиссииБанка И ВыделятьНДССКомиссииБанка
		И Не ЭтоЦифровойРубль;
	
	ЗаголовокСчетРасчетов = ЗаголовокСчетаРасчетов;
	
	ДоступностьРеквизитовЦР = ЭтоЦифровойРубль И Не ЗначениеЗаполнено(Объект.Ссылка); // ограничиваем только для нового элемента
	Элементы.Контрагент.ТолькоПросмотр = ДоступностьРеквизитовЦР;
	Элементы.ДоговорКонтрагента.Доступность = ЗначениеЗаполнено(Объект.Организация)
		И ЗначениеЗаполнено(Объект.Контрагент) И Не ДоступностьРеквизитовЦР;
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Элементы.ТипОплаты.СписокВыбора.Количество() = 0 Тогда
		...
		Элементы.ТипОплаты.СписокВыбора.Добавить(Перечисления.ТипыОплат.ЦифровойРубль);
...
&НаСервере
Процедура ОбработатьЦифровойРубль()
	
	Если Объект.ТипОплаты = Перечисления.ТипыОплат.ЦифровойРубль Тогда
		ДанныеБанка = Справочники.ВидыОплатОрганизаций.ДанныеКонтрагентаПлатформаЦифровогоРубля();
		Если Не ЗначениеЗаполнено(Объект.Контрагент) И Объект.Контрагент <> ДанныеБанка.Ссылка Тогда
			Объект.Контрагент = ДанныеБанка.Ссылка;
		КонецЕсли;
	КонецЕсли;
