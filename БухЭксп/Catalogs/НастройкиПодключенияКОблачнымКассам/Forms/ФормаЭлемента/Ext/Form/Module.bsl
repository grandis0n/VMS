...
Функция ЗаполнитьДоступныеОблачныеКассыНаСервере(Знач АдресРезультата)
	...
	Если ПустаяСтрока(РезультатОперации.КодОшибки) Тогда
		...
		Если Не ПустаяСтрока(Объект.Идентификатор)
			И Элементы.Идентификатор.СписокВыбора.НайтиПоЗначению(Объект.Идентификатор) = Неопределено Тогда
			
			Элементы.Идентификатор.СписокВыбора.Добавить(Объект.Идентификатор, Объект.Идентификатор);
