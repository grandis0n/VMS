...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененаНастройкаОбменаСБП" Или ИмяСобытия = "ИзмененаНастройкаОбменСБанками" Тогда
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПравоДоступаИсторияРегистрацииПФР Тогда
		ОтключитьПроверкуДатыЗапретаИзменения = ТекущийОбъект.ДополнительныеСвойства.Свойство("ЗаписьНового")
			Или ОбъектПерезаполненПоИНН; 
		РедактированиеПериодическихСведений.ЗаписатьЗаписьПослеРедактированияВФорме(ЭтотОбъект,
			"ИсторияРегистрацийВОрганеПФР",
			ОрганизацияСсылка,
			,
			Новый Структура("ОтключитьПроверкуДатыЗапретаИзменения", ОтключитьПроверкуДатыЗапретаИзменения));
	КонецЕсли;
...
Процедура ПерезаполнитьРеквизитыЗавершение(Ответ, АдресРеквизитовОрганизации) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		УстановитьСвойстваФизическогоЛицаПослеЗаполнения();
		ОбъектПерезаполненПоИНН = Истина;
	КонецЕсли;
...
Процедура ПодсказкаНомерСчетаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ДиректБанкПомощникПодключенияОткрыть" Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ТолькоОткрытьПомощник", Истина);
		ОбменСБанкамиКлиент.ОткрытьСоздатьНастройкуОбмена(Объект.Ссылка, БанковскийСчет.Банк, БанковскийСчет.НомерСчета, ,
			ДополнительныеПараметры);
	КонецЕсли;
...
Процедура ПоказатьЭлементыНастроекПодключения(Форма, ТекущийТекстНомераСчета = "")
	...
	ВидимостьИзмененияПодключенияДиректБанка = Ложь;
	...
	Если РазрешенаНастройкаОбмена Тогда
		...
		ВидимостьИзмененияПодключенияДиректБанка = Форма.ВидимостьЭлементовДиректБанк = "ПоказатьИзменениеПодключения"
			Или Форма.ВидимостьЭлементовДиректБанк = "ПоказатьТолькоИнтегратора";
	КонецЕсли;
	...
	Если ВидимостьИзмененияПодключенияДиректБанка Тогда
		ПодсказкаИзменения = ПодсказкаИзмененияПодключенияДиректБанка(Объект.Ссылка, Форма.БанковскийСчет.Ссылка);
		СоставСтрокиПодсказки.Добавить(ПодсказкаИзменения);
		
		Если Форма.ВидимостьЭлементовДиректБанк = "ПоказатьИзменениеПодключения" Тогда
			СтрокаГиперСсылки = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Изменить'"),, Форма.ЦветГиперссылки,, "ДиректБанкПомощникПодключенияОткрыть");
			СоставСтрокиПодсказки.Добавить(СтрокаГиперСсылки);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПодсказкаИзмененияПодключенияДиректБанка(Организация, БанковскийСчет)
	...
	НастройкаОбмен = ОбменСБанками.НастройкаОбменаПоОтбору(ПараметрыОтбора);
	БанкНастройки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаОбмен, "Банк");
	
	Если БанкНастройки = Банк Тогда
		ТекстСервисПодключен = НСтр("ru = 'Подключен прямой обмен с банком. '");
	Иначе
		ШаблонСервисПодключен = НСтр("ru = 'Подключен прямой обмен через %1. '");
		НаименованиеБанка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанкНастройки, "Наименование");
		ТекстСервисПодключен = СтрШаблон(ШаблонСервисПодключен, НаименованиеБанка);
	КонецЕсли;
	
	Возврат ТекстСервисПодключен;
