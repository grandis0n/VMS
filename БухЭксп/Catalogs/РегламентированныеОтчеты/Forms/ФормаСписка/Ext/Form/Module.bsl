...
&НаСервере
Функция ПолучитьОтчет(НаименованиеОтчета = Неопределено, НеСоздаватьНовый = Ложь)
	Перем Расширение;
	...
	Запрос = Новый Запрос("
		...
		|	РегламентированныеОтчеты.НеСоздаватьНовый КАК НеСоздаватьНовый
		|ИЗ
	...
	НеСоздаватьНовый = Выборка.НеСоздаватьНовый;
...
Функция СоздатьНовыйОтчет()
	...
	НаименованиеОтчета = "";
	НеСоздаватьНовый = Ложь;
	ТекФорма = ПолучитьОтчет(НаименованиеОтчета, НеСоздаватьНовый);
	...
	// Запрет создания новых отчетов видов, отмеченных в справочнике регламентированных отчетов.
	Если НеСоздаватьНовый Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Создание регламентированного отчета данного вида запрещено.'");
		Сообщение.Сообщить();
