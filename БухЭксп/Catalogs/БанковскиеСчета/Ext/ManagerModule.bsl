...
Функция БанковскиеСчетаСНастройкамиПрямогоОбмена(Организация) Экспорт
	...
	|	Банки.Код КАК БИК,
	|	ЕСТЬNULL(
	|		ЕСТЬNULL(НастройкиОбменСБанками.Ссылка, НастройкиОбменСБанкамиПоБИК.Ссылка),
	|		НастройкиОбменаДополнительныеБанки.Ссылка)
	|	КАК НастройкаОбмена,
	|	ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиОбменСБанками.ДополнительныеБанки КАК НастройкиОбменаДополнительныеБанки
	|		ПО БанковскиеСчета.Банк = НастройкиОбменаДополнительныеБанки.Банк
	|			И (НастройкиОбменаДополнительныеБанки.Ссылка.Организация = &Организация)
	|			И (НЕ НастройкиОбменаДополнительныеБанки.Ссылка.ПометкаУдаления)
	|			И (НЕ НастройкиОбменаДополнительныеБанки.Ссылка.Недействительна)
	|ГДЕ
	...
	|			ИЛИ НастройкиОбменаДополнительныеБанки.Ссылка ЕСТЬ НЕ NULL)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет,
	|	БИК";
...
Функция БИКБанковСИнформациейОНастройкеПрямогоОбмена(Организация) Экспорт
	...
	|			ЕСТЬNULL(НастройкиОбменСБанками.Ссылка, НастройкиОбменСБанкамиПоБИК.Ссылка),
	|			НастройкиОбменаДополнительныеБанки.Ссылка),
	|		Значение(Справочник.НастройкиОбменСБанками.ПустаяСсылка))
	|	КАК НастройкаОбмена,
	|	ВЫБОР
	|		КОГДА НастройкиОбменаДополнительныеБанки.Ссылка ЕСТЬ NULL
	|			ТОГДА Неопределено
	|		ИНАЧЕ НастройкиОбменаДополнительныеБанки.Ссылка.Банк.Наименование
	|	КОНЕЦ КАК ИнтеграторВыписки
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БанковскиеСчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Банки КАК Банки
	|		ПО БанковскиеСчета.Банк = Банки.Ссылка
	...
	Пока ВыборкаРезультат.Следующий() Цикл
		...
		ДанныеНастройки.Вставить("НастройкаОбмена", ВыборкаРезультат.НастройкаОбмена);
		ДанныеНастройки.Вставить("ИнтеграторВыписки", ВыборкаРезультат.ИнтеграторВыписки);
		БИКБанковДляПроверки.Вставить(ВыборкаРезультат.БИК, ДанныеНастройки);
	КонецЦикла;
	...
	// проверяем по макету возможность прямого обмена
	...
	Для Индекс = 2 По КоличествоЗаписей Цикл
		...
		ДанныеПоНастройке = БИКБанковДляПроверки.Получить(БИКБанка);
		Если ДанныеПоНастройке = Неопределено Тогда
			НастройкаОбмена = Неопределено;
			ИнтеграторВыписки = Неопределено;
		Иначе
			НастройкаОбмена = ДанныеПоНастройке.НастройкаОбмена;
			ИнтеграторВыписки = ДанныеПоНастройке.ИнтеграторВыписки;
		КонецЕсли;
		...
		Если ВидПроектаРублевый ИЛИ ВидПроектаВалютный Тогда
			...
			ИнформацияОНастройкеПрямогоОбмена.Настройка =
				ЗначениеЗаполнено(НастройкаОбмена) Или ЗначениеЗаполнено(ИнтеграторВыписки);
			...
			ИнформацияОНастройкеПрямогоОбмена.ИнтеграторВыписки = ИнтеграторВыписки;
	...
	Если БИКБанковДляПроверки.Количество() > 0 Тогда 
		...
		Для Каждого КлючИЗначение из БИКБанковДляПроверки Цикл
			Если ЗначениеЗаполнено(КлючИЗначение.Значение) Тогда
				НастройкиОбмена.Добавить(КлючИЗначение.Значение.НастройкаОбмена);
		...
		Если НастройкиСВидамиВалюты.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение из БИКБанковСНастройкойОбмена Цикл
				ВидОбмена = НастройкиСВидамиВалюты.Получить(КлючИЗначение.Значение.НастройкаОбмена);
				...
				Если ВидОбмена <> Неопределено Тогда
					...
					ИнформацияОНастройкеПрямогоОбмена.ИнтеграторВыписки = КлючИЗначение.Значение.ИнтеграторВыписки;
...
Функция ИнформацияОНастройкеПрямогоОбменаСБанком(Организация, Банк) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
		...
		Если ЗначениеЗаполнено(БанкНастройки) И БанкНастройки <> Банк Тогда
			ИнформацияОНастройкеПрямогоОбмена.ИнтеграторВыписки = БанкНастройки;
		КонецЕсли;
	КонецЕсли;
...
//     * Рублевый - Булево - если Истина, то возможен обмен рублевыми платежными документами.
//     * Валютный - Булево - если Истина, то возможен обмен валютными платежными документами.
//     * ИнтеграторВыписки - Неопределено,СправочникСсылка.Банки - банк интегратор для загрузки выписки.
//
Функция СведенияОНастройкеПрямогоОбмена()
	...
	Результат.Вставить("ИнтеграторВыписки", Неопределено);
