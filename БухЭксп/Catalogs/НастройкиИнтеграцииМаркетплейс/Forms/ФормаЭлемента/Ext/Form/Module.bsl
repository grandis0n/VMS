...
Процедура ТокенВБРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "Доступ к API" Тогда
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "Тип токена" Тогда
		СсылкаНаСправкуOZON = "https://dev.wildberries.ru/docs/openapi/api-information#tag/Avtorizaciya/Pravila-ispolzovaniya-tokenov-dostupa-k-API";
...
Процедура ЗаполнитьДанныеТокенаВБ(Токен = Неопределено)
	...
	НадписьТипТокена = Новый Массив;
	НадписьТипТокена.Добавить(НадписьНоваяСтрока);
	Если ДанныеТокенаВБ.КодТипаТокена = 0 Тогда // токен не выбран
		НадписьТипТокена.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Рекомендуется использовать Персональный токен'"),,,, "Тип токена"));
	ИначеЕсли ДанныеТокенаВБ.КодТипаТокена = 3 Тогда // используется персональный токен
		НадписьТипТокена.Добавить(Новый ФорматированнаяСтрока(ДанныеТокенаВБ.ТипТокена, ЖирныйШрифт));
		НадписьТипТокена.Добавить(НадписьНоваяСтрока);
		НадписьТипТокена.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Оптимален для загрузки отчетов'"),,,, "Тип токена"));
	Иначе
		НадписьТипТокена.Добавить(Новый ФорматированнаяСтрока(ДанныеТокенаВБ.ТипТокена, ЖирныйШрифт));
		НадписьТипТокена.Добавить(НадписьНоваяСтрока);
		НадписьТипТокена.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Рекомендуется перейти на Персональный токен'"),,,, "Тип токена"));
	КонецЕсли;
	...
	НадписьСозданиеТокена.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Контент, Маркетплейс, Финансы, Документы'"), ЖирныйШрифт));
	...
	Элементы.ТокенВБ.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(
		НадписьСостояниеТокена, НадписьТипТокена, НадписьКатегории, НадписьДоступно, НадписьНеДоступно, НадписьСозданиеТокена);
