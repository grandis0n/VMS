...
Функция СформироватьОбъектXDTOДоверенности(ДанныеМЧД) Экспорт
	...
	СброситьВзаимоисключающиеНеподдерживаемыеИПустыеСвойстваXDTOМЧД(Объект);
...
Функция СформироватьОбъектXDTOПередоверия(ДанныеМЧД) Экспорт
	...
	ЗаполнитьОбъектМЧДРекурсивно(Объект, ДанныеМЧД, "Доверенность", ОшибкиЗаполнения);
	СброситьВзаимоисключающиеНеподдерживаемыеИПустыеСвойстваXDTOМЧДПередоверия(Объект);
...
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//
Процедура СброситьВзаимоисключающиеНеподдерживаемыеИПустыеСвойстваXDTOМЧДПередоверия(Доверенность)
	...
	Для Каждого Представитель Из Представители Цикл
		...
		СброситьПустыеСвойстваПредставителя(Представитель);
	КонецЦикла;
...
//  Доверенность - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.Доверенность
//
Процедура СброситьВзаимоисключающиеНеподдерживаемыеИПустыеСвойстваXDTOМЧД(Доверенность)
...
// Сбрасывает пустые свойства представителя.
// 
// Параметры:
//  Представитель - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
Процедура СброситьПустыеСвойстваПредставителя(Представитель)
	
	 СброситьПустыеСвойстваПредставителяФизическогоЛица(Представитель);
	 СброситьПустыеСвойстваПредставителяИП(Представитель)
		
КонецПроцедуры

// Сбрасывает пустые свойства представителя - физического лица.
// 
// Параметры:
//  Представитель - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
Процедура СброситьПустыеСвойстваПредставителяФизическогоЛица(Представитель)
	
	Если Представитель.ТипПред <> ТипыПредставителей().ФизическоеЛицо Тогда
		Возврат;
	КонецЕсли;
	
	СведенияФизическогоЛица = Представитель.Пред.СведФизЛ.СведФЛ;
	СброситьПустоеСвойствоДУЛПредставителяФизическогоЛица(СведенияФизическогоЛица);
	
КонецПроцедуры

// Сбрасывает пустые свойства представителя - ИП.
// 
// Параметры:
//  Представитель - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвУпПредТип
Процедура СброситьПустыеСвойстваПредставителяИП(Представитель)
	
	Если Представитель.ТипПред <> ТипыПредставителей().ИндивидуальныйПредприниматель Тогда
		Возврат;
	КонецЕсли;
	
	СведенияФизическогоЛица = Представитель.Пред.СведИП.СведФЛ;
	СброситьПустоеСвойствоДУЛПредставителяФизическогоЛица(СведенияФизическогоЛица);
	
КонецПроцедуры	

// Сбрасывает свойство УдЛичнФЛ (документ, удостоверяющий личность физического лица), если одновременно не заполнены 
// все свойства, входящие в его состав.
// Если этого не сделать, то при записи объекта XDTO будет записан пустой узел <УдЛичнФЛ/> и это приведет к ошибке 
// регистрации доверенности.
// 
// Параметры:
//  СведенияФизическогоЛица - см. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СведФЛТип
Процедура СброситьПустоеСвойствоДУЛПредставителяФизическогоЛица(СведенияФизическогоЛица)
	
	СвойстваЗаполнены = Ложь;
		
	УдостоверениеЛичностиФизическогоЛица = СведенияФизическогоЛица.УдЛичнФЛ;
	
	Для Каждого Свойство Из УдостоверениеЛичностиФизическогоЛица.Свойства() Цикл
		Если УдостоверениеЛичностиФизическогоЛица.Установлено(Свойство.Имя) Тогда
			СвойстваЗаполнены = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если Не СвойстваЗаполнены Тогда
		СведенияФизическогоЛица.Сбросить("УдЛичнФЛ");
	КонецЕсли;
