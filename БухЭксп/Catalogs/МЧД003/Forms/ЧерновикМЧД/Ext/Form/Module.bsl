...
Процедура ДеревоПолномочийСОграничениямиПередНачаломИзменения(Элемент, Отказ)
	...
	Правила = ПолучитьПараметрыРедактированияПравила(ТекущийПорядковыйНомер, Истина, Истина);
...
Процедура ПерезаполнитьСведенияОДоверителе(Доверитель)
	...
	Если ИнтеграцияЭДО.ТипУчастникаЭДО(Доверитель).ЭтоФизЛицо Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Формирование доверенности от физического лица доступно только через нотариат.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПерезаполнитьСведенияОЛицеБезДоверенности(ЛицоБезДоверенности)
	...
	Если ЗначениеЗаполнено(ЛицоБезДоверенности) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Иначе
			ТипУчастникаЭДО = ИнтеграцияЭДО.ТипУчастникаЭДО(ЛицоБезДоверенности);
...
Процедура ПерезаполнитьСведенияОПредставителе(Представитель)
	...
	Если ЗначениеЗаполнено(Представитель) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Иначе
			ТипУчастникаЭДО = ИнтеграцияЭДО.ТипУчастникаЭДО(Представитель);
