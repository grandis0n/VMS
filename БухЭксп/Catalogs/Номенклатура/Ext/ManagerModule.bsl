...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчиститьВидМаркировки");
	Методы.Вставить("ЗаполнитьОписаниеНоменклатурыИС");
	Методы.Вставить("ИсправитьЗаписиОписанияНоменклатурыИС");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Обработки.ПечатьЭтикеток) Тогда
		...
		КомандаПечати.СписокФорм    = "ФормаСписка,ФормаЭлемента,ФормаТовары";
...
Функция ТекстЗапросаВидМаркировки()
	...
	|	И СпрНоменклатура.УдалитьВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПустаяСсылка)
...
Процедура ЗаполнитьОписаниеНоменклатурыИС() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Номенклатура.Ссылка КАК Ссылка,
	|	Номенклатура.УдалитьВидМаркировки КАК ВидМаркировки,
	|	Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|ИЗ
	|	Справочник.Номенклатура КАК Номенклатура
	|ГДЕ
	|	НЕ Номенклатура.ЭтоГруппа
	|	И НЕ Номенклатура.УдалитьВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПустаяСсылка)
	|	И НЕ Номенклатура.УдалитьВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.АлкогольнаяПродукцияДо9Процентов)";
	...
	ОписанияНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ТаблицаНоменклатуры.ВыгрузитьКолонку("Ссылка"));
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
		...
		ВидПродукции                 = СтрокаТаблицы.ВидМаркировки;
		...
		ЭтоМернаяЕдиницаИзмерения = ИнтеграцияИСМПБПВызовСервера.ЭтоМернаяЕдиница(СтрокаТаблицы.ЕдиницаИзмерения); // Истина для мерной единицы измерения
		Если ЭтоМернаяЕдиницаИзмерения
			И ТекущееОписание.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.ПотребительскаяУпаковка Тогда
	...
	Попытка
		ОбщегоНазначенияИС.УстановитьУправляемуюБлокировку(Метаданные.РегистрыСведений.ОписаниеНоменклатурыИС.ПолноеИмя());
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось обработать номенклатуру при переходе на конструктор настроек видов продукции по причине:
				       |%1'"),
...
Процедура ИсправитьЗаписиОписанияНоменклатурыПоМернойЕдиницыИзмерения(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ОписаниеНоменклатурыИС.Номенклатура.ЕдиницаИзмерения.Код В (&КодыМерныхЕдиниц)
		|	И ОписаниеНоменклатурыИС.ВариантИспользованияЕдиницыХранения = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияЕдиницыХраненияИС.ПотребительскаяУпаковка)";
	
	Запрос.УстановитьПараметр("КодыМерныхЕдиниц", КодыМерныхЕдиницИзмерения());
	...
	ПараметрыОбработчика.ОбработкаЗавершена = Ложь;
	ОбработканоЗаписей = 0;
	МассивНоменклатуры = Результат.Выгрузить().ВыгрузитьКолонку("Номенклатура");
	...
	Для Каждого Номенклатура Из МассивНоменклатуры Цикл
		...
		Попытка
			...
			Для Каждого СтрокаНабора Из НаборЗаписей Цикл
				...
				СтрокаНабора.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.МернаяПродукцияТребуетУказанияЗначения;
				СтрокаНабора.КоличествоВПотребительскойУпаковке  = 0;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать ""%1"" при изменении описания номенклатуры ИС по причине:
					       |%2'"),
				Номенклатура,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбновлениеИнформационнойБазы.ЗаписатьОшибкуВЖурналРегистрации(
...
Процедура ИсправитьЗаписиОписанияНоменклатурыИС(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ КлассификаторЕдиницИзмерения.Код В (&КодыМерныхЕдиниц)
		|	И ОписаниеНоменклатурыИС.ВариантИспользованияЕдиницыХранения = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияЕдиницыХраненияИС.МернаяПродукцияТребуетУказанияЗначения)";
	...
	ПараметрыОбработчика.ОбработкаЗавершена = Ложь;
	ОбработканоЗаписей = 0;
	МассивНоменклатуры = Результат.Выгрузить();
	Для Каждого СтрокаТаблицы Из МассивНоменклатуры Цикл
		НаборЗаписей = РегистрыСведений.ОписаниеНоменклатурыИС.СоздатьНаборЗаписей();
		...
		Попытка
			...
			НаборЗаписей.Прочитать();
			Для Каждого СтрокаНабора Из НаборЗаписей Цикл
				СтрокаНабора.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИС.ПотребительскаяУпаковка;
				СтрокаНабора.КоличествоВПотребительскойУпаковке  = ?(СтрокаТаблицы.КоличествоВПотребительскойУпаковке = 0, 1, СтрокаТаблицы.КоличествоВПотребительскойУпаковке);
			КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать ""%1"" при изменении описания номенклатуры ИС по причине:
					       |%2'"),
				СтрокаТаблицы.Номенклатура, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбновлениеИнформационнойБазы.ЗаписатьОшибкуВЖурналРегистрации(СтрокаТаблицы.Номенклатура,
				Строка(СтрокаТаблицы.Номенклатура), ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Функция КодыМерныхЕдиницИзмерения()
	...
	КодыМерныхЕдиниц = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаКодовМерныхЕдиниц, , Истина, Истина);
	
	Возврат КодыМерныхЕдиниц;
	
КонецФункции

Процедура ОчиститьВидМаркировки(ПараметрыОбработчика) Экспорт
	
	ТекстЗапроса = ТекстЗапросаОчиститьВидМаркировки();
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ОбъектБылИзмененДругимСеансом = НЕ ЗначениеЗаполнено(НоменклатураОбъект.УдалитьВидМаркировки);
			...
			СвойстваМаркируемойПродукции = ОбщегоНазначенияИС.СвойстваМаркируемойПродукции(Выборка.Ссылка);
			
			Если ЗначениеЗаполнено(НоменклатураОбъект.УдалитьВидМаркировки) 
				И (ЗначениеЗаполнено(СвойстваМаркируемойПродукции.ВидПродукции) 
				ИЛИ НоменклатураОбъект.УдалитьВидМаркировки = Перечисления.ВидыПродукцииИС.АлкогольнаяПродукцияДо9Процентов) Тогда
				НоменклатураОбъект.УдалитьВидМаркировки = Перечисления.ВидыПродукцииИС.ПустаяСсылка();
			КонецЕсли;
		...
		Исключение
			...
			ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Сообщение = СтрШаблон(
				НСтр("ru = 'Ошибка при выполнении обработчика Справочники.Номенклатура.ОчиститьВидМаркировки.
				|%1'"),
				ПредставлениеОшибки);
...
Функция ТекстЗапросаОчиститьВидМаркировки()
	
	ТекстЗапроса =
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	СпрНоменклатура.Ссылка КАК Ссылка,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(СпрНоменклатура.Ссылка) КАК ПредставлениеСсылки,
	|	СпрНоменклатура.УдалитьВидМаркировки КАК ВидМаркировки
	|ИЗ
	|	Справочник.Номенклатура КАК СпрНоменклатура
	|ГДЕ
	|	НЕ СпрНоменклатура.ЭтоГруппа
	|	И НЕ СпрНоменклатура.УдалитьВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПустаяСсылка)
	|	И НЕ СпрНоменклатура.Услуга
	|	И &Условие";
