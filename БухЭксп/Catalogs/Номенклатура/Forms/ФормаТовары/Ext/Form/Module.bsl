...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РаботаСНоменклатуройБП.ПриСозданииНаСервереФормаСпискаНоменклатуры(
		ЭтотОбъект,
		Элементы.Группа1СНоменклатура,
		Элементы.Список);
...
Процедура НайтиПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		СервисАктивен = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "СервисАктивен");
		ИнтернетПоддержкаПодключена =
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ИнтернетПоддержкаПодключена");
	КонецЕсли;
