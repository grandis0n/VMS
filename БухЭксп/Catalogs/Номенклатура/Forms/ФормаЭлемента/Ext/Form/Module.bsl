...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая()
		...
		ВидПродукцииЕГАИСОбъект = Перечисления.ВидыПродукцииИС.Алкогольная;
	...
	ЕстьДоступныеВидыМаркировки          = Истина; 
		...
		Если ЗначениеЗаполнено(Объект.ВидНоменклатуры) Тогда
			...
			Если Объект.Ссылка.Пустая() Тогда
				...
				Если ЗначениеЗаполнено(Объект.КодВидаПодакцизногоТовара) Тогда
					ПараметрыАкциза = УчетАкцизовВызовСервера.ПараметрыАкцизаПоВидуТовара(Объект.КодВидаПодакцизногоТовара);
					Объект.ЕдиницаИзмеренияДляРасчетаАкциза = ПараметрыАкциза.ЕдиницаИзмеренияАкциза;
					РасчетАкцизаЗаЛитрСпирта = ПараметрыАкциза.РасчетАкцизаЗаЛитрСпирта;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Номенклатура", ТекущийОбъект.Ссылка);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиЧастичногоВыбытияПродукцииИС");
		ЭлементБлокировки.УстановитьЗначение("Номенклатура", ТекущийОбъект.Ссылка);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		
		Если ЭтоАдресВременногоХранилища(АдресДанныхНастроекПараметровНоменклатурыИС) Тогда
			ДанныеНастроек = ПолучитьИзВременногоХранилища(АдресДанныхНастроекПараметровНоменклатурыИС);
			ЗаполнитьЗначенияСвойств(ДанныеНастроек.ОписаниеНоменклатурыИС, ЭтаФорма);
			РегистрыСведений.ОписаниеНоменклатурыИС.УстановитьОписание(ДанныеНастроек.ОписаниеНоменклатурыИС);
			
			НаборЗаписей = РегистрыСведений.НастройкиЧастичногоВыбытияПродукцииИС.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Номенклатура.Установить(ТекущийОбъект.Ссылка);
			Для Каждого Строка Из ДанныеНастроек.НастройкиЧастичногоВыбытияПродукцииИС Цикл
		...
		Иначе
			...
			ДанныеНастроек.ОписаниеНоменклатурыИС.Номенклатура = ТекущийОбъект.Ссылка;
			ЗаполнитьЗначенияСвойств(ДанныеНастроек.ОписаниеНоменклатурыИС, ЭтаФорма);
			РегистрыСведений.ОписаниеНоменклатурыИС.УстановитьОписание(ДанныеНастроек.ОписаниеНоменклатурыИС);
		КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Произошла ошибка при записи данных настройки номенклатуры ИС:
			           |%1'"),
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.КодВидаПодакцизногоТовара) Тогда
		ПараметрыАкциза = УчетАкцизовВызовСервера.ПараметрыАкцизаПоВидуТовара(Объект.КодВидаПодакцизногоТовара);
		Элементы.ПроцентСпиртаВПродукции.Видимость = ПараметрыАкциза.РасчетАкцизаЗаЛитрСпирта И ПараметрыАкциза.ЭтоИмпортныйТовар;
	Иначе
		Элементы.ПроцентСпиртаВПродукции.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОчиститьДанныеНастроекПараметровНоменклатурыИС(ЭтотОбъект);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(КлючВидовПродукции) Тогда
...
Процедура ВидыПродукцииФГИСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если ТребуетсяОбновлениеКлассификатораВидовПродукции Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо обновить классификаторы. Обратитесь к администратору.'"));
...
Процедура ОчиститьДанныеНастроекПараметровНоменклатурыИС(Форма)
	
	Если ЭтоАдресВременногоХранилища(Форма.АдресДанныхНастроекПараметровНоменклатурыИС) Тогда
		УдалитьИзВременногоХранилища(Форма.АдресДанныхНастроекПараметровНоменклатурыИС);
		Форма.АдресДанныхНастроекПараметровНоменклатурыИС = "";
	КонецЕсли;
...
Функция ПараметрыОписанияМаркируемогоТовара(Ссылка, ВидМаркировки)
	...
	ПараметрыОписанияМаркируемогоТовара.АдресДанныхНастроекПараметровНоменклатуры       = АдресДанныхНастроекПараметровНоменклатурыИС;
...
&НаКлиенте
Процедура СвойстваМаркируемогоТовараЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	АдресДанныхНастроекПараметровНоменклатурыИС = Результат;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаИнформацияЭПД.Видимость           = Форма.ИспользоватьДоставкуАвтотранспортом И НЕ Объект.Услуга;
	...
	Элементы.ГруппаИнтеграцияГосИС.Видимость         = Не Объект.Услуга И Форма.ЕстьДоступныеВидыМаркировки;
	...
	Элементы.ГруппаПрослеживаемость.Видимость         = Не Объект.Услуга И Форма.ВедетсяУчетПрослеживаемыхТоваров 
															И НЕ ЗначениеЗаполнено(Форма.КлючВидовПродукции);
	...
	Элементы.ГруппаАлкогольнаяПродукция.Видимость = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(Форма.КлючВидовПродукции);
...
Процедура ПрочитатьСведенияОбАлкогольнойПродукции()
	...
	Если ЗаполнитьСведенияОбАлкогольнойПродукции
		ИЛИ НЕ КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(КлючВидовПродукции) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДлительнаяОперацияРезультатПоискаНоменклатурыСервиса()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"РаботаСНоменклатуройБП.РезультатПоискаНоменклатурыСервиса", ПараметрыФункции);
...
Процедура ВидНоменклатурыПриИзмененииНаСервере()
	...
	Если ЕстьУчетПодакцизныхТоваров И ЗначениеЗаполнено(ВидТовара) Тогда
		Объект.КоэффициентПересчета = 0;
		Объект.ПроцентСпиртаВПродукции = 0;
		РасчетАкцизаЗаЛитрСпирта = Ложь;
		УчетАкцизов.ЗаполнитьКодВидаПодакцизногоТовараПоУмолчанию(ВидТовара, Объект.КодВидаПодакцизногоТовара);
		Если ЗначениеЗаполнено(Объект.КодВидаПодакцизногоТовара) Тогда
			ПараметрыАкциза = УчетАкцизовВызовСервера.ПараметрыАкцизаПоВидуТовара(Объект.КодВидаПодакцизногоТовара);
			Объект.ЕдиницаИзмеренияДляРасчетаАкциза = ПараметрыАкциза.ЕдиницаИзмеренияАкциза;
...
Процедура ВывестиОписаниеМаркируемогоТовараНаСервере()
	...
	ПараметрыОписанияМаркируемогоТовара = ПараметрыОписанияМаркируемогоТовара(Объект.Ссылка, КлючВидовПродукции);
...
Процедура НастроитьВидимостьГруппыАкцизы()
	...
	Если ЗначениеЗаполнено(ВидТовара) И ЕстьУчетПодакцизныхТоваров Тогда
		...
		Элементы.ПроцентСпиртаВПродукции.Видимость = РасчетАкцизаЗаЛитрСпирта;
	Иначе
...
&НаКлиенте
Процедура ВидМаркировкиПриИзменении()
...
Процедура АдаптироватьМКШапкаФормы()
	...
	+ ",ГруппаВидНоменклатуры,ПериодичностьУслуги"
...
&НаКлиенте
Процедура НаименованиеГрузаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	...
	ЭлементОтбора.ПравоеЗначение = "Груз";
	ЭлементОтбора.Использование = Истина; 
	ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ФиксированныеНастройки",      НастройкиКомпоновкиФормы);
	ПараметрыФормы.Вставить("ГруппаДанных",                "Груз");
	ПараметрыФормы.Вставить("Тип",                         "Груз");
	ПараметрыФормы.Вставить("ИмяОбъекта",                  "Документ.ЭлектроннаяТранспортнаяНакладная");
	ПараметрыФормы.Вставить("РежимВыбора", Истина);
	
	ОписаниеОповещения = НОвый ОписаниеОповещения("ВыборГрузаЗавершение", ЭтотОбъект);
	 
	ОткрытьФорму("Справочник.ХранимыеДанныеЭПД.ФормаВыбора", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте
Процедура ВыборГрузаЗавершение(ГрузЗначение, ДополнительныеПараметры) Экспорт
	Если ГрузЗначение = Неопределено 
		ИЛИ ГрузЗначение = Объект.ТипГруза Тогда
	
		Возврат;
	КонецЕсли;
	
	Объект.ТипГруза = ГрузЗначение;
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура НаименованиеГрузаСоздание(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОписаниеОповещения = НОвый ОписаниеОповещения("НаименованиеГрузаСозданиеЗавершение", ЭтотОбъект);
	ОткрытьФорму("Документ.ЭлектроннаяТранспортнаяНакладная.Форма.Груз",,ЭтотОбъект,,,,ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте
Процедура НаименованиеГрузаСозданиеЗавершение(НаименованиеГрузаЗначение, ЭтотОбъект) Экспорт
	Если НЕ ЗначениеЗаполнено(НаименованиеГрузаЗначение) 
		ИЛИ НЕ НаименованиеГрузаЗначение.Свойство("ДанныеЗаполненияРеквизитовФормы") Тогда
		
		Возврат;
	КонецЕсли;
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("ГруппаДанных","Груз");
	ДанныеЗаполнения.Вставить("Тип", "Груз");
	ДанныеЗаполнения.Вставить("ИмяОбъекта", "Документ.ЭлектроннаяТранспортнаяНакладная");
	ДанныеЗаполнения.Вставить("ДанныеЗаполненияРеквизитовФормы", НаименованиеГрузаЗначение.ДанныеЗаполненияРеквизитовФормы);
	
	Объект.ТипГруза = ОбменСГИСЭПДБПВызовСервера.СоздатьХранимыеДанныеЭПД(ДанныеЗаполнения);
