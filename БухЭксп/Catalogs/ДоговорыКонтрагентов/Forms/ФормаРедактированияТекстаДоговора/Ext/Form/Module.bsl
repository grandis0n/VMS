...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ФайлДоговора) Тогда
		...
		КонецЕсли;
	Иначе
		ПодключитьОбработчикОжидания("Подключаемый_ЗагрузитьШаблон", 0.1, Истина);
	КонецЕсли;
...
Процедура ЗаполнитьПоШаблонуПослеПодтверждения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ЗагрузитьШаблонДоговораНаКлиенте();
...
&НаКлиенте
Процедура ЗагрузитьШаблонДоговораНаКлиенте()
	
	ЗагрузитьШаблонДоговора();
	ОжидатьЗагрузкиШаблона();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ЗагрузитьШаблон()
...
&НаКлиенте
Процедура ОжидатьЗагрузкиШаблона()
...
Функция ПодготовитьHTMLНаОсновеФорматированногоДокумента(ИмяПапки, ИмяФайла)
	...
	ФайловаяСистема.УдалитьВременныйКаталог(ПутьКПапкеКартинок);
...
Функция ПоместитьФайлыВоВременноеХранилищеНаСервере(ФорматыСохранения)
	...
	КонецЦикла;
	ФайловаяСистема.УдалитьВременныйКаталог(Папка);
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	Элементы.ПросмотрИПечатьЭД.Заголовок = НСтр("ru = 'Просмотр'");
	...
	Если Элементы.Найти("КомандыЭДО") <> Неопределено Тогда
		МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВМККомандыФормы(Элементы, Элементы.КомандыЭДО);
	КонецЕсли;
	МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВМККомандыФормы(Элементы, Элементы.ПросмотрИПечатьЭД);
	МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВМККомандыФормы(Элементы, Элементы.ФормаОтправитьДополнительноеСоглашениеПоЭДО);
