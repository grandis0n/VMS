...
Процедура АдаптироватьМКСписокФормы()
	...
	Если Элементы.Найти("ПредставлениеСостояния") <> Неопределено
		И ЭлектронноеВзаимодействиеБП.НастроенОбменЭДО() Тогда
		МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.ПредставлениеСостояния, 3);
	КонецЕсли;
