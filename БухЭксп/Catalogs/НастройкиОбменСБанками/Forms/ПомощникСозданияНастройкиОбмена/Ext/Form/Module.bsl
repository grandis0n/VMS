...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Банк) Тогда
			...
			ПрограммаБанкаПоддерживается = Ложь;
			АутентификацияПоддерживается = Ложь;
			
			Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
				
				РеквизитыНастройки = Новый Структура("ПрограммаБанка,ИспользуетсяКриптография");
				ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(НастройкаОбмена, РеквизитыНастройки);
				
				ПрограммаБанкаПоддерживается = РеквизитыНастройки.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
					ИЛИ РеквизитыНастройки.ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI;
				АутентификацияПоддерживается = Не РеквизитыНастройки.ИспользуетсяКриптография;
			Иначе
				...
				Если ПоставляемыеНастройкиОбмена <> Неопределено Тогда
					ПрограммаБанкаПоддерживается = 
						(ПоставляемыеНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
							ИЛИ ПоставляемыеНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI
							// В поставляемой настройке указано СбербанкОнлайн, но настройка выполняется для SberAPI
							ИЛИ ПоставляемыеНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн);
					АутентификацияПоддерживается = (ПоставляемыеНастройкиОбмена.СпособАутентификации = "ПоЛогину");
				КонецЕсли;
			КонецЕсли;
			ЭтоМобильныйКлиентОграниченныйРежим = Не (ПрограммаБанкаПоддерживается
				И АутентификацияПоддерживается);
		...
		Иначе
			...
			// Подключение по УПШ не поддерживается
			Элементы.ГруппаПодключитсяПоСтаромуМетоду.Видимость = Ложь;
			Элементы.СертификатПодключенияПароль.ПодсказкаВвода = НСтр("ru = 'Пароль'");
			
			// Скроем предупреждение
			Элементы.ГруппаМобильныйКлиентНадпись.Видимость = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	ИначеЕсли СпособСозданияНастройки = 1 И ЗначениеЗаполнено(АдресФайлаНастроекОбмена) Тогда
		...
		Иначе
			...
			#КонецЕсли
			ПодключитьОбработчикОжидания("Подключаемый_СоздатьНастройкуОбмена", 0.1, Истина);
			Элементы.ШапкаПредупреждениеОСинхронизации.Видимость = Ложь;
...
Процедура Далее(Команда)
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборОрганизацииИБанка Тогда
		...
		Если ОбменСБанкамиСлужебныйКлиент.ЭтоМобильныйКлиент()
			И Не ОбменСБанкамиСлужебныйКлиент.ДоступнаРаботаНаМобильномКлиенте(ПрограммаБанка, ИспользуетсяКриптография) Тогда
			
			ОтменитьРаботуЭтоНеДоступныйБанкДляМобильныйКлиент();
			Возврат;
...
Функция ДоступноСозданиеПоУПШ()
	...
	// Сбер постепенно отключает протокол УПШ. К 01.01.2027 УПШ уже не будет работать вообще.
	Если Не ЗначениеЗаполнено(Организация) Или Не ЗначениеЗаполнено(Банк)
		ИЛИ ТекущаяДатаСеанса() >= Дата("20270101") Тогда
		Возврат Ложь;
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст =
...
&НаКлиенте
Процедура ОтменитьРаботуЭтоНеДоступныйБанкДляМобильныйКлиент()
...
Процедура СоздатьНастройкуОбменаSberAPI(ЭтоТокен = Ложь)
	...
	Иначе
		...
		ОбработатьРезультатПолученияНастроекССервераБанка(Истина);
		Элементы.СозданиеНастройкиОбменаКартинка.Картинка = БиблиотекаКартинок.ДлительнаяОперация48;
		ПодключитьОбработчикОжидания("Подключаемый_ПроверитьНаличиеЗарплатныхДоговоровSberAPI", 0.1, Истина);
...
&НаКлиенте
Процедура Подключаемый_ПроверитьНаличиеЗарплатныхДоговоровSberAPI() Экспорт
	
	ЕстьЗарплатныйДоговорSberAPI = Ложь;
	
	Результат = ЗапуститьПроверкуНаличиеЗарплатныхДоговоровSberAPI();
	
	Оповещение = Новый ОписаниеОповещения("ПослеПроверкиНаличиеЗарплатныхДоговоровSberAPI", ЭтотОбъект);
...
&НаСервере
Функция ЗапуститьПроверкуНаличиеЗарплатныхДоговоровSberAPI()
	
	НаименованиеЗадания = НСтр("ru = 'Получение зарплатных договоров организации'");
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение зарплатных договоров организации SberAPI'");
	...
	ПараметрыПроцедуры.Вставить("ИгнорироватьСохраненныеНастройки", Истина);
	
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиSberAPIСлужебный.ПроверитьНаличиеЗарплатныхДоговоровSberAPI", ПараметрыПроцедуры, ПараметрыВыполнения);
	
КонецФункции

&НаКлиенте
Процедура ПослеПроверкиНаличиеЗарплатныхДоговоровSberAPI(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗадания) И ТипЗнч(РезультатЗадания) = Тип("Массив")
			И РезультатЗадания.Количество() <> 0 Тогда
			ЕстьЗарплатныйДоговорSberAPI = Истина;
		КонецЕсли;
	...
	Иначе // Ошибка
		...
		ОбработатьРезультатПолученияНастроекССервераБанка(Ложь);
		ВидОперации = НСтр("ru = 'Получение зарплатных договоров организации'");
...
Процедура ЗагрузитьФайлНастроекSberAPIЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	// Для мобильной среды расширение файла может быть неопределено
	Если Не ОбменСБанкамиСлужебныйКлиент.ЭтоМобильныйКлиент()
		И НРег(ЧастиИмени.Расширение) <> ".json" Тогда
...
Процедура ПослеБазовойАутентификацииСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Структура") И Результат.ТребуетсяТокен Тогда
		...
		Если ОбменСБанкамиСлужебныйКлиент.ЭтоМобильныйКлиент() Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'На стороне банка включена аутентификация с помощью токена.
				|Обмен с банком возможен только с компьютера'"));
			ОбработатьРезультатПолученияНастроекССервераБанка(Ложь);
...
Функция СтруктураСозданияНастройкиОбмена()
	...
	Если ЭтоSberAPI Тогда
		...
		КонецЦикла;
		Если ЕстьЗарплатныйДоговорSberAPI Тогда
			СтруктураПараметров.Вставить("ЕстьЗарплатныйДоговорSberAPI", ЕстьЗарплатныйДоговорSberAPI);
		КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_СоздатьНастройкуОбмена()
