...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоМобильныйКлиент Тогда
		...
		Если Не Параметры.Ключ.Пустая() Тогда
			ПрограммаБанкаПоддерживается = (Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
				Или Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI);
			АутентификацияПоддерживается = Не Объект.ИспользуетсяКриптография
				И Не Объект.АутентификацияПоСертификату;
		...
		Иначе
			Элементы.СтраницыВнешнегоМодуля.Видимость = Ложь;
		КонецЕсли;
	...
	Если ИспользуетсяВнешняяКомпонента Тогда
		Если ЗначениеЗаполнено(Объект.ИмяВнешнегоМодуля) Тогда
			...
			Если Объект.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн")
				...
				ИначеЕсли ЭтоМобильныйКлиент Тогда
					ФлагПоказыватьВнешнююКомпоненту = Ложь;
				КонецЕсли;
...
Процедура НастроитьЭлементыSberApi(Форма, ИспользуетсяКриптография)
	...
	Если Форма.ЭтоМобильныйКлиент Тогда
		// В моб.клиенте управление внешней компонентой недоступно
