...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВосстановитьОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС");
...
Процедура ВосстановитьОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС() Экспорт
	
	Попытка
		ВидНалога = ПредопределенноеЗначение("Перечисление.ВидыНалогов.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС");
	Исключение
		Возврат;
	КонецПопытки;
	...
	Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалога, Ложь,,, Ложь);
	Если ЗначениеЗаполнено(Налог) Тогда
		НалогОбъект = Налог.ПолучитьОбъект();
		Если НалогОбъект.ПометкаУдаления Тогда
			НалогОбъект.ПометкаУдаления = Ложь;
			Если НалогОбъект.Код = "000000003" Тогда
				НалогОбъект.УстановитьНовыйКод();
			КонецЕсли;
			
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НалогОбъект);
		КонецЕсли;
	КонецЕсли;
