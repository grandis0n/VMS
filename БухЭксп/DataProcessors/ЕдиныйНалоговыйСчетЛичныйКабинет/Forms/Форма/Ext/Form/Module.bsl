...
Процедура НайтиСертификатПоОтпечаткуЗавершение(Результат, ВходящийКонтекст) Экспорт 
	...
	Если Результат.Выполнено
		...
		ИспользуетсяДоверенностьНаПолучениеДанных = СвойстваВладельца.Получить("OID1_2_643_100_5") = Неопределено 
			И СвойстваВладельца.Получить("OID1_2_643_100_1") = Неопределено
			И СвойстваВладельца.Получить("OGRN") = Неопределено 
			И СвойстваВладельца.Получить("OGRNIP") = Неопределено
			И СтрНайти(НРег(СвойстваВладельца.Получить("OID2_5_4_12")), "нотариус") = 0;
