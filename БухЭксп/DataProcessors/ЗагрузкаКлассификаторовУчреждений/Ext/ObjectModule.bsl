...
Процедура ДобавитьНовыйКлассификатор(ИмяФайла, Хранение, УникальныйИдентификатор) Экспорт
	...
	Иначе
		...
		Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ИмяФайла) Тогда
			ВызватьИсключение "Загружать классификатор можно только из файла во временном каталоге";
		КонецЕсли;
...
Функция ПроверитьРазархивироватьФайл(Файл, ТипЧитателя, ПервичныйИсточник, MIMEТип = "",УникальныйИдентификатор)
	...
	Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(Файл.ПолноеИмя) Тогда
		ВызватьИсключение "Загружать классификатор можно только из файла во временном каталоге";
	КонецЕсли;
	...
	ИначеЕсли НРег(Файл.Расширение) = ".zip"
		...
		Для Каждого Элемент Из Zip.Элементы Цикл
			...
			Исключение
				...
				Продолжить
			КонецПопытки;
			ИмяФайлаИзАрхива = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
				Элемент.Имя);
			НовоеИмя = ТемпКаталог + "\" + ИмяФайлаИзАрхива;
