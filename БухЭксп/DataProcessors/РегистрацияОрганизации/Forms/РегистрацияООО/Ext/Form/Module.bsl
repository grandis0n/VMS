...
Функция НайтиУведомлениеОСпецРежимеНалогообложения(Знач ОрганизацияСсылка, Знач ДатаСозданияЗаявления)
...
Процедура УведомлениеОПереходеНаУСН()
	...
	УведомлениеОСпецРежимеНалогообложения = НайтиУведомлениеОСпецРежимеНалогообложения(
		ОрганизацияСсылка, ДатаСозданияЗаявления);
...
Функция ДоступнаПечатьPDF417(ВыбраннаяФорма, ИмяОтчета = Неопределено)
	...
	Иначе
		ОбъектМетаданные = Отчеты.РегламентированноеУведомлениеФормаР11001;
	КонецЕсли;
...
Процедура ОчиститьПодписанныеДокументыНаСервере(РучнаяОчистка = Ложь)
	...
	Если РучнаяОчистка Тогда
		...
		ДанныеУведомленияОПереходеНаУСН = НайтиУведомлениеОСпецРежимеНалогообложения(
			Организация.Ссылка, ДатаСозданияЗаявления);
