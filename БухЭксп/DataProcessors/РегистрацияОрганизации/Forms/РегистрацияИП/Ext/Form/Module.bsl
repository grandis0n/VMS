...
Процедура ОчиститьПодписанныеДокументы(Команда)
	...
	ОчиститьПодписанныеДокументыНаСервере(Истина);
...
Процедура УведомлениеОПереходенНаУСН()
	...
	УведомлениеОСпецРежимеНалогообложения = НайтиУведомлениеОПереходеНаУСН(
		Организация.Ссылка, СтруктураУчетнойПолитики, ДатаСозданияЗаявления);
	...
	Если ЗначениеЗаполнено(УведомлениеОСпецРежимеНалогообложения.Ссылка) Тогда
		...
		УведомлениеОПереходе = УведомлениеОСпецРежимеНалогообложения.Ссылка;
		
		РегистрацияОрганизацииКлиент.ОткрытьПечатнуюФормуУведомленияОПереходеНаУСН(ЭтотОбъект, УведомлениеОПереходе);
...
&НаСервереБезКонтекста
Функция НайтиУведомлениеОПереходеНаУСН(Знач ОрганизацияСсылка, Знач СтруктураУчетнойПолитики, Знач ДатаСозданияЗаявления)
	...
	ИмяОтчета = УчетУСНКлиентСервер.ИмяОтчетаУведомленияОПереходеНаУСН(ДатаСозданияЗаявления, Ложь);
	
	ИмяФормыПоУмолчанию = Обработки.РегистрацияОрганизации.ПолноеИмяФормыПоУмолчанию(ИмяОтчета);
...
&НаСервере
Процедура ОчиститьПодписанныеДокументыНаСервере(РучнаяОчистка = Ложь)
	...
	Если РучнаяОчистка Тогда
		СтатусУведомлений = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусВРаботеСтрокой();
		
		ДанныеЗаявленияНаПатент = НайтиЗаявлениеПолучениеПатента(Организация.Ссылка, СтруктураУчетнойПолитики);
		ЗаявлениеНаПатент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЗаявленияНаПатент, "Ссылка");
		Если ЗначениеЗаполнено(ЗаявлениеНаПатент) Тогда
			ИнтерфейсыВзаимодействияБРО.СохранитьСтатусОтправкиУведомления(ЗаявлениеНаПатент, СтатусУведомлений);
		КонецЕсли;
		
		ДанныеУведомленияОПереходеНаУСН = НайтиУведомлениеОПереходеНаУСН(
			Организация.Ссылка, СтруктураУчетнойПолитики, ДатаСозданияЗаявления);
		УведомлениеОПереходеНаУСН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ДанныеУведомленияОПереходеНаУСН, "Ссылка");
		Если ЗначениеЗаполнено(УведомлениеОПереходеНаУСН) Тогда
			ИнтерфейсыВзаимодействияБРО.СохранитьСтатусОтправкиУведомления(УведомлениеОПереходеНаУСН, СтатусУведомлений);
		КонецЕсли;
	КонецЕсли;
