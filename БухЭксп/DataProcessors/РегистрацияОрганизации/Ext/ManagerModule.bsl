...
Процедура ПодготовитьДанныеДляЗавершенияРегистрацииИзмененийЕГРВФоне(Организация, АдресРезультата) Экспорт
	...
	Результат.ДанныеПомощникаЗаполнения = ПреобразоватьДанныеПомощника(ДанныеПомощника);
...
Процедура ЗаполнитьУведомлениеОПостановкеНаУчетИП(ПараметрыОтчета, Контейнер) Экспорт
	...
	СведенияОбИП.ТелефонСлужебный = РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(
		СведенияОбОрганизации.ТелСлуж, ИмяФормы);
...
Процедура ЗаполнитьЗаявлениеОПостановкеНаУчетООО(ПараметрыОтчета, Контейнер) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеУчредителей.Учредители Цикл
		...
		НовыйЗаявитель.Телефон = РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(
			СведенияОЗаявителе.Телефон, ПараметрыОтчета.ИмяФормы);
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийЕГРЮЛ(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если Изменено.Руководитель Тогда
		...
		НовыйЛист.Телефон = РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(
			ПредставлениеТелефона, ИмяФормы);
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийЕГРИП(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ПараметрыЗаполнения.Свойство("ДанныеЗаявителя", ДанныеЗаявителя) Тогда
		...
		ПредставлениеТелефона = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(
			ДанныеЗаявителя.ТелефонРабочийЗначениеJSON);
		СведенияОбИзменениях.ТелефонРабочий = РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(
			ПредставлениеТелефона, ПараметрыОтчета.ИмяФормы);
...
Процедура ЗаполнитьЗаявлениеОПрекращенииДеятельностиИП(ПараметрыУведомления, Контейнер) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("ДанныеЗаявителя", ДанныеЗаявителя) Тогда
		...
		ПредставлениеТелефона = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(
			ДанныеЗаявителя.ТелефонРабочийЗначениеJSON);
		СведенияОбИзменениях.ТелефонРабочий = РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(
			ПредставлениеТелефона, ПараметрыУведомления.ИмяФормы);
...
Функция ПреобразоватьДанныеПомощника(Данные)
	
	// Сконвертируем свойства в читаемые на клиенте (таблица значений -> массив структур)
	ТипДанных = ТипЗнч(Данные);
	
	Если ТипДанных = Тип("ТаблицаЗначений") Тогда
		Результат = Новый Массив;
		Для Каждого ТекущаяСтрока Из Данные Цикл
			Результат.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ТекущаяСтрока));
		КонецЦикла;
	ИначеЕсли ТипДанных = Тип("Структура") Тогда
		Результат = Новый Структура;
		Для Каждого ТекущееСвойство Из Данные Цикл
			Результат.Вставить(ТекущееСвойство.Ключ, ПреобразоватьДанныеПомощника(ТекущееСвойство.Значение));
		КонецЦикла;
	Иначе
		Возврат Данные;
	КонецЕсли;
...
Функция ЗаписатьДанныеУведомленияОПереходеНаУСН(Организация, КодНалоговогоОргана, Подписант, СпособРегистрации, ДатаСоставления, ОбъектНалогообложения = Неопределено)
	
	ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УведомлениеОПереходеНаУСН;
	
	ПрименяетсяЕдиноеУведомлениеУСН =
		ДатаСоставления >= УчетУСНКлиентСервер.ДатаНачалаПримененияЕдиногоУведомленияУСН();
	
	Если ПрименяетсяЕдиноеУведомлениеУСН Тогда
		ИмяФормыПоУмолчанию = ПолноеИмяФормыПоУмолчанию("РегламентированноеУведомлениеУСН", ДатаСоставления);
		
		ПараметрыЕдиногоУведомленияУСН = ПараметрыЕдиногоУведомленияОПереходеНаУСН(Организация,
			КодНалоговогоОргана,
			Подписант,
			СпособРегистрации,
			ВидУведомления,
		...
		ЗаявлениеОПереходеНаУСН = Отчеты.РегламентированноеУведомлениеУСН.НовоеУведомлениеПоУСН(
			ПараметрыЕдиногоУведомленияУСН);
	Иначе
...
Функция ПараметрыУведомленияОПереходеНаУСН(Организация, КодНалоговогоОргана, Подписант, СпособРегистрации, ОбъектНалогообложения = Неопределено)
	...
	Если ЗначениеЗаполнено(ОбъектНалогообложения) Тогда
		КодОбъекта = ?(ОбъектНалогообложения = Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы, "2", "1");
...
Функция ПараметрыЕдиногоУведомленияОПереходеНаУСН(Организация, КодНалоговогоОргана, Подписант, СпособРегистрации, ВидУведомления, ИмяФормыПолное, ДатаСоставления, ОбъектНалогообложения = Неопределено)
	
	ИмяФормы = Сред(ИмяФормыПолное, СтрНайти(ИмяФормыПолное, ".Форма.") + 7);
	
	ДанныеУведомления = Новый Структура;
	ПараметрыОтчета = Новый Структура;
	ПараметрыЗаполнения = Новый Структура;
	
	ДанныеУведомления.Вставить("ИмяФормы", ИмяФормы);
	
	ДанныеСозданногоУведомления = НайтиУведомлениеОСпецРежимеНалогообложения(Организация, ВидУведомления, ИмяФормыПолное);
	Если ЗначениеЗаполнено(ДанныеСозданногоУведомления.Ссылка) Тогда
		ДанныеУведомления.Вставить("Ссылка", ДанныеСозданногоУведомления.Ссылка);
	КонецЕсли;
	
	ДанныеУведомления.Вставить("Организация", Организация);
	ПараметрыОтчета.Вставить("Организация", Организация);
	
	РегистрацияВИФНС = Справочники.Организации.РегистрацияВНалоговомОрганеНаДату(Организация, ДатаСоставления);
	ДанныеУведомления.Вставить("РегистрацияВИФНС", РегистрацияВИФНС);
	
	ПараметрыЗаполнения.Вставить("ДатаСоставления", ДатаСоставления);
	ПараметрыЗаполнения.Вставить("ЭтоПереходНаУСН", Истина);
	ПараметрыЗаполнения.Вставить("ОбъектНалогообложения", ОбъектНалогообложения);
	ПараметрыЗаполнения.Вставить("КодНО", КодНалоговогоОргана);
	
	Если СпособРегистрации = 1 Тогда
		ДанныеУведомления.Вставить("ПодписантФамилия", Подписант.Фамилия);
		ДанныеУведомления.Вставить("ПодписантИмя", Подписант.Имя);
		ДанныеУведомления.Вставить("ПодписантОтчество", Подписант.Отчество);
		
		ПодписантФИО = СтрШаблон("%1 %2 %3", Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
		
		ТелефонПодписанта = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(
			Подписант.ТелефонРабочийЗначениеJSON);
		
		ПараметрыЗаполнения.Вставить("ПодписываетНалогоплательщик", Истина);
		ПараметрыЗаполнения.Вставить("ПодписантФИО", ПодписантФИО);
		ПараметрыЗаполнения.Вставить("ТелефонПредставителя", ТелефонПодписанта);
	КонецЕсли;
