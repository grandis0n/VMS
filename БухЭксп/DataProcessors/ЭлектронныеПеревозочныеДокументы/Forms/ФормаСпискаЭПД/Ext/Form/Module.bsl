...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДемоРежимПоказатьСтатусДемоРежима(ЭтотОбъект,  ОбменСГИСЭПД.ДемоРежимВключен());
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, СтрокиСписка)
	...
	Если СтрокаСписка.Данные.Свойство("ДокументОснование") Тогда
		...
		Запрос.Текст =
			...
			|
			|ВЫБРАТЬ
			|	ЭлектронноеПоручениеЭкспедиторуДокументыОснования.Ссылка,
			|	ЭлектронноеПоручениеЭкспедиторуДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектронноеПоручениеЭкспедитору.ДокументыОснования КАК ЭлектронноеПоручениеЭкспедиторуДокументыОснования
			|ГДЕ
			|	ЭлектронноеПоручениеЭкспедиторуДокументыОснования.Ссылка В (&Документы)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ЭлектроннаяЭкспедиторскаяРаспискаДокументыОснования.Ссылка,
			|	ЭлектроннаяЭкспедиторскаяРаспискаДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектроннаяЭкспедиторскаяРасписка.ДокументыОснования КАК ЭлектроннаяЭкспедиторскаяРаспискаДокументыОснования
			|ГДЕ
			|	ЭлектроннаяЭкспедиторскаяРаспискаДокументыОснования.Ссылка В (&Документы)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ЭлектроннаяCкладскаяРаспискаДокументыОснования.Ссылка,
			|	ЭлектроннаяCкладскаяРаспискаДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектроннаяCкладскаяРасписка.ДокументыОснования КАК ЭлектроннаяCкладскаяРаспискаДокументыОснования
			|ГДЕ
			|	ЭлектроннаяCкладскаяРаспискаДокументыОснования.Ссылка В (&Документы)";
...
КонецПроцедуры
 ...
 &НаКлиенте
Процедура СоздатьЭПЭ(Команда)
	ОткрытьФорму("Документ.ЭлектронноеПоручениеЭкспедитору.ФормаОбъекта");
КонецПроцедуры

&НаКлиенте
Процедура СоздатьЭЭР(Команда)
	ОткрытьФорму("Документ.ЭлектроннаяЭкспедиторскаяРасписка.ФормаОбъекта");
КонецПроцедуры

&НаКлиенте
Процедура СоздатьЭСР(Команда)
	ОткрытьФорму("Документ.ЭлектроннаяCкладскаяРасписка.ФормаОбъекта");
КонецПроцедуры
...
Процедура ИнициализироватьХозяйственныеОперацииИДокументы(ХозяйственныеОперацииИДокументы, КлючНазначенияИспользования)
	...
	Строка = ХозяйственныеОперацииИДокументы.Добавить();
	Строка.ПолноеИмяДокумента             = Метаданные.Документы.ЭлектронноеПоручениеЭкспедитору.ПолноеИмя();
	Строка.ТипДокумента					  = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору");
	Строка.КлючНазначенияИспользования    = КлючНазначенияИспользования;
	Строка.ЗаголовокРабочегоМеста         = НСтр("ru = 'Поручение экспедитору'");
	Строка.ДобавитьКнопкуСоздать          = Истина;
	Строка.ПравоДоступаДобавление         = Истина;
	Строка.Отбор                          = Истина;
	Строка.ПравоДоступаИзменение          = Истина;
	...
	Строка = ХозяйственныеОперацииИДокументы.Добавить();
	Строка.ПолноеИмяДокумента             = Метаданные.Документы.ЭлектроннаяЭкспедиторскаяРасписка.ПолноеИмя();
	Строка.ТипДокумента					  = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка");
	Строка.КлючНазначенияИспользования    = КлючНазначенияИспользования;
	Строка.ЗаголовокРабочегоМеста         = НСтр("ru = 'Экспедиторская расписка'");
	Строка.ДобавитьКнопкуСоздать          = Истина;
	Строка.ПравоДоступаДобавление         = Истина;
	Строка.Отбор                          = Истина;
	Строка.ПравоДоступаИзменение          = Истина;
	...
	Строка = ХозяйственныеОперацииИДокументы.Добавить();
	Строка.ПолноеИмяДокумента             = Метаданные.Документы.ЭлектроннаяCкладскаяРасписка.ПолноеИмя();
	Строка.ТипДокумента					  = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка");
	Строка.КлючНазначенияИспользования    = КлючНазначенияИспользования;
	Строка.ЗаголовокРабочегоМеста         = НСтр("ru = 'Складская расписка'");
	Строка.ДобавитьКнопкуСоздать          = Истина;
	Строка.ПравоДоступаДобавление         = Истина;
	Строка.Отбор                          = Истина;
	Строка.ПравоДоступаИзменение          = Истина;
...
&НаКлиентеНаСервереБезКонтекста
Функция ДемоРежимПолучитьПунктМеню(Форма)
	Возврат Форма.Элементы.ВклВыклДемоРежим;
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ДемоРежимПоказатьСтатусДемоРежима(Форма, ДемоРежимВкл)
	ДемоРежимПунктМеню = ДемоРежимПолучитьПунктМеню(Форма);
	ДемоРежимПунктМеню.Пометка = ДемоРежимВкл;
	Форма.Элементы.ГруппаДемоРежим.Видимость = ДемоРежимПунктМеню.Пометка;  
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ДемоРежимВклВыклДемоРежимНаСервере(Знач Включить)
	ОбменСГИСЭПД.ДемоРежимВключить(Включить);
КонецПроцедуры

&НаКлиенте
Процедура ДемоРежимВклВыклДемоРежим(Команда)
	ДемоРежимВкл = Не ДемоРежимПолучитьПунктМеню(ЭтотОбъект).Пометка;
	ДемоРежимВклВыклДемоРежимНаСервере(ДемоРежимВкл);
	ДемоРежимПоказатьСтатусДемоРежима(ЭтотОбъект, ДемоРежимВкл);
КонецПроцедуры
