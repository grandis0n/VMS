...
Функция ВыполнитьЗагрузкуКурсов()
	...
	РегламентноеЗадание = Метаданные.РегламентныеЗадания.CW_ЗагрузкаКурсовВалют_ЕЦБ;
...
Процедура ОбработатьРезультатЗагрузки(РезультатЗагрузки)
	...
	Если ЕстьУспешноЗагруженныеКурсы Тогда
		...
		КонецЦикла;
		Оповестить("Запись_ЗагрузкаКурсовВалют_ЕЦБ", ПараметрыЗаписи, МассивОбновляемыхВалют);
...
Процедура ОбработатьРезультатЗагрузкиПриОтветеНаВопрос(РезультатВопроса, ПредставлениеОшибки) Экспорт
	...
	ОткрытьФорму("Обработка.CW_ЗагрузкаКурсовВалют_ЕЦБ.Форма.СообщенияОбОшибках", Новый Структура("Текст", ПредставлениеОшибки));
