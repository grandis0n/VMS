...
&НаКлиенте
Перем НастроитьВидимостьЭлементовИКомандОтложенно;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользуетсяОбменСГИСЭПД = ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД");
	ИспользуетсяОбменМПЭПД = ОбщегоНазначения.ПодсистемаСуществует(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СобытиеОбновитьСписки(ИмяСобытия) Тогда
		...
		НастроитьВидимостьЭлементовИКомандОтложенно = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	Если Не ПараметрыПриложения[ИмяПараметра].Свойство("ОткрытаФормаПервогоПодключения") Тогда
		ПараметрыПриложения[ИмяПараметра].Вставить("ОткрытаФормаПервогоПодключения", Ложь);
	КонецЕсли;
...
Процедура БоковоеМенюПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ИначеЕсли ДанныеСобытия.Element.id = "epd-button" Тогда
		ОткрытьЖурналЭПД();
		Возврат;
	Иначе
		Возврат;
...
&НаКлиенте
Процедура ОткрытьСписокМобильныхУстройствЭПД(Команда)

	МодульСервисВзаимодействияМПЭПДКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисВзаимодействияМПЭПДКлиент");
	МодульСервисВзаимодействияМПЭПДКлиент.ОткрытьФормуСпискаМП(ЭтотОбъект);
...
Процедура ОтложенноеОбновление()
	...
	КонецЕсли;
	Если НастроитьВидимостьЭлементовИКомандОтложенно = Неопределено Тогда
		НастроитьВидимостьЭлементовИКомандОтложенно = Ложь;
	КонецЕсли;
	...
	Если НастроитьВидимостьЭлементовИКомандОтложенно Тогда
		ВидимостьЭлементовИКоманд();
	КонецЕсли;
...
Процедура КоличествоДокументов()
	...
	Если Не ИспользуетсяОбменСГИСЭПД Тогда
		ПанельНавигации = СтрЗаменить(ПанельНавигации,
		"<hr><a href = ""EPD"" class = ""menu-link"" id=""epd-button"">Перевозочные документы</a>", "");
	КонецЕсли;
...
Процедура ВидимостьЭлементовИКоманд()
	...
	Элементы.ОткрытьСписокМобильныхУстройств.Видимость = ИспользуетсяОбменМПЭПД;
...
&НаКлиенте
Процедура ОткрытьЖурналЭПД()
	
	ПараметрыФормы = Новый Структура("КлючНазначенияФормы", "ФормаСпискаЭПД");
	ОткрытьФорму("Обработка.ЭлектронныеПеревозочныеДокументы.Форма.ФормаСпискаЭПД", ПараметрыФормы);
