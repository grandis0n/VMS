...
&НаКлиенте
Перем ОбновитьНавигациюПоРазделамОтложенно;
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, Знач НастройкиОтображения,
	...
	ИначеЕсли Раздел = "НаКонтроле" Тогда
		...
		ЗначенияСостояниеЭДО.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент"));
...
Процедура ПослеВыполненияДействийПоЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбновитьНавигациюПоРазделамОтложенно = Истина;
	ОбновитьОтложенно();
	ПодключитьОбработчикОжидания("ОбновитьОбластьПросмотра", 0.2, Истина);
	РазблокироватьЗаблокированныеЭлементыФормы();
...
Процедура ОтложенноеОбновление()
	...
	Если ОбновитьНавигациюПоРазделамОтложенно = Неопределено Тогда
		ОбновитьНавигациюПоРазделамОтложенно = Ложь;
	КонецЕсли;
	
	// Для отложенного обновления необходимо чтобы хотя бы что-нибудь нужно было обновить.
	НужноОбновитьОтложенно = ОбновитьНавигациюПоРазделамОтложенно;
	Если Не НужноОбновитьОтложенно Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбновитьНавигациюПоРазделамОтложенно Тогда
		ОбновитьНавигациюПоРазделам();
	КонецЕсли;
...
Функция РаспознатьДокумент(ЭлектронныйДокумент)
	...
	Если Не ДокументРаспознан 
		...
		РезультатРаспознавания = МодульОбменСГИСЭПД.ПриРаспознаванииДокумента(ЭлектронныйДокумент, ДокументРаспознан); 
	КонецЕсли;
