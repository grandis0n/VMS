...
&НаКлиенте
Перем ОбновитьДинамическийСписокТекущегоРазделаОтложенно;

&НаКлиенте
Перем ОбновитьДинамическиеСпискиФормыОтложенно;

&НаКлиенте
Перем ОбработатьПереключениеРазделаОтложенно;

&НаКлиенте
Перем НастроитьВидимостьКомандТекущейСтрокиОтложенно;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Раздел) Тогда
		ПоказатьРаздел(Параметры.Раздел);
	Иначе
		ТекущийРаздел = "ВходящиеЭлектронныеДокументы";
	КонецЕсли;
	
	УстановитьОтборДокументов(Параметры.ОтборУчетныхДокументов, "Сформировать", "ОтборУчетныхДокументов");
	УстановитьОтборДокументов(Параметры.ОтборВходящихДокументов, "ВходящиеЭлектронныеДокументы");
	УстановитьОтборДокументов(Параметры.ОтборИсходящихДокументов, "ИсходящиеЭлектронныеДокументы");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли СобытиеОбновитьСписки(ИмяСобытия) Тогда
		...
		ОбновитьДинамическийСписокТекущегоРазделаОтложенно = Истина;
		ОбработатьПереключениеРазделаОтложенно = Истина;
		ОбновитьОтложенно();
...
&НаКлиенте
Процедура СменитьОтветственногоКоманда(Команда)
...
Процедура ПредставлениеСостояния(ДанныеСтроки)
	...
	ИначеЕсли ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтзывом Тогда
		
		ДанныеСтроки.Состояние = НСтр("ru = 'Отозван'");
		
	ИначеЕсли ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент Тогда
		
		ДанныеСтроки.Состояние = НСтр("ru = 'Обработать'");
...
Функция ДокументТребуетВнимания(Знач ДанныеСтроки)
	...
	СостоянияТребующиеВнимания = СостоянияДокументовТребующиеВнимания();
	
	Если СостоянияТребующиеВнимания.НайтиПоЗначению(ДанныеСтроки.СостояниеЭДО) <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ДокументВСостоянииЗавершен(Знач ДанныеСтроки)
	...
	Возврат ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ЗакрытПринудительно 
		...
		ИЛИ ДанныеСтроки.СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтзывом;
...
Функция СостоянияДокументовТребующиеВнимания()
	...
	СписокСостояний.Добавить(Перечисления.СостоянияДокументовЭДО.ТребуетсяПодписаниеИзвещения);
	СписокСостояний.Добавить(Перечисления.СостоянияДокументовЭДО.ТребуетсяПодписаниеИзвещенияПоОтклонению);
	СписокСостояний.Добавить(Перечисления.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент);
...
&НаСервере
Процедура УстановитьОтборДокументов(Знач Отбор, Знач ИмяСписка, Знач ИмяГруппыОтбора = "ОтборЭлектронныхДокументов")
	...
	ГруппаОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементОтбораПоПредставлению(Список.Отбор.Элементы,
		ИмяГруппыОтбора);
	Если ГруппаОтбора = Неопределено Тогда
		ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Список.Отбор.Элементы, ИмяГруппыОтбора,
...
Процедура ПоказатьРаздел(Знач Раздел)
	...
	РазделыТекущихДелЭДО = ОбменСКонтрагентамиКлиентСервер.РазделыТекущихДелЭДО();

	Если Раздел = РазделыТекущихДелЭДО.Входящие
		Или Раздел = РазделыТекущихДелЭДО.УтвердитьВходящие 
		Или Раздел = РазделыТекущихДелЭДО.ПодписатьВходящие
		Или Раздел = РазделыТекущихДелЭДО.ИсправитьВходящие
		Или Раздел = РазделыТекущихДелЭДО.ОтразитьВУчете
		Или Раздел = РазделыТекущихДелЭДО.АннулироватьВходящие
		Или Раздел = РазделыТекущихДелЭДО.НаКонтролеВходящие Тогда
		ТекущийРаздел = "ВходящиеЭлектронныеДокументы";
		Заголовок = НСтр("ru = 'ЭДО: Входящие документы'");
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВходящиеИсходящие; 
		Элементы.СтраницыВходящиеИсходящие.ТекущаяСтраница = Элементы.СтраницаВходящиеЭлектронныеДокументы;
	ИначеЕсли Раздел = РазделыТекущихДелЭДО.Исходящие
		Или Раздел = РазделыТекущихДелЭДО.ПодписатьИсходящие
		Или Раздел = РазделыТекущихДелЭДО.ИсправитьИсходящие
		Или Раздел = РазделыТекущихДелЭДО.АннулироватьИсходящие
		Или Раздел = РазделыТекущихДелЭДО.НаКонтролеИсходящие Тогда
		ТекущийРаздел = "ИсходящиеЭлектронныеДокументы";
		Заголовок = НСтр("ru = 'ЭДО: Исходящие документы'");
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВходящиеИсходящие; 
		Элементы.СтраницыВходящиеИсходящие.ТекущаяСтраница = Элементы.СтраницаИсходящиеЭлектронныеДокументы;
	ИначеЕсли Раздел = РазделыТекущихДелЭДО.Создать Тогда
		ТекущийРаздел = "Сформировать";
		Заголовок = НСтр("ru = 'ЭДО: Отправить'");
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСформировать;
	Иначе
...
&НаКлиенте
Процедура ОбновитьОтложенно()
	
	ПодключитьОбработчикОжидания("ОтложенноеОбновление", 0.1, Истина);
...
Процедура ОтложенноеОбновление()
	...
	// Инициализируем переменные формы, на случай если не все еще инициализированы.
	Если ОбновитьДинамическийСписокТекущегоРазделаОтложенно = Неопределено Тогда
		ОбновитьДинамическийСписокТекущегоРазделаОтложенно = Ложь;
	КонецЕсли;
	Если ОбновитьДинамическиеСпискиФормыОтложенно = Неопределено Тогда
		ОбновитьДинамическиеСпискиФормыОтложенно = Ложь;
	КонецЕсли;
	Если ОбработатьПереключениеРазделаОтложенно = Неопределено Тогда
		ОбработатьПереключениеРазделаОтложенно = Ложь;
	КонецЕсли;
	Если НастроитьВидимостьКомандТекущейСтрокиОтложенно = Неопределено Тогда
		НастроитьВидимостьКомандТекущейСтрокиОтложенно = Ложь;
	КонецЕсли;
	
	// Для отложенного обновления необходимо чтобы хотя бы что-нибудь нужно было обновить.
	НужноОбновитьОтложенно =
		ОбновитьДинамическийСписокТекущегоРазделаОтложенно Или ОбновитьДинамическиеСпискиФормыОтложенно;
	Если Не НужноОбновитьОтложенно Тогда
		Возврат;
	КонецЕсли;
	
	ОтложенноеОбновлениеНаСервере(
		ОбновитьДинамическийСписокТекущегоРазделаОтложенно,
		ОбновитьДинамическиеСпискиФормыОтложенно);
	Если ОбработатьПереключениеРазделаОтложенно И ТребуетсяПереключениеРаздела Тогда
		УстановитьТекущимРазделомВходящиеЭлектронныеДокументы();
	КонецЕсли;
	Если НастроитьВидимостьКомандТекущейСтрокиОтложенно Тогда
		НастроитьВидимостьКомандТекущейСтроки(Элементы[ТекущийРаздел].ТекущиеДанные);
	КонецЕсли;
	
	// Сбрасываем переменные формы, т.к. завершили отложенное обновление.
	ОбновитьДинамическийСписокТекущегоРазделаОтложенно = Ложь;
	ОбновитьДинамическиеСпискиФормыОтложенно = Ложь;
	ОбработатьПереключениеРазделаОтложенно = Ложь;
	НастроитьВидимостьКомандТекущейСтрокиОтложенно = Ложь;
...
&НаСервере
Процедура ОтложенноеОбновлениеНаСервере(
	Знач ОбновитьДинамическийСписокТекущегоРазделаОтложенно,
	Знач ОбновитьДинамическиеСпискиФормыОтложенно)
	
	Если ОбновитьДинамическийСписокТекущегоРазделаОтложенно Тогда
		
		ОбновитьДинамическийСписокТекущегоРаздела();
		
	КонецЕсли;
	
	Если ОбновитьДинамическиеСпискиФормыОтложенно Тогда
		
		ОбновитьДинамическиеСпискиФормы();
		
		// Обновление количества документов уже есть в ОбновитьДинамическийСписокТекущегоРаздела().
		Если Не ОбновитьДинамическийСписокТекущегоРазделаОтложенно Тогда
			КоличествоДокументов();
		КонецЕсли;
...
Процедура УстановитьВидимостьКомандФормыПоПравам()
	...
	Элементы.СменитьОтветственногоВходящие.Видимость = ЕстьПравоОбработкиДокументов;
	Элементы.СменитьОтветственногоИсходящие.Видимость = ЕстьПравоОбработкиДокументов;
	Элементы.ВходящиеЭлектронныеДокументыСменитьОтветственного.Видимость = ЕстьПравоОбработкиДокументов;
	Элементы.ИсходящиеЭлектронныеДокументыСменитьОтветственного.Видимость = ЕстьПравоОбработкиДокументов;
...
Функция АннулированиеДоступно(ТипДокумента, СостоянияДокумента)
	...
	Если ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.Черновик") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОтправкаПолучениеЭДЗавершение(Результат, Контекст) Экспорт
	...
	ОбновитьДинамическиеСпискиФормыОтложенно = Истина;
	ОбновитьОтложенно();
...
Процедура ПослеВыполненияДействийПоЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбновитьДинамическийСписокТекущегоРазделаОтложенно = Истина;
	НастроитьВидимостьКомандТекущейСтрокиОтложенно = Истина;
	ОбновитьОтложенно();
...
Функция АннулированиеДоступноДляДокументов(Документы)
	...
	СостоянияДокументовЭДО = ЭлектронныеДокументыЭДО.СостоянияДокументов(Документы);
	...
	Для Каждого СостояниеПоДокументу Из СостоянияДокументовЭДО Цикл
