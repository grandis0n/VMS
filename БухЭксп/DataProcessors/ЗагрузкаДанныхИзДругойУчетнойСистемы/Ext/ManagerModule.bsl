...
Функция ПрочитатьВыбранныеФайлы(Свойства)
	...
	Для Каждого СвойстваФайла Из Свойства.ДанныеФайлов Цикл
		...
		РасширениеФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
...
Функция НовоеОписаниеЗагружаемыхФайлов()
	...
	ОписаниеЗагружаемыхФайлов.Колонки.Добавить("ПриоритетноеЗначение");
...
Процедура ЗаполнитьОписаниеИменамиФайловДляЗагрузкиИзЭлектронныхТаблиц(Описание)
	...
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяКонтрагенты(), "название(фио)");
	...
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяНоменклатура(), "наименование", , Ложь);
	...
	// другой формат оборотно-сальдовой ведомости
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяОборотыИСальдо(), ПризнакЗаголовкаОборотыИСальдо());
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяОсновныеСредства(), "типос", НСтр("ru = 'Основные средства'"));
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяРеестрИмущества(), "амортизация", НСтр("ru = 'Основные средства'"));
...
Процедура ДобавитьВОписаниеФайлыЭлектронныхТаблиц(Файлы, ОписаниеФайлов)
	...
	Для Каждого СтрокаФайла Из Файлы Цикл
		...
		Для НомерСтроки = 1 По ГраницаСтрока Цикл
			ВозможноеСовпадение = Неопределено;
			Для НомерКолонки = 1 По ГраницаКолонка Цикл
				...
				Для Каждого Строка Из ОписаниеФайлов Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Если СтрокаОписания = Неопределено И ВозможноеСовпадение <> Неопределено Тогда
					СтрокаОписания = ВозможноеСовпадение;
				КонецЕсли;
				Если СтрокаОписания = Неопределено Тогда
					Продолжить;
				ИначеЕсли Не СтрокаОписания.ПриоритетноеЗначение И Не НомерКолонки = ГраницаКолонка Тогда
					ВозможноеСовпадение = СтрокаОписания;
					Продолжить;
...
Функция ПрочитатьДанныеИзЭлектроннойТаблицы(СтрокаОписания, Ошибки, ЗагружаемыеДанные)
	...
	ИначеЕсли СтрокаОписания.ИмяФайла = ИмяНоменклатура() Тогда
		...
		ДобавитьЗагрузкуНачальныхОстатковПоСправочнику(ПрочитанныеДанные, ЗагружаемыеДанные, ИмяОборотыИСальдо());
	ИначеЕсли СтрокаОписания.ИмяФайла = ИмяОборотноСальдоваяВедомость()
		Или СтрокаОписания.ИмяФайла = ИмяОборотыИСальдо() Тогда
	...
	ИначеЕсли СтрокаОписания.ИмяФайла = ИмяРеестрИмущества() Тогда
		ДобавитьЗагрузкуНачальныхОстатковПоСправочнику(ПрочитанныеДанные, ЗагружаемыеДанные, ИмяРеестрИмущества());
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ПрочитанныеДанные, ЗагружаемыеДанные);
	Иначе
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ПрочитанныеДанные, ЗагружаемыеДанные);
	КонецЕсли;
...
Процедура ЗаполнитьКоличествоОбъектов(Описание, ТаблицаДанных, ЗагружаемыеДанные)
	...
	Если ТаблицаДанных.Колонки.Найти("ТипДокумента") = Неопределено Тогда
		...
		Если НайденнаяСтрока = Неопределено Тогда
			...
			Если ИмяТаблицы = ИмяРеестрИмущества() Тогда
				НовСтрока.ИмяРеквизитаФормы = ИмяОсновныеСредства();
			Иначе
...
Процедура ДобавитьЗагрузкуНачальныхОстатковПоСправочнику(ПрочитанныеДанные, ЗагружаемыеДанные, ИмяФайла)
	
	Если ПрочитанныеДанные.Колонки.Найти("Количество") = Неопределено
		И ПрочитанныеДанные.Колонки.Найти("Остаток") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Данные = Новый ТаблицаЗначений;
	Данные.Колонки.Добавить("ТипДокумента");
	Данные.Колонки.Добавить("КоличествоОбъектов");
	
	НоваяСтрока = Данные.Добавить();
	НоваяСтрока.ТипДокумента = "ВводНачальныхОстатков";
	Описание = Новый Структура;
	Описание.Вставить("ИмяФайла", ИмяФайла);
	ЗаполнитьКоличествоОбъектов(Описание, Данные, ЗагружаемыеДанные);
...
Функция ПрочитатьЭлектроннуюТаблицу(СтрокаОписанияФайла, Ошибки)
	...
	ИначеЕсли ИмяФайла = ИмяОборотыИСальдо() Тогда
		Возврат ПрочитатьОборотыИСальдо(СоставДанных, Таблица);
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли (ИмяФайла = ИмяНоменклатура() Или ИмяФайла = ИмяРеестрИмущества())
			И Таблица.КоличествоУровнейГруппировокСтрок() > 0 Тогда
			ДополнитьТаблицуУровнямиГруппировок(ТаблицаДанных, Таблица);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьСоответствияРеквизитовЭлектронныхТаблиц(ИмяФайла, СоставДанных)
	...
	ИначеЕсли ИмяФайла = ИмяОборотыИСальдо() Тогда
		ЗаполнитьСоответствияРеквизитовОборотыИСальдо(СоставДанных);
	ИначеЕсли ИмяФайла = ИмяОсновныеСредства() Тогда
		ЗаполнитьСоответствияРеквизитовОсновныеСредства(СоставДанных);
	ИначеЕсли ИмяФайла = ИмяРеестрИмущества() Тогда
		ЗаполнитьСоответствияРеквизитовРеестрИмущества(СоставДанных);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуДанных(
	...
	ЗаполнятьНомерСтроки = СоставДанных.Найти(ИмяКолонкиНомерСтроки(), ИмяРеквизита1С()) <> Неопределено;
	...
	Для НомерСтроки = Начало По Окончание Цикл
		...
		Иначе
			...
			Если ЗаполнятьНомерСтроки Тогда
				ДанныеОбъекта[ИмяКолонкиНомерСтроки()] = НомерСтроки;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗагрузитьДанныеИзЭлектронныхТаблиц(ДанныеЗагрузки, Результат, АдресРезультата)
	...
	Если ДанныеЗагрузки.Свойство(ИмяРеестрИмущества()) Тогда
		ЗагрузитьРеестрИмущества(ДанныеЗагрузки, Результат.Ошибки);
		ДлительныеОперации.СообщитьПрогресс( , ИмяРеестрИмущества());
	КонецЕсли;
	...
	Если ДанныеЗагрузки.Свойство(ИмяОборотноСальдоваяВедомость())
		Или ДанныеЗагрузки.Свойство(ИмяОборотыИСальдо()) Тогда
...
Процедура ЗагрузитьРеестрИмущества(ЗагружаемыеДанные, Ошибки)
	
	Организация = ЗагружаемыеДанные.Организация;
	Подразделение = ОсновноеПодразделениеОрганизации(Организация);
	РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства;
	
	Если Не УспешнаяПроверкаДублейНачальныхОстатков(Организация, Подразделение, РазделУчета) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеОС = ЗагружаемыеДанные[ИмяРеестрИмущества()];
	ГруппыОС = ДополнитьСправочникГруппамиСУчетомУровнейВложенности(
		ДанныеОС, ИмяОсновныеСредства(), "Наименование");
	Наименования = ДанныеОС.ВыгрузитьКолонку("Наименование");
	...
	Запрос.Текст = 
		...
		|		ПО ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство = ОсновныеСредства.Ссылка
		|ГДЕ
		|	ОсновныеСредства.Наименование В(&Наименования)
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	ВзятыеВРаботуОС = Новый Массив;
	
	ВыборкаНаименование = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	ДанныеОС.ЗаполнитьЗначения(Неопределено, "Ссылка");
	Для Каждого СтрокаОС Из ДанныеОС Цикл
		Если ВыборкаНаименование.НайтиСледующий(СтрокаОС.Наименование) Тогда
			Выборка = ВыборкаНаименование.Выбрать();
			Пока Выборка.Следующий() Цикл
				Если ВзятыеВРаботуОС.Найти(Выборка.Ссылка) = Неопределено Тогда
					СтрокаОС.Ссылка = Выборка.Ссылка;
					ВзятыеВРаботуОС.Добавить(Выборка.Ссылка);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ВыборкаНаименование.Сбросить();
	КонецЦикла;
	
	Отбор = Новый Структура("Ссылка", Неопределено);
	НовыеОС = ДанныеОС.НайтиСтроки(Отбор);
	Для Каждого СтрокаОС Из НовыеОС Цикл
		ДанныеЗаполнения = Новый Структура;
		ДанныеЗаполнения.Вставить("Наименование", СтрокаОС.Наименование);
		ДанныеЗаполнения.Вставить("НаименованиеПолное", СтрокаОС.Наименование);
		Если ЗначениеЗаполнено(СтрокаОС.Родитель) Тогда
			СтрокаРодитель = ГруппыОС.Найти(СтрокаОС.Родитель);
			Если СтрокаРодитель <> Неопределено Тогда
				ДанныеЗаполнения.Вставить("Родитель", СтрокаРодитель.Ссылка);
			КонецЕсли;
		КонецЕсли;
		Если СтрНайти(НРег(СтрокаОС.Родитель), НСтр("ru = 'земельный'")) > 0
			Или СтрНайти(НРег(СтрокаОС.Наименование), НСтр("ru = 'земельный'")) > 0 Тогда
			ДанныеЗаполнения.Вставить("ГруппаОС", Перечисления.ГруппыОС.ЗемельныеУчастки);
			ДанныеЗаполнения.Вставить("НедвижимоеИмущество", Истина);
		ИначеЕсли СтрНайти(НРег(СтрокаОС.Родитель), НСтр("ru = 'здание'")) > 0
			Или СтрНайти(НРег(СтрокаОС.Наименование), НСтр("ru = 'здание'")) > 0 Тогда
			ДанныеЗаполнения.Вставить("ГруппаОС", Перечисления.ГруппыОС.Здания);
			ДанныеЗаполнения.Вставить("НедвижимоеИмущество", Истина);
		ИначеЕсли СтрНайти(НРег(СтрокаОС.Родитель), НСтр("ru = 'транспорт'")) > 0
			Или СтрНайти(НРег(СтрокаОС.Наименование), НСтр("ru = 'транспорт'")) > 0
			Или СтрНайти(НРег(СтрокаОС.Родитель), НСтр("ru = 'автомобил'")) > 0
			Или СтрНайти(НРег(СтрокаОС.Наименование), НСтр("ru = 'автомобил'")) > 0 Тогда
			ДанныеЗаполнения.Вставить("ГруппаОС", Перечисления.ГруппыОС.ТранспортныеСредства);
		Иначе
			ДанныеЗаполнения.Вставить("ГруппаОС", Перечисления.ГруппыОС.МашиныИОборудование);
		КонецЕсли;
		ДанныеЗаполнения.Вставить("ТипОС", Перечисления.ТипыОС.ОбъектОС);
		
		НовыйОС = Справочники.ОсновныеСредства.СоздатьЭлемент();
		НовыйОС.Заполнить(ДанныеЗаполнения);
		НовыйОС.ОбменДанными.Загрузка = Истина;
		НовыйОС.Записать();
		СтрокаОС.Ссылка = НовыйОС.Ссылка;
	КонецЦикла;
	
	ДатаВводаОстатков = ДатаВводаОстатков(Организация);
	ПроверитьУстановитьЗаписьУчетнойПолитики(Организация, ДатаВводаОстатков);
	...
	НовыйДокумент.РазделУчета = РазделУчета;
	НомерДокументаПринятияКУчету = 1;
	СпособОтражения = СпособОтраженияРасходовПоАмортизации("26");
	СпособНачисленияАмортизации = СпособНачисленияАмортизации();
	
	Для Каждого ОсновноеСредство Из ДанныеОС Цикл
		СтрокаДокумента = НовыйДокумент.ОС.Добавить();
		СтрокаДокумента.ОсновноеСредство = ОсновноеСредство.Ссылка;
		СтрокаДокумента.ПервоначальнаяСтоимостьБУ = ОсновноеСредство.Стоимость;
		СтрокаДокумента.ПервоначальнаяСтоимостьНУ = ОсновноеСредство.СтоимостьНУ;
		СтрокаДокумента.ТекущаяСтоимостьБУ = ОсновноеСредство.Стоимость;
		СтрокаДокумента.ТекущаяСтоимостьНУ = ОсновноеСредство.СтоимостьНУ;
		СтрокаДокумента.НакопленнаяАмортизацияБУ = ОсновноеСредство.Амортизация;
		СтрокаДокумента.НакопленнаяАмортизацияНУ = ОсновноеСредство.АмортизацияНУ;
		СтрокаДокумента.СпособНачисленияАмортизацииБУ = СпособНачисленияАмортизации;
		...
		СтрокаДокумента.СрокПолезногоИспользованияБУ = 60;
		СтрокаДокумента.ДатаПринятияКУчетуРегл = НачалоГода(НачалоГода(ДатаВводаОстатков) - 1);
...
Функция ДанныеЗаполненияКонтрагента(СтрокаДанных, ДанныеИзСервиса)
	...
	ФИОРуководителя = СтрШаблон(
		"%1 %2 %3", СтрокаДанных.ФамилияРуководителя, СтрокаДанных.ИмяРуководителя, СтрокаДанных.ОтчествоРуководителя);
	ДанныеЗаполнения.Вставить("Руководитель", СокрЛП(ФИОРуководителя));
	ДанныеЗаполнения.Вставить("ДолжностьРуководителя", НСтр("ru = 'Директор'"));
...
Функция СоставРеквизитовКонтрагентов()
	...
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "Наименование", , "название (фио)");
	...
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "ОГРН", , "огрн/огрнип");
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "КодПоОКПО", "окпо", "окпо");
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "Адрес", "адрес контрагента", "адрес");
	...
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "ФамилияРуководителя", , "фамилия руководителя");
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "ИмяРуководителя", , "имя руководителя");
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "ОтчествоРуководителя", , "отчество руководителя");
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(
...
Процедура ЗагрузитьНачальныеОстатки(ЗагружаемыеДанные, Ошибки)
	...
	Если ЗагружаемыеДанные.Свойство(ИмяОборотноСальдоваяВедомость()) Тогда
		ТаблицаДанных = ЗагружаемыеДанные[ИмяОборотноСальдоваяВедомость()];
	ИначеЕсли ЗагружаемыеДанные.Свойство(ИмяОборотыИСальдо()) Тогда
		ТаблицаДанных = ЗагружаемыеДанные[ИмяОборотыИСальдо()];
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьОстаткиПоНоменклатуре(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	...
	ОтступПередСкладом = "    ";
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		Если СтрНачинаетсяС(Строка.Аналитика, ОтступПередСкладом) Тогда // Игнорируем строки с аналитикой по складу
			Продолжить;
		КонецЕсли;
	...
	Организация = ЗагружаемыеДанные.Организация;
	ПодразделениеОрганизации = ОсновноеПодразделениеОрганизации(Организация);
	...
	Если Не УспешнаяПроверкаДублейНачальныхОстатков(Организация, ПодразделениеОрганизации, РазделУчета) Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", Организация);
	ДанныеЗаполнения.Вставить("ПодразделениеОрганизации", ПодразделениеОрганизации);
	Документ.Заполнить(ДанныеЗаполнения);
	Документ.Дата = ДатаВводаОстатков;
	...
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		...
		Если НайденнаяНоменклатура <> Неопределено Тогда
			...
			Если ДокументПартии = Неопределено Тогда
				...
				НоваяПартия.Дата = ДатаВводаОстатков;
				НоваяПартия.Записать();
				ДокументПартии = НоваяПартия.Ссылка;
			КонецЕсли;
			НоваяСтрока.Склад = НайтиСоздатьОсновнойСклад();
...
Процедура СоздатьВводНачальныхОстатковИзКаталогаНоменклатуры(НоваяНоменклатура, ЗагружаемыеДанные, Ошибки)
	...
	ЕстьСумма = НоваяНоменклатура.Колонки.Найти("Остаток") <> Неопределено;
	ЕстьКоличество = НоваяНоменклатура.Колонки.Найти("Количество") <> Неопределено;
	ПартииДляЗаписи = Новый Массив;
	Склад = НайтиСоздатьОсновнойСклад();
	
	Для Каждого Позиция Из НоваяНоменклатура Цикл
		Если Не ЗначениеЗаполнено(Позиция.Ссылка)
			Или (Не ЕстьСумма Или Не ЗначениеЗаполнено(Позиция.Остаток))
			И (Не ЕстьКоличество Или Не ЗначениеЗаполнено(Позиция.Количество)) Тогда
			Продолжить;
		КонецЕсли;
		Строка = Документ.НоменклатураНаСкладе.Добавить();
		Строка.Номенклатура = Позиция.Ссылка;
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(
			Организация, Строка.Номенклатура);
		ЗаполнитьЗначенияСвойств(Строка, СчетаУчета);
		Если ЕстьСумма Тогда
			Строка.Сумма = Позиция.Остаток;
		КонецЕсли;
		Если ЕстьКоличество Тогда
			Строка.Количество = Позиция.Количество;
		КонецЕсли;
		Строка.ВалютнаяСумма = Строка.Сумма;
		Строка.СуммаНУ = Строка.Сумма;
		Строка.Склад = Склад;
		ДокументПартии = Документы.ДокументРасчетовСКонтрагентом.СоздатьДокумент();
		ДокументПартии.Организация = Организация;
		ДокументПартии.Дата = ДатаВводаОстатков;
		ДокументПартииСсылка = Документы.ДокументРасчетовСКонтрагентом.ПолучитьСсылку();
		ДокументПартии.УстановитьСсылкуНового(ДокументПартииСсылка);
		Строка.Партия = ДокументПартииСсылка;
		ПартииДляЗаписи.Добавить(ДокументПартии);
		Строка.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются;
		Строка.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеСписано;
	КонецЦикла;
	
	Если Не Документ.НоменклатураНаСкладе.Количество() Тогда
		Возврат;
	КонецЕсли;
	...
	НачатьТранзакцию();
	Попытка
		Для Каждого ДокументПартии Из ПартииДляЗаписи Цикл
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстОшибки = НСтр("ru = 'Не удалось записать документ ""Ввод начальных остатков"" для раздела учета ""Товары"" по причине:'");
		ДобавитьОшибкуПриИсключении(Ошибки, ТекстОшибки, Метаданные.Документы.ВводНачальныхОстатков, ИнформацияОбОшибке());
		Возврат;
	КонецПопытки;
	СтрокаДокументКПроведению = ЗагружаемыеДанные.ДокументыКПроведению.Добавить();
	СтрокаДокументКПроведению.Дата = Документ.Дата;
...
Процедура ЗагрузитьОстаткиПоРасчетамСКонтрагентами(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	...
	Если Счет = "60" Тогда
		РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСПоставщикамиИПодрядчиками;
	...
	ПроверитьУстановитьЗаписьУчетнойПолитики(Организация, ДатаВводаОстатков);
	ИспользоватьДоговоры = ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам");
	...
	ЕстьИНН = ЗагружаемыеДанные.Свойство(ИмяОборотыИСальдо());
	
	НовыеКонтрагенты = НовыеРеквизитыКонтрагента();
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		НовыйКонтрагент = НовыеКонтрагенты.Добавить();
		НовыйКонтрагент.Наименование = Строка.Аналитика;
		Если ЕстьИНН И ЗначениеЗаполнено(Строка.ИНН_КПП) Тогда
			НайденныеЧисла = СтрНайтиПоРегулярномуВыражению(Строка.ИНН_КПП, "(\d{10,12})(?:\s+(\d{9}))?");
			ИНН_КПП = НайденныеЧисла.ПолучитьГруппы();
			Если Не ЗначениеЗаполнено(ИНН_КПП) Тогда
				Продолжить;
			КонецЕсли;
			НовыйКонтрагент.ИНН = ИНН_КПП[0].Значение;
			Строка.ИНН = НовыйКонтрагент.ИНН;
			Если ИНН_КПП.Количество() > 1 Тогда
				НовыйКонтрагент.КПП = ИНН_КПП[1].Значение;
				Строка.КПП = НовыйКонтрагент.КПП;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если ЗначениеЗаполнено(НовыеКонтрагенты) Тогда
		НайтиСоздатьКонтрагентов(НовыеКонтрагенты, Ошибки);
	КонецЕсли;
	...
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		Для Каждого Элемент Из ИменаСуммовыхРеквизитов Цикл
			...
			НоваяСтрока.Контрагент = НайтиСозданногоКонтрагента(НовыеКонтрагенты, Строка, ЕстьИНН);
			НоваяСтрока.СчетУчета = Элемент.Значение;
			Если Элемент.Ключ = "ДебетСумма" Тогда
				НоваяСтрока.Сумма = Строка[Элемент.Ключ];
			ИначеЕсли Элемент.Ключ = "КредитСумма" Тогда
				НоваяСтрока.СуммаКт = Строка[Элемент.Ключ];
			КонецЕсли;
			НоваяСтрока.ВалютнаяСумма = НоваяСтрока.Сумма;
			НоваяСтрока.СуммаНУ = НоваяСтрока.Сумма;
			...
			ДокументРасчета.Дата = ДатаВводаОстатков;
			ДокументРасчета.СуммаДокумента = НоваяСтрока.Сумма;
			Если ИспользоватьДоговоры Тогда
				НоваяСтрока.ДоговорКонтрагента = РаботаСДоговорамиКонтрагентовБП.ДоговорКонтрагентаПоВидамДоговоров(
					Организация, НоваяСтрока.Контрагент, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВидДоговора));
				Если Не ЗначениеЗаполнено(НоваяСтрока.ДоговорКонтрагента) Тогда
					ЗначенияЗаполнения = Новый Структура;
					ЗначенияЗаполнения.Вставить("Организация", Организация);
					ЗначенияЗаполнения.Вставить("Владелец", НоваяСтрока.Контрагент);
...
Процедура ЗагрузитьОстаткиПоРасчетнымСчетам(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	...
	Организация = ЗагружаемыеДанные.Организация;
	Подразделение = ОсновноеПодразделениеОрганизации(Организация);
	РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ДенежныеСредства;
	...
	ДанныеЗаполнения.Вставить("ПодразделениеОрганизации", Подразделение);
	НовыйДокумент.Заполнить(ДанныеЗаполнения);
	НовыйДокумент.Дата = ДатаВводаОстатков(Организация);
	НовыйДокумент.РазделУчета = РазделУчета;
	...
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		...
		РасчетныйСчет.ВидСчета = "Расчетный";
		РасчетныйСчет.ВалютаДенежныхСредств = ВалютаРегламентированногоУчета;
		РасчетныйСчет.ОбменДанными.Загрузка = Истина;
		Попытка
			РасчетныйСчет.Записать();
		Исключение
			ШаблонОшибки = НСтр("ru = 'Не удалось добавить расчетный счет %1 по причине:'");
			ТекстОшибки = СтрШаблон(ШаблонОшибки, Строка.Аналитика);
			ДобавитьОшибкуПриИсключении(Ошибки, ТекстОшибки, Метаданные.Справочники.БанковскиеСчета, ИнформацияОбОшибке());
			Возврат;
		КонецПопытки;
...
Процедура ДобавитьОшибкуПриИсключении(Ошибки, ТекстОшибки, ОбъектМетаданных, ИнформацияОбОшибке)
	
	ПолныйТекстОшибки = ТекстОшибки + Символы.ПС + ОбработкаОшибок.СообщениеОбОшибкеДляПользователя(ИнформацияОбОшибке);
	ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "", ПолныйТекстОшибки, "");
	ЗаписьЖурналаРегистрации(ЗагрузкаДанныхИзВнешнихФайлов.СобытиеЖурналаРегистрации(),
...
Функция УспешнаяПроверкаДублейНачальныхОстатков(Организация, Подразделение, РазделУчета)
	...
	Запрос.Текст = 
		...
		|	И ВводНачальныхОстатков.ПодразделениеОрганизации = &ПодразделениеОрганизации
	...
	РезультатЗапроса = Запрос.Выполнить();
	ДублиОтсутствуют = РезультатЗапроса.Пустой();
	
	Если Не ДублиОтсутствуют Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		ТекстСообщения = СтрШаблон(НСтр(
			"ru = 'Для организации ""%1"" уже существует документ ввода начальных остатков в разделе ""%2""'"),
			Организация,
			РазделУчета);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Выборка.Ссылка);
	КонецЕсли;
	
	Возврат ДублиОтсутствуют;
...
Функция ПрочитатьОборотноСальдовуюВедомость(СоставДанных, Таблица)
	...
	НайденныеЦифры = СтрНайтиПоРегулярномуВыражению(РеквизитыШапкиВедомости.Счет, "^\D*(\d+)");
	ГруппыЦифр = НайденныеЦифры.ПолучитьГруппы();
	Если ЗначениеЗаполнено(ГруппыЦифр) Тогда
		РеквизитыШапкиВедомости.Счет = ГруппыЦифр[0].Значение;
	Иначе
		Возврат ТаблицаДанных;
	КонецЕсли;
...
Функция ПрочитатьОборотыИСальдо(СоставДанных, Таблица)
	
	ТаблицаДанных = НоваяТаблицаДанных(СоставДанных);
	РеквизитыШапкиВедомости = РеквизитыШапкиОборотыИСальдо(Таблица);
	СтрокаАналитика = СоставДанных.Найти("Аналитика", ИмяРеквизита1С());
	Если СтрокаАналитика = Неопределено Тогда
		Возврат ТаблицаДанных;
	Иначе
		// В данном формате файла колонка аналитики не имеет заголовка. Исходим из того, что аналитика отражается
		// в первой колонке
		СтрокаАналитика.Значение = 1;
	КонецЕсли;
	ТекстКонечныхОстатков = "сальдоконечное";
	МаксСтрока = Таблица.ПолучитьРазмерОбластиДанныхПоВертикали();
	МаксКолонка = Таблица.ПолучитьРазмерОбластиДанныхПоГоризонтали();
	ПоложениеРеквизитовОбнаружено = Ложь;
	Для НомерСтроки = РеквизитыШапкиВедомости.НомерСтроки По МаксСтрока Цикл
		Для НомерКолонки = 1 По МаксКолонка Цикл
			ТекстЯчейки = Таблица.Область(НомерСтроки, НомерКолонки).Текст;
			Если Не ЗначениеЗаполнено(ТекстЯчейки) Тогда
				Продолжить;
			...
			ТекстЯчейкиФорматированный = НРег(ТекстЯчейкиФорматированный);
			Если СтрЧислоВхождений(ТекстЯчейкиФорматированный, ТекстКонечныхОстатков) > 0 Тогда
				ЗаполнитьЗначениеВНайденнойСтроке(СоставДанных, "ДебетСумма", ИмяРеквизита1С(), НомерКолонки);
				ЗаполнитьЗначениеВНайденнойСтроке(СоставДанных, "КредитСумма", ИмяРеквизита1С(), НомерКолонки + 1);
				ПоложениеРеквизитовОбнаружено = Истина;
				Прервать;
			Иначе
				СтрокаРеквизит = СоставДанных.Найти(ТекстЯчейкиФорматированный, ИмяРеквизитаВФайле());
				Если СтрокаРеквизит <> Неопределено Тогда
					СтрокаРеквизит.Значение = НомерКолонки;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если ПоложениеРеквизитовОбнаружено Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	ПредставлениеФайла = СтрШаблон(НСтр("ru = 'Оборотно-сальдовая ведомость по счету %1'"), РеквизитыШапкиВедомости.Счет);
	ЗаполнитьТаблицуДанных(
		ТаблицаДанных, СоставДанных, Таблица, НомерСтроки + 2, "", Неопределено, Ложь,
		ЗначенияЗаполнения, ПредставлениеФайла);
	
	СтрокиКУдалению = Новый Массив;
	Для Каждого Строка Из ТаблицаДанных Цикл
		Если Не ЗначениеЗаполнено(Строка.ДебетСумма) И Не ЗначениеЗаполнено(Строка.КредитСумма) Тогда
			СтрокиКУдалению.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьТаблицуУровнямиГруппировок(Таблица, ТабличныйДокумент)
	
	УровниГруппировок = УровниГруппировкиСтрок(ТабличныйДокумент);
	Для Каждого Строка Из Таблица Цикл
		Строка[ИмяКолонкиУровеньГруппировки()] = УровниГруппировок.Получить(Строка[ИмяКолонкиНомерСтроки()]);
	КонецЦикла;
	
КонецПроцедуры

Функция УровниГруппировкиСтрок(Таблица)
	
	УровниГруппировок = Новый Соответствие;
	...
	ПрочитанныеДанные = ПостроительДОМ.Прочитать(ЧтениеХМЛ);
	УзлыУровней = ПрочитанныеДанные.ПолучитьЭлементыПоИмени("vg");
	Для Индекс = 0 По УзлыУровней.Количество() - 1 Цикл
		УзелГруппы = УзлыУровней[Индекс];
		НачалоГруппировки = 0;
		КонецГруппировки = 0;
		Для Сч = 0 По УзелГруппы.ДочерниеУзлы.Количество() - 1 Цикл
			ВложенныйУзел = УзелГруппы.ДочерниеУзлы[Сч];
			Если ВложенныйУзел.ИмяУзла = "b" Тогда
				НачалоГруппировки = Число(ВложенныйУзел.ТекстовоеСодержимое);
			ИначеЕсли ВложенныйУзел.ИмяУзла = "e" Тогда
				КонецГруппировки = Число(ВложенныйУзел.ТекстовоеСодержимое);
			КонецЕсли;
		КонецЦикла;
		...
		Для Номер = НачалоГруппировки По КонецГруппировки Цикл
			НайденноеЗначение = УровниГруппировок.Получить(Номер);
			Если НайденноеЗначение = Неопределено Тогда
				УровниГруппировок.Вставить(Номер, РодительскийУровень + 1);
				Если Номер = НачалоГруппировки Тогда
					РодительскийУровень = 1;
				КонецЕсли;
			Иначе
				Если Номер = НачалоГруппировки Тогда
					РодительскийУровень = НайденноеЗначение;
				Иначе
					УровниГруппировок.Вставить(Номер, РодительскийУровень + 1);
				КонецЕсли;
			КонецЕсли;
...
Функция ВидыОперацийДокументов()
	...
	ВидыОпераций.Вставить("входящие_документы_акт выполненных услуг", Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Услуги);
	...
	ВидыОпераций.Вставить("исходящие_документы_акт выполненных услуг", Перечисления.ВидыОперацийРеализацияТоваров.Услуги);
	...
	ВидыОпераций.Вставить("документы_продажа_счет на оплату", Перечисления.ВидыОперацийСчетаПокупателю.ТоварыИУслуги);
...
Функция ИмяТабличнойЧасти(ТипДокумента)
	...
	Если ТипДокумента <> "Счёт-фактура" Тогда
		ЭтоДокументСУслугами = ТипыДокументовСВидомУслуги().Найти(НРег(ТипДокумента)) <> Неопределено;
		ИмяТабЧасти = ?(ЭтоДокументСУслугами, "Услуги", "Товары");
	КонецЕсли;
...
Процедура ЗаполнитьСоответствияРеквизитовДокументовПокупкиПродажи(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Цена", "цена", Тип("Число"));
...
Процедура ЗаполнитьСоответствияРеквизитовОборотноСальдоваяВедомость(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "товар, склад");
...
Процедура ЗаполнитьСоответствияРеквизитовОборотыИСальдо(ТаблицаРеквизитов)
	
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДебетСумма", "дебет", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "КредитСумма", "кредит", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипДокумента");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Дата");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Счет");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ИНН_КПП", "иннкпп");
	// Колонки заполняемые в ходе загрузки:
	ДобавитьСоответствие(ТаблицаРеквизитов, "ИНН");
	ДобавитьСоответствие(ТаблицаРеквизитов, "КПП");
...
Процедура ЗаполнитьСоответствияРеквизитовРеестрИмущества(ТаблицаРеквизитов)
	
	ДобавитьСоответствие(ТаблицаРеквизитов, "Наименование", "наименование");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Количество", "количество");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Остаток", "остаток");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ОстатокНУ", "остаток ну");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Стоимость", "стоимость");
	ДобавитьСоответствие(ТаблицаРеквизитов, "СтоимостьНУ", "стоимость ну");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Амортизация", "амортизация");
	ДобавитьСоответствие(ТаблицаРеквизитов, "АмортизацияНУ", "амортизация ну");
	ДобавитьСоответствие(ТаблицаРеквизитов, ИмяКолонкиНомерСтроки());
	// Свойства, заполняемые в момент загрузки
	ДобавитьСоответствие(ТаблицаРеквизитов, ИмяКолонкиУровеньГруппировки());
	ДобавитьСоответствие(ТаблицаРеквизитов, "Родитель");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Ссылка");
...
Функция ТипыДокументовСВидомУслуги()
	...
	ИменаТипов.Добавить("акт выполненных услуг");
	Возврат ИменаТипов;
...
Процедура ЗаполнитьСоответствияРеквизитовНоменклатуры(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "ЕдиницаИзмерения",         "единицы измерения");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ЦенаПродажи",              "цена продажи", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ЦенаПродажи",              "цена", Тип("Число"));
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Количество",               "остаток");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Остаток",                  "сумма остатка");
	ДобавитьСоответствие(ТаблицаРеквизитов, ИмяКолонкиНомерСтроки());
...
Процедура ЗагрузитьНоменклатуру(ИмяРаздела, ПрочитанныеДанные, Ошибки)
	...
	Если ИмяРаздела = ИмяНоменклатура() Тогда
		ГруппыНоменклатуры = ДополнитьСправочникГруппамиСУчетомУровнейВложенности(
			СуществующаяНоменклатура, ИмяНоменклатура(), ИмяКолонкиИдентифицирующейНоменклатуру());
		...
		Если Выборка.НайтиСледующий(Наименование, "Наименование") Тогда
			...
			Если ИзменитьОбъект Тогда
				...
				Исключение
					...
					ТекстОшибки = СтрШаблон(ШаблонОшибки, Наименование);
					ДобавитьОшибкуПриИсключении(Ошибки, ТекстОшибки, Метаданные.Справочники.Номенклатура, ИнформацияОбОшибке());
				КонецПопытки;
			КонецЕсли;
		Иначе
	...
	Если ИмяРаздела = ИмяНоменклатура() Тогда
		Если НоваяНоменклатура.Колонки.Найти("Количество") <> Неопределено
			Или НоваяНоменклатура.Колонки.Найти("Остаток") <> Неопределено Тогда
			СоздатьВводНачальныхОстатковИзКаталогаНоменклатуры(НоваяНоменклатура, ПрочитанныеДанные, Ошибки);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура СоздатьНоменклатуру(СтрокаНоменклатура, ГруппыНоменклатуры, ПараметрыСоздания, Ошибки)
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(ШаблонОшибки, СтрокаНоменклатура[РеквизитНаименование]);
		ДобавитьОшибкуПриИсключении(Ошибки, ТекстОшибки, Метаданные.Справочники.Номенклатура, ИнформацияОбОшибке());
	КонецПопытки;
	
КонецПроцедуры

// Создаёт группы в справочнике (имя справочника передаётся через параметр), с учётом их возможной вложенности
// друг в друга, заполняет колонку "Родитель" в переданной таблице загружаемых данных. Возвращает таблицу,
// перечисляющую группы справочника, существующие в базе данных. Используется, если файл с данными обозначает
// группы через вложенность строк (существуют и другие форматы,
// например, когда название группы указывается в отдельной колонке).
//
// Параметры:
//  ЗагружаемыеДанные - ТаблицаЗначений - состав колонок определяется в методах, соответствующих переданному справочнику,
//    например, ЗаполнитьСоответствияРеквизитовНоменклатуры для справочника Номенклатура,
//    ЗаполнитьСоответствияРеквизитовРеестрИмущества для справочника ОсновныеСредства
//  ИмяТаблицы - Строка - имя загружаемого справочника
//  ИмяИдентифицирующейКолонки - Строка - имя колонки, в которой передается наименование элемента справочника, зависит
//    от формата загружаемого файла
//
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Наименование - Строка - наименование номенклатурной группы
//   * Ссылка - СправочникСсылка.Номенклатура - ссылка на номенклатурную группу
//
Функция ДополнитьСправочникГруппамиСУчетомУровнейВложенности(ЗагружаемыеДанные, ИмяТаблицы, ИмяИдентифицирующейКолонки)
	
	СтрокаСЗаполненнымУровнемГруппировки = ЗагружаемыеДанные.Найти(1, ИмяКолонкиУровеньГруппировки());
	Если СтрокаСЗаполненнымУровнемГруппировки = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	СхемаЗапроса = Новый СхемаЗапроса;
	Оператор = СхемаЗапроса.ПакетЗапросов[0].Операторы[0];
	ПолноеИмя = СтрШаблон("Справочник.%1", ИмяТаблицы);
	Оператор.Источники.Добавить(ПолноеИмя);
	Оператор.ВыбираемыеПоля.Добавить(СтрШаблон("%1.Наименование", ИмяТаблицы), 0);
	Оператор.ВыбираемыеПоля.Добавить(СтрШаблон("%1.Ссылка", ИмяТаблицы), 1);
	СхемаЗапроса.ПакетЗапросов[0].Колонки[0].Псевдоним = "Наименование";
	СхемаЗапроса.ПакетЗапросов[0].Колонки[1].Псевдоним = "Ссылка";
	Оператор.Отбор.Добавить(СтрШаблон("%1.ЭтоГруппа", ИмяТаблицы));
	Оператор.Отбор.Добавить(СтрШаблон("НЕ %1.ПометкаУдаления", ИмяТаблицы));
	
	Запрос = Новый Запрос;
	Запрос.Текст = СхемаЗапроса.ПолучитьТекстЗапроса();
	
	СуществующиеГруппы = Запрос.Выполнить().Выгрузить();
	
	ЗагружаемыеДанные.Сортировать(ИмяКолонкиНомерСтроки());
	ПредыдущийУровень = 0;
	СтекГрупп = Новый Соответствие;
	СтрокиКУдалению = Новый Массив;
	Для Индекс = 0 По ЗагружаемыеДанные.Количество() - 1 Цикл
		Строка = ЗагружаемыеДанные[Индекс];
		ТекущийУровень = Строка[ИмяКолонкиУровеньГруппировки()];
		Если ТекущийУровень = 0 Или ТекущийУровень = Неопределено Тогда
			// Позиция из корневой ветки справочника, не входит ни в какую группу
			СтекГрупп.Очистить();
			Продолжить;
		ИначеЕсли ТекущийУровень = 1 Тогда
			// Группа, которая расположена в корневой ветке справочника
			СтекГрупп.Очистить();
			Наименование = Строка[ИмяИдентифицирующейКолонки];
			СозданнаяГруппа = СоздатьГруппуСправочникаЕслиНеНайдена(ИмяТаблицы, Наименование, СуществующиеГруппы);
			СтекГрупп.Вставить(1, СозданнаяГруппа);
			ПредыдущийУровень = 1;
			СтрокиКУдалению.Добавить(Строка);
		ИначеЕсли ТекущийУровень > ПредыдущийУровень Тогда
			// Предыдущая строка обозначала группу, поскольку текущая строка является вложенной в неё
			Если ПредыдущийУровень <> 1 Тогда // для уровня 1 группа создаётся сразу, без проверки последующих строк
				ПредыдущаяСтрока = ЗагружаемыеДанные[Индекс - 1];
				СозданнаяГруппа = СоздатьГруппуСправочникаЕслиНеНайдена(ИмяТаблицы, ПредыдущаяСтрока[ИмяИдентифицирующейКолонки],
					СуществующиеГруппы, СтекГрупп.Получить(ПредыдущийУровень - 1));
				СтекГрупп.Вставить(ПредыдущийУровень, СозданнаяГруппа);
				СтрокиКУдалению.Добавить(ПредыдущаяСтрока);
			КонецЕсли;
			Строка.Родитель = СтекГрупп.Получить(ПредыдущийУровень).Наименование;
			ПредыдущийУровень = ТекущийУровень;
		ИначеЕсли ТекущийУровень = ПредыдущийУровень Тогда
			Строка.Родитель = СтекГрупп.Получить(ПредыдущийУровень - 1).Наименование;
		Иначе
			// Текущая строка имеет уровень вложенности меньший, чем у предыдущей строки. Это означает, что
			// закончилось перечисление позиций, расположенных в группе
			СтекГрупп.Удалить(ТекущийУровень);
			ПредыдущийУровень = ТекущийУровень;
			Строка.Родитель = СтекГрупп.Получить(ТекущийУровень - 1).Наименование;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого УдаляемаяСтрока Из СтрокиКУдалению Цикл
		// Группы уже созданы, повторная обработка строк с группами не требуется
		ЗагружаемыеДанные.Удалить(УдаляемаяСтрока);
	КонецЦикла;
	
	Возврат СуществующиеГруппы;
	
КонецФункции

Функция СоздатьГруппуСправочникаЕслиНеНайдена(ИмяСправочника, Наименование, СуществующиеГруппы, Родитель = Неопределено)
	
	НайденнаяГруппа = СуществующиеГруппы.Найти(Наименование, "Наименование");
	Если НайденнаяГруппа = Неопределено Тогда
		НоваяГруппа = Справочники[ИмяСправочника].СоздатьГруппу();
		НоваяГруппа.ОбменДанными.Загрузка = Истина;
		НоваяГруппа.Наименование = Наименование;
		Если Родитель <> Неопределено Тогда
			НоваяГруппа.Родитель = Родитель.Ссылка;
		КонецЕсли;
		НоваяГруппа.Записать();
		СтрокаГруппы = СуществующиеГруппы.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаГруппы, НоваяГруппа);
		НайденнаяГруппа = СтрокаГруппы;
	КонецЕсли;
...
Функция ИмяКолонкиУровеньГруппировки()
	
	Возврат "УровеньГруппировки";
	
КонецФункции

Функция ИмяКолонкиНомерСтроки()
...
Функция ИмяРеестрИмущества()
	
	Возврат "РеестрИмущества";
...
Функция ИмяОборотыИСальдо()
	
	Возврат "ОборотыИСальдо";
...
Функция ПризнакЗаголовкаОборотыИСальдо()
...
Процедура ДобавитьСтрокуВОписаниеФайлов(Описание, Имя, Заголовок, Представление = "", ПриоритетноеЗначение = Истина)
	...
	НоваяСтрока.ПриоритетноеЗначение = ПриоритетноеЗначение;
...
Функция РеквизитыШапкиОборотыИСальдо(Таблица)
	...
	Для НомерСтроки = 1 По ГраницаСтрока Цикл
		Если ЗначениеЗаполнено(Результат.НомерСтроки) Тогда
			Прервать;
		КонецЕсли;
		Для НомерКолонки = 1 По ГраницаКолонка Цикл
			ТекстЯчейки = СокрЛП(Таблица.Область(НомерСтроки, НомерКолонки).Текст);
			...
			Если Не ЗначениеЗаполнено(ТекстЯчейки) Тогда
				Продолжить;
			КонецЕсли;
			Если СтрНачинаетсяС(ТекстЯчейки, ПризнакЗаголовкаОборотыИСальдо()) Тогда
				ТекстЯчейки = СокрЛП(Таблица.Область(НомерСтроки + 1, НомерКолонки).Текст);
				РезультатПоиска = СтрНайтиПоРегулярномуВыражению(
					ТекстЯчейки, "\d{2}\.\d{2}\.\d{2,4}\s*-\s*(\d{2}\.\d{2}\.\d{2,4})");
				НайденныеДаты = РезультатПоиска.ПолучитьГруппы();
				Если Не ЗначениеЗаполнено(НайденныеДаты) Тогда
					Возврат Результат;
				КонецЕсли;
				ОкончаниеПериода = НайденныеДаты[0].Значение;
				ЧастиПериода = СтрРазделить(ОкончаниеПериода, ".");
				ГодИзОкончанияПериода = ЧастиПериода[ЧастиПериода.ВГраница()];
				Если СтрДлина(ГодИзОкончанияПериода) = 2 Тогда
					ИсходноеВыражение = "(\d*)(\d{2})$";
					ОтредактированноеВыражение = "$120$2";
					ОкончаниеПериода = СтрЗаменитьПоРегулярномуВыражению(
						ОкончаниеПериода, ИсходноеВыражение, ОтредактированноеВыражение);
				КонецЕсли;
				Результат.Дата = ОбщегоНазначенияКлиентСервер.СтрокаВДату(ОкончаниеПериода);
			ИначеЕсли СтрНачинаетсяС(ТекстЯчейки, "счет:") Тогда
				РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ТекстЯчейки, "(?<=\D|^)(\d+)(?=\D|$)");
				НайденныеЧисла = РезультатПоиска.ПолучитьГруппы();
				Если Не ЗначениеЗаполнено(НайденныеЧисла) Тогда
					Возврат Результат;
				КонецЕсли;
				Результат.Счет = НайденныеЧисла[0].Значение;
				Результат.НомерСтроки = НомерСтроки + 2;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ЗагрузитьОстаткиПоОсновнымСредствам(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	...
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		...
		СтрокаДокумента.ТекущаяСтоимостьБУ = СтрокаДокумента.ПервоначальнаяСтоимостьБУ;
		...
		СтрокаДокумента.ДатаПринятияКУчетуРегл = НачалоГода(НачалоГода(НовыйДокумент.Дата) - 1);
		СтрокаДокумента.НазваниеДокументаПринятияКУчетуРегл = НСтр("ru = 'Поступление основных средств'");
		СтрокаДокумента.НомерДокументаПринятияКУчетуРегл = НомерДокументаПринятияКУчету;
		НомерДокументаПринятияКУчету = НомерДокументаПринятияКУчету + 1;
		СтрокаДокумента.СчетУчетаБУ = ПланыСчетов.Хозрасчетный.ОСвОрганизации;
...
Функция СпособНачисленияАмортизации()
...
Функция ДатаВводаОстатков(Организация)
	...
	Возврат НачалоГода(ТекущаяДата()) - 1;
	
КонецФункции

Функция НайтиСозданногоКонтрагента(Контрагенты, Строка, ЕстьИНН)
	
	Отбор = Новый Структура;
	Если ЗначениеЗаполнено(Строка.Аналитика) Тогда
		Отбор.Вставить("Наименование", Строка.Аналитика);
	КонецЕсли;
	Если ЕстьИНН И ЗначениеЗаполнено(Строка.ИНН) Тогда
		Отбор.Вставить("ИНН", Строка.ИНН);
	КонецЕсли;
	Если ЕстьИНН И ЗначениеЗаполнено(Строка.КПП) Тогда
		Отбор.Вставить("КПП", Строка.КПП);
	КонецЕсли;
	
	Найденное = Контрагенты.НайтиСтроки(Отбор);
	Если ЗначениеЗаполнено(Найденное) Тогда
		Возврат Найденное[0].Ссылка;
	Иначе
