...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	СтрокаФильтра = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы выгрузки (*.%1)|*.%1|Все файлы(%2)|%2'"), "xml", ПолучитьМаскуВсеФайлы());
...
Процедура УдалитьВременныйФайл(ИмяФайла)
	...
	Если НЕ ЭтоИмяВременногоФайла(ИмяФайла) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			"ИмяФайла", "КонсолидацияОтчетностиПоНДС.УдалитьВременныйФайл", ИмяФайла);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЭтоИмяВременногоФайла(ИмяФайла)
	
	// Ожидается, что имя файла получено методом ПолучитьИмяВременногоФайла().
	Возврат СтрНачинаетсяС(СтрЗаменить(ИмяФайла, "/", "\"), СтрЗаменить(КаталогВременныхФайлов(), "/", "\"));
...
Функция КонсолидироватьНаСервере(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено) Экспорт
	...
	Для Каждого ДанныеФайлаВыгрузки Из ФайлыВыгрузки Цикл
		...
		Если ЗначениеЗаполнено(ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере)
		И НЕ ЭтоИмяВременногоФайла(ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверное значение параметра %1 в %2:
					|Файл не является временным ""%3"".'"), 
				"ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере)",
				"КонсолидацияОтчетностиПоНДС.КонсолидироватьНаСервере",
				ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере);
		КонецЕсли;
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок <> Неопределено
			...
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Версия файла выгрузки ""%1"" пока не поддерживается'"), ПараметрыВыгрузки.ВерсФорм));
			Возврат Ложь;
		КонецЕсли;
...
Функция КомпоновщикXML_СсылкаНаВременноеХранилище(КомпоновщикXML)
	...
	Если НЕ ЭтоИмяВременногоФайла(КомпоновщикXML.ИмяФайла) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение параметра %1 в %2:
				|Файл не является временным ""%3"".'"), 
			"КомпоновщикXML.ИмяФайла", "КонсолидацияОтчетностиПоНДС.КомпоновщикXML_СсылкаНаВременноеХранилище",
			КомпоновщикXML.ИмяФайла);
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела8(СтрокаРаздела8, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		...
		Для Каждого НомерДатаЧастСчФ Из СтрокаРаздела8.СвЧастСчФ Цикл
			...
			НомДатаЧастСчФ = ?(ЗначениеЗаполнено(НомерЧастСчФ), "" + НомерЧастСчФ + НСтр("ru=' от '"), "") + ДатаЧастСчФ;
