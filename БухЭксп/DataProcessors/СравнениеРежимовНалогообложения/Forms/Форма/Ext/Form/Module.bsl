...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазместитьНавигацию(СтраницаПоУмолчанию());
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	СравнитьРежимыНалогообложения(ЭтотОбъект);
	Если ЭтоПомощникУтратыПраваНПД(РежимРаботыПомощника) Тогда
		РазместитьНавигацию(НавигацияПараметрФормы);
	КонецЕсли;
...
Процедура КомандаНазад(Команда)
	...
	ИначеЕсли РежимРаботыПомощника = ВозможныеРежимы.УтратаПраваНПД Тогда
		...
		Если Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаЗаявление Тогда
			ИмяТекущейСтраницы = СтраницаПоУмолчанию();
...
Процедура УправлениеФормойПриСозданииНаСервере()
	...
	Если ЭтоПомощникУтратыПраваНПД(РежимРаботыПомощника) Тогда
		...
		Элементы.ПереходНаДругиеРежимыШаг1Пояснение.Заголовок = ТекстПереходаВПриложение;
...
Процедура УправлениеФормой(Форма)
	...
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаОсновная Тогда
		...
		Если ЭтоПомощникУтратыПраваНПД(Форма.РежимРаботыПомощника) Тогда
			Элементы.КомандаНазад.Видимость = (Форма.НавигацияПараметрФормы <> СтраницаПоУмолчанию());
		Иначе
	...
	Иначе
		...
		Если ЭтоПомощникУтратыПраваНПД(Форма.РежимРаботыПомощника) Тогда
			...
			НужноЗаявление = НалогНаПрофессиональныйДоходКлиентСервер.ПодаватьЗаявлениеПриУтратеПраваНПД(Форма.ВыбранныйРежим);
			
			Если НужноЗаявление Тогда
				Элементы.ГруппаЗаявлениеУСН.Видимость = Истина;
				Элементы.ГруппаДругиеРежимы.Видимость = Ложь;
			Иначе
				Элементы.ГруппаЗаявлениеУСН.Видимость = Ложь;
				Элементы.ГруппаДругиеРежимы.Видимость = Истина;
				Элементы.ДругиеРежимыЗаголовок.Заголовок = ТекстЗаголовокПереходаНаДругиеРежимы(Форма.ВыбранныйРежим);
				Если ЭтоОбщийРежим Тогда
					Элементы.ПереходНаДругиеРежимыШаг3Картинка.Картинка = БиблиотекаКартинок.Нумерация3;
					Элементы.ПереходНаДругиеРежимыШагАУСН.Видимость = Ложь;
				Иначе
					Элементы.ПереходНаДругиеРежимыШаг3Картинка.Картинка = БиблиотекаКартинок.Нумерация4;
					Элементы.ПереходНаДругиеРежимыШагАУСН.Видимость = Истина;
					Элементы.ПереходНаДругиеРежимыШагАУСНПояснение.Заголовок = ТекстПодсказкиПереходНаАУСН();
				КонецЕсли;
			КонецЕсли;
			...
			ИначеЕсли Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаЗаявление Тогда
				Элементы.КомандаЗаявлениеДалее.КнопкаПоУмолчанию = НужноЗаявление;
				Элементы.КомандаЗаявлениеГотово.КнопкаПоУмолчанию = Не НужноЗаявление;
				Элементы.КомандаЗаявлениеДалее.Видимость = НужноЗаявление;
				Элементы.КомандаЗаявлениеГотово.Видимость = Не НужноЗаявление;
			ИначеЕсли Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаДокументы Тогда
...
Процедура РазместитьНавигацию(ИмяТекущейСтраницы)
	...
	Если ЭтоПомощникУтратыПраваНПД(РежимРаботыПомощника) Тогда
		...
		Если НавигацияПараметрФормы = СтраницаПоУмолчанию() Тогда
			ПерейтиНаСтраницу(ЭтотОбъект, "СтраницаОсновная");
...
&НаКлиентеНаСервереБезКонтекста
Функция ТекстЗаголовокПереходаНаДругиеРежимы(ВыбранныйРежим)
	
	Если ВыбранныйРежим = "ОСНОИП" Тогда
		Возврат НСтр("ru = 'Переход на общую систему налогообложения осуществляется с даты утраты права применения спецрежима.'");
	Иначе
		// п.6 ст.15 Федеральный закон от 27.11.2018 N 422-ФЗ
		Возврат НСтр("ru = 'В течение 20 дней после утраты права применения спецрежима Налоговую инспекцию нужно уведомить о переходе на АУСН'");
	КонецЕсли;
	
КонецФункции

&НаСервереБезКонтекста
Функция ТекстПодсказкиПереходНаАУСН()
	
	СсылкаНаЛичныйКабинетНалогоплательщика = Обработки.РегистрацияОрганизации.СсылкаНаЛичныйКабинетНалогоплательщика(Ложь);
	
	Возврат СтроковыеФункции.ФорматированнаяСтрока(СтрШаблон(
		НСтр("ru = 'В течение 20 дней с даты снятия с учета в качестве плательщика Налога на профессиональный доход нужно подать уведомление о переходе на АУСН.
		|Зайдите в <a href=""%1"">Личный кабинет налогоплательщика</a>.
		|В меню выберите пункт для перехода на АУСН и банк из предложенного списка партнеров.'"),
