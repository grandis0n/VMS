...
Процедура СертификатОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипПодписанта = "ПодписатьСертификатом" Тогда
		...
		МобильныйЭДОСлужебныйКлиент.ЗаполнитьСвойстваПоСертификату(ПараметрыКомпонентыРутокен,
			СвойстваСертификатов);
	КонецЕсли;
...
&НаКлиенте
Процедура ПодключитьБиометрию(Команда)

	ПодключитьБиометрию = Не ПодключитьБиометрию;
	МобильныйЭДОСлужебныйКлиент.ПодключитьОтключитьБиометрию(ЭтотОбъект, СвойстваПароля);
	СохранитьНастройкуПодключитьБиометрию(ПодключитьБиометрию);
	Элементы.ФормаПодключитьБиометрию.Пометка = ПодключитьБиометрию;
...
Процедура ПродолжитьОткрытиеЗавершение(Контекст, Результат = Неопределено)
	...
	ПараметрыКомпонентыРутокен.Форма = ЭтотОбъект;
	ПараметрыКомпонентыРутокен.ОписаниеДанных = ОписаниеДанных;
	СвойстваСертификатов = ПолучитьИзВременногоХранилища(СвойстваСертификатовАдрес);
	
	МобильныйЭДОСлужебныйКлиент.ЗаполнитьСвойстваПоСертификату(ПараметрыКомпонентыРутокен,
		СвойстваСертификатов);
	
	ОповещениеПриступитьКПодписаниюПослеЧтенияБиометрии = Новый ОписаниеОповещения("ПриступитьКПодписаниюПослеЧтенияБиометрии", ЭтотОбъект);
	
	МобильныйЭДОСлужебныйКлиент.ЗаполнитьПоДаннымБиометрии(ЭтотОбъект,,,
		ОповещениеПриступитьКПодписаниюПослеЧтенияБиометрии);
...
Асинх Процедура ПодписатьДанныеПослеВыполненияНаСторонеСервера(Результат, Контекст) Экспорт
	...
	Иначе
		...
		МобильныйЭДОКлиент.НачатьОбработкуОперацииРутокен(ПараметрыКомпонентыРутокен,
			"Подписание",
...
&НаКлиенте
Процедура ПриступитьКПодписаниюПослеЧтенияБиометрии(Результат, ДополнительныеПараметры) Экспорт
	...
	СвойстваПароля.Вставить("Значение", Результат);
...
&НаСервереБезКонтекста
Процедура СохранитьНастройкуПодключитьБиометрию(ПодключитьБиометрию)
	МобильныйЭДО.СохранитьНастройкуРутокен("ПодключитьБиометрию", ПодключитьБиометрию);
КонецПроцедуры
