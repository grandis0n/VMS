...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонОшибки = 
		НСтр("ru = 'В итогах документа указана ставка НДС %1, при этом не указано, в какой строке документа эта ставка используется'");
	Для Каждого СтавкаНДС Из ТекущаяИнформацияПодрядчика.Итоги.ОтчетныйПериод.НалогиПоСтавкам Цикл
		Если СтавкаНДС.Ключ = ОсновнаяСтавка Или СтавкаНДС.Ключ = ОсновнаяСтавкаДо2026 Тогда
			Продолжить;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтавкаНДС.Значение.ПозицииАкта) Тогда
			ПараметрыОшибки = ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки();
			ПараметрыОшибки.ТекстОшибки = СтрШаблон(ШаблонОшибки, СтавкаНДС.Ключ);
			ОшибкиЗаполнения.Добавить(ПараметрыОшибки);
		КонецЕсли;
	КонецЦикла;
