...
Процедура ОбработкаОтветаМетода_ДетальныйСтатус(Ответ)
	...
	Если СтатусЗапроса = СтатусыЗапросов.Выполнено Тогда
		...
		ОповеститьОбИзмененииНастроек(СписокИзменений);
	ИначеЕсли СтатусЗапроса = СтатусыЗапросов.Выполняется Тогда
...
Функция ОписаниеИзмененияЗначений(СписокИзменений)
	...
	Для Индекс = 0 По ПоследнийИндекс Цикл
		...
		Если Изменение.Имя = РеквизитыИзСервиса().ДатаПостановкиНаУчет.Имя Тогда
			НаЧтоИзменили = Формат(Изменение.Значение, "ДФ=dd.MM.yyyy");
		Иначе
...
Функция ОбработкаОтветаМетода_ДетальныйСтатусНаСервере(Ответ, Организация, ЭтоНачалоРаботы)
	...
	Если ТипЗнч(Ответ.Результат) = Тип("Структура") Тогда
		...
		ДатаПостановкиНаУчет = ДатаПостановкиНаУчетНалогоплательщика(Организация);
		ДатаПостановкиНаУчетИзСервиса = РеквизитыИзСервиса.ДатаПостановкиНаУчет.Значение;
		Если ЗначениеЗаполнено(ДатаПостановкиНаУчет)
			И ЗначениеЗаполнено(ДатаПостановкиНаУчетИзСервиса)
			И ДатаПостановкиНаУчет < НачалоДня(ДатаПостановкиНаУчетИзСервиса)
			И ДатаПостановкиНаУчетИзСервиса >= НалогНаПрофессиональныйДоходКлиентСервер.ДатаНачалаЭксперимента() Тогда
			...
			Если ЗначениеЗаполнено(НаборЗаписей) Тогда
				...
				ЗаписьНабора.Период = НачалоМесяца(ДатаПостановкиНаУчетИзСервиса);
				ЗаписьНабора.ДатаИзменения = НачалоДня(ДатаПостановкиНаУчетИзСервиса);
			КонецЕсли;
			НаборЗаписей.Записать();
			СписокИзменений.Добавить(РеквизитыИзСервиса.ДатаПостановкиНаУчет);
		КонецЕсли;
	КонецЕсли;
...
Функция РеквизитыИзСервиса()
	...
	ДобавитьРеквизитСервиса(Результат, "ДатаПостановкиНаУчет", НСтр("ru='дата постановки на учет'"));
...
&НаКлиенте
Процедура ОповеститьОбИзмененииНастроек(СписокИзменений)
	
	Для Каждого Реквизит Из СписокИзменений Цикл
		Если Реквизит.Имя = РеквизитыИзСервиса().ДатаПостановкиНаУчет.Имя Тогда
			ПараметрыОповещения = Новый Структура("Организация, Период", Объект.Организация, НачалоМесяца(Реквизит.Значение));
			Оповестить("Запись_НастройкиСистемыНалогообложения", ПараметрыОповещения);
			Оповестить("ИзменениеУчетнойПолитики", Объект.Организация);
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ДатаПостановкиНаУчетНалогоплательщика(Организация)
	...
	Если Выборка.Следующий()
		И Выборка.СистемаНалогообложения = Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход Тогда
