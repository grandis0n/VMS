...
Функция ЭтоЗадачаУСНПереходНаОСНО(КодЗадачи) Экспорт
	
	Возврат КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиДекларацияОрганизацияУтраченоПравоНаУСН()
		Или КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиУплатаОрганизацияУтраченоПравоНаУСН();
...
Функция ОписаниеСледующегоНалогаОтчета(СтрокиИндикатора, ЭтоНалог, КодЗадачи, ЭтоПерваяЗадача, ПараметрыМонитора, Срок = Неопределено)
	...
	ИначеЕсли ЭтоЗадачаУСН(КодЗадачи) Тогда
		...
		Параметр2КакИмя = ЭтоЗадачаУСНПереходНаОСНО(СтрокиИндикатора[0].КодПравила);
...
Процедура СобратьСтатистику()
	...
	КлючНастроек = "ДатаПоследнегоЗапросаМонитораНалоговИОтчетности";
	ИмяСчетчика = "ЗапросМонитораНалоговИОтчетности";
	ТекущаяДатаСеанса = ТекущаяДатаСеанса();
	НачалоТекущегоМесяца = НачалоМесяца(ТекущаяДатаСеанса);
	ДатаПоследнегоЗапросаМонитораНалоговИОтчетности = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		КлючОбъекта, КлючНастроек, '00010101');
	
	Если ДатаПоследнегоЗапросаМонитораНалоговИОтчетности < НачалоТекущегоМесяца Тогда
		СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие(ИмяСчетчика);
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъекта, КлючНастроек, ТекущаяДатаСеанса);
	КонецЕсли;
...
Функция СформироватьТекущиеПредстоящиеЗадачи(Организация, ТекущийДень, ДеятельностьОтсутствует)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭтоЗадачаУСНПереходНаОСНО = ЭтоЗадачаУСНПереходНаОСНО(Выборка.КодПравила);
		...
		Если Выборка.ЕстьТекущаяЗадача Тогда
			...
			Если ЭтоЗадачаУСНПереходНаОСНО Тогда
				НоваяСтрока.ПериодСобытия    = КонецГода(Выборка.ТекЗадачаПериодСобытия);
			Иначе
			...
			Если ЭтоЗадачаУСНПереходНаОСНО И ЗначениеЗаполнено(Выборка.ТекЗадачаПериодСобытия) Тогда
				НоваяСтрока.НачалоПериодаСобытия = НачалоГода(Выборка.ТекЗадачаПериодСобытия);
			КонецЕсли;
		...
		Иначе
			...
			Если ЭтоЗадачаУСНПереходНаОСНО Тогда
				НоваяСтрока.ПериодСобытия    = КонецГода(Выборка.БудЗадачаПериодСобытия);
			Иначе
			...
			Если ЭтоЗадачаУСНПереходНаОСНО Тогда
				НоваяСтрока.НачалоПериодаСобытия = НачалоГода(Выборка.БудЗадачаПериодСобытия);
			КонецЕсли;
...
Функция СокращенноеНаименованиеЗадачи(КодЗадачи, КодПравила, ИмяЗадачи, ОписаниеПравила, НачалоПериода, КонецПериода, КодФНС = Неопределено)
	...
	Если ЭтоЗадачаУСНПереходНаОСНО(КодПравила) Тогда
		Параметр2 = КалендарьБухгалтера.ПредставлениеПериодаСобытия(НачалоГода(НачалоПериода), КонецГода(КонецПериода));
	Иначе	
...
Функция ПолучитьСуммыНалогов(ОбрабатываемыеЗадачи, Организация, ТекущийДень, СчетаУчетаДС, ВыполнитьЗапрос = Истина)
	...
	Для Каждого Задача Из ЗадачиПоПериодам Цикл
		...
		Если Задача.СчетаУчета.Найти(ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате) <> Неопределено
			Или Задача.СчетаУчета.Найти(ПланыСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате) <> Неопределено
