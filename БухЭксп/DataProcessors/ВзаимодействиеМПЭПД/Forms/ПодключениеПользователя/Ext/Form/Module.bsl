...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьДанныеДоверенности(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьДанныеДоверенности(Форма)
	...
	ТолькоМЧД = Форма.ВидЭП <> ПредопределенноеЗначение("Перечисление.ВидыПодписиМПЭПД.УНЭПГосключ");
	
	Форма.Элементы.ГиперссылкаНаДоверенность.Видимость = Не ТолькоМЧД;
	Форма.Элементы.Доверенность.Видимость = ТолькоМЧД;
	
	Если ТолькоМЧД Тогда
		Форма.ЕстьБумажнаяДоверенность = Ложь;
		Форма.БумажнаяДоверенностьНомер = "";
		Форма.БумажнаяДоверенностьДата = Дата(1,1,1);
	Иначе
		ДоверенностьЗаполнена = 
			(ЗначениеЗаполнено(Форма.БумажнаяДоверенностьНомер)
