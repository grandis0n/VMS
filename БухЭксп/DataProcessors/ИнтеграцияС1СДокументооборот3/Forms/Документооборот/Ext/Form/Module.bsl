...
Процедура ОбновитьПредпросмотрСвязи(ОбновитьДанные = Ложь, ИгнорироватьМаксРазмер = Ложь)
	...
	ДеревоСвязейТекущиеДанные = ИнтеграцияС1СДокументооборот3Клиент.ДеревоСвязейТекущиеДанныеДляПросмотра(
		Элементы.ДеревоСвязей);
	Если ДеревоСвязейТекущиеДанные = Неопределено Или ДеревоСвязейТекущиеДанные.ОжиданиеПредпросмотра Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьПредпросмотрСвязиЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат <> Неопределено И Результат.Статус = "Ошибка" Тогда
		...
		HTMLПредпросмотрОбъекта.Вставить("ID", ПараметрыОповещения.СвязанныйОбъектID);
		HTMLПредпросмотрОбъекта.Вставить("Тип", ПараметрыОповещения.СвязанныйОбъектТип);
		HTMLПредпросмотрОбъекта.Вставить("Расширение", ПараметрыОповещения.РасширениеФайла);
		HTMLПредпросмотрОбъекта.Вставить("ПредставлениеHTML", СообщениеHTML);
...
Процедура ОбновитьПредпросмотрФайла(ОбновитьДанные = Ложь, ИгнорироватьМаксРазмер = Ложь)
	...
	ТекущиеДанные = Элементы.ДеревоФайлов.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.ОжиданиеПредпросмотра Тогда
		Возврат;
	КонецЕсли;
	...
	Если Результат <> Неопределено И Результат.Статус = "Ошибка" Тогда
		СообщениеHTML =
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СообщениеВПредпросмотр(
				...
				ЗаголовокСообщенияВОбластиПредпросмотра);
		HTMLПредпросмотрОбъекта = Новый Структура;
		HTMLПредпросмотрОбъекта.Вставить("ID", ПараметрыОповещения.ID);
		HTMLПредпросмотрОбъекта.Вставить("Тип", ПараметрыОповещения.Тип);
		HTMLПредпросмотрОбъекта.Вставить("Расширение", ПараметрыОповещения.Расширение);
		HTMLПредпросмотрОбъекта.Вставить("ПредставлениеHTML", СообщениеHTML);
		HTMLПредпросмотрОбъекта.Вставить("ПредпросмотрУрезан", Ложь);
		HTMLПредпросмотрОбъекта.Вставить("ДвоичныеДанные", Неопределено);
		Оповестить("ПолученHTMLПредпросмотрОбъекта", HTMLПредпросмотрОбъекта, ЭтотОбъект);
