...
Процедура ЗаполнитьОписаниеФункциональности(ОписаниеФункциональности)
	...
	ЗаполнитьОписаниеРазделаОбменСФГИС(ОписаниеФункциональности);
...
Процедура ЗаполнитьОписаниеРазделаОбменСФГИС(ОписаниеФункциональности)
	
	Раздел = "ОбменСФГИС";
	ЭтоВерсияКОРП = ВариантыПриложений.ЭтоВерсияКОРП();
	
	// Обмен с Честный знак
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности,
		Раздел, "ИспользоватьМаркировку",
		НСтр("ru = 'Маркировка (Честный знак)'"),
	...
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности,
		...
		НСтр("ru = 'ЕГАИС'"),
		НСтр("ru = 'Учет объема производства, импорта и оборота алкогольной продукции, имеющей идентификационные коды в системе ""ЕГАИС""'"));
	Функциональность.ДоступнаВПростомИнтерфейсе = Истина;
	Функциональность.ДоступнаНаТарифахБезПолногоИнтерфейса = Ложь;
	...
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности,
		...
		НСтр("ru = 'ВетИС (Меркурий)'"),
		НСтр("ru = 'Ветеринарная сертификация и контроль за перемещением товаров животного происхождения на основе информации в журнале продукции ФГИС ""ВетИС"" (Меркурий)'"));
	Функциональность.ДоступнаВПростомИнтерфейсе = Истина;
	Функциональность.ДоступнаНаТарифахБезПолногоИнтерфейса = Ложь;
	...
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности,
		...
		НСтр("ru = 'Зерно'"),
		НСтр("ru = 'Отражение операций производства, закупки, перевозки, утилизации партий зерна и продуктов его переработки в ФГИС ""Зерно""'"));
	Функциональность.Доступна = ЭтоВерсияКОРП;
	Функциональность.ДоступнаВПростомИнтерфейсе = Истина;
	Функциональность.ДоступнаНаТарифахБезПолногоИнтерфейса = Ложь;
	...
	Функциональность = ДобавитьОписаниеФункциональности(ОписаниеФункциональности,
		...
		НСтр("ru = 'Сатурн'"),
		НСтр("ru = 'Отражение операций производства, закупки, перевозки, утилизации  пестицидов, агрохимикатов, тукосмесей в ФГИС ""Сатурн""'"));
	Функциональность.ДоступнаВПростомИнтерфейсе = Истина;
	Функциональность.ДоступнаНаТарифахБезПолногоИнтерфейса = Ложь;
...
Функция ТекстЗапросаДокументыСУпаковкамиНоменклатуры()
	...
	Возврат УпаковкиВызовСервера.ПолучитьТекстЗапросаПоДокументамСУпаковками(МассивДокументов, Истина);
...
Процедура СформироватьОтчетПоДокументам(Параметры, АдресХранилища) Экспорт
	...
	ИначеЕсли ИдентификаторОтчетаПоДокументам = "ИспользоватьУпаковкиНоменклатуры" Тогда
		СформироватьСписокОбъектовПрепятствующихОтключениюИспользоватьУпаковкиНоменклатуры(Параметры, АдресХранилища);
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюИспользоватьУпаковкиНоменклатуры(Параметры, АдресХранилища)
	
	СтруктураРезультата = Новый Структура;
	СтруктураРезультата.Вставить("Результат", Новый ТабличныйДокумент);
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаДокументыСУпаковкамиНоменклатуры();
	Запрос.УстановитьПараметр("ЕстьУпаковка", Ложь);
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ПараметрыШапки.Вставить("Заголовок", НСтр("ru = 'Документы в которых использованы упаковки номенклатуры'"));
		ПараметрыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения опции использования упаковок необходимо пометить на удаление документы, у которых, в табличной " +   
					   "части ""Товары"" используются упаковки, или заменить их на единицы измерения номенклатуры и перепровести документы'"));
...
// Устанавливает константы ВестиУчетПоОднойОрганизацииПолныйИнтерфейс, ВестиУчетПоНесколькимОрганизациямПолныйИнтерфейс
//
...
Функция ОтказСнятьИспользованиеКонстантыПоДаннымБазы(ИмяКонстанты) Экспорт
	...
	ИначеЕсли ИмяКонстанты = "ИспользоватьУпаковкиНоменклатуры" Тогда
		ОтказСнять = УпаковкиВызовСервера.УпаковкаИспользуетсяВДокументах();
	...
	ИначеЕсли ЭтоКонстантаОбменаСФГИСПолныйИнтерфейс(ИмяКонстанты) Тогда
		ОтказСнять = Ложь;
	КонецЕсли;
...
Функция УсловияДоступностиДокументовПоТарифу() Экспорт
	...
	ДобавитьОграниченияОпцииОбменСФГИС(УсловияДоступности);
...
Процедура ДобавитьОграниченияОпцииОбменСФГИС(УсловияДоступности)
	
	КонстантыОбменаСФГИСПолныйИнтерфейс = КонстантыОбменаСФГИСПолныйИнтерфейс();
	
	Для Каждого ТекущаяКонстанта Из КонстантыОбменаСФГИСПолныйИнтерфейс Цикл
		ДобавитьОграничениеДоступности(УсловияДоступности, ТекущаяКонстанта, Неопределено);
	КонецЦикла;
...
Функция НовыеУсловияДоступностиДокументовПоТарифу()
	...
	ТипыОбъекта.Добавить(Новый ОписаниеТипов("Тип"));
	ТипыОбъекта.Добавить(Тип("Неопределено"));
	...
	УсловияДоступности.Колонки.Добавить("ТипОбъекта", Новый ОписаниеТипов(ТипыОбъекта));
...
Функция ЭтоКонстантаОбменаСФГИСПолныйИнтерфейс(ИмяКонстанты)
	
	КонстантыГоссистем = КонстантыОбменаСФГИСПолныйИнтерфейс();
	Возврат КонстантыГоссистем.Найти(ИмяКонстанты) <> Неопределено;
	
КонецФункции

Функция КонстантыОбменаСФГИСПолныйИнтерфейс()
	
	КонстантыГоссистем = Новый Массив;
	КонстантыГоссистем.Добавить("ВестиСведенияДляДекларацийПоАлкогольнойПродукции");
	КонстантыГоссистем.Добавить("ВестиУчетПодконтрольныхТоваровВЕТИС");
	КонстантыГоссистем.Добавить("ВестиУчетЗернаИПродуктовПереработкиЗЕРНО");
	КонстантыГоссистем.Добавить("ВестиУчетПестицидовАгрохимикатовТукосмесейСАТУРН");
	
	Возврат КонстантыГоссистем;
