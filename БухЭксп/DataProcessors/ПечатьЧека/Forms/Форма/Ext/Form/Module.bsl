...
Процедура ЗаполнитьТаблицаТоваров()
	...
	Для каждого ПозицияЧека Из ПараметрыОперацииФискализацииЧека.ПозицииЧека Цикл
		...
		Если ЗначениеЗаполнено(ПозицияЧека.Штрихкод) И ЭтоАдресВременногоХранилища(ПозицияЧека.Штрихкод) Тогда
			...
			РезультатРаспределения = ПолучитьИзВременногоХранилища(ПозицияЧека.Штрихкод);
			Для каждого ШтрихкодыНоменклатуры Из РезультатРаспределения Цикл
				Для каждого СтруктураШтрихкода Из ШтрихкодыНоменклатуры.Значение Цикл
					...
					ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ШтрихкодыНоменклатуры.Ключ);
					
					// количество упаковок весового товара
					Если СтрокаТаблицаШтрихкоды.ОбъемноСортовойУчет 
						И ОписаниеНоменклатуры[ШтрихкодыНоменклатуры.Ключ].ТребуетВзвешивания Тогда
