...
Функция ПолучитьСписокОтчетов() Экспорт
	...
	ДеревоОтчетов.Колонки.Добавить( "НеСоздаватьНовый", ОписаниеТиповСтрока );
	...
	Для Инд = 0 По МакетСписокОтчетов.Области.Количество() - 1 Цикл
		...
		Если Не ПустаяСтрока(ИмяГруппы) Тогда
			...
			Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
				...
				НеСоздаватьНовый = СокрЛП(МакетСписокОтчетов.Область(Ном, 7).Текст);
				...
				СтрокаУровня2.НеСоздаватьНовый = НеСоздаватьНовый;
		...
		Иначе
			...
			Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
				...
				СтрокаУровня1.НеСоздаватьНовый = НеСоздаватьНовый;
...
Процедура ЗаполнитьСписокОтчетов(ДеревоОтчетов) Экспорт
	...
	Для Каждого СтрокаУровня1 Из ДеревоОтчетов.Строки Цикл
		...
		НеСоздаватьНовыйИзМакета = СокрЛП(СтрокаУровня1.НеСоздаватьНовый);
		...
		Если ЭтоГруппа Тогда
			...
			Если СтрокаУровня1.Строки.Количество() > 0 Тогда
				...
				Для Каждого СтрокаУровня2 Из СтрокаУровня1.Строки Цикл
					...
					НеСоздаватьНовыйИзМакета = СокрЛП(СтрокаУровня2.НеСоздаватьНовый);
					...
					Если НайденныйЭлемент = Неопределено Тогда
						...
						НовыйЭлемент.НеСоздаватьНовый = ПризнакНеСоздаватьНовыйИзМакета(НеСоздаватьНовыйИзМакета);
					...
					Иначе
						...
						ПрименитьПризнакНеСоздаватьНовый(ТекЭлемент, НеСоздаватьНовыйИзМакета);
...
// Преобразует значение ячейки макета "Не создавать новый" в значение реквизита
// для вновь создаваемого элемента справочника.
// Правила: "-" -> Ложь; любой другой непустой символ -> Истина; пустая строка -> Ложь.
//
Функция ПризнакНеСоздаватьНовыйИзМакета(ЗначениеЯчейки)
	
	Если ЗначениеЯчейки = "-" Тогда
		Возврат Ложь;
	ИначеЕсли ПустаяСтрока(ЗначениеЯчейки) Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Применяет значение признака "Не создавать новый" из макета к существующему элементу
// справочника. Правила: "-" -> Ложь; любой другой непустой символ -> Истина; пустая строка -> оставить без изменений.
//
Процедура ПрименитьПризнакНеСоздаватьНовый(ЭлементСправочника, ЗначениеЯчейки)
	...
	Если ЗначениеЯчейки = "-" Тогда
		ЭлементСправочника.НеСоздаватьНовый = Ложь;
	Иначе
		ЭлементСправочника.НеСоздаватьНовый = Истина;
	КонецЕсли;
