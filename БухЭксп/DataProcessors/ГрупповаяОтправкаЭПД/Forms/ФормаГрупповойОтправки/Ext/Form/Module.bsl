...
Процедура ЗаписатьНаСервере(Отказ, ПараметрыЗаписи, СсылкаДляЗаписи)
	...
	Если МассивВерсийТитула <> Неопределено Тогда
		...
		СтруктураРеквизитовДляВставки = МассивВерсийТитула[ПараметрыЗаписи.ИдентификаторСтроки + 1];
		
		Если ПараметрыЗаписи.Свойство("ТребуетсяОбновитьДанныеОПодписантах") Тогда
			
			СтруктураРеквизитовДляОбновления = ОбменСГИСЭПДКлиентСервер.СкопироватьИзФиксированногоЗначенияРекурсивно(
				СтруктураРеквизитовДляВставки);
			
			ОбменСГИСЭПД.ОбновитьДанныеОПодписантах(ТекущийОбъектСтруктура, НомерВерсии, 
				СтруктураРеквизитовДляОбновления);
				
			СтруктураРеквизитовДляВставки = 
				ОбменСГИСЭПДКлиентСервер.СкопироватьВФиксированноеЗначенияРекурсивно(СтруктураРеквизитовДляОбновления);
				
		КонецЕсли;
		
		ТекущийОбъект.ДополнительныеСвойства.Вставить("СтруктураРеквизитов", СтруктураРеквизитовДляВставки);
		...
		Если КоличествоОтправленоТекущее <> Неопределено Тогда
			Если КоличествоОтправленоТекущее > 0 Тогда
				Для Каждого ИдентификаторСтроки Из ДополнительныеПараметры.ИдентификаторыСтрок Цикл
					...
					ПараметрыЗаписи.Вставить("ТребуетсяОбновитьДанныеОПодписантах");
