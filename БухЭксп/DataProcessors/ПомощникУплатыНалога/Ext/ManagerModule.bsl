...
Процедура ЗаполнитьСведенияУплатаСтраховыхВзносов(ПараметрыЗадачи, СведенияОРасчете, ВариантВыборкиСтраховыхВзносов)
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация)
		...
		// В этом случае есть или ПерерасчетСтраховыхВзносов или РегистрацияОблагаемойБазыРуководителя.
		ИмяДокумента = "ПерерасчетСтраховыхВзносов";
		ОперацияВыполнена = ВыполнениеЗадачБухгалтера.ОперацияНачисленияВзносовВыполнена(Организация, ПериодСобытия, ДокументЗарплаты, НесколькоДокументов, ИмяДокумента);
		Если Не ОперацияВыполнена Тогда
			ИмяДокумента = "РегистрацияОблагаемойБазыРуководителя";
			ОперацияВыполнена = ВыполнениеЗадачБухгалтера.ОперацияНачисленияВзносовВыполнена(Организация, ПериодСобытия, ДокументЗарплаты, НесколькоДокументов, ИмяДокумента);
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ИмяДокумента = "РегистрацияОблагаемойБазыРуководителя" Тогда
			ОписаниеДействияРасчет.Наименование = НСтр("ru = 'Ввести данные о взносах руководителя'");
		Иначе
