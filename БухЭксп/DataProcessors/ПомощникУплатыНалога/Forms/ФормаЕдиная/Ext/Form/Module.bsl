...
Процедура УчестьПодготовленныеУведомления(ТаблицаРасчета)
	...
	// Исключим строки с налогами, по которым не требуется уведомление, либо уведомления уже подготовлены.
	Налоги = НалогиНеТребующиеУведомлений();
...
&НаСервере
Функция НалогиНеТребующиеУведомлений()
	...
	|ГДЕ
	|	ВидыНалоговИПлатежейВБюджет.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.НДС)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Налог";
