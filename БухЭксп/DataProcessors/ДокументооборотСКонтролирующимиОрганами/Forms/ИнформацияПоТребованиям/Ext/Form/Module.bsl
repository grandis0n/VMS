...
Процедура ОткрытьВсе(Команда)
	...
	Иначе
		ЭтоОтчетность24 = ДокументооборотСКОВызовСервера.ЭтоРежимОблачной1СО();
		Если ЭтоОтчетность24 Тогда
			ПараметрыФормы = Новый Структура("Раздел,Форма", "incoming-button", Неопределено);
			МодульКалендарьОтчетностиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("КалендарьОтчетностиКлиент");
			МодульКалендарьОтчетностиКлиент.ОткрытьРазделОтчетности(ПараметрыФормы);
		ИначеЕсли НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ФормаОткрыта("ОбщаяФорма.РегламентированнаяОтчетность") Тогда
