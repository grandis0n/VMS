...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользуетсяСогласованиеПлатежей = УчетДенежныхСредствБП.ДоступностьСогласованияПлатежейПоБанковскомуСчету(
	...
	Если Параметры.Свойство("ЭлектроннаяВыпискаБанка")
		...
		ЗаполнитьПериод = Ложь;
		Если ЭлектронныеВыпискиБанка.Количество() > 1 Тогда
			ПериодыВыписок = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ЭлектронныеВыпискиБанка, "ДатаНачала, ДатаОкончания");
			Периоды = Новый СписокЗначений;
			Для Каждого КлючИЗначение Из ПериодыВыписок Цикл
				Если КлючИЗначение.Значение.ДатаНачала = КлючИЗначение.Значение.ДатаОкончания Тогда
					Периоды.Добавить(КлючИЗначение.Значение.ДатаНачала);
				Иначе
					Прервать;
				КонецЕсли;
			КонецЦикла;
			...
			Если Периоды.Количество() > 0 Тогда
				...
				Объект.НачалоПериода = Периоды.Получить(0).Значение;
				Объект.КонецПериода  = Периоды.Получить(Периоды.Количество() - 1).Значение;
				СообщениеБанка = Неопределено;
				ЗаполнитьПериод = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СообщениеБанка) Тогда
			ПериодыВыписки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СообщениеБанка, "ДатаНачала, ДатаОкончания");
			Объект.НачалоПериода = ПериодыВыписки.ДатаНачала;
			Объект.КонецПериода = ?(ЗначениеЗаполнено(ПериодыВыписки.ДатаОкончания),
				ПериодыВыписки.ДатаОкончания, ПериодыВыписки.ДатаНачала);
			ЗаполнитьПериод = Истина;
		КонецЕсли;
		
		Если ЗаполнитьПериод Тогда
	...
	Если ЭтоМобильныйКлиент Тогда
		Если Параметры.Свойство("АдресФайлаВыписки", АдресФайлаВыписки) Тогда
			СпособЗагрузки = "Файл";
			ЗагрузкаЧерезПоделитьсяМК = Истина;
		КонецЕсли;
...
Процедура СкрытьРезультатСозданияДокументов(Форма)
	...
	Если Элементы.ГруппаСообщениеОРасхождении.Видимость Тогда
		Элементы.ГруппаСообщениеОРасхождении.Видимость = Ложь;
	КонецЕсли;
...
Процедура ТекстРасхожденияНажатие(Элемент)
	...
	КонецПериода = НачалоПериодаРасхождения;
	Для каждого СтрокаТаблицы Из ДокументыКИмпорту Цикл
		КонецПериода = Макс(СтрокаТаблицы.ДатаПроведения, КонецПериода);
	КонецЦикла;
	
	БанковскийСчет = Объект.БанковскийСчет;
	
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		МассивСчетов = Новый Массив;
		Для Каждого Строка из ДокументыКИмпорту Цикл
			МассивСчетов.Добавить(Строка.БанковскийСчетОрганизации);
		КонецЦикла;
		ИтоговыйМассив = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивСчетов);
		Если ИтоговыйМассив.Количество() = 1 Тогда
			БанковскийСчет = ДокументыКИмпорту[0].БанковскийСчетОрганизации;
		КонецЕсли;
	КонецЕсли;
	
	НаборПараметров = Новый Структура;
	НаборПараметров.Вставить("БанковскийСчет"					, БанковскийСчет);
	НаборПараметров.Вставить("Организация"						, Объект.Организация);
	НаборПараметров.Вставить("НачалоПериода"					, НачалоПериодаРасхождения);
	НаборПараметров.Вставить("КонецПериода" 					, КонецПериода);
	НаборПараметров.Вставить("ИспользуютсяСтандартныеНастройки"	, Истина);
	НаборПараметров.Вставить("СформироватьОтчет"				, Истина);
	
	ОткрытьФорму("Отчет.СверкаБанковскойВыписки.Форма.ФормаОтчета", НаборПараметров,, Новый УникальныйИдентификатор);
...
Процедура ЗагрузитьНастройкиДляБанковскогоСчета(БанковскийСчетУстановленИзФайла = Ложь, ЭтоИзменениеБанкСчета = Ложь)
	...
	ЗагрузкаВыписокЧерезИнтегратор = Ложь;
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Если (ЗначениеЗаполнено(Объект.БанковскийСчет) Или ЗначениеЗаполнено(АдресВыписки))
			И Не ЗагружатьВыпискуПоВсемСчетамБанка Или ЗначениеЗаполнено(АдресФайлаВыписки) Тогда
			...
			Если ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
				...
				Если РеквизитыСчета.ЦифровойСчет Или ВалютаДенежныхСредств <> ВалютаРегламентированногоУчета Тогда
					...
					СпособЗагрузки = ?(ЗначениеЗаполнено(АдресВыписки) Или ПрямойОбменТолькоЗагрузка, "ДиректБанк", "Файл");
					СпособВыгрузки = "Файл";
		...
		Если Настройки.ЗагрузкаВыписокЧерезИнтегратор Тогда
			ЗагрузкаВыписокЧерезИнтегратор = Истина;
			УстановитьТекстЗагрузкиЧерезИнтегратор(Настройки.НаименованиеИнтегратора);
		КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Иначе
		...
		Если СпособЗагрузки = "ДиректБанкПоБанку" 
			Или СпособЗагрузки = "ДиректБанк" И ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
			
			Элементы.ДекорацияКартинкаЗагрузкаЧерезИнтегратор.Видимость = Форма.ЗагрузкаВыписокЧерезИнтегратор;
			Элементы.ДекорацияТекстЗагрузкаЧерезИнтегратор.Видимость = Форма.ЗагрузкаВыписокЧерезИнтегратор;
		Иначе
			Элементы.ДекорацияКартинкаЗагрузкаЧерезИнтегратор.Видимость = Ложь;
			Элементы.ДекорацияТекстЗагрузкаЧерезИнтегратор.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьРезультатПрочитаннойВыпискиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИзменилсяБанковскийСчет И ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
	...
	ИначеЕсли ИзмениласьОрганизация Тогда
		ПоВсемСчетам = Не ЗначениеЗаполнено(Объект.БанковскийСчет) И (ИспользоватьНесколькоБанковскихСчетов()
			Или Не ЗначениеЗаполнено(ДополнительныеПараметры.БанковскийСчет));
	...
	Иначе
		...
		Если СпособЗагрузки = "ДиректБанкПоБанку" Тогда
			...
			ОбменСБанкамиКлиент.ПолучитьВыпискуПоНастройкеОбмена(
				Оповещение, Объект.СоглашениеПрямогоОбменаСБанками, Объект.НачалоПериода, Объект.КонецПериода, Банк);
		Иначе
...
Процедура РазместитьНаФормеРаспознанныеДанныеИзБанка(АдресРезультата, ВыводитьСообщения = Истина)
	...
	Если РаспознанныеДанныеИзБанка.Свойство("ЕстьРасхождение") И РаспознанныеДанныеИзБанка.Свойство("ДатаРасхождения") Тогда
		Если ЗначениеЗаполнено(РаспознанныеДанныеИзБанка.ДатаРасхождения) Тогда
			ПоказатьСообщениеОРасхождении(РаспознанныеДанныеИзБанка.ДатаРасхождения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДокументыКИмпортуПоРаспознаннымДаннымИзБанка(ДокументыКИмпорту, РаспознанныеДанныеИзБанка, ВыводитьСообщения)
	...
	Для Каждого КлючИЗначение Из РаспознанныеДанныеИзБанка.Выписки Цикл
		...
		Если СпособЗагрузки = "Файл" Тогда
			РаспознанныеДанныеИзБанка.Вставить("ИсточникЗагрузки", Перечисления.ИсточникиЗагрузкиБанковскихВыписок.Файл);
			НачалоПериода = Выписка.НачалоПериода
		Иначе
			РаспознанныеДанныеИзБанка.Вставить("ИсточникЗагрузки", Перечисления.ИсточникиЗагрузкиБанковскихВыписок.ДиректБанк);
			НачалоПериода = Объект.НачалоПериода
		КонецЕсли;
		
	КонецЦикла;
	
	ДатаРасхождения = Обработки.КлиентБанк.ПроверитьРасхождениеНачальногоОстатка(
		Организация,МассивБанкСчетов, НачалоПериода);
	
	РаспознанныеДанныеИзБанка.Вставить("ЕстьРасхождение", Ложь);
	
	Элементы.ГруппаСообщениеОРасхождении.Видимость = Ложь;
	
	Если ЗначениеЗаполнено(ДатаРасхождения) Тогда
		РаспознанныеДанныеИзБанка.ЕстьРасхождение = Истина;
		РаспознанныеДанныеИзБанка.Вставить("ДатаРасхождения", ДатаРасхождения);
		НачалоПериодаРасхождения = ДатаРасхождения;
	КонецЕсли;
...
Процедура ДобавитьРасчетныйСчетВДеревоКонтрагентов(СтрокаКонтрагент, СчетКонтрагентаСсылкаДляНового, УчастникиОпераций);
	...
	Если КоллекцияБанковскиеСчета.Колонки.Найти("ИдентификаторЦифровогоСчета") <> Неопределено
		...
		СтрокаРеквизит.Представление = "ID кошелька";
...
Функция ПараметрыЗагрузкиДокументовНаСервере()
	...
	Если СпособЗагрузки <> "Файл" Тогда
		ПараметрыЗагрузкиДокументов.Вставить("ДатаНачалаВыписки", Объект.НачалоПериода);
		ПараметрыЗагрузкиДокументов.Вставить("ДатаОкончанияВыписки", Объект.КонецПериода);
	КонецЕсли;
...
Функция ТекстовыйДокументИзВременногоХранилищаФайла(АдресФайла, Кодировка, Расширение = "txt") Экспорт
	...
	ИмяВременногоФайла  = ПолучитьИмяВременногоФайла(
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(Расширение));
...
Процедура ЗаполнитьСпособВыгрузки(СохранятьТекущуюНастройку = Ложь, ЕстьПрямойОбмен = Ложь)
	...
	Если (ПрямойОбменСБанками Или ЕстьПрямойОбмен) И Не ЗагрузкаВыписокЧерезИнтегратор Тогда
...
&НаСервере
Процедура УстановитьТекстЗагрузкиЧерезИнтегратор(НаименованиеИнтегратора)
	
	ЗаголовокДекорации = СтрШаблон(НСтр("ru = 'Данные будут загружены через %1'"), НаименованиеИнтегратора);
	Элементы.ДекорацияТекстЗагрузкаЧерезИнтегратор.Заголовок = ЗаголовокДекорации;
	
КонецПроцедуры

&НаСервере
Процедура ПоказатьСообщениеОРасхождении(ДатаРасхождения)
	
	ДатаРасхождения = Формат(ДатаРасхождения, "ДФ=dd.MM.yyyy");
	
	Элементы.ГруппаСообщениеОРасхождении.Видимость = Истина;
	
	ШаблонТекста = НСтр("ru = 'Обнаружены расхождения при сверке выписки на начало %1'");
	
	Элементы.ТекстРасхождения.Заголовок = СтрШаблон(ШаблонТекста, ДатаРасхождения);
...
#Область ОтправкаПлатежей
	...
	Если ЗагрузкаЧерезПоделитьсяМК Тогда
		Элементы.СтраницыМК.ТекущаяСтраница = Элементы.ЗагрузкаМК;
	КонецЕсли;
...
#КонецОбласти
	...
	Если НЕ ДиректБанкЗагрузкаДоступенМК И НЕ ЗагрузкаЧерезПоделитьсяМК Тогда
...
Процедура УправлениеВидимостьюМК()
	...
	Элементы.ГруппаПериодЗагрузкаМК.Видимость                = ДиректБанкЗагрузкаДоступенМК ИЛИ (НЕ ЗагрузкаЧерезПоделитьсяМК);
	Элементы.ГруппаДокументыКИмпортуМК.Видимость             = ДиректБанкЗагрузкаДоступенМК ИЛИ (ЗагрузкаЧерезПоделитьсяМК);
	Элементы.ГруппаКомандЗагрузкиМК.Видимость                = ДиректБанкЗагрузкаДоступенМК ИЛИ (ЗагрузкаЧерезПоделитьсяМК);
	Элементы.НадписьНедоступенДиректбанкЗагрузкаМК.Видимость = НЕ ДиректБанкЗагрузкаДоступенМК И (НЕ ЗагрузкаЧерезПоделитьсяМК);
	
	Если Элементы.Найти("КнопкаМКОтправитьПлатежиВБанк") <> Неопределено Тогда
		Элементы.КнопкаМКОтправитьПлатежиВБанк.Видимость = ДиректБанкОтправкаДоступенМК;
	КонецЕсли;
