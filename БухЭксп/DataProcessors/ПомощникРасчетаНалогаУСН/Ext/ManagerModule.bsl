...
Функция ПодсказкаПоНалогу(НомерКвартала, СрокУплаты, СрокПодачи, УтратаПраваНаУСН = Ложь) Экспорт
	...
	Если НомерКвартала <> 4 И Не УтратаПраваНаУСН Тогда
...
Функция ОписаниеДействияДекларация(Организация, Период, ТекстОшибки = "") Экспорт
	...
	ДатаПереходаНаОСНО = Обработки.ПомощникПереходаСУСННаОСНО.ДатаПереходаНаОСНОЗаПериод(
		Организация, НачалоГода(Период), КонецДня(Период));
	УтратаПраваНаУСН = ЗначениеЗаполнено(ДатаПереходаНаОСНО) И ДатаПереходаНаОСНО <> КонецГода(ДатаПереходаНаОСНО);
	
	Если КонецКвартала(Период) <> НалоговыйПериод.Конец И Не УтратаПраваНаУСН Тогда
		Возврат ОписаниеДействия;
	КонецЕсли;
	...
	Если ПорядокПредоставленияОтчета <> Неопределено Тогда
		...
		Если УтратаПраваНаУСН Тогда
			ПериодСобытия = КонецГода(Период);
			Периодичность = Перечисления.Периодичность.Год;
		Иначе
			ПериодСобытия = ПорядокПредоставленияОтчета.ПериодСобытия;
...
Функция ПорядокОтчетаУплатыНалога(Организация, Период) Экспорт
	...
	Если ЗначениеЗаполнено(ПорядокУплаты) Тогда
		...
		ЭтоПереходНаОСНО = Обработки.МониторНалоговИОтчетности.ЭтоЗадачаУСНПереходНаОСНО(ПорядокУплаты.Правило.Код);
	...
	Если Период = КонецГода(Период) Или ЭтоПереходНаОСНО Тогда
...
Процедура ЗаполнитьПоясненияКРасчетуОбъектДоходыМинусРасходы(ТаблицаЭтапы, Показатели)
	...
	БылПереходНаОСНО              = Показатели.БылПереходНаОСНО;
	
	Если (НомерКвартала = 4 Или БылПереходНаОСНО)
		И Показатели.НалогИсчисленныйВсего < Показатели.МинимальныйНалог Тогда
		НалогРассчитанный =
	...
	Если НомерКвартала = 4 Или БылПереходНаОСНО Тогда
...
Процедура ОпределитьПорядокДействий(Показатели)
	...
	Если Показатели.ЕстьДекларация Тогда
		...
		Если (Показатели.НомерКвартала = 4 Или Показатели.БылПереходНаОСНО)
			И Не Показатели.ОбъектНалогообложенияДоходы Тогда
		...
		Если Не Показатели.БылПереходНаОСНО Тогда
...
Функция ОрганизацияПлательщикУСН(Организация, Период, НачалоНалоговогоПериода)
	...
	ДатаПереходаНаОСНО = БухгалтерскийУчетПовтИсп.ДатаПереходаНаОСНОЗаПериод(
		Организация, НачалоНалоговогоПериода, Период);
	УтратаПраваНаУСН = ЗначениеЗаполнено(ДатаПереходаНаОСНО);
	...
	Если УтратаПраваНаУСН И КонецКвартала(ДатаПереходаНаОСНО) < Период Тогда
		ПлательщикУСН = Ложь;
	ИначеЕсли Не ПлательщикУСН И ДатаИзменения > НачалоНалоговогоПериода Тогда
...
Функция НовыйПоказатели(Организация, Период)
	...
	ДатаПереходаНаОСНОЗаПериод = Обработки.ПомощникПереходаСУСННаОСНО.ДатаПереходаНаОСНОЗаПериод(
		Организация, НачалоКвартала(Период), КонецКвартала(Период));
	Показатели.Вставить("БылПереходНаОСНО", ЗначениеЗаполнено(ДатаПереходаНаОСНОЗаПериод));
