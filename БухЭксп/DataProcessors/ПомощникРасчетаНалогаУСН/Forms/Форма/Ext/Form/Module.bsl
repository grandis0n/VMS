...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УтратаПраваНаУСН = УтратаПраваНаУСН(Объект.Организация, Объект.Период);
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если ЭтоМобильныйКлиент Тогда
		Элементы.ГруппаЭлементовМК.Видимость = ?(ЗначениеЗаполнено(Объект.Организация), Истина, Ложь);
	КонецЕсли;
...
Функция НужноПодготовитьДекларацию(Форма)
	...
	Возврат (Форма.НомерКвартала = 4 Или Форма.УтратаПраваНаУСН) И НетОшибокБлокирующихРасчетНалога(Форма);
...
Процедура ЗаполнитьПравилоИСрокЗадачи()
	...
	ПодсказкаПоНалогу = Обработки.ПомощникРасчетаНалогаУСН.ПодсказкаПоНалогу(
		НомерКвартала, Срок, СрокОтчета, УтратаПраваНаУСН);
...
&НаСервереБезКонтекста
Функция УтратаПраваНаУСН(Организация, Период)
	
	ДатаПереходаНаОСНО = Обработки.ПомощникПереходаСУСННаОСНО.ДатаПереходаНаОСНОЗаПериод(
		Организация, НачалоКвартала(Период), Период);
