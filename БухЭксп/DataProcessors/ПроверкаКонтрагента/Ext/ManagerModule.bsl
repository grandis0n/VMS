	...
	Если ДанныеОтчета.СостояниеОтчета = "ПолученыДанные" Тогда
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			СформироватьПодразделения(
				...
				ДанныеОтчета,
				ДанныеПроверкиКонтрагента);
			СформироватьПравопреемники(
				ИнформацияОСвязях,
				ДанныеОтчета,
				ДанныеПроверкиКонтрагента);
			СформироватьПравопредшественники(
				ИнформацияОСвязях,
				ДанныеОтчета,
				ДанныеПроверкиКонтрагента);
		КонецЕсли;
...
Процедура СформироватьРезультатДанныеГосРеестров(
	...
	ВывестиРазделУстав(
		...
		Сведения,
		ПараметрыВывода);
	ВывестиРазделКорпоративныйДоговор(
		ТабличныйДокумент,
		Сведения,
		ПараметрыВывода);
	ВывестиРазделДержательРеестраАкционеров(
		ТабличныйДокумент,
		Сведения,
		ПараметрыВывода);
...
Процедура ВывестиРазделРегистрация(
	...
	Если ИнформацияОСвязях.РегистрацияВНалоговомОргане <> Неопределено
		...
		Или ИнформацияОСвязях.РегистрацияВФСС <> Неопределено Тогда
			...
			ВывестиРегистрацияНаДеньПринятия(
				ТабличныйДокумент,
				ИнформацияОСвязях,
				ПараметрыВывода);
...
Процедура ВывестиРегистрацияНаДеньПринятия(
		ТабличныйДокумент,
		ИнформацияОСвязях,
		ПараметрыВывода)
	
	Если Не ПараметрыВывода.ЭтоЮридическоеЛицо Тогда
		Возврат;
	КонецЕсли;
	
	СведенияОРегистрацииНаДеньПринятия = ИнформацияОСвязях.СведенияОРегистрацииНаДеньПринятия;
	Если СведенияОРегистрацииНаДеньПринятия = Неопределено
		Или СведенияОРегистрацииНаДеньПринятия.ИдентификационныйКод = Неопределено
		Или СведенияОРегистрацииНаДеньПринятия.НаименованиеРегистрирующегоОргана = Неопределено
		Или СведенияОРегистрацииНаДеньПринятия.РегистрационныйНомер = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОбластиМакета = ПараметрыВывода.ОбластиМакета;
	
	Параметры = ОбластиМакета.РегистрацияНаДеньПринятия.Параметры;
	Параметры.РегистрационныйНомер = СведенияОРегистрацииНаДеньПринятия.РегистрационныйНомер;
	Параметры.РегистрирующийОрган  = СведенияОРегистрацииНаДеньПринятия.НаименованиеРегистрирующегоОргана;
	Параметры.ИдентификационныйКод = СведенияОРегистрацииНаДеньПринятия.ИдентификационныйКод;
	
	ТабличныйДокумент.Вывести(ОбластиМакета.РегистрацияНаДеньПринятия);
...
Процедура ВывестиРазделРеквизиты(
	...
	Если ЭтоЮридическоеЛицо Тогда
		Если ИнформацияОСвязях.СтатусМКФ <> Неопределено Тогда
			ОбластиМакета.СтатусМКФ.Параметры.Заполнить(ИнформацияОСвязях);
			ТабличныйДокумент.Вывести(ОбластиМакета.СтатусМКФ);
		КонецЕсли;
	...
	Иначе
		...
		Если ИнформацияОСвязях.ЯвляетсяГлавойКФХ Тогда
			ОбластиМакета.ГлаваКФХОписание.Параметры.Заполнить(ИнформацияОСвязях);
			ТабличныйДокумент.Вывести(ОбластиМакета.ГлаваКФХОписание);
		КонецЕсли;
	КонецЕсли;
	ТабличныйДокумент.Вывести(ОбластиМакета.Разделитель);
...
Процедура ВывестиРазделКорпоративныйДоговор(
		ТабличныйДокумент,
		ИнформацияОСвязях,
		ПараметрыВывода)
	
	Если Не ПараметрыВывода.ЭтоЮридическоеЛицо
		Или ИнформацияОСвязях.КорпоративныйДоговор = Неопределено Тогда
	...
	ОбластиМакета = ПараметрыВывода.ОбластиМакета;
	ОбластиМакета.КорпоративныйДоговор.Параметры.КорпоративныйДоговор = ИнформацияОСвязях.КорпоративныйДоговор;
	ТабличныйДокумент.Вывести(ОбластиМакета.КорпоративныйДоговор);
	
	ТабличныйДокумент.Вывести(ОбластиМакета.Разделитель);
	
КонецПроцедуры

Процедура ВывестиРазделДержательРеестраАкционеров(
		ТабличныйДокумент,
		ИнформацияОСвязях,
		ПараметрыВывода)
	
	Если Не ПараметрыВывода.ЭтоЮридическоеЛицо
		Или ИнформацияОСвязях.ДержательРеестраАкционеров = Неопределено Тогда
	...
	ОбластиМакета = ПараметрыВывода.ОбластиМакета;
	ОбластиМакета.ДержательРеестраАкционеров.Параметры.ДержательРеестраАкционеров =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ИНН %1 ОГРН %2
				|Наименование %3'"),
			ИнформацияОСвязях.ДержательРеестраАкционеров.ИНН,
			ИнформацияОСвязях.ДержательРеестраАкционеров.ОГРН,
			ИнформацияОСвязях.ДержательРеестраАкционеров.Наименование);
	ТабличныйДокумент.Вывести(ОбластиМакета.ДержательРеестраАкционеров);
...
Процедура СформироватьВидыДеятельности(
	...
	Если Сведения.ВидДеятельности = Неопределено Тогда
		...
		Иначе
			ОсновнойВидДеятельности = Сведения.ВидыДеятельности[0];
		КонецЕсли;
...
Процедура СформироватьПравопреемники(Сведения, ДанныеОтчета, ДанныеПроверкиКонтрагента)
	
	КарточкаПравопреемники = Новый Структура;
	КарточкаПравопреемники.Вставить("ЕстьДанные", Ложь);
	КарточкаПравопреемники.Вставить("Правопреемники", Новый Массив);
	КарточкаПравопреемники.Вставить("СкрытыеДанные", Ложь);
	КарточкаПравопреемники.Вставить("Описание", "");
	
	ДанныеПроверкиКонтрагента.Вставить("КарточкаПравопреемники", КарточкаПравопреемники);
	
	Если Сведения.ДанныеПравопреемников.СкрытыеДанные Тогда
		КарточкаПравопреемники.СкрытыеДанные = Истина;
		КарточкаПравопреемники.ЕстьДанные = Истина;
		КарточкаПравопреемники.Описание = Сведения.ДанныеПравопреемников.ПредставлениеСкрытыхДанных;
	Иначе
		
		Для Каждого Правопреемник Из Сведения.ДанныеПравопреемников.Правопреемники Цикл
			ДанныеОрганизации = Новый Структура;
			ДанныеОрганизации.Вставить("ИНН", Правопреемник.ИНН);
			ДанныеОрганизации.Вставить("НаименованиеОрганизации", ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
				Правопреемник.Наименование));
			ДанныеОрганизации.Вставить("СкрытыеДанные", Правопреемник.СкрытыеДанные);
			ДанныеОрганизации.Вставить("ПредставлениеСкрытыхДанных", Правопреемник.ПредставлениеСкрытыхДанных);
			КарточкаПравопреемники.Правопреемники.Добавить(ДанныеОрганизации);
		КонецЦикла;
		КарточкаПравопреемники.ЕстьДанные = (КарточкаПравопреемники.Правопреемники.Количество() > 0);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура СформироватьПравопредшественники(Сведения, ДанныеОтчета, ДанныеПроверкиКонтрагента)
	
	КарточкаПравопредшественники = Новый Структура;
	КарточкаПравопредшественники.Вставить("ЕстьДанные", Ложь);
	КарточкаПравопредшественники.Вставить("Правопредшественники", Новый Массив);
	КарточкаПравопредшественники.Вставить("СкрытыеДанные", Ложь);
	КарточкаПравопредшественники.Вставить("Описание", "");
	
	ДанныеПроверкиКонтрагента.Вставить("КарточкаПравопредшественники", КарточкаПравопредшественники);
	
	Если Сведения.ДанныеПравопредшественников.СкрытыеДанные Тогда
		КарточкаПравопредшественники.СкрытыеДанные = Истина;
		КарточкаПравопредшественники.ЕстьДанные = Истина;
		КарточкаПравопредшественники.Описание = Сведения.ДанныеПравопредшественников.ПредставлениеСкрытыхДанных;
	Иначе
		
		Для Каждого Правопредшественник Из Сведения.ДанныеПравопредшественников.Правопредшественники Цикл
			ДанныеОрганизации = Новый Структура;
			ДанныеОрганизации.Вставить("ИНН", Правопредшественник.ИНН);
			ДанныеОрганизации.Вставить("НаименованиеОрганизации", ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
				Правопредшественник.Наименование));
			ДанныеОрганизации.Вставить("СкрытыеДанные", Правопредшественник.СкрытыеДанные);
			ДанныеОрганизации.Вставить("ПредставлениеСкрытыхДанных", Правопредшественник.ПредставлениеСкрытыхДанных);
			КарточкаПравопредшественники.Правопредшественники.Добавить(ДанныеОрганизации);
		КонецЦикла;
		КарточкаПравопредшественники.ЕстьДанные = (КарточкаПравопредшественники.Правопредшественники.Количество() > 0);
...
Процедура СформироватьСвязанныеОрганизации(Сведения, ДанныеОтчета, ДанныеПроверкиКонтрагента)
	...
	ПараметрыСвязанныеОрганизации = Новый Структура;
	ПараметрыСвязанныеОрганизации.Вставить("ЭтоЮридическоеЛицо",            ДанныеОтчета.ЭтоЮридическоеЛицо);
	ПараметрыСвязанныеОрганизации.Вставить("ДобавленныеОрганизации",        ДобавленныеОрганизации);
	ПараметрыСвязанныеОрганизации.Вставить("СвязанныеОрганизации",          СвязанныеОрганизации);
	ПараметрыСвязанныеОрганизации.Вставить("РезультатСвязанныеОрганизации", РезультатСвязанныеОрганизации);
	СформироватьСвязанныеОрганизацииПоРуководителюИУчредителю(Сведения, ПараметрыСвязанныеОрганизации);
	Если ДанныеОтчета.ЭтоЮридическоеЛицо Тогда
		СформироватьСвязанныеОрганизацииДействующиеОтИмениЮридическогоЛица(Сведения, ПараметрыСвязанныеОрганизации);
		ДобавитьВСвязанныеОрганизацииПравопреемников(Сведения, ПараметрыСвязанныеОрганизации);
		ДобавитьВСвязанныеОрганизацииПравопредшественников(Сведения, ПараметрыСвязанныеОрганизации);
	КонецЕсли;
	...
	ДанныеПроверкиКонтрагента.Вставить("РезультатСвязанныеОрганизации",
		ПараметрыСвязанныеОрганизации.РезультатСвязанныеОрганизации);
...
Процедура СформироватьСвязанныеОрганизацииПоРуководителюИУчредителю(Сведения, Параметры)
	...
	Параметры.РезультатСвязанныеОрганизации = Новый ТабличныйДокумент;
	ОбластиМакета = ВсеОбластиМакета("СвязанныеОрганизации");
	Параметры.РезультатСвязанныеОрганизации.Вывести(
	...
	Для Итератор = 0 По ВГраница Цикл
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			ОбластьРуководитель.Параметры.Руководитель = Руководитель;
			Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластьРуководитель);
		Иначе
			ОбластиМакета.Предприниматель.Параметры.Предприниматель = ТекущийРуководитель.Наименование;
			Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.Предприниматель);
		КонецЕсли;
		...
		Если КоличествоОрганизаций > 0 Тогда
			...
			СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				?(ТекущийРуководитель.ОрганизацииГдеРуководительПоФИО,
					...
					""));
			ОбластиМакета.СколькоЕщеОрганизаций.Параметры.СколькоЕщеОрганизаций = СколькоЕщеОрганизаций;
			Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.СколькоЕщеОрганизаций);
			...
			Для Каждого ОрганизацияГдеРуководитель Из ТекущийРуководитель.ОрганизацииГдеРуководитель Цикл
				...
				Если ЗначениеЗаполнено(ДанныеОрганизации.ИНН) Тогда
					...
					Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета["ЕщеОрганизации"]);
					Если Параметры.ДобавленныеОрганизации.Получить(ДанныеОрганизации.ИНН) = Неопределено Тогда
						Параметры.ДобавленныеОрганизации.Вставить(ДанныеОрганизации.ИНН, Истина);
						ДобавитьОрганизацию = Истина;
					КонецЕсли;
				Иначе
					ОбластиМакета["ЕщеОрганизацииБезРасшифровки"].Параметры.Заполнить(ДанныеОрганизации);
					Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета["ЕщеОрганизацииБезРасшифровки"]);
					Если Параметры.ДобавленныеОрганизации.Получить(ДанныеОрганизации.НаименованиеОрганизации) = Неопределено Тогда
						Параметры.ДобавленныеОрганизации.Вставить(ДанныеОрганизации.НаименованиеОрганизации, Истина);
						ДобавитьОрганизацию = Истина;
					КонецЕсли;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ДанныеОрганизации.КодСтатуса)
					...
					Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.СтатусОрганизации);
				КонецЕсли;
				
				Если ДобавитьОрганизацию Тогда
					Параметры.СвязанныеОрганизации.Добавить(ДанныеОрганизации);
				КонецЕсли;
		...
		Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.Разделитель);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура СформироватьСвязанныеОрганизацииДействующиеОтИмениЮридическогоЛица(Сведения, Параметры)
	
	Если Не Параметры.ЭтоЮридическоеЛицо
		Или Сведения.ДействующиеОтИмениЮридическогоЛица = Неопределено Тогда
		
		Возврат;
	КонецЕсли;
	
	ОбластиМакета = ВсеОбластиМакета("СвязанныеОрганизации");
	Если Параметры.СвязанныеОрганизации.Количество() = 0 Тогда
		Параметры.РезультатСвязанныеОрганизации = Новый ТабличныйДокумент;
		Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.СвязанныеОрганизацииЗаголовок);
	КонецЕсли;
	
	Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.ЮридическиеЛицаИмеющиеПравоШапка);
	
	Для Каждого Организация Из Сведения.ДействующиеОтИмениЮридическогоЛица Цикл
		
		ДанныеОрганизации = НовыеРеквизитыОрганизации();
		ДанныеОрганизации.ИНН = Организация.ИНН;
		ДанныеОрганизации.НаименованиеОрганизации = Организация.Наименование;
		
		ОбластиМакета["ЕщеОрганизацииИмеющиеПраво"].Параметры.Заполнить(ДанныеОрганизации);
		Расшифровка = Новый Структура("Действие,ИНН", "Открыть", ДанныеОрганизации.ИНН);
		ОбластиМакета["ЕщеОрганизацииИмеющиеПраво"].Параметры.Расшифровка = Расшифровка;
		Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета["ЕщеОрганизацииИмеющиеПраво"]);
...
Процедура ДобавитьВСвязанныеОрганизацииПравопреемников(Сведения, Параметры)
	
	ДанныеПравопреемников = Сведения.ДанныеПравопреемников;
	Правопреемники = ДанныеПравопреемников.Правопреемники;
	
	Если ДанныеПравопреемников.Правопреемники.Количество() = 0
		И Не ДанныеПравопреемников.СкрытыеДанные Тогда
		
		Возврат;
	КонецЕсли;
	
	ОбластиМакета = ВсеОбластиМакета("СвязанныеОрганизации");
	ОбластьПравопреемник = ОбластиМакета["ПравопреемникШапка"];
	...
	Если ДанныеПравопреемников.СкрытыеДанные Тогда
		ДанныеПервойСтроки = НовыеРеквизитыОрганизации();
		ДанныеПервойСтроки.НаименованиеОрганизации = ДанныеПравопреемников.ПредставлениеСкрытыхДанных;
	Иначе
		ДанныеПервойСтроки = НовыеРеквизитыОрганизации();
		ДанныеПервойСтроки.ИНН = Правопреемники[0].ИНН;
		ДанныеПервойСтроки.НаименованиеОрганизации = ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
			Правопреемники[0].Наименование);
		Расшифровка = Новый Структура("Действие,ИНН", "Открыть", ДанныеПервойСтроки.ИНН);
		ОбластьПравопреемник.Параметры.Расшифровка = Расшифровка;
	КонецЕсли;
	
	ОбластьПравопреемник.Параметры.Заполнить(ДанныеПервойСтроки);
	Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластьПравопреемник);
	
	Если Правопреемники.Количество() > 0 Тогда
		
		Для Индекс = 1 По Правопреемники.ВГраница() Цикл
			
			ДанныеОрганизации = НовыеРеквизитыОрганизации();
			ДанныеОрганизации.ИНН = Правопреемники[Индекс].ИНН;
			ДанныеОрганизации.НаименованиеОрганизации = ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
				Правопреемники[Индекс].Наименование);
			
			ОбластьЕщеОрганизации = ОбластиМакета["ЕщеОрганизации"];
			ОбластьЕщеОрганизации.Параметры.Заполнить(ДанныеОрганизации);
			Расшифровка = Новый Структура("Действие,ИНН", "Открыть", ДанныеОрганизации.ИНН);
			ОбластьЕщеОрганизации.Параметры.Расшифровка = Расшифровка;
			
			Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластьЕщеОрганизации);
...
Процедура ДобавитьВСвязанныеОрганизацииПравопредшественников(Сведения, Параметры)
	
	ДанныеПравопредшественников = Сведения.ДанныеПравопредшественников;
	Правопредшественники = ДанныеПравопредшественников.Правопредшественники;
	
	Если ДанныеПравопредшественников.Правопредшественники.Количество() = 0
		И Не ДанныеПравопредшественников.СкрытыеДанные Тогда
		
		Возврат;
	КонецЕсли;
	
	ОбластиМакета = ВсеОбластиМакета("СвязанныеОрганизации");
	ОбластьПравопреемник = ОбластиМакета["ПравопредшественникШапка"];
	
	Если Параметры.СвязанныеОрганизации.Количество() = 0
		И Сведения.ДанныеПравопреемников.Правопреемники.Количество() = 0 Тогда
		
		Параметры.РезультатСвязанныеОрганизации = Новый ТабличныйДокумент;
		Параметры.РезультатСвязанныеОрганизации.Вывести(ОбластиМакета.СвязанныеОрганизацииЗаголовок);
	КонецЕсли;
	
	Если ДанныеПравопредшественников.СкрытыеДанные Тогда
		ДанныеПервойСтроки = НовыеРеквизитыОрганизации();
		ДанныеПервойСтроки.НаименованиеОрганизации = ДанныеПравопредшественников.ПредставлениеСкрытыхДанных;
	Иначе
		ДанныеПервойСтроки = НовыеРеквизитыОрганизации();
		ДанныеПервойСтроки.ИНН = Правопредшественники[0].ИНН;
		ДанныеПервойСтроки.НаименованиеОрганизации = ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
			Правопредшественники[0].Наименование);
		Расшифровка = Новый Структура("Действие,ИНН", "Открыть", ДанныеПервойСтроки.ИНН);
		ОбластьПравопреемник.Параметры.Расшифровка = Расшифровка;
	КонецЕсли;
	...
	Если Правопредшественники.Количество() > 0 Тогда
		
		Для Индекс = 1 По Правопредшественники.ВГраница() Цикл
			
			ДанныеОрганизации = НовыеРеквизитыОрганизации();
			ДанныеОрганизации.ИНН = Правопредшественники[Индекс].ИНН;
			ДанныеОрганизации.НаименованиеОрганизации = ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
				Правопредшественники[Индекс].Наименование);
...
Функция СредняяРентабельностьАктивовПоОтрасли(Сведения, Год)
	...
	Если ВидДеятельности = Неопределено Тогда
		ОсновнойКодОКВЭД = "";
	ИначеЕсли ВидДеятельности.Классификатор = "ОКВЭД2" Тогда
		Если Год > "2016" Тогда
...
Функция СредняяРентабельностьПродажПоОтрасли(Сведения, Год)
	...
	Если Сведения.ВидДеятельности = Неопределено Тогда
		ОсновнойКодОКВЭД = "";
	ИначеЕсли Сведения.ВидДеятельности.Классификатор = "ОКВЭД2" Тогда
		Если Год > "2016" Тогда
...
Процедура СформироватьРезультатГлавное(
	...
	Если ДанныеОтчета.ЭтоЮридическоеЛицо Тогда
		...
		ОбщиеСведения.Вставить("ВидПолномочий", Сведения.ВидПолномочий);
	Иначе
	...
	ОбщиеСведения.Вставить("ЭлектроннаяПочта",  Сведения.ЭлектроннаяПочта);
...
Процедура ВывестиВРезультатНегативныеСписки(
	...
	Если НегативныеСписки = Неопределено
		...
		Результат.Вывести(ОбластиМакета.НетИнформации);
		Возврат;
	КонецЕсли;
