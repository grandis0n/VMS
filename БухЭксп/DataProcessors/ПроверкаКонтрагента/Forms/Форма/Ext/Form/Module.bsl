...
Процедура ДекорацияEmailНажатие(Элемент)
	...
	ПараметрыПисьма.Представление = Элементы.ДекорацияEmail.Заголовок;
	ПараметрыПисьма.ОжидаемыйВид = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты");
	УправлениеКонтактнойИнформациейКлиент.СоздатьЭлектронноеПисьмо(
		Элементы.ДекорацияEmail.Заголовок,
		ПараметрыПисьма);
КонецПроцедуры
...
&НаКлиенте
Процедура ДекорацияВсеПравопреемникиВсеНажатие(Элемент)
...
&НаКлиенте
Процедура ДекорацияПравопреемник0ОбработкаНавигационнойСсылки(
		Элемент,
		НавигационнаяСсылкаФорматированнойСтроки,
		СтандартнаяОбработка)
...
#Область Правопредшественники

&НаКлиенте
Процедура ДекорацияВсеПравопредшественникиВсеНажатие(Элемент)
...
&НаКлиенте
Процедура ДекорацияПравопредшественник0ОбработкаНавигационнойСсылки(
		Элемент,
		НавигационнаяСсылкаФорматированнойСтроки,
		СтандартнаяОбработка)
...
&НаКлиенте
Процедура СкопироватьEmail(Команда)
	ПоместитьТекстВБуфераОбмена(Элементы.ДекорацияEmail.Заголовок);
КонецПроцедуры
...
Процедура СкрытьЭлементыПриСоздании()
	...
	Элементы.ГруппаПравопреемники.Видимость = Ложь;
	Элементы.ГруппаПравопредшественники.Видимость = Ложь;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Если Не ОжиданиеОтвета
		...
		ВывестиКарточкуПравопреемники(ДанныеПроверкиКонтрагента);
		ВывестиКарточкуПравопредшественники(ДанныеПроверкиКонтрагента);
...
Функция Группы1СКонтрагент()
	...
	Результат.Добавить(Элементы.ГруппаПравопреемники);
	Результат.Добавить(Элементы.ГруппаПравопредшественники);
...
Процедура ВывестиКарточкуОбщиеСведения(Сведения)
	...
	ВывестиКонтактнуюИнформацию(Сведения);
...
Процедура ВывестиРуководителя(Сведения)
	...
	Элементы.ДекорацияРуководителиВидПолномочий.Видимость = Ложь;
...
&НаСервере
Процедура ВывестиВидПолномочий(Сведения)
	
	Если Сведения.ВидПолномочий = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ДекорацияРуководителиВидПолномочий.Видимость = Истина;
	Если Сведения.ВидПолномочий = "1" Тогда
		Элементы.ДекорацияРуководителиВидПолномочий.Заголовок = НСтр("ru='* действуют совместно'");
		Элементы.ДекорацияРуководителиВидПолномочий.ЦветТекста = ЦветаСтиля.ЦветТекстаНекорректногоКонтрагента;
	Иначе
		Элементы.ДекорацияРуководителиВидПолномочий.Заголовок = НСтр("ru='* действуют независимо'");
		Элементы.ДекорацияРуководителиВидПолномочий.ЦветТекста = ЦветаСтиля.ЦветТекстаДополнительныеСведения;
	КонецЕсли;
...
&НаСервере
Процедура ВывестиКонтактнуюИнформацию(Сведения)
	
	Если Сведения.ЭлектроннаяПочта = Неопределено Тогда
		Элементы.ДекорацияEmail.Видимость = Ложь;
		Элементы.ДекорацияEmail.Заголовок = "";
	Иначе
		Элементы.ДекорацияEmail.Видимость = Истина;
		Элементы.ДекорацияEmail.Заголовок = Сведения.ЭлектроннаяПочта;
	КонецЕсли;
...
Функция ТекстДекорацииУчредителя(ДанныеУчредителя)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонСтроки,
		СсылкаИНН,
		ПроверкаКонтрагентаКлиентСервер.СократитьНаименование(
...
#Область КарточкаПравопреемники

&НаСервере
Процедура ВывестиКарточкуПравопреемники(Сведения)
	
	Если ЭтоЮридическоеЛицо Тогда
		Элементы.ГруппаПравопреемники.Видимость = Истина;
	Иначе
		Элементы.ГруппаПравопреемники.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ДанныеКарточки = Сведения.КарточкаПравопреемники;
	
	НастроитьСвойстваЭлементовПравопреемники(ДанныеКарточки);
	
	Если Не ДанныеКарточки.ЕстьДанные Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеКарточки.СкрытыеДанные Тогда
		Элементы.ДекорацияПравопреемникиОписание.Заголовок = ДанныеКарточки.Описание;
	Иначе
		ЕстьДанные = Ложь;
		Для Каждого Правопреемник Из ДанныеКарточки.Правопреемники Цикл
			ЕстьДанные = Ложь;
			Если Правопреемник.СкрытыеДанные Тогда
				Продолжить;
			КонецЕсли;
			Элементы.ДекорацияПравопреемник0.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				ТекстСвязаннойОрганизации(Правопреемник));
			ЕстьДанные = Истина;
			Прервать;
		КонецЦикла;
		Элементы.ДекорацияПравопреемник0.Видимость = ЕстьДанные;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьСвойстваЭлементовПравопреемники(ДанныеКарточки)
	...
	Если ДанныеКарточки.СкрытыеДанные Тогда
		Элементы.ДекорацияПравопреемникиОписание.Видимость = Истина;
		Элементы.ДекорацияВсеПравопреемники.Видимость = Ложь;
		Элементы.ДекорацияПравопреемник0.Видимость = Ложь;
	Иначе
		Элементы.ДекорацияПравопреемникиОписание.Видимость = Ложь;
		Если ДанныеКарточки.Правопреемники.Количество() > 1 Тогда
			Элементы.ДекорацияПравопреемники.Заголовок = НСтр("ru='Правопреемники'");
			Элементы.ДекорацияВсеПравопреемники.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Все правопреемники (%1)'"),
				ДанныеКарточки.Правопреемники.Количество());
			Элементы.ДекорацияВсеПравопреемники.Видимость = Истина;
		Иначе
			Элементы.ДекорацияПравопреемники.Заголовок = НСтр("ru='Правопреемник'");
			Элементы.ДекорацияВсеПравопреемники.Видимость = Ложь;
		КонецЕсли;
...
#Область КарточкаПравопредшественники

&НаСервере
Процедура ВывестиКарточкуПравопредшественники(Сведения)
	
	Если ЭтоЮридическоеЛицо Тогда
		Элементы.ГруппаПравопредшественники.Видимость = Истина;
	Иначе
		Элементы.ГруппаПравопредшественники.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ДанныеКарточки = Сведения.КарточкаПравопредшественники;
	
	НастроитьСвойстваЭлементовПравопредшественники(ДанныеКарточки);
	...
	Если ДанныеКарточки.СкрытыеДанные Тогда
		Элементы.ДекорацияПравопредшественникиОписание.Заголовок = ДанныеКарточки.Описание;
	Иначе
		
		ЕстьДанные = Ложь;
		Для Каждого Правопредшественник Из ДанныеКарточки.Правопредшественники Цикл
			ЕстьДанные = Ложь;
			Если Правопредшественник.СкрытыеДанные Тогда
				Продолжить;
			КонецЕсли;
			Элементы.ДекорацияПравопредшественник0.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				ТекстСвязаннойОрганизации(Правопредшественник));
			ЕстьДанные = Истина;
			Прервать;
		КонецЦикла;
		Элементы.ДекорацияПравопредшественник0.Видимость = ЕстьДанные;
...
&НаСервере
Процедура НастроитьСвойстваЭлементовПравопредшественники(ДанныеКарточки)
	...
	Элементы.ДекорацияПравопредшественники.Заголовок = НСтр("ru='Правопредшественник'");
	Элементы.ДекорацияВсеПравопредшественники.Видимость = Ложь;
	
	Если ДанныеКарточки.СкрытыеДанные Тогда
		Элементы.ДекорацияПравопредшественникиОписание.Видимость = Истина;
		Элементы.ДекорацияПравопредшественник0.Видимость = Ложь;
	Иначе
		Элементы.ДекорацияПравопредшественникиОписание.Видимость = Ложь;
		Если ДанныеКарточки.Правопредшественники.Количество() > 1 Тогда
			Элементы.ДекорацияПравопредшественники.Заголовок = НСтр("ru='Правопредшественники'");
			Если ДанныеКарточки.Правопредшественники.Количество() > 2 Тогда
				Элементы.ДекорацияВсеПравопредшественники.Видимость = Истина;
				Элементы.ДекорацияВсеПравопредшественники.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Все правопредшественники (%1)'"),
					ДанныеКарточки.Правопредшественники.Количество());
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиКарточкуВидыДеятельности(Сведения)
	...
	Инкремент = 0;
	Если ДанныеКарточки.ОсновнойВид <> Неопределено Тогда 
	...
	Иначе
		Элементы.ДекорацияВсеВидыДеятельности.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДанныеКарточки.Дополнительные.Количество() + Инкремент);
	КонецЕсли;
...
Процедура НастроитьСвойстваЭлементовВидыДеятельности(ДанныеКарточки)
	...
	Если ДанныеКарточки.ЕстьДанные Тогда
		...
		Элементы.ГруппаОсновнойВидДеятельности.Видимость = (ДанныеКарточки.ОсновнойВид <> Неопределено);
