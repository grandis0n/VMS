...
Процедура НаложенныйПлатежВидОплатыПриИзменении(Элемент)
	...
	Если НаложенныйПлатежВидОплаты > 0 И ГрузВидРасшифровки <> "ПоТоварам" Тогда
		ГрузВидРасшифровки = "ПоТоварам";
		ОбработатьИзменениеРеквизитаФормы("ГрузВидРасшифровки", Истина);
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ВидРасшифровкиГрузовыхМестПриИзменении(Элемент)
	
	ТребуетсяОбработкаНаСервере = ТребуетсяОбработкаНаСервереВидаРасшифровкиГрузовыхМест();
	
	Если НаложенныйПлатежДоступен(ЭтотОбъект) И НаложенныйПлатежВидОплаты > 0 И ГрузВидРасшифровки <> "ПоТоварам" Тогда
		ТекстСообщения = НСтр("ru = 'При использовании наложенного платежа доступна расшифровка только по товарам.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		ГрузВидРасшифровки = "ПоТоварам";
		Возврат;
	КонецЕсли;
	
	Если РазмещениеГрузаРучнойВвод И ТипЗаказа = Кэш.Типызаказов.ИнтернетМагазин И ТребуетсяОбработкаНаСервере Тогда
...
&НаКлиенте
Процедура РазмещениеГрузаЦенаПриИзменении(Элемент)
	ДанныеСтроки = Элементы.РазмещениеГруза.ТекущиеДанные;
	
	Если ДанныеСтроки <> Неопределено Тогда
		// Цена не пересчитывается, т.к. не используется при регистрации заказа в СДЭК (н-р при расшифровке ПоВидам цена отсутствует) 
		ЗначениеНДС = СервисДоставкиКлиент.ЗначениеСтавкиНДС(ДанныеСтроки.СтавкаНДС);
		ДанныеСтроки.Сумма = ДанныеСтроки.Цена * ДанныеСтроки.Количество;
		ДанныеСтроки.СуммаНДС = ДанныеСтроки.Сумма / (100 + ЗначениеНДС) * ЗначениеНДС;
	КонецЕсли;
КонецПроцедуры
...
Асинх Процедура РазмещениеГрузаРучнойВвод(Команда)
	...
	Если ДокументыОснования.Количество() > 0 Тогда
		Если Не РазмещениеГрузаРучнойВвод И ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин Тогда
			ОбработатьИзменениеРеквизитаФормыНаСервере("РазмещениеГрузаРучнойВвод");
		Иначе
...
Процедура ОбработатьНаселенныеПунктыАвтоподбор(Элемент, Кэш, Источник)
	...
	ТекущийПунктПредставление = Кэш.НаселенныеПункты.Получить(ЭтотОбъект[Имя]);
...
Процедура ЗаполнитьПоШаблонуНаСервере(Знач ПараметрыШаблона)
	...
	ШаблонОчищаетПоля = РегистрыСведений.НастройкиОбщиеСервисДоставки.ПолучитьЗначениеНастройки(
		СервисДоставкиКлиентСервер.ИмяНастройкиШаблонОчищаетПоляЗаказа(), ТипГрузоперевозки);
	
	Если ШаблонОчищаетПоля = Неопределено Или Не ШаблонОчищаетПоля Тогда
		СервисДоставки.ОчиститьНезаполненныеПоляШаблона(ПараметрыЗаказа, ТипГрузоперевозки);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	// Запрет изменения данных объявленной стоимости (РазмещениеГрузаСтоимость) в строках дерева "РазмещениеГруза"
	// Только Просмотр = ЭтоГрузовоеМесто И ДокументыОснования.КоличествоСтрок > 0
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	...
	ГруппаОтборовИ = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтборовИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;

	ОтборЭлемента = ГруппаОтборовИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РазмещениеГруза.ЭтоГрузовоеМесто");
	ОтборЭлемента.ПравоеЗначение = Истина;
	
	ОтборЭлемента = ГруппаОтборовИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДокументыОснования.КоличествоСтрок");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Больше;
	ОтборЭлемента.ПравоеЗначение = 0;
	
	// Запрет изменения данных Цена с НДС (РазмещениеГрузаЦена) в строках дерева "РазмещениеГруза"
	// Только Просмотр = ТипЗаказа = Доставка Или ЭтоГрузовоеМесто И ТипЗаказа = ИнтернетМагазин
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РазмещениеГрузаЦена.Имя);
	
	ГруппаОтборовИли = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтборовИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	
	ОтборЭлемента = ГруппаОтборовИли.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТипЗаказа");
	ОтборЭлемента.ПравоеЗначение = 2; // Доставка
	
	ГруппаОтборов1 = ГруппаОтборовИли.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтборов1.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;

	ОтборЭлемента = ГруппаОтборов1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РазмещениеГруза.ЭтоГрузовоеМесто");
	ОтборЭлемента.ПравоеЗначение = Истина; 
	...
	Для Каждого ЭлементРазмещениеГруза Из Элементы.РазмещениеГруза.ПодчиненныеЭлементы Цикл
		...
		Если Не ЭлементРазмещениеГруза = Элементы.РазмещениеГрузаНаименование
			...
			И Не ЭлементРазмещениеГруза = Элементы.РазмещениеГрузаСтоимость
		...
		Если ТипЗнч(ЭлементРазмещениеГруза) = Тип("ГруппаФормы") Тогда
			...
			Для Каждого ЭлементГруппыРазмещениеГруза Из ЭлементРазмещениеГруза.ПодчиненныеЭлементы Цикл
				...
				Если Не ЭлементГруппыРазмещениеГруза = Элементы.РазмещениеГрузаЦена Тогда
					ПолеЭлемента = ЭлементУО_ТолькоПросмотр.Поля.Элементы.Добавить();
					ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ЭлементГруппыРазмещениеГруза.Имя);
				КонецЕсли;
			КонецЦикла;
...
Процедура УстановитьВидимостьДоступностьНаложенныйПлатеж(Знач Форма)
	...
	ОсновноеПодвалВидимость = НаложенныйПлатежДоступен(Форма);
	...
	Если ОсновноеПодвалВидимость И Форма.НаложенныйПлатежВидОплаты > 0 Тогда
		УстановитьСуммуНаложенногоПлатежа(Форма);
		Элементы.РазмещениеГрузаСумма.Заголовок = НСтр("ru = 'Сумма с НДС (Наложенный платеж)'");
		Элементы.РазмещениеГрузаЦена.Видимость = Истина;
	Иначе
		Форма.СуммаНаложенногоПлатежа = 0;
		Элементы.РазмещениеГрузаСумма.Заголовок = НСтр("ru = 'Сумма с НДС'");
		Элементы.РазмещениеГрузаЦена.Видимость = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция НаложенныйПлатежДоступен(Форма)
	Возврат Форма.Кэш.ФункциональныеОпции.ИспользоватьНаложенныеПлатежи
		И Форма.ТипЗаказа = Форма.Кэш.ТипыЗаказов.ИнтернетМагазин
		И Не Форма.НаложенныйПлатежЗапрещенПоДокументуОснованию;
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСуммуНаложенногоПлатежа(Форма)
...
Процедура ОбработатьИзменениеРеквизитаФормыНаСервере(ИмяРеквизита, Отказ = Ложь)
	...
	ИначеЕсли ИмяРеквизита = "ГрузВидРасшифровки" Или ИмяРеквизита = "РазмещениеГрузаРучнойВвод" Тогда
...
Процедура ОбработатьНаселенныеПункты(Источник)
	...
	Если Источник.Свойство("ОтправительНаселенныйПункт") И ТипЗнч(Источник.ОтправительНаселенныйПункт) = Тип("Структура") Тогда
	...
	Если Источник.Свойство("ПолучательНаселенныйПункт") И ТипЗнч(Источник.ПолучательНаселенныйПункт) = Тип("Структура") Тогда
...
Процедура РассчитатьПоказателиГрузовыхМест(ТолькоИзмененные = Ложь, Показатели = "", КлючОперации = "")
	...
	Если ДокументыОснования.Количество() > 0 И ИменаКолонокДляПересчета.Найти("Стоимость") = Неопределено Тогда
...
Процедура РассчитатьСводнуюИнформациюПоРаспределениюГруза()
	...
	Если ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин Тогда
		ПолнаяСтоимость = ГрузСуммаСНДС;
		Если НаложенныйПлатежДоступен(ЭтотОбъект) И НаложенныйПлатежВидОплаты > 0 Тогда
			УстановитьСуммуНаложенногоПлатежа(ЭтотОбъект);
		КонецЕсли;
	Иначе
...
Процедура УстановитьОформлениеШапкиРазмещениеГруза()
	...
	Для Каждого ОписаниеКолонки Из АвтопересчетСписокКолонок Цикл
		...
		Если ОписаниеКолонки.Пометка И Не РазмещениеГрузаРучнойВвод Тогда
	...
	Если ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин Тогда
		Если Не РазмещениеГрузаРучнойВвод Тогда
			Элементы.РазмещениеГрузаЦена.КартинкаШапки = БиблиотекаКартинок.Калькулятор;
		Иначе
			Элементы.РазмещениеГрузаЦена.КартинкаШапки = БиблиотекаКартинок.Изменить;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьТолькоПросмотрПоляРазмещениеГруза()
	...
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		...
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаВес);
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаДлина);
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаШирина);
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаВысота);
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаСтоимость);
		РазрешенныеПоля.Добавить(Элементы.РазмещениеГрузаЦена);
		
		Если РазрешенныеПоля.Найти(Поле) <> Неопределено Тогда
			
			Если (Поле = Элементы.РазмещениеГрузаСтоимость Или Поле = Элементы.РазмещениеГрузаЦена)
				И ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин Тогда
