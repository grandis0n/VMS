...
&НаСервере
Функция ПечатныеБланкиРегламентированногоОтчета()
	Идентификаторы = Новый Массив;
	Для Каждого Лист Из СписокЛистов.ПолучитьЭлементы() Цикл
		Если Лист.Пометка Тогда
			Идентификаторы.Добавить(Лист.Идентификатор);
		КонецЕсли;
	КонецЦикла;
	...
	|ГДЕ
	|	ПечатныеБланкиРегламентированныхОтчетов.УникальныйИдентификатор В (&Идентификаторы)";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл 
			...
			ПечатныеБланки.Вставить(Выборка.УникальныйИдентификатор, СтруктураВыборки);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПечататьСразу() Экспорт
	...
	ПечатныеБланкиРегламентированногоОтчета = ПечатныеБланкиРегламентированногоОтчета();
	...
	Для Каждого Эл Из СписокПечатаемыхЛистов Цикл
		...
		СтруктураПараметров.Вставить("ПечатныеБланкиРегламентированногоОтчета", ПечатныеБланкиРегламентированногоОтчета);
...
Процедура ДобавитьТабличныеДокументыВПакетОтображаемыхДокументов(ПакетТаблДок, СтруктураПараметров)
	ПечатныеБланкиРегламентированногоОтчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		СтруктураПараметров, "ПечатныеБланкиРегламентированногоОтчета", Новый Соответствие);
	...
	Если НЕ ЗначениеЗаполнено(УИДВременногоХранилища) Тогда
		УИДВременногоХранилища = УникальныйИдентификатор;
	КонецЕсли;
	...
	Если СписокПечатаемыхЛистов.Количество() > 0
		...
		ПакетТаблДок.Состав.Добавить(
			...
			ПечатныеБланкиРегламентированногоОтчета[СтруктураПараметров.Идентификатор]),
	...
	Иначе
		...
		Если УИДВременногоХранилища <> УникальныйИдентификатор Тогда
...
Функция ПоместитьЛистыВоВременноеХранилищеНаСервере(ВФормате,
	...
	ПечатныеБланкиРегламентированногоОтчета = ПечатныеБланкиРегламентированногоОтчета();
	Если ВФормате = ТипФайлаТабличногоДокумента.PDF ИЛИ ВФормате = ТипФайлаТабличногоДокумента.XLSX Тогда
		ПакетТаблДок = Новый ПакетОтображаемыхДокументов;
		...
		Если ПакетТаблДок.Состав.Количество() > 0 Тогда
			...
			АдресаЛистовВоВременномХранилище.Добавить(Новый Структура("АдресВоВременномХранилище, Наименование",
				ПоместитьВоВременноеХранилище(ФайлЛиста, УникальныйИдентификатор), 
		...
		Для Каждого Лист Из СписокЛистов.ПолучитьЭлементы() Цикл
			Если Лист.Пометка Тогда
				...
				Если СписокПечатаемыхЛистов.Количество() > 0
					...
					ТаблДок = ТабличныйДокументПечатногоБланкаРегламентированногоОтчета(
							ПечатныеБланкиРегламентированногоОтчета[Лист.Идентификатор]);
				Иначе
