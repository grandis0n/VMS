...
Процедура НастроитьСписок(ПервичнаяНастройка)
	...
	//Установка отбора по виду продукции ИС
	Если ЭтаФорма.Параметры.Свойство("Отбор") 
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма.Параметры.Отбор, "ВидПродукцииИС") 
		И ЭтаФорма.Параметры.Отбор.ВидПродукцииИС.Количество() > 0 Тогда
		ВидПродукцииИС = Новый ФиксированныйМассив(Параметры.Отбор.ВидПродукцииИС);
		СобытияФормИСПереопределяемый.ПереопределитьТаблицуПараметровДинамическогоСпискаВыбораНоменклатуры(ЭтаФорма, "СписокНоменклатуры", ВидПродукцииИС);
	КонецЕсли;
    //Конец Установка отбора по виду продукции ИС
