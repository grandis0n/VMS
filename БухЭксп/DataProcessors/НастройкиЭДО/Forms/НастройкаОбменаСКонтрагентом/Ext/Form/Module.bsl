...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтправительФизЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(Организация).ЭтоФизЛицо;
...
Процедура ИдентификаторПолучателяПриИзменении(Элемент)
	...
	ОбработатьИзменениеИдентификатораПолучателя();
...
Процедура ПодписьПоМаршрутуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не НастройкаДокумента Тогда
...
&НаКлиенте
Процедура ИдентификаторПолучателяНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтруктураПараметров = Новый Структура("ИдентификаторыКонтрагента, Контрагент, ИдентификаторОрганизации", 
		Элементы.ИдентификаторПолучателя.СписокВыбора, Контрагент, ИдентификаторОтправителя);
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПолучитьИдентификаторПолучателя", ЭтотОбъект);
	
	ОткрытьФорму("РегистрСведений.АбонентыЭДО.Форма.ФормаВыбора", СтруктураПараметров, ЭтотОбъект,,,,
...
Процедура Готово(Команда)
	...
	Если ТребуетсяОтправкаПриглашения(ЭтотОбъект) Или СоздаватьАвтоматическоеПриглашение Тогда
		...
		Если СоздаватьАвтоматическоеПриглашение Тогда
			
			СоздатьАвтоматическоеПриглашение();
			Возврат;
	...
	ИначеЕсли (ПредварительнаяНастройка И ПроверкаОтправкаПриглашений = "Отправлено"
		...
		СоздатьНастройкиОтправкиЭДО(Организация, Контрагент, ДоговорКонтрагента, ИдентификаторОтправителя, 
			ИдентификаторПолучателя, Отказ);
...
&НаКлиенте
Процедура СоздатьАвтоматическоеПриглашение()
	...
	Приглашения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Приглашение);
	Контекст = Новый Структура("Приглашения", Приглашения);
	
	Оповещение = Новый ОписаниеОповещения("СоздатьАвтоматическоеПриглашениеПослеОтправкиВЦентрЭДО", ЭтотОбъект, Контекст);
	...
	Если ИспользоватьЦентрЭДО Тогда
		...
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДОКлиент.СформироватьАвтоматическиеПриглашения(Оповещение, Приглашения);
		
	Иначе
		
		КонтекстДиагностики = ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики();
		СтатусыПриглашений = 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("Статус", СтатусПриглашения));
		РезультатПодготовки = Новый Структура("КонтекстДиагностики, Обработано, СтатусыПриглашений", 
			КонтекстДиагностики, 1, СтатусыПриглашений);
		ВыполнитьОбработкуОповещения(Оповещение, РезультатПодготовки);
...
&НаКлиенте
Процедура СоздатьАвтоматическоеПриглашениеПослеОтправкиВЦентрЭДО(РезультатПодготовки, Контекст) Экспорт
	
	Отказ = Ложь;
	
	Если РезультатПодготовки.Обработано = 0 Тогда
		
		Отказ = Истина;
		
	Иначе
		
		СтатусПриглашенияВЦентреЭДО = РезультатПодготовки.СтатусыПриглашений[0];
		ПараметрыПриглашения = Контекст.Приглашения[0];
		
		Если Не СтатусПриглашенияВЦентреЭДО.Статус = 
			ПредопределенноеЗначение("Перечисление.СтатусыПриглашений.Принято") 
			И ПараметрыПриглашения.ИспользоватьЦентрЭДО Тогда
				
			ТекстВопроса = НСтр("ru = 'По данным ""Центра ЭДО"" абонент требует отправку приглашения перед началом ЭДО.
			| Отправить приглашение?'");
			
			ОписаниеОповещения = 
				Новый ОписаниеОповещения("ПродолжитьПослеВопросаОбОтправкеПриглашения", ЭтотОбъект, ПараметрыПриглашения);
		...
		Иначе
			...
			СоздатьАвтоматическоеПриглашениеИНастройкиОтправки(ПараметрыПриглашения, ДоговорКонтрагента, Отказ);
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось создать автоматическое приглашение и настройки отправки.
		|Подробнее см. в журнале регистрации'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПродолжитьПослеВопросаОбОтправкеПриглашения(РезультатВопроса, Приглашение) Экспорт
...
&НаСервереБезКонтекста
Процедура СоздатьАвтоматическоеПриглашениеИНастройкиОтправки(ПараметрыПриглашения, Знач ДоговорКонтрагента, Отказ)
	
	ПриглашенияЭДО.СоздатьАвтоматическоеПриглашенияПоКлючевымРеквизитам(ПараметрыПриглашения, Отказ);
			
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	СоздатьНастройкиОтправкиЭДО(ПараметрыПриглашения.Организация, ПараметрыПриглашения.Контрагент, ДоговорКонтрагента, 
		ПараметрыПриглашения.ИдентификаторОрганизации, ПараметрыПриглашения.ИдентификаторКонтрагента, Отказ);
...
Процедура ОбработатьРезультатыНастройкиСвязиСКонтрагентом(АдресРезультата)
	...
	Если ЗначениеЗаполнено(ПараметрыСвязиИдентификаторов) Тогда
		...
		Иначе 
			ПараметрыПриглашения = НовыеПараметрыПриглашения(ПараметрыСвязиИдентификаторов, ОтправительФизЛицо, ПолучательФизЛицо);
		КонецЕсли;
...
Процедура ОбновитьДанныеПриглашенияКонтрагента(Форма, ПараметрыПриглашения = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыПриглашения.СтатусПриглашенияПредставление) Тогда
		Форма.СтатусПриглашенияПредставление = Новый ФорматированнаяСтрока(ПараметрыПриглашения.СтатусПриглашенияПредставление);
	КонецЕсли;
	
	Форма.НавигационнаяСсылкаНаПриглашение   = ПараметрыПриглашения.НавигационнаяСсылкаНаПриглашение;
	Форма.СоздаватьАвтоматическоеПриглашение = ПараметрыПриглашения.АбонентНеТребуетПриглашений;
	
	Если Форма.КлючНастроекОтправки.ВсегдаСоздаватьАвтоматическоеПриглашение Тогда
		Форма.СоздаватьАвтоматическоеПриглашение = Истина;
	КонецЕсли;
...
Функция ПолучитьПараметрыПриглашения(Форма, ФормироватьСписокВыбора = Истина)
	...
	ПараметрыПриглашения = ПолучитьПараметрыПриглашенияНаСервере(ПараметрыПолученияСвязи, Форма.ОтправительФизЛицо, Форма.ПолучательФизЛицо);
...
&НаСервереБезКонтекста
Функция ПолучитьПараметрыПриглашенияНаСервере(Знач ПараметрыПолучения, ОтправительФизЛицо, ПолучательФизЛицо)
...
&НаСервереБезКонтекста
Функция НовыеПараметрыПриглашения(ПараметрыСвязиИдентификаторов, ОтправительФизЛицо, ПолучательФизЛицо)
	...
	ПроверитьВозможностьРоуминга(ПараметрыПриглашения, СтрокаПараметровСвязи, ОтправительФизЛицо, ПолучательФизЛицо);
	...
	ПараметрыПриглашения.АбонентНеТребуетПриглашений = СтрокаПараметровСвязи.АбонентНеТребуетПриглашений;
...
&НаСервереБезКонтекста
Процедура ПроверитьВозможностьРоуминга(ПараметрыПриглашения, ПараметрыСвязиИдентификаторов, ОтправительФизЛицо, ПолучательФизЛицо)
	...
	АктуальнаяВерсияОтправителя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Лев(ПараметрыСвязиИдентификаторов.ИдентификаторОрганизации, 3));
	АктуальнаяВерсияПолучателя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Лев(ПараметрыСвязиИдентификаторов.ИдентификаторКонтрагента, 3));
	...
	ОбменСФизЛицамиНеПоддерживается = (ОтправительФизЛицо Или ПолучательФизЛицо)
	...
	ИначеЕсли ОбменСФизЛицамиНеПоддерживается Тогда
		ПараметрыПриглашения.ПроверкаВозможностьРоуминга = "Ошибка";
...
Функция ТребуетсяОтправкаПриглашения(Форма)
	...
	Если Форма.СоздаватьАвтоматическоеПриглашение Тогда
		ТребуетсяОтправка = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ПолучитьИдентификаторПолучателя(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ИдентификаторПолучателя = Результат;
...
&НаКлиенте
Процедура ОбработатьИзменениеИдентификатораПолучателя()
