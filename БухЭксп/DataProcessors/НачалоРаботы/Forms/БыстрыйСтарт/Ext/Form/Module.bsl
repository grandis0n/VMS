...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбучениеДоступно = Обработки.ОбучениеРаботеВБизнесСтарт.ОбучениеДоступно();
...
Процедура ПерейтиНаСтраницу(СтраницаПерехода)
	...
	Иначе
		...
		ИначеЕсли АктивнаяСтраница = "СтраницаПереходИзДругихСистем" Тогда
			...
			ОткрытьОбучение = ОбучениеДоступно;
		КонецЕсли;
...
Процедура ЗавершитьРаботуПомощника()
	...
	Если ОткрытьОбучение Тогда
		УстановитьПоказыватьОбучениеПриЗапуске();
...
Процедура ВариантНачалаРаботыПриИзменении(Элемент)
	...
	ОткрытьОбучение = ВариантНачалаРаботы <> "ЛокальнаяВерсия1С" И ОбучениеДоступно;
	УправлениеФормой(ЭтотОбъект);
...
Процедура НастроитьСтраницуПереходИзДругихСистем(Форма)
	...
	Если Форма.ВариантНачалаРаботы = ИмяВариантаДругаяУчетнаяСистема() Тогда
		Элементы.ДругаяУчетнаяСистема.Видимость = Истина;
	Иначе
		Элементы.ДругаяУчетнаяСистема.Видимость = Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура УстановитьПоказыватьОбучениеПриЗапуске()
	
	Обработки.ОбучениеРаботеВБизнесСтарт.СохранитьНастройку("Показывать", Истина);
