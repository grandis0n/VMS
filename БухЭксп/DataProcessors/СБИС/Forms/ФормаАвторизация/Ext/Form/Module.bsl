...
Процедура СохранитьПараметрыАвторизации()
	...
	Если НЕ (ЗапомнитьПароль ИЛИ ЗапомнитьСертификат) Тогда 
		Кэш.ТекущийСеанс.Вставить("ПараметрыПереходаНаExtSDK2", Новый Структура("Пароль, Сертификат", Пароль, Кэш.Парам.Сертификат));
	КонецЕсли;
	Если Не ЗапомнитьПароль Тогда
		Кэш.Парам.Пароль="";
	Иначе
