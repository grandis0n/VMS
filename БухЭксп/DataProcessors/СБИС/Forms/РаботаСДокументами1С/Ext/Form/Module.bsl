...
Функция ПолучитьСторону(Кэш, знач Контекст) Экспорт
	...
	Если Не Кэш.ТекущийСеанс.РасчитанныеЗначения.Свойство("мСторона") Тогда
		Кэш.ТекущийСеанс.РасчитанныеЗначения.Вставить("мСторона", Новый Структура);
	КонецЕсли;
	
	Кэш.ТекущийСеанс.РасчитанныеЗначения.мСторона.Вставить("_" + ИНН + "_" + КПП, 
			Новый Структура("ИспользоватьКакПолучателя", РассчитатьЗначение("ИспользоватьКакПолучателя",Контекст,Кэш) = Истина));
...
Функция ПолучитьАдресДоставкиУП(Контекст) Экспорт
	...
	Если Контекст.Свойство("АдресДоставкиЗначение") И ЗначениеЗаполнено(Контекст.АдресДоставкиЗначение) Тогда
...
Функция сбисОпределитьРеквизитНоменклатурыКонтрагента(Контекст, ИмяРеквизита) Экспорт
	...
	Если ЗначениеЗаполнено(ГлавноеОкно.Кэш) Тогда
		...
		Попытка
			стрКонтрагент = сбисЗаполнитьСтруктуруКонтрагента(Контекст.Контрагент);
			стрНоменклатура = Новый Структура("Номенклатура,Характеристика", Контекст.Номенклатура,?(Контекст.Свойство("Характеристика"),Контекст.Характеристика,Неопределено));
			Возврат фрм.сбисПолучитьРеквизитНоменклатурыПоставщика(стрКонтрагент, стрНоменклатура, ИмяРеквизита, ГлавноеОкно.КаталогНастроек, ГлавноеОкно.Кэш.Ини);	
		Исключение
			Возврат "";		
		КонецПопытки;
	Иначе
...
КонецФункции
&НаКлиенте
Функция НайтиКонтрагентаИзДокументаСБИС(Ини, оУчастник, ИниДок = Неопределено, МножественныйВыбор = Ложь, СообщитьЕслиНеНайден = Ложь) Экспорт
...
Процедура ДобавитьЗаписиВПланВидовХарактеристик(ИниКонфигурация) Экспорт
	...
	Если Метаданные.ПланыВидовХарактеристик.Найти(ИмяПланаВидовХарактеристикСвойствОбъектов) = Неопределено Тогда
		Сообщить("В конфигурации отсутствует план видов характеристик "+ИмяПланаВидовХарактеристикСвойствОбъектов+". Невозможно создать дополнительные свойства для документов и контрагентов. Часть функционала внешней обработки будет недоступна (хранение статусов электронных документов, отправка документов на определенный код филиала, фильтрация документов по способу доставки)");	
		Возврат;	
	КонецЕсли;  
	Отказ = Ложь;
	сбисДобавитьЗаписьВПланВидовХарактеристик("ДокументСБИС_Ид", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристик("ДокументСБИС_ИдВложения", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристик("ДокументСБИС_Статус", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристик("ДокументСБИС_СтатусГос", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);	// alo СтатусГос
	сбисДобавитьЗаписьВПланВидовХарактеристик("ДокументСБИС_ТекущийЭтап", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристикПоКонтрагенту("СБИС_СпособДоставки", ИмяПланаВидовХарактеристикСвойствОбъектов, ИниКонфигурация, "СправочникСвойствОбъектов", Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристикПоКонтрагенту("СБИС_КодФилиала", ИмяПланаВидовХарактеристикСвойствОбъектов, ИниКонфигурация, "Строка", Отказ);
	сбисДобавитьЗаписьВПланВидовХарактеристикПоКонтрагенту("СБИС_ЗапретРедакций", ИмяПланаВидовХарактеристикСвойствОбъектов, ИниКонфигурация, "Булево", Отказ);
	...
	Если ИниКонфигурация.Свойство("СтатусРегламент") Тогда
		МассивСтатусРегламент = ?( ИниКонфигурация.Свойство("СтатусРегламент"),РазбитьСтрокуВМассив(ИниКонфигурация.СтатусРегламент.Значение,","),Новый Массив );
		Для Каждого Элемент из МассивСтатусРегламент Цикл
			сбисДобавитьЗаписьВПланВидовХарактеристик(Элемент+"ДокументСБИС_Ид", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
			сбисДобавитьЗаписьВПланВидовХарактеристик(Элемент+"ДокументСБИС_ИдВложения", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
			сбисДобавитьЗаписьВПланВидовХарактеристик(Элемент+"ДокументСБИС_Статус", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);
			сбисДобавитьЗаписьВПланВидовХарактеристик(Элемент+"ДокументСБИС_СтатусГос", ИмяПланаВидовХарактеристикСвойствОбъектов,, Отказ);	// alo СтатусГос
		КонецЦикла;
	КонецЕсли;
	//<-- KES 050751151 Статусы в разрезе регламентов (ОБРАБОТКА НАСТРОЙКИ КОНФИГУРАЦИИ)      
	
	Если Отказ = Истина Тогда
		Сообщить("Возникли проблемы при создании дополнительных свойств для документов и контрагентов. Часть функционала внешней обработки будет недоступна (хранение статусов электронных документов, отправка документов на определенный код филиала, фильтрация документов по способу доставки). Для корректного создания дополнительных свойств необходимо провести первичный запуск обработки под пользователем с правами на план видов характеристик "+ИмяПланаВидовХарактеристикСвойствОбъектов+". Для этого обратитесь к вашему администратору 1С.");	
	КонецЕсли;
...
Функция ПодготовитьСтруктуруДокумента1С(СтрокаСпискаДокументов, Кэш) Экспорт
	...
	Для Каждого Пакет Из Контекст.МассивПакетов Цикл
		...
		Если Пакет.Свойство("Конверт") Тогда
			...
			Если	Не	ПолныйСоставПакета.Свойство("НеЗапускатьВДокументооборот")
				Или		ПолныйСоставПакета.НеЗапускатьВДокументооборот = Неопределено Тогда
				ПолныйСоставПакета.Вставить("НеЗапускатьВДокументооборот",	НеЗапускатьВДокументооборот);
			КонецЕсли;
		КонецЕсли;
		Если Пакет.Свойство("Ошибка") Тогда
			ПолныйСоставПакета.Вставить("Ошибка", СбисИсключение(Пакет.Ошибка, "РаботаСДокументами1С.ПодготовитьСтруктуруДокумента1С"));	
		КонецЕсли;
		...
		Для Каждого Вложение Из Пакет.Вложение Цикл
			...
			ИначеЕсли Вложение.Свойство("СтруктураДокумента") Тогда // формируем xml документов
				...
				Если ГенераторВключенДляДокумента(Кэш, Вложение) Тогда
					ШаблонXML = "";
					...
					Если Не фрм = Ложь Тогда
						...
						РезультатПодставновки = фрм.ПрименитьПодстановку(Кэш, Вложение, Новый Структура, ОшибкаФормирования);
						Если ОшибкаФормирования Тогда
							ПолныйСоставПакета.Вставить("Ошибка", СбисИсключение(РезультатПодставновки, "РаботаСДокументами1С.ПодготовитьСтруктуруДокумента1С"));
							Прервать;
						Иначе
		...
		Если Кэш.ДопВложение.Количество()>0 Тогда
			...
			Конеццикла;
		КонецЕсли;
		//Тут пока неразбериха с ошибками. Нужен рефактор для единой обработки исключений при формировании пакета
		Если ПолныйСоставПакета.Свойство("Ошибка") Тогда
			//В пакете ошибка. Дальнейшее выполнение не имеет смысла
			Если Кэш.Свойство("РезультатОтправки") Тогда
				Кэш.РезультатОтправки.НеСформировано = Кэш.РезультатОтправки.НеСформировано + 1;
				Кэш.РезультатОтправки.ОшибкиДоОтправки = Кэш.РезультатОтправки.ОшибкиДоОтправки + 1;
				ДобавитьОшибкуВРезультатОтправки(Кэш, ПолныйСоставПакета.Ошибка.message, ПолныйСоставПакета.Ошибка.details, СтрокаСпискаДокументов.СоставПакета[0].Значение, ПолныйСоставПакета.Ошибка.code);
			Иначе
				Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, ПолныйСоставПакета.Ошибка);
			КонецЕсли;
			Продолжить;
		ИначеЕсли	Не МассивВложений.Количество() Тогда
			...
			ОшибкаПодготовки = СбисИсключение(, "РаботаСДокументами1С.ПодготовитьСтруктуруДокумента1С",726, "Документ не сформирован", "В документе "+строка(СтрокаСпискаДокументов.СоставПакета[0].Значение)+" не заполнены табличные части. Электронный документ не сформирован.");
			Если Кэш.Свойство("РезультатОтправки") Тогда
				Кэш.РезультатОтправки.НеСформировано = Кэш.РезультатОтправки.НеСформировано+1;
				ПолныйСоставПакета.Вставить("Ошибка", ОшибкаПодготовки);
				Кэш.РезультатОтправки.ОшибкиДоОтправки = Кэш.РезультатОтправки.ОшибкиДоОтправки + 1;
			...
			Иначе
				Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, ОшибкаПодготовки, Новый Структура("Отправлять", Ложь));
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		...
		Если Пакет.Свойство("Конверт") Тогда  // если нет структуры конверта, то берем все с первого вложения
			Продолжить;
		КонецЕсли;
		...
		Если	ОснованиеДляПакета.Свойство("СтруктураФайла",					СтрГрузополучатель)
			...
			ПолучательИНН = ?(СтрПолучатель.Свойство("СвЮЛ") и СтрПолучатель.СвЮЛ.Свойство("ИНН"), СтрПолучатель.СвЮЛ.ИНН, ?(СтрПолучатель.Свойство("СвФЛ") и СтрПолучатель.СвФЛ.Свойство("ИНН"), СтрПолучатель.СвФЛ.ИНН, "")); 
			ГрузополучательИНН = ?(СтрГрузополучатель.Свойство("СвЮЛ") и СтрГрузополучатель.СвЮЛ.Свойство("ИНН"), СтрГрузополучатель.СвЮЛ.ИНН, ?(СтрГрузополучатель.Свойство("СвФЛ") и СтрГрузополучатель.СвФЛ.Свойство("ИНН"), СтрГрузополучатель.СвФЛ.ИНН, "")); 
			ПолучательКПП = ?(СтрПолучатель.Свойство("СвЮЛ") и СтрПолучатель.СвЮЛ.Свойство("КПП"), СтрПолучатель.СвЮЛ.КПП, ""); 
			ГрузополучательКПП = ?(СтрГрузополучатель.Свойство("СвЮЛ") и СтрГрузополучатель.СвЮЛ.Свойство("КПП"), СтрГрузополучатель.СвЮЛ.КПП, "");  
			ПолучательКодФилиала = ?(СтрПолучатель.Свойство("СвЮЛ") и СтрПолучатель.СвЮЛ.Свойство("КодФилиала"), СтрПолучатель.СвЮЛ.КодФилиала, ?(СтрПолучатель.Свойство("СвФЛ") и СтрПолучатель.СвФЛ.Свойство("КодФилиала"), СтрПолучатель.СвФЛ.КодФилиала, "")); 
			ГрузополучательКодФилиала = ?(СтрГрузополучатель.Свойство("СвЮЛ") и СтрГрузополучатель.СвЮЛ.Свойство("КодФилиала"), СтрГрузополучатель.СвЮЛ.КодФилиала, ?(СтрГрузополучатель.Свойство("СвФЛ") и СтрГрузополучатель.СвФЛ.Свойство("КодФилиала"), СтрГрузополучатель.СвФЛ.КодФилиала, "")); 
			...
			КлючСтороны = "_" + ПолучательИНН + "_" + ПолучательКПП;
			Если Не (Кэш.ТекущийСеанс.РасчитанныеЗначения.Свойство("мСторона")
						И Кэш.ТекущийСеанс.РасчитанныеЗначения.мСторона.Свойство(КлючСтороны)
						И Кэш.ТекущийСеанс.РасчитанныеЗначения.мСторона[КлючСтороны].Свойство("ИспользоватьКакПолучателя")
						И Кэш.ТекущийСеанс.РасчитанныеЗначения.мСторона[КлючСтороны].ИспользоватьКакПолучателя = Истина)
		...
		Если ЗначениеЗаполнено(ПодтипПакета) Тогда
			ПолныйСоставПакета.Вставить("Подтип", ПодтипПакета);	
		КонецЕсли;
	...
	Кэш.ТекущийСеанс.РасчитанныеЗначения.Удалить("мСторона");
	...
	Если ПараметрыНоменклатуры.НадоСопоставлять и ЗначениеЗаполнено(КонтрагентДляСопоставления) Тогда
		...
		Если ЗначениеЗаполнено(ТаблДок) Тогда
			...
			Для Каждого СтрТабл Из ТаблДок Цикл
				Если НЕ ТаблДок[счСтр].Свойство("НеЗагружать") Тогда
...
Функция СоздатьПерезаполнитьДокументы(Кэш,СоставПакета,СписокВложений,Параметры,ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТаблДок из СписокВложений Цикл
		...
		КонецЕсли;
		Если СтрокаТаблДок.Отмечен = Ложь Тогда
			Продолжить;			
		КонецЕсли;
...
Функция ЗагрузитьУпорядоченныйСоставПакетаВыбраннымСпособом(РезультатВопроса, ПараметрыЗагрузки) Экспорт	
	...
	Для Каждого Вложение Из УпорядоченныйСоставПакета Цикл
		...
		Иначе
			...
			Если МассивОснований.Найти(Документ1С) = Неопределено Тогда
...
Функция сбисРассчитатьЦену(Контекст) Экспорт
	Попытка
		...
		Если Не ЗначениеЗаполнено(СуммаСНДС) Тогда
			...
			СуммаСНДС = Число(?(СтрокаФайла.Свойство("СуммаСНДС"), СтрокаФайла.СуммаСНДС, ?(СтрокаФайла.Свойство("Сумма"), СтрокаФайла.Сумма, СтрокаФайла.СуммаБезНал)));
		КонецЕсли;
...
Функция сбисЗаполнитьДанныеИсходногоДокументаНаСервере(Знач Контекст, Отказ)
	...
	ДокументПоступленияДата = сбисСтрокаВДату(Контекст.ДокументПоступленияДата);
	ДокументПоступленияНомер = Контекст.ДокументПоступленияНомер;
	Если Контекст.Файл_Формат = "счФактКор" или Контекст.Файл_Формат = "УкдКсчфДис" Тогда
		ТекстЗапроса = "ВЫБРАТЬ
		|	СчетФактураПолученный.Ссылка КАК Ссылка,
...
Функция СбисПроверитьНаличиеОбновленияОбработки(Кэш, ПараметрыОбновления, Отказ)
	...
	Если СтатусВерсииПользователя = 0 Тогда    								// Версия актуальна
		Кэш.ГлавноеОкно.ПодключитьОбработчикОжидания("сбисПроверитьНаличиеОбновленийАвто", 60*60, Истина);
...
Процедура СбисДействияПриОбновленииВерсии(Кэш, СтараяВерсия, НоваяВерсия, Отказ) Экспорт
	...
	Если ЭтоНоваяВерсия("2.47", СтараяВерсия) и нрег(Кэш.ПараметрыСистемы.Конфигурация.Имя) = "бухгалтерияпредприятия" и ЭтоНоваяВерсия(Кэш.ПараметрыСистемы.Конфигурация.Версия, "3.0.105.30") Тогда 
		сбисПереносНоменклатурыПоставщиковВНоменклатуруКонтрагентовНаСервере();
	КонецЕсли;
	//Если ЭтоНоваяВерсия("2.48", СтараяВерсия) Тогда
	//	ПараметрыПереключения = Новый Структура("Отказ, Кэш", Ложь, Кэш);
	//	Кэш.ГлавноеОкно.СбисПоказатьСостояние("Переносим настройки в СБИС...", Кэш.ГлавноеОкно);
	//	РезультатОбновления = Кэш.ФормаНастроекОбщее.ПереключитьСпособХраненияНастроекВСбис(,ПараметрыПереключения);
	//	Кэш.ГлавноеОкно.СбисСпрятатьСостояние(Кэш.ГлавноеОкно);
	//	Если ПараметрыПереключения.Отказ Тогда
	...
	Если Отказ Тогда
		СбисПараметрыСтатистики = Новый Структура("Действие, Ошибка", "Запись ошибки", РезультатДобавленияПФ);
		Кэш.ОбщиеФункции.сбисСтатистика_СформироватьИЗаписатьСтатистикуНаСервис(Кэш, СбисПараметрыСтатистики, Ложь);
		Отказ = Ложь;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура СбисДействияПриПервомЗапуске(Кэш, Версия, Отказ) Экспорт
	РезультатДобавленияПФ = Кэш.ФормаНастроекОбщее.СбисДобавитьПечатныеФормы(Кэш, Отказ);
	Если Отказ Тогда
		СбисПараметрыСтатистики = Новый Структура("Действие, Ошибка", "Запись ошибки", РезультатДобавленияПФ);
		Кэш.ОбщиеФункции.сбисСтатистика_СформироватьИЗаписатьСтатистикуНаСервис(Кэш, СбисПараметрыСтатистики, Ложь);
...
&НаКлиенте
Процедура ИзменитьСпособОбменаПриОбновлении(Кэш, СбисДополнительныеПараметры=Неопределено)
	
	СпособыПерехода = ПолучитьСоответствиеСпособовОбменаДляПерехода();
	ДанныеПерехода = СпособыПерехода.Получить(Кэш.Парам.СпособОбмена);
	Если ДанныеПерехода <> Неопределено Тогда
		Если Кэш.Парам.СпособОбмена <> ДанныеПерехода.СпособОбмена Тогда
			Кэш.Парам.СпособОбмена = ДанныеПерехода.СпособОбмена;
			Кэш.ТекущийСеанс.Вставить("ВыполненПереходНаExtSDK2", Истина);
			Сообщить ("Выполнен переход на способ обмена: " + ДанныеПерехода.Название + ".");
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьСоответствиеСпособовОбменаДляПерехода() Экспорт
	
	СоответствиеСпособовОбмена = Новый Соответствие;
	СоответствиеСпособовОбмена.Вставить(4, Новый Структура("СпособОбмена, Название, НазваниеДо", 6, "ExtSDK2", "ExtSDK"));
	СоответствиеСпособовОбмена.Вставить(5, Новый Структура("СпособОбмена, Название, НазваниеДо", 7, "ExtSDK2Crypto", "ExtSDKCrypto"));
	
	Возврат СоответствиеСпособовОбмена;
...
Функция СбисПолучитьСписокЗадач(Кэш) Экспорт
	...
	Для сч = 0 По Размер - 1 Цикл
		...
		НоваяСтр.Вставить("Статус",			сбисИндексКартинкиПоСтатусуЭД(Строка(оДокумент.Состояние.Код)));
...
Функция СбисЗаменитьНедопустимыеСимволы(СтрГдеЗаменить) Экспорт
	Возврат	СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрГдеЗаменить,".","_"), " ", ""),"-", "_"),"+", "_");
КонецФункции
...
// Функция ПОЧТИ идентична предыдущей, но пришлось написать свою из-за логических изменений
&НаКлиенте
Функция ЭтоНестабильнаяВерсия(ТекущаяВерсия, НестабильнаяВерсия) Экспорт
	// Функция проверяет, является ли текущая версия обработки или SDK новой по отношению к предыдущей запущенной	
	Если НЕ ЗначениеЗаполнено(НестабильнаяВерсия) ИЛИ НЕ ЗначениеЗаполнено(ТекущаяВерсия) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	СтрТекущаяВерсия		= СтрЗаменить(ТекущаяВерсия, ".", Символы.ПС);
	СтрНестабильнаяВерсия	= СтрЗаменить(НестабильнаяВерсия, ".", Символы.ПС);
	сЧислоРазрядов = Макс(СтрЧислоСтрок(СтрТекущаяВерсия),СтрЧислоСтрок(СтрНестабильнаяВерсия));
	Для сч = 1 По сЧислоРазрядов Цикл
		ЗначениеРазрядаТекВер	= СтрПолучитьСтроку(СтрТекущаяВерсия, сч);
		ЗначениеРазрядаСтарВер	= СтрПолучитьСтроку(СтрНестабильнаяВерсия, сч);
		Попытка
			ЗначениеРазрядаТекВер = Число(ЗначениеРазрядаТекВер);
		Исключение
			ЗначениеРазрядаТекВер = 0;
		КонецПопытки;
		Попытка
			ЗначениеРазрядаСтарВер = Число(ЗначениеРазрядаСтарВер);
		Исключение
			ЗначениеРазрядаСтарВер = 0;
		КонецПопытки;
		
		Если ЗначениеРазрядаТекВер > ЗначениеРазрядаСтарВер Тогда
			Возврат Ложь;
		ИначеЕсли ЗначениеРазрядаТекВер < ЗначениеРазрядаСтарВер Тогда
			Возврат Истина;
		Иначе
			Продолжить;
		КонецЕсли;
	КонецЦикла;
	// Возвращаем ложь, если текущая версия равна последней стабильной
	Возврат Ложь;
КонецФункции
...
Функция СбисСформироватьОбщуюИнформациюДляТП(Кэш, ПутьККаталогу) Экспорт
	...
	ОписаниеЗавершения = Кэш.ОбщиеФункции.СбисОписаниеОповещения(Кэш, "ДействияСНастройками_ЗавершениеВызоваКоманды", Кэш.ГлавноеОкно, Новый Структура("Кэш, Отказ, ОбновитьСоставНастроек", Кэш, Ложь, Ложь));
...
Функция СбисОписаниеОповещения(Кэш, СбисПроцедура, СбисМодуль, СбисДопПараметры = Неопределено, СбисПроцедураИсключения = Неопределено, СбисМодульИсключения = Неопределено) Экспорт
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ОписаниеОповещенияРезультат = Новый Структура("Функция, Модуль, Параметры, ФункцияОшибки, МодульОшибки", СбисПроцедура, СбисМодуль, СбисДопПараметры, ?(СбисПроцедураИсключения=Неопределено, СбисПроцедура, СбисПроцедураИсключения), ?(СбисМодульИсключения=Неопределено, СбисМодуль, СбисМодульИсключения));
	#Иначе
		ОписаниеОповещенияРезультат = Новый ОписаниеОповещения(СбисПроцедура, СбисМодуль, СбисДопПараметры, ?(СбисПроцедураИсключения=Неопределено, СбисПроцедура, СбисПроцедураИсключения), ?(СбисМодульИсключения=Неопределено, СбисМодуль, СбисМодульИсключения));
	#КонецЕсли
...
Процедура СбисВыполнитьОписаниеОповещения(Кэш, РезультатВызова=Неопределено, СбисОписаниеОповещения) Экспорт
	...
	КонецЕсли;
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		Выполнить("СбисОписаниеОповещения.Модуль." + СбисОписаниеОповещения.Функция + "(РезультатВызова, СбисОписаниеОповещения.Параметры)");
	#Иначе
...
&НаКлиенте
Функция ГенераторВключенДляДокумента(Кэш, ФайлДанные) Экспорт
	Возврат Кэш.Парам.ИспользоватьГенератор 
			И ФайлДанные.Свойство("ИспользоватьГенератор")
			И ФайлДанные.ИспользоватьГенератор = Истина ;
КонецФункции
...
Функция СбисВыполнитьДействие(Кэш, ПараметрыДействия, Отказ) Экспорт 
	...
	Если ПараметрыДействия.Свойство("СертификатДляПодписания") и ЗначениеЗаполнено(ПараметрыДействия.СертификатДляПодписания) Тогда
		РезультатПодписания = Кэш.Интеграция.сбисПодписатьВложения(Кэш, attachmentList, ПараметрыДействия.СертификатДляПодписания, Отказ);
		Если Отказ Тогда 
...
Функция ПолучитьИнформациюОКонтрагенте(Кэш, СтруктураКонтрагента, ДопПараметры, Отказ) Экспорт
	ДопПараметрыЗапроса	= Новый Структура("ЕстьРезультат, СообщатьОбОшибке, ВернутьОшибку", Истина, Ложь, Истина);
...
Функция СбисСтатистика_СформироватьИЗаписатьСтатистикуНаСервис(Кэш, ПараметрыСтатистики, Отказ) Экспорт
	Если Не Кэш.СБИС.ОбменВключен Тогда
		//При отключенном обмене записать ничего не сможем.
		Возврат Ложь;
	КонецЕсли;
...
//Функция генерирует сообщение при переходе на старый способ обмена для сервиса статистики
&НаКлиенте
Процедура сбисСтатистика_ОтправитьПереключениеНаСтарыйСпособОбмена(Кэш, СпособОбмена, СпособОбменаДо) Экспорт
	
	СпособыПерехода = ПолучитьСоответствиеСпособовОбменаДляПерехода();
	ДанныеПерехода = СпособыПерехода.Получить(СпособОбмена);
	Если ДанныеПерехода <> Неопределено Тогда
		ПараметрыСтатистики = Новый Структура("Действие, ДетальноеСообщение", 
		"ПереключениеНаСтарыйСпособОбмена", 
		"Выполнен переход на " + ДанныеПерехода.НазваниеДо + " с " + ДанныеПерехода.Название);
		сбисСтатистика_СформироватьИЗаписатьСтатистикуНаСервис(Кэш, ПараметрыСтатистики, Ложь);
	КонецЕсли;
...
Функция ИдентификаторСБИСПоДокументу(Кэш, ДокументСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(МассивСтатусРегламент) Тогда 
		ИниРеестра = Кэш.ФормаНастроек.Ини(Кэш, Кэш.Текущий.ТипДок).мФайл;
		//первый ини
	...
	фрм = Кэш.ГлавноеОкно.СбисНайтиФормуФункции("ПрочитатьСвойствоДокументаПоСсылке", Кэш.ФормаРаботыСоСтатусами, "", Кэш);
	Если фрм = Ложь Тогда
		фрм = Кэш.ГлавноеОкно.СбисНайтиФормуФункции("ПрочитатьПараметрыДокументаСБИС", Кэш.ФормаРаботыСоСтатусами, "", Кэш);
		Возврат фрм.ПрочитатьПараметрыДокументаСБИС(ДокументСсылка, Кэш.Парам.КаталогНастроек, СтатусРегламент + "ДокументСБИС_Ид", Кэш.Ини);
	КонецЕсли;
	Возврат фрм.ПрочитатьСвойствоДокументаПоСсылке(Кэш, ДокументСсылка, СтатусРегламент + "ДокументСБИС_Ид", Новый Структура);
КонецФункции
