...
&НаКлиенте
Перем ИмяРегистраСвойствОбъектов;

//Настраивает форму дял работы
&НаКлиенте
Функция ИнициироватьФормуСтатусов(Кэш, ДопПараметры=Неопределено) Экспорт
	ЛокальныйКэш = Кэш;
	ИмяРегистраСвойствОбъектов = ИмяРегистраСвойствОбъектов(Кэш.Ини.Конфигурация);
КонецФункции
...
Функция СбисЗаписатьИзмененияПоДокументам1С(Кэш, МассивДокументов, ДополнительныеПараметры) Экспорт
	...
	Для Каждого КлючИЗначениеКарты Из КартаСписка Цикл
		...
		Для Каждого ИндексПакета Из КлючИЗначениеКарты.Значение Цикл
			...
			СбисОписаниеОбъектаТекЭтап = ПолучитьСвойствоСУчетомРегламента(Кэш,ПараметрыЗапроса,СбисСтатусРегламент + "ДокументСБИС_ТекущийЭтап", Отказ);
			...
			СтрокаЗапросаСпискаИд.Вставить("ДокументСБИС_ТекущийЭтап",СбисОписаниеОбъектаТекЭтап.Значение);
...
Функция ПрочитатьСвойствоДокументаПоСсылке(Кэш, ДокументСсылка, ИмяСвойства, ДопПараметры) Экспорт
	Возврат ПрочитатьСвойствоДокументаПоСсылкеНаСервере(ДокументСсылка, ИмяСвойства, ИмяРегистраСвойствОбъектов)
КонецФункции

&НаСервереБезКонтекста
Функция ПрочитатьСвойствоДокументаПоСсылкеНаСервере(Знач ДокументСсылка, Знач ИмяСвойства, Знач ИмяРегистраСвойствОбъектов)
	Запрос = новый Запрос;
