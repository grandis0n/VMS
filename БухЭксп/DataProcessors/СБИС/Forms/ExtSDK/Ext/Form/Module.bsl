...
Функция сбисСохранитьВФайлПоСсылке(Кэш, сбисПараметрыФайла, Отказ) Экспорт
	...
	Если Лев(сбисПараметрыФайла.URL, 8) = "https://" Тогда   
...
Функция СбисВыполнитьДействие(Кэш, СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета) Экспорт
	...
	Иначе
		ИННОрг = "";
	КонецЕсли;
	Если	Кэш.КэшСертификатов.Свойство("с" + ИННОрг, Сертификат)
		И	Сертификат <> Неопределено
		И	ТекущаяДата() - Кэш.КэшСертификатов.ДатаОбновления <= 60 Тогда
		//Сертификат в кэше
	Иначе
		...
		Попытка
			Для Каждого Серт Из Действие.Сертификат Цикл
				Если Серт.Ключ.Тип = "Отложенный" Тогда
					// для отложенных берем из действия, т.к. они возвращаются с сервера в ПрочитатьДокумент
					Сертификат = Серт;
					Прервать;
				КонецЕсли;
			КонецЦикла;
	...
	Результат = Кэш.ОбщиеФункции.СбисВыполнитьДействие(Кэш, Новый Структура("СоставПакета, Этап, Действие, Комментарий, Сертификат", СоставПакета, Этап, Действие, Комментарий, Сертификат), Отказ);
	Если Отказ Тогда
		Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, Результат);
		Возврат Ложь;
	КонецЕсли;
...
&НаКлиенте
Функция СБИСПлагин_Запустить(Кэш) Экспорт
...
Функция XmlSerializer_ReadSubNode(Node, Parent)
	...
	Исключение
		ВызватьИсключение("Ошибка чтения узла по пути " + Parent.Путь + ": недопустимые символы в ответе");
	КонецПопытки; 
