...
Функция ПрочитатьДокумент(Кэш, ИдДок, ДополнительныеПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	ПараметрыЧтения = Новый Структура("Документ",Новый Структура("Идентификатор,ДопПоля", строка(ИдДок), "ЭтапВернутьВсеСертификаты"));
...
Функция сбисВыполнитьДействие(Кэш, СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета) Экспорт
	...
	Если Действие.ТребуетПодписания="Да" Тогда
		Если Действие.Свойство("Сертификат") и Действие.Сертификат.Количество() Тогда
			...
			Если Не СертификатВыбран и ТекстОшибки<>"" Тогда
				Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, Кэш.ОбщиеФункции.СбисИсключение(, "API.СбисВыполнитьДействие", 755,, ТекстОшибки + ". " + ПредставлениеПакета));
				Возврат Ложь;
			КонецЕсли;
		Иначе
	...
	Результат = Кэш.ОбщиеФункции.СбисВыполнитьДействие(Кэш, Новый Структура("СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета, Сертификат, СертификатДляПодписания", СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета, Сертификат, СертификатДляПодписания), Отказ);
	Если Отказ Тогда
...
&НаКлиенте
Функция сбисПодписатьВложения(Кэш, attachmentList, СертификатДляПодписания, Отказ) Экспорт
	ТекстОшибки = "";
	сч = 0;
	Для Каждого Вложение Из attachmentList Цикл
		Если Вложение.Файл.Свойство("Ссылка") Тогда    // файл с онлайна
			ПолноеИмяФайла = Кэш.ОбщиеФункции.СбисПолучитьИмяВременногоФайлаКлиент();   
			Кэш.Интеграция.СохранитьВложениеПоСсылкеВФайл(Кэш,Вложение.Файл.Ссылка,ПолноеИмяФайла);
			Подпись = Кэш.ФормаЭП.сбисПодписатьФайл(Кэш, ПолноеИмяФайла, СертификатДляПодписания, ТекстОшибки);
		Иначе
			Подпись = Кэш.ФормаЭП.сбисПодписатьДвоичныеДанные(Кэш, Вложение.Файл.ДвоичныеДанные, СертификатДляПодписания, ТекстОшибки);
		КонецЕсли;
...
Функция ОбработатьСлужебныеДокументыПоПакету(Кэш, СоставПакета) Экспорт
	...
	Для Каждого Док Из Результат.Документ Цикл
		...
		Если Действие.ТребуетПодписания="Да" Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Выполнено = Кэш.ОбщиеФункции.СбисВыполнитьДействие(Кэш, Новый Структура("СоставПакета, Этап, Действие, Комментарий, Сертификат, СертификатДляПодписания", Док, Док.Этап[0], Действие, "", Сертификат, СертификатДляПодписания), Отказ);
		Если Отказ Тогда
			Сообщить(Выполнено.details);
			Отказ = Ложь;//Старая логика. Продолжаем
		КонецЕсли;
	КонецЦикла;
КонецФункции
&НаКлиенте
...
Функция сбисПолучитьПараметрыАктуальнойВерсии(Кэш, ПараметрыОбновления, Отказ) Экспорт
	...
	ПараметрыЗапроса.Вставить("Таймаут",				?(ПараметрыОбновления.Свойство("Таймаут"), ПараметрыОбновления.Таймаут, 0));
...
Функция сбисСохранитьВФайлПоСсылке(Кэш, СбисПараметрыФайла, Отказ) Экспорт
	...
	Если		СбисПараметрыФайла.Свойство("URL",	ПараметрыЗапроса.АдресРесурсаВЗапрос) Тогда
		...
		Если Лев(сбисПараметрыФайла.URL, 8) = "https://" Тогда   //передан полный url
