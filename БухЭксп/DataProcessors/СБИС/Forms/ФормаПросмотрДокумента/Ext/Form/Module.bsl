...
Процедура ЗаполнитьПрохождение(СоставПакета) Экспорт
	...
	ТекущийРаздел = МестныйКэш.Разделы["р"+МестныйКэш.Текущий.Раздел];
	фрм = МестныйКэш.ГлавноеОкно.сбисНайтиФормуФункции("ЗаполнитьПрохождение","Раздел_"+ТекущийРаздел+"_"+МестныйКэш.Текущий.ТипДок,"Раздел_"+ТекущийРаздел+"_Шаблон",МестныйКэш, ЭтаФорма);	
	Если фрм<>Ложь Тогда
...
// << alo Меркурий
	...
	Если		Не ЗначениеЗаполнено(ИниДокумента) Тогда
		Сообщить("Нет подходящей настройки для формирования расхождения.");
		Возврат;
	...
	ЗначениеИни.Вставить("ИспользоватьШтрихкодыНоменклатурыКонтрагентов",	Новый Структура("Значение,РассчитанноеЗначение", МестныйКэш.Парам.ИспользоватьШтрихкодыНоменклатурыКонтрагентов, МестныйКэш.Парам.ИспользоватьШтрихкодыНоменклатурыКонтрагентов));
	Контекст.Вставить("ДокументДанные", МестныйКэш.ОбщиеФункции.ПолучитьДанныеДокумента1С(ЗначениеИни, Документ1С, МестныйКэш.КэшЗначенийИни, МестныйКэш.Парам));  // alo Меркурий
	...
	Для Каждого Вложение Из СоставПакета[ИмяРеквизитаВложений] Цикл
		...
		Если Вложение.Свойство("Подпись") Тогда
			Для Каждого Подпись Из Вложение.Подпись Цикл
				НоваяСтр.Подпись = 	НоваяСтр.Подпись+
									?(Подпись.Сертификат.Свойство("ФИО"), 		Подпись.Сертификат.ФИО+", ", 		"")+
									?(Подпись.Сертификат.Свойство("Должность"), Подпись.Сертификат.Должность+", ",	"")+
									?(Подпись.Сертификат.Свойство("Название"),	Подпись.Сертификат.Название,	"")+Символы.ПС;
			КонецЦикла;
		КонецЕсли;
...
Процедура сбисИнформацияДляТП(Команда)
	...
	Если ДиалогОткрытия.Выбрать() Тогда 
		...
		СтруктурыДокументов1С = сбисПолучитьСтруктурыДокументов1С(); 
		...
		КаталогОтладки = МестныйКэш.Парам.КаталогОтладки; 
		РежимОтладки = МестныйКэш.Парам.РежимОтладки;
		МестныйКэш.Парам.КаталогОтладки = ПутьККаталогу;
		МестныйКэш.Парам.РежимОтладки = Истина;
		Если СоставПакета.Свойство("Вложение") Тогда
		    Если МестныйКэш.Текущий.Раздел = "3" или МестныйКэш.Текущий.Раздел = "4" Тогда
				Для Каждого Вложение Из СоставПакета.Вложение Цикл
					Если Вложение.Свойство("СтруктураФайла") Тогда
						ШаблонXML = МестныйКэш.ОбщиеФункции.сбисПолучитьXMLФайлаИзСтруктуры(МестныйКэш, Вложение);
					КонецЕсли;
				КонецЦикла;
			Иначе
				МестныйКэш.ОбщиеФункции.РазобратьСтруктуруДокументаСбис(СоставПакета, МестныйКэш, ИмяРеквизитаВложений);
			КонецЕсли;
		КонецЕсли;  
...
Функция сбисПолучитьСтруктурыДокументов1С() 
	...
	Для Каждого СтрДок Из ТаблицаДокументов	Цикл
		Для Каждого Док1С Из СтрДок.Документы1С Цикл  
			...
			РезИтог.Вставить(СтрЗаменить(Строка(Док1С.Значение),":", "_"), Рез);
		КонецЦикла;
	КонецЦикла;  
	Возврат РезИтог;
КонецФункции

Функция сбисЗаписатьДанныеОбъекта1СвТекстовыйДокумент(Рез, Объект1С, ИмяОбъектаДляЗапроса, УровеньВложенности, СписокТиповСправочников)
	ТекстДок = Новый ТекстовыйДокумент;
	Запрос = Новый Запрос ("Выбрать * Из "+ИмяОбъектаДляЗапроса+" ГДЕ Ссылка = &Док");
				...
				Если Колонка.Имя <> "Ссылка" и УровеньВложенности < 2 и СписокТиповСправочников.НайтиПоЗначению(ТипЗнч(Док[Колонка.Имя])) <> Неопределено  Тогда
					Попытка
						Если НЕ Док[Колонка.Имя].Пустая() Тогда
							сбисЗаписатьДанныеОбъекта1СвТекстовыйДокумент(Рез, Док[Колонка.Имя], "Справочник."+Док[Колонка.Имя].Метаданные().Имя, УровеньВложенности+1, СписокТиповСправочников);
						КонецЕсли;
					Исключение
	...
	Если УровеньВложенности = 1 Тогда 
		Рез.Документы.Добавить(ТекстДок,Строка(ТипЗнч(Объект1С))+"_"+Строка(Объект1С));	
	Иначе
		Рез.Подобъекты.Добавить(ТекстДок,Строка(ТипЗнч(Объект1С))+"_"+Строка(Объект1С));	
	КонецЕсли;
КонецФункции 
...
Процедура СбисВыбратьПочтовыйЯщик(Кэш, Элемент, ДопПараметры)
	...
	Если	Не	ИнформацияОКонтрагенте.Свойство("Идентификатор", СписокЯщиков)
		...
		Кэш.ГлавноеОкно.СбисСообщитьПользователю(Кэш, Новый Структура("Текст, ФормаВладелец, ЭлементНазначения", "Указывать абонентский ящик не требуется", ЭтаФорма, "АбонентскийЯщик"));
		Возврат;
	КонецЕсли;
	...
	Кэш.ГлавноеОкно.СбисВыбратьИзСписка(Кэш, СписокВыбора, ПараметрыВыбораСписка);
КонецПроцедуры
