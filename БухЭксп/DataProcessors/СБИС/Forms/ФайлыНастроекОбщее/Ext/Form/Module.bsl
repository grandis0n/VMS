...
&НаСервереБезКонтекста
Функция		ПолучитьПрефиксФайловНовойВерсииНастроек(знач СписокТиповНастроек, ПрефиксСервис, ДопПараметры=Неопределено)	Экспорт
...
Функция		ОчиститьСтруктуруИниОтСлужебныхЗначений(Знач СлужебнаяСтруктура)						Экспорт
	...
	Для Каждого Узел Из СлужебнаяСтруктура Цикл
		...
		Если	СбисКлюч = "ошибка"
			...
			Или СбисКлюч = "входящийконтекст"
			Или СбисКлюч = "использоватьштрихкодыноменклатурыконтрагентов"
			...
			Или СбисКлюч = "сбиспараметрыузла"
			Тогда
				Продолжить;
		КонецЕсли;
...
Процедура	СбисДобавитьПечатныеФормыНаСервере(ДанныеОбработки, ПараметрыФормированияНаСервере) Экспорт
	...
	Если ДанныеОбработки.ОбработкаХранитсяВСправочнике Тогда
		...
		Сообщить("Хранилище успешно получено!");
	Иначе
...
Функция		ПереключитьСпособХраненияНастроекВСбис(Аргумент, ДопПараметры) Экспорт
	...
	Если		Кэш.Парам.СпособХраненияНастроек = 1
		Или Не	Кэш.Интеграция.ДоступныСерверныеНастройки() Тогда 
		Возврат Ложь;
	КонецЕсли;
	...
	Кэш.ГлавноеОкно.СпособХраненияНастроек = ВременныйКэш.Парам.СпособХраненияНастроек;
	РезультатПредустановки = ВременныйКэш.ФормаНастроек.СбисПрименитьПредустановленныеНастройки(ВременныйКэш, ДопПараметры.Отказ, ПродолжитьВыполнение, ПараметрыПредустановки);
	Если ДопПараметры.Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(РезультатПредустановки, "ФайлыНастроекОбщее.ПереключитьСпособХраненияНастроекВСбис");
	ИначеЕсли Не РезультатПредустановки = Неопределено Тогда
		...
		Возврат Истина;
	КонецЕсли;
	СписокНастроек = Кэш.ФормаНастроек.СобратьСпискиНастроекНаСервере(Кэш.ОбщиеФункции.КэшДляСервера(Кэш), Кэш.Парам.ИдентификаторНастроек, ДопПараметры.Отказ);
	Если ДопПараметры.Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(РезультатПредустановки, "ФайлыНастроекОбщее.ПереключитьСпособХраненияНастроекВСбис");
	КонецЕсли;
	Если	СписокНастроек.XSLT.Количество()
		...
		//Должна быть запись в статистику
		Возврат Ложь;
	КонецЕсли;
	Кэш.ФормаНастроек.СбисЗаполнитьНеполученныеНастройки(Кэш, ДопПараметры.Отказ);
	ВременныйКэш.Ини = Кэш.Ини;
	//Создаем шаблон из текущих настроек	
	НовыйШаблонКоннекшена = ВременныйКэш.ФормаНастроек.СформироватьСтруктуруТекущихНастроек(ВременныйКэш, Новый Структура("ТипНастроек", "Шаблон"), ДопПараметры.Отказ);
	Если ДопПараметры.Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(НовыйШаблонКоннекшена, "ФайлыНастроекОбщее.ПереключитьСпособХраненияНастроекВСбис");
	КонецЕсли;
	...
	ПараметрыЗаписи = Новый Структура("Отказ, ПродолжитьВыполнение", ДопПараметры.Отказ, Истина);
	НовыйШаблонКоннекшена.Название = Кэш.Парам.ИдентификаторНастроек;
	ВременныйКэш.ИниВПорядке = Кэш.ИниВПорядке;
	РезультатЗаписи = ВременныйКэш.ФормаНастроек.КлассIntegrationConnection_СохранитьПодключение(ВременныйКэш, НовыйШаблонКоннекшена, Новый Структура("ТихийРежим", ВременныйКэш.ТихийРежим), ДопПараметры.Отказ);
	Если ДопПараметры.Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(РезультатЗаписи, "ФайлыНастроекОбщее.ПереключитьСпособХраненияНастроекВСбис");
	КонецЕсли;
	Если РезультатЗаписи.ЗаписьВыполнялась Тогда
		...
		ЗаполнитьЗначенияСвойств(Кэш, ВременныйКэш);
	КонецЕсли;
	Возврат Не ПараметрыЗаписи.Отказ;
КонецФункции
...
Функция		УстановитьсбисПараметры(Кэш, СтруктураНастроек, Идентификатор)	Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ГлавноеОкно.ДатаПоследнегоЗапросаСтатусов) или ЗначениеЗаполнено(ГлавноеОкно.ИдентификаторПоследнегоСобытия) или ЗначениеЗаполнено(ГлавноеОкно.ИспользоватьГенератор) Тогда
