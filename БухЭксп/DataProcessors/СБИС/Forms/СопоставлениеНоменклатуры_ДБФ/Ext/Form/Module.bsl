	...
	Если Recordset.EOF=Ложь Тогда 
		Если ЗначениеЗаполнено(RecordSet.Fields("IDNOM").value) и ЗначениеЗаполнено(RecordSet.Fields("SPRNOM").value) Тогда
			...
			Если ЗначениеЗаполнено(RecordSet.Fields("IDUNTORG").value) Тогда
				Результат.ЕдИзмОрг = Справочники["КлассификаторЕдиницИзмерения"].НайтиПоНаименованию(RecordSet.Fields("IDUNTORG").value);	
			КонецЕсли;
...
// Процедура устанавливает/удаляет соответствие номенклатуры	
	...
	Если стрНоменклатураПоставщика.Свойство("ЕдИзмОрг") Тогда
		IDUNTORG = Лев(Строка(стрНоменклатураПоставщика.ЕдИзмОрг), 240);
	КонецЕсли;
	Если стрНоменклатураПоставщика.Свойство("Коэффициент") Тогда
		RATIO = Лев(стрНоменклатураПоставщика.Коэффициент, 240);
	КонецЕсли;
	IDUNPOST = "";
...
// Процедура ищет реквизит номенклатуры контрагента	
	Возврат сбисПолучитьРеквизитНоменклатурыПоставщикаНаСервере(стрКонтрагент, стрНоменклатура, ИмяРеквизита, КаталогНастроек, Ини.Конфигурация);
КонецФункции
&НаСервереБезКонтекста
