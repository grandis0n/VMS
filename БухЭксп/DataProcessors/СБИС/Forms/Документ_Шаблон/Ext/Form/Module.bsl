...
#Область include_local_ДокументШаблон
...
Функция СоздатьДокумент(Кэш, Вложение, Ини, СоставПакета, МассивОснований, Документ1С = Неопределено) Экспорт
	...
	Если МассивОснований.Найти(Документ1С) = Неопределено Тогда // если документ уже загружен по текущему пакету, то не перепривязываем его к новому вложению (иначе будут проблемы при повторной загрузке)
...
Функция ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента(Кэш, СтруктураФайла, Ини, ДокОбъект, МассивОснований, СоставПакета, Вложение, ДопПараметры=Неопределено) Экспорт
	...
	Ини,	СтруктураФайла, ДокОбъект,		"Документ",	Новый Структура,Вложение,	СоставПакета,	Кэш.Ини.Конфигурация,	МассивОснований, Новый Структура("Шапка", КлючиИниВерхнегоУровня));
...
Функция ВычислитьОтборНаКлиенте(Аргумент, ПараметрыОбработчика)
	...
	Если	Не СбисИндексСтроки = Неопределено Тогда
		...
		КонецПопытки;
	#Если ТолстыйКлиентОбычноеПриложение Тогда	
	ИначеЕсли	ТипЗнч(РасчитанноеЗначение) = Тип("ТаблицаЗначений") Тогда
...
Функция ЗаполнитьДанныеДокумента1С(Кэш, ини, СтруктураФайла, МассивОснований,СоставПакета, Вложение, Документ1С = Неопределено, ДополнительныеДанные=Неопределено) Экспорт
	...
	сбисДополнительныеПараметры = Новый Структура("ОбъектПерезаполнитьСсылка, ТипОбъекта, ПодТипОбъекта, ДополнятьТабЧасть, РедактироватьТабЧасть, Ини, ПерезаполнятьТолькоНепроведенные", Документ1С, ТипОбъекта, ТипДокумента, ДополнятьТабЧасть, РедактироватьТабЧасть, ини, Кэш.Парам.ПерезаполнятьТолькоНепроведенные);
		...
		Попытка
			Если ТипЗнч(Элемент.Значение) = Тип("Массив") Тогда
				Если РедактироватьТабЧасть = Истина Тогда
					...
					КонецЕсли;
				Иначе
					Если Не ДополнятьТабЧасть Тогда
...
Процедура Обработчик_УзелИни(Аргумент, ПараметрыОбработчика) Экспорт
	...
	Если ПараметрыОбработчика.Обработчики.Свойство(Аргумент.Ключ, СбисОбработчикДляУзла) Тогда
		...
		КонецЕсли;
		ОписаниеОповещения = Кэш.ОбщиеФункции.СбисОписаниеОповещения(Кэш, СбисОбработчикДляУзла.Метод, СбисОбработчикДляУзла.Модуль, СбисПараметрыДляОбработчика);
		Кэш.ОбщиеФункции.СбисВыполнитьОписаниеОповещения(Кэш, СбисАргументДляОбработчика, ОписаниеОповещения);
...
Процедура Обработчик_мСторона(Аргумент, ПараметрыОбработчика) Экспорт
	...
	Если ТекущееЗначение.Свойство(ОрганизацияРоль) Тогда
		ЗначениеПараметра = Кэш.ОбщиеФункции.НайтиОрганизациюИзДокументаСБИС(Кэш.Ини.Конфигурация, Кэш.ОбщиеФункции.РассчитатьЗначениеИзСтруктуры(ТекущееЗначение[ОрганизацияРоль].Сторона.Данные,СтруктураФайла));
		...
		КонецЕсли;
		ЗаполнитьЗначениеРеквизита(ЗначениеПараметра, ТекущееЗначение[ОрганизацияРоль].Сторона, СбисКонтекст, ДопПараметры);	
	КонецЕсли;
	Для Каждого Сторона Из ТекущееЗначение Цикл
		...
		Если Сторона.Значение.Сторона.Свойство("Данные") Тогда
			СтруктураСтороны = Кэш.ОбщиеФункции.РассчитатьЗначениеИзСтруктуры(Сторона.Значение.Сторона.Данные,СтруктураФайла);
			Если ЗначениеЗаполнено(СтруктураСтороны) Тогда
				...
				Если Сторона.Ключ = КонтрагентРоль Тогда
					...
					КонецЕсли;
				КонецЕсли;
				ЗаполнитьЗначениеРеквизита(ЗначениеПараметра, Сторона.Значение.Сторона, СбисКонтекст, ДопПараметры);
			КонецЕсли;
		КонецЕсли;
		Если Сторона.Значение.Сторона.Свойство("Вычислить") Тогда
			//ЗначениеПараметра = НайтиСоздатьКонтрагента(РассчитатьЗначение(Сторона.Значение.Сторона.Вычислить,Новый Структура("Ини, Документ, Переменные, СтруктураФайла,СоставПакета", Ини, ДокОбъект, ПеременныеДок, СтруктураФайла,СоставПакета),Кэш), Сторона.Значение);
			ЗначениеПараметра = РассчитатьЗначение(Сторона.Значение.Сторона.Вычислить, СбисКонтекст, Кэш);
			Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда  // структура в формате СБИС
				ЗначениеПараметра = Кэш.ОбщиеФункции.НайтиКонтрагентаИзДокументаСБИС(Кэш.Ини.Конфигурация, ЗначениеПараметра, Ини);
			КонецЕсли;
			ЗаполнитьЗначениеРеквизита(ЗначениеПараметра, Сторона.Значение.Сторона, СбисКонтекст, ДопПараметры);
		КонецЕсли;	
	КонецЦикла;
