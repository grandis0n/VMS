...
// 
// Иногда возможна иерархия объектов - среди них выделяются "основные" и "уточняющие" - см. реквизит ЭтоУточняющийОбъект в РедакторНастроек.
// В таких случаях настраиваются преимущественно основные объекты, а уточняющие применяются в каких-то нестандартных случаях.
// Предполагается, что уточняющих может быть слишком много для вывода всех их на форме.
// Классифицируемые объекты, над которыми выполняются настройки (то есть, все основные и примененные уточняющие) называются "настраиваемыми".
// См. ЗаполнениеБухгалтерскойОтчетностиНастраиваемая.НастраиваемыеОбъекты
// В контексте этого модуля КлассифицируемыеОбъекты и НастраиваемыеОбъекты - это синонимы.
// То есть, все релевантные классифицируемые объекты - настраиваемые.
//
...
&НаКлиенте
Процедура РедакторНастроекОбъектПриИзменении(Элемент)
	
	Настройка = Элементы.РедакторНастроек.ТекущиеДанные;
	
	ПравилоНастройки = ПравилаНастройки.ПоАлгоритмам[Настройка.Алгоритм]; // см. НовыйПравилаНастройкиАлгоритма
	Если ПравилоНастройки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПравилоНастройки.ИерархияОбъектов Тогда
		Возврат;
	КонецЕсли;
	
	Настройка.ЭтоУточняющийОбъект = ЭтоУточняющийОбъект(Настройка.Объект, Настройка.Алгоритм);
...
Функция НовыйРедактированиеСтруктурыОтчета()
	...
	// См. ЗаполнениеБухгалтерскойОтчетностиНастраиваемая.НастраиваемыеОбъекты
	Редактирование.Вставить("НастраиваемыеОбъекты", Новый Соответствие);
...
Функция НовыйПравилаНастройкиАлгоритма()
	...
	Редактирование = Новый Структура;
	Редактирование.Вставить("Настраиваемый", Ложь); // см. также ВозможноРедактированиеСоставаПоказателей
	Редактирование.Вставить("ТипОбъектов", Новый ОписаниеТипов);
	Редактирование.Вставить("ИерархияОбъектов", Ложь); // наличие иерархии означает, что могут быть уточняющие объекты
	Редактирование.Вставить("ГруппыИЭлементы", ГруппыИЭлементы.Элементы); // уточняющие объекты могут быть элементами, а основные - группами, см. ЗаполнениеБухгалтерскойОтчетностиПравила.УстановитьВыборГруппЭлементовИерархияОбъектов
...
Функция ЗачетПоказателейПредставления()
	...
	Представления.Добавить("СтатьиОбъекты", НСтр("ru = 'По статьям, объектам'"));
...
&НаСервереБезКонтекста
Функция ПорядокНастройкиКлассифицируемыхОбъектов(РедактированиеСтруктурыОтчета)
	
	Возврат ЗаполнениеБухгалтерскойОтчетностиНастраиваемая.ПорядокНастройкиКлассифицируемыхОбъектов(
		РедактированиеСтруктурыОтчета.ЗаполняемыеПоказатели);
		
КонецФункции

&НаСервереБезКонтекста
Процедура ЗаполнитьКлассифицируемыеОбъекты(РедактированиеСтруктурыОтчета, ПорядокНастройкиКлассифицируемыхОбъектов = Неопределено)

	Если ПорядокНастройкиКлассифицируемыхОбъектов = Неопределено Тогда
		ПорядокНастройкиКлассифицируемыхОбъектов = ПорядокНастройкиКлассифицируемыхОбъектов(РедактированиеСтруктурыОтчета);
	КонецЕсли;
	
	РедактированиеСтруктурыОтчета.НастраиваемыеОбъекты.Очистить();
	
	Для Каждого ОписаниеОбъектовАлгоритмаЗаполнения Из ПорядокНастройкиКлассифицируемыхОбъектов Цикл
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			РедактированиеСтруктурыОтчета.НастраиваемыеОбъекты,
			ОписаниеОбъектовАлгоритмаЗаполнения.Значение.ОбъектыПоказателей);
	КонецЦикла;
...
Процедура ЗаполнитьРедакторНастроек(РедактированиеСтруктурыОтчета)
	...
	ПравилаНастройки = ОбщиеПравилаНастройки(ПорядокНастройкиКлассифицируемыхОбъектов);
	
	ЗаполнитьКлассифицируемыеОбъекты(РедактированиеСтруктурыОтчета, ПорядокНастройкиКлассифицируемыхОбъектов);
...
&НаСервереБезКонтекста
Функция ОбщиеПравилаНастройки(ПорядокНастройки)
	
	ПравилаНастройки = НовыйПравилаНастройки();
	
	Для Каждого ОписаниеАлгоритма Из ПорядокНастройки Цикл
		
		ПравилаНастройкиАлгоритма = НовыйПравилаНастройкиАлгоритма();
		ПравилаНастройки.ПоАлгоритмам.Вставить(ОписаниеАлгоритма.Ключ, ПравилаНастройкиАлгоритма);
		
		Если Не ЗаполнениеБухгалтерскойОтчетностиПравила.РазрешенаНастройкаПользователем(ОписаниеАлгоритма.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		
		ТипОбъектов = ЗаполнениеБухгалтерскойОтчетностиПравила.ТипЗначенияКлассифицируемыхОбъектов(ОписаниеАлгоритма.Ключ);
		Если ТипОбъектов = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПравилаНастройкиАлгоритма.Настраиваемый = Истина;
		ПравилаНастройкиАлгоритма.ТипОбъектов = ТипОбъектов;
		ПравилаНастройкиАлгоритма.ИерархияОбъектов = ОписаниеАлгоритма.Значение.ПрименяетсяУточнениеОбъектов;
		Если ПравилаНастройкиАлгоритма.ИерархияОбъектов Тогда
			ЗаполнениеБухгалтерскойОтчетностиПравила.УстановитьВыборГруппЭлементовИерархияОбъектов(
				ПравилаНастройкиАлгоритма.ГруппыИЭлементы,
				ОписаниеАлгоритма.Ключ);
		КонецЕсли;
		
		ИнициализироватьЗачетПоказателей(ПравилаНастройкиАлгоритма.ЗачетПоказателей, ОписаниеАлгоритма.Ключ);
		
	КонецЦикла;
	
	Возврат ПравилаНастройки;
...
Процедура ЗаполнитьБазовуюСтруктуруОтчета(РедакторНастроек, РедактированиеСтруктурыОтчета, ПоискПодразделов, ПравилаНастройки)
	...
	Для Каждого ОписаниеГруппы Из РедактированиеСтруктурыОтчета.ЗаполняемыеПоказатели.ГруппыПоказателей Цикл
		...
		Для Каждого Показатель Из СоставГруппы Цикл
			...
			АлгоритмЗаполнения = Показатель.Заполнение.Идентификатор; // может быть Неопределено
			ПравилаНастройкиАлгоритма = ПравилаНастройки.ПоАлгоритмам[АлгоритмЗаполнения]; // см. НовыйПравилаНастройкиАлгоритма
			...
			Если ПравилаНастройкиАлгоритма <> Неопределено Тогда
				ТипОбъектов = ПравилаНастройкиАлгоритма.ТипОбъектов;
			КонецЕсли;
			
			УстановитьХарактеристикиПоказателя(РедакторПоказателя, Показатель, ТипОбъектов);
				
			Если ПравилаНастройкиАлгоритма <> Неопределено Тогда
			
				Если ПравилаНастройкиАлгоритма.Настраиваемый Тогда
...
Процедура ДобавитьКлассифицируемыеОбъекты(РедакторПоказателя, ИдентификаторПоказателя, КлассифицируемыеОбъекты, ПравилаНастройкиАлгоритма)
	...
	Для Каждого ОписаниеОбъекта Из КлассифицируемыеОбъектыПоказателя Цикл
		...
		РедакторОбъектов.ЭтоУточняющийОбъект = (ОписаниеОбъекта.Картинка = ЗаполнениеБухгалтерскойОтчетностиНастраиваемая.КартинкаИдентификаторУточняющийОбъект());
...
Процедура НастроитьКнопкиДобавитьОбъект()
	...
	Для Каждого ОписаниеПравилаНастройки Из ПравилаНастройки.ПоАлгоритмам Цикл
		...
		Если Не ОписаниеПравилаНастройки.Значение.Настраиваемый Тогда
			Продолжить;
		КонецЕсли;
		
		ТипОбъектов = ОписаниеПравилаНастройки.Значение.ТипОбъектов;
		
		Для Каждого ОписаниеКнопки Из КнопкиДобавитьОбъект Цикл
			Если ТипОбъектов.СодержитТип(ОписаниеКнопки.Ключ) Тогда
...
Процедура ЗаполнитьКнопкиДобавитьОбъект(КнопкиДобавитьОбъект)
	...
	КнопкиДобавитьОбъект.Вставить(
		Тип("СправочникСсылка.ПрочиеДоходыИРасходы"),
...
Процедура УстановитьУсловноеОформлениеПредметНастройки()
	...
	// Группы объектов выделяем шрифтом, так как для возможности их редактирования они выводятся не в первой колонке,
	// а картинки доступны только в первой
	Оформление = НачатьОформлениеПоля(Элементы.РедакторНастроекОбъект, "РедакторНастроек.ЭтоУточняющийОбъект", Истина);
	УстановитьОформлениеУточняющийОбъект(Оформление);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УстановитьОформлениеУточняющийОбъект(Оформление)
	Курсив = Истина;
	Оформление.УстановитьЗначениеПараметра("Шрифт", Новый Шрифт( , , , Курсив));
КонецПроцедуры
...
Процедура НастроитьЭлементыЗависимыеОтСтрокиОтложенно() Экспорт
	...
	Элементы.РедакторНастроекОбъект.ВыборГруппИЭлементов = ПравилоНастройки.ГруппыИЭлементы;
...
&НаСервереБезКонтекста
Функция ЭтоУточняющийОбъект(Знач Объект, Знач АлгоритмЗаполнения)
	
	Возврат ЗаполнениеБухгалтерскойОтчетностиПравила.ЭтоУточняющийОбъект(Объект, АлгоритмЗаполнения);
...
Процедура ПовыситьПоказательДоУровняГруппы(НоваяГруппа, НовыйПоказатель = Неопределено)
	...
	НастраиваемыеОбъекты = НоваяГруппа.ПолучитьЭлементы();
	
	Если Не ЗначениеЗаполнено(НастраиваемыеОбъекты) Тогда
		Возврат;
	КонецЕсли;
	...
	ПеренестиКлассифицируемыеОбъектыВнутрьПоказателя(НастраиваемыеОбъекты, НовыйПоказатель);
...
Процедура УдалитьСвязаннныеКлассифицируемыеОбъекты(УдаляемыеНастройки)
	...
	Для Каждого УдаляемаяНастройка Из УдаляемыеНастройки Цикл
		...
		Если Показатель.Уровень = "ПоказательОсновной" Тогда
			Если УдаляемаяНастройка.ЭтоУточняющийОбъект Тогда
				УдалитьНастройку(УдаляемаяНастройка, РедакторНастроек);
			КонецЕсли;
		Иначе
			ПеренестиКлассифицируемыйОбъектВОсновныеПоказатели(УдаляемаяНастройка, УдаляемаяНастройка.Алгоритм);
...
Функция НовыйОписаниеСохраненногоПоказателя()
	...
	ОписаниеПоказателя.Вставить("НастраиваемыеОбъекты", Новый Массив); // из ЛюбаяСсылка
...
Функция СохраненныйПоказатель(Настройка, ЗаполняемыеПоказатели, ИндексПоказатели, КлассифицируемыеОбъекты)
	...
	КлассифицируемыеОбъектыПоказателя = КлассифицируемыеОбъекты[ИдентификаторПоказателя];
	Если КлассифицируемыеОбъектыПоказателя <> Неопределено Тогда
		ОписаниеПоказателя.НастраиваемыеОбъекты = КлассифицируемыеОбъектыПоказателя.ВыгрузитьЗначения();
	КонецЕсли;
...
Процедура СохранитьСоставКлассифицируемыхОбъектовПоказателя(Заполнение, Настройка, ПредыдущийПоказатель)
	...
	КлассифицируемыеОбъекты = Новый СписокЗначений;
	
	Для Каждого РедакторСтатьи Из Настройка.ПолучитьЭлементы() Цикл
		КлассифицируемыеОбъекты.Добавить(РедакторСтатьи.Объект, , РедакторСтатьи.ЭтоУточняющийОбъект);
	КонецЦикла;
	
	Если Не СоставКлассифицируемыхОбъектовПоказателяИзменился(КлассифицируемыеОбъекты.ВыгрузитьЗначения(), ПредыдущийПоказатель) Тогда
...
Функция СоставКлассифицируемыхОбъектовПоказателяИзменился(НовыйСоставКлассифицируемыхОбъектов, ПредыдущийПоказатель)
	...
	Возврат Не ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(
		...
		ПредыдущийПоказатель.НастраиваемыеОбъекты,
		Истина);
...
Процедура ЗаписатьСтатистикуПодсказки(ВидимостьПанели)
	...
	ИмяСобытия = СтрШаблон("СтатистикаБП.ПанельПодсказок.%1.%2", ИмяОперации, Действие);
	ОбщегоНазначенияБП.ЗаписатьОперациюБизнесСтатистики(ИмяСобытия);
...
Процедура УстановитьКартинкуНастройки(Настройка)
	...
	// Однако в отдельных случаях картинка ЗаполнениеБухгалтерскойОтчетностиНастраиваемая.КартинкаИдентификаторУточняющийОбъект
	// используется для идентификации уточняющих объектов.
	// В качестве изобразительного приема для них используется см. УстановитьОформлениеУточняющийОбъект
