...
Функция ФормаЗаполняетсяПомощником(ВыбраннаяФорма) Экспорт
	...
	ПоддерживаемыеФормы.Добавить(НРег("ФормаОтчета2026Кв3"));
...
Функция НоваяСтруктураРасчетаСтандартныхИСоциальныхВычетов(ПредварительныйРасчет = Истина)
	...
	РасчетСоциальныхВычетов.Вставить("ДолгосрочноеСтрахование", 0);
	РасчетСоциальныхВычетов.Вставить("ДолгосрочноеСтрахованиеДети", 0);
...
Процедура ДобавитьРасчетСтандартныхИСоциальныхВычетов(Вид, ДанныеВычета, РасчетВычетов, ВыбраннаяФорма)
	...
	ИначеЕсли Вид = Перечисления.ВычетыФизическихЛиц.ДобровольныеПенсионныеВзносыИСтрахованиеЖизни Тогда
		...
		ИначеЕсли ВычетПоДоговорамДолгосрочногоСтрахования(ДанныеВычета.ВидДоговораПенсионногоОбеспечения, ВыбраннаяФорма) Тогда
			
			Если ПомощникЗаполнения3НДФЛКлиентСервер.ЗастрахованыДети(ДанныеВычета.Застрахованный) Тогда
				РасчетВычетов.ДолгосрочноеСтрахованиеДети = РасчетВычетов.ДолгосрочноеСтрахованиеДети
					+ ДанныеВычета.СуммаВзносовПенсионныйФондКВычету;
			Иначе
				РасчетВычетов.ДолгосрочноеСтрахование = РасчетВычетов.ДолгосрочноеСтрахование
					+ ДанныеВычета.СуммаВзносовПенсионныйФондКВычету;
			КонецЕсли;
...
Процедура ДобавитьРасчетНалоговойБазыПоВидуДохода(Вид, ДоходВычет, РасчетНалоговойБазы, ВыбраннаяФорма)
	...
	Если ЭтоДоходОтПредпринимательскойДеятельности(Вид) И ДоходВычет.Свойство("ОбщаяСуммаДоходаИП") Тогда
		...
		РасчетНалоговойБазы.СуммаУплаченныхАвансовыхПлатежей = РасчетНалоговойБазы.СуммаУплаченныхАвансовыхПлатежей
			+ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДоходВычет, "СуммаУплаченныхАвансовыхПлатежейИП", 0);
...
Функция ВычетПоДоговорамДолгосрочныхСбереженийНПФ(ВидДоговора, ВыбраннаяФорма)
	...
	КодСбережения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КодыВидовДоговоров,
		ПомощникЗаполнения3НДФЛКлиентСервер.ИдентификаторДолгосрочныеСбережения(), "");
	
	Возврат ВидДоговора = КодСбережения;
	
КонецФункции

Функция ВычетПоДоговорамДолгосрочногоСтрахования(ВидДоговора, ВыбраннаяФорма)
	
	// Вычет согласно пп.5 п.1 ст. 219.2 НК РФ
	...
	КодСтрахование = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КодыВидовДоговоров,
		ПомощникЗаполнения3НДФЛКлиентСервер.ИдентификаторДолгосрочноеСтрахование(), "");
	
	Возврат ВидДоговора = КодСтрахование;
...
Функция ДанныеЗаполнения_ПенсионныеВзносыСтрахованиеЖизни(ДанныеФормы)
	...
	Застрахованный = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеФормы, "Застрахованный", "");
	ДанныеДекларации.Вставить("Застрахованный", Застрахованный);
...
Функция КлючеваяСтавкаЦБ(ВыбраннаяФорма)
	...
	Если ВыбраннаяФорма = "ФормаОтчета2026Кв3" Тогда
		Возврат 16;
	ИначеЕсли ВыбраннаяФорма = "ФормаОтчета2025кв1" Или ВыбраннаяФорма = "ФормаОтчета2025Кв4" Тогда
		Возврат 21;
...
Функция ТаблицаНалоговойБазы2025(ТаблицаДанныхЗаполнения, ПределыВычетов, ВыбраннаяФорма, Период)
	...
	ПределДолгосрочныеСбережения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПределыВычетов,
		"ДолгосрочныеСбереженияНПФ",
		0);
	
	ПределДолгосрочныеСбереженияДети = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПределыВычетов,
		"ДолгосрочноеСтрахованиеДети",
		0);
	...
	Для Каждого ТекущийВычет Из ДанныеРасчета.Вычеты Цикл
		...
		Для Каждого ТекущаяСтрока Из РаспределениеВычета Цикл
			...
			Если ТекущийВычет.Вид = Перечисления.ВычетыФизическихЛиц.ДобровольныеПенсионныеВзносыИСтрахованиеЖизни Тогда
				
				Если ЭтоДоговорПДС(ТекущийВычет.СтруктураДанных.ВидДоговораПенсионногоОбеспечения, ВыбраннаяФорма) Тогда
					СуммаВычетаНакопительная = Мин(Мин(РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахование
						+ РасчетСтандартныхСоциальныхВычетов.ДолгосрочныеСбереженияНПФ, ПределДолгосрочныеСбережения)
						+ РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахованиеДети, ПределДолгосрочныеСбереженияДети);
					Если ПомощникЗаполнения3НДФЛКлиентСервер.ЗастрахованыДети(ТекущийВычет.СтруктураДанных.Застрахованный) Тогда
						ПредельнаяСуммаВычета = ПределДолгосрочныеСбереженияДети;
						СуммаВычета = Мин(ТекущаяСтрока.Значение, Макс(ПредельнаяСуммаВычета - СуммаВычетаНакопительная, 0));
					Иначе
						ПредельнаяСуммаВычета = ПределДолгосрочныеСбережения;
						СуммаЗаДетей = СуммаВычетаНакопительная - РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахование;
						Если СуммаЗаДетей > 0 Тогда
							ПредельнаяСуммаВычета = ПределДолгосрочныеСбереженияДети;
						КонецЕсли;
						СуммаВычета = Мин(ТекущаяСтрока.Значение, Макс(ПредельнаяСуммаВычета - СуммаВычетаНакопительная, 0));
					КонецЕсли;
	...
	РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахование =
		Мин(Мин(РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахование, ПределДолгосрочныеСбережения)
			+ РасчетСтандартныхСоциальныхВычетов.ДолгосрочноеСтрахованиеДети, ПределДолгосрочныеСбереженияДети);
	...
	ЗаполнитьИтогиВычетаДобровольныеПенсионныеВзносыИСтрахованиеЖизни(
		ТаблицаДанныхЗаполнения,
		РасчетСтандартныхСоциальныхВычетов,
		ВыбраннаяФорма);
...
Процедура ПересчитатьНалоговыеБазыПоГруппе(ГруппаДоходов, НалоговыеБазыПоВидамДоходов, Период, ВыбраннаяФорма)
	...
	Для Каждого ТекущаяСтрока Из НалоговыеБазы Цикл
		...
		Если ТекущаяСтрока.СтруктураДанных.СуммаНалогаИсчисленнаяКУплате > 0 Тогда
			ТекущаяСтрока.СуммаНалогаПодлежащаяУплате = ТекущаяСтрока.СтруктураДанных.СуммаНалогаИсчисленнаяКУплате;
			ТекущаяСтрока.СуммаНалогаПодлежащаяВозврату = 0;
		Иначе
			ТекущаяСтрока.СуммаНалогаПодлежащаяУплате = 0;
			ТекущаяСтрока.СуммаНалогаПодлежащаяВозврату = - ТекущаяСтрока.СтруктураДанных.СуммаНалогаИсчисленнаяКУплате;
		КонецЕсли;
	КонецЦикла;
...
Функция ТаблицаСведенийОСуммахНалога2025(ПараметрыВыполнения)
	...
	Если ТаблицаСведенийОСуммахНалога.Количество() = 0 И ТаблицаРасчетов.Количество() = 0 Тогда
		СтрокаРасчета = ТаблицаСведенийОСуммахНалога.Добавить();
		...
		СтрокаРасчета.ОКТМО = ОКТМОналогоплательщика;
		СтрокаРасчета.СтруктураДанных = ДанныеЗаполнения_СведенияОСуммахНалогаИП(СтрокаРасчета,
			ПризнакиУплатыВозврата,
			ВыбраннаяФорма,
			ТаблицаДанныхЗаполнения,
			ПараметрыВыполнения.Период);
...
Процедура ДобавитьСуммыНалогаПоКБК(Результат, ПараметрыВыполнения, ПериодСобытия, ОКТМО)
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыполнения.НалогКУплате)
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			ИмяПоля = КлючПоляСуммаНалогаКУменьшению(НомерМесяца - 3);
...
Процедура ДобавитьДанныеЗаявленияНаВозвратНалога2025(ТаблицаДанныхЗаполнения, СчетОрганизации, ВыбраннаяФорма3НДФЛ)
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДанныхЗаполнения Цикл
		...
		СуммаАвансов = СуммаАвансов + ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ТекущаяСтрока.СтруктураДанных,
			"СуммаУплаченныхАвансовыхПлатежейИП",
			0);
		...
		Если Не ЗначениеЗаполнено(ГруппаДоходов) Тогда
			Продолжить;
		КонецЕсли;
		
		СуммаКУплате = СуммаКУплате + ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ТекущаяСтрока.СтруктураДанных,
			"СуммаНалогаПодлежащаяУплате",
			0);
		СуммаКВозврату = СуммаКВозврату + ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ТекущаяСтрока.СтруктураДанных,
			"СуммаНалогаПодлежащаяВозврату",
			0);
		
	КонецЦикла;
	
	ИтогоКВозврату = СуммаАвансов - (СуммаКУплате - СуммаКВозврату);
	
	Если ИтогоКВозврату <= 0 Тогда
		Возврат;
	КонецЕсли;
	...
	СтруктураЗаявления = Новый Структура;
	СтруктураЗаявления.Вставить("СуммаВозврат", ИтогоКВозврату);
...
Процедура ДоработатьДанныеПредпринимательскойДеятельности(ПараметрыВыполнения, КодВидаДеятельности)
	...
	ОбщаяСуммаРасходовВПределахНорматива = 0;
	ЗаполнятьИсчисленныеАвансовыеПлатежи = Ложь;
	АвансовыеПлатежиРаспределены = Ложь;
	ИндексСтроки = 0;
	...
	Для Каждого СтрокаДанныхЗаполнения Из НайденныеСтроки Цикл
		...
		Если Не КодВидаДеятельностиСовпадает(СтруктураДанных, КодВидаДеятельности) Тогда
			// Авансовые платежи по умолчанию всегда добавляем к первому виду предпринимательской деятельности
			АвансовыеПлатежиРаспределены = (ИндексСтроки = 0);
			Продолжить;
		КонецЕсли;
	...
	АвансовыеПлатежиИП = ?(АвансовыеПлатежиРаспределены, 0, ДанныеПомощникаЗаполнения.АвансовыеПлатежиИП);
	ИтоговаяСтрока = ТаблицаДанныхЗаполнения[ИндексИтоговойСтроки];
	...
	ИтоговаяСтрока.СуммаНалогаИсчисленная = УчетНДФЛПредпринимателя.СуммаНалога(
		ОбщаяСуммаДохода - ОбщаяСуммаВычета,
		ПараметрыВыполнения.Период);
	ИтоговаяСтрока.СуммаНалогаУплаченнаяУдержанная = АвансовыеПлатежиИП;
	
	Если Не АвансовыеПлатежиРаспределены Тогда
		ИтоговаяСтрока.СтруктураДанных.Вставить("СуммаНачисленныхАвансовыхПлатежейИП", АвансовыеПлатежиИП);
		ИтоговаяСтрока.СтруктураДанных.Вставить("СуммаУплаченныхАвансовыхПлатежейИП", АвансовыеПлатежиИП);
	КонецЕсли;
...
Процедура ЗаполнитьИтогиВычетаДобровольныеПенсионныеВзносыИСтрахованиеЖизни(ТаблицаДанныхЗаполнения, ИтогиРасчета, ВыбраннаяФорма)
	
	ОтборСтрок = Новый Структура;
	ОтборСтрок.Вставить("Вид", Перечисления.ВычетыФизическихЛиц.ДобровольныеПенсионныеВзносыИСтрахованиеЖизни);
	...
	СтруктураДанных = НайденныеСтроки[0].СтруктураДанных;
	
	ЭтоДолгосрочноеСтрахование = ВычетПоДоговорамДолгосрочногоСтрахования(СтруктураДанных.ВидДоговораПенсионногоОбеспечения, ВыбраннаяФорма);
	
	Если ЭтоДолгосрочноеСтрахование И ИтогиРасчета.ДолгосрочноеСтрахованиеДети > 0 Тогда
		ПредельнаяСуммаПоПДС = ПределыВычетов.ДолгосрочноеСтрахованиеДети;
	ИначеЕсли ЭтоДолгосрочноеСтрахование Тогда
		ПредельнаяСуммаПоПДС = ПределыВычетов.ДолгосрочныеСбереженияНПФ;
	Иначе
		ПредельнаяСуммаПоПДС = ПределыВычетов.СоциальныеВычеты;
	КонецЕсли;
	
	СуммаПоДоговорамПДФ = Мин(ИтогиРасчета.ДолгосрочныеСбереженияНПФ + ИтогиРасчета.ДолгосрочноеСтрахование, ПредельнаяСуммаПоПДС);
	СуммаПоДоговорамИИС = Мин(ИтогиРасчета.ДолгосрочныеСбереженияИИС, ПределыВычетов.ДолгосрочныеСбереженияИИС);
	
	СтруктураДанных.Вставить("ДолгосрочныеСбереженияНПФ", ИтогиРасчета.ДолгосрочныеСбереженияНПФ);
	СтруктураДанных.Вставить("ДолгосрочныеСбереженияИИС", ИтогиРасчета.ДолгосрочныеСбереженияИИС);
	СтруктураДанных.Вставить("ДолгосрочноеСтрахование", ИтогиРасчета.ДолгосрочноеСтрахование);
	СтруктураДанных.Вставить("ДолгосрочныеСбереженияОбщаяСумма", СуммаПоДоговорамПДФ + СуммаПоДоговорамИИС);
	
КонецПроцедуры

Функция ЭтоДоговорПДС(ВидДоговора, ВыбраннаяФорма)
	
	// Вычет предоставляется в соответствие со ст. 219.2 НК РФ
	
	ЭтоПенсионныеНакопления = ВычетПоДоговорамДолгосрочныхСбереженийНПФ(ВидДоговора, ВыбраннаяФорма);
	ЭтоДолгосрочноеСтрахование = ВычетПоДоговорамДолгосрочногоСтрахования(ВидДоговора, ВыбраннаяФорма);
	
	Возврат ЭтоПенсионныеНакопления Или ЭтоДолгосрочноеСтрахование;
...
Процедура ЗаполнитьНачальныеПоказатели(Показатели)
	...
	ПараметрыВычета = УчетНДФЛПредпринимателя.НовыйПараметрыВычета();
	ПараметрыВычета.НачалоПериода     = Макс(Показатели.НачалоНалоговогоПериода, Показатели.НачалоПериода);
	ПараметрыВычета.КонецПериода      = КонецПериода;
	ПараметрыВычета.Организация       = Организация;
