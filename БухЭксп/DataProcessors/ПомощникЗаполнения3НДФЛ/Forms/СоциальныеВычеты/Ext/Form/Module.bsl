...
Функция ТекстПодсказкиОбучениеДетей(ВыбраннаяФорма)
	...
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма)
		Или ВыбраннаяФорма = "ФормаОтчета2025кв1" Тогда
		
		СоциальныйВычетНаОбучениеДетей = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПомощникЗаполнения3НДФЛ.ПределыВычетов(ВыбраннаяФорма),
			"СоциальныйВычетНаОбучениеДетей", 0);
		ВычетСтрокой = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru=';%1 рубль;;%1 рубля;%1 рублей;'"),
			СоциальныйВычетНаОбучениеДетей);
		
		Результат = СтрШаблон(НСтр("ru = 'Сумма расходов на обучение детей по очной форме (не более %1 за каждого ребенка на обоих родителей).
		|Исключаются расходы, оплаченные за счет средств материнского капитала.
		|Если обучение осуществляется в иностранных организациях, сумму расходов нужно указать отдельно через 1С-Отчетность.
		|
		|<a href=""https://its.1c.ru/bmk/ndfl_deduction/education"">Подробнее</a>'"), ВычетСтрокой);
	Иначе
