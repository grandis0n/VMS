...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаСоставленияДекларации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Декларация3НДФЛ, "ДатаПодписи");
...
Процедура ДоходыВычетыПослеУдаления(Элемент)
	...
	ДоступныеДоходы = ДоступныеДоходы(ЭтотОбъект);
...
Процедура ПерезаполнитьДоходыИВычеты()
	...
	Если НавигацияПараметрФормы <> "Вычеты" Тогда
...
Процедура ДобавитьНовыйДоходВычет(Вид, КлючНазначенияФормы = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяФормыДоходаВычета) Тогда
		...
		ПараметрыФормы.Вставить("ДатаСоставленияДекларации", ДатаСоставленияДекларации);
...
Процедура ДобавлениеДоходаВычетаЗавершение(СтруктураДоходовВычетов, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтруктураДоходовВычетов) = Тип("Структура") Тогда
		...
		ИначеЕсли ЭтоСтандартныйВычет(НоваяСтрока.Вид) Тогда
			 ПроверитьИСохранитьСтандартныйВычет(НоваяСтрока);
		КонецЕсли;
...
Процедура ИзменениеДоходаВычетаЗавершение(Результат, ТекущиеДанные) Экспорт
	...
	ИначеЕсли ЭтоСтандартныйВычет(ТекущиеДанные.Вид) Тогда
		
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "СуммаВычета", 0) = 0 Тогда
			Объект.ДоходыВычеты.Удалить(ТекущиеДанные);
		КонецЕсли;
		
		ПроверитьИСохранитьСтандартныйВычет(ТекущиеДанные);
...
Процедура ЗаполнитьСтандартныеВычеты()
	...
	ВведенныеДоходы = ДоступныеДоходы(ЭтотОбъект);
...
&НаКлиенте
Функция ЭтоСтандартныйВычет(ВидВычета)
	
	Возврат ВидВычета = ПредопределенноеЗначение("Перечисление.ВычетыФизическихЛиц.СтандартныйНаНалогоплательщика")
		Или ВидВычета = ПредопределенноеЗначение("Перечисление.ВычетыФизическихЛиц.СтандартныйНаДетей")
	
КонецФункции

&НаКлиенте
Процедура ПроверитьИСохранитьСтандартныйВычет(СтрокаДоходыВычеты)
	
	СтруктураВычета = СтрокаДоходыВычеты.СтруктураДоходовВычетов;
	
	Если СтрокаДоходыВычеты.Вид = ПредопределенноеЗначение("Перечисление.ВычетыФизическихЛиц.СтандартныйНаДетей") Тогда
		ДанныеВычета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураВычета, "СведенияОДетях");
	Иначе
		ДанныеФормы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураВычета, "ДанныеФормы");
		ДанныеВычета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеФормы, "СуммаВычетаПоМесяцам");
	КонецЕсли;
	
	Если ДанныеВычета <> Неопределено Тогда
		ВычетНалогоплательщика = СтандартныйВычетПоТекущимДанным(СтрокаДоходыВычеты);
		Если СохранитьСтандартныйВычет(Объект.Организация, Объект.Период, ДанныеВычета, ВычетНалогоплательщика) Тогда
			ПодключитьПроверкуАктуальности();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Функция СтандартныйВычетПоТекущимДанным(СтрокаДоходыВычеты)
	
	Результат = Новый Структура("Вид, СтруктураДоходовВычетов");
	Результат.Вид = СтрокаДоходыВычеты.Вид;
	Результат.СтруктураДоходовВычетов = СтрокаДоходыВычеты.СтруктураДоходовВычетов;
...
&НаСервереБезКонтекста
Функция СохранитьСтандартныйВычет(Организация, Период, ДанныеВычета, ВычетНаНалогоплательщика)
	
	ВидВычета = ВычетНаНалогоплательщика.Вид;
	
	Если Не ЗначениеЗаполнено(ВидВычета) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	СданыНормыГТО = Неопределено;
	
	Если ВидВычета = Перечисления.ВычетыФизическихЛиц.СтандартныйНаНалогоплательщика Тогда
		ИмяРегистра = "РегистрСведений.ИПСтандартныеВычетыНаНалогоплательщика";
		СтруктураВычета = ВычетНаНалогоплательщика.СтруктураДоходовВычетов;
		СданыНормыГТО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураВычета, "СданыНормыГТО");
	Иначе
	...
	Если СданыНормыГТО <> Неопределено Тогда
		РеквизитыЗаписи.СданыНормыГТО = СданыНормыГТО;
		Для Каждого ТекущаяСтрока Из ДанныеВычета Цикл
			ТекущаяСтрока.Вставить("СданыНормыГТО", СданыНормыГТО);
		КонецЦикла;
	КонецЕсли;
	
	Возврат УчетНДФЛПредпринимателя.СохранитьСтандартныйВычетЗаПериод(ДанныеВычета, РеквизитыЗаписи);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ДоступныеДоходы(Форма)
	
	СтрокиДоходов = Форма.Объект.ДоходыВычеты.НайтиСтроки(Новый Структура("ЭтоВычет", Ложь));
