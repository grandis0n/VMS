...
Асинх Процедура Очистить(Команда)
	...
	РезультатВопроса = Ждать ВопросАсинх(
		НСтр("ru = 'Журнал будет очищен полностью. Продолжить?'"),
...
Функция ОчиститьИсториюОперацийОчередиЧеков()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			...
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийОчередиЧеков");
		// Конец Вызов БСП
	Иначе
...
Функция ОчиститьИсториюПлатежныхОпераций()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюПлатежныхОпераций");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюПлатежныхОпераций",
...
Функция ОчиститьИсториюФискальныхОпераций()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюФискальныхОпераций");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюФискальныхОпераций",
...
Функция ОчиститьИсториюОперацийПроверкиКМ()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийПроверкиКМ");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийПроверкиКМ",
...
Функция ОчиститьИсториюОперацийСОборудованием()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяБСП() Тогда
		...
		Возврат МодульДлительныеОперации.ВыполнитьПроцедуру(
			ПараметрыВыполненияПроцедуры, 
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийСОборудованием");
		// Конец Вызов БСП
	Иначе
		Задание = ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийСОборудованием",
...
Функция АктивностьФоновыхЗаданийНаСервере()
	...
	Отбор.Вставить("ИмяМетода", "ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюПлатежныхОпераций");
	...
	Отбор.Вставить("ИмяМетода", "ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюФискальныхОпераций");
	Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	Массив = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Результат.Вставить("ОчиститьФискальныеОперации", Массив.Количество() > 0);

	Отбор.Вставить("ИмяМетода", "ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийОчередиЧеков");
	Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	Массив = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Результат.Вставить("ОчиститьОчередь", Массив.Количество() > 0);
	
	Отбор.Вставить("ИмяМетода", "ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийПроверкиКМ");
	Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	Массив = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Результат.Вставить("ОчиститьОперацииПроверкиКМ", Массив.Количество() > 0);

	Отбор.Вставить("ИмяМетода", "ЛогированиеОперацийБПО.ОчиститьПолностьюИсториюОперацийСОборудованием");
	Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	Массив = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Результат.Вставить("ОчиститьОперацииСОборудованием", Массив.Количество() > 0);
...
Процедура Подключаемый_УстановитьДоступностьКнопокОчистка()
	...
	КонецЕсли;
	Если АктивностьФоновыхЗаданий.ОчиститьОперацииПроверкиКМ Тогда
		Элементы.ОчиститьОперацииПроверкиКМ.Доступность = Ложь;
		Элементы.ОчиститьОперацииПроверкиКМ.Заголовок = НСтр("ru = 'Очищается...'");
	Иначе
		Элементы.ОчиститьОперацииПроверкиКМ.Доступность = Истина;
		Элементы.ОчиститьОперацииПроверкиКМ.Заголовок = НСтр("ru = 'Очистить'");
	КонецЕсли;
	Если АктивностьФоновыхЗаданий.ОчиститьОперацииСОборудованием Тогда
		Элементы.ОчиститьОперацииСОборудованием.Доступность = Ложь;
		Элементы.ОчиститьОперацииСОборудованием.Заголовок = НСтр("ru = 'Очищается...'");
	Иначе
		Элементы.ОчиститьОперацииСОборудованием.Доступность = Истина;
		Элементы.ОчиститьОперацииСОборудованием.Заголовок = НСтр("ru = 'Очистить'");
	КонецЕсли;

	
	Если АктивностьФоновыхЗаданий.ОчиститьОчередь
		Или АктивностьФоновыхЗаданий.ОчиститьПлатежныеОперации
		Или АктивностьФоновыхЗаданий.ОчиститьФискальныеОперации
		Или АктивностьФоновыхЗаданий.ОчиститьОперацииПроверкиКМ
		Или АктивностьФоновыхЗаданий.ОчиститьОперацииСОборудованием Тогда
