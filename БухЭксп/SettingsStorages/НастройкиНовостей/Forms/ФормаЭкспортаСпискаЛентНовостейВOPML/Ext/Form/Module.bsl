...
Процедура КомандаЭкспорт(Команда)
	...
	СтрокаXML = ПолучитьOPMLНаСервере();
...
Функция ПолучитьOPMLНаСервере()
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("opml");
	ЗаписьXML.ЗаписатьАтрибут("version", "1.1");
		ЗаписьXML.ЗаписатьНачалоЭлемента("head");
			ЗаписьXML.ЗаписатьНачалоЭлемента("title");
				ЗаписьXML.ЗаписатьТекст(НСтр("ru='Список лент новостей (1С)'"));
			ЗаписьXML.ЗаписатьКонецЭлемента(); // title
			ЗаписьXML.ЗаписатьНачалоЭлемента("dateCreated");
				ЗаписьXML.ЗаписатьТекст(ПолучитьТекущуюДатуВФорматеRFC822());
			ЗаписьXML.ЗаписатьКонецЭлемента(); // dateCreated
		ЗаписьXML.ЗаписатьКонецЭлемента(); // head
		ЗаписьXML.ЗаписатьНачалоЭлемента("body");
			ЗаписьXML.ЗаписатьНачалоЭлемента("outline");
				ЗаписьXML.ЗаписатьАтрибут("text", НСтр("ru='Новости 1С'"));
				Для каждого ТекущаяЛентаНовостей Из ЭтотОбъект.ЛентыНовостей Цикл
					Если ТекущаяЛентаНовостей.Пометка = Истина Тогда
						...
						ЗаписьXML.ЗаписатьНачалоЭлемента("outline");
							ЗаписьXML.ЗаписатьАтрибут("text", ТекущаяЛентаНовостей.Значение.Наименование);
							ЗаписьXML.ЗаписатьАтрибут("title", ТекущаяЛентаНовостей.Значение.Наименование);
							ЗаписьXML.ЗаписатьАтрибут("xmlUrl", лкИмяФайлаНаСервере);
							ЗаписьXML.ЗаписатьАтрибут("htmlUrl", лкИмяФайлаНаСервере);
						ЗаписьXML.ЗаписатьКонецЭлемента(); // outline
					КонецЕсли;
				КонецЦикла;
			ЗаписьXML.ЗаписатьКонецЭлемента(); // outline
		ЗаписьXML.ЗаписатьКонецЭлемента(); // body
	ЗаписьXML.ЗаписатьКонецЭлемента(); // opml
