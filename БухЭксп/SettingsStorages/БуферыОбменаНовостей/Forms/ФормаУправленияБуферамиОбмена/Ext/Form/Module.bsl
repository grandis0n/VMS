...
&НаСервере
Функция ВыгрузитьСписокБуферовОбменаНаСервере()
	...
	ЗаписьXML.УстановитьСтроку(
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента("Clipboards", "http://v8.1c.ru/news1c/wsAuxiliaryTypes"); // Самый верхний тег-обертка
			ЗаписьXML.ЗаписатьСоответствиеПространстваИмен("n1c", "http://v8.1c.ru/news1c/wsAuxiliaryTypes");

			ЗаписьXML.ЗаписатьНачалоЭлемента("ClipboardSet", "http://v8.1c.ru/news1c/wsAuxiliaryTypes"); // КлючОбъекта
			ЗаписьXML.ЗаписатьАтрибут("id", ЭтотОбъект.КлючОбъекта);

			Для каждого ТекущийЭлементСпискаБуферОбмена Из ЭтотОбъект.СписокКлючейНастроек Цикл
				ЗаписьXML.ЗаписатьНачалоЭлемента("Clipboard", "http://v8.1c.ru/news1c/wsAuxiliaryTypes"); // КлючНастройки
					ЗаписьXML.ЗаписатьАтрибут("id", ТекущийЭлементСпискаБуферОбмена.Значение);
					...
					Если ТипЗнч(ЗначениеБуфераОбмена)= ТипСписокЗначений Тогда
						Для каждого ТекущееЗначениеСтрокиБуфераОбмена Из ЗначениеБуфераОбмена Цикл
							Если ТипЗнч(ТекущееЗначениеСтрокиБуфераОбмена.Значение) = ТипСтруктура Тогда
								ЗаписьXML.ЗаписатьНачалоЭлемента("ClipboardRow", "http://v8.1c.ru/news1c/wsAuxiliaryTypes");
									СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ТекущееЗначениеСтрокиБуфераОбмена.Значение, НазначениеТипаXML.Явное, ФормаXML.Элемент);
...
&НаСервере
Процедура ЗагрузитьСписокБуферовОбменаНаСервере(Знач АдресВременногоХранилища)
	...
	ОбъектXDTO = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ФабрикаXDTO.Тип("http://v8.1c.ru/news1c/wsAuxiliaryTypes", "Clipboards"));
	Если ТипЗнч(ОбъектXDTO) = Тип("ОбъектXDTO") Тогда
		Если (ОбъектXDTO.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsAuxiliaryTypes")
				И (ВРег(СокрЛП(ОбъектXDTO.Тип().Имя)) = ВРег(СокрЛП("Clipboards"))) Тогда
			Свойство_ClipboardSet = ОбъектXDTO.Свойства().Получить("ClipboardSet");
			Если (ТипЗнч(Свойство_ClipboardSet) = Тип("СвойствоXDTO"))
					И (Свойство_ClipboardSet.ВерхняяГраница = -1) Тогда
				КлючиОбъектов = ОбъектXDTO.ПолучитьСписок(Свойство_ClipboardSet);
		...
		КонецЕсли;
	КонецЕсли;
	ЧтениеXML.Закрыть(); // Иначе не даст удалить "ИмяВременногоФайла".
	...
	Исключение
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru='Новости. Сервис и регламент. ЗагрузитьСписокБуферовОбменаНаСервере'", КодОсновногоЯзыка), // ИдентификаторШага.
...
Процедура ПослеВыгрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		СтрокаXML = ВыгрузитьСписокБуферовОбменаНаСервере();
		// ТекстовыйДокумент в отличие от ЗаписьТекста работает в веб-клиенте.
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(СтрокаXML);
		ТекстовыйДокумент.Записать(ИмяФайла, "UTF-8", Символы.ПС);
...
Процедура ПослеЗагрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
			И (ВыбранныеФайлы.Количество() > 0) Тогда
		// Помещать в "ЭтаФорма.УникальныйИдентификатор", чтобы при закрытии формы данные удалялись.
		АдресВременногоХранилища = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ВыбранныеФайлы[0]), ЭтаФорма.УникальныйИдентификатор);
		ЗагрузитьСписокБуферовОбменаНаСервере(АдресВременногоХранилища);
		ПоказатьОповещениеПользователя(
