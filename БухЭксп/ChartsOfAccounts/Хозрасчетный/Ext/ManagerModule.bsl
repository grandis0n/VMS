...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьОтсутствующиеСубконто_68_32_42");
...
Процедура ДобавитьОтсутствующиеСубконто_68_32_42(Параметры) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Хозрасчетный.Ссылка КАК СчетБезСубконто
	|ИЗ
	|	ПланСчетов.Хозрасчетный КАК Хозрасчетный
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Хозрасчетный.ВидыСубконто КАК ХозрасчетныйВидыСубконто
	|		ПО Хозрасчетный.Ссылка = ХозрасчетныйВидыСубконто.Ссылка
	|ГДЕ
	|	Хозрасчетный.Ссылка В (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНалоговогоАгента), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате))
	...
	Пока Выборка.Следующий() Цикл
		...
		КодСчета = Строка(Выборка.СчетБезСубконто);;
		Попытка
			СчетОбъект = Выборка.СчетБезСубконто.ПолучитьОбъект();
			ДобавитьПредопределенныйВидСубконто(
				СчетОбъект.ВидыСубконто, ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты, Истина);
			ДобавитьПредопределенныйВидСубконто(
				СчетОбъект.ВидыСубконто, ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры, Истина);
			ДобавитьПредопределенныйВидСубконто(
				СчетОбъект.ВидыСубконто, ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СФПолученные, Истина);
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(СчетОбъект);
			// Повторяем запись в режиме с бизнес-логикой
			СчетОбъект = Выборка.СчетБезСубконто.ПолучитьОбъект();
			СчетОбъект.ОбменДанными.Загрузка = Ложь;
			СчетОбъект.Записать();
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре %1 не удалось обработать счет %2 по причине:
					|%3'"),
					"ПланыСчетов.Хозрасчетный.ДобавитьОтсутствующиеСубконто_68_32_42()",
					КодСчета,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.ПланыСчетов.Хозрасчетный, 
				Выборка.СчетБезСубконто, 
				ТекстСообщения);
		КонецПопытки;
...
Процедура ДобавитьПредопределенныйВидСубконто(ВидыСубконто, НовыйВид, ТолькоОбороты)
	
	НоваяСтрока = ВидыСубконто.Добавить();
	НоваяСтрока.ВидСубконто = НовыйВид;
	НоваяСтрока.ТолькоОбороты = ТолькоОбороты;
	НоваяСтрока.Предопределенное = Истина;
	НоваяСтрока.Суммовой = Истина;
	НоваяСтрока.Валютный = Истина;
	НоваяСтрока.Количественный = Истина;
