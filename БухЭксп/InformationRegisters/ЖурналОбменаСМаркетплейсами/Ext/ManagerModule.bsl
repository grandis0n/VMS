...
Процедура ЗаписатьВЖурнал(ПараметрыЗаписи, Отказ = Ложь, Лог = "") Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗаписи.ИмяФайла) Тогда
		
		Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ПараметрыЗаписи.ИмяФайла) Тогда
			ВызватьИсключение "Добавлять файл в архив можно только из каталога временных файлов";
		КонецЕсли;
		
		НоваяЗапись.Файл = ХранилищеДанныхФайла(ПараметрыЗаписи.ИмяФайла);
...
Функция ХранилищеДанныхФайла(ИмяФайла)
	...
	Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ИмяФайла) Тогда
		ВызватьИсключение "Получать данные файла можно только из каталога временных файлов";
	КонецЕсли;
