...
// Выполняет полную очистку регистра.
//
Процедура ОчиститьРегистрПолностью() Экспорт
...
Процедура ЗаписатьЛогОперацииНаСервере(Записи) Экспорт
	...
	КлючиОпераций = НаборЗаписей.Выгрузить(, "Оборудование,Дата,ГодМесяц");
	КлючиОпераций.Свернуть("Оборудование,Дата,ГодМесяц");
	НачатьТранзакцию();
	Попытка
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОперацииСПодключаемымОборудованием");
		ЭлементБлокировки.ИсточникДанных = КлючиОпераций;
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Оборудование", "Оборудование");
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Дата", "Дата");
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("ГодМесяц", "ГодМесяц");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		...
		УстановитьСвободныеНомераОпераций(НаборЗаписей, КлючиОпераций);
...
Процедура УстановитьСвободныеНомераОпераций(НаборЗаписей, КлючиОпераций)
	
	МаксимальныеНомера = МаксимальныеНомераОпераций(КлючиОпераций);
	
	Для Каждого Запись Из НаборЗаписей Цикл
		
		Отбор = Новый Структура;
		Отбор.Вставить("Оборудование", Запись.Оборудование);
		Отбор.Вставить("Дата", Запись.Дата);
		Отбор.Вставить("ГодМесяц", Запись.ГодМесяц);
		СтрокиНомеров = МаксимальныеНомера.НайтиСтроки(Отбор);
		
		Если СтрокиНомеров.Количество() = 0 Тогда
			СтрокаНомера = МаксимальныеНомера.Добавить();
			СтрокаНомера.Оборудование = Запись.Оборудование;
			СтрокаНомера.Дата = Запись.Дата;
			СтрокаНомера.ГодМесяц = Запись.ГодМесяц;
			СтрокаНомера.НомерОперации = 0;
		Иначе
			СтрокаНомера = СтрокиНомеров[0];
		КонецЕсли;
		
		Запись.НомерОперации = СтрокаНомера.НомерОперации + 1;
		СтрокаНомера.НомерОперации = Запись.НомерОперации;
		
	КонецЦикла;
	
КонецПроцедуры

Функция МаксимальныеНомераОпераций(КлючиОпераций)
	
	Результат = Новый ТаблицаЗначений;
	Если КлючиОпераций.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КлючиОпераций.Оборудование КАК Оборудование,
		|	КлючиОпераций.Дата КАК Дата,
		|	КлючиОпераций.ГодМесяц КАК ГодМесяц
		|ПОМЕСТИТЬ ВТ_КлючиОпераций
		|ИЗ
		|	&КлючиОпераций КАК КлючиОпераций
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_КлючиОпераций.Оборудование КАК Оборудование,
		|	ВТ_КлючиОпераций.Дата КАК Дата,
		|	ВТ_КлючиОпераций.ГодМесяц КАК ГодМесяц,
		|	ЕСТЬNULL(МАКСИМУМ(ОперацииСПодключаемымОборудованием.НомерОперации), 0) КАК НомерОперации
		|ИЗ
		|	ВТ_КлючиОпераций КАК ВТ_КлючиОпераций
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОперацииСПодключаемымОборудованием КАК ОперацииСПодключаемымОборудованием
		|		ПО ОперацииСПодключаемымОборудованием.Оборудование = ВТ_КлючиОпераций.Оборудование
		|			И ОперацииСПодключаемымОборудованием.Дата = ВТ_КлючиОпераций.Дата
		|			И ОперацииСПодключаемымОборудованием.ГодМесяц = ВТ_КлючиОпераций.ГодМесяц
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_КлючиОпераций.Оборудование,
		|	ВТ_КлючиОпераций.Дата,
		|	ВТ_КлючиОпераций.ГодМесяц";
	
	Запрос.УстановитьПараметр("КлючиОпераций", КлючиОпераций);
	РезультатЗапроса = Запрос.Выполнить();
	Результат = РезультатЗапроса.Выгрузить();
	Результат.Индексы.Добавить("Оборудование,Дата,ГодМесяц");
