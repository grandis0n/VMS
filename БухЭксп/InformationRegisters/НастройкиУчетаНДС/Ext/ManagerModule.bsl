...
// Проверяет использование раздельного учета НДС, отсутствующего в тарифе абонента.
// 
// Возвращаемое значение:
//   Булево - Истина, если используется неразрешенный раздельный учет НДС.
//
Функция ЕстьИспользованиеРаздельногоУчетаНДСБезТарифа() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	НастройкиУчетаНДССрезПоследних.СложныйУчетНДС КАК СложныйУчетНДС
	|ИЗ
	|	РегистрСведений.НастройкиУчетаНДС.СрезПоследних(&НаДату) КАК НастройкиУчетаНДССрезПоследних
	|ГДЕ
	|	НЕ НастройкиУчетаНДССрезПоследних.Организация.ПометкаУдаления
	|	И НастройкиУчетаНДССрезПоследних.СложныйУчетНДС";
	
	Возврат Не Запрос.Выполнить().Пустой();
	
КонецФункции

// Возвращает организацию, для которой включен раздельный учет НДС.
//
// Возвращаемое значение:
//   СправочникСсылка.Организации - организация с включенным раздельным учетом НДС.
//
Функция ОрганизацияИспользующаяРаздельныйУчетНДС() Экспорт
