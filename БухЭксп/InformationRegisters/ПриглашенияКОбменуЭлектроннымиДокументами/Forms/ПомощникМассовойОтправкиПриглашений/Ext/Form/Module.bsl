...
Процедура ПриглашенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПриглашенияПредупреждение И ТекущиеДанные.АбонентНеТребуетПриглашений Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("ru='Для выбранного абонента отправка приглашения не требуется.'");
...
Процедура ЗаполнитьКонтрагентов(МассивКонтрагентов)
	...
	Для Каждого Контрагент Из МассивКонтрагентов Цикл
		НовоеПриглашение = Приглашения.Добавить();
		НовоеПриглашение.Контрагент = Контрагент;
		УстановитьПризнакФизЛицаДляПолучателя(Контрагент, НовоеПриглашение.ПолучательФизЛицо);
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуПриглашений(ПараметрыСвязиИдентификаторов)
	...
	Для Каждого СтрокаПараметровСвязи Из ПараметрыСвязиИдентификаторов Цикл
		...
		УстановитьПризнакФизЛицаДляПолучателя(СтрокаПараметровСвязи.Контрагент, НоваяСтрока.ПолучательФизЛицо);
...
Процедура ЗаполнитьРеквизитыПриглашения(ТекущиеДанные, ПараметрыСвязи)
	...
	ТекущиеДанные.АбонентНеТребуетПриглашений = ПараметрыСвязи.АбонентНеТребуетПриглашений;
...
Процедура ЗаполнитьПредупреждениеПриглашения(ТекущиеДанные, ПараметрыСвязиПолучены);
	...
	АктуальнаяВерсияОтправителя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Лев(ИдентификаторОрганизации, 3));
	АктуальнаяВерсияПолучателя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			ТекущиеДанные.ОператорКонтрагента);
	...
	ОбменСФизЛицамиНеПоддерживается = (ОтправительФизЛицо Или ТекущиеДанные.ПолучательФизЛицо)
	...
	Если ТекущиеДанные.АбонентНеТребуетПриглашений Тогда
		
		ТекущиеДанные.Предупреждение = НСтр("ru = 'Абонент не требует приглашений'");
		ТекущиеДанные.Отбор = Отбор_РекомендуетсяПроверить();
	...
	Если ТекущиеДанные.ОператорыСовпадают Тогда
		Возврат;
	ИначеЕсли ТекущиеДанные.СвязьДоступна = -1
...
Процедура ЗаполнитьОтметкуВыбора(ТекущиеДанные)
	...
	ТекущиеДанные.Выбран = ТекущиеДанные.Отбор <> Отбор_ТребуютсяДействия()
		И ТекущиеДанные.Отбор <> Отбор_СвязьУжеНастроена() И Не ТекущиеДанные.АбонентНеТребуетПриглашений;
	Элементы.EmailОрганизации.Видимость = ТребуетсяЗаполнитьEmailОрганизации();
