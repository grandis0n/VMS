...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		УстановитьПризнакФизЛицаДляПолучателя(Контрагент, ПолучательФизЛицо);
	КонецЕсли;
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Элементы.ГруппаИдентификаторКонтрагентаИнформация.Видимость = Ложь;
	Элементы.ГруппаИдентификаторКонтрагентаИнформацияНаСтраницеВыбора.Видимость = Ложь;
...
Процедура УчетныеЗаписиКонтрагентаПриАктивизацииСтроки(Элемент)
	...
	Если Не Элемент.ТекущиеДанные = Неопределено 
		И Элементы.СтраницыПолучательСпособВыбора.ТекущаяСтраница = Элементы.СтраницаПолучательВыборОператора Тогда
...
Процедура ПриИзмененииИдентификатораКонтрагента()
	...
	Элементы.НазначениеКонтрагентаНаСтраницеВыбора.Заголовок = "";
	Элементы.ДокументыКонтрагентаНаСтраницеВыбора.Заголовок = "";
	Элементы.ОператорКонтрагента.Заголовок = "";
	Элементы.ДокументыКонтрагента.Заголовок = "";
	Элементы.НазначениеКонтрагента.Заголовок = "";
	...
	Если ЗначениеЗаполнено(ИдентификаторКонтрагента) Тогда
		...
		Если ИдентификаторАбонентаКорректный Тогда
			...
			СтрокиОператора = ТаблицаОператоровЭДО.НайтиСтроки(Новый Структура("ИдентификаторОператора", 
				Лев(ИдентификаторКонтрагента, 3)));
			ЗаголовокОператора = ?(ЗначениеЗаполнено(СтрокиОператора), 
				СтрокиОператора[0].Представление, НСтр("ru = 'Оператор не определён'"));
			...
			Элементы.ГруппаИдентификаторКонтрагентаИнформацияНаСтраницеВыбора.Видимость = Истина;
			Элементы.ГруппаИдентификаторКонтрагентаИнформация.Видимость = Истина;
...
Процедура ПодготовитьФормуПослеПолученияДанныхДляНастройкиОтправкиПриглашения(Знач АдресРезультата)
	...
	Если ЗначениеЗаполнено(ПараметрыСвязиИдентификаторов) Тогда
		...
		Для Каждого ИдентификаторДляВыбора Из ПараметрыТекущейСвязи.ДополнительныеДанныеСпискаВыбора Цикл
			...
			ОписаниеТиповДокументов = РегистрыСведений.ОператорыЭДО.ПредставлениеПоддерживаемыхТиповДокументов(
				СвойстваУчетнойЗаписи.КодыТиповДокументовКОбмену);
			ОписаниеТиповДокументов = СтрШаблон(НСтр("ru = 'Для документов: %1'"),
				?(ЗначениеЗаполнено(ОписаниеТиповДокументов), ОписаниеТиповДокументов, НСтр("ru='Все'")));
			...
			ОписаниеУчетнойЗаписи.ПоддерживаемыеТипыДокументов = ОписаниеТиповДокументов;
			ОписаниеУчетнойЗаписи.СтатусПриглашений = ?(СвойстваУчетнойЗаписи.АбонентНеТребуетПриглашений,
				НСтр("ru = 'Без приглашений'"), "");
...
Процедура ЗаполнитьИдентификаторыОрганизации()
	...
	УстановитьПризнакФизЛицаДляОтправителя(ЭтотОбъект);
...
Процедура УстановитьРеквизитыКонтрагентаВПриглашение(Приглашение)
	...
	Приглашение.ВидУчетнойЗаписи = ВидУчетнойЗаписи(Контрагент);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВидУчетнойЗаписи(Контрагент)
	
	ВидУчетнойЗаписи = Перечисления.ВидУчетнойЗаписиЭДО.ПустаяСсылка();
	Если ИнтеграцияЭДОВызовСервера.ПоддержанаРаботаСФизическимиЛицами() Тогда
		Если ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(Контрагент).ЭтоФизЛицо Тогда
			ВидУчетнойЗаписи = Перечисления.ВидУчетнойЗаписиЭДО.ФизическоеЛицо;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ВидУчетнойЗаписи;
...
Процедура УстановитьДоступностьИВидимостьКоманднойПанелиОсновнойСтраницы(Форма)
	...
	ДанныеПоИдентификаторам = ДанныеДляПриглашенияПоИдентификаторамАбонентов(Форма.УчетныеЗаписиКонтрагента,
		Форма.ИдентификаторОрганизации, Форма.ИдентификаторКонтрагента);
	
	АктуальнаяВерсияОтправителя = ДанныеПоИдентификаторам.АктуальнаяВерсияОтправителя;
	АктуальнаяВерсияПолучателя = ДанныеПоИдентификаторам.АктуальнаяВерсияПолучателя;
	
	ОтправительПоддерживает_5_03_ИВыше = СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальнаяВерсияОтправителя, "5.03") >= 0;
	ПолучательПоддерживает_5_03_ИВыше = СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальнаяВерсияПолучателя, "5.03") >= 0;
	
	ОбменСФизЛицамиНеПоддерживается = (Форма.ОтправительФизЛицо Или Форма.ПолучательФизЛицо)
		И (ОтправительПоддерживает_5_03_ИВыше Или ПолучательПоддерживает_5_03_ИВыше)
		И (ОтправительПоддерживает_5_03_ИВыше <> ПолучательПоддерживает_5_03_ИВыше);
		
	СвязьОператоровДоступна = СвязьОператоровДоступна(Форма.ИдентификаторОрганизации, Форма.ИдентификаторКонтрагента)
		И Не ОбменСФизЛицамиНеПоддерживается;
	...
	ЗаголовокДокументов = ?(ЗначениеЗаполнено(ДанныеПоИдентификаторам.ПоддерживаемыеТипыДокументов), 
		ДанныеПоИдентификаторам.ПоддерживаемыеТипыДокументов, НСтр("ru = 'Все'"));
	
	ЭлементыФормы.НазначениеКонтрагента.Заголовок = ДанныеПоИдентификаторам.Назначение;
	ЭлементыФормы.НазначениеКонтрагентаНаСтраницеВыбора.Заголовок = ДанныеПоИдентификаторам.Назначение;
	ЭлементыФормы.ДокументыКонтрагента.Заголовок = ЗаголовокДокументов;
	ЭлементыФормы.ДокументыКонтрагентаНаСтраницеВыбора.Заголовок = ЗаголовокДокументов;
...
&НаСервереБезКонтекста
Процедура УстановитьПризнакФизЛицаДляОтправителя(Форма)
	
	Форма.ОтправительФизЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(Форма.Организация).ЭтоФизЛицо;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УстановитьПризнакФизЛицаДляПолучателя(Контрагент, ПолучательФизЛицо)
	
	ПолучательФизЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(Контрагент).ЭтоФизЛицо;
...
// Возвращает данные для приглашения абонента по идентификаторам организации и контрагента
// 
// Параметры:
//  УчетныеЗаписиКонтрагента - ДанныеФормыКоллекция
//  ИдентификаторОрганизации - Строка - Идентификатор организации
//  ИдентификаторКонтрагента - Строка - Идентификатор контрагента
// 
// Возвращаемое значение:
//  Структура:
//	* АктуальнаяВерсияОтправителя - Строка - Актуальная версия отправителя
//	* АктуальнаяВерсияПолучателя - Строка - Актуальная версия получателя
//	* ПоддерживаемыеТипыДокументов - Строка - Поддерживаемые типы документов
//	* Назначение - Строка - Описание назначения
//	* ТребуетсяПриглашение - Булево - Признак использования приглашений
//
&НаСервереБезКонтекста
Функция ДанныеДляПриглашенияПоИдентификаторамАбонентов(Знач УчетныеЗаписиКонтрагента, 
			Знач ИдентификаторОрганизации, Знач ИдентификаторКонтрагента)
	
	ПараметрыТекущейЗаписи = Новый Структура("Идентификатор", ИдентификаторКонтрагента);
	НайденныеСтроки = УчетныеЗаписиКонтрагента.НайтиСтроки(ПараметрыТекущейЗаписи);
	Если НайденныеСтроки.Количество() Тогда
		КодыТиповДокументовКОбмену = НайденныеСтроки[0].КодыТиповДокументовКОбмену;
		Назначение = НайденныеСтроки[0].Комментарий;
		ТребуетсяПриглашение = Не НайденныеСтроки[0].АбонентНеТребуетПриглашений;
	Иначе
		КодыТиповДокументовКОбмену = "";
		Назначение = "";
		ТребуетсяПриглашение = Ложь;
	КонецЕсли;
	
	СтруктураОтвета = Новый Структура;
	
	ИдентификаторОператораОрганизации = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(ИдентификаторОрганизации);
	ИдентификаторОператораКонтрагента = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(ИдентификаторКонтрагента);
	 
	СтруктураОтвета.Вставить("АктуальнаяВерсияОтправителя",
		СервисНастроекЭДОПовтИсп.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(ИдентификаторОператораОрганизации));
			
	СтруктураОтвета.Вставить("АктуальнаяВерсияПолучателя",
		СервисНастроекЭДОПовтИсп.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(ИдентификаторОператораКонтрагента));
	
	СтруктураОтвета.Вставить("ПоддерживаемыеТипыДокументов",
		РегистрыСведений.ОператорыЭДО.ПредставлениеПоддерживаемыхТиповДокументов(КодыТиповДокументовКОбмену));
			
	СтруктураОтвета.Вставить("Назначение", Назначение);
	СтруктураОтвета.Вставить("ТребуетсяПриглашение", ТребуетсяПриглашение);
