...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		УстановитьПризнакФизЛицаДляПолучателя(ЭтотОбъект);
	КонецЕсли;
...
&НаКлиенте
Процедура ИдентификаторКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтруктураПараметров = Новый Структура("ИдентификаторыКонтрагента, Контрагент, ИдентификаторОрганизации", 
		Элементы.ИдентификаторКонтрагента.СписокВыбора, Контрагент, ИдентификаторОрганизации);
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПолучитьИдентификаторКонтрагента", ЭтотОбъект);
...
&НаСервереБезКонтекста
Процедура УстановитьПризнакФизЛицаДляПолучателя(Форма)
	
	Форма.ПолучательФизЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(Форма.Контрагент).ЭтоФизЛицо;
...
Процедура ОбновитьНастройкуОтправкиПриглашения(Форма, ПараметрыОтправки = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыОтправки.СтатусПриглашенияПредставление) Тогда
		Форма.СтатусПриглашенияПредставление = 
			Новый ФорматированнаяСтрока(ПараметрыОтправки.СтатусПриглашенияПредставление);
	КонецЕсли;
	...
	АктуальнаяВерсияОтправителя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Форма.ОператорОрганизации);
	АктуальнаяВерсияПолучателя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Форма.ОператорКонтрагента);
	...
	Если ЗначениеЗаполнено(Форма.ИдентификаторКонтрагента)
			И (Форма.ТипПредупреждения <> ТипПредупреждения_ТребуетсяИдентификаторЭДО()
				И Не ИдентификаторКонтрагентаКорректный(Форма))
			Или ОбменСФизЛицамиНеПоддерживается Тогда
...
Функция НовыеПараметрыОтправкиПриглашения(ТипИнформацииОКонтрагенте, ПараметрыСвязиИдентификаторов = Неопределено)
	...
	ПараметрыОтправки.Вставить("АбонентНеТребуетПриглашений", Ложь);
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОтправки, СтрокаПараметровСвязи,, "АбонентНеТребуетПриглашений");
...
&НаКлиенте
Процедура ПолучитьИдентификаторКонтрагента(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ИдентификаторКонтрагента = Результат;
