...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидыДокументовСФ = ЭлектронныеДокументыЭДО.ВидыДокументовСФ();
	
	УстановитьУсловноеОформление();
	
	Элементы.ГруппаОжидание.Видимость = Ложь;
	Элементы.ГруппаОжиданиеУдаления.Видимость = Ложь;
	ИспользуютсяНастройкиФормированияУниверсальныхДокументов = 
		ИнтеграцияЭДО.ИспользуютсяНастройкиФормированияУниверсальныхДокументов();
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаИспользовать.Доступность = НЕ Форма.ОрганизацияЭтоФизЛицо;
...
Процедура УстановитьОтборПоУПД_УКД(Форма)
	...
	ВидыДокументовСФ = Форма.ВидыДокументовСФ;
	Элементы = Форма.Элементы;
	...
	Для Каждого СтрокаТЧ Из Форма.ИсходящиеДокументы Цикл
		...
		Если Форма.ОрганизацияЭтоФизЛицо И 
			(СтрокаТЧ.ВидДокумента = ВидыДокументовСФ.СФ ИЛИ СтрокаТЧ.ВидДокумента = ВидыДокументовСФ.КСФ) Тогда
			СтрокаТЧ.Отображать = Ложь;
		КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере(ПараметрыФормы)
	...
	ЗаполнитьОрганизацияЭтоФизЛицо();
...
Процедура УстановитьИдентификаторыЭДО(АдресРезультата)
	...
	АктуальнаяВерсияОтправителя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Лев(Идентификаторы.ИдентификаторОтправителя, 3));
	АктуальнаяВерсияПолучателя =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(
			Лев(Идентификаторы.ИдентификаторПолучателя, 3));
	...
	ОбменСФизЛицамиНеПоддерживается = (ОрганизацияЭтоФизЛицо Или ПолучательФизЛицо)
	...
	Если ОбменСФизЛицамиНеПоддерживается Тогда
		ИдентификаторПолучателя = "";
	КонецЕсли;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	Если НЕ ОрганизацияЭтоФизЛицо Тогда
		ПолучитьОбщиеПараметрыНастроек();
	Иначе
		ИспользоватьУКД = Ложь;
		ИспользоватьУПД = Ложь;
	КонецЕсли;
		
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОрганизацияЭтоФизЛицо()
	
	ОрганизацияЭтоФизЛицо = Ложь;
		
	Если ЗначениеЗаполнено(Организация) Тогда
		ОрганизацияЭтоФизЛицо = ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(Организация).ЭтоФизЛицо;
	КонецЕсли;
