...
&НаСервере
Функция ПроверкаНаличияШаблонаВБазе(ШаблонЭВФ, ТаблицаШаблоновВБазе)
	...
	СтруктураПоиска.Вставить("ОКУД", ОКУД);
	СтруктураПоиска.Вставить("КодШаблона", ШаблонЭВФ.КодШаблона);
	СтруктураПоиска.Вставить("Версия", ШаблонЭВФ.Версия);
	СтруктураПоиска.Вставить("Размер", ШаблонЭВФ.Размер);
	СтруктураПоиска.Вставить("КодПериодичности", ШаблонЭВФ.КодПериодичности);
	
	Возврат ЗначениеЗаполнено(ТаблицаШаблоновВБазе.НайтиСтроки(СтруктураПоиска));
КонецФункции

&НаСервере
Функция ТаблицаШаблоновВБазе()
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ШаблоныЭВФОтчетовСтатистики.Размер КАК Размер,
		|	ШаблоныЭВФОтчетовСтатистики.КодПериодичности КАК КодПериодичности
		|ИЗ
		|	РегистрСведений.ШаблоныЭВФОтчетовСтатистики КАК ШаблоныЭВФОтчетовСтатистики";
	
	ТаблицаШаблоновВБазе = Запрос.Выполнить().Выгрузить();
	СтрокиКУдалению = Новый Соответствие;
	Для Каждого ШаблонЭВФ Из ТаблицаШаблоновВБазе Цикл
		Если СтрДлина(ШаблонЭВФ.ОКУД) = 6 Тогда 
			ШаблонЭВФ.ОКУД = "0" + ШаблонЭВФ.ОКУД;
		ИначеЕсли СтрДлина(ШаблонЭВФ.ОКУД) = 7 Тогда 
			ШаблонЭВФ.ОКУД = ШаблонЭВФ.ОКУД;
		Иначе
			СтрокиКУдалению.Вставить(ШаблонЭВФ);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого КЗ Из СтрокиКУдалению Цикл 
		ТаблицаШаблоновВБазе.Удалить(КЗ.Ключ);
	КонецЦикла;
	
	Возврат ТаблицаШаблоновВБазе;
КонецФункции
...
Процедура ЗагрузитьШаблоны(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	ТаблицаШаблоновВБазе = ТаблицаШаблоновВБазе();
	...
	Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
		...
		Если ПроверкаНаличияШаблонаВБазе(ШаблонЭВФ, ТаблицаШаблоновВБазе) Тогда 
			Продолжить;
		КонецЕсли;
