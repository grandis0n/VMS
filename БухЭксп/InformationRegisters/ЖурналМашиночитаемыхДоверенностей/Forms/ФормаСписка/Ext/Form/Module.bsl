...
Процедура ПереформироватьТабличныйДокумент(Знач Ссылка)
	...
	Попытка
		...
		ПротоколПроверкиМЧД = СтатусыДоверенности.ПротоколПроверки.Получить();
	Исключение
		ТабличныйДокументПредпросмотра = Новый ТабличныйДокумент;
...
//  * ПротоколПроверки - ХранилищеЗначения - см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
//
&НаСервереБезКонтекста
Функция ПолучитьСтатусыДоверенности(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.МЧД003") Тогда
		СтатусыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
			...
			|ТекстОшибкиПроверкиМЧД, ПротоколПроверки");
...
Процедура УстановитьОтборДинамическогоСпискаМЧД()  
	...
	Сертификат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ДополнительныеПараметры, "СертификатКриптографии");
	
	ИННДоверителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ДополнительныеПараметры, "ИННДоверителя");
	
	Доверитель = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ДополнительныеПараметры, "Доверитель");
	...
	Если ЭлементОтбораДоверенность <> Неопределено 
		...
		НовыйОтбор.Доверитель = ?(ЗначениеЗаполнено(Доверитель), Доверитель, ИнтеграцияЭДО.ПустаяСсылкаОрганизации());
		НовыйОтбор.ИННДоверителя = ИННДоверителя;
