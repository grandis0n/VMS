...
Функция ЗапуститьОбновлениеЧекаВФоне()
	...
	ПараметрыОбновления = ЧекиНПДКлиентСервер.НовыйПараметрыОбновленияСведенийОСформированныхЧеках();
	ПараметрыОбновления.Организация = Организация;
	ПараметрыОбновления.НачалоПериода = ЧекиНПДВызовСервера.ДатаОбновленияСформированныхЧековНПД(Организация);
	ПараметрыОбновления.КонецПериода = ЧекиНПДВызовСервера.ДатаСведенийНПД();
	ПараметрыОбновления.УникальныйИдентификатор = УникальныйИдентификатор;
	Если ЗначениеЗаполнено(СведенияОЧеке) Тогда
		ПараметрыОбновления.ДокументОснование = СведенияОЧеке.ДокументОснование;
	КонецЕсли;
	
	ДлительнаяОперация = ЧекиНПДВызовСервера.ЗапуститьОбновлениеСведенийОСформированныхЧекахНПД(ПараметрыОбновления);
...
Процедура ОжидатьОбновлениеЧекаВФонеЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДлительнаяОперация)
		...
		Иначе
			НомераОбновленныхЧеков = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "НомераОбновленныхЧеков");
			Если ЗначениеЗаполнено(НомераОбновленныхЧеков)
				И НомераОбновленныхЧеков.Найти(СведенияОЧеке.НомерЧека) <> Неопределено Тогда
				УстановитьСведенияОЧеке();
				ОповеститьОбИзмененияхЧека();
			Иначе
				ОтобразитьЭтапПоСведениямОЧеке();
