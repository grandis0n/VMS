...
// Обновляет чеки по списку аннулированных в сервисе Мой налог.
//
// Параметры:
//  ПараметрыОбновления - Структура - см. ЧекиНПДКлиентСервер.НовыйПараметрыОбновленияСведенийОСформированныхЧеках() -
//                                    параметры, в которых передается список аннулированных чеков
//
// Возвращаемое значение:
//  Массив из Строка - номера обновленных чеков.
Функция ОбновитьЧекиПоСпискуАннулированныхВФоне(ПараметрыОбновления) Экспорт
	
	ОбновленныеЧеки = Новый Массив;
	
	Организация = ПараметрыОбновления.Организация;
	АннулированныеЧеки = ПараметрыОбновления.СписокАннулированныхЧеков;
	
	Если Не ЗначениеЗаполнено(АннулированныеЧеки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.Комментарий)
			И ЗначениеЗаполнено(СведенияОбАннулировании.ПричинаАннулирования)
			И СведенияОЧеке.ДокументОснование <> ПараметрыОбновления.ДокументОснование Тогда
			Попытка
				ДокументОбъект = СведенияОЧеке.ДокументОснование.ПолучитьОбъект();
				ДокументОбъект.Заблокировать();
...
Функция ПараметрыРегистрацииДохода(ПараметрыЧека)
	...
	ПараметрыРегистрацииДохода.Услуги = ПараметрыЧека.Услуги.Скопировать();
