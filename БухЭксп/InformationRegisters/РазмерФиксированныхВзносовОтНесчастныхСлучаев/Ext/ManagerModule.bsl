...
Функция ДанныеРазмераФиксированныхВзносовОтНесчастныхСлучаев(ДатаАктуальности = Неопределено) Экспорт
	...
	ДанныеФиксированныхВзносов = Новый Структура("Размер, Период");
	
	СрезПоследних = РегистрыСведений.РазмерФиксированныхВзносовОтНесчастныхСлучаев.СрезПоследних(ДатаАктуальности);
	Если СрезПоследних.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Сведения о размере фиксированного взноса от несчастных случаев не заполнены'");
	КонецЕсли;
...
Процедура Обновить(ТекстXML) Экспорт
	...
	НаборЗаписей = РегистрыСведений.РазмерФиксированныхВзносовОтНесчастныхСлучаев.СоздатьНаборЗаписей();
...
Функция РазмерФиксированныхВзносовОтНесчастныхСлучаевЗаполнен() Экспорт
	...
	|	ИСТИНА КАК Поле1
	|ИЗ
	|	РегистрСведений.РазмерФиксированныхВзносовОтНесчастныхСлучаев КАК РазмерФиксированныхВзносовОтНесчастныхСлучаев");
