...
Процедура ЗаполнитьДеревоНастроек()
	...
	Запрос.Текст = 
		...
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличествоСпособОбработкиВручную,
		|	СУММА(ВЫБОР
		|		КОГДА НастройкиПолученияЭлектронныхДокументов.СпособОбработки = ""НеОтражать""
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличествоСпособОбработкиНеОтражать,
		|	СУММА(ВЫБОР
		|		КОГДА НастройкиПолученияЭлектронныхДокументов.СпособОбработки <> ""НеОтражать""
		|			И НастройкиПолученияЭлектронныхДокументов.СпособОбработки <> ""Вручную""
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличествоСпособОбработкиАвтоматически,
		|	ВЫРАЗИТЬ("""" КАК СТРОКА(100)) КАК СпособОбработки,
	...
	ТекстРазличныеСпособыОтражения = НСтр("ru = 'Различные способы отражения'");
	Для Каждого СтрокаТаблицы Из ТаблицаНастроекКонтрагента Цикл
		Если СтрокаТаблицы.КоличествоСпособОбработкиАвтоматически > 0
			И (СтрокаТаблицы.КоличествоСпособОбработкиВручную > 0
			Или СтрокаТаблицы.КоличествоСпособОбработкиНеОтражать > 0) Тогда
			СтрокаТаблицы.СпособОбработки = ТекстРазличныеСпособыОтражения
		ИначеЕсли СтрокаТаблицы.КоличествоСпособОбработкиВручную > 0
			И СтрокаТаблицы.КоличествоСпособОбработкиНеОтражать > 0 Тогда
			СтрокаТаблицы.СпособОбработки = ТекстРазличныеСпособыОтражения;
		ИначеЕсли СтрокаТаблицы.КоличествоСпособОбработкиВручную > 0 Тогда
			СтрокаТаблицы.СпособОбработки = НСтр("ru = 'Вручную'");
		ИначеЕсли СтрокаТаблицы.КоличествоСпособОбработкиНеОтражать > 0 Тогда
			СтрокаТаблицы.СпособОбработки = НСтр("ru = 'Не отражать'");
		Иначе
			СтрокаТаблицы.СпособОбработки = НСтр("ru = 'Автоматически'");
		КонецЕсли;
	КонецЦикла;
