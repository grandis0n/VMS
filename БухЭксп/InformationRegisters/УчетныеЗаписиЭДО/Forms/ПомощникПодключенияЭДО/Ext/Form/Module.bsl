...
Процедура НазначениеУчетнойЗаписиПриИзменении(Элемент)
	НазначениеУчетнойЗаписи = СокрЛП(НазначениеУчетнойЗаписи);
КонецПроцедуры
#КонецОбласти
...
Процедура ПолучитьСпособОбменаПоИдентификатору()
	...
	Если ЗначениеЗаполнено(ИдентификаторОрганизацииСуществующий) Тогда
...
Функция ПроверкаЗаполнения()
	...
	Если ДлинаНазначения > 0 И ДлинаНазначения < МинимальнаяДлина Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Длина назначения должна быть не менее %1 символов'"), МинимальнаяДлина), ,
			"НазначениеУчетнойЗаписи", , Отказ);
	КонецЕсли;
...
Процедура ВыполнитьОтправкуДанныхВСервисНастроек()
	...
	Если ИспользоватьЦентрЭДО Тогда
		...
		ПараметрыОбновления.ИдентификаторЭДО = ИдентификаторОрганизации;
		ПараметрыОбновления.Наименование = НаименованиеУчетнойЗаписи;
		ПараметрыОбновления.Назначение = НазначениеУчетнойЗаписи;
		ПараметрыОбновления.ПодробноеОписание = ПодробноеОписаниеУчетнойЗаписи;
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
...
Функция СохранитьУчетнуюЗапись()
	...
	Если ОператорПоддерживаетНастройкуТребованияПриглашений
...
Процедура ИнициализироватьОперациюПодключенияЭДО(Знач ОперацияЭДО)
	...
	НеИспользоватьПриглашения = 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОперацияЭДО.Параметры, 
			"НеИспользоватьПриглашения", Неопределено);
	Если ЗначениеЗаполнено(НеИспользоватьПриглашения) Тогда
		ТребоватьПриглашенияПередНачаломЭДО = Не НеИспользоватьПриглашения;
	КонецЕсли;
...
Процедура ЗаполнитьОперациюПодключенияЭДО()
	...
	Операция.Параметры.НеИспользоватьПриглашения = Не ТребоватьПриглашенияПередНачаломЭДО;
	Операция.Параметры.КодыТиповДокументовКОбмену = УчетныеЗаписиЭДОКлиентСервер.КодыТиповДокументов(КодыТиповДокументов);
...
Процедура ОбработатьДополнительныеПараметры(Знач ДополнительныеПараметры)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры.ДополнительныеПараметры);
	Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаОжиданиеРегистрации;
...
Процедура СформироватьАдрес(Знач ДополнительныеПараметры)
	...
	СтрокаКлючей = "Индекс,Регион,КодРегиона,Район,Город,НаселенныйПункт,Улица,Дом,Корпус,Квартира";
	ДанныеАдреса = Новый Структура(СтрокаКлючей);
...
Процедура ЗаполнитьСписокИдентификаторов(СписокИдентификаторов)
	...
	АбонентыИТребованияПриглашений = 
		Новый ФиксированноеСоответствие(РаботаСАбонентамиЭДО.АбонентыИТребованияПриглашений(СписокИдентификаторов));
	
	ИдентификаторОрганизацииСуществующий = СписокИдентификаторов[0].Значение;
	ТребоватьПриглашенияПередНачаломЭДО = АбонентыИТребованияПриглашений.Получить(ИдентификаторОрганизацииСуществующий) <> Неопределено
		И Не АбонентыИТребованияПриглашений[ИдентификаторОрганизацииСуществующий];
...
Процедура ОбновитьТребованиеПриглашенияПоДаннымАбонента()
	...
	Если ТипРегистрации = ТипРегистрацииНовыйИдентификатор() Тогда
		ТребоватьПриглашенияПередНачаломЭДО =
			УчетныеЗаписиЭДОКлиентСервер.ТребоватьПриглашенияПередНачаломЭДОЗначениеПоУмолчанию();
		Возврат;
	КонецЕсли;
	...
	Если АбонентыИТребованияПриглашений = Неопределено Тогда
		ТребоватьПриглашенияПередНачаломЭДО = 
			Не РаботаСАбонентамиЭДОВызовСервера.АбонентНеТребуетПриглашений(ИдентификаторОрганизацииСуществующий);
		Возврат;
	КонецЕсли;
	
	Если АбонентыИТребованияПриглашений.Получить(ИдентификаторОрганизацииСуществующий) = Неопределено Тогда
		ТребоватьПриглашенияПередНачаломЭДО = 
			Не РаботаСАбонентамиЭДОВызовСервера.АбонентНеТребуетПриглашений(ИдентификаторОрганизацииСуществующий);
		Возврат;
	КонецЕсли;
	
	ТребоватьПриглашенияПередНачаломЭДО = Не АбонентыИТребованияПриглашений[ИдентификаторОрганизацииСуществующий];
