...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("Записать") И ПараметрыЗаписи.Записать Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПодготовитьКодыТиповДокументов(ТекущийОбъект);
...
Процедура НазначениеУчетнойЗаписиПриИзменении(Элемент)
	...
	Если РегистрационныйПакетПоддерживаетПередачуНазначения Тогда
...
&НаКлиенте
Процедура НеИспользоватьПриглашенияПриИзменении(Элемент)
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыКодыТиповДокументов

&НаКлиенте
Процедура КодыТиповДокументовИспользуетсяПриИзменении(Элемент)
...
Процедура Удалить(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПриЗавершенииУдаленияУчетнойЗаписи", ЭтотОбъект);
	УчетныеЗаписиЭДОСлужебныйКлиент.УдалитьУчетнуюЗапись(Запись.ИдентификаторЭДО, Оповещение, ЭтотОбъект);
...
Процедура ВыполнитьСохранениеДанныхВСервисахПослеПолученияВсехСертификатов(ДанныеСертификатов, Этапы) Экспорт
	...
	МассивКодовТиповДокументов = Новый Массив;
	НайденныеСтроки = КодыТиповДокументов.НайтиСтроки(Новый Структура("Используется", Истина));
	
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		
		Если НайденнаяСтрока.Используется Тогда
			 МассивКодовТиповДокументов.Добавить(НайденнаяСтрока.Код);
		КонецЕсли;
	...
	ИначеЕсли Запись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		Если Этапы.ИзмененСоставСертификатов ИЛИ Этапы.ИзмененыРегистрационныеДанные Тогда
			...
			ПараметрыРегистрации.НеИспользоватьПриглашения = Запись.НеИспользоватьПриглашения;
			ПараметрыРегистрации.КодыТиповДокументовКОбмену = МассивКодовТиповДокументов;
			ПараметрыРегистрации.Комментарий = Запись.НазначениеУчетнойЗаписи;
			ПараметрыРегистрации.ВидУчетнойЗаписи = Запись.ВидУчетнойЗаписи;
	...
	Если Этапы.ИзмененыПараметрыУчетнойЗаписи Тогда
		...
		ОписаниеИзменений = СервисНастроекЭДОКлиентСервер.ОписаниеИзмененийДанныхАбонентаСервисаНастроек();
		ОписаниеИзменений.ИдентификаторУчетнойЗаписи = Запись.ИдентификаторЭДО;
		ОписаниеИзменений.НазначениеУчетнойЗаписи = Запись.НазначениеУчетнойЗаписи;
		ОписаниеИзменений.ПодробноеОписаниеУчетнойЗаписи = Запись.ПодробноеОписаниеУчетнойЗаписи;
		ОписаниеИзменений.НеИспользоватьПриглашения = Запись.НеИспользоватьПриглашения;
		ОписаниеИзменений.КодыТиповДокументовКОбмену = МассивКодовТиповДокументов;
		ОписаниеИзменений.СертификатыАбонента = ДанныеСертификатов.Список;
		
		СервисНастроекЭДОКлиент.ОтправитьДанныеАбонентаВСервисНастроек(ОписаниеИзменений, Оповещение, 
			ЭтотОбъект);
...
Процедура ОбновитьПараметрыУчетнойЗаписиЧерезЦентрЭДО(ПараметрыСохранения)
	...
	ПараметрыОбновления.ИдентификаторЭДО = Запись.ИдентификаторЭДО;
	ПараметрыОбновления.Наименование = Запись.НаименованиеУчетнойЗаписи;
	ПараметрыОбновления.Назначение = Запись.НазначениеУчетнойЗаписи;
	ПараметрыОбновления.ПодробноеОписание = Запись.ПодробноеОписаниеУчетнойЗаписи;
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДОКлиент");
...
Процедура ПодготовитьФормуНаСервере()
	...
	Элементы.СтраницаУведомления.Видимость = Запись.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
	...
	ЗаполнитьПризнакПоддержкиРегПакетомПередачиНазначения();
	
	ЗаполнитьКодыТиповДокументов();
	
	Элементы.НеИспользоватьПриглашения.Видимость = 
		РегистрационныйПакетПоддерживаетПередачуНазначения И Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО);
		
	Элементы.КодыТиповДокументов.Видимость = 
		РегистрационныйПакетПоддерживаетПередачуНазначения И Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО);
	...
	Элементы.ПодробноеОписаниеУчетнойЗаписи.Видимость = ЗначениеЗаполнено(Запись.ПодробноеОписаниеУчетнойЗаписи);
...
&НаСервере
Процедура ЗаполнитьПризнакПоддержкиРегПакетомПередачиНазначения()
	
	РегистрационныйПакетПоддерживаетПередачуНазначения = 
		СервисНастроекЭДОПовтИсп.ОператорПоддерживаетВерсиюФорматаРегистрации(Запись.ОператорЭДО, "5.04");
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьКодыТиповДокументов()
	
	ТаблицаКодов = РегистрыСведений.ОператорыЭДО.КодыПоддерживаемыхТиповДокументов();
	
	МассивКодов = ?(ЗначениеЗаполнено(Запись.КодыТиповДокументовКОбмену),
		ОбщегоНазначения.JSONВЗначение(Запись.КодыТиповДокументовКОбмену), Новый Массив);
	
	Для Каждого СтрокаКода Из ТаблицаКодов Цикл
		
		НоваяСтрока = КодыТиповДокументов.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаКода,, "Используется");
		НоваяСтрока.Используется = Не МассивКодов.Найти(СтрокаКода.Код) = Неопределено;
...
Процедура ЗарегистрироватьСертификатТакском(ДобавляемыйСертификат)
	...
	КриптографияБЭДКлиент.ТестНастроекСПроверкойСертификата(ДобавляемыйСертификат, ЭтотОбъект,
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Если КодыТиповДокументов.НайтиСтроки(Новый Структура("Используется", Истина)).Количество() = 0 
		И РегистрационныйПакетПоддерживаетПередачуНазначения 
		И Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда

		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Необходимо указать типы документов для обмена'"), ,
			"КодыТиповДокументов", , ОшибкаЗаполнения);
	...
	ПроверитьЗаполненностьНазначения(ОшибкаЗаполнения);
...
Функция ДанныеСертификатов(Знач СписокСертификатов, ДоступныеСертификаты)
	...
	СтрокаКлючей = "Список, ОтборСертификатов, Доверенности, Должности";
	Результат = Новый Структура(СтрокаКлючей);
...
&НаКлиенте
Процедура ПроверитьЗаполненностьНазначения(ОшибкаЗаполнения);
	
	МинимальнаяДлина = УчетныеЗаписиЭДОКлиентСервер.МинимальнаяДопустимаяДлинаНазначенияУчетнойЗаписи();
	ДлинаНазначения = СтрДлина(СокрЛП(Запись.НазначениеУчетнойЗаписи));
	
	Если ДлинаНазначения = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено назначение учетной записи'"), ,
			"Запись.НазначениеУчетнойЗаписи", , ОшибкаЗаполнения);
...
Процедура ДобавитьСертификатИзСервисовМобильногоПодписания(ВидМобильнойПодписи)
	...
	ОткрытьФорму("Обработка.ПодписаниеИШифрованиеДанныхВМобильномЭДО.Форма.ФормаПодписанияСоглашенияНаЗагрузкуСертификата",
		ПараметрыФормы,
		ЭтотОбъект,,,,
		ЗарегистрироватьДобавитьСертификат,
...
// Подготавливает коды типов документов для записи в регистр
// 
// Параметры:
//  УчетнаяЗапись - РегистрСведенийМенеджерЗаписи.УчетныеЗаписиЭДО - Учетная запись
&НаСервере
Процедура ПодготовитьКодыТиповДокументов(УчетнаяЗапись)
	
	МассивКодов = Новый Массив;
	ТаблицаКодов = РеквизитФормыВЗначение("КодыТиповДокументов");
	
	Для Каждого СтрокаКода Из ТаблицаКодов Цикл
		
		Если СтрокаКода.Используется Тогда
			МассивКодов.Добавить(СтрокаКода.Код);
		КонецЕсли;
		
	КонецЦикла;
	
	УчетнаяЗапись.КодыТиповДокументовКОбмену = ОбщегоНазначения.ЗначениеВJSON(МассивКодов);
