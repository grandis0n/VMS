...
Процедура ПроверитьИнформациюДляКонтрагентов(Отказ)
	...
	ДлиннаНазначения = УчетныеЗаписиЭДОКлиентСервер.МинимальнаяДопустимаяДлинаНазначенияУчетнойЗаписи();
	КоличествоСтрокИнформации = ИнформацияДляКонтрагентов.Количество();
	
	Если КоличествоСтрокИнформации > 1 Тогда
		ШаблонВопроса = НСтр("ru = 'Длина назначения в строке %1 должна быть не менее %2 символов'");
	Иначе
		ШаблонВопроса = НСтр("ru = 'Длина назначения должна быть не менее %1 символов'");
	КонецЕсли;
	
	НомерСтроки = 0;
	Для Каждого СтрокаИнформации Из ИнформацияДляКонтрагентов Цикл
		НомерСтроки = НомерСтроки + 1;
		Если Не ЗначениеЗаполнено(СтрокаИнформации.НазначениеУчетнойЗаписи) Или 
			СтрДлина(СтрокаИнформации.НазначениеУчетнойЗаписи) < ДлиннаНазначения Тогда
			Отказ = Истина;
			
			Если КоличествоСтрокИнформации > 1 Тогда
				ТекстВопроса = СтрШаблон(ШаблонВопроса, НомерСтроки, ДлиннаНазначения);
				ПутьКЯчейке = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ИнформацияДляКонтрагентов", 
					НомерСтроки, "ИнформацияДляКонтрагентовНазначениеУчетнойЗаписи");
			Иначе
				ТекстВопроса = СтрШаблон(ШаблонВопроса, ДлиннаНазначения);
				ПутьКЯчейке ="НазначениеУчетнойЗаписи";
			КонецЕсли;
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстВопроса,, ПутьКЯчейке);
...
Процедура ЗаполнитьИнформациюДляКонтрагентов(ИдентификаторыЭДО)
	...
	Элементы.ПодробноеОписаниеУчетнойЗаписи.Видимость = 
		ЗначениеЗаполнено(ИнформацияДляКонтрагентов[0].ПодробноеОписаниеУчетнойЗаписи);
