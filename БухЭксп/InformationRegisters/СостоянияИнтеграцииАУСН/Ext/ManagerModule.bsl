...
// 
// Параметры:
//  Организация - СправочникСсылка.Организации, Неопределено - организация, по которой проверяем, или в целом по программе
// 
// Возвращаемое значение:
//  Булево - Истина, если есть настроенная передача НДФЛ из программы
//
Функция МожемПередатьНДФЛИзПрограммы(Организация = Неопределено) Экспорт
	...
	|	И СостоянияИнтеграцииАУСН.Организация В(&Организация)";
	
	Если ЗначениеЗаполнено(Организация) Тогда
		ДоступныеОрганизации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Организация);
	Иначе
...
// Возвращает дату подключения с настроенной передачей расчета НДФЛ или Неопределено, если расчет не передается
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация, по которой передается расчет НДФЛ
//
// Возвращаемое значение:
//   Дата, Неопределено
//
Функция ДатаПодключенияПередачиНДФЛ(Организация) Экспорт
...
// Возвращает банк, по которому настроена передача расчета НДФЛ, или Неопределено, если расчет не передается
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация, по которой передается расчет НДФЛ
//
// Возвращаемое значение:
//   СправочникСсылка.КлассификаторБанков, Неопределено
//
Функция БанкПередачиНДФЛИзПрограммы(Организация) Экспорт
...
Функция ОшибкиОтправкиПоДокументуНДФЛ(Организация, Документ) Экспорт
	
	Результат = ИнтеграцияАУСН.НовыйОписаниеОшибокОтправки();
	
	БанкПередачиНДФЛ = БанкПередачиНДФЛИзПрограммы(Организация);
	Если БанкПередачиНДФЛ = Неопределено тогда
		Возврат Результат;
	КонецЕсли;
	
	НаборЗаписей = РегистрыСведений.СостоянияИнтеграцииАУСН.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Организация.Установить(Организация);
	НаборЗаписей.Отбор.Банк.Установить(БанкПередачиНДФЛ);
	НаборЗаписей.Прочитать();
	Если ЗначениеЗаполнено(НаборЗаписей) Тогда
		ОшибкиОтправки = НаборЗаписей[0].ОшибкиОтправки.Получить();
		Если ТипЗнч(ОшибкиОтправки) = Тип("ТаблицаЗначений") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ОшибкиОтправки, Результат);
		КонецЕсли;
	КонецЕсли;
	
	Для Индекс = -Результат.Количество() + 1 По 0 Цикл
		ОписаниеОшибки = Результат[-Индекс];
		Если ЗначениеЗаполнено(ОписаниеОшибки.БанковскаяОперация)
			И ОписаниеОшибки.БанковскаяОперация.Найти(Документ) = Неопределено Тогда
