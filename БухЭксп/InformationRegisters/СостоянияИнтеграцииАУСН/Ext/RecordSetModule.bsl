...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		КонецЕсли;
		Если Запись.Состояние = СостояниеПодключено И Не ЗначениеЗаполнено(Запись.Банк) Тогда
			СервисЛКАУСНПодключен = Истина;
		КонецЕсли;
	КонецЦикла;
	...
	Если ИнтеграцияАУСНВызовСервераПовтИсп.СервисПодключен() <> СервисПодключен
		Или ИнтеграцияАУСНВызовСервераПовтИсп.СервисЛКАУСНПодключен() <> СервисЛКАУСНПодключен Тогда
