...
&НаСервере
Процедура ПерекрытьПользовательскиеОтборы()
	
	Для Каждого БыстрыйОтбор Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		ПользовательскаяНастройка = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(БыстрыйОтбор.ИдентификаторПользовательскойНастройки);
		Если ПользовательскаяНастройка <> Неопределено Тогда
			ПользовательскаяНастройка.ВидСравнения = БыстрыйОтбор.ВидСравнения;
			ПользовательскаяНастройка.Использование = БыстрыйОтбор.Использование;
			ПользовательскаяНастройка.ПравоеЗначение = БыстрыйОтбор.ПравоеЗначение;
		КонецЕсли;
	КонецЦикла;
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПерекрытьПользовательскиеОтборы();
