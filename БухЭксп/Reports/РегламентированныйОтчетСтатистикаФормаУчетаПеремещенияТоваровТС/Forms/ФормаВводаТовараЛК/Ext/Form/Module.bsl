...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДиапазонКурсов = Новый Структура("Минимум, Максимум", 0, 0);
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	СформироватьСписки();
	ЗаполнитьЗначенияСвойств(ДиапазонКурсов, ВладелецФормы.СтруктураРеквизитовФормы.ДиапазонКурсов);
	
	Если ЗначениеЗаполнено(ДиапазонКурсов.Минимум) И ЗначениеЗаполнено(ДиапазонКурсов.Максимум) Тогда 
		Элементы.ДекорацияДиапазонКурсов.Заголовок = "Диапазон курсов " + Формат(ДиапазонКурсов.Минимум, "ЧДЦ=2; ЧРД=.")
			+ " - " + Формат(ДиапазонКурсов.Максимум, "ЧДЦ=2; ЧРД=.");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьПоказатели(Команда)
	...
	Если Не ЗначениеЗаполнено(StatisticalCostRUB) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не указана статистическая стоимость в рублях", , "StatisticalCostRUB");
		ЕстьОшибки = Истина;
	ИначеЕсли Не ЗначениеЗаполнено(StatisticalCostUSD) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не указана статистическая стоимость в долларах", , "StatisticalCostUSD");
		ЕстьОшибки = Истина;
	ИначеЕсли ЗначениеЗаполнено(ДиапазонКурсов.Минимум) И ЗначениеЗаполнено(ДиапазонКурсов.Максимум) Тогда 
		Если (((1.0 * StatisticalCostRUB) / StatisticalCostUSD) <= 0.9 * ДиапазонКурсов.Минимум)
			Или (((1.0 * StatisticalCostRUB) / StatisticalCostUSD) >= 1.1 * ДиапазонКурсов.Максимум) Тогда 
			
			ОбщегоНазначенияКлиент.СообщитьПользователю("Отношение стоимости RUB/USD, не соответствует курсу", , "StatisticalCostRUB");
			ЕстьОшибки = Истина;
		КонецЕсли;
	ИначеЕсли Не СоотношениеКурсаДолларРубльКорректно(StatisticalCostUSD, StatisticalCostRUB) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Отношение стоимости RUB/USD, не соответствует курсу", , "StatisticalCostRUB");
		ЕстьОшибки = Истина;
	КонецЕсли;
...
Функция РезультатЗакрытия()
	...
	РезультатЗакрытия.Вставить("КорректностьЗаполнения", 
		ФормаУчетаПеремещенияТоваровТС.ПроверитьЗаполнениеПоказателейТовара(ЭтотОбъект, ДиапазонКурсов));
