...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.Вставить("ДиапазонКурсов",
		ФормаУчетаПеремещенияТоваровТС.ДиапазонКурсовДоллараВПериоде(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
			...
			Если ПараметрыВыгрузкиСтрокиТовара.Свойство("Товар_ДекларацияНаТовары") Тогда 
				...
				Для Каждого Декларация Из ПараметрыВыгрузкиСтрокиТовара.Товар_ДекларацияНаТовары Цикл 
					...
					ДекларацияОписание = ДекларацияОписание + СимволПС + Декларация.CustomsCode + "/"
						+ Формат(РегламентированнаяОтчетностьЗагрузка.ДатаИзСтрокиЛюбогоФормата(Декларация.RegistrationDate), "ДФ=ddMMyy")
						+ "/" + Декларация.GTDNumber;
...
Асинх Процедура ЗаполнитьДаннымиПервичногоДокументаПослеВыбора(Результат, ДополнительныеПараметры) Экспорт 
	...
	НеобходимПересчетДиапазонаКурсов = Ложь;
	Если НачалоМесяца(РеквизитыДокумента.Дата) <> НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда 
		...
		Если РезультатВопроса = КодВозвратаДиалога.Да Тогда 
			...
			Модифицированность = Истина;
			НеобходимПересчетДиапазонаКурсов = Истина;
		ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Отмена Тогда 
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	РезультатВыполнения = ЗаполнитьДаннымиПервичногоДокумента(Результат, НеобходимПересчетДиапазонаКурсов);
...
&НаСервере
Функция ЗаполнитьДаннымиПервичногоДокумента(Ссылки, НеобходимПересчетДиапазонаКурсов)
	Если НеобходимПересчетДиапазонаКурсов Тогда 
		СтруктураРеквизитовФормы.Вставить("ДиапазонКурсов",
			ФормаУчетаПеремещенияТоваровТС.ДиапазонКурсовДоллараВПериоде(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета))
	КонецЕсли;
...
Процедура ЗагрузитьТовары(Ссылка, ДанныеПервичногоДокумента)
	...
	Для Каждого Стр Из ТаблицаТоварыДляВыгрузки Цикл
		...
		ЗаполнитьЗначенияСвойств(Товары.Добавить(),
			ФормаУчетаПеремещенияТоваровТС.СформироватьСтруктуруТовараПриЗагрузке(СтруктураТовара, СтруктураРеквизитовФормы.ДиапазонКурсов));
	КонецЦикла;
КонецПроцедуры
