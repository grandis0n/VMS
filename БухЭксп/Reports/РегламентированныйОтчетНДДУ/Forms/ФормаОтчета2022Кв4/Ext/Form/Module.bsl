...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел2") Тогда
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Усл_КодыП4Стр010 = СтрРазделить("010,020,030,040,050", ",", Ложь);
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				Если ЗаполненаСтрокаМногострочнойЧасти(СтрокаМнЧ) Тогда
					...
					ЗначКодОсн = СокрЛП(СтрокаМнЧ.Данные["П00104М101003"]);
					Если НЕ ЗначениеЗаполнено(ЗначКодОсн) Тогда
						...
						СтатусПроверки = Ложь;
					ИначеЕсли ВозмКодыП4Стр010.МассивЗначений.Найти(ЗначКодОсн) = Неопределено Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Приложение 4: " + Текст_СтрокаГруппыСтрок
					...
					Иначе
						...
						Если Усл_КодыП4Стр010.Найти(ЗначКодОсн) <> Неопределено Тогда
							
							ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00104М101103"]);
							Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
								СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Приложение 4: " + Текст_СтрокаГруппыСтрок
									+ "должен быть задан код ОКОФ, если код основания восстановления фактических расходов равен одному из значений: "
									+ СтрСоединить(Усл_КодыП4Стр010, ", "), "Приложение4", "П00104М101103_" + НомСтроки, КоординатыПриложения);
								СтатусПроверки = Ложь;
							Иначе
								ШаблоныОКОФ = Неопределено;
							...
							Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
								СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Приложение 4: " + Текст_СтрокаГруппыСтрок
									+ "должен быть задан признак номера, если код основания восстановления фактических расходов равен одному из значений: "
									+ СтрСоединить(Усл_КодыП4Стр010, ", "), "Приложение4", "П00104М101203_" + НомСтроки, КоординатыПриложения);
								СтатусПроверки = Ложь;
							...
							Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
								СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Приложение 4: " + Текст_СтрокаГруппыСтрок
									+ "должен быть задан номер, если код основания восстановления фактических расходов равен одному из значений: "
									+ СтрСоединить(Усл_КодыП4Стр010, ", "), "Приложение4", "П00104М101303_" + НомСтроки, КоординатыПриложения);
								СтатусПроверки = Ложь;
							КонецЕсли;
						...
						Иначе
							...
							ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00104М101203"]);
							Если ЗначениеЗаполнено(ЗначПок) И ВозмКодыП4Стр012.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
								СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Приложение 4: " + Текст_СтрокаГруппыСтрок
								+ "признак номера может принимать одно из значений: " + ВозмКодыП4Стр012.СтрокаЗначений,
								"Приложение4", "П00104М101203_" + НомСтроки, КоординатыПриложения);
								СтатусПроверки = Ложь;
							КонецЕсли;
