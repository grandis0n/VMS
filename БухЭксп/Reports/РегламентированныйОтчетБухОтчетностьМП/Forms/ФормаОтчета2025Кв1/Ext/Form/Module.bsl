...
&НаКлиенте
Перем РежимЗаполнения; // тип: "Строка"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДействуетФормат505(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		Элементы.ИмяФайлаАудиторскогоЗаключения.Заголовок = "Аудиторский отчет";
	КонецЕсли;
...
Процедура ИмяФайлаАудиторскогоЗаключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Если ДействуетФормат505(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		ЗаголовокОкнаВыбораФайла = НСтр("ru = 'Выбор файла аудиторского отчета'");
	Иначе
		ЗаголовокОкнаВыбораФайла = НСтр("ru = 'Выбор файла аудиторского заключения'");
	КонецЕсли;
	ДобавитьФайл("ФайлАудиторскогоЗаключения", ЗаголовокОкнаВыбораФайла);
...
Процедура Печатать(Команда)
	...
	ПараметрыФормы.Вставить("НетШаблонаPDF417", Истина);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПечататьЗавершение", ЭтотОбъект);
	
	ИмяФормыВыбора = Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") + 6) + "ВыборПечатнойФормы2020";
...
Процедура ПослеВыбораИсточникаДанныхДляОблака()
	...
	РезультатЗапускаЗаполненияОтчетаВФоне(РежимЗаполнения);
...
Процедура УстановитьПервоначальнуюВидимостьЭлементов()
	...
	Элементы.Проверить.Видимость = Истина;
...
Функция МакетОтчета(ИмяРаздела)
	...
	ПараметрыПодготовкиМакета.Вставить("ОтчетнаяДата", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
...
Функция ПараметрыЗаполненияОтчета(ЗаполняемыеРазделы)
	...
	Если Не ПараметрыЗаполнения.Свойство("УникальныйИдентификаторФормы") Тогда
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ДанныеКС.Вставить("Погрешность"	, 2000);
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Бухгалтерская отчетность упрощенная");
...
Процедура СформироватьСлужебныеДанные()
	...
	ЗаменаНекорректныхСтрок.Вставить("2130", "2330");
...
Функция СформироватьПоказатели(ИменаПоказателей)
	...
	Для каждого Элемент Из ИменаПоказателей Цикл
		...
		ПодстрокаСтроки 		= ДанныеКС.ЗаменаНекорректныхСтрок.Получить(Сред(Элемент.Значение, 8, 4));
...
Функция РавенствоНеВыполнено(Знач ЛевоеЗначение, Знач ПравоеЗначение, КодСравнения = 1)
	...
	Если ДанныеКС.УчитыватьПогрешность Тогда
		...
		Если КодСравнения = 1 Тогда
			// равно
			Если СтрНайти(НастройкиКС.ШаблонДетализации, "(±") = 0 Тогда
...
&НаСервере
Функция ПроверитьИЗаменитьСтроку(Знач НомерСтроки) 
	
	Результат = ДанныеКС.ЗаменаНекорректныхСтрок.Получить(НомерСтроки);
	
	Возврат ?(Результат = Неопределено, НомерСтроки, Результат);
...
&НаСервере
Процедура ПроверкиКС_01_01_03() 
	
	// гр.04,05,06 стр.1600 = гр.04,05,06 стр.(1150 + 1170 + 1210 + 1250 + 1240)
	...
	Для ИндексПроверок 	= 0 По 2 Цикл 
		
		СчетчикПроверок = СчетчикПроверок + 1;
        ПоказательИтога = "1600";
		Графа = МассивГраф[ИндексПроверок];
		СтрокаПоказателей 	= "1150,1170,1210,1250,1240";
		ШаблонДетализации2 	= "%1 + %2 + %3 + %4 + %5";
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерКС"				, "01." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
		...
		ПроверкаКС_01_шаблон(СтруктураПараметров);
...
Процедура ПроверкаКС_01_код()
	...
	Для каждого КодСтроки Из МассивКодовСтрок Цикл 
		...
		НастройкиКС.ШаблонДетализации2 = СтрЗаменить(НастройкиКС.ШаблонДетализации2, "%" + Счетчик, 
			?(РасчетнаяСуммаПоКоду >= 0, РасчетнаяСуммаПоКоду, "(" + РасчетнаяСуммаПоКоду + ")"));
	...
	Если МассивКодовСтрок.Количество() = 1 Тогда
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
			Показатели[ИмяПоказателяИтога].Значение, РасчетнаяСумма);
	Иначе
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
&НаСервере
Процедура ПроверкиКС_02_01_03() 
	
	// гр.04,05,06 стр.1600 = гр.04,05,06 стр.1700
	...
	Для ИндексПроверок 	= 0 По 2 Цикл 
        ...
        ПоказательИтога = "1600";
		Графа = МассивГраф[ИндексПроверок];
		СтрокаПоказателей 	= "1700";
		ШаблонДетализации2 	= "%1";
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерКС"				, "02." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
		СтруктураПараметров.Вставить("Описание"				, Описание);
		СтруктураПараметров.Вставить("Рекомендации"			, Рекомендации);
		СтруктураПараметров.Вставить("ШаблонДетализации"	, "%1 = %2");
		СтруктураПараметров.Вставить("ШаблонДетализации2"	, ШаблонДетализации2); 
		СтруктураПараметров.Вставить("ПоказательИтога"		, ПоказательИтога);
		СтруктураПараметров.Вставить("Показатели"			, СтрокаПоказателей);
		СтруктураПараметров.Вставить("Графа"				, Графа);
...
&НаСервере
Процедура ПроверкиКС_03_01_03() 
	
	// гр.04,05,06 стр.1700 = гр.04,05,06 стр.(1300 + 1410 + 1450 + 1510 + 1520 + 1550) для коммерческих
	...
	Для ИндексПроверок 	= 0 По 2 Цикл 
		...
		СчетчикПроверок = СчетчикПроверок + 1;
        ПоказательИтога = "1700";
		Графа = МассивГраф[ИндексПроверок];
		СтрокаПоказателей 	= "1300,1410,1450,1510,1520,1550";
		ШаблонДетализации2 	= "%1 + %2 + %3 + %4 + %5 + %6";
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерКС"				, "03." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
		СтруктураПараметров.Вставить("Описание"				, Описание);
		СтруктураПараметров.Вставить("Рекомендации"			, Рекомендации);
		СтруктураПараметров.Вставить("ШаблонДетализации"	, "%1 = %2 (%3)");
		СтруктураПараметров.Вставить("ШаблонДетализации2"	, ШаблонДетализации2); 
...
&НаСервере
Процедура ПроверкиКС_04_01_03() 
	
	// гр.04,05,06 стр.1700 = гр.04,05,06 стр.(1350 + 1410 + 1450 + 1510 + 1520 + 1550) для некоммерческих
	...
	Для ИндексПроверок 	= 0 По 2 Цикл 
        ...
        ПоказательИтога = "1700";
		Графа = МассивГраф[ИндексПроверок];
		СтрокаПоказателей 	= "1350,1410,1450,1510,1520,1550";
		ШаблонДетализации2 	= "%1 + %2 + %3 + %4 + %5 + %6";
		...
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерКС"				, "04." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
		СтруктураПараметров.Вставить("Описание"				, Описание);
		СтруктураПараметров.Вставить("Рекомендации"			, Рекомендации);
		СтруктураПараметров.Вставить("ШаблонДетализации"	, "%1 = %2 (%3)");
		СтруктураПараметров.Вставить("ШаблонДетализации2"	, ШаблонДетализации2); 
...
&НаСервере
Процедура ПроверкиКС_05_01_36()

	МассивНомеровСтрок = СтрРазделить(
		"1150,1170,1210,1250,1240,1600,1410,1450,1510,1520,1550,1700", ","); 
	...
	Для каждого НомерСтроки Из МассивНомеровСтрок Цикл 
		Для СчетчикГраф = 4 По 6 Цикл 
			...
			СтруктураПараметров.Вставить("НомерКС"		, "05." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
			...
			ПроверкаКС_05_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_05_шаблон(СтруктураПараметров)
	
	ПредставлениеЯчейки = "гр." + Прав(СтруктураПараметров.Графа, 1) + " стр." 
		+ ПроверитьИЗаменитьСтроку(СтруктураПараметров.НомерСтроки);
	...
	Если НЕ СтруктураПараметров.МеньшеНуля Тогда
		Заголовки.Вставить("Описание"			, "Сумма в " + ПредставлениеЯчейки + " >= 0" + УпоминаниеОПогрешности());
...
Процедура ПроверкаКС_05_код()
	...
	Если НастройкиКС.МеньшеНуля Тогда
		НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя1].Значение, 0, 3);
	Иначе
		НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя1].Значение, 0, 2);
	КонецЕсли;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
	...
	Если НастройкиКС.МногоСтрок Тогда
		...
		Для Счетчик = 1 По КолвоСтрок Цикл
			...
			Если НастройкиКС.МеньшеНуля Тогда
				НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя2].Значение, 0, 3);
			Иначе
				НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя2].Значение, 0, 2);
			КонецЕсли;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
&НаСервере
Процедура ПроверкиКС_06_01_30()

	МассивНомеровСтрок = СтрРазделить(
		"1150,1170,1210,1250,1240,1410,1450,1510,1520,1550", ","); 
	МассивПризнаковНеМногострочности = СтрРазделить(
		"", ",");
	СчетчикПроверок = 0;
	...
	Для каждого НомерСтроки Из МассивНомеровСтрок Цикл 
		Для СчетчикГраф = 4 По 6 Цикл 
			...
			СтруктураПараметров.Вставить("НомерКС"		, "06." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
			СтруктураПараметров.Вставить("НомерСтроки"	, НомерСтроки);
			СтруктураПараметров.Вставить("Графа"		, Формат(СчетчикГраф, "ЧЦ=2; ЧВН="));  
			СтруктураПараметров.Вставить("МногоСтрок"	, МассивПризнаковНеМногострочности.Найти(НомерСтроки) = Неопределено);
			СтруктураПараметров.Вставить("МеньшеНуля"	, Ложь);
			
			ПроверкаКС_06_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_06_шаблон(СтруктураПараметров)  
	...
	Если НЕ СтруктураПараметров.МеньшеНуля Тогда
		Заголовки.Вставить("Описание"			, "Сумма в " + ПредставлениеЯчейки + " >= сумме " +
			"подчиненных строк" + УпоминаниеОПогрешности());
...
Процедура ПроверкаКС_06_код()
	...
	Если НастройкиКС.МеньшеНуля Тогда
		НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя1].Значение, 
			Показатели[ИмяПоказателя2].Итог, 3);
	Иначе
		НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя1].Значение, 
			Показатели[ИмяПоказателя2].Итог, 2);
	КонецЕсли;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
	...
	Если НастройкиКС.МногоСтрок Тогда
		...
		Для Счетчик1 = 1 По КолвоСтрок1 Цикл
			...
			Если НастройкиКС.МеньшеНуля Тогда
				НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя3].Значение, 
					Показатели[ИмяПоказателя4].Итог, 3);
			Иначе
				НастройкиКС.ЭтоОшибка = РавенствоНеВыполнено(Показатели[ИмяПоказателя3].Значение, 
					Показатели[ИмяПоказателя4].Итог, 2);
			КонецЕсли;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
&НаСервере
Процедура ПроверкиКС_07_01_02() 
	
	// гр.04,05 стр.2400 = гр.04,05 стр.(2110 - 2120 - 2330 + 2340 - 2350 - 2410)
	...
	Для ИндексПроверок 	= 0 По 1 Цикл 
		...
		СчетчикПроверок = СчетчикПроверок + 1;
        ПоказательИтога = "2400";
		Графа = МассивГраф[ИндексПроверок];
		СтрокаПоказателей 	= "2110,2120,2130,2340,2350,2410"; 
		СтрокаПоказателей2 	= "2110,2120,2330,2340,2350,2410"; 
		ШаблонДетализации2 	= "%1 + %2 + %3 + %4 + %5 + %6"; 
		ШаблонДетализации3 	= "%1 - %2 - %3 + %4 - %5 - %6"; 
		
		Описание = СтрШаблон(ШаблонОписания, 
			Число(Графа), ПоказательИтога, ПодставитьПараметрыВСтрокуИзМассива(ШаблонДетализации3, 
			СтрРазделить(СтрокаПоказателей2, ",")));
		Рекомендации = СтрШаблон(ШаблонРекомендаций,
			Число(Графа), ПоказательИтога);
		
		СтруктураПараметров.Вставить("НомерКС"				, "07." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
		СтруктураПараметров.Вставить("Описание"				, Описание);
		СтруктураПараметров.Вставить("Рекомендации"			, Рекомендации);
		СтруктураПараметров.Вставить("ШаблонДетализации"	, "%1 = %2 (%3)");
		СтруктураПараметров.Вставить("ШаблонДетализации2"	, ШаблонДетализации2); 
...
&НаСервере
Процедура ПроверкиКС_08_01_10()

	МассивНомеровСтрок = СтрРазделить(
		"2110,2120,2130,2340,2350", ","); 
	МассивПризнаковНеМногострочности = СтрРазделить(
		"", ",");
	...
	Для каждого НомерСтроки Из МассивНомеровСтрок Цикл 
		Для СчетчикГраф = 4 По 5 Цикл 
			...
			СтруктураПараметров.Вставить("НомерКС"		, "08." + Формат(СчетчикПроверок, "ЧЦ=2; ЧВН="));
			СтруктураПараметров.Вставить("НомерСтроки"	, НомерСтроки);
			СтруктураПараметров.Вставить("Графа"		, Формат(СчетчикГраф, "ЧЦ=2; ЧВН="));  
			СтруктураПараметров.Вставить("МногоСтрок"	, МассивПризнаковНеМногострочности.Найти(НомерСтроки) = Неопределено);
...
Процедура ПроверкиКС_10_01_03() 
	...
	Для ГруппаПроверок = 1 По 3 Цикл 
		Для ИндексПроверок = 0 По 0 Цикл 
			СчетчикПроверок = СчетчикПроверок + 1;
			...
			ПроверкаКС_10_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_10_шаблон(СтруктураПараметров)
...
&НаСервере
Процедура ПроверкиКС_11_01_14() 
	
	// гр.04 стр. 3250 = гр.04 стр. (3101 + 3230 + 3216 + 3240)
	// гр.05 стр. 3250 = гр.05 стр. (3101 + 3230 + 3216 + 3240)
	// гр.06 стр. 3250 = гр.06 стр. (3101 + 3212 + 3230 + 3216 + 3240)
	// гр.07 стр. 3250 = гр.07 стр. (3101 + 3211 + 3212 + 3230 + 3216 + 3240)
	// гр.08 стр. 3250 = гр.08 стр. (3101 + 3211 + 3230 + 3216 + 3240)
	// гр.09 стр. 3250 = гр.09 стр. (3101 + 3211 + 3212 + 3227 + 3230 + 3216 + 3240)
	// гр.10 стр. 3250 = гр.10 стр. (3101 + 3211 + 3212 + 3227 + 3230 + 3216 + 3240)
	...
	Для ГруппаПроверок = 1 По 2 Цикл 
		Для ИндексПроверок = 0 По 6 Цикл 
			...
			СчетчикПроверок = СчетчикПроверок + 1;
            ПоказательИтога = ?(ГруппаПроверок = 1, "3250", "3300");
			Графа = МассивГраф[ИндексПроверок];
			СтрокаПоказателей = "";
			...
			ПроверкаКС_11_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_11_шаблон(СтруктураПараметров)
...
Процедура ПроверкиКС_12_01_10() 
	...
	Для ГруппаПроверок = 1 По 5 Цикл 
		Для ИндексПроверок = 0 По 1 Цикл 
			...
			ПроверкаКС_12_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_12_шаблон(СтруктураПараметров)
	...
	Для ГруппаПроверок = 1 По 3 Цикл 
		Для ИндексПроверок = 0 По 1 Цикл 
			Для ИндексГраф = 0 По 1 Цикл 
				...
				ПроверкаКС_13_шаблон(СтруктураПараметров);
...
&НаСервере
Процедура ПроверкаКС_13_шаблон(СтруктураПараметров)
...
&НаСервере
Процедура ПроверкиКС_15_01_06() 
	
	// гр.04,05 стр.6200 = гр.04,05 стр.(6220 + 6240 + 6250)
	// гр.04,05 стр.6300 = гр.04,05 стр.(6310 + 6320 + 6330 + 6350)
	// гр.04,05 стр.6400 = гр.04,05 стр.(6100 + 6200 + 6300)
	...
	МассивГраф 			= СтрРазделить(
		"04,05", ",");
	ОднаСтрока			= СтрРазделить(
		"6100,6200,6250,6300,6350", ",");
	
    СтрокаПоказателей1 	= "6220,6240,6250";
    СтрокаПоказателей2 	= "6310,6320,6330,6350";
    СтрокаПоказателей3 	= "6100,6200,6300"; 
	ШаблонДетализации3 = "%1 + %2 + %3 + %4";
	ШаблонДетализации4 = "%1 + %2 + %3";
	СчетчикПроверок 	= 0;
	...
	Для ГруппаПроверок = 2 По 4 Цикл 
		Для ИндексПроверок 	= 0 По 1 Цикл 
			
			Если ГруппаПроверок = 2 Тогда
				СтрокаПоказателей = СтрокаПоказателей1;
			ИначеЕсли ГруппаПроверок = 3 Тогда
				СтрокаПоказателей = СтрокаПоказателей2;
			Иначе
				СтрокаПоказателей = СтрокаПоказателей3;
			КонецЕсли; 
				
			СчетчикПроверок = СчетчикПроверок + 1;
	        ПоказательИтога = "6" + ГруппаПроверок + "00";
			Графа = МассивГраф[ИндексПроверок];
			ШаблонДетализации2 	= ?(ГруппаПроверок = 3, ШаблонДетализации3, ШаблонДетализации4);
...
&НаСервере
Процедура ПроверкиКС_16_01_18() 
	
	СтрокаПоказателей1 = 	"6200,6220,6240,6250,6300,6310,6320,6330,6350";
	СтрокаПоказателей2 = 	"6300,6310,6320,6330,6350";
...
&НаСервере
Процедура ПроверкиКС_17_01_14()

	МассивНомеровСтрок = СтрРазделить(
		"6220,6240,6250,6310,6320,6330,6350", ","); 
	МассивПризнаковНеМногострочности = СтрРазделить(
		"6250,6350", ",");
	СчетчикПроверок = 0;
...
&НаСервере
Функция ДействуетФормат505(КонецПериодаОтчета)
	
	Возврат ОбъектОтчета().ДействуетФормат505(КонецПериодаОтчета);
