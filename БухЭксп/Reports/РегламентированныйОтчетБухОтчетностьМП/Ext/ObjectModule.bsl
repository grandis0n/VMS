...
Функция РелизованаПоддержкаЗаполненияВОблачнойОтчетности(ИДРедакцииОтчета) Экспорт
	...
	МассивПоддерживаемых.Добавить("ФормаОтчета2015Кв4");
	МассивПоддерживаемых.Добавить("ФормаОтчета2012Кв4_2");
	МассивПоддерживаемых.Добавить("ФормаОтчета2012Кв4");
...
Функция ИмяМакетаСтруктурыВыгрузки(ИмяФормы, ДатаДействия)
	...
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда
		...
		Если ДействуетФормат505(ДатаДействия) Тогда
			ИмяМакета = "СхемаВыгрузки505";
		КонецЕсли;
...
Процедура ПодготовитьМакетОтчета_Баланс_2025Кв1(МакетОтчета, ПараметрыПодготовки)
	...
	Если ДействуетФормат505(ПараметрыПодготовки.ОтчетнаяДата) Тогда
		Если МакетОтчета.Области.Найти("П000010135002") <> Неопределено Тогда
			МакетОтчета.Области.П000010135002.Текст = "Целевое финансирование";
		КонецЕсли;
		Если МакетОтчета.Области.Найти("П000010135003") <> Неопределено Тогда
			МакетОтчета.Области.П000010135003.Текст = "1300";
		КонецЕсли;
		Если МакетОтчета.Области.Найти("П10001А135003_1") <> Неопределено Тогда
			МакетОтчета.Области.П10001А135003_1.Текст = "1300";
		КонецЕсли;
	КонецЕсли;
	
	УстановитьРазмерПоляПояснений(МакетОтчета, ПараметрыПодготовки);
...
Процедура УстановитьРазмерПоляПояснений(МакетОтчета, ПараметрыПодготовки)
	
	Если ДействуетФормат505(ПараметрыПодготовки.ОтчетнаяДата) Тогда
		// С версии формата 5.05 для номера пояснения задан размер 10 символов.
		// Этот размер установлен базовым в макетах отчетов.
		Возврат;
	КонецЕсли;
	
	ОбновленныйТип = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(4, ДопустимаяДлина.Переменная));
	
	Для Каждого Область Из МакетОтчета.Области Цикл
		ИмяОбласти = Область.Имя;
		ЭтоПолеПояснений = (СтрНачинаетсяС(ИмяОбласти, "П00001") ИЛИ СтрНачинаетсяС(ИмяОбласти, "П10001"))
			И Сред(ИмяОбласти, 12, 2) = "01";
		Если ЭтоПолеПояснений Тогда
			Если Область.СодержитЗначение = Истина Тогда
				Область.ТипЗначения = ОбновленныйТип;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ДействуетФормат505(КонецПериодаОтчета) Экспорт
	
	Возврат КонецПериодаОтчета >= '2026-06-27';
