...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		Элементы.ГруппаПредставлениеФормы.Видимость = Ложь;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "") Экспорт
	...
	ИначеЕсли ИмяРаздела = "Раздел22" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела И ЗначениеЗаполнено(ОбластьИмя) Тогда
			...
			КонецЕсли;
			Если ГруппаВодопользования = "2" Тогда
				Стр110 = Окр(Ставка * 1000 * 4.65 * Стр120 / 4);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	ЭтоКрупнейшийНалогоплательщикПоКодуМестаПредставления
		= ПараметрыВыгрузки.ПоМесту = "213" ИЛИ ПараметрыВыгрузки.ПоМесту = "216";
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел21_1") Тогда
		...
		Для каждого Ветка21_1 Из ДеревоРаздела.Строки Цикл
			...
			Если ЭтоКрупнейшийНалогоплательщикПоКодуМестаПредставления Тогда
				
				ДлинаЗначПок = СтрДлина(СокрЛП(Дан21_1["П000210000103"]));
				Если ДлинаЗначПок = 0 Тогда
					СообщитьВТС("Раздел 2.1 -> Страница " + НомСтраницы21_1
						+ ": должен быть задан КПП, если код места представления на Титульном листе равен "
						+ ПараметрыВыгрузки.ПоМесту, "Раздел21_1", "П000210000103", НомСтраницы21_1);
					СтатусПроверки = Ложь;
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СокрЛП(Дан21_1["П000210000103"])) Тогда
					СообщитьВТС("Раздел 2.1 -> Страница " + НомСтраницы21_1
						+ ": не должен быть задан КПП, если код места представления на Титульном листе не равен 213 или 216",
						"Раздел21_1", "П000210000103", НомСтраницы21_1);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел22") Тогда
		...
		Для каждого Ветка22 Из ДеревоРаздела.Строки Цикл
			...
			Если ЭтоКрупнейшийНалогоплательщикПоКодуМестаПредставления Тогда
				...
				Если ДлинаЗначПок = 0 Тогда
					СообщитьВТС("Раздел 2.2 -> Страница " + НомСтраницы22
						+ ": должен быть задан КПП, если код места представления на Титульном листе равен "
						+ ПараметрыВыгрузки.ПоМесту, "Раздел22", "П000220000103", НомСтраницы22);
					СтатусПроверки = Ложь;
				ИначеЕсли ДлинаЗначПок <> 9 Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СокрЛП(Дан22["П000220000103"])) Тогда
					СообщитьВТС("Раздел 2.2 -> Страница " + НомСтраницы22
						+ ": не должен быть задан КПП, если код места представления на Титульном листе не равен 213 или 216",
						"Раздел22", "П000220000103", НомСтраницы22);
					СтатусПроверки = Ложь;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Водный налог");
	СтруктураРезультата.Вставить("НаименованиеФормы"	, "2026Кв2");
...
Процедура СформироватьСлужебныеДанные()
	...
	ИменаРазделов.Вставить("Р20", "Раздел21_1");
	ИменаРазделов.Вставить("Р21", "Раздел21_2");
	...
	ПредставленияРазделов.Вставить("Р20", "Подраздела 2.1 Раздела 2");
	...
	ДанныеРазделов.Добавить(Новый Структура("Раздел,Подраздел,Уровень,Индекс", "Раздел21_1"	, "Раздел21_2"	, 1, 0));
	...
	// Раздел 2
	МассивПолейСписочныхЗначений.Добавить("П000210005003");
	МассивПолейСписочныхЗначений.Добавить("П000210008003");
	МассивПолейСписочныхЗначений.Добавить("П000210014003");
	МассивПолейСписочныхЗначений.Добавить("П000220006003");
	
	ТаблицаСписков = РеквизитФормыВЗначение("СвойстваПоказателей", Тип("ТаблицаЗначений"));
	СоответствиеСписков = Новый Соответствие;
	
	ПоляПреобразованияСтр = "П000210014003";
...
Процедура СформироватьДанныеОтчета(ОтчетСсылка, СтруктураОтчета)
	...
	ИменаПоказателей.Вставить("Р01С010"	, "П000010001000");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл
		ЗаполнитьПоказатель(Показатели.Р01С010, СтраницаРаздела);
...
Процедура ПроверкаКС_12_01_к()
	...
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Строка %1 Страница %2 Раздела 1'"));
...
&НаСервере
Процедура ПроверкаКС_12_02_к()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, ДанныеКС.КритичныйРаздел);
	Заголовки.Вставить("Критичность"		, 3);
	Заголовки.Вставить("НомерКС"			, НСтр("ru='12.02'"));
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Раздела 2 по соответствующим КБК и ОКТМО'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4), КБК %5, ОКТМО %6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.030 Раздела 1'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	...
	ИменаПоказателей.Вставить("Р01С020"	, "П00001М102000_");
	ИменаПоказателей.Вставить("Р01С030"	, "П00001М103000_");
	ИменаПоказателей.Вставить("Р20С010"	, "П000210001003");
	ИменаПоказателей.Вставить("Р20С020"	, "П000210002003");
	ИменаПоказателей.Вставить("Р21С160"	, "П000210016003");
	ИменаПоказателей.Вставить("Р22С010"	, "П000220001003");
	...
	Для каждого Ключ_Р1 Из ДанныеКС.МассивКБК_ОКТМО_Раздел1 Цикл
		...
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл 
			...
			Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокП00001М1 Цикл
				...
				Если НЕ ТекущийКлюч = Ключ_Р1 Тогда
					Продолжить;
				КонецЕсли;
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С030, СтраницаРаздела, Счетчик);
				КБК_Стр 	= ЗначСтр(Показатели.Р01С010.Значение);
				ОКТМО_Стр 	= ЗначСтр(Показатели.Р01С020.Значение);
				
			КонецЦикла;
		КонецЦикла;
		
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл 
			
			ЗаполнитьПоказатель(Показатели.Р20С010, СтраницаРаздела);
			ЗаполнитьПоказатель(Показатели.Р20С020, СтраницаРаздела);
			ТекущийКлюч = Показатели.Р20С010.Значение + Показатели.Р20С020.Значение;
			Если НЕ ТекущийКлюч = Ключ_Р1 Тогда
				Продолжить;
			КонецЕсли;
			
			Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
		...
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
			...
			ЗаполнитьПоказатель(Показатели.Р22С010, СтраницаРаздела);
			ЗаполнитьПоказатель(Показатели.Р22С020, СтраницаРаздела);
			ТекущийКлюч = Показатели.Р22С010.Значение + Показатели.Р22С020.Значение;
			Если НЕ ТекущийКлюч = Ключ_Р1 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПроверкаКС_1_01()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='КБК в стр.010 Раздела 1 = стр.010 подр.2.1 Раздела 2 или '") + 
		НСтр("ru='стр.010 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный КБК'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните КБК в стр.010 Раздела 1'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл 
		...
		Для каждого СтраницаРаздела21 Из ДанныеКС.Раздел21_1 Цикл 
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р20С010, СтраницаРаздела21);
			КодНайден = КодНайден ИЛИ Показатели.Р01С010.Значение = Показатели.Р20С010.Значение;
		КонецЦикла;
		
		Для каждого СтраницаРаздела22 Из ДанныеКС.Раздел22 Цикл 
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р22С010, СтраницаРаздела22);
			КодНайден = КодНайден ИЛИ Показатели.Р01С010.Значение = Показатели.Р22С010.Значение;
		КонецЦикла;
		
		НастройкиКС.ЭтоОшибка = НЕ КодНайден;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_02()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код ОКТМО в стр.020 Раздела 1 = стр.020 подр.2.1 Раздела 2 или '") + 
		НСтр("ru='стр.020 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код ОКТМО'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните код ОКТМО в стр.020 Раздела 1'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Строка %1 Страница %2 Раздела 1'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл 
		Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокП00001М1 Цикл
			...
			ЗарегистрироватьПроверку();
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С020, СтраницаРаздела, Счетчик); 
			КодНайден = Ложь;
				
			Для каждого СтраницаРаздела21 Из ДанныеКС.Раздел21_1 Цикл 
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р20С020, СтраницаРаздела21);
				КодНайден = КодНайден ИЛИ Показатели.Р01С020.Значение = Показатели.Р20С020.Значение;
			КонецЦикла;
			
			Для каждого СтраницаРаздела22 Из ДанныеКС.Раздел22 Цикл 
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р22С020, СтраницаРаздела22);
				КодНайден = КодНайден ИЛИ Показатели.Р01С020.Значение = Показатели.Р22С020.Значение;
			КонецЦикла;
...
Процедура ПроверкаКС_2_01()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='КБК в стр.010 подр.2.1 Раздела 2 = стр.010 Раздела 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный КБК'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните КБК в стр.010 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	...
	НастройкиКС.Вставить("Показатель"		, "Р20С010");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000210001003");
	НастройкиКС.Вставить("Раздел_"			, "Раздел21_1");
...
Процедура ПроверкаКС_2_01_код()
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить(НастройкиКС.Показатель, НастройкиКС.ИмяПоказателя);
	ИменаПоказателей.Вставить("Р01С010"		, "П000010001000");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС[НастройкиКС.Раздел_] Цикл
			...
			ЗарегистрироватьПроверку();
			ЗаполнитьИРасшифроватьПоказатель(Показатели[НастройкиКС.Показатель], СтраницаРаздела);
			КодНайден = Ложь;
			
			Для каждого СтраницаРаздела1 Из ДанныеКС.Раздел1 Цикл 
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С010, СтраницаРаздела1);
				КодНайден = КодНайден ИЛИ Показатели[НастройкиКС.Показатель].Значение = Показатели.Р01С010.Значение;
			КонецЦикла;
			
			НастройкиКС.ЭтоОшибка = СтраницаРаздела.РазделЗаполнен И НЕ КодНайден;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_2_02()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код ОКТМО в стр.020 подр.2.1 Раздела 2 = стр.020 Раздела 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код ОКТМО'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните код ОКТМО в стр.020 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "Р20С020");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000210002003");
	НастройкиКС.Вставить("Раздел_"			, "Раздел21_1");
...
Процедура ПроверкаКС_2_02_код()
	...
	Для каждого СтраницаРаздела Из ДанныеКС[НастройкиКС.Раздел_] Цикл
			...
			Для каждого СтраницаРаздела1 Из ДанныеКС.Раздел1 Цикл 
				Для Счетчик = 1 По СтраницаРаздела1.КолвоСтрокП00001М1 Цикл
					ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С020, СтраницаРаздела1, Счетчик);
					КодНайден = КодНайден ИЛИ Показатели[НастройкиКС.Показатель].Значение = Показатели.Р01С020.Значение;
				КонецЦикла;
			КонецЦикла;
...
Процедура ПроверкаКС_2_03()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код водопользования в стр.050 подр.2.1 Раздела 2 должен '") +
		НСтр("ru='соответствовать списку допустимых значений'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Недопустимое значение %1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код водопользования'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Выберите код водопользования в стр.050 подр.2.1 Раздела 2 из списка значений'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р21С050");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000210005003");
	...
	Если НЕ Список = Неопределено Тогда
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
			Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
				...
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					?(ТипЗнч(Показатели[НастройкиКС.Показатель1].Значение) = Тип("Строка"), 
					Показатели[НастройкиКС.Показатель1].ЗначениеСтр, Показатели[НастройкиКС.Показатель1].Значение));
				НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
...
Процедура ПроверкаКС_2_04()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Общий объем забранной воды в стр.060 подр.2.1 Раздела 2 >= '") +
		НСтр("ru='стр.(070, 100) подр.2.1 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2, %1 >= %3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный объем забранной воды'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните объем забранной воды в стр.060 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			НастройкиКС.ЭтоОшибка = Показатели.Р21С060.Значение < Показатели.Р21С070.Значение
				ИЛИ Показатели.Р21С060.Значение < Показатели.Р21С100.Значение;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				Показатели.Р21С060.Значение, Показатели.Р21С070.Значение, Показатели.Р21С100.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_05()
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210006003");
	ИменаПоказателей.Вставить("Р21С002"		, "П000210007003");
	ИменаПоказателей.Вставить("Р21С003"		, "П000210010003");
	НастройкиКС.Вставить("Показатели"		, СформироватьПоказатели(ИменаПоказателей)); 
...
Процедура ПроверкаКС_2_05_код()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			ЗарегистрироватьПроверку();
			ЗаполнитьИРасшифроватьПоказатели(НастройкиКС.Показатели, СтраницаПодраздела);
			
			РасчетнаяСумма = НастройкиКС.Показатели.Р21С002.Значение + НастройкиКС.Показатели.Р21С003.Значение;
			НастройкиКС.ЭтоОшибка = НЕ НастройкиКС.Показатели.Р21С001.Значение = РасчетнаяСумма;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				НастройкиКС.Показатели.Р21С001.Значение, РасчетнаяСумма, НастройкиКС.Показатели.Р21С002.Значение, 
				НастройкиКС.Показатели.Р21С003.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_06()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код цели забора воды в стр.080 подр.2.1 Раздела 2 должен '") +
		НСтр("ru='соответствовать списку допустимых значений'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Недопустимое значение %1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код цели забора воды'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Выберите код цели забора воды в стр.080 подр.2.1 Раздела 2 из списка значений'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р21С080");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000210008003");
...
Процедура ПроверкаКС_2_07()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Объем забранной воды в стр.100 подр.2.1 Раздела 2 = '") +
		НСтр("ru='стр.(101 + 102) подр.2.1 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный объем забранной воды'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните объем забранной воды в стр.100 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210010003");
	ИменаПоказателей.Вставить("Р21С002"		, "П000210010103");
	ИменаПоказателей.Вставить("Р21С003"		, "П000210010203");
	НастройкиКС.Вставить("Показатели"		, СформироватьПоказатели(ИменаПоказателей)); 
...
Процедура ПроверкаКС_2_08()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Коэффициент в стр.140 подр.2.1 Раздела 2 должен '") +
		НСтр("ru='соответствовать списку допустимых значений'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Недопустимое значение %1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный коэффициент'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Выберите коэффициент в стр.140 подр.2.1 Раздела 2 из списка значений'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р21С140");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000210014003");
...
Процедура ПроверкаКС_2_09()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Коэффициент в стр.150 подр.2.1 Раздела 2 может '") +
		НСтр("ru='принимать значения 0 или 10'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный коэффициент'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение коэффициента в стр.150 подр.2.1 Раздела 2 = 0 или 10'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			НастройкиКС.ЭтоОшибка = МассивЗначений.Найти(Показатели.Р21С150.Значение) = Неопределено;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_2_10()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.160 подр.2.1 Раздела 2 = '") +
		НСтр("ru='стр.(101 х 110 + 102 х 120) подр.2.1 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4 + %5 х %6)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.160 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С160"		, "П000210016003");
	ИменаПоказателей.Вставить("Р21С101"		, "П000210010103");
	ИменаПоказателей.Вставить("Р21С110"		, "П000210011003");
	ИменаПоказателей.Вставить("Р21С102"		, "П000210010203");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			РасчетнаяСумма = Окр(Показатели.Р21С101.Значение * Показатели.Р21С110.Значение +
				Показатели.Р21С102.Значение * Показатели.Р21С120.Значение);
			НастройкиКС.ЭтоОшибка = НЕ Показатели.Р21С160.Значение = РасчетнаяСумма;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				Показатели.Р21С160.Значение, РасчетнаяСумма, Показатели.Р21С101.Значение, Показатели.Р21С110.Значение,
				Показатели.Р21С102.Значение, Показатели.Р21С120.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_11()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='КБК в стр.010 подр.2.2 Раздела 2 = стр.010 Раздела 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный КБК'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните КБК в стр.010 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	...
	НастройкиКС.Вставить("Показатель"		, "Р22С010");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220001003");
	НастройкиКС.Вставить("Раздел_"			, "Раздел22");
...
Процедура ПроверкаКС_2_13()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код водопользования в стр.060 подр.2.2 Раздела 2 должен '") +
		НСтр("ru='соответствовать списку допустимых значений'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Недопустимое значение %1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код водопользования'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Выберите код водопользования в стр.060 подр.2.2 Раздела 2 из списка значений'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р22С060");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220006003");
...
Процедура ПроверкаКС_2_12()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Код ОКТМО в стр.020 подр.2.2 Раздела 2 = стр.020 Раздела 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = (...)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный код ОКТМО'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните код ОКТМО в стр.020 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "Р22С020");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220002003");
	НастройкиКС.Вставить("Раздел_"			, "Раздел22");
...
Процедура ПроверкаКС_2_14()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатели в стр.(080, 090, 100) подр.2.2 Раздела 2 не '") +
		НСтр("ru='заполняются, если код водопользования в стр.060 подр.2.2 Раздела 2 = 21010 - 22140'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='(%1, %2, %3) = 0, если %4 = 21010 - 22140'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателей отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значения показателей в стр.(080, 090, 100) подр.2.2 Раздела 2 = 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р22С080", "П000220008003");
	ИменаПоказателей.Вставить("Р22С090", "П000220009003");
	ИменаПоказателей.Вставить("Р22С100", "П000220010003");
	ИменаПоказателей.Вставить("Р22С060", "П000220006003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "2";
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И (ЗначениеЗаполнено(Показатели.Р22С080.Значение) ИЛИ 
			ЗначениеЗаполнено(Показатели.Р22С090.Значение) ИЛИ ЗначениеЗаполнено(Показатели.Р22С100.Значение)); 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С080.Значение, Показатели.Р22С090.Значение, Показатели.Р22С100.Значение, 
			Показатели.Р22С060.ЗначениеСтр);
...
Процедура ПроверкаКС_2_15()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатели в стр.(070, 090, 100) подр.2.2 Раздела 2 не '") +
		НСтр("ru='заполняются, если код водопользования в стр.060 подр.2.2 Раздела 2 = 30100 - 32100'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='(%1, %2, %3) = 0, если %4 = 30100 - 32100'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателей отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значения показателей в стр.(070, 090, 100) подр.2.2 Раздела 2 = 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р22С070", "П000220007003");
	ИменаПоказателей.Вставить("Р22С090", "П000220009003");
	ИменаПоказателей.Вставить("Р22С100", "П000220010003");
	ИменаПоказателей.Вставить("Р22С060", "П000220006003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "3";
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И (ЗначениеЗаполнено(Показатели.Р22С070.Значение) ИЛИ 
			ЗначениеЗаполнено(Показатели.Р22С090.Значение) ИЛИ ЗначениеЗаполнено(Показатели.Р22С100.Значение)); 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С070.Значение, Показатели.Р22С090.Значение, Показатели.Р22С100.Значение, 
			Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_16()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатели в стр.(070, 080) подр.2.2 Раздела 2 не '") +
		НСтр("ru='заполняются, если код водопользования в стр.060 подр.2.2 Раздела 2 = 40100 - 41200'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='(%1, %2) = 0, если %3 = 40100 - 41200'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателей отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значения показателей в стр.(070, 080) подр.2.2 Раздела 2 = 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "4";
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И (ЗначениеЗаполнено(Показатели.Р22С070.Значение) ИЛИ 
			ЗначениеЗаполнено(Показатели.Р22С080.Значение)); 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С070.Значение, Показатели.Р22С080.Значение, 
			Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_17()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатель в стр.070 подр.2.2 Раздела 2 >= 0, '") +
		НСтр("ru='если код водопользования в стр.060 подр.2.2 Раздела 2 = 21010 - 22140'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0, если %2 = 21010 - 22140'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение показателя в стр.070 подр.2.2 Раздела 2 >= 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ГруппаВодопользования", "2");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220007003"); 
...
Процедура ПроверкаКС_2_17_код()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = НастройкиКС.ГруппаВодопользования = ГруппаВодопользования;
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И Показатели.Р22С001.Значение < 0; 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С001.Значение, Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_18()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатель в стр.080 подр.2.2 Раздела 2 >= 0, '") +
		НСтр("ru='если код водопользования в стр.060 подр.2.2 Раздела 2 = 30100 - 32100'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0, если %2 = 30100 - 32100'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение показателя в стр.080 подр.2.2 Раздела 2 >= 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ГруппаВодопользования", "3");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220008003"); 
...
Процедура ПроверкаКС_2_19()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатель в стр.090 подр.2.2 Раздела 2 >= 0, '") +
		НСтр("ru='если код водопользования в стр.060 подр.2.2 Раздела 2 = 40100 - 41200'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0, если %2 = 40100 - 41200'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение показателя в стр.090 подр.2.2 Раздела 2 >= 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ГруппаВодопользования", "4");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220009003"); 
...
Процедура ПроверкаКС_2_20()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Показатель в стр.100 подр.2.2 Раздела 2 >= 0, '") +
		НСтр("ru='если код водопользования в стр.060 подр.2.2 Раздела 2 = 40100 - 41200'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0, если %2 = 40100 - 41200'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение показателя в стр.100 подр.2.2 Раздела 2 >= 0'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ГруппаВодопользования", "4");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000220010003"); 
...
Процедура ПроверкаКС_2_21()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.130 подр.2.2 Раздела 2 = стр.(070 х 110) '") +
		НСтр("ru='подр.2.2 Раздела 2, если код водопользования в стр.060 подр.2.2 Раздела 2 = 21010 - 22140'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4), если %5 = 21010 - 22140'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.130 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.2 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей.Вставить("Р22С130", "П000220013003");
	ИменаПоказателей.Вставить("Р22С070", "П000220007003");
	ИменаПоказателей.Вставить("Р22С110", "П000220011003");
	ИменаПоказателей.Вставить("Р22С060", "П000220006003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "2"; 
		РасчетнаяСумма = Окр(Показатели.Р22С070.Значение * Показатели.Р22С110.Значение);
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р22С130.Значение = РасчетнаяСумма; 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С130.Значение, РасчетнаяСумма, Показатели.Р22С070.Значение, Показатели.Р22С110.Значение, 
			Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_22()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.130 подр.2.2 Раздела 2 = стр.(080 х 110) '") +
		НСтр("ru='подр.2.2 Раздела 2, если код водопользования в стр.060 подр.2.2 Раздела 2 = 30100 - 32100'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4), если %5 = 30100 - 32100'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.130 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "3"; 
		РасчетнаяСумма = Окр(Показатели.Р22С080.Значение * Показатели.Р22С110.Значение);
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р22С130.Значение = РасчетнаяСумма; 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С130.Значение, РасчетнаяСумма, Показатели.Р22С080.Значение, Показатели.Р22С110.Значение, 
			Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_23()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.130 подр.2.2 Раздела 2 = стр.(090 х 100 х 110) '") +
		НСтр("ru='подр.2.2 Раздела 2 / 100, если код водопользования в стр.060 подр.2.2 Раздела 2 = 40100 - 41200'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4 х %5 / 100), если %6 = 40100 - 41200'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.130 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@, Письмо ФНС России от 07.09.2016 N СД-4-3/16671@'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
		...
		ГруппаВодопользования = Лев(Показатели.Р22С060.Значение, 1);
		УсловиеВыполнено = ГруппаВодопользования = "4"; 
		РасчетнаяСумма = Окр(Показатели.Р22С090.Значение * Показатели.Р22С100.Значение * Показатели.Р22С110.Значение / 100);
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р22С130.Значение = РасчетнаяСумма; 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р22С130.Значение, РасчетнаяСумма, Показатели.Р22С090.Значение, Показатели.Р22С100.Значение, 
			Показатели.Р22С110.Значение, Показатели.Р22С060.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_24()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Объем воды, забранной в пределах лимита в стр.101 подр.2.1 '") +
		НСтр("ru='Раздела 2 <= стр.090 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <= %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный объем воды, забранной в пределах лимита'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните объем воды в стр.101 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			НастройкиКС.ЭтоОшибка = Показатели.Р21С101.Значение > Показатели.Р21С090.Значение;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				Показатели.Р21С101.Значение, Показатели.Р21С090.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_25()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Налоговая ставка в стр.120 подр.2.1 Раздела 2 = стр. 110 '") +
		НСтр("ru='подр.2.1 Раздела 2, если стр.080 подр.2.1 Раздела 2 = 1 и стр.102 подр.2.1 Раздела 2 > 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2, если %3 = 1 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная налоговая ставка'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните налоговую ставку в стр.120 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ДляНаселения"		, Истина);
	ПроверкаКС_2_25_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_2_25_код()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			Если НастройкиКС.ДляНаселения Тогда
				УсловиеВыполнено = Показатели.Р21С080.Значение = "1" И Показатели.Р21С102.Значение > 0;
				НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р21С120.Значение = Показатели.Р21С110.Значение;
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					Показатели.Р21С120.Значение, Показатели.Р21С110.Значение, Показатели.Р21С080.ЗначениеСтр, 
					Показатели.Р21С102.Значение);
			Иначе
				УсловиеВыполнено = ЗначениеЗаполнено(Показатели.Р21С080.Значение) И Показатели.Р21С080.Значение <> "1" 
					И Показатели.Р21С102.Значение > 0; 
				РасчетнаяСумма = Окр(Показатели.Р21С110.Значение * 5, 3);
				НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р21С120.Значение = РасчетнаяСумма;
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					Показатели.Р21С120.Значение, РасчетнаяСумма, Показатели.Р21С110.Значение, Показатели.Р21С080.ЗначениеСтр, 
					Показатели.Р21С102.Значение);
			КонецЕсли;
...
Процедура ПроверкаКС_2_26()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Налоговая ставка в стр.120 подр.2.1 Раздела 2 = стр. 110 '") +
		НСтр("ru='подр.2.1 Раздела 2 х 5, если стр.080 подр.2.1 Раздела 2 <> 1 и стр.102 подр.2.1 Раздела 2 > 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х 5), если %4 <> 1 и %5 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная налоговая ставка'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните налоговую ставку в стр.120 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	...
	НастройкиКС.Вставить("ДляНаселения"		, Ложь);
	ПроверкаКС_2_25_код();
...
Процедура ПроверкаКС_2_27()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='Коэффициент в стр.150 подр.2.1 Раздела 2 = 10, если '") +
		НСтр("ru='стр.080 подр.2.1 Раздела 2 = 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 10, если %2 = 2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный коэффициент'"));
	Заголовки.Вставить("Рекомендации"		, 
		НСтр("ru='Укажите значение коэффициента в стр.150 подр.2.1 Раздела 2 = 10'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Приказ ФНС России от 26.03.2026 N ЕД-1-3/205@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.2.1 Раздела 2'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел21_1 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел21_2 Цикл
			...
			УсловиеВыполнено = Показатели.Р21С080.Значение = "2";
			НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р21С150.Значение = 10;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				Показатели.Р21С150.Значение, Показатели.Р21С080.ЗначениеСтр);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
			ДополнитьТаблицуРезультата();
