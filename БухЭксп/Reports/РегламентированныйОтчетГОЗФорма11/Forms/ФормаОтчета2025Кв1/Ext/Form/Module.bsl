...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		Для Инд = 1 По ИсхКолвоКолонокРазделов Цикл
			...
			Если Инд = 1 Или Инд = 2 Или Инд = 7 Тогда
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	ИмяЯчейкиНомерСроки = "П001001000101_1";
	Если Не ЗначениеЗаполнено(Области[ИмяЯчейкиНомерСроки].Значение) Тогда
		Области[ИмяЯчейкиНомерСроки].Значение = "1";
	КонецЕсли;
	
	ИмяЯчейкиПерваяСтатья = "П001001000102_1";
	ТекстСтатьи = "Амортизация зданий, сооружений, производственного оборудования, транспортных средств (имущества)";
	Если Не ЗначениеЗаполнено(Области[ИмяЯчейкиПерваяСтатья].Значение) Тогда
		Области[ИмяЯчейкиПерваяСтатья].Значение = ТекстСтатьи;
	КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["НаименованиеПодразделения"].Значение, "НаименованиеПодразделения");
