...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.ПрПодп = "2" ИЛИ (НЕ ПараметрыВыгрузки.ЭтоИП И ПараметрыВыгрузки.ПрПодп = "1") Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
			СообщитьВТС("Не задана фамилия подписанта налогоплательщика", "Титульный", "ОргПодписант");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяПодписанта) Тогда
			СообщитьВТС("Не задано имя подписанта налогоплательщика", "Титульный", "ОргПодписант");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	Если ПараметрыВыгрузки.ПрПодп = "2" И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимДокПред) Тогда
		СообщитьВТС("Не задано наименование документа, подтверждающего полномочия представителя налогоплательщика",
		"Титульный", "ДокУпПред");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Титульный.ПрПредИной_050 = "1" Тогда
			Если ЗначениеЗаполнено(Титульный.ИнойНалог_050_1) Тогда
				...
				КонецЕсли;
			Иначе
				СообщитьВТС("При указании признака представления ""1"" необходимо заполнение номера главы (или номера главы с номером пункта) из второй части НК РФ в строке 050",
				"Титульный", "ИнойНалог_050_1");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;	
