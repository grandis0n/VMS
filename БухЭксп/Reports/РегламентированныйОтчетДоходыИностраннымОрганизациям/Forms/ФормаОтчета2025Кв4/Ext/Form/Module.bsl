		...
		Для каждого СтраницаРаздела31 Из ДеревоРаздела31.Строки Цикл
			...
			Если ЗначениеЗаполнено(ДокПдтвМНДатаДок)
				...
				Если НЕ ЗначениеЗаполнено(ДокПдтвМНДатаДок) Тогда
					...
					+ "подтверждающего постоянное местонахождение (резидентство) в иностранном государстве получателя дохода (код строки 060).
					|Строки с кодами 060-080 должны быть или одновременно заполнены, или одновременно не заполнены.",
				...
				Если НЕ ЗначениеЗаполнено(ДокПдтвМННомДок) Тогда
					...
					+ "подтверждающего постоянное местонахождение (резидентство) в иностранном государстве получателя дохода (код строки 070).
					|Строки с кодами 060-080 должны быть или одновременно заполнены, или одновременно не заполнены.",
				...
				Если ДлинаОКСМ <> 3 Тогда
					...
					+ "государстве получателя дохода (код строки 080).
					|Строки с кодами 060-080 должны быть или одновременно заполнены, или одновременно не заполнены.",
	...
	Если Ячейка.Раздел = "Раздел1" ИЛИ Ячейка.Раздел = "Раздел2" ИЛИ Ячейка.Раздел = "Раздел31" ИЛИ Ячейка.Раздел = "Раздел41" ИЛИ Ячейка.Раздел = "Раздел5" Тогда
		...
		Для каждого НайденныйРаздел Из РазделыОтчета.ПолучитьЭлементы() Цикл
			Если НайденныйРаздел.КолонкаРазделыОтчетаСокрНаим = Ячейка.Раздел Тогда
				...
				ИдентификаторТекущейСтроки = НайденныйРаздел.ПолучитьИдентификатор();
				Если (Ячейка.Раздел = "Раздел31" ИЛИ Ячейка.Раздел = "Раздел41") И ЗначениеЗаполнено(Ячейка.Страница) Тогда
					НомерСтраницы = Число(Ячейка.Страница);
					Для каждого НайденнаяСтраница Из НайденныйРаздел.ПолучитьЭлементы() Цикл
						Если НайденнаяСтраница.КолонкаНомерСтраницыРазделаОтчета = НомерСтраницы Тогда
							ИдентификаторТекущейСтроки = НайденнаяСтраница.ПолучитьИдентификатор();
							Прервать;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				Элементы.РазделыОтчета.ТекущаяСтрока = ИдентификаторТекущейСтроки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
