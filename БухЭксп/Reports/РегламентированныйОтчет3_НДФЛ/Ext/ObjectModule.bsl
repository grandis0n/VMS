...
Функция РелизованаПоддержкаЗаполненияВОблачнойОтчетности(ИДРедакцииОтчета) Экспорт
	...
	МассивПоддерживаемых.Добавить("ФормаОтчета2026Кв3");
	МассивПоддерживаемых.Добавить("ФормаОтчета2025Кв4");
...
Процедура РассчитатьИтоги(ИмяФормы, Контейнер, СтруктураРеквизитовФормы, СтруктураМногоуровневыхРазделов)
	...
	Если ЗаполненыСведенияОбОбъектеИРасходах Тогда
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв3") Тогда
			
			Стр140 = Стр140 - Окр(ВеткаДрРазд.Данные["П000П50025503"], 2);
...
Функция ИзвлечьСтруктуруXML(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв3") Тогда
		Макет = ЭтотОбъект.ПолучитьМакет("СхемаВыгрузки521");
	Иначе// ФормаОтчета2025Кв4
...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв3") Тогда
		...
		Если ТекущаяДатаСеанса() < '20260901' Тогда
			
			СообщитьВТС(ИмяФормы, РезультатВыгрузки, "До 31.08.2026 (включительно) представление декларации доступно только "
				+ "по форме, утвержденной приказом ФНС России от 20.10.2025 № ЕД-7-11/913@", "Титульный", "НомерКорректировки");
	...
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2025Кв4") Тогда
		
		Если ТекущаяДатаСеанса() >= '20260901' И СтруктураРеквизитовФормы.мСохраненныйДок.Вид = 0
			И КонецДня(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) >= КонецДня('20251231') Тогда
			
			СообщитьВТС(ИмяФормы, РезультатВыгрузки, "С 01.09.2026 представление первичной декларации, начиная с отчета за "
				+ "2025 год, доступно только по форме, утвержденной приказом ФНС России от 20.10.2025 № ЕД-7-11/913@ "
				+ "(в редакции приказа ФНС России от 25.05.2026 № ЕД-1-11/333@)", "Титульный", "НомерКорректировки");
...
Функция ВозможныеЗначенияИзСпискаВыбораПоказателя(ИмяФормы, ИмяГруппыЗначений)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв3") Тогда
		МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СпискиВыбора2026Кв3");
	Иначе// ФормаОтчета2025Кв4
