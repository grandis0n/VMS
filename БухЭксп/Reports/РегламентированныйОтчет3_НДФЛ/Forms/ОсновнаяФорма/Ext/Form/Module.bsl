...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		Если Год(Форма.мДатаКонцаПериодаОтчета) <= 2024 Тогда
	...
	Если Форма.ДатаПодписи < '20260901'
		И Форма.РезультирующаяТаблица.Количество() = 2
		И СокрЛП(Форма.РезультирующаяТаблица[0].ФормаОтчета) = "ФормаОтчета2026Кв3"
		И СокрЛП(Форма.РезультирующаяТаблица[1].ФормаОтчета) = "ФормаОтчета2025Кв4" Тогда
