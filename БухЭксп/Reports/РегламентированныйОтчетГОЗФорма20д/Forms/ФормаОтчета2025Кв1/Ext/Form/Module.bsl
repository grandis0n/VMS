...
Процедура ДополнитьРеквизитыФормы()
	...
	КатегорииКлассификатора =
		СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010020001", ";", Истина);
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["ОбоснованиеПрибыли"].Значение,        "ОбоснованиеПрибыли");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 20 (20д)");
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_4"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.Расшифровка) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан расчет и обоснование прибыли",
			ИмяПоляТаблДокумента, "ОбоснованиеПрибыли");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ОбоснованиеПрибыли", СокрЛП(ТабДокумент.ОбоснованиеПрибыли));
