...
Функция ОстаткиТовараСоСрокомХраненияДляМонитораРуководителя(Организация, Дата, СрокХраненияТоваров) Экспорт 
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В (&СчетаУчетаНоменклатура), &СубконтоНоменклатура, Организация = &Организация) КАК ХозрасчетныйОстатки
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Подразделение,
		...
		|	ИЗ
		|		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоИнтервала, &КонецИнтервала, , Счет В (&СчетаУчетаНоменклатура), &СубконтоНоменклатура, Организация = &Организация, , ) КАК ХозрасчетныйОбороты
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|				&КонецИнтервала,
		|				,
		|				Счет В (&СчетаУчетаСкладИНоменклатура),
		|				&СубконтоНоменклатура,
		|				Организация = &Организация
		|					И Подразделение = ПодразделениеКор,
		|				КорСчет В (&СчетаУчетаСкладИНоменклатура),
		|				&СубконтоНоменклатура) КАК ХозрасчетныйОбороты
		|	ГДЕ
