...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010010001;П0010010002;П0010020001;П0010020002", ";", Истина);
КонецФункции
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если ИмяГруппы = "П0010010001" Или ИмяГруппы = "П0010010002" Тогда
		ГраФаКон  = 16;
		ГраФаКон2 = 23;
	ИначеЕсли ИмяГруппы = "П0010020001" Или ИмяГруппы = "П0010020002" Тогда
		ГраФаКон  = 23;
		ГраФаКон2 = 16;
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Раздел = 1 По 2 Цикл
		...
		Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
			...
			Иначе
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 5");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
