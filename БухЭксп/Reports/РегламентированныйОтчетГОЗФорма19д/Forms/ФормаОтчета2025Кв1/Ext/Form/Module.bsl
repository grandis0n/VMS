...
Процедура ДополнитьРеквизитыФормы()
	...
	ИсхКолвоСтрокРазделов    = 4;
	
	ТипКолонкиСтрока    = Новый ОписаниеТипов("Строка");
	ТипКолонкиЧисло15_0 = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0));
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		Если Раздел = "1" Тогда
			...
			ИсхКолвоКолонокРазделов = 6;
			
			Для Инд = 1 По 6 Цикл
				
				Если Инд = 1 Или Инд = 2 Тогда
					ТипКолонки = ТипКолонкиСтрока;
				ИначеЕсли Инд = 3 Или Инд = 4 Или Инд = 5 Тогда
					ТипКолонки = ТипКолонкиЧисло15_0;
				Иначе
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_3"));
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 19 (19д)");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	...
	МассивПроверки = Новый Массив;
	МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_1"));
	МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_2"));
	МассивПроверки.Добавить(ТекущаяОбластьМногострочнойЧасти);
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		ТекущаяОбластьМногострочнойЧасти = ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть(ТекущийВерх, , ТекущийВерх);
		МассивПроверки.Добавить(ТекущаяОбластьМногострочнойЧасти);
		МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_3"));
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 19 (19д)");
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3_1")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 19 (19д)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
