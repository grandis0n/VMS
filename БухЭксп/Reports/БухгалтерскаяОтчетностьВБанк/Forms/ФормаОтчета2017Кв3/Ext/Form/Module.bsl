...
Процедура ИнициализироватьДеревоРазделовОтчета()
	...
	// Отчеты, по которым в конфигурации-потребителе не реализован учет, и нет ручного заполнения.
	УчетЦифровогоРубляНаОтдельномСчете = ЗначениеЗаполнено(
		ОтчетностьВБанкиСлужебный.ОбщиеНастройки().БанковскиеСчета.ЦифровойРубль);
	ИсключитьИлиНеПоказыватьЦифровойРубль = ?(УчетЦифровогоРубляНаОтдельномСчете, ЗначениеНастройки.НеПоказывать,
		ЗначениеНастройки.Исключить);
	...
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = ИсключитьИлиНеПоказыватьЦифровойРубль;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
	СтрокаУровня2.АвторасчетПоказателей    = Истина;
	СтрокаУровня2.СтраницаВывода           = "СтраницаОтчета";
