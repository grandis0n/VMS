...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ПараметрыЗаполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПараметрыЗаполнения", Новый Структура);
		Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УведомлениеПоУСН;
		Объект.РегистрацияВИФНС = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "НалоговыйОрган",
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект["ДанныеУведомления"]["Титульная"], ПараметрыЗаполнения);
	КонецЕсли;
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.по УСН",
	...
	КонецЕсли;
	ПоказатьТекущуюСтраницу("Титульная_2025_1", Неопределено);
КонецПроцедуры
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Иначе
		СтрокаСведений = "ФамилияИП,ИмяИП,ОтчествоИП,ИННФЛ,ТелДом,ДатаРожд,КодУдЛичн,СерияУдЛичн,НомерУдЛичн";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
...
Процедура СохранитьДанные() Экспорт
	...
	Объект.ВидУведомления = Отчеты[Объект.ИмяОтчета].ВидУведомленияПоПризнаку(СтруктураПараметров.ДанныеУведомления.Титульная.ПрОснПрУвед);
...
Процедура ЗаполнитьАвтоНаСервере(ПараметрыЗаполнения = Неопределено)
	...
	Контейнер = Отчеты[Объект.ИмяОтчета].КонтейнерДляЗаполнения(Объект.ИмяФормы);
	ЗаполнитьЗначенияСвойств(Контейнер.Титульная, ЭтотОбъект["ДанныеУведомления"].Титульная);
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(Объект.ИмяОтчета, Объект.ИмяФормы, ПараметрыОтчета, Контейнер);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект["ДанныеУведомления"].Титульная, Контейнер.Титульная);
	Модифицированность = Истина;
КонецПроцедуры
