...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек,
		"Счет", CW_БухгалтерскийУчетПовтИспМСФО.СчетаВИерархии(ПараметрыОтчета.Счет));
	...
	Если ПараметрыОтчета.СальдоНаНачалоДт Тогда
		...
		Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
			Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
				...
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(Группа, ВыбранноеПоле);
			КонецЕсли;
		КонецЦикла;
	...
	Если ПараметрыОтчета.ОборотыСоСчетамиДт Тогда
		...
		Если Не ПараметрыОтчета.ПоСубсчетамКорСчетов Тогда
			ЗначениеОтбора = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Колонка.Отбор, "SystemFields.LevelInGroup", 1);
			ЗначениеОтбора.Применение = ТипПримененияОтбораКомпоновкиДанных.Иерархия;
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(Колонка, "ВыводитьОтбор", ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
		КонецЕсли;
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(Колонка, "РасположениеОбщихИтогов", РасположениеИтоговКомпоновкиДанных.Нет);
		
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(Колонка.Выбор, "КорСчет");
		...
		Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
			Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
				...
				ВыводимыеПоля.Добавить(ВыбранноеПоле);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(Группа, ВыбранноеПоле);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ОтборГруппаИЛИ, ВыбранноеПоле, 0, ВидСравненияКомпоновкиДанных.НеРавно);
			КонецЕсли;
		КонецЦикла;
		Для Каждого ИмяПоказателя Из МассивПоказателейДоп Цикл
			Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	CW_БухгалтерскиеОтчетыМСФО.ОтключитьИгнорированиеNull(МакетКомпоновки.НаборыДанных.ОстаткиИОборотыПоКорсчетам.Поля);
...
//
// Параметры:
//  Правила - ТаблицаЗначений - Правила расшифровки этого отчета см. CW_БухгалтерскиеОтчетыРасшифровкаМСФО.НовыйПравилаРасшифровки.
//
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт

	Правило = Правила.Добавить();
	Правило.Отчет = "CW_ОтчетПоПроводкамМСФО";
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "БухТипРесурса", "БухТипРесурса");
...
Процедура НастроитьСхемуКомпоновкиДанных(ПараметрыОтчета, Схема) Экспорт
	...
	Для Индекс = 1 По КоличествоСубконто Цикл
		Для Каждого НаборДанных Из МассивНаборовДанных Цикл
			...
			Если Поле.ТипЗначения.СодержитТип(CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ТипОсновныеСредства()) Тогда
...
Процедура ДобавитьУсловноеОформлениеОстатков(Настройки, ПоляОстатков)
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(УсловноеОформление.Оформление, "Текст", "");
...
Функция ЗаменитьНомераСубконтоВТексте(Знач ТекстЗапроса, СтрокаКартыСубконто)
	...
	Для НомерСубконто = 1 По 3 Цикл
		...
		Иначе
			...
			НомерСимвола = СтрНайти(ТекстЗапроса, "РегистрБухгалтерии.CW_МСФО.");
...
Функция КартаСубконто(СчетГруппа, НомераИспользуемыхСубконтоСчетаГруппы, Субсчета)
	...
	// В параметре &ВидыСубконто виртуальной таблицы ОстаткиИОбороты() нужно передать только один элемент ПВХ.CW_ВидыСубконтоМСФО.Договоры.
	...
	ОписаниеТиповВидСубконто = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО");
	
	ДетальнаяКартаСубконто = Новый ТаблицаЗначений;
	ДетальнаяКартаСубконто.Колонки.Добавить("Счет",         Новый ОписаниеТипов("ПланСчетовСсылка.CW_МСФО"));
	...
	СвойстваСчетаГруппы = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(СчетГруппа);
...
Процедура ИзменитьСхемуДляОтборовПоСубконтоСубсчетов(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	CW_БухгалтерскиеОтчетыМСФО.СписокОтборовПоСубконто(КомпоновщикНастроек.Настройки.Отбор.Элементы, НомераОтбираемыхСубконто);
	...
	Для НомерКомбинацииСубконто = 1 По КартаСубконто.Количество() Цикл
		...
		ПараметрСхемы.ТипЗначения              = Новый ОписаниеТипов("ПланСчетовСсылка.CW_МСФО");
		...
		ПараметрСхемы.Заголовок                = ПараметрСхемы.Имя;
		ПараметрСхемы.ТипЗначения              = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО");
		ПараметрСхемы.ДоступенСписокЗначений   = Истина;
		ПараметрСхемы.ВключатьВДоступныеПоля   = Истина;
		ПараметрСхемы.ОграничениеИспользования = Истина;
