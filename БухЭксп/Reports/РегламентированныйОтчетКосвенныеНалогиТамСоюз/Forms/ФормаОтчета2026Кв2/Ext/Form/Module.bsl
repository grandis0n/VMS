...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЗаполнитьПериодОтчета Тогда
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаПредставлениеФормы.Видимость = ТекущаяДатаСеанса() < '20260701';
	КонецЕсли;
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	СтруктураРеквизитовФормы.мИдГруппы4М1 = "П00004М1";
	СтруктураРеквизитовФормы.мИдГруппы4М2 = "П00004М2";
	...
	ИменаГраф.Добавить("П00002М101000");
	ИменаГраф.Добавить("П00002М102000");
	ИменаГраф.Добавить("П00002М103000");
	ИменаГраф.Добавить("П00002М104000");
	ИменаГраф.Добавить("П00002М105000");
	ИменаГраф.Добавить("П00002М106000");
	ИменаГраф.Добавить("П00002М107000");
	ИменаГраф.Добавить("П00002М108000");
	...
	ИменаГраф.Добавить("П00003М103100");
	ИменаГраф.Добавить("П00003М103200");
	ИменаГраф.Добавить("П00003М103300");
	ИменаПодчиненныхГрупп = Новый Массив;
	...
	ИменаГраф.Добавить("П00003М300005");
	ИменаГраф.Добавить("П00003М305000");
	ИменаПодчиненныхГрупп = Новый Массив;
	...
	// Группа П00004М1.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00004М101000");
	ИменаПодчиненныхГрупп = Новый Массив;
	ИменаПодчиненныхГрупп.Добавить(СтруктураРеквизитовФормы.мИдГруппы4М2);
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы4М1, Новый Структура(
	"ИменаГраф,ИсхКолСтрок,УровеньПодчинения,ИменаПодчиненныхГрупп", ИменаГраф, 1, 0, ИменаПодчиненныхГрупп));
	
	// Группа П00004М2.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00004М202000");
	ИменаГраф.Добавить("П00004М203000");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы4М2, Новый Структура(
	"ИменаГраф,ИсхКолСтрок,УровеньПодчинения,ИменаПодчиненныхГрупп", ИменаГраф, 1, 1, ИменаПодчиненныхГрупп));
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	СтруктураРаздела = Новый Структура;
	ИменаГруппМнЧ = Новый Массив;
	ИменаГруппМнЧ.Добавить(СтруктураРеквизитовФормы.мИдГруппы4М1);
	СтруктураРаздела.Вставить("ИменаГруппМнЧ", ИменаГруппМнЧ);
...
Процедура СформироватьСпискиВыбора()
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					Графа3_Текст = СокрП(МакетСоставаПоказателей.Область(НомСтр, 3).Текст);
					
					Если Область.Имя = "Периоды" Тогда
						
						СтрокаСпискаСоответствуетМесяцу = ЗначениеЗаполнено(Графа3_Текст) И (Графа3_Текст = МесяцОтчета_Стр
							ИЛИ (Графа3_Текст = ?(СтруктураРеквизитовФормы.ИП, "ИП", "ЮЛ") + МесяцОтчета_Стр));
						
						СтрокаСпискаУдовлетворяетУсловиям = НЕ ЗначениеЗаполнено(Графа3_Текст) ИЛИ СтрокаСпискаСоответствуетМесяцу;
						
						Если НЕ СтрокаСпискаУдовлетворяетУсловиям Тогда
					...
					НовСтрока.КодПоОКЕИ = Графа3_Текст;
				КонецЕсли;
			КонецЦикла;
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Период", 2, ,
		"Выбор кода налогового периода", КоллекцияСписковВыбора["Периоды"]);
	...
	// Соответствие КБК и кодов вида подакцизного товара в разделе 3.
	//
	ПрефиксСекцииСПодгруппамиКодовВидаПТ = ПрефиксСекцииСПодгруппамиКодовВидаПТ(ЭтотОбъект, "Раздел3");
	Для каждого СпВыбора Из КоллекцияСписковВыбора Цикл
		Если СтрНачинаетсяС(СпВыбора.Ключ, ПрефиксСекцииСПодгруппамиКодовВидаПТ) Тогда
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, СпВыбора.Ключ);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00003М300001", 3, ,
		"Выбор кода вида подакцизного товара", КоллекцияСписковВыбора["КодыВидаПТ_Раздел3"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00003М300002", 3, ,
		"Выбор кода единицы измерения по ОКЕИ", КоллекцияСписковВыбора["КодыОКЕИ_Раздел3"]);
	
	//Раздел 4.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00004М101000", 0, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002М101000") ИЛИ СтрНачинаетсяС(Область.Имя, "П00002М102000") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПрефиксИмениОблКодПериода = "П00002М102000";
		ПрефиксИмениОблГодПериода = "П00002М101000";
	...
	Иначе
		...
		Если СтрНачинаетсяС(Область.Имя, "П00003М300001") Тогда
			...
			Если ОбластьКБК <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(ЗначениеКБК) Тогда
					...
					ЭтотОбъект, ПрефиксСекцииСПодгруппамиКодовВидаПТ + ЗначениеКБК);
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00003М300002") Тогда
			ОблКодВидаПТ = ТабличныйДокумент.Области.Найти("П00003М300001" + Сред(Область.Имя,
			СтруктураРеквизитовФормы.ДлинаИмениГрафы + 1));
			КодВидаПТ = СокрЛП(ОблКодВидаПТ.Значение);
			Если ЗначениеЗаполнено(КодВидаПТ) Тогда
				ТЗКодовВидаПТ = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "П00003М300001").ТаблицаЗначений;
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					ТЗКодовПоОКЕИ = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "П00003М300002").ТаблицаЗначений;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000010001003" ИЛИ ИмяПоказателя = "П000030001003" Тогда
	...
	Если ИмяПоказателя = "П000010002003"
		ИЛИ СтрНачинаетсяС(ИмяПоказателя, "П00003М102000") ИЛИ СтрНачинаетсяС(ИмяПоказателя, "П00004М101000") Тогда
	...
	Если ИмяПоказателя = "П000010007003" ИЛИ ИмяПоказателя = "П000010008003" Тогда
		ЗначПок1 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект,
			ТабличныйДокумент.Области["П000010007003"].Значение);
		ЗначПок2 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект,
			ТабличныйДокумент.Области["П000010008003"].Значение);
		Если ИмяПоказателя = "П000010007003" Тогда
			ОблПарногоПоказателя = ТабличныйДокумент.Области["П000010008003"];
		Иначе
			ОблПарногоПоказателя = ТабличныйДокумент.Области["П000010007003"];
		КонецЕсли;
		Если ЗначениеЗаполнено(ЗначПок1) И ЗначениеЗаполнено(ЗначПок2) Тогда
			ОблПарногоПоказателя.Очистить();
			ТекстСообщения
				= "ru='Не допускается одновременное заполнение строк 070 и 080. Значение в строке %1 очищено'";
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(ТекстСообщения), Сред(ОблПарногоПоказателя.Имя, 9, 3)));
			Мофицированность = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если СтрНачинаетсяС(ИмяПоказателя, "П00003М103200") ИЛИ СтрНачинаетсяС(ИмяПоказателя, "П00003М103300") Тогда
		Постф = Сред(ИмяПоказателя, СтруктураРеквизитовФормы.ДлинаИмениГрафы + 1);
		ЗначПок1 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект,
			ТабличныйДокумент.Области["П00003М103200" + Постф].Значение);
		ЗначПок2 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект,
			ТабличныйДокумент.Области["П00003М103300" + Постф].Значение);
		Если СтрНачинаетсяС(ИмяПоказателя, "П00003М103200") Тогда
			ОблПарногоПоказателя = ТабличныйДокумент.Области["П00003М103300" + Постф];
		Иначе
			ОблПарногоПоказателя = ТабличныйДокумент.Области["П00003М103200" + Постф];
		КонецЕсли;
		Если ЗначениеЗаполнено(ЗначПок1) И ЗначениеЗаполнено(ЗначПок2) Тогда
			ОблПарногоПоказателя.Очистить();
			ТекстСообщения
				= "ru='Не допускается одновременное заполнение строк 032 и 033. Значение в строке %1 очищено'";
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(ТекстСообщения), Сред(ОблПарногоПоказателя.Имя, 9, 3)));
			Мофицированность = Истина;
		КонецЕсли;
	...
	// Особая обработка: если для выбранного кода вида подакцизного товара предусмотрен единственный код по ОКЕИ,
	// тогда выполняется установка такого кода по ОКЕИ.
	Если СтрНачинаетсяС(ИмяПоказателя, "П00003М300001") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		Мас = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П00003М300001"));
		Если Мас.Количество() > 0 Тогда
			Мас_ = Мас[0].ТаблицаЗначений.НайтиСтроки(Новый Структура("Код", ЗначПоказателя));
			Если Мас_.Количество() = 1 Тогда
				КодыПоОКЕИ = СтрРазделить(СокрЛП(Мас_[0].КодПоОКЕИ), ", ", Ложь);
				Если КодыПоОКЕИ.Количество() = 1 Тогда
					Обл = ТабличныйДокумент.Области.Найти(СтрЗаменить(ИмяПоказателя, "П00003М300001", "П00003М300002"));
					Если Обл <> Неопределено Тогда
						Обл.Значение = СокрЛП(КодыПоОКЕИ[0]);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если ИмяРаздела = "Титульный" И НЕ СтруктураРеквизитовФормы.ИП Тогда
		ЗаполнитьПериодОтчета(ЭтотОбъект);
	КонецЕсли;
	
	Если СтруктураЗависимыхЯчеекДляАвторасчета.Свойство(ИмяРаздела) Тогда
		
		Если ИмяРаздела = "Раздел1" ИЛИ ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел4" Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.АвтозаполняемыеПоказатели.Количество() = 0
			ИЛИ РегламентированнаяОтчетностьВызовСервера.ИспользуетсяОднаОрганизация() Тогда
			
			ОрганизацияДляЗаполнения = ТДО.ОрганизацииДляЗаполненияОтчета;
			ОрганизацияДляЗаполнения.Гиперссылка = Ложь;
			ОрганизацияДляЗаполнения.Текст = "";
			
		КонецЕсли;
		
		ОблПериод = ТДО.Период;
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		Для каждого Пок Из СтруктЗависимыхЯчеек Цикл
			Если НЕ СтрНачинаетсяС(Пок.Ключ, Форма.СтруктураРеквизитовФормы.мИдГруппы3М3) Тогда
				АвтоРасчПоказатели.Добавить(Пок.Ключ);
			КонецЕсли;
		КонецЦикла;
		
		Для каждого Обл Из ТДО Цикл
			Если СтрНачинаетсяС(Обл.Имя, "П00003М300002") Тогда
				...
				Если КодПоОКЕИ = "831" ИЛИ КодПоОКЕИ = "251" Тогда
					АвтоРасчПоказатели.Добавить("П00003М300005" + _Координаты);
				Иначе
					РазрешитьВводДанныхВОбласть(Форма, ТДО.Найти("П00003М300005" + _Координаты));
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	Для каждого ИмяПок Из АвтоРасчПоказатели Цикл
		...
		Если Сред(ИмяПок, 7, 1) = "М" Тогда
			...
			Если СтрНачинаетсяС(ИмяПок, "П00003М300005") Тогда
...
Процедура ВставитьДополнительнуюСтроку()
	...
	Если СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей.Свойство(ИмяГруппы) Тогда
		...
		// Специальная обработка после вставки строки в разделе 3, в связи с
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	Если ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы3М1 Тогда
		Подстрока = "строку";
		ТекстВопроса = НСтр("ru='Удалить выбранную строку?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы3М2 Тогда
		Подстрока = "страну";
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "") Экспорт
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела И ЗначениеЗаполнено(ОбластьИмя) Тогда
			...
			Если Лев(ОбластьИмя, Форма.СтруктураРеквизитовФормы.ДлинаИмениГруппы)
				= Форма.СтруктураРеквизитовФормы.мИдГруппы3М3
				ИЛИ Лев(ОбластьИмя, Форма.СтруктураРеквизитовФормы.ДлинаИмениГрафы) = "П00003М103100" Тогда
				
				Если Лев(ОбластьИмя, Форма.СтруктураРеквизитовФормы.ДлинаИмениГруппы)
					= Форма.СтруктураРеквизитовФормы.мИдГруппы3М3 Тогда
					
					КодПоОКЕИ = СокрЛП(Форма.ТабличныйДокумент.Область("П00003М300002" + _Координаты).Значение);
					Графа3 = Форма.ТабличныйДокумент.Область("П00003М300003" + _Координаты).Значение;
					Графа4 = Форма.ТабличныйДокумент.Область("П00003М300004" + _Координаты).Значение;
					Если КодПоОКЕИ = "831" Тогда
						Форма.ТабличныйДокумент.Область("П00003М300005" + _Координаты).Значение = Графа3 * Графа4 / 100;
					ИначеЕсли КодПоОКЕИ = "251" Тогда
						Форма.ТабличныйДокумент.Область("П00003М300005" + _Координаты).Значение = Графа3 * Графа4;
					КонецЕсли;
					...
					Для каждого Обл Из Форма.ТабличныйДокумент.Области Цикл
						Если СтрНачинаетсяС(Обл.Имя, "П00003М305000" + ПодстрокаПоиска) Тогда
				...
				_Индекс = "_" + СтрРазделить(ОбластьИмя, "_")[1];
				Стр030 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма,
					Форма.ТабличныйДокумент.Области["П00003М103000" + _Индекс].Значение);
				Стр031 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма,
					Форма.ТабличныйДокумент.Области["П00003М103100" + _Индекс].Значение);
				СумАкц = Стр030 - Стр031;
				Форма.ТабличныйДокумент.Области["П00003М103200" + _Индекс].Значение = Макс(СумАкц, 0);
				Форма.ТабличныйДокумент.Области["П00003М103300" + _Индекс].Значение = -Мин(СумАкц, 0);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" ИЛИ ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел4" Тогда
		...
		Р2_ИтогСтр070 = 0;
		ДеревоМнЧ = ВеткаР2.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппы2М1];
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			Р2_ИтогСтр070 = Р2_ИтогСтр070 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				Форма, СтрокаМнЧ.Данные["П00002М107000"]);
		КонецЦикла;
		
		ДеревоР2 = Неопределено;
		
		Р1_Стр030 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма,
		...
		ВеткаР1.Данные.Вставить("П000010003003", Р1_Стр030);
		
		ДеревоР4 = ПолучитьИзВременногоХранилища(
			Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел4"]);
		
		Р4_ИтогСтр030 = 0;
		Р1_КБК = СокрЛП(ВеткаР1.Данные["П000010002003"]);
		Для каждого ВеткаР4 Из ДеревоР4.Строки Цикл
			ДеревоМнЧ_4М1 = ВеткаР4.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппы4М1];
			Для каждого СтрокаМнЧ_4М1 Из ДеревоМнЧ_4М1.Строки Цикл
				Если Р1_КБК = СокрЛП(СтрокаМнЧ_4М1.Данные["П00004М101000"]) Тогда
					ДеревоМнЧ_4М2 = СтрокаМнЧ_4М1.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппы4М2];
					Для каждого СтрокаМнЧ_4М2 Из ДеревоМнЧ_4М2.Строки Цикл
						Р4_ИтогСтр030 = Р4_ИтогСтр030 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
							Форма, СтрокаМнЧ_4М2.Данные["П00004М203000"]);
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		
		ВеткаР1.Данные.Вставить("П000010006003", Р4_ИтогСтр030);
		
		ВеткаР1.Данные.Вставить("П000010007003", Макс(0, Р1_Стр030 - Р4_ИтогСтр030));
		
		ВеткаР1.Данные.Вставить("П000010008003", -Мин(0, Р1_Стр030 - Р4_ИтогСтр030));
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		ДеревоМнЧ1 = ДеревоДанных.Строки[0].ДанныеМногострочныхЧастей[ИмяГруппыМнЧ1];
		...
		Для каждого СтрокаМнЧ1 Из ДеревоМнЧ1.Строки Цикл
			...
			Для каждого СтрокаМнЧ2 Из ДеревоМнЧ2.Строки Цикл
				...
				Для каждого СтрокаМнЧ3 Из ДеревоМнЧ3.Строки Цикл
					...
					КодПоОКЕИ = СокрЛП(СтрокаМнЧ3.Данные["П00003М300002"]);
					Графа3 = СтрокаМнЧ3.Данные["П00003М300003"];
					Графа4 = СтрокаМнЧ3.Данные["П00003М300004"];
					Если КодПоОКЕИ = "831" Тогда
						СтрокаМнЧ3.Данные.Вставить("П00003М300005", Графа3 * Графа4 / 100);
					ИначеЕсли КодПоОКЕИ = "251" Тогда
						СтрокаМнЧ3.Данные.Вставить("П00003М300005", Графа3 * Графа4);
					КонецЕсли;
					
					Стр030 = Стр030 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
						Форма, СтрокаМнЧ3.Данные["П00003М305000"]);
			...
			Стр031 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма,
				СтрокаМнЧ1.Данные["П00003М103100"]);
			
			СумАкц = Стр030 - Стр031;
			СтрокаМнЧ1.Данные.Вставить("П00003М103200", Макс(СумАкц, 0));
			СтрокаМнЧ1.Данные.Вставить("П00003М103300", -Мин(СумАкц, 0));
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П00003М300005");
	Структ.Вставить("П00003М103000");
	Структ.Вставить("П00003М103200");
	Структ.Вставить("П00003М103300");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Раздел3", Структ);
	
	// Раздел 1.
	Структ = Новый Структура;
	Структ.Вставить("П000010003003");
	Структ.Вставить("П000010006003");
	Структ.Вставить("П000010007003");
	Структ.Вставить("П000010008003");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ТекущаяДатаСеанса() < '20260701' Тогда
		
		СообщитьВТС("До 30.06.2026 (включительно) представление декларации доступно только по форме, утвержденной "
			+ "приказом ФНС России от 27.09.2017 № СА-7-3/765@ (в ред. приказа ФНС России от 12.04.2023 № ЕД-7-3/238@)",
	...
	Если Окр(ДанныеРаздела1["П000010007003"]) <> 0 И Окр(ДанныеРаздела1["П000010008003"]) <> 0 Тогда
		СообщитьВТС("Раздел 1: не допускается одновременное заполнение строк 070 и 080",
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел2") Тогда
		...
		ДеревоМнЧ = ДеревоРаздела.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы2М1];
		
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			
			НомСтрокиМнЧ = Формат(ДеревоМнЧ.Строки.Индекс(СтрокаМнЧ) + 1, "ЧГ=");
			
			ИмяПок = "П00002М101000";
			ЗначПок = СокрЛП(СтрокаМнЧ.Данные[ИмяПок]);
			ДлинаПок = СтрДлина(ЗначПок);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " не задан отчетный год",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ДлинаПок <> 4 ИЛИ НЕ СтроковыеФункцииКлиентсервер.ТолькоЦифрыВСтроке(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " задан отчетный год, не состоящий из 4 цифр",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00002М102000";
			ЗначПок = СокрЛП(СтрокаМнЧ.Данные[ИмяПок]);
			ДлинаПок = СтрДлина(ЗначПок);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " не задан налоговый период Заявления",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ДлинаПок <> 2 Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " задан налоговый период Заявления, не состоящий из 2 символов",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00002М103000";
			ЗначПок = СокрЛП(СтрокаМнЧ.Данные[ИмяПок]);
			ДлинаПок = СтрДлина(ЗначПок);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " не задан номер Заявления",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ДлинаПок <> 16 ИЛИ НЕ СтроковыеФункцииКлиентсервер.ТолькоЦифрыВСтроке(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " задан номер Заявления, не состоящий из 16 цифр",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ИмяПок = "П00002М104000";
			ЗначПок = СтрокаМнЧ.Данные[ИмяПок];
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ + " не задана дата Заявления",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(ЗначПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрокиМнЧ
					+ " задана дата Заявления, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
					"Раздел2", ИмяПок + "_" + НомСтрокиМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Р2_ИтогСтр070 = Р2_ИтогСтр070 + Окр(СтрокаМнЧ.Данные["П00002М107000"]);
	...
	Если Окр(ДанныеРаздела1["П000010003103"]) = 0 И Окр(ДанныеРаздела1["П000010003203"]) = 0
		...
		И Окр(ДанныеРаздела1["П000010003503"]) = 0 И Р2_ИтогСтр070 = 0 И Окр(ДанныеРаздела1["П000010003003"]) <> 0 Тогда
		
		СообщитьВТС("Раздел 1: строка 030 не должна быть заполнена, если не заполнена хотя бы одна из строк "
			+ "031, 032, 033, 034, 035 Раздела 1 и строк 070 Раздела 2", "Раздел1", "П000010003003", "1");
		СтатусПроверки = Ложь;
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел3") Тогда
		...
		Для каждого СтрокаМнЧ1 Из ДеревоМнЧ1.Строки Цикл
			...
			ДлинаКБК = СтрДлина(КБК);
			Если ДлинаКБК <> 20 Тогда
				СообщитьВТС("Раздел 3: в строке " + НомСтрокиМнЧ1 + " не" + ?(ДлинаКБК = 0, "", "верно")
					+ " задан код бюджетной классификации", "Раздел3", ИмяПокКБК + "_" + НомСтрокиМнЧ1, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если Окр(СтрокаМнЧ1.Данные["П00003М103200"]) <> 0 И Окр(СтрокаМнЧ1.Данные["П00003М103300"]) <> 0 Тогда
				СообщитьВТС("Раздел 3: в строке " + НомСтрокиМнЧ1 + " не допускается одновременное заполнение строк 032 и 033",
					"Раздел3", "П00003М103300_" + НомСтрокиМнЧ1, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ2 Из ДеревоМнЧ2.Строки Цикл
				...
				ИначеЕсли СтрДлина(ОКСМ) <> 3
					...
					СообщитьВТС("Раздел 3: в строке " + НомСтрокиМнЧ1 + " задан код страны по ОКСМ, не состоящий из трех цифр",
						"Раздел3", ИмяПокОКСМ + Постфикс2, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого СтрокаМнЧ3 Из ДеревоМнЧ3.Строки Цикл
					...
					Если НЕ ЗначениеЗаполнено(КодВидаПТ) Тогда
						СообщитьВТС("Раздел 3: в строке " + НомСтрокиМнЧ1 + " не задан код вида подакцизного товара",
					...
					КодОКЕИ = СокрЛП(СтрокаМнЧ3.Данные["П00003М300002"]);
					Если НЕ ЗначениеЗаполнено(КодОКЕИ) Тогда
						СообщитьВТС("Раздел 3: в строке " + НомСтрокиМнЧ1 + " не задан код единицы измерения по ОКЕИ",
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел4") Тогда
		...
		ДеревоМнЧ1 = ДеревоРаздела.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы4М1];
		
		ТаблКБК = Новый ТаблицаЗначений;
		ТаблКБК.Колонки.Добавить("КБК");
		ТаблКБК.Колонки.Добавить("НомерСтроки");
		ТаблКБК.Колонки.Добавить("СчетчикСтрок");
		
		Для каждого СтрокаМнЧ1 Из ДеревоМнЧ1.Строки Цикл
			
			НомСтрокиМнЧ1 = Формат(ДеревоМнЧ1.Строки.Индекс(СтрокаМнЧ1) + 1, "ЧГ=");
			
			ИмяПокКБК = "П00004М101000";
			КБК = СокрЛП(СтрокаМнЧ1.Данные[ИмяПокКБК]);
			
			ДлинаКБК = СтрДлина(КБК);
			Если ДлинаКБК <> 20 Тогда
				СообщитьВТС("Раздел 4: в строке " + НомСтрокиМнЧ1 + " не" + ?(ДлинаКБК = 0, "", "верно")
					+ " задан код бюджетной классификации", "Раздел4", ИмяПокКБК + "_" + НомСтрокиМнЧ1, НомСтраницы);
				СтатусПроверки = Ложь;
			Иначе
				НоваяСтрокаТаблКБК = ТаблКБК.Добавить();
				НоваяСтрокаТаблКБК.КБК = КБК;
				НоваяСтрокаТаблКБК.НомерСтроки = НомСтрокиМнЧ1;
				НоваяСтрокаТаблКБК.СчетчикСтрок = 1;
			КонецЕсли;
			
			ДеревоМнЧ2 = СтрокаМнЧ1.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы4М2];
			...
			Для каждого СтрокаМнЧ2 Из ДеревоМнЧ2.Строки Цикл
				...
				ИмяПок = "П00004М202000";
				ЗначПок = СокрЛП(СтрокаМнЧ2.Данные[ИмяПок]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 4: в строке " + НомСтрокиМнЧ1 + " не задан номер документа",
						"Раздел4", ИмяПок + Постфикс2, НомСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ (СтрДлина(ЗначПок) = 13 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначПок)) Тогда
					СообщитьВТС("Раздел 4: в строке " + НомСтрокиМнЧ1 + " задан номер документа, не состоящий из 13 цифр",
						"Раздел4", ИмяПок + Постфикс2, НомСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли СтроковыеФункцииКлиентСервер.СтрокаВДату(СтрШаблон("%1/%2/%3", Лев(ЗначПок, 2), Сред(ЗначПок, 3, 2),
					Сред(ЗначПок, 5, 2))) = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата")) Тогда
					СообщитьВТС("Раздел 4: в строке " + НомСтрокиМнЧ1 + " задан номер документа, первые 6 цифр которого не "
						+ "соответствуют шаблону ДДММГГ (ДД – день, ММ – месяц, ГГ – год)", "Раздел4", ИмяПок + Постфикс2, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
		...
		ТаблКБК_ = ТаблКБК.Скопировать();
		ТаблКБК_.Свернуть("КБК", "СчетчикСтрок");
		
		ПредставленияСтрокСНеуникальнымиКБК = Новый СписокЗначений;
		
		Если ТаблКБК.Количество() <> ТаблКБК_.Количество() Тогда
			Для каждого СтрокаТабл_ Из ТаблКБК_ Цикл
				Если СтрокаТабл_.СчетчикСтрок > 1 Тогда
					СтрокиТабл = ТаблКБК.НайтиСтроки(Новый Структура("КБК", СтрокаТабл_.КБК));
					Для каждого СтрокаТабл Из СтрокиТабл Цикл
						ПредставленияСтрокСНеуникальнымиКБК.Добавить(СтрокаТабл.НомерСтроки);
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Если ПредставленияСтрокСНеуникальнымиКБК.Количество() > 0 Тогда
			
			ПредставленияСтрокСНеуникальнымиКБК.СортироватьПоЗначению();
			
			СообщитьВТС("Раздел 4: заданы неуникальные значения КБК в строках "
				+ СтрСоединить(ПредставленияСтрокСНеуникальнымиКБК.ВыгрузитьЗначения(), ", "),
				"Раздел4", "П00004М101000_" + ПредставленияСтрокСНеуникальнымиКБК[0].Значение, НомСтраницы);
			СтатусПроверки = Ложь;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	Для каждого Эл Из СумНалПУ.Строки Цикл
		...
		Если Эл.Код = "СумНалПУД" И Окр(ДанныеРаздела1["П000010007003"]) = 0 И Окр(ДанныеРаздела1["П000010008003"]) = 0 Тогда
			Эл.Обязательность = "О";
		КонецЕсли;
	...
	ДеревоМнЧ = ДеревоДанныхРаздел2.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы2М1];
	Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
	...
	ВывестиПоказательВXML(ЭкзСумАкцПТПУ.Строки[0], ДеревоДанныхРаздел3.Строки[0].Данные["П000030001003"]); // ОКТМО
	...
	Для каждого СтрокаМнЧ1 Из ДеревоМнЧ1.Строки Цикл
		...
		Для каждого Эл Из ЭкзСумАкцПТПУ_КБК.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				...
				Если Эл.Код = "СумАкцПУД"
					И Окр(СтрокаМнЧ1.Данные["П00003М103200"]) = 0 И Окр(СтрокаМнЧ1.Данные["П00003М103300"]) = 0 Тогда
					Эл.Обязательность = "О";
				КонецЕсли;
	...
	//
	// Многострочная часть уровня 0.
	СвДокПостТов = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СвДокПостТов");
	ДеревоМнЧ1 = ДеревоДанныхРаздел4.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы4М1];
	Для каждого СтрокаМнЧ1 Из ДеревоМнЧ1.Строки Цикл
		ЭкзСвДокПостТов = СкопироватьУзел(УзелРодитель, СвДокПостТов, УзелРодитель.Строки.Количество() - 1);
		Для каждого Эл Из ЭкзСвДокПостТов.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, СтрокаМнЧ1.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		...
		// Многострочная часть уровня 1.
		СвДокСумНал = ПолучитьПодчиненныйЭлемент(ЭкзСвДокПостТов, "СвДокСумНал");
		ДеревоМнЧ2 = СтрокаМнЧ1.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы4М2];
		Для каждого СтрокаМнЧ2 Из ДеревоМнЧ2.Строки Цикл
			ЗаполнитьДаннымиУзел(СтрокаМнЧ2.Данные,
				СкопироватьУзел(ЭкзСвДокПостТов, СвДокСумНал, ЭкзСвДокПостТов.Строки.Количество() - 1));
		КонецЦикла;
		
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(СвДокПостТов);
...
Процедура УстановитьФорматВыводаНаСервере(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		ИменаНемногострочныхПоказателей.Добавить("П000010008003");
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		ИменаМногострочныхПоказателей.Добавить("П00002М105000");
		ИменаМногострочныхПоказателей.Добавить("П00002М106000");
		ИменаМногострочныхПоказателей.Добавить("П00002М107000");
		ИменаМногострочныхПоказателей.Добавить("П00002М108000");
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		ИменаМногострочныхПоказателей.Добавить("П00003М103000");
		ИменаМногострочныхПоказателей.Добавить("П00003М103100");
		ИменаМногострочныхПоказателей.Добавить("П00003М103200");
		ИменаМногострочныхПоказателей.Добавить("П00003М103300");
		ИменаМногострочныхПоказателей.Добавить("П00003М305000");
	КонецЕсли;
	...
	Если ИмяРаздела = "Раздел1" Тогда
		Если РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
			ЭтотОбъект, ТабличныйДокумент.Области["П000010008003"].Значение) = 0 Тогда
			ФорматируемаяОбласть = ТабличныйДокумент.Области["П000010007003"];
			ФорматируемаяОбласть.Формат = СтрЗаменить(ФорматируемаяОбласть.Формат, "ЧН=-", "ЧН=0");
		Иначе
			ФорматируемаяОбласть = ТабличныйДокумент.Области["П000010007003"];
			ФорматируемаяОбласть.Формат = СтрЗаменить(ФорматируемаяОбласть.Формат, "ЧН=0", "ЧН=-");
		КонецЕсли;
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П000010006003");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П000010007003");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П000010008003");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00002М105000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00002М106000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00002М107000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00002М108000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00003М103000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00003М103100");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00003М103200");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00003М103300");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00003М305000");
	ИменаЦелочисленныхСуммовыхПоказателей.Добавить("П00004М203000");
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел1" ИЛИ ИмяТекРаздела = "Раздел2"
		ИЛИ ИмяТекРаздела = "Раздел3" ИЛИ ИмяТекРаздела = "Раздел4" Тогда
		...
		Для каждого Показатель Из СтраницаРаздела.Данные Цикл
			Если Показатель.Ключ = "П000010007003"
				И Окр(СтраницаРаздела.Данные["П000010007003"]) = 0 И Окр(СтраницаРаздела.Данные["П000010008003"]) = 0 Тогда
				
				ЗаполнитьПоказательМЧБ(МЧБ, "0--------------", Показатель.Ключ + "_");
...
Процедура ВывестиНаПечатьЛистМногоуровневогоРаздела(МЧБ, Ветка, ИмяГруппы, СтруктураПараметров, Знач КоординатыРодителя = "_")
	...
	Для Каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
		...
		Если ТребуетсяВыводЛиста
			...
			"ПечатныйБланк2026Кв2_" + СтруктураПараметровРаздела["ИмяСтраницы"]);
		...
		Для каждого Показатель Из СтрокаМнЧ.Данные Цикл
			...
			Если Показатель.Ключ = "П00004М203000" И Окр(СтрокаМнЧ.Данные["П00004М203000"]) = 0 Тогда
				
				ЗаполнитьПоказательМЧБ(МЧБ, "0--------------", НачалоИмениПоказателяНаМЧБ);
...
Процедура ЗаполнитьПоказательМЧБ(МЧБ, ЗначениеПоказателя, НачалоИмениПоказателяНаМЧБ)
	...
	Если ТипЗнч(ЗначениеПоказателя) = Тип("Строка")
		...
		Если НачалоИмениПоказателяНаМЧБ = "П000010001003_" ИЛИ НачалоИмениПоказателяНаМЧБ = "П000030001003_" Тогда
			...
			Если ОбластьМЧБ.Текст = "." Тогда
				Если ЗначениеЗаполнено(ЗначЦелойЧасти) Тогда
					...
					Пока ОбластьДробнойЧастиМЧБ <> Неопределено Цикл
						ОбластьДробнойЧастиМЧБ.Текст = ?(НомДробнойЧасти > ДлинаДробнойЧасти,
							"0", Сред(ЗначДробнойЧасти, НомДробнойЧасти, 1));
	...
	Для каждого Раздел Из мСтруктураМногоуровневыхРазделов Цикл
		Если НЕ Раздел.Значение.Свойство("Подчиненность") Тогда
			...
			ИначеЕсли Раздел.Ключ = "Раздел3" Тогда
				...
				ДанныеСтраницыРаздела2.П000030001003 = СокрЛП(ДанныеСтраницыРаздела2.П000030001003);
			КонецЕсли;
...
Процедура Проверить(Команда)
	...
	ПоказатьПредупреждение(, НСтр(
		...
		|от 08.05.2026 № КЧ-1-3/299@, будет реализована
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "Раздел1" ИЛИ Ячейка.Раздел = "Раздел2" ИЛИ Ячейка.Раздел = "Раздел3"
