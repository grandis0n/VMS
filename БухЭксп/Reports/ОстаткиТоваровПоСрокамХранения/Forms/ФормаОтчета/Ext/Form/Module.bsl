...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Отчет.РежимРасшифровки  Тогда
		Отчеты.ОстаткиТоваровПоСрокамХранения.ЗаполнитьИнтервалыПоУмолчанию(Отчет.Интервалы);
	КонецЕсли;
...
Функция ПодготовитьПараметрыОтчетаНаСервере() Экспорт
	...
	ПараметрыОтчета.КлючВарианта                      = КлючТекущегоВарианта;

	Для Каждого Показатель Из МенеджерОтчета.ПолучитьНаборПоказателей() Цикл
		ПараметрыОтчета.Вставить("Показатель" + Показатель, Отчет["Показатель" + Показатель]);
	КонецЦикла;
