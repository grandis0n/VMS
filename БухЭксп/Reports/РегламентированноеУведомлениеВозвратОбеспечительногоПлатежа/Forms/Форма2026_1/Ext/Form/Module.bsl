...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ВозвратОбеспечительногоПлатежа;
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Если ЭтотОбъект["ЭтоЮЛ"] Тогда
		СтрокаСведений = "ИННЮЛ,КППЮЛ,ТелОрганизации,БанкСчетНомер,БанкСчетНаимБанка,БанкСчетБИКБанка";
	...
	Иначе
		СтрокаСведений = "ИННФЛ,ТелДом,БанкСчетНомер,БанкСчетНаимБанка,БанкСчетБИКБанка";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
