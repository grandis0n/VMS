...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	CW_БухгалтерскиеОтчетыМСФО.ИзменитьСхемуДляОтборовПоДопСвойствамСубконто(ПараметрыОтчета, Схема, КомпоновщикНастроек);
	
	ЗначениеПараметра = CW_БухгалтерскийУчетПовтИспМСФО.СчетаВИерархии(ПараметрыОтчета.Счет);
	Если ЗначениеЗаполнено(ПараметрыОтчета.Счет) Тогда
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "Счет", ЗначениеПараметра);
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "НачалоПериода", ЗначениеПараметра);
	ЗначениеПараметра = ?(ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода),
		КонецДня(ПараметрыОтчета.КонецПериода), Дата(3999, 12, 31));
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "КонецПериода", ЗначениеПараметра);
	...
	Если ПараметрыОтчета.Периодичность = 0 Тогда
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "ПС", Символы.ПС);
	...
	Для Каждого ЭлементМакет Из МассивМакетов Цикл
		...
		Схема.Макеты[ЭлементМакет].Макет = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьКопиюОписанияМакета(ОписаниеМакета);
		ОписаниеМакета = Схема.Макеты[ЭлементМакет].Макет;
		
		МассивСтрокДляУдаления = Новый Массив;
		Индекс = 0;
		Для Каждого ЭлементМассива Из ПараметрыОтчета.НаборПоказателей Цикл
			 Если 	Не CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПоказательДоступен(КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора, ЭлементМассива + "ОборотДт") 
		...
		Если КоличествоСтрок > 0 Тогда
			Для Индекс = 0 По 12 Цикл
				...
				ПараметрГраницы = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(ОписаниеМакета[0].Ячейки[Индекс].Оформление.Элементы, "СтильГраницы");
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ПараметрГраницы.ЗначенияВложенныхПараметров, "СтильГраницы.Сверху", ЛинияСплошная);
				ПараметрГраницы = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(ОписаниеМакета[ПоследняяСтрока].Ячейки[Индекс].Оформление.Элементы, "СтильГраницы");
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ПараметрГраницы.ЗначенияВложенныхПараметров, "СтильГраницы.Снизу", ЛинияСплошная);	
			КонецЦикла;
		КонецЕсли;
		
		Для Индекс = 1 По КоличествоСтрок - 1 Цикл
			ОписаниеМакета[Индекс].Ячейки[0].Элементы.Очистить();	
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[0].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[0].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
			ОписаниеМакета[Индекс].Ячейки[1].Элементы.Очистить();
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[1].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[1].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
			ОписаниеМакета[Индекс].Ячейки[2].Элементы.Очистить();
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[2].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[2].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
			ОписаниеМакета[Индекс].Ячейки[3].Элементы.Очистить();
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[3].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[3].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
			Если ЭлементМакет = "ПроводкиЗаголовок" Тогда
				ОписаниеМакета[Индекс].Ячейки[5].Элементы.Очистить();
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[5].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[5].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
				ОписаниеМакета[Индекс].Ячейки[8].Элементы.Очистить();
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[8].Оформление.Элементы, "ОбъединятьПоВертикали", Истина);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(ОписаниеМакета[Индекс].Ячейки[8].Оформление.Элементы, "Расшифровка", Неопределено, Ложь);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Если Не ПараметрыОтчета.ПоказательБУ Тогда
		...
		Для Каждого ЭлементМассива Из ПараметрыОтчета.НаборПоказателей Цикл
			Если ЭлементМассива <> "БУ" И ПараметрыОтчета["Показатель" + ЭлементМассива] Тогда
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаОтборов, ЭлементМассива + "ОборотДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаОтборов, ЭлементМассива + "ОборотКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	МакетПроводки = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Проводки", Истина);
...
Процедура ЗаполнитьНастройкиРасшифровки(Настройки, Объект, ДанныеРасшифровки, ИдентификаторРасшифровки, РеквизитыРасшифровки) Экспорт
	...
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ЗаполнитьНастройкиРасшифровкиПоДаннымСтандартногоОтчета(
		Настройки,
		ДанныеРасшифровки,
...
Процедура АдаптироватьНастройки(Настройки) Экспорт
	...
	CW_БухгалтерскиеОтчетыМСФО.УдалитьАвтоотступИзУсловногоОформления(Настройки.УсловноеОформление);
...
Процедура ИзменитьСхемуДляОтборовПоСубконтоСубсчетов(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	CW_БухгалтерскиеОтчетыМСФО.СписокОтборовПоСубконто(КомпоновщикНастроек.Настройки.Отбор.Элементы, ОтбираемыеВидыСубконто);
	Если ОтбираемыеВидыСубконто.Количество() = 0 Тогда // нет отборов по субконто
		Возврат;
	КонецЕсли;
	
	ПараметрСчет = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(КомпоновщикНастроек, "Счет");
	...
	|ИЗ
	|	ПланСчетов.CW_МСФО.ВидыСубконто КАК ХозрасчетныйВидыСубконто_СчетГруппа
	...
	ИсточникВидовСубконто = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(ПараметрыОтчета.Счет);
	НомерСубконто = 0;
	...
	Для каждого НомерСубконтоСубсчета Из ОтбираемыеВидыСубконто Цикл
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(КомпоновщикНастроек,
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "ВидыСубконтоОтборов", ВидыСубконтоОтборов);
...
Процедура ИзменитьСхемуДляПоказателяКонтроль(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	//	КонецЕсли;
	//	
	//	CW_БухгалтерскиеОтчетыМСФО.ДобавитьСторноДляПоказателяКонтроль(ПараметрыОтчета, ЗапросПакета);
	//
	//КонецЦикла;
	//
