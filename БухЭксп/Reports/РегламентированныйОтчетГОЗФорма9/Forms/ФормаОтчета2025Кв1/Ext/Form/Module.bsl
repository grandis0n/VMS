...
Процедура ПолеТабличногоДокументаФормаОтчетаПриАктивизации(Элемент)
	...
	Если ЗначениеЗаполнено(ИмяГруппы) И СтрНайти(ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, "_") > 0 Тогда
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Инд = 1;
	Для МнгСтр = 1 По 2 Цикл
		ИмяЯчейкиНомерСроки = "П001001000" + МнгСтр + "01_1";
		Если Не ЗначениеЗаполнено(Области[ИмяЯчейкиНомерСроки].Значение) Тогда
			Области[ИмяЯчейкиНомерСроки].Значение = Инд;
		КонецЕсли;
		Инд = Инд + 1;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 9");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	...
	МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_5"));
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверкиВывода = Новый Массив;
		МассивПроверкиВывода.Добавить(ТекущаяОбластьМногострочнойЧасти);
		МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_5"));
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверкиВывода) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 9");
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 9");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
