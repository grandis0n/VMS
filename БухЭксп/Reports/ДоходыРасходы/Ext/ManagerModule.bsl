...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	КонецЕсли;
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ПрочиеДоходыИРасходы", ПараметрыОтчета.ПрочиеДоходыИРасходы);
		...
		Попытка
			Если ТипЗнч(Рисунок.Объект) = Тип("Диаграмма") Тогда
				...
				Для Каждого Серия Из Рисунок.Объект.Серии Цикл
					Если Серия.Текст = "Доходы от продаж (выручка)"
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	ДополнительныеСвойства.Вставить("ПрочиеДоходыИРасходы",              ДанныеОтчета.Объект.ПрочиеДоходыИРасходы);
...
Функция НастройкиОтчетаСохраняемыеВРассылке() Экспорт
	...
	КоллекцияНастроек.Вставить("ПрочиеДоходыИРасходы"             , Ложь);
...
Функция ПустыеПараметрыКомпоновкиОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ПрочиеДоходыИРасходы",         Ложь);
	ПараметрыОтчета.Вставить("ОкруглениеСумм", БыстрыеНастройкиОтчетовСервер.ОкруглениеСуммДоЦелых());
...
Процедура ВывестиПримечания(ПараметрыОтчета, Результат)
	...
	Иначе
		ЗавершениеТаблицы = Результат.ВысотаТаблицы + 2;
	КонецЕсли;
	
	Примечания = ПолучитьМакет("Примечания");
	ОбластьПримечание = Примечания.Область("Примечание");
	
	Результат.ВставитьОбласть(ОбластьПримечание,
		Результат.Область(СтрШаблон("R%1", Формат(ЗавершениеТаблицы, "ЧГ=0"))),
		ТипСмещенияТабличногоДокумента.ПоГоризонтали,
		Истина);
	ЗавершениеПримечания = ЗавершениеТаблицы + ОбластьПримечание.Низ;
	Если ПараметрыОтчета.ПрочиеДоходыИРасходы Тогда

		ОбластьПримечаниеПрочиеДоходыРасходы = Примечания.Область("ПримечаниеПрочиеДоходыРасходы");
	
		Результат.ВставитьОбласть(ОбластьПримечаниеПрочиеДоходыРасходы,
			Результат.Область(СтрШаблон("R%1", Формат(ЗавершениеПримечания, "ЧГ=0"))),
			ТипСмещенияТабличногоДокумента.ПоГоризонтали,
			Истина);
		ЗавершениеПримечания = ЗавершениеПримечания + 2;
	КонецЕсли;
	
	ОбластьПримечаниеНДС = Примечания.Область("ПримечаниеНДС");
	
	Результат.ВставитьОбласть(ОбластьПримечаниеНДС,
		Результат.Область(СтрШаблон("R%1", Формат(ЗавершениеПримечания, "ЧГ=0"))),
		ТипСмещенияТабличногоДокумента.ПоГоризонтали,
		Истина);
