	...
	Пока ДанныеРаздела.Свойство("П000010001001_" + Формат(Инд, "ЧГ=")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДанныеРаздела["П000010001001_" + НомерСтроки]) Тогда
				...
				СтатусПроверки = Ложь;
			ИначеЕсли СтрНайти(ДанныеРаздела["П000010001001_" + НомерСтроки], "Т") > 0 Тогда
				СообщитьВТС("В строке " + НомерСтроки
					+ " раздела 1 литера ""Т"" в номере отметки о регистрации заявления должна быть буквой латинского алфавита",
					"Раздел1", "П000010001001_" + НомерСтроки);
				СтатусПроверки = Ложь;
			КонецЕсли;
