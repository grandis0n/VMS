...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	CW_БухгалтерскиеОтчетыКлиентМСФО.ПередЗакрытием(ЭтотОбъект, Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Trial balance of the account %1%2'"), Отчет.Счет,
...
Процедура ИзменениеСхемыКомпоновкиДанныхНаСервере() Экспорт
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(Отчет.КомпоновщикНастроек, "Счет", Отчет.Счет);
	
	CW_БухгалтерскиеОтчетыМСФО.ОбработатьНаборДанныхСвязаннойИнформации(Схема, "ДанныеОС"     , ПараметрыОС);
	//CW_БухгалтерскиеОтчетыМСФО.ОбработатьНаборДанныхСвязаннойИнформации(Схема, "ДанныеНМА"    , ПараметрыНМА);
	//CW_БухгалтерскиеОтчетыМСФО.ОбработатьНаборДанныхСвязаннойИнформации(Схема, "ДанныеФизЛица", ПараметрыФизЛица);
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка", ЗаполнятьКоллекцию) И ЗаполнятьКоллекцию Тогда
		...
		Для Индекс = 1 По КоличествоСубконто Цикл
			...
			Если ИспользоватьОднуНоменклатурнуюГруппу 
				И ВидСубконто = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.НоменклатурныеГруппы Тогда
				Продолжить;
			ИначеЕсли НЕ ВестиУчетПоДоговорам 
				И ВидСубконто = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ВидСубконто = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.СтатьиЗатрат Тогда
