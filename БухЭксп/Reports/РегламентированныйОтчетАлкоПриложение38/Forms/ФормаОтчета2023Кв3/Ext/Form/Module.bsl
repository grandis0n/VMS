...
Процедура ПослеВыбораИсточникаДанныхДляОблака()
	...
	ВыполнитьДлительнуюОперацию_НаСервере(
		ПредопределенноеЗначение("Перечисление.ВидыДлительныхОперацийРегламентированныхОтчетов.ЗаполнениеПоданнымИБ"));
...
Процедура ПослеПолученияДанныхДляОблака()
	...
	ЗаполнениеДанныхВРегистрах();
	ВидДлительнойОперации = ПредопределенноеЗначение(
		"Перечисление.ВидыДлительныхОперацийРегламентированныхОтчетов.ЗаполнениеПоданнымИБ");
	Попытка
		РегламентированнаяОтчетностьАЛКОКлиент.ПослеВыполненияДлительнойОперацииАЛКО(ЭтотОбъект);
	Исключение
		// Если в учетной базе отчет не заполняется, то вызов генерирует исключение из-за отсутствия данных в структуре.
	КонецПопытки;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнениеДанныхВРегистрах()
	
	АдресВоВременномХранилище = СтруктураРеквизитовФормы.АдресВоВременномХранилище;
	СтруктураРезультата = ПолучитьИзвременногоХранилища(АдресВоВременномХранилище); // Структура
	
	Если ТипЗнч(СтруктураРезультата) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из СтруктураРезультата.Регистры Цикл
		ИмяРегистра = КлючИЗначение.Ключ;
		Для Каждого Элемент Из КлючИЗначение.Значение Цикл
			МетодЗаполнения = Элемент.Метод;
			ПараметрыМетода = Элемент.Параметры;
			
			мСохраненныйДок = СтруктураРеквизитовФормы.мСохраненныйДок;
			
			Если МетодЗаполнения = "ДобавитьТаблицуЗначенийВРегистрСведенийАлко" Тогда
				РегламентированнаяОтчетностьАЛКО.ДобавитьТаблицуЗначенийВРегистрСведенийАлко(мСохраненныйДок,
					ПараметрыМетода.ИмяФормыОбъекта, ИмяРегистра, ПараметрыМетода.ИндексСтраницыРегистра,
					ПараметрыМетода.ТаблицаЗначений, ПараметрыМетода.КоличествоТекСтрок);
			Иначе
				РегламентированнаяОтчетностьАЛКО.ДобавитьВРегистрЖурнала(мСохраненныйДок, ИмяРегистра,
					ПараметрыМетода.ИндексСтраницыРегистра, ПараметрыМетода.ИндексСтроки, ПараметрыМетода.Действие,
					ПараметрыМетода.Ресурсы);
			КонецЕсли;
			
		КонецЦикла;
		
	КонецЦикла;
	СтруктураРезультата.Удалить("Регистры");
	ПоместитьВоВременноеХранилище(СтруктураРезультата, АдресВоВременномХранилище);
