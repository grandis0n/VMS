...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		ИначеЕсли Раздел = "2" Тогда
			...
			ИсхКолвоКолонокРазделов = 23;
			
			ДобавляемыйРеквизит = Новый РеквизитФормы(
				ИмяДобавляемогоРеквизита + "01", ТипКолонкиСтрока, ИмяДобавляемогоРеквизита);
			ДобавляемыеРеквизиты.Добавить(ДобавляемыйРеквизит);
			
			Для Инд = 16 По ИсхКолвоКолонокРазделов Цикл
				
				Если Инд = 16 Тогда
					ТипКолонки = ТипКолонкиСтрока;
				ИначеЕсли Инд > 16 И Инд < 20 Тогда
					ТипКолонки = ТипКолонкиЧисло15_2;
...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010010001;П0010010002;П0010010003;П0010020001;П0010020002;П0010020003", ";", Истина);
КонецФункции
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если ИмяГруппы = "П0010010001" Или ИмяГруппы = "П0010010002" Или ИмяГруппы = "П0010010003" Тогда
		ГраФаКон  = 16;
		ГраФаКон2 = 23;
	ИначеЕсли ИмяГруппы = "П0010020001" Или ИмяГруппы = "П0010020002" Или ИмяГруппы = "П0010020003" Тогда
		ГраФаКон  = 23;
		ГраФаКон2 = 16;
	КонецЕсли;
...
Процедура УдалитьДополнительнуюСтрокуИзГруппы(Форма, СпециальноеДобавление, ИмяКолонкиНомера, ТекущаяСтраница, ИмяГруппы, НомУдаляемойСтроки, НомерСтрокиТаблицы)
	...
	ИначеЕсли ИмяГруппы = "П0010020001" Тогда
		ИмяГруппыСинхронизируемой1 = "П0010010001";
		ИмяЯчейкиСинхронизируемой1 = "П001001" + Сред(ИмяГруппыСинхронизируемой1, 8, 4) + "01_" + Сред(ИмяЯчейки, 15);
	ИначеЕсли ИмяГруппы = "П0010010002" Тогда
		ИмяГруппыСинхронизируемой1 = "П0010020002";
		ИмяЯчейкиСинхронизируемой1 = "П001002" + Сред(ИмяГруппыСинхронизируемой1, 8, 4) + "01_" + Сред(ИмяЯчейки, 15);
	ИначеЕсли ИмяГруппы = "П0010020002" Тогда
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Раздел = 1 По 2 Цикл
		...
		Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
			...
			Иначе
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		...
		ИначеЕсли Раздел = 2 Тогда
			...
			Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_1")) Тогда
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			КонецЕсли;
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_1"));
			
			Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_2")) Тогда
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			КонецЕсли;
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_2"));
			
			Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_3")) Тогда
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			КонецЕсли;
