...
Функция РелизованаПоддержкаЗаполненияВОблачнойОтчетности(ИДРедакцииОтчета) Экспорт
	...
	МассивПоддерживаемых.Добавить("ФормаОтчета2011Кв3");
...
Функция ИмяМакетаСтруктурыВыгрузки(ИмяФормы, СтруктураРеквизитовФормы)
	...
	ДатаДействия = СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания;
	
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда
		ИмяМакетаСхемы = "СхемаВыгрузки510";
		Если ДействуетФормат511(ДатаДействия) Тогда
			ИмяМакетаСхемы = "СхемаВыгрузки511";
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ДатаДействия >= '2013-01-01' Тогда
...
Процедура УстановитьРазмерПоляПояснений(МакетОтчета, ПараметрыПодготовки)
	...
	Если ДействуетФормат511(ПараметрыПодготовки.ОтчетнаяДата) Тогда
		// С версии формата 5.11 для номера пояснения задан размер 10 символов.
...
Функция ДействуетФормат511(КонецПериодаОтчета) Экспорт
