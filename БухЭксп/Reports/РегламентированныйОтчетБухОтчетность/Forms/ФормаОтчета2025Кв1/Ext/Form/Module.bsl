...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДействуетФормат511(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		Элементы.ИмяФайлаАудиторскогоЗаключения.Заголовок = "Аудиторский отчет";
		ТекстПодсказки = Новый ФорматированнаяСтрока(
			"Для предоставления аудиторского отчета после сдачи отчетности воспользуйтесь отправкой ",
			Новый ФорматированнаяСтрока("доп. документов в ФНС", , , , "ДопДокументы"));
		Элементы.ИмяФайлаАудиторскогоЗаключения.РасширеннаяПодсказка.Заголовок = ТекстПодсказки;
		Элементы.ПодсказкаПоАудиторскомуЗаключению.Заголовок = ТекстПодсказки;
	КонецЕсли;
...
&НаСервере
Функция ДействуетФормат511(КонецПериодаОтчета)
	
	Возврат ОбъектОтчета().ДействуетФормат511(КонецПериодаОтчета);
