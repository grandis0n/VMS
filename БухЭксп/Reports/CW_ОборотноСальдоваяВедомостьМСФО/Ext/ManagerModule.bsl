...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода));
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	CW_БухгалтерскиеОтчетыМСФО.УстановитьМинимальнуюШиринуПоказателей(ПараметрыАлгоритма.Схема, ПолучитьНаборПоказателей());
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "СчетаИсключенныеИзЗапросаПоСчетам", СчетаИсключенныеИзЗапросаПоСчетам);
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "ВыводитьЗабалансовыеСчета", ПараметрыОтчета.ВыводитьЗабалансовыеСчета);
	
	// Формирование структуры отчета.
	ПараметрыАлгоритма.Вставить("СписокСчетовПоСубсчетам", CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьСписокСчетовПоСубсчетам(ПараметрыАлгоритма.НастройкаГруппировки));
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	МакетГруппировкиСчет = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Счет");
				...
				Для ИндексЭлемента = 0 По КоличествоПоказателей - 1 Цикл
					...
					Для Каждого Ячейка Из СтрокаМакета.Ячейки Цикл
						...
						Для Каждого Элемент Из Ячейка.Элементы Цикл
							...
							Если НЕ (Не истина И ИндексЭлемента = (КоличествоПоказателей - 1)
								И истина) И ПараметрРезультата <> Неопределено Тогда
		...
		Если СтрокаГруппировкиПоСчету <> Неопределено Тогда
			...
			Для Индекс = 1 По КоличествоСубконто Цикл
				...
				Если ИндексСубконто = "0" Тогда
					...
					СтрокаГруппировки = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйСтрокаГруппировки();
			...
			Если НужноПоменятьНумерациюСубконто Тогда
				...
				CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ПоменятьНумерациюСубконтоВТабличнойЧасти("ТаблицаГруппировка", Настройки, Счет.Родитель, Счет);
				
			КонецЕсли;
			
			CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ПоменятьНумерациюСубконтоВОтборе(Настройки, Счет, КартаВидовСубконто);
...
//
// Параметры:
//  Правила - ТаблицаЗначений с правилами расшифровки этого отчета см. CW_БухгалтерскиеОтчетыРасшифровкаМСФО.НовыйПравилаРасшифровки().
//
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	
	Правило = Правила.Добавить();
	Правило.Отчет = "CW_ОборотноСальдоваяВедомостьПоСчетуМСФО";
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	Правило.ШаблонПредставления = НСтр("ru = 'Trial balance of the account [Счет]'");
	...
	Правило = Правила.Добавить();
	Правило.Отчет = "CW_АнализСчетаМСФО";
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	Правило.ШаблонПредставления = НСтр("ru = 'Account analysis [Счет]'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "CW_ОборотыСчетаМСФО";
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Периодичность", "", 9);
	Правило.ШаблонПредставления = НСтр("ru = 'Account turnover [Счет] by month'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "CW_ОборотыСчетаМСФО";
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	CW_БухгалтерскиеОтчетыРасшифровкаМСФО.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Периодичность", "", 6);
	Правило.ШаблонПредставления = НСтр("ru = 'Account turnover [Счет] by day'");
...
Процедура СформироватьСтруктуруОтчета(КомпоновщикНастроек, ПараметрыОтчета, ПараметрыАлгоритма)
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(КомпоновщикНастроек.Настройки.Выбор, "Счет");
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаЭлементовОтбора, "ПараметрыДанных.ПоСубсчетам", Истина);
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаЭлементовОтбора, "СистемныеПоля.УровеньВГруппировке", 1);
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаЭлементовОтбора, "Счет", ПараметрыАлгоритма.СписокСчетовПоСубсчетам, ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии, ПараметрыАлгоритма.СписокСчетовПоСубсчетам.Количество() > 0);
	
	// Устанавливаем отбор по признаку Забалансовый.
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппировкаПоБалансовымСчетам.Отбор, "Счет.Забалансовый", Ложь);
	
	// Отключим вывод отбора.
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(ГруппировкаПоБалансовымСчетам, "ВыводитьОтбор", ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
	...
	// Устанавливаем отбор по признаку Забалансовый.
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппировкаПоЗабалансовымСчетам.Отбор, "Счет.Забалансовый", Истина);
	
	// Отключим вывод отбора и итогов.
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(ГруппировкаПоЗабалансовымСчетам, "ВыводитьОтбор", ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(ГруппировкаПоЗабалансовымСчетам, "РасположениеОбщихИтогов", РасположениеИтоговКомпоновкиДанных.Нет);
	
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "ПоСубсчетам", ПараметрыОтчета.ПоСубсчетам);
	...
	Если ПараметрыОтчета.ПоказательВалютнаяСумма Тогда
		...
		Для Каждого Группировка Из ГруппировкиПоСчетам Цикл
			...
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Структура.Отбор, "Счет.Валютный", Истина);
			// Отключим вывод отбора и итогов.
	...
	Если ПараметрыАлгоритма.ЕстьДетализацияПоСубконто Тогда
		...
		Для Каждого Группировка Из ГруппировкиПоСчетам Цикл
			...
			Для Индекс = 1 По ПараметрыАлгоритма.КоличествоСубконтоДетализации Цикл 
				...
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Структура.Отбор, ПолеГруппировки.Поле,"", ВидСравненияКомпоновкиДанных.Заполнено);
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(Структура, "ВыводитьОтбор", ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
				...
				Если ПараметрыОтчета.ПоказательВалютнаяСумма Тогда
					...
					// Устанавливаем отбор по признаку Валютный.
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппировкаПоВалюте.Отбор, "Счет.Валютный", Истина);
					
					// Отключим вывод отбора и итогов.
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(ГруппировкаПоВалюте, "ВыводитьОтбор", ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
...
Функция СчетаССовпадающимНаборомСубконто(СчетаГруппы, СубконтоДетализацииПоСчетам);
	...
	|ИЗ
	|	ПланСчетов.CW_МСФО.ВидыСубконто КАК ХозрасчетныйВидыСубконто
	|ГДЕ
...
Процедура ДобавитьОтборПоРесурсам(ПараметрыОтчета, КомпоновщикНастроек)
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "СальдоНаНачалоПериода.КонтрольНачальныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.Равно);
	
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "СальдоНаНачалоПериода.КонтрольНачальныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.Равно);
	
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "СальдоНаКонецПериода.КонтрольКонечныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.Равно);
	
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "СальдоНаКонецПериода.КонтрольКонечныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.Равно);
			
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "ОборотыЗаПериод.КонтрольОборотДт", 0, ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Группа, "ОборотыЗаПериод.КонтрольОборотКт", 0, ВидСравненияКомпоновкиДанных.НеЗаполнено);
...
Процедура ДобавитьОформлениеИтогов(ПараметрыОтчета, КомпоновщикНастроек)
	...
	// Устанавливаем параметр оформления.
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(УсловноеОформление.Оформление, "Шрифт", ЖирныйШрифт);
КонецПроцедуры
...
Процедура СоздатьКолонкиОтчета(КолонкиСтруктурыОтчета, ПараметрыОтчета)
	...
	Если КоличествоПоказателей > 1 Тогда
		...
		Для Каждого ИмяПоказателя Из ПараметрыОтчета.НаборПоказателей Цикл
			Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаПоказатели, "Показатели." + ИмяПоказателя);
			КонецЕсли;
		КонецЦикла;
	...
	ГруппаСальдоНаНачало.Заголовок     = НСтр("ru = 'Balance at beginning of period'");
	...
	ГруппаСальдоНаНачалоДт.Заголовок     = НСтр("ru = 'Debit'");
	...
	ГруппаСальдоНаНачалоКт.Заголовок     = НСтр("ru = 'Credit'");
	...
	ГруппаОбороты.Заголовок     = НСтр("ru = 'Turnover for the period'");
	...
	ГруппаОборотыДт.Заголовок     = НСтр("ru = 'Debit'");
	...
	ГруппаОборотыКт.Заголовок     = НСтр("ru = 'Credit'");
	...
	ГруппаСальдоНаКонец.Заголовок     = НСтр("ru = 'Balance at the end of the period'");
	...
	ГруппаСальдоНаКонецДт.Заголовок     = НСтр("ru = 'Debit'");
	...
	ГруппаСальдоНаКонецКт.Заголовок     = НСтр("ru = 'Credit'");
	...
	Для Каждого ИмяПоказателя Из ПараметрыОтчета.НаборПоказателей Цикл
		Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаСальдоНаНачалоДт, "СальдоНаНачалоПериода." + ИмяПоказателя + "НачальныйОстатокДт");
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаСальдоНаНачалоКт, "СальдоНаНачалоПериода." + ИмяПоказателя + "НачальныйОстатокКт");
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаОборотыДт,        "ОборотыЗаПериод."       + ИмяПоказателя + "ОборотДт");
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаОборотыКт,        "ОборотыЗаПериод."       + ИмяПоказателя + "ОборотКт");
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаСальдоНаКонецДт,  "СальдоНаКонецПериода."  + ИмяПоказателя + "КонечныйОстатокДт");
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(ГруппаСальдоНаКонецКт,  "СальдоНаКонецПериода."  + ИмяПоказателя + "КонечныйОстатокКт");
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьИтогиРазвернутогоСальдо(ПараметрыАлгоритма)
	...
	Для СчетчикСубконто = 1 По 3 Цикл
		...
		Для Каждого СчетСразворотом Из ПараметрыАлгоритма.СчетаСРазворотомПоСубконто[СчетчикСубконто - 1] Цикл
			...
			CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(Схема, "СчетСРазворотомПоСубконто" + + СчетчикСубконто + "_" + СчетчикСчетов, "СчетСРазворотомПоСубконто", СчетСразворотом);
...
Процедура ДоработатьНаборДанных_ПоСубконто(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	ТекстЗапросДетализацииПоСубконто       = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало ЗапросПоСубконто Детализация", "//Конец ЗапросПоСубконто Детализация");
	ТекстУсловиеСчетаДетализацииПоСубконто = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало УсловиеСчета Детализация", "//Конец УсловиеСчета Детализация");
	ТекстСубконтоДетализацииПоСубконто     = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало Субконто Детализация", "//Конец Субконто Детализация");
	...
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("Счет",                 Новый ОписаниеТипов("ПланСчетовСсылка.CW_МСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации",  Новый ОписаниеТипов("СписокЗначений"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации1", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации2", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоДетализации3", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	...
	НастройкаГруппировки = CW_БухгалтерскиеОтчетыМСФО.УникальныеНастройкиОтчетаПоСчетам(ПараметрыОтчета.Группировка);
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		СубконтоДетализации = СтрокиДетализацииСПодходящимиПараметрами[3].СубконтоДетализации;
		...
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СчетДетализации" + ИндексПодзапроса, "СчетДетализации", СубсчетаСписок);
		
		// Добавление и установка значения параметра СубконтоДетализации{Индекс}.
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СубконтоДетализации" + ИндексПодзапроса, "СубконтоДетализации", СубконтоДетализации);
	...
	Если СчетаБезСубконто.Количество() > 0 Тогда
		...
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СчетДетализации_БезСубконто", "СчетДетализации", СубсчетаСписок);
...
Процедура ДоработатьНаборДанных_ПоСубконтоРазвернутое(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	ТекстЗапросПоСубконтоРазвернутое       = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало ЗапросПоСубконто РазвернутоеСальдо", "//Конец ЗапросПоСубконто РазвернутоеСальдо");
	ТекстУсловиеСчетаПоСубконтоРазвернутое = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало УсловиеСчета РазвернутоеСальдо"    , "//Конец УсловиеСчета РазвернутоеСальдо");
	ТекстСубконтоПоСубконтоРазвернутое     = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстПоМаркерам(ТекстЗапроса, "//Начало Субконто РазвернутоеСальдо"        , "//Конец Субконто РазвернутоеСальдо");
	
	// Оставим уникальные строки со счетами.
	НастройкаРазвернутоеСальдо = CW_БухгалтерскиеОтчетыМСФО.УникальныеНастройкиОтчетаПоСчетам(ПараметрыОтчета.РазвернутоеСальдо);
	...
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо1",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо2",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("СубконтоРазвернутоеСальдо3",             Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
	ПараметрыФормируемыхЗапросов.Колонки.Добавить("ПоСубсчетам",                            Новый ОписаниеТипов("Булево"));
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		Для Каждого Счет Из СчетаИтогов Цикл
			...
			CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СчетРазвернутоеСальдоИтоги" + ИндексПодзапроса  + "_" + ИндексСубСчета, "СчетРазвернутоеСальдоИтоги", Счет);
		...
		Если Не ОписаниеПодзапроса.ЕстьРазворотПоСубконтоТекущегоСчета Тогда
			...
			CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СчетРазвернутоеСальдо" + ИндексПодзапроса, "СчетРазвернутоеСальдо", СубсчетаСписок);
			Продолжить;
		КонецЕсли;
		...
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СубконтоРазвернутый" + ИндексПодзапроса, "СубконтоРазвернутый", СубконтоРазвернутоеСальдо);
	...
	Если СчетаДляОтдельногоЗапроса.Количество() > 0 Тогда
		...
		// Установка параметра СчетРазвернутоеСальдо.
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.СкопироватьПараметрСхемыКомпоновкиДанных(ПараметрыАлгоритма.Схема, "СчетРазвернутоеСальдо_БезСубконто", "СчетРазвернутоеСальдо", СписокСчетаБезСубконтоСРазвернутымСальдо);
