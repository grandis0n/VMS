		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если КодНапрТС = "D" Тогда
					// Возможно отсутствие рег. номера таможенной декларации при вывозе. По этой причине удалена указанная
					// в формате проверка на обязательность заполнения рег. номера таможенной декларации при вывозе.
					Если ЗначениеЗаполнено(Ветка_П11000.Данные["П1100006"])
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " неверно задан регистрационный "
						+ "номер таможенной декларации (при вывозе). Возможное количество символов в номере: 23 или от 25 до 29",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
