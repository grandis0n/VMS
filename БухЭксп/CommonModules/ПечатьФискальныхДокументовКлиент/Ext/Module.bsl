...
Функция ВерсииФорматовФД(Знач ВерсияФН, Знач ВерсияККТ) Экспорт
	...
	ВерсияФД = ?(ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияФН, ВерсияККТ) > 0, ВерсияККТ, ВерсияФН);
	
	Возврат НовыйВерсииФорматов(ВерсияККТ, ВерсияФН, ВерсияФД);
КонецФункции
...
Функция НовыйВерсииФорматов(ВерсияККТ = "1.0.0.0", ВерсияФН = "1.0.0.0", ВерсияФД = "1.0.0.0") Экспорт
	СтруктураВерсий = Новый Структура;
	СтруктураВерсий.Вставить("ВерсияККТ", ВерсияККТ);
	СтруктураВерсий.Вставить("ВерсияФН",  ВерсияФН);
	СтруктураВерсий.Вставить("ВерсияФД",  ВерсияФД);
