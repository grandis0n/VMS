...
Функция ПолучитьОтпечаткиСертификатовНаСервере(ПоказыватьОшибку = Ложь) Экспорт
	...
	Если Не ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере() И Не ЭтоМобильныйКлиент Тогда
		Возврат РезультатПолучения;
	КонецЕсли;
	...
	Если ЭтоМобильныйКлиент Тогда
		ДоступныеОтпечаткиСертификатовСервера = МобильныйЭДОСлужебный.ДоступныеОтпечаткиСертификатовСервера();
		Для Каждого ДоступныйОтпечаток Из ДоступныеОтпечаткиСертификатовСервера Цикл
			РезультатПолучения.Отпечатки.Добавить(ДоступныйОтпечаток);
		КонецЦикла;
	КонецЕсли;
