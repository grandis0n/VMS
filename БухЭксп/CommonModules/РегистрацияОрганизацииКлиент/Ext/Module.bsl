...
Процедура ВыборОтсканированногоФайлаНажатие(Форма, ВладелецФайла, ИмяЭлемента) Экспорт
	...
	ПараметрыОткрытия.Вставить("ТолькоПросмотр", Форма.ТолькоПросмотр Или ЗначениеЗаполнено(Форма.ПодписанныеДокументы));
...
// Открывает печатную форму Уведомления о переходе на УСН
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма из которой вызывается открытие печатной формы
//   УведомлениеОПереходеНаУСН - ДокументСсылка.УведомлениеОСпецрежимахНалогообложения - ссылка на уведомление
//
Процедура ОткрытьПечатнуюФормуУведомленияОПереходеНаУСН(Форма, УведомлениеОПереходеНаУСН) Экспорт
	
	ДанныеУведомления = РегистрацияОрганизацииВызовСервера.ДанныеЗаявления(УведомлениеОПереходеНаУСН);
	Если Не ЗначениеЗаполнено(ДанныеУведомления) Тогда
		Возврат;
	КонецЕсли;
	
	Организация = ДанныеУведомления.Организация;
	ИмяОтчета = ДанныеУведомления.ИмяОтчета;
	ИмяФормы = ДанныеУведомления.ИмяФормы;
	
	ДоступнаПечатьPDF417 = РегистрацияОрганизацииВызовСервера.ДоступнаПечатьPDF417(ИмяОтчета, ИмяФормы);
	
	Если ДоступнаПечатьPDF417 Тогда
	...
	Иначе
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", УведомлениеОПереходеНаУСН);
		ПараметрыФормы.Вставить("Организация", Организация);
		
		ИмяФормыУведомления = СтрШаблон("Отчет.%1.Форма.%2", ИмяОтчета, ИмяФормы);
		
		ФормаОтчета = ПолучитьФорму(ИмяФормыУведомления, ПараметрыФормы, Форма, Организация);
		ОткрытьФормуПредварительногоПросмотраЗаявленияБезДвумерногоКода(
			ФормаОтчета, ИмяОтчета, Нстр("ru = 'Уведомление о переходе на УСН.pdf'"));
	КонецЕсли;
