...
Процедура СкорректироватьПроводкиВозвратТоваровПрибылиУбыткиПрошлыхЛет(Проводки, ДатаРеализации, ДатаВозврата, Организация) Экспорт
	...
	НельзяПеренестиНаТекущийПериод = НалоговыйУчет.НельзяПеренестиНаТекущийПериод(Организация, ДатаРеализации, ДатаВозврата, Истина);
	
	Если НельзяПеренестиНаТекущийПериод Тогда
		СкорректироватьВыручкуСебестоимостьВПрошломПериоде(Проводки, ДатаВозврата, ДатаРеализации, Организация);
	Иначе
...
Функция НельзяПеренестиНаТекущийПериод(Организация, ДатаОперации, ДатаИсправления, ЭтоВозврат = Ложь) Экспорт
	
	СтавкиНалогаРеализация    = РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.ПолучитьПоследнее(ДатаОперации, Новый Структура("Организация", Организация));
	СтавкиНалогаКорректировка = РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.ПолучитьПоследнее(ДатаИсправления, Новый Структура("Организация", Организация));
	СтавкаНалогаУвеличилась  = (СтавкиНалогаРеализация.СтавкаФБ + СтавкиНалогаРеализация.СтавкаСубъектРФ) < (СтавкиНалогаКорректировка.СтавкаФБ + СтавкиНалогаКорректировка.СтавкаСубъектРФ);
	
	Если ЭтоВозврат Тогда
		Если НЕ СтавкиНалогаКорректировка.УчитыватьИзменениеСтавкиПриВозврате Тогда
			СтавкаНалогаУвеличилась  = Ложь;
		КонецЕсли;
	ИначеЕсли ДатаИсправления < НастройкиУчетаНалогаНаПрибыльФормыКлиентСервер.ДатаИзмененияСт54НК_425ФЗ_28_11_2025()
		И НЕ СтавкиНалогаКорректировка.УчитыватьИзменениеСтавкиПриИсправлении Тогда
		СтавкаНалогаУвеличилась  = Ложь;
	КонецЕсли;
		
	Возврат СтавкаНалогаУвеличилась;
...
Процедура СкорректироватьВыручкуСебестоимостьВПрошломПериоде(Проводки, ТекущийПериод, ПрошлыйПериод, Организация)
	
	СчетаВыручкиСебестоимости = СчетаВыручкиСебестоимости(ТекущийПериод , Организация);
	...
	Для Каждого Проводка Из Проводки Цикл
		...
		НоваяПроводка.Период = ПрошлыйПериод;
