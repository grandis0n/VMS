...
Процедура ПолучитьПравоВыполненияЗадачи(ЕстьПраво, Задача) Экспорт
	...
	ИначеЕсли ТипЗнч(Задача) = Тип("ПеречислениеСсылка.РазовыеЗадачи") Тогда
		
		Если Задача = Перечисления.РазовыеЗадачи.ПродлитьСертификатЭлектроннойПодписи Тогда
			
			ЕстьПраво = ПравоДоступа("Изменение", Метаданные.Справочники.УчетныеЗаписиДокументооборота);
...
Процедура ПолучитьДанныеЗадачиБухгалтера(ДанныеЗадачиБухгалтера, Организация) Экспорт
	...
	|ВЫБРАТЬ
	|	ВТ_АктуальныеЗадачи.Срок,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.РазовыеЗадачи),
	|	ВТ_АктуальныеЗадачи.Наименование,
	|	1,
	|	РазовыеЗадачи.Ссылка,
	|	ЛОЖЬ
	|ИЗ
	|	ВТ_АктуальныеЗадачи КАК ВТ_АктуальныеЗадачи
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Перечисление.РазовыеЗадачи КАК РазовыеЗадачи
	|		ПО ВТ_АктуальныеЗадачи.Правило = РазовыеЗадачи.Ссылка
	|ИТОГИ
