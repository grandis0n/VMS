...
Процедура СоздатьСлужебногоПользователяОповещенийЭДО() Экспорт
	...
	Свойства.Имя = ИмяСлужебногоПользователяОповещенийЭДО();
	СсылкаНового = Справочники.Пользователи.ПолучитьСсылку(
		Новый УникальныйИдентификатор("946135e8-a9a7-405f-a5df-19e734c712c7"));
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось получить служебного пользователя оповещений ЭДО по причине:'")
...
Процедура ОповеститьВсехПользователейОНовыхДокументах()
	...
	УникальныйИдентификаторБота = ?(ЗначениеЗаполнено(Бот), Бот.УникальныйИдентификатор, Неопределено);
	
	Запрос.УстановитьПараметр("EDIEvents", УникальныйИдентификаторБота);
...
Функция ИдентификаторБотаВСистемеВзаимодействия() 
	...
	Если ИдентификаторБота = Неопределено Тогда
		Попытка
			...
			ПользовательСистемыВзаимодействия.ПолноеИмя = НСтр("ru='Бот 1С-ЭДО'");
