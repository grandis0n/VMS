...
Процедура ЗаполнитьДанныеКомпонентаПанельКомандВходящегоДокумента(ДанныеКомпонента, СвойстваДокумента) Экспорт
	...
	Если СвойстваДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.Черновик Тогда
		ДанныеКомпонента.ТипДокументаЧерновика = ЧерновикиЭДО.ТипДокументаЧерновика(СвойстваДокумента.ЧерновикВладелец);
	КонецЕсли;
...
Функция НовыеДанныеКомпонентаПанельКомандВходящегоДокумента() Экспорт
	...
	ДанныеКомпонента.Вставить("ТипДокументаЧерновика", Перечисления.ТипыДокументовЭДО.ПустаяСсылка());
	Возврат ДанныеКомпонента;
...
Процедура ЗаполнитьДанныеКомпонентаПанельСостояния(ДанныеКомпонента, СвойстваДокумента) Экспорт
	...
	Если Не РезультатыЗапросов[0].Пустой() Тогда
		...
		Если СводноеСостояние = Неопределено Тогда
			СтрокаСостояния = СостоянияДокументовСКомментарием.Найти(СвойстваДокумента.Ссылка, "ЭлектронныйДокумент");
			СводноеСостояние = СтрокаСостояния.Состояние;
		Иначе
...
Процедура ЗаполнитьДанныеКомпонентаПанельОтраженияВУчете(ДанныеКомпонента, СвойстваДокумента,
	...
	ДанныеКомпонента.ЭтоФормализованныйДокумент = ЗначениеЗаполнено(СвойстваДокумента.ФорматОсновногоТитула);
	...
	Если СвойстваДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
		И ДанныеКомпонента.ЭтоФормализованныйДокумент Тогда
...
Функция НовыеДанныеКомпонентаПанельОтраженияВУчете() Экспорт
	...
	ДанныеКомпонента.Вставить("ЭтоВнутреннийВидДокумента", Ложь);
	ДанныеКомпонента.Вставить("ЭтоПрикладнойВидДокумента", Ложь);
	ДанныеКомпонента.Вставить("ЭтоФормализованныйДокумент", Ложь);
	Возврат ДанныеКомпонента;
...
Процедура ЗаполнитьДанныеКомпонентаПанельКарточкиПоПараметрам(ДанныеКомпонента, СвойстваДокумента,
	...
	ПараметрыПредставления.ПредставлениеЧерновика = СвойстваДокумента.ПредставлениеЧерновика;
