...
Процедура ПриОпределенииОписанияДействия(ОписаниеДействия, Событие) Экспорт
	...
	ИначеЕсли ТипЗнч(Событие.Правило) = Тип("ПеречислениеСсылка.РазовыеЗадачи") Тогда
		ОписаниеДействия = ВыполнениеЗадачБухгалтера.ОписаниеДействияРазовыеЗадачи(Событие);
	Иначе
...
Процедура ПриПолученииДанныхДляУплатыИмущественныхНалогов(Платежи, ПараметрыЗадачи, ДанныеЕНП, ГруппироватьПоНалоговойБазе) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Платеж.Наименование = Строка(Платеж.Налог);
		Если ГруппироватьПоНалоговойБазе И ИмяЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиНалогНаИмущество() Тогда
...
Процедура ПриОпределенииПризнакаОперацияНачисленияВзносовВыполнена(ОперацияВыполнена, Организация, ПериодСобытия, ДокументЗарплаты, НесколькоДокументов, ИмяДокумента) Экспорт
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрацияОблагаемойБазыРуководителя.Ссылка,
	|	РегистрацияОблагаемойБазыРуководителя.Проведен
	|ИЗ
	|	Документ.РегистрацияОблагаемойБазыРуководителя КАК РегистрацияОблагаемойБазыРуководителя
	|ГДЕ
	|	&Документ = ""РегистрацияОблагаемойБазыРуководителя""
	|	И НЕ РегистрацияОблагаемойБазыРуководителя.ПометкаУдаления
	|	И РегистрацияОблагаемойБазыРуководителя.Организация = &Организация
	|	И РегистрацияОблагаемойБазыРуководителя.ПериодРегистрации МЕЖДУ НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ) И КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Проведен УБЫВ";
...
Процедура УстановитьДополнительныеСвойстваОплатыАУСН(ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СписаниеСРасчетногоСчета")
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("ПрименяетсяАУСН", Истина);
...
Процедура ЗарегистрироватьИсполнениеЗадачиПриЗаписиПлатежа(РеквизитыПлатежа,
	...
	ВыполнениеЗадачБухгалтераБП.ОбновитьСтатусЗадачиАУСН(РеквизитыПлатежа, ДополнительныеСвойства);
