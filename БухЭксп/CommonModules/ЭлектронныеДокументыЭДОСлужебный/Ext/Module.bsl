...
Процедура ПродолжитьВыполнениеДействийПоЭДО(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Аннулировать) Тогда
		...
		Если ЗначениеЗаполнено(РезультатДействий.ОшибкиФормирования) Тогда
			ОбщегоНазначенияБЭД.ЗакончитьЗамер("Переформировать", ОписаниеЗамера, Обработано,
				НСтр("ru = 'Ошибки при аннулировании документа'"));
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьДействиеСформироватьПоОбъектамУчета(ПараметрыВыполнения, РезультатДействий)
	...
	ЕстьДополнительныеПараметрыДействия = ЗначениеЗаполнено(ПараметрыВыполнения.ДополнительныеПараметрыДействий);
	...
	Для Каждого ПараметрыФормирования Из НаборПараметровФормирования Цикл
		...
		Черновик = Неопределено;
		Если ЕстьДополнительныеПараметрыДействия Тогда
			Черновик = ПараметрыВыполнения.ДополнительныеПараметрыДействий[ПараметрыФормирования.ОбъектУчета].Черновик;
		КонецЕсли;
		...
		Попытка
			ДокументОбъект = СоздатьИсходящийДокумент(ПараметрыФормирования.НастройкиДокумента, ОписанияСообщений,
				...
				ПараметрыФормирования.ИдентификаторыОснований, Черновик);
...
//  ИдентификаторыОснований - Массив из см. НовыйЭлементКоллекцииИдентификаторыОснованияДокумента
//  Черновик - Неопределено
//           - ДокументСсылка.ЧерновикЭлектронногоДокументаЭДО
//
// Возвращаемое значение:
//  ДокументОбъект.ЭлектронныйДокументИсходящийЭДО
//
Функция СоздатьИсходящийДокумент(НастройкиОтправки, ОписанияСообщений, КонтекстДиагностики,
	СвязующийОбъектУчета = Неопределено, ОбъектыУчета = Неопределено, ИдентификаторыОснований = Неопределено,
	Черновик = Неопределено) Экспорт
	...
	ДокументОбъект = НовыйИсходящийДокументОбъект(НастройкиОтправки, ОписанияСообщений, ИдентификаторСвязи,
		ИдентификаторыОснований, Черновик);
...
Функция НовыйИсходящийДокументОбъект(НастройкиОтправки, ОписанияСообщений, ИдентификаторСвязи,
	ИдентификаторыОснований = Неопределено, Черновик = Неопределено)
...
Процедура ЗаполнитьИсходящийДокументОбъект(ДокументОбъект, НастройкиОтправки, ОписаниеСообщения,
	ИдентификаторСвязи = "", ИдентификаторыОснований = Неопределено, Черновик = Неопределено)
	...
	Если ЗначениеЗаполнено(Черновик) Тогда
		ДокументОбъект.ЧерновикОснование = Черновик;
		ДокументОбъект.ИдентификаторЧерновика = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Черновик, "Идентификатор");
	КонецЕсли;
...
Функция СоздатьСообщение(ОписаниеСообщения, ЭлектронныйДокумент, НастройкиДокумента) Экспорт
	...
	СервисEDIСлужебный.ЗафиксироватьСведенияОФайлахСообщенийЭДО(СообщениеОбъект, ОписаниеСообщения, НастройкиДокумента);
...
Функция СформироватьИсправление(ЭлектронныйДокумент, ПараметрыВыполнения, РезультатДействий)
	...
	ВидИсправляемогоДокумента =
		ЭлектронныеДокументыЭДО.СвойстваДокумента(ЭлектронныйДокумент, "ВидДокумента").ВидДокумента;
	Если НаборПараметровФормирования.Количество() > 1 Тогда
		Для Каждого ПараметрыФормированияДляПроверки Из НаборПараметровФормирования Цикл
			Для Каждого ОписаниеСообщения Из ПараметрыФормированияДляПроверки.ОписаниеСообщения Цикл
				Если ОписаниеСообщения.ВидСообщения = ВидИсправляемогоДокумента Тогда
					ПараметрыФормирования = ПараметрыФормированияДляПроверки;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВыполнитьДействиеСформироватьИзвещение(ПараметрыВыполнения, РезультатДействий)
	...
	Если РезультатЗапроса = Неопределено
		Или РезультатЗапроса.Пустой() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеДанныхУчастниковЭДО(ДанныеУчастниковЭДО, ТекстОшибки = "")
	...
	Если Не ЗначениеЗаполнено(ДанныеУчастниковЭДО.Контрагент) Тогда
		...
		Если ЗначениеЗаполнено(Приглашение) Тогда
			...
			ИначеЕсли НастройкиЭДО.СоздаватьКонтрагентовАвтоматически()
				И НЕ ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(Приглашение.ИдентификаторКонтрагента) Тогда
				
				РеквизитыКонтрагента = ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента();
				РеквизитыКонтрагента.Наименование = Приглашение.КонтрагентНаименование;
				РеквизитыКонтрагента.ИНН = Приглашение.КонтрагентИНН;
				РеквизитыКонтрагента.КПП = Приглашение.КонтрагентКПП;
				ВидКонтрагента = ОбменСКонтрагентамиКлиентСервер.ВидКонтрагента(Приглашение.КонтрагентИНН, Приглашение.ТипПользователя);
				РеквизитыКонтрагента.ВидКонтрагента = ВидКонтрагента;
...
Функция ПриглашениеПоИдентификаторамОрганизацииКонтрагента(ИдентификаторОрганизации, ИдентификаторКонтрагента)
	...
	|	ПриглашенияКОбменуЭлектроннымиДокументами.ТипПользователя
	|ИЗ
	|	РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами КАК ПриглашенияКОбменуЭлектроннымиДокументами
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		Приглашение = Новый Структура("Ключ,КонтрагентИНН,КонтрагентКПП,КонтрагентНаименование,ИдентификаторОрганизации,ИдентификаторКонтрагента,ТипПользователя");
...
Функция ВозобновитьДокумент(ЭлектронныйДокумент, Действие, КонтекстДиагностики, Комментарий = "")
	...
	Попытка
		...
		ПараметрыДокумента = РезультатыЗапроса[3].Выбрать();
		ПараметрыДокумента.Следующий();
...
Функция Аннулировать(Сообщение, КонтекстДиагностики, Комментарий = "")
	...
	Попытка
		...
		НетУведомленияОбУточнении = СостоянияСообщений.Найти(Перечисления.ТипыЭлементовРегламентаЭДО.УОУ,
			"ТипЭлементаРегламента") = Неопределено
			И СостоянияСообщений.Найти(Перечисления.ТипыЭлементовРегламентаЭДО.ПОА_УОУ,
			"ТипЭлементаРегламента") = Неопределено;
		
		ЭтоПовторноеАннулирование = СостоянияСообщений.Найти(Перечисления.ТипыЭлементовРегламентаЭДО.ПОА,
			"ТипЭлементаРегламента") <> Неопределено;
		Если ЭтоПовторноеАннулирование И НетУведомленияОбУточнении Тогда
		...
		ИначеЕсли НетУведомленияОбУточнении Тогда
...
Функция ДанныеСообщенияДляИнтерактивногоПодписания(ПараметрыПодписания, ДвоичныеДанные)
	...
	ДанныеСообщения.Представление.Вставить("ИмяФайла", ПараметрыПодписания.ПолноеИмяФайла);
...
Функция ЗаполнитьПодписантовИДоверенностиПоСертификату(ДанныеДляЗаполнения,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
			...
			МодульПодсистемы.ПриЗаполненииОписанияФайла(Выборка, ОписаниеФайла);
		КонецЕсли;
...
Функция ТекстЗапросаДанныхДляПодготовкиКОтправке(ЭтоВходящийЭДО, ОтборПоДокументамПакета = Ложь)
	ТекстЗапроса = 
		...
		|	ИдентификаторыОснованийДокументаЭДО.ИдентификаторСвязи КАК ИдентификаторСвязи,
		|	ИдентификаторыОснованийДокументаЭДО.ИдентификаторДокументооборота КАК ИдентификаторДокументооборота
		|ИЗ
	...
	Если Не ЭтоВходящийЭДО Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, """"" КАК ИдентификаторЧерновика",
			"ДокументЭДО.ИдентификаторЧерновика КАК ИдентификаторЧерновика");
	КонецЕсли;
...
Процедура ЗаполнитьДанныеОбъектаДляПодготовкиКОтправке(ДанныеОбъекта, СвойстваДокумента, СвойстваСообщения, ТипСообщения, ФайлыСообщения, ИдентификаторыОснований, КонтекстДиагностики)
	...
	ДанныеОбъекта.ИдентификаторЧерновика = СвойстваДокумента.ИдентификаторЧерновика;
		...
		Если ЗначениеЗаполнено(Содержание.Отправитель.ИНН) Тогда
			...
			Иначе
				СоздаватьКонтрагентовАвтоматическиБЭД = НастройкиЭДО.СоздаватьКонтрагентовАвтоматически()
					И НЕ ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(ПараметрыПриглашения.ИдентификаторКонтрагента);
			...
			Если Не ЗначениеЗаполнено(КонтрагентИзСодержания)
				...
				РеквизитыКонтрагента = ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента();
				РеквизитыКонтрагента.ИНН = Содержание.Отправитель.ИНН;
				РеквизитыКонтрагента.КПП = Содержание.Отправитель.КПП;
				РеквизитыКонтрагента.Наименование = Содержание.Отправитель.Наименование;
...
Функция ЗагрузитьСообщениеДокумента(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики)
	...
	ПараметрыЧерновика = ПараметрыЗагрузки.ПараметрыЧерновикаДокумента;
	...
	Если ПараметрыЗагрузки.ЭтоНовыйДокумент Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыЧерновика) Тогда
			СоздатьЧерновикДокументаЭДО(ДокументОбъект, ПараметрыЧерновика);
		КонецЕсли;
	Иначе
	...
	Иначе
		...
		ПриЗагрузкеСообщенияУжеСозданногоДокумента(СообщениеОбъект);
	КонецЕсли;
...
// Параметры:
//  ДокументОбъект - ДокументОбъект.ЭлектронныйДокументВходящийЭДО
//  ПараметрыЧерновика - См. ЧерновикиЭДО.НовыеПараметрыЧерновикаДокументаДляЗагрузки
Процедура СоздатьЧерновикДокументаЭДО(ДокументОбъект, ПараметрыЧерновика)
	
	ПараметрыЧерновика.ЭлектронныйДокументИсточник = ДокументОбъект.Ссылка;
	ЧерновикОбъект = ЧерновикиЭДО.СоздатьЧерновикДокументаЭДО(ПараметрыЧерновика);
	
	ДокументОбъект.ЧерновикВладелец = ЧерновикОбъект.Ссылка;
	
	СвойстваДокументаДляПредставленияЧерновика =
		ЧерновикиЭДОКлиентСервер.НовыеСвойстваДокументаДляПолученияПредставленияЧерновика();
	СвойстваДокументаДляПредставленияЧерновика.ВидЧерновика = ЧерновикОбъект.ВидЧерновика;
	СвойстваДокументаДляПредставленияЧерновика.НомерДокумента = ЧерновикОбъект.Номер;
	СвойстваДокументаДляПредставленияЧерновика.ДатаДокумента = ЧерновикОбъект.Дата;
	
	ДокументОбъект.ПредставлениеЧерновика = ЧерновикиЭДО.ПредставлениеЧерновика(
		СвойстваДокументаДляПредставленияЧерновика);
		
	ДокументОбъект.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.Черновик);
...
// Параметры:
//  СообщениеОбъект - ДокументОбъект.СообщениеЭДО
Процедура ПриЗагрузкеСообщенияУжеСозданногоДокумента(СообщениеОбъект)
	
	ЧерновикиЭДО.ПриЗагрузкеСообщенияУжеСозданногоДокумента(СообщениеОбъект);
...
//  * ДанныеУчастниковЭДО - см. НовыеДанныеУчастниковЭДО
//  * ПараметрыЧерновикаДокумента - Неопределено
//                                - См. ЧерновикиЭДО.НовыеПараметрыЧерновикаДокументаДляЗагрузки
//
Функция ПараметрыЗагрузкиСообщения(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики) Экспорт
	...
	Результат.Вставить("ПараметрыЧерновикаДокумента", Неопределено);
	...
	Если ДанныеОбъекта.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя
		...
		ЧерновикиЭДО.ЗаполнитьПараметрыЧерновикаДокумента(Результат.ПараметрыЧерновикаДокумента, ДанныеОбъекта);
...
Функция ДатаДокументаИзНаименованияФайла(ИмяФайла)
	...
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{2}\/\d{2}\/\d{4}"); // DD/MM/YYYY
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("(?<!\d)\d{1,2}[\s_]+[A-Za-zА-Яа-яЁё]+[\s_]+20\d{2}"); // DD MM YYYY 22 апреля 2025, 22_апреля_2025
...
Функция ТекстЗапросаПараметровОбновленияСостоянияДокумента(ЭтоВходящийЭДО)
	...
	Если ЭтоВходящийЭДО Тогда
		ТекстыЗапросов.Добавить(
			...
			|	ЭлектронныйДокументВходящийЭДО.ФорматОсновногоТитула КАК ФорматОсновногоТитула,
			|	ЧерновикЭлектронногоДокументаЭДО.Обработан КАК ЧерновикОбработан
			|ИЗ
			|	Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧерновикЭлектронногоДокументаЭДО КАК ЧерновикЭлектронногоДокументаЭДО
			|		ПО ЭлектронныйДокументВходящийЭДО.ЧерновикВладелец = ЧерновикЭлектронногоДокументаЭДО.Ссылка
			|ГДЕ
...
// Возвращаемое значение:
//  Булево
Функция ОбновитьСостояниеДокументаПоДействию(ЭлектронныйДокумент, Действие, КонтекстДиагностики, Комментарий = "") Экспорт
	...
	СостоянияСообщений = НовыйСостоянияСообщений();
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатыЗапроса[0].Выгрузить(), СостоянияСообщений);
	...
	ПараметрыДокумента = РезультатыЗапроса[2].Выбрать();
	ПараметрыДокумента.Следующий();
	
	СостояниеДокумента = УстановитьСостояниеДокументаПриИзменении(ПараметрыДокумента, СостоянияСообщений,
		ДатаИзменения, КонтекстДиагностики);

	СвойстваДляЗаписиДействия = Новый Структура;
	СвойстваДляЗаписиДействия.Вставить("Ссылка", ЭлектронныйДокумент);
	СвойстваДляЗаписиДействия.Вставить("Ответственный", Пользователи.ТекущийПользователь());
		
	РегистрыСведений.ЖурналДействийПоЭДО.ЗаписатьДействие(Действие, СвойстваДляЗаписиДействия, СостояниеДокумента,
		ДатаИзменения, СвойстваСообщения, Комментарий);
...
Функция ДоступныеДействияПоЭДО(СостояниеДокумента, СвойстваДокумента) Экспорт
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтзывом
		Или СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент Тогда
...
Функция ДействиеАннулироватьДоступно(СвойстваДокумента)
	...
	Если ЗначениеЗаполнено(ВидДокумента) Тогда
		...
		Возврат ТипДокумента <> Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность
			И ТипДокумента <> Перечисления.ТипыДокументовЭДО.Черновик;
	КонецЕсли;
