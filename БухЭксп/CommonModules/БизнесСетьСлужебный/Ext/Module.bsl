...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр(
		"ru = 'Устанавливает активность и расписание обновления идентификаторов 1С:Бизнес-сеть'")
	
КонецПроцедуры

// Обработчик обновления.
// Устанавливает расписание регламентного задания ОбновлениеИдентификаторовБизнесСети случайным образом 
...
Процедура УстановитьРасписаниеОбновленияИдентификаторовКонтрагентов() Экспорт
	...
	МетаданныеЗадания = Метаданные.РегламентныеЗадания.ОбновлениеИдентификаторовБизнесСети;
	...
	Использование = БизнесСеть.ИспользоватьОбменБизнесСеть();
...
// * Ссылка        - ОпределяемыйТип.ОрганизацияКонтрагентEDI - Ссылка на организацию или контрагента
// * Идентификатор - Строка - Идентификатор организации в сервисе 1С:Бизнес-сеть
// * ИНН           - Строка - ИНН контрагента
// * КПП           - Строка - КПП контрагента
// * Найден        - Булево - Признак успешного поиска 
Функция НоваяТаблицаПоиска() Экспорт
	
	Результат = Новый ТаблицаЗначений;
	МетаданныеРегистра = Метаданные.РегистрыСведений.КонтрагентыБизнесСеть;
	
	Результат.Колонки.Добавить("Ссылка"        , Метаданные.ОпределяемыеТипы.ОрганизацияКонтрагентEDI.Тип);
	...
	Результат.Колонки.Добавить("Закеширован"   , Новый ОписаниеТипов("Булево"));
...
// 
// Параметры:
//  ТаблицаПоискаКонтрагентов - См. БизнесСетьСлужебный.НоваяТаблицаПоиска
...
// Обработчик оповещения подсистем внешних библиотек о регистрации контрагентов
// 
// Параметры:
//  Контрагенты - См. БизнесСетьСлужебный.НовыеЗарегистрированныеКонтрагенты
Процедура ПриРегистрацииКонтрагентов(Знач Контрагенты) Экспорт
	...
	ИнтеграцияПодсистемБЭД.ПриРегистрацииКонтрагентов(Контрагенты);
	
КонецПроцедуры

// Обработчик оповещения подсистем внешних библиотек об изменении идентификаторов контрагентов
// 
// Параметры:
//  Контрагенты - См. БизнесСетьСлужебный.НовыеИзмененныеКонтрагенты
Процедура ПриИзмененииИдентификаторовКонтрагентов(Знач Контрагенты) Экспорт
	...
	ИнтеграцияПодсистемБЭД.ПриИзмененииИдентификаторовКонтрагентов(Контрагенты);
	
КонецПроцедуры

// Обработчик оповещения подсистем внешних библиотек об изменении идентификаторов организаций
// 
// Параметры:
//  Организации - См. БизнесСетьСлужебный.НовыеИзмененныеОрганизации
Процедура ПриИзмененииИдентификаторовОрганизаций(Знач Организации) Экспорт
	...
	ИнтеграцияПодсистемБЭД.ПриИзмененииИдентификаторовОрганизаций(Организации);
...
//   * Идентификатор - Строка                        - Идентификатор контрагента в сервисе 1С:Бизнес-сеть
//   * ИНН           - Строка                        - ИНН контрагента
//   * КПП           - Строка                        - КПП контрагента
Функция НовыеЗарегистрированныеКонтрагенты() Экспорт
	...
	Результат.Колонки.Добавить("Идентификатор" , МетаданныеРегистра.Ресурсы.Идентификатор.Тип);
	Результат.Колонки.Добавить("ИНН"           , МетаданныеРегистра.Реквизиты.ИНН.Тип);
	Результат.Колонки.Добавить("КПП"           , МетаданныеРегистра.Реквизиты.КПП.Тип);
...
//  ТаблицаЗначений:
//   * Контрагент          - ОпределяемыйТип.КонтрагентБЭД - Ссылка на контрагента
//   * СтарыйИдентификатор - Строка - Идентификатор контрагента в сервисе 1С:Бизнес-сеть до изменения
//   * СтарыйИНН           - Строка - ИНН контрагента до изменения
//   * СтарыйКПП           - Строка - КПП контрагента до изменения
//   * НовыйИдентификатор  - Строка - Идентификатор контрагента в сервисе 1С:Бизнес-сеть после изменения
//   * НовыйИНН            - Строка - ИНН контрагента после изменения
//   * НовыйКПП            - Строка - КПП контрагента после изменения
Функция НовыеИзмененныеКонтрагенты() Экспорт
	
	Результат          = Новый ТаблицаЗначений;
	МетаданныеРегистра = Метаданные.РегистрыСведений.КонтрагентыБизнесСеть;
	
	Результат.Колонки.Добавить("Контрагент"          , МетаданныеРегистра.Измерения.Контрагент.Тип);
	Результат.Колонки.Добавить("СтарыйИдентификатор" , МетаданныеРегистра.Ресурсы.Идентификатор.Тип);
	Результат.Колонки.Добавить("СтарыйИНН"           , МетаданныеРегистра.Реквизиты.ИНН.Тип);
	Результат.Колонки.Добавить("СтарыйКПП"           , МетаданныеРегистра.Реквизиты.КПП.Тип);
	Результат.Колонки.Добавить("НовыйИдентификатор"  , МетаданныеРегистра.Ресурсы.Идентификатор.Тип);
	Результат.Колонки.Добавить("НовыйИНН"            , МетаданныеРегистра.Реквизиты.ИНН.Тип);
	Результат.Колонки.Добавить("НовыйКПП"            , МетаданныеРегистра.Реквизиты.КПП.Тип);
...
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Организация         - ОпределяемыйТип.ОрганизацияКонтрагентEDI - Ссылка на организацию
//   * СтарыйИдентификатор - Строка - Идентификатор контрагента в сервисе 1С:Бизнес-сеть до изменения
//   * СтарыйИНН           - Строка - ИНН контрагента до изменения
//   * СтарыйКПП           - Строка - КПП контрагента до изменения
//   * НовыйИдентификатор  - Строка - Идентификатор контрагента в сервисе 1С:Бизнес-сеть после изменения
//   * НовыйИНН            - Строка - ИНН контрагента после изменения
//   * НовыйКПП            - Строка - КПП контрагента после изменения
Функция НовыеИзмененныеОрганизации() Экспорт
	
	Результат          = Новый ТаблицаЗначений;
	МетаданныеРегистра = Метаданные.РегистрыСведений.ОрганизацииБизнесСеть;
	
	Результат.Колонки.Добавить("Организация"         , МетаданныеРегистра.Измерения.Организация.Тип);
	Результат.Колонки.Добавить("СтарыйИдентификатор" , МетаданныеРегистра.Ресурсы.Идентификатор.Тип);
	Результат.Колонки.Добавить("СтарыйИНН"           , МетаданныеРегистра.Реквизиты.ИНН.Тип);
	Результат.Колонки.Добавить("СтарыйКПП"           , МетаданныеРегистра.Реквизиты.КПП.Тип);
	Результат.Колонки.Добавить("НовыйИдентификатор"  , МетаданныеРегистра.Ресурсы.Идентификатор.Тип);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьРасписаниеОбновленияИдентификаторовКонтрагентов", Ложь);
...
#Область ПоискКонтрагентовВСервисе
