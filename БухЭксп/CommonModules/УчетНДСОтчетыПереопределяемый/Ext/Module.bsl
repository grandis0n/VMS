...
// В процедуре производится заполнение документа "Отражение ДОПП в декларации по косвенным налогам"
//
// Параметры:
//  СтруктураПараметров - Структура - Организация, дата, ссылка.
//  ДанныеДляЗаполнения - Структура - две таблицы: СведенияИзДокументовОПредстоящейПоставке, ОбеспечительныеПлатежи.
//  АдресХранилища - Строка - адрес временного хранилища для помещения результата выполнения.
//
Процедура ЗаполнитьДокументОтражениеДОППВДекларацииПоКосвеннымНалогам(СтруктураПараметров, ДанныеДляЗаполнения, АдресХранилища) Экспорт
	
	СведенияИзДокументовОПредстоящейПоставке = ДанныеДляЗаполнения.СведенияИзДокументовОПредстоящейПоставке;
	ОбеспечительныеПлатежи = ДанныеДляЗаполнения.ОбеспечительныеПлатежи;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация",              СтруктураПараметров.Организация);
	Запрос.УстановитьПараметр("ДатаТекущегоДокумента",    КонецДня(СтруктураПараметров.Дата));
	Запрос.УстановитьПараметр("ТекущийДокумент",          СтруктураПараметров.Ссылка);
	Запрос.УстановитьПараметр("КонецПериода",             КонецДня(КонецМесяца(СтруктураПараметров.Дата)));
	Запрос.УстановитьПараметр("НачалоПериода",            НачалоДня(НачалоМесяца(СтруктураПараметров.Дата)));
	Запрос.УстановитьПараметр("НачалоПредыдущегоПериода", НачалоДня(НачалоМесяца(ДобавитьМесяц(СтруктураПараметров.Дата, -1))));
	Запрос.УстановитьПараметр("НулевойКБК",               НулевойКБКПриВвозеТоваров());
	Запрос.УстановитьПараметр("КБКНДС",                   КБКНДСПриВвозеТоваров());
	
	Запрос.Текст = ТекстЗапросаОтражениеДОППВДекларацииПоКосвеннымНалогам();
	...
	ВыборкаПоДОПП = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаПоДОПП.Следующий() Цикл
		
		КлючСтроки = Новый УникальныйИдентификатор;
		
		СведенияИзДокументов = СведенияИзДокументовОПредстоящейПоставке.Добавить();
		ЗаполнитьЗначенияСвойств(СведенияИзДокументов, ВыборкаПоДОПП);
		СведенияИзДокументов.КлючСтроки = КлючСтроки;
		СведенияИзДокументов.РегистрационныйНомерДОПП = СокрЛП(ВыборкаПоДОПП.РегистрационныйНомерДОПП);
		
		ВыборкаПоКБК = ВыборкаПоДОПП.Выбрать();
		
		Если ВыборкаПоКБК.Количество() > 1 Тогда
			ОбеспечительныйПлатежПредставление = СтрШаблон(
				НСтр("ru = '%1 руб. по %2 КБК'"),
				ВыборкаПоДОПП.ОбеспечительныйПлатежКОтражению,
				ВыборкаПоКБК.Количество());
		Иначе
			ОбеспечительныйПлатежПредставление = СтрШаблон(
				НСтр("ru = '%1 руб.'"),
				ВыборкаПоДОПП.ОбеспечительныйПлатежКОтражению);
		КонецЕсли;
		СведенияИзДокументов.ОбеспечительныйПлатежПредставление = ОбеспечительныйПлатежПредставление;
		
		Пока ВыборкаПоКБК.Следующий() Цикл
			Платеж = ОбеспечительныеПлатежи.Добавить();
			ЗаполнитьЗначенияСвойств(Платеж, ВыборкаПоКБК);
			Платеж.КлючСтроки = КлючСтроки;
		КонецЦикла;
...
Функция ТекстЗапросаОтражениеДОППВДекларацииПоКосвеннымНалогам()

	Возврат
	"ВЫБРАТЬ
	|	КосвенныеНалогиСведенияИзДОПП.УчетныйДокументДОПП КАК ДОПП,
	|	СУММА(КосвенныеНалогиСведенияИзДОПП.СуммаНалогаАкциза) КАК СуммаНалогаАкциза,
	|	КосвенныеНалогиСведенияИзДОПП.КБК КАК КБК
	|ПОМЕСТИТЬ РанееОтраженныеВОтчетностиДОПП
	|ИЗ
	|	РегистрСведений.КосвенныеНалогиСведенияИзДОПП КАК КосвенныеНалогиСведенияИзДОПП
	|ГДЕ
	|	КосвенныеНалогиСведенияИзДОПП.Период <= &ДатаТекущегоДокумента
	|	И КосвенныеНалогиСведенияИзДОПП.Регистратор <> &ТекущийДокумент
	|	И КосвенныеНалогиСведенияИзДОПП.Организация = &Организация
	|
	|СГРУППИРОВАТЬ ПО
	|	КосвенныеНалогиСведенияИзДОПП.УчетныйДокументДОПП,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РанееОтраженныеВОтчетностиДОПП.ДОПП КАК ДОПП
	|ПОМЕСТИТЬ РанееОтраженныеВОтчетностиДОППСНулевымКБК
	|ИЗ
	|	РанееОтраженныеВОтчетностиДОПП КАК РанееОтраженныеВОтчетностиДОПП
	|ГДЕ
	|	РанееОтраженныеВОтчетностиДОПП.КБК = &НулевойКБК
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РанееОтраженныеВОтчетностиДОПП.ДОПП
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаявлениеОВвозеТоваров.Ссылка КАК ДОПП,
	|	НЕОПРЕДЕЛЕНО КАК ДокументОприходования,
	|	0 КАК СтатусВвозаЧисло
	|ПОМЕСТИТЬ ТаблицаПредполагаемыхПоставокПоДатеВвоза
	|ИЗ
	...
	|	И ЗаявлениеОВвозеТоваров.ДатаВвоза МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеТоваровУслуг.ДОПП КАК ДОПП,
	|	ПоступлениеТоваровУслуг.Ссылка КАК ДокументОприходования,
	|	ПоступлениеТоваровУслуг.Дата КАК ДатаВвоза,
	|	1 КАК СтатусВвозаЧисло
	|ПОМЕСТИТЬ ФактическийВвоз
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Организация = &Организация
	|	И ПоступлениеТоваровУслуг.ДОПП <> ЗНАЧЕНИЕ(Документ.ЗаявлениеОВвозеТоваров.ПустаяСсылка)
	|	И ПоступлениеТоваровУслуг.Дата МЕЖДУ &НачалоПредыдущегоПериода И &КонецПериода
	...
	|	НЕОПРЕДЕЛЕНО КАК ДокументОприходования,
	|	0 КАК СтатусВвозаЧисло
	|ПОМЕСТИТЬ НецелевыеДОПП
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров КАК ЗаявлениеОВвозеТоваров
	|ГДЕ
	|	НЕ ЗаявлениеОВвозеТоваров.Ссылка В
	|				(ВЫБРАТЬ
	|					ТаблицаПредполагаемыхПоставокПоДатеВвоза.ДОПП
	|				ИЗ
	|					ТаблицаПредполагаемыхПоставокПоДатеВвоза
	|		
	|				ОБЪЕДИНИТЬ ВСЕ
	|		
	|				ВЫБРАТЬ
	|					ФактическийВвоз.ДОПП
	|				ИЗ
	|					ФактическийВвоз
	|				ГДЕ
	|					ФактическийВвоз.ДатаВвоза МЕЖДУ &НачалоПериода И &КонецПериода
	|		
	|				ОБЪЕДИНИТЬ ВСЕ
	|		
	|				ВЫБРАТЬ
	|					РанееОтраженныеВОтчетностиДОПП.ДОПП
	|				ИЗ
	|					РанееОтраженныеВОтчетностиДОПП)
	|	И ЗаявлениеОВвозеТоваров.Организация = &Организация
	|	И ЗаявлениеОВвозеТоваров.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП)
	|	И ЗаявлениеОВвозеТоваров.Дата МЕЖДУ &НачалоПредыдущегоПериода И &КонецПериода
	|	И ЗаявлениеОВвозеТоваров.Проведен
	|	И НЕ ЗаявлениеОВвозеТоваров.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РезультирующаяТаблица.ДОПП КАК ДОПП,
	|	РезультирующаяТаблица.ДокументОприходования КАК ДокументОприходования,
	|	РезультирующаяТаблица.СтатусВвозаЧисло КАК СтатусВвозаЧисло
	|ПОМЕСТИТЬ ДокументыВвозаИКБКПредварительная
	|ИЗ
	|	(ВЫБРАТЬ
	|		ФактическийВвоз.ДОПП КАК ДОПП,
	|		ФактическийВвоз.ДокументОприходования КАК ДокументОприходования,
	|		ФактическийВвоз.СтатусВвозаЧисло КАК СтатусВвозаЧисло
	|	ИЗ
	|		ФактическийВвоз КАК ФактическийВвоз
	|	ГДЕ
	|		ФактическийВвоз.ДатаВвоза МЕЖДУ &НачалоПериода И &КонецПериода
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|	ВЫБРАТЬ
	|		ТаблицаПредполагаемыхПоставокПоДатеВвоза.ДОПП,
	|		ЕСТЬNULL(ФактическийВвоз.ДокументОприходования, НЕОПРЕДЕЛЕНО),
	|		ЕСТЬNULL(ФактическийВвоз.СтатусВвозаЧисло, 0)
	|	ИЗ
	|		ТаблицаПредполагаемыхПоставокПоДатеВвоза КАК ТаблицаПредполагаемыхПоставокПоДатеВвоза
	|			ЛЕВОЕ СОЕДИНЕНИЕ ФактическийВвоз КАК ФактическийВвоз
	|			ПО ТаблицаПредполагаемыхПоставокПоДатеВвоза.ДОПП = ФактическийВвоз.ДОПП
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ЕСТЬNULL(ФактическийВвоз.СтатусВвозаЧисло, 0)
	|	ИЗ
	|		НецелевыеДОПП КАК НецелевыеДОПП
	|			ЛЕВОЕ СОЕДИНЕНИЕ ФактическийВвоз КАК ФактическийВвоз
	|			ПО НецелевыеДОПП.ДОПП = ФактическийВвоз.ДОПП) КАК РезультирующаяТаблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РанееОтраженныеВОтчетностиДОППСНулевымКБК КАК РанееОтраженныеВОтчетностиДОППСНулевымКБК
	|		ПО (РанееОтраженныеВОтчетностиДОППСНулевымКБК.ДОПП = РезультирующаяТаблица.ДОПП)
	|ГДЕ
	|	РанееОтраженныеВОтчетностиДОППСНулевымКБК.ДОПП ЕСТЬ NULL
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДОПП
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыВвозаИКБКПредварительная.ДОПП КАК ДОПП,
	|	МАКСИМУМ(ДокументыВвозаИКБКПредварительная.ДокументОприходования) КАК ДокументОприходования,
	|	МАКСИМУМ(ДокументыВвозаИКБКПредварительная.СтатусВвозаЧисло) КАК СтатусВвозаЧисло
	|ПОМЕСТИТЬ ДокументыВвозаИКБК
	|ИЗ
	|	ДокументыВвозаИКБКПредварительная КАК ДокументыВвозаИКБКПредварительная
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументыВвозаИКБКПредварительная.ДОПП
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДОПП
	|;
	...
	|	СуммыПоДОПП.КБК КАК КБК,
	|	СУММА(ВЫБОР
	|			КОГДА СуммыПоДОПП.ОбеспечительныйПлатеж = 0
	|				ТОГДА 0
	|			ИНАЧЕ СуммыПоДОПП.ОбеспечительныйПлатеж - СуммыПоДОПП.СуммаНДС
	|		КОНЕЦ) КАК ОбеспечительныйПлатеж
	|ПОМЕСТИТЬ АвансыПоКБК
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК СуммыПоДОПП
	|ГДЕ
	|	СуммыПоДОПП.Ссылка В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ДокументыВвозаИКБК.ДОПП
	|			ИЗ
	|				ДокументыВвозаИКБК)
	|	И СуммыПоДОПП.КБК <> """"
	|	И НЕ СуммыПоДОПП.МаркируемаяПродукция
	|	И СуммыПоДОПП.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП)
	|
	|СГРУППИРОВАТЬ ПО
	|	СуммыПоДОПП.Ссылка,
	|	СуммыПоДОПП.КБК
	...
	|ВЫБРАТЬ
	|	ДокументыВвозаИКБКПредварительная.ДОПП КАК ДОПП,
	|	СуммыПоЗаявлениюОВвозе.КБК КАК КБК,
	|	ВЫРАЗИТЬ(СУММА(ЕСТЬNULL(СуммыПоЗаявлениюОВвозе.СуммаАкциза, 0)) КАК ЧИСЛО(15, 0)) КАК Начислено
	|ПОМЕСТИТЬ НачисленияПоКБК
	|ИЗ
	|	ДокументыВвозаИКБКПредварительная КАК ДокументыВвозаИКБКПредварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявлениеОВвозеТоваров.Товары КАК СуммыПоЗаявлениюОВвозе
	|		ПО ДокументыВвозаИКБКПредварительная.ДокументОприходования = СуммыПоЗаявлениюОВвозе.ДокументПартии
	|ГДЕ
	|	СуммыПоЗаявлениюОВвозе.Ссылка.Проведен
	|	И СуммыПоЗаявлениюОВвозе.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе)
	|	И СуммыПоЗаявлениюОВвозе.КБК <> """"
	|	И НЕ СуммыПоЗаявлениюОВвозе.МаркируемаяПродукция
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументыВвозаИКБКПредварительная.ДОПП,
	|	СуммыПоЗаявлениюОВвозе.КБК
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ДокументыВвозаИКБКПредварительная.ДОПП,
	|	&КБКНДС,
	|	ВЫРАЗИТЬ(СУММА(ЕСТЬNULL(СуммыПоЗаявлениюОВвозе.СуммаНДС, 0)) КАК ЧИСЛО(15, 0))
	|ИЗ
	|	ДокументыВвозаИКБКПредварительная КАК ДокументыВвозаИКБКПредварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявлениеОВвозеТоваров.Товары КАК СуммыПоЗаявлениюОВвозе
	|		ПО ДокументыВвозаИКБКПредварительная.ДокументОприходования = СуммыПоЗаявлениюОВвозе.ДокументПартии
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АвансыПоКБК.ДОПП КАК ДОПП,
	|	АвансыПоКБК.КБК КАК КБК,
	|	ВЫРАЗИТЬ(АвансыПоКБК.ОбеспечительныйПлатеж КАК ЧИСЛО(15, 0)) КАК ОбеспечительныйПлатежВсего,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0)) КАК РанееОтраженнаяСумма,
	|	ВЫРАЗИТЬ(АвансыПоКБК.ОбеспечительныйПлатеж - ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0)) КАК ОбеспечительныйПлатежКОтражению,
	|	ЕСТЬNULL(НачисленияПоКБК.Начислено, 0) КАК НачисленоПоЗаявлениям,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(НачисленияПоКБК.Начислено, 0) - АвансыПоКБК.ОбеспечительныйПлатеж + ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0)) КАК КДоплатеСЕНС
	|ПОМЕСТИТЬ ОбеспечительныеПлатежиКОтражению
	|ИЗ
	|	АвансыПоКБК КАК АвансыПоКБК
	|		ЛЕВОЕ СОЕДИНЕНИЕ РанееОтраженныеВОтчетностиДОПП КАК РанееОтраженныеВОтчетностиДОПП
	|		ПО (РанееОтраженныеВОтчетностиДОПП.ДОПП = АвансыПоКБК.ДОПП)
	|			И (РанееОтраженныеВОтчетностиДОПП.КБК = АвансыПоКБК.КБК)
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачисленияПоКБК КАК НачисленияПоКБК
	|		ПО (НачисленияПоКБК.ДОПП = АвансыПоКБК.ДОПП)
	|			И (НачисленияПоКБК.КБК = АвансыПоКБК.КБК)
	|ГДЕ
	|	(АвансыПоКБК.ОбеспечительныйПлатеж = 0
	|			ИЛИ (ВЫРАЗИТЬ(АвансыПоКБК.ОбеспечительныйПлатеж - ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0))) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДОПП
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РеглОтчетДОПП.Ссылка КАК Ссылка,
	|	РеглОтчетДОПП.Основание КАК ДОПП
	|ПОМЕСТИТЬ СписокРеглОтчетовДОПП
	|ИЗ
	|	Документ.УведомлениеОСпецрежимахНалогообложения КАК РеглОтчетДОПП
	|ГДЕ
	|	РеглОтчетДОПП.Основание В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ДокументыВвозаИКБК.ДОПП
	|			ИЗ
	|				ДокументыВвозаИКБК)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписокРеглОтчетовДОПП.ДОПП КАК ДОПП,
	|	ДокументыРеализацииПолномочийНалоговыхОрганов.НомерУведомления КАК РегНомерДОПП
	|ПОМЕСТИТЬ РегНомераДОПП1СОтчетность
	|ИЗ
	|	СписокРеглОтчетовДОПП КАК СписокРеглОтчетовДОПП
	...
	|		ПО СписокРеглОтчетовДОПП.Ссылка = ДокументыРеализацииПолномочийНалоговыхОрганов.ДокументОснование
	|ГДЕ
	|	ДокументыРеализацииПолномочийНалоговыхОрганов.Организация = &Организация
	|	И ДокументыРеализацииПолномочийНалоговыхОрганов.ВидДокумента = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговыхДокументов.КвитанцияОПрисвоенииСтатусаДОПП)
	|	И ДокументыРеализацииПолномочийНалоговыхОрганов.СтатусДОПП = ""2""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДОПП
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыВвозаИКБК.ДОПП КАК ДОПП,
	|	ДокументыВвозаИКБК.ДокументОприходования КАК ДокументОприходования,
	|	ВЫБОР
	|		КОГДА ДокументыВвозаИКБК.СтатусВвозаЧисло = 1
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияПоставкиТоваровИзЕАЭС.Завершена)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СостоянияПоставкиТоваровИзЕАЭС.Ожидается)
	|	КОНЕЦ КАК Состояние,
	|	ДокументыВвозаИКБК.ДОПП.Контрагент КАК Контрагент,
	|	ЕСТЬNULL(РегНомераДОПП1СОтчетность.РегНомерДОПП, ДокументыВвозаИКБК.ДОПП.НомерДОПП) КАК РегистрационныйНомерДОПП,
	|	ОбеспечительныеПлатежиКОтражению.КБК КАК КБК,
	|	ОбеспечительныеПлатежиКОтражению.ОбеспечительныйПлатежВсего КАК ОбеспечительныйПлатежВсего,
	|	ВЫБОР
	|		КОГДА ДокументыВвозаИКБК.СтатусВвозаЧисло = 1
	|			ТОГДА ВЫРАЗИТЬ(ОбеспечительныеПлатежиКОтражению.ОбеспечительныйПлатежКОтражению КАК ЧИСЛО(15, 0))
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ОбеспечительныйПлатежКОтражению,
	|	ОбеспечительныеПлатежиКОтражению.НачисленоПоЗаявлениям КАК НачисленоПриВвозе,
	|	ВЫБОР
	|		КОГДА ДокументыВвозаИКБК.СтатусВвозаЧисло = 1
	|			ТОГДА ОбеспечительныеПлатежиКОтражению.КДоплатеСЕНС
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КДоплате
	|ИЗ
	|	ДокументыВвозаИКБК КАК ДокументыВвозаИКБК
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегНомераДОПП1СОтчетность КАК РегНомераДОПП1СОтчетность
	|		ПО ДокументыВвозаИКБК.ДОПП = РегНомераДОПП1СОтчетность.ДОПП
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОбеспечительныеПлатежиКОтражению КАК ОбеспечительныеПлатежиКОтражению
	|		ПО ДокументыВвозаИКБК.ДОПП = ОбеспечительныеПлатежиКОтражению.ДОПП
	|ГДЕ
	|	НЕ ОбеспечительныеПлатежиКОтражению.КБК ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДокументыВвозаИКБК.ДОПП.Дата
	|ИТОГИ
	|	МАКСИМУМ(ДокументОприходования),
	|	МАКСИМУМ(Состояние),
	|	МАКСИМУМ(Контрагент),
	|	МАКСИМУМ(РегистрационныйНомерДОПП),
	|	СУММА(ОбеспечительныйПлатежВсего),
	|	СУММА(ОбеспечительныйПлатежКОтражению),
	|	СУММА(НачисленоПриВвозе),
	|	СУММА(КДоплате)
	|ПО
	|	ДОПП";
...
КонецФункции // НулевойКБКПриВвозеТоваров()

Функция СписокВыбораКБККосвенныеНалоги() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ОтчетОбъект = РегламентированнаяОтчетностьВызовСервера.ОтчетОбъект("РегламентированныйОтчетКосвенныеНалогиТамСоюз");
	МакетСоставаПоказателей = ОтчетОбъект.ПолучитьМакет("СпискиВыбора2026Кв2");
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Область = МакетСоставаПоказателей.Области.Найти("КБК_Раздел4");
	
	КБКРаздел4 = Новый ТаблицаЗначений;
	КБКРаздел4.Колонки.Добавить("Код",      ОбщегоНазначения.ОписаниеТипаСтрока(20));
	КБКРаздел4.Колонки.Добавить("Название", ОбщегоНазначения.ОписаниеТипаСтрока(256));
	
	Для НомСтр = Область.Верх По Область.Низ Цикл
		КодКБК = СокрП(МакетСоставаПоказателей.Область(НомСтр, 1).Текст);
		Если Не ПустаяСтрока(КодКБК) И КодКБК <> "00000000000000000000" Тогда
			Строка = КБКРаздел4.Добавить();
			Строка.Код = КодКБК;
			Строка.Название = СокрП(МакетСоставаПоказателей.Область(НомСтр, 2).Текст);
		КонецЕсли;
	КонецЦикла;
...
#Область ДекларацияКосвенныеНалогиПриИмпортеТоваров
	
	// Создаёт список автоматически заполняемых показателей для раздела 3 отчета "Косвенные налоги при импорте товаров"
	Процедура ПолучитьСведенияОПоказателяхОтчетаКосвенныеНалогиПриИмпортеТоваровФорма2026Кв2(ПоказателиОтчета) Экспорт
		
		УчетАкцизов.ПолучитьСведенияОПоказателяхОтчетаКосвенныеНалогиФорма2026Кв2(ПоказателиОтчета);
		
	КонецПроцедуры
	
	// Заполняет показатели раздела 3 отчета "Косвенные налоги при импорте товаров"
	Процедура ЗаполнитьОтчетКосвенныеНалогиПриИмпортеТоваровФормаОтчета2026Кв2(ПараметрыОтчета, Контейнер) Экспорт
		
		УчетАкцизов.ЗаполнитьРазделАкцизовРегламентированныйОтчетКосвенныеНалогиФорма2026Кв2(ПараметрыОтчета, Контейнер);
...
Функция ТекстЗапросаРеквизитыСчетовФактурПостановление981КнигаПокупок() Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаСчетаФактурыДокументыПредварительная.СчетФактураДокумент.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.КорректировочнаяСправка)
	|			ТОГДА ТаблицаСчетаФактурыДокументыПредварительная.СчетФактураДокумент.ИсправляемыйСчетФактура
	|		ИНАЧЕ ВЫБОР
	...
	|		ПО (ТаблицаСчетаФактурыДокументыПредварительная.СчетФактураДокумент.ВидСчетаФактуры <> ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.КорректировочнаяСправка))
	|			И ТаблицаСчетаФактурыДокументыПредварительная.ДокументОснованиеСчетаФактуры = ВТ_СводныеСправки.ДокументВозврата
