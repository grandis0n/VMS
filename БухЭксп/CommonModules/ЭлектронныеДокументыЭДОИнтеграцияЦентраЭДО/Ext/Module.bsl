...
Функция НовыйРезультатДействийЭДО() Экспорт
	...
	РезультатДействий.Вставить("ЗашифрованныеМаркеры", Неопределено);
...
Функция СоздатьПакетМетодовОперацииПоДействиямЭДО(ПараметрыОперации, КонтекстДиагностики)
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Загрузить) Тогда
		...
		МашиночитаемыеДоверенностиИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаПорцияСтатусовДоверенностейПоМеткамВремени(
			ПакетМетодов, ПараметрыОперации);
		
		МашиночитаемыеДоверенностиИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаСтатусыДоверенностейСОшибкамиСинхронизации(
			ПакетМетодов, ПараметрыОперации);
...
Функция ВыполнитьДействияПоУчетнойЗаписи(ПараметрыОперации, КонтекстДиагностики)
	...
	Если ЗначениеЗаполнено(ОшибкиФормирования) Тогда
		ЗавершитьОбработкуДействийЭДО(РезультатДействийЭДО, ПараметрыОперации);
...
Процедура ЗавершитьОбработкуДействийЭДО(РезультатДействийЭДО, ПараметрыОперации)
	ОбработаноПоДействиям = РезультатДействийЭДО.Итог.ОбработаноПоДействиям;
...
// Параметры:
//  ИдентификаторЭДО - Строка
//  ДатаВыгрузкиДокументов - Дата
//  ВыгружатьНезавершенные - Булево
// 
// Возвращаемое значение:
//  См. РегистрыСведений.СостоянияВыгрузкиВЦентрЭДО.СостояниеПоИдентификаторуЭДО
Функция ЗапуститьВыгрузкуВЦентрЭДО(ИдентификаторЭДО, ДатаВыгрузкиДокументов = '00010101',
	ВыгружатьНезавершенные = Ложь) Экспорт
	...
	ЗаписьНабора.ДатаВыгрузкиДокументов = ДатаВыгрузкиДокументов;
	ЗаписьНабора.ВыгружатьНезавершенные = ВыгружатьНезавершенные;
...
Процедура ВыполнитьВыгрузкуВЦентрЭДО() Экспорт
	...
	ТекстЗапроса = 
		...
		|	СостоянияВыгрузкиВЦентрЭДО.ДатаНачала КАК ДатаНачала,
		|	СостоянияВыгрузкиВЦентрЭДО.ДатаЗавершенияРегистрации КАК ДатаЗавершенияРегистрации,
		|	СостоянияВыгрузкиВЦентрЭДО.ДатаВыгрузкиДокументов КАК ДатаВыгрузкиДокументов,
		|	СостоянияВыгрузкиВЦентрЭДО.ВыгружатьНезавершенные КАК ВыгружатьНезавершенные
		|ИЗ
		|	РегистрСведений.НастройкиИнтеграцииЦентраЭДО КАК НастройкиЦентраЭДО
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияВыгрузкиВЦентрЭДО КАК СостоянияВыгрузкиВЦентрЭДО
		|		ПО НастройкиЦентраЭДО.ИдентификаторЭДО = СостоянияВыгрузкиВЦентрЭДО.ИдентификаторЭДО
		|ГДЕ
		|	СостоянияВыгрузкиВЦентрЭДО.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)";
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(Выборка.ДатаЗавершенияРегистрации) Тогда
				...
				ЗарегистрироватьДокументыДляВыгрузкиВЦентрЭДО(Выборка.ИдентификаторЭДО, Выборка.УчетнаяЗаписьЦентраЭДО,
					Выборка.ДатаВыгрузкиДокументов, Выборка.ВыгружатьНезавершенные);
			КонецЕсли;
...
Процедура ЗарегистрироватьДокументыДляВыгрузкиВЦентрЭДО(ИдентификаторЭДО, УчетнаяЗаписьЦентраЭДО,
	ДатаВыгрузкиДокументов, ВыгружатьНезавершенные = Ложь)
	...
	ЗарегистрироватьДокументыПоНаправлению(Истина, ИдентификаторЭДО, УчетнаяЗаписьЦентраЭДО,
		ДатаВыгрузкиДокументов, ВыгружатьНезавершенные);
...
Процедура ЗарегистрироватьДокументыПоНаправлению(ЭтоВходящийЭДО, ИдентификаторЭДО, УчетнаяЗаписьЦентраЭДО,
	ДатаВыгрузкиДокументов = '00010101', ВыгружатьНезавершенные = Ложь)
	...
	ТекстЗапроса = 
		...
		|		ПО ЭлектронныеДокументы.Ссылка = ИдентификаторыСинхронизируемыхОбъектовЦентраЭДО.Ссылка
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеДокументыВЦентрЭДО КАК ВыгружаемыеДокументыВЦентрЭДО
		|		ПО ЭлектронныеДокументы.Ссылка = ВыгружаемыеДокументыВЦентрЭДО.ЭлектронныйДокумент
		|		И ВыгружаемыеДокументыВЦентрЭДО.ИдентификаторЭДО = &ИдентификаторЭДО
		|ГДЕ
		|	ЭлектронныеДокументы.Ссылка > &Ссылка
		|	И ЭлектронныеДокументы.ИдентификаторОрганизации = &ИдентификаторЭДО
		|	И ИдентификаторыСинхронизируемыхОбъектовЦентраЭДО.Ссылка ЕСТЬ NULL
		|	И ВыгружаемыеДокументыВЦентрЭДО.ЭлектронныйДокумент ЕСТЬ NULL
		|	И &УсловиеПоДатеВыгрузкиДокументов
		|	И &УсловиеВыгружатьНезавершенные
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка";
	...
	Если ЗначениеЗаполнено(ДатаВыгрузкиДокументов) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоДатеВыгрузкиДокументов",
			"(ЭлектронныеДокументы.ДатаДокумента >= &ДатаВыгрузкиДокументов
			|	ИЛИ ЭлектронныеДокументы.ДатаДокумента = ДАТАВРЕМЯ(1, 1, 1)
			|		И ЭлектронныеДокументы.Дата >= &ДатаВыгрузкиДокументов)");
	КонецЕсли;
	
	Если ВыгружатьНезавершенные Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеВыгружатьНезавершенные",
			"НЕ СостоянияДокументовЭДО.Состояние В (&СостоянияЗавершенногоЭДО)");
		СостоянияЗавершенногоЭДО = РегламентыЭДО.СостоянияЗавершенногоЭДО();
	Иначе
		СостоянияЗавершенногоЭДО = Новый Массив; // см. РегламентыЭДО.СостоянияЗавершенногоЭДО
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		Если ЗначениеЗаполнено(ДатаВыгрузкиДокументов) Тогда
			Запрос.УстановитьПараметр("ДатаВыгрузкиДокументов", ДатаВыгрузкиДокументов);
		Иначе
			Запрос.УстановитьПараметр("УсловиеПоДатеВыгрузкиДокументов", Истина);
		КонецЕсли;
		
		Если ВыгружатьНезавершенные Тогда
			Запрос.УстановитьПараметр("СостоянияЗавершенногоЭДО", СостоянияЗавершенногоЭДО);
		Иначе
			Запрос.УстановитьПараметр("УсловиеВыгружатьНезавершенные", Истина);
		КонецЕсли;
