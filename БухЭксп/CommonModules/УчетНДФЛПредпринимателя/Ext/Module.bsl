...
Функция ФормаДекларацииПоУмолчанию(Период) Экспорт
	...
	ПериодНачалаВычетаСтрахование = ПомощникЗаполнения3НДФЛКлиентСервер.НачалоДействияВычетаЗаДолгосрочноеСтрахование();
	
	Если Год(Период) >= Год(НачалоДействияУпрощеннойФормы) - 1 Тогда
		Если ТекущаяДата >= ПериодНачалаВычетаСтрахование Тогда
			Результат = "ФормаОтчета2026Кв3";
		Иначе
...
Функция НачисленныеСуммыПоВидамНалога(Показатели, НалоговыеСтавки = Неопределено) Экспорт
	...
	Для Каждого ТекущаяСтавка Из НалоговыеСтавки Цикл
		...
		Если ТекущаяСтавка.МаксимальнаяСуммаДохода = 0 Тогда
			МаксимальнаяСуммаДохода = Показатели.НалоговаяБаза;
		Иначе
			МаксимальнаяСуммаДохода = ТекущаяСтавка.МаксимальнаяСуммаДохода;
		КонецЕсли;
		...
		Если НалогКУплате > 0 Тогда
			...
			НоваяСтрока.НалоговаяБаза = Мин(Показатели.НалоговаяБаза, МаксимальнаяСуммаДохода);
...
// Возвращает признак того, что данные измененного вычета записаны в регистр стандартных вычетов
//
// Параметры:
//   ПараметрыВычета - Массив из Структура - Данные вычета для записи в регистр.
//   РеквизитыЗаписи - Структура - см. УчетНДФЛПредпринимателяКлиентСервер.НовыеРеквизитыЗаписиВычета()
//
// Возвращаемое значение:
//   Булево - Истина, если данные вычета изменены и записаны в регистр, иначе данные вычета не менялись
//
Функция СохранитьСтандартныйВычетЗаПериод(ПараметрыВычета, РеквизитыЗаписи) Экспорт
	...
	СравниваемаяТаблица = ВычетыЗаПериод.СкопироватьКолонки();
	Для Каждого ТекущаяСтрока Из ПараметрыВычета Цикл
		ЗаполнитьЗначенияСвойств(СравниваемаяТаблица.Добавить(), ТекущаяСтрока);
	КонецЦикла;
	
	СравниваемаяТаблица.Сортировать("НомерМесяца");
	
	Если ОбщегоНазначения.КоллекцииИдентичны(ВычетыЗаПериод, СравниваемаяТаблица, , "Организация, Период") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция СведенияОДоходахИВычетах(ПараметрыВыполнения) Экспорт
	...
	ВычетНаДетейЗависитОтНалоговойБазы = ВычетНаДетейЗависитОтНалоговойБазы(ПараметрыВыполнения.НачалоПериода);
	
	ОбщаяСуммаДоходов = 0;
	Для Каждого СтрокаДоходыРасходы Из ТаблицаДоходовРасходов Цикл
		
		Если ВычетНаДетейЗависитОтНалоговойБазы Тогда
			ОбщаяСуммаДоходов = ОбщаяСуммаДоходов + СтрокаДоходыРасходы.ИтогоДоходов - СтрокаДоходыРасходы.ИтогоРасходов;
		Иначе
...
Функция ВычетНаДетейЗависитОтНалоговойБазы(Период)
	
	// п.38 ст.2 Федеральный закон от 28.11.2025 N 425-ФЗ
	// Вычет на детей действует до месяца, в котором сумма основной налоговой базы, превысила предельную величину
