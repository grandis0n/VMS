...
Функция НовыйУниверсальныеНастройки() Экспорт
	...
	Настройки.Вставить("Счет",								ПредопределенноеЗначение("ПланСчетов.CW_МСФО.ПустаяСсылка"));
...
Функция МестоРасхожденияИтогов(Форма) Экспорт
	...
	МестоРасхождения.Вставить("ИмяОтчета",          CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИмяОтчетаПоИмениФормы(Форма));
...
Функция КореньПоля(Поле) Экспорт
	...
	// См. также CW_БухгалтерскиеОтчетыВызовСервераМСФО.ЗаполнитьДополнительныеПоляИзНастроек()
...
Процедура ОрганизацияПриИзменении(Форма, Организация) Экспорт 
	...
	Отчет = Форма.Отчет;
	ТипыСвязанныеСОрганизацией = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ТипыСвязанныеСОрганизацией();
...
Функция ПолучитьПредставлениеПериода(НачалоПериода = '00010101', КонецПериода = '00010101', ТолькоДаты  = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(КонецПериода) Тогда 
		Если КонецПериода >= НачалоПериода Тогда
			ТекстПериод = ?(ТолькоДаты, "", " " + НСтр("ru = 'for'")+ " ") 
...
Функция ОтчетПоддерживаетУниверсальныеНастройки(ИмяОтчета) Экспорт
	...
	//СписокОтчетов.Добавить("CW_АнализСубконтоМСФО");
	СписокОтчетов.Добавить("CW_АнализСчетаМСФО");
	//СписокОтчетов.Добавить("CW_КарточкаСубконтоМСФО");
	СписокОтчетов.Добавить("CW_КарточкаСчетаМСФО");
	СписокОтчетов.Добавить("CW_ОборотноСальдоваяВедомостьМСФО");
	СписокОтчетов.Добавить("CW_ОборотноСальдоваяВедомостьПоСчетуМСФО");
	//СписокОтчетов.Добавить("CW_ОборотыМеждуСубконтоМСФО");
	СписокОтчетов.Добавить("CW_ОборотыСчетаМСФО");
	СписокОтчетов.Добавить("CW_ОтчетПоПроводкамМСФО");
	//СписокОтчетов.Добавить("CW_СводныеПроводкиМСФО");
	//СписокОтчетов.Добавить("ШахматнаяВедомостьМСФО");
...
Функция ИмяМакетаОформленияПоУмолчанию() Экспорт
	...
	CW_БухгалтерскиеОтчетыКлиентСерверПереопределяемыйМСФО.УстановитьИмяМакетаОформленияПоУмолчанию(ЗначениеПоУмолчанию);
...
// CW_БухгалтерскиеОтчетыВызовСервераМСФО.ИнициализацияКомпоновщикаНастроек
//
...
// Он используется в CW_БухгалтерскиеОтчетыВызовСервераМСФО.ИнициализацияКомпоновщикаНастроек() для отчетов,
...
Функция ОпределитьПараметрыОтчетаРасчетыСКонтрагентами(Форма) Экспорт
	...
	ТипВидыСубконто = Тип("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО");
	...
	ТипCW_МСФО         = Тип("ПланСчетовСсылка.CW_МСФО");
	...
	Для Каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
			...
			Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = ТипCW_МСФО Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
			...
			Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("СписокЗначений") Тогда
				...
				Для Каждого ЭлементСписка Из ЭлементОтбора.ПравоеЗначение Цикл
					Если ТипЗнч(ЭлементСписка.Значение) = ТипCW_МСФО Тогда
						Результат.Счета.Добавить(ЭлементОтбора.ПравоеЗначение);
