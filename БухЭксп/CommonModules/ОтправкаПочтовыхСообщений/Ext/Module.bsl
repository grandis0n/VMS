...
Функция ПараметрыЭлектронногоПисьма(НаборПечатныхФорм, МенеджерПечати, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Вложения.Количество() > 0 Тогда
		...
		Для Каждого ПечатныеФормы Из НаборПечатныхФорм Цикл
			...
			ФормироватьЭДПечатнойФормы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ПечатныеФормы.ПараметрыВывода, "ФормироватьЭД", Ложь);
			
			ДобавленныеТипы = Новый Соответствие;
			Для Каждого ОбъектПечати Из ОбъектыПечатиПоПечатнойФорме Цикл
				ТипОбъекта = ТипЗнч(ОбъектПечати);
				Если ДобавленныеТипы.Получить(ТипОбъекта) = Неопределено Тогда
					ДобавленныеТипы.Вставить(ТипОбъекта);
					ПолноеИмяОбъекта = ОбщегоНазначения.ИмяТаблицыПоСсылке(ОбъектПечати);
					Если ФормироватьЭД.Получить(ПолноеИмяОбъекта) <> Истина Тогда
						ФормироватьЭД.Вставить(ПолноеИмяОбъекта, ФормироватьЭДПечатнойФормы);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
