...
// Возвращает доступные протоколы обмена ФЭС НСПК.
//  
// Возвращаемое значение:
//  ПротоколыОбменаФЭСНСПК - Массив - Протоколы обмена ФЭС НСПК
//
Функция ДоступныеПротоколыОбменаФЭСНСПК() Экспорт
	
	ПротоколыОбменаФЭСНСПК = Новый Массив();  
	ПротоколыОбменаФЭСНСПК.Добавить(Перечисления.ПротоколОбменаФЭСНСПК.ВерсияV2);
	МенеджерОборудованияВызовСервераПереопределяемый.ДоступныеПротоколыОбменаФЭСНСПК(ПротоколыОбменаФЭСНСПК);
	Возврат ПротоколыОбменаФЭСНСПК; 
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьПараметрыСеансаПодключаемогоОборудования");
