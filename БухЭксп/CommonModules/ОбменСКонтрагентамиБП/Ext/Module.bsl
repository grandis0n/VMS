...
Процедура ЗаполнитьПараметрыЭДПоИсточнику(Источник, ПараметрыЭД) Экспорт
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		...
		Если Документы.СчетФактураВыданный.ЭтоУниверсальныйДокумент(Источник)
				И ОснованиеСФУчаствуетВЭДО(Источник) Тогда
			СтрокаПараметровЭД.ФормированиеУниверсальногоДокумента = 
					ОбменСКонтрагентами.ВариантыФормированияУниверсальныхДокументов().Обязательно;
		Иначе
			СтрокаПараметровЭД.ФормированиеУниверсальногоДокумента = 
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Источник, "Организация, Контрагент, ДоговорКонтрагента, ВидОперации");
		...
		Если ЗначенияРеквизитов.ВидОперации = Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОПродажах Тогда
			СтрокаПараметровЭД = ПараметрыЭД.Добавить();
	...
	ИначеЕсли ТипИсточника = Тип("ДокументОбъект.ОтчетКомитентуОПродажах") Тогда
		...
		Если Источник.ВидОперации = Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОПродажах Тогда
			СтрокаПараметровЭД = ПараметрыЭД.Добавить();
			СтрокаПараметровЭД.Тип					 = ТипыДокументов.ОтчетОПродажахКомиссионногоТовара;
			СтрокаПараметровЭД.Направление 			 = НаправленияДокументов.Исходящий;
			СтрокаПараметровЭД.Организация			 = Источник.Организация;
...
Процедура СпособыОтраженияВУчетеТипаЭлектронногоДокумента(ТипДокумента, СпособыОтраженияВУчете) Экспорт 
	...
	ИначеЕсли ТипДокумента = ТипыДокументов.ЗаказТовара Тогда
		СпособыОтраженияВУчете.Добавить(СпособыОбработки.ЗаказПокупателя, 
			НСтр("ru = 'Счет покупателю'"),Истина);
	КонецЕсли;
...
Процедура ИмяОбъектаМетаданныхПоСпособуОтраженияВУчете(СпособОтраженияВУчете, ИмяДокумента) Экспорт 
	...
	ИначеЕсли СпособОтраженияВУчете = СпособыОбработки.ЗаказПокупателя Тогда
		ИмяДокумента = Метаданные.Документы.СчетНаОплатуПокупателю.Имя;
...
Процедура СохранитьДанныеОбъектаВБД(СтрокаДляЗагрузки, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект = Неопределено, ОписаниеОшибки) Экспорт
	...
	ИначеЕсли СтрокаДляЗагрузки.ВидЭД = Перечисления.ТипыДокументовЭДО.ЗаказТовара Тогда // торговые предложения
		
		НайденныйОбъект = НайтиСоздатьСчетНаОплатуПокупателю(СтрокаДляЗагрузки, ДеревоРазбора, ПараметрыОбработки.СсылкаНаВладельца);
...
Функция НайтиСсылкуНаОбъект(ТипОбъекта, ИдОбъекта = "", ДополнительныеРеквизиты = Неопределено) Экспорт
	...
	ИначеЕсли (ТипОбъекта = "БанковскиеСчетаОрганизаций" Или ТипОбъекта = "БанковскиеСчетаКонтрагентов") И ЗначениеЗаполнено(ДополнительныеРеквизиты)Тогда
		...
		Если ДополнительныеРеквизиты.Свойство("Владелец", Владелец) Тогда
			...
			Если ДополнительныеРеквизиты.Свойство("БИК") Тогда
				Банк = Новый Соответствие;
				Банк.Вставить("Банк.Код", ДополнительныеРеквизиты.БИК);
				Результат = НайтиСсылкуНаОбъектПоРеквизиту(
					ИмяПрикладногоСправочника, "НомерСчета", ИдОбъекта, Владелец, Банк);
			Иначе
...
Процедура КомандыЭДО_ФормаСписка(Форма, Входящий = Ложь) Экспорт 
	...
	Если БизнесСеть.ИспользоватьОбменБизнесСеть()
		И ПолучитьФункциональнуюОпцию("ИспользоватьТорговыеПредложения")
		И Форма.Команды.Найти("ЗагрузитьЧерезБизнесСеть") <> Неопределено Тогда
		
		НовыйЭлемент            = Форма.Элементы.Добавить("ЗагрузитьЧерезБизнесСеть", Тип("КнопкаФормы"), Элементы.КомандыЭДО);
		НовыйЭлемент.Вид        = ВидКнопкиФормы.КнопкаКоманднойПанели;
		НовыйЭлемент.ИмяКоманды = "ЗагрузитьЧерезБизнесСеть";
...
Процедура СоздатьКонтрагентаПоРеквизитам(Знач РеквизитыКонтрагента, Контрагент, Отказ = Ложь) Экспорт
	...
	ВидКонтрагента = "";
	Если РеквизитыКонтрагента.Свойство("ВидКонтрагента") Тогда
		ВидКонтрагента = СокрЛП(РеквизитыКонтрагента.ВидКонтрагента);
	КонецЕсли;
	...
	Если СтрДлина(ИНН) > 10 Тогда
		...
		КонтрагентОбъект.ИндивидуальныйПредприниматель = ВРег(ВидКонтрагента) = ВРег("ИП");
	Иначе
...
Функция СчетФактураНеТребуется(СсылкаНаДокумент)
	...
	Если ЭтоКомиссионнаяТорговля(СсылкаНаДокумент) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ЭтоКомиссионнаяТорговля(СсылкаНаОбъект)
	...
	Если ТипЗнч(ДанныеПервичногоДокумента.Ссылка) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
		// Пользователь создает УПД на комиссионное вознаграждение, что являетя обычным актом об оказании услуг,
		// а не передачей комиссионного товара.
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеУчастника(ДеревоДанных, СведенияОбУчастнике, ВидУчастника, ВидАдреса = Неопределено, ДатаСведений = '00010101')
	...
	Иначе
		...
		Если ЭлектронноеВзаимодействие.СуществуетРеквизитВДереве(ДеревоДанных, ПутьКРеквизитуДерева + ".ИП")
...
Процедура ЗаполнитьДанныеУчастникаУПД(ДеревоДанных, СведенияОбУчастнике, ВидУчастника, ВидАдреса, ДатаСведений = '00010101')
	...
	Иначе
		...
		Если СведенияОбУчастнике.ЭтоИндивидуальныйПредприниматель Тогда
			...
			Если ЭлектронноеВзаимодействие.СуществуетРеквизитВДереве(ДеревоДанных, ПутьВДереве + ".ОГРН") Тогда
...
Процедура ЗаполнитьДанныеДляСЧФДОПИнформацияПродавцаФНС_2019(Основание, ДанныеДляФормированияЭД, ДеревоДанных, Описание, Отказ)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		СчетФактура = Основание;
		ДанныеДляФормированияЭД.ЭтоКомиссионнаяТорговля  = ЭтоКомиссионнаяТорговля(СчетФактура);
...
Процедура ЗаполнитьДанныеДляДОПИнформацияПродавцаФНС_2019(Основание, ДанныеДляФормированияЭД, ДеревоДанных, Отказ)
	...
	ДанныеДляФормированияЭД.ЭтоКомиссионнаяТорговля = ЭтоКомиссионнаяТорговля(Основание);
	ЗаполнитьОсновнуюЧастьУПД_2019(ДанныеДляФормированияЭД, ДеревоДанных, Отказ);
	ЗаполнитьРасширеннуюЧастьУПД_2019(ДанныеДляФормированияЭД, ДеревоДанных); 
...
Функция ПодготовитьСтруктуруДляПоступленияТоваровУслугУПД_2019(ДеревоДанных, СпособОбработки, ОписаниеОшибки = "")
	...
	ДанныеТабличныхЧастей.ШтрихкодыУпаковок = ЭлектронноеВзаимодействиеИСМП.НоваяТаблицаШтрихкодыУпаковок(Истина); //БгосИС 2.0.7
	...
	//БгосИС 2.0.7 
	//После обработки строк ЭД, но ДО вызова метода ЭлектронноеВзаимодействиеИСМП.СвернутьТаблицуШтрихкодовУпаковок(ШтрихкодыУпаковок);
	//Добавить вызов ЭлектронноеВзаимодействиеИСМП.ЗаписатьСоответствиеНоменклатурыСырьяПоЭДО(ШтрихкодыУпаковок);
	ЭлектронноеВзаимодействиеИСМП.ЗаписатьСоответствиеНоменклатурыСырьяПоЭДО(ДанныеТабличныхЧастей.ШтрихкодыУпаковок);
	//Конец БгосИС 2.0.7
...
Процедура ЗаполнитьСведенияОПокупателяхУПД(ДеревоДанных, ДанныеДляФормированияЭД)
	...
	Иначе
		...
		Если ДанныеДокументаУчета.Свойство("ШтрихкодыУпаковок")
				...
				ВыбытиеИзОборота = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
					ДанныеДокументаУчета.ДанныеШапки, 
					"ПередачаДляСобственныхНуждИСМП", 
					Ложь);
				
				ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
					"СведенияОВыбытииМаркированныхТоваров", ?(ВыбытиеИзОборота, 1, 0));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеДляСЧФДОПИнформацияПродавцаФНС_5_02(Основание, ДанныеДляФормированияЭД, ДеревоДанных, Описание, Отказ)
	...
	Иначе
		...
		СчетФактура = Неопределено;
		Если ТипЗнч(ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах")
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПервичныйДокумент, "ВидОперации") = Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОЗакупках Тогда
			// Отбор при получении счета-фактуры нужен, чтобы не подхватывались данные переопределяемого счета-фактуры при комиссионых закупках
			СтруктураОтбора.Вставить("Продавец", Справочники.Контрагенты.ПустаяСсылка());
			СчетФактура = УчетНДСПереопределяемый.НайтиПодчиненныйСчетФактуруВыданныйНаРеализацию(ПервичныйДокумент,,, СтруктураОтбора);
		Иначе
			СчетФактура = УчетНДСПереопределяемый.НайтиПодчиненныйСчетФактуруВыданныйНаРеализацию(ПервичныйДокумент);
		КонецЕсли;
		Описание.Основания.Добавить(СчетФактура)
...
Функция ПодготовитьСтруктуруДляПоступленияТоваровУслугУПД_5_02(ДанныеЭД, ДеревоДанных, СпособОбработки, ОписаниеОшибки = "")
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		НовСтрока.Номенклатура = НоменклатураИБ(СведенияОТоваре, "ТаблицаСчетаФактуры.СведенияОбОтгруженныхПозициях.НомерСтроки.Сопоставление"); 
		ЭлектронноеВзаимодействиеИСМП.ДобавитьШтрихкодыТаблицуШтрихкодовУпаковок_5_02(ДанныеТабличныхЧастей.ШтрихкодыУпаковок, СведенияОТоваре, НовСтрока);
	КонецЦикла;
...
Функция ПодготовитьСтруктуруДляСчетаФактурыУПД_5_02(ДанныеЭД, ОписаниеОшибки = "")
	...
	//Заполняем документы-основания.
	ОснованияСФ = Новый Массив;
	Если Не (ДанныеЭД.ДокументыОснованияСчетаФактуры = Неопределено
			Или ДанныеОбъекта.ЭтоПеревыставленныйСФНаКомиссионныеЗакупки) Тогда
		Если ТипЗнч(ДанныеЭД.ДокументыОснованияСчетаФактуры) = Тип("Массив") Тогда
			Для Каждого Элемент Из ДанныеЭД.ДокументыОснованияСчетаФактуры Цикл
				ОснованияСФ.Добавить(Элемент);
			КонецЦикла;
...
Процедура ОпределитьУчастниковПоДаннымЭД_5_02(ДанныеЭД, ДанныеОбъекта, ДатаСведений, ОписаниеОшибки = "")
	...
	Для каждого ДанныеГрузоотправителя Из ДанныеЭД.СведенияОСчетеФактуре.СведенияОГрузоотправителях Цикл
		Если ДанныеГрузоотправителя.ОнЖе = Истина Тогда
			ДанныеОбъекта.Вставить("Грузоотправитель", Неопределено); // при совпадении с контрагентом передаем Неопределено
		Иначе
	...
	Если ИНН_КППОбъектовСовпадает(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОбъекта, "Организация"),
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОбъекта, "Грузополучатель")) Тогда
		// Если Покупатель и Грузополучатель сопадают, то поле Грузополучатель не заполняем в документе поступления
...
Процедура ЗаполнитьСпециальныеОбстоятельстваФормированияУПД_5_02(ДеревоДанных, ДанныеДляФормированияЭД, ДанныеПервичногоДокумента, ПутьВДереве = "")
	...
	Если ЭтоФункцияДОП(Настройки.Функция) Тогда // УПД со статусом 2
		...
		Если ДанныеДляФормированияЭД.ЭтоКомиссионнаяТорговля Тогда
			Если ДанныеПервичногоДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
...
Процедура ЗаполнитьОсновнуюЧастьУКД_2020(ДанныеДляФормированияЭД, Настройки, ДеревоДанных, Отказ = Ложь)
	...
	Если ЭтоКомиссионнаяТорговля(ДанныеДляФормированияЭД.Ссылка)
...
Функция ПодготовитьСтруктуруДляКорректировкиПоступленияУКД_2020(ДеревоДанных, ОписаниеОшибки = "")
	...
	СведенияОТоварах = ДеревоДанных.Строки.Найти("СведенияОТоварах", "ПолныйПуть");
	ИспользуетсяОбратноеНачислениеНДС = БухгалтерскийУчетПереопределяемый.ИспользуетсяОбратноеНачислениеНДС();
	ДанныеТабличныхЧастей.ШтрихкодыУпаковок = ЭлектронноеВзаимодействиеИСМП.НоваяТаблицаШтрихкодыУпаковок(Истина);
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
...
#Область СозданиеСчетаНаОплатуТорговыеПредложения

Функция НайтиСоздатьСчетНаОплатуПокупателю(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено)
	
	ДанныеДляЗагрузки = ПодготовитьСтруктуруДляСчетаПокупателю(СтрокаДляЗагрузки, ДеревоРазбора);
	...
	Иначе // создаем новый 
		...
		УчетДенежныхСредствБП.УстановитьБанковскийСчет(ДокументОбъект.СтруктурнаяЕдиница,
			ДокументОбъект.Организация, ДокументОбъект.ВалютаДокумента);
			
		ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(ДокументОбъект.Организация, ДокументОбъект.Дата)
			Или УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ДокументОбъект.Организация, ДокументОбъект.Дата);
...
Функция ПодготовитьСтруктуруДляСчетаПокупателю(СтрокаДляЗагрузки, ДеревоРазбора)
	...
	Для Каждого СтрокаРеквизита Из СтрокаДляЗагрузки.Строки Цикл
		...
		Иначе
			...
			ВидОперацииПоСтроке = ?(
				ТипЗнч(ДанныеДляЗаполненияСтрокиТЧ.Номенклатура) = Тип("СправочникСсылка.ОсновныеСредства"), 
					Перечисления.ВидыОперацийСчетаПокупателю.ОсновныеСредства,
					Перечисления.ВидыОперацийСчетаПокупателю.ТоварыИУслуги);
			
			Если ВидОперации = Неопределено Тогда
				ВидОперации = ВидОперацииПоСтроке;
			ИначеЕсли ВидОперации <> ВидОперацииПоСтроке Тогда
				Продолжить;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидОперации) Тогда
		ДанныеЗаполненияШапки.Вставить("ВидОперации", ВидОперации);
	КонецЕсли;
...
Функция ЗапросКонструктораДополнительныхПолейШапкиПервичногоДокумента()
	...
	|	РеализацияТоваровУслуг.ПередачаДляСобственныхНуждИСМП КАК ПередачаДляСобственныхНуждИСМП,
...
//
Функция НайтиСсылкуНаОбъектПоРеквизиту(ИмяСправочника, ИмяРеквизита, ЗначРеквизита,
	Владелец = Неопределено, ДопРеквизит = Неопределено)
	...
	Если ДопРеквизит <> Неопределено Тогда
		Индекс = 0;
		Для Каждого КлючИЗначение Из ДопРеквизит Цикл
			ПараметрЗапроса = "ДопРеквизит" + Формат(Индекс, "ЧГ=");
			Запрос.Текст = Запрос.Текст + " И ИскСправочник." + КлючИЗначение.Ключ + " = &" + ПараметрЗапроса;
			Запрос.УстановитьПараметр(ПараметрЗапроса, КлючИЗначение.Значение);
			Индекс = Индекс + 1;
		КонецЦикла;
	КонецЕсли;
...
Функция ИНН_КППОбъектовСовпадает(Ссылка1, Ссылка2)
	
	Если Не (ЗначениеЗаполнено(Ссылка1) И ЗначениеЗаполнено(Ссылка2)) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ИменаРеквизитов = Новый Массив;
	ИменаРеквизитов.Добавить("ИНН");
	ИменаРеквизитов.Добавить("КПП");
	Если Не (ЕстьРеквизитыОбъекта(Ссылка1, ИменаРеквизитов) И ЕстьРеквизитыОбъекта(Ссылка2, ИменаРеквизитов)) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	РеквизитыОбъектов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивСсылок, ИменаРеквизитов);
	
	Возврат СокрЛП(РеквизитыОбъектов[Ссылка1].ИНН) = СокрЛП(РеквизитыОбъектов[Ссылка2].ИНН)
		И СокрЛП(РеквизитыОбъектов[Ссылка1].КПП) = СокрЛП(РеквизитыОбъектов[Ссылка2].КПП);	
	
КонецФункции

Функция ЕстьРеквизитыОбъекта(Ссылка, Реквизиты)
	
	МетаданныеОбъекта = Ссылка.Метаданные();
	Для Каждого Реквизит Из Реквизиты Цикл
		Если Не ОбщегоНазначения.ЕстьРеквизитОбъекта(Реквизит, МетаданныеОбъекта) Тогда
			Возврат Ложь
		КонецЕсли;
	КонецЦикла;
