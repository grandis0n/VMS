...
Функция ПараметрыДоговораДляОбъекта(Знач СсылкаНаОбъект) Экспорт
	...
	Возврат ПечатьДоговоров.ПараметрыДоговораДляОбъекта(СсылкаНаОбъект);
...
Функция СообщенияЗаполненияОбязательныхРеквизитов(ДоговорСсылка, ФорматированныйДокумент = Неопределено) Экспорт
	...
	Если НастройкиОтправки[ДоговорСсылка] <> Неопределено Тогда
		...
		Если ФорматированныйДокумент = Неопределено И Не ЗначениеЗаполнено(ФайлДоговора) Тогда
			
			ТаблицаСообщений = ПечатьДоговоров.ТаблицаСообщений();
			СтрокаСообщений = ТаблицаСообщений.Добавить();
			СтрокаСообщений.РеквизитДоговорногоДокумента = НСтр("ru = 'Файл договора'");
			СтрокаСообщений.Описание = НСтр("ru = 'Файл договора не содержит текста для выгрузки. Сформируйте файл по кнопке ""Печать""'");
