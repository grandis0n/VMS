...
Процедура ТоварыНоменклатураПриИзменении(СтрокаТабличнойЧасти, Знач ДанныеОбъекта, Знач СчетаУчетаКЗаполнению) Экспорт
	...
	СведенияОНоменклатуреИС 							= ОбщегоНазначенияИС.ОписаниеНоменклатуры(СтрокаТабличнойЧасти.Номенклатура);
	ДанныеНоменклатурыИС 								= СведенияОНоменклатуреИС[СтрокаТабличнойЧасти.Номенклатура];
	СтрокаТабличнойЧасти.МаркируемаяПродукцияГосИС      = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(ДанныеНоменклатурыИС.КлючВидовПродукции);
	СтрокаТабличнойЧасти.ПродукцияМаркируемаяДляГИСМ	= КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ДанныеНоменклатурыИС.КлючВидовПродукции);
...
Процедура ОпределитьНеобходимостьРасчетаСуммыВРознице(Форма) Экспорт
	...
	//Поступление на несколько складов
	Если Форма.Объект.СкладВСписке И Форма.Объект.Товары.Количество() > 0 Тогда
