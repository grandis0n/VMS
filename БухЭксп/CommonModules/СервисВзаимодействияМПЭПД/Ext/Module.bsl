...
Функция СервисДобавитьПользователя(ИдентификаторЭДО, ДанныеВодителя) Экспорт
	...
	ПараметрыЗапроса.Вставить("LedVersion", ВерсияОбменаСМобильнымПриложением());
...
Функция СервисИзменитьПользователя(ИдентификаторЭДО, ДанныеВодителя) Экспорт
	...
	ИначеЕсли Не ЕстьБумажнаяДоверенность И ЗначениеЗаполнено(ДанныеВодителя.МЧД) Тогда
		...
		СтруктураДанныхДоверенности.Вставить("ДатаОкончания", ДанныеДоверенности.ДатаОкончания);
		СтруктураДанныхДоверенности.Вставить("СсылкаНаДоверенностьВРеестре", ДанныеДоверенности.СсылкаНаДоверенностьВРеестре);
		СтруктураДанныхДоверенности.Вставить("СсылкаНаРеестр", ДанныеДоверенности.СсылкаНаРеестр);
...
Процедура ДобавитьДанныеМобильногоПриложенияДляЗаписиВРеестр(КоллекцияЗаписейРегистра, ЗаписьИзФормы,
	...
	// СоответствиеЭлектронныхДокументов передается при записи новых документов.
	ЗапросСОтборомПоЭлектронномуДокументу = СоответствиеЭлектронныхДокументов <> Неопределено;
	ЭтоНовыйДокумент = СоответствиеЭлектронныхДокументов <> Неопределено;
	...
	Для Каждого ЗаписьРегистра Из КоллекцияЗаписейРегистра Цикл
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
			...
			Если Не ЭтоНовыйДокумент
				И (МассивДоступныхИндексов.Количество() = 0	Или Не ПоТитуламМогутБытьВзаимодействия) Тогда
				Продолжить;
			КонецЕсли;
		Иначе
			Если Не ИндексРаботыДоступен И Не ЭтоНовыйДокумент Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоНовыйДокумент Тогда
			МассивДоступныхВидовВзаимодействия = Новый Массив;
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаВодителя);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаВодителя);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаМедработникаДоРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаМедработникаДоРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаМеханика);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаМеханика);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаОтветственногоЗаОдометрДоРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаОтветственногоЗаОдометрДоРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаОтветственногоЗаОдометрПослеРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаОтветственногоЗаОдометрПослеРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ПривязкаМедработникаПослеРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.ОтвязкаМедработникаПослеРейса);
			МассивДоступныхВидовВзаимодействия.Добавить(Перечисления.ТипыВзаимодействияМПЭПД.Сообщение);
		Иначе
...
Процедура ПриПолученииСпискаИсходящихДокументовСервисаЭДО(СтруктураСоСпискомВходящих, СтруктураСоСпискомИсходящих, ИдентификаторУчетнойЗаписи) Экспорт
	...
	// ЭТрН
	МассивЗагружаемыхКодов.Добавить("CarrierInformationAcceptanceCargo");
	МассивЗагружаемыхКодов.Добавить("CarrierInformationDeliveryCargo");
	МассивЗагружаемыхКодов.Добавить("ConsigneeCancellationOfferReject");
	МассивЗагружаемыхКодов.Добавить("CarrierCancellationOfferReject");
...
Процедура ПередЗаписьюВходящегоСообщенияЭДО(СообщениеОбъект, ОписаниеСообщения, ЭлектронныйДокумент, НастройкиДокумента) Экспорт
	...
	ВозможныИзмененияВДокументеПоСвойствамСообщения =
		...
		И (ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЛ_ОткМ
			...
			Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2
			Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул4
			Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул4
			Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ОткП
			Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ОткТ3);
...
Функция ВерсияОбменаСМобильнымПриложением()
...
Функция АдресСервисаВзаимодействия()
	...
	Если ОбменСГИСЭПД.ДемоРежимВключен() Тогда
		Результат = "https://app-110604.1cmycloud.com";
	КонецЕсли;
...
Функция АдресВерсииИнтерфейса(ВерсияВзаимодействия = 1)
	...
	Если ОбменСГИСЭПД.ДемоРежимВключен() Тогда
		Результат = "applications/EDMS/api/v1";
		Возврат Результат;
	Иначе
...
Функция ФайлаМожетБытьОтправленСМП(ИдентификаторФайла)
	...
	ФайлаМожетБытьОтправленСМП = 
		...
		Или СтрНачинаетсяС(ИдентификаторФайла, "ON_TRNACLPPRIN")
		Или СтрНачинаетсяС(ИдентификаторФайла, "ON_TRNACLPVYN")
		Или СтрНачинаетсяС(ИдентификаторФайла, "DP_UVUTOCH");
...
Функция ПолучитьИДОтправителяДокументаПоИдФайл(ИдентификаторФайла)
	...
	// УОУ. Отправитель: медик, перевозчик, грузополучатель.
	Если СтрНачинаетсяС(ИдентификаторФайла, "DP_UVUTOCH") Тогда
		ИдентификаторОтправителя = МассивЧастейФайла[3];
		
	// ЭТрН Титул2. Отправитель перевозчик.
	ИначеЕсли СтрНачинаетсяС(ИдентификаторФайла, "ON_TRNACLPPRIN") Тогда
		ИдентификаторОтправителя = МассивЧастейФайла[4];
		
	// ЭТрН Титул4. Отправитель перевозчик.
	ИначеЕсли СтрНачинаетсяС(ИдентификаторФайла, "ON_TRNACLPVYN") Тогда
		ИдентификаторОтправителя = МассивЧастейФайла[4];
