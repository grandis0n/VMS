...
Процедура ВыполнитьПроверкуКодПоОКТМО(Проверка)
	...
	Если ЗначениеЗаполнено(Проверка.Адрес) Тогда
		...
		Если СтрДлина(КодОКТМОБюджетополучателя) <> ДлинаКодаОКТМОБюджетополучателя Тогда
			Возврат;
		ИначеЕсли Прав(КодОКТМОБюджетополучателя, 6) = "000000" Тогда
			// При проверке классификатор вернул код ОКТМО верхнего уровня. Пользователю следует перепроверить адрес.
			РезультатПроверки.Успешно = Ложь;
			РезультатПроверки.ТекстОшибки =
				НСтр("ru = 'Не удалось проверить код ОКТМО. Попробуйте перезаполнить адрес организации.'");
			Возврат;
