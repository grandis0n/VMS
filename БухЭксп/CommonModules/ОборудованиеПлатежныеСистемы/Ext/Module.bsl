...
Процедура ОчиститьПолностьюИсториюПлатежныхОпераций() Экспорт
	...
	РегистрыСведений.ПлатежныеОперации.ОчиститьРегистрПолностью();
		...
		Если ЧтениеXML.Имя = "Table" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
			Пока ЧтениеXML.Прочитать() Цикл  
				Если ЧтениеXML.Имя = "Record" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					КонецЕсли; 
					Если ЧтениеXML.ЗначениеАтрибута("ElectronicCertificateDiscount") <> Неопределено Тогда
						Операция.СкидкаЭлектроннымиСертификатами = Число(ЧтениеXML.ЗначениеАтрибута("ElectronicCertificateDiscount"));
					КонецЕсли;        
