...
Функция СчетаУчетаРасчетовПоставщиков() Экспорт
	...
	Возврат СчетаУчетаРасчетов(CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(2));
...
Функция СчетаУчетаРасчетовПокупателей() Экспорт
	...
	Возврат СчетаУчетаРасчетов(CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(1));
...
Функция СчетаУчетаТоваров(ИсключаемыеСчета = Неопределено) Экспорт
	...
	СчетаУчетаТоваров = CW_БухгалтерскийУчетМСФО.СформироватьМассивСубсчетов(СчетаУчетаТоваровПредопределенные);
...
// подробнее см. CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности().
// 
// Возвращаемое значение:
...
Функция СчетаРасчетовСПокупателямиССубсчетами() Экспорт
	...
	СчетаРасчетовСПокупателями.Добавить(ПланыСчетов.CW_МСФО.РасчетыСПокупателямиИЗаказчиками);
	СчетаРасчетовСПокупателями.Добавить(ПланыСчетов.CW_МСФО.РасчетыСПрочимиПокупателямиИЗаказчиками);
	СчетаРасчетовСПокупателями.Добавить(ПланыСчетов.CW_МСФО.РасчетыСПрочимиПокупателямиИЗаказчикамиВал);
	СчетаРасчетовСПокупателями.Добавить(ПланыСчетов.CW_МСФО.РасчетыСПрочимиПокупателямиИЗаказчикамиУЕ);
	СчетаРасчетовСПокупателями.Добавить(ПланыСчетов.CW_МСФО.РасчетыСРазнымиДебиторамиИКредиторами);
	
	Возврат CW_БухгалтерскийУчетМСФО.СформироватьМассивСубсчетов(СчетаРасчетовСПокупателями);
...
Функция БазовыеРеквизитыЗначенияПоУмолчанию() Экспорт
	...
	ЗначенияПоУмолчанию.Организация   = CW_БухгалтерскийУчетПереопределяемыйМСФО.ПолучитьЗначениеПоУмолчанию("ОсновнаяОрганизация");
...
Функция ЭтоТипЭлементаСоСложнымПодбором(ТипЭлемента) Экспорт
	...
	Если ТипЭлемента = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ТипЗначенияБанковскогоСчетаОрганизации() 
		ИЛИ ТипЭлемента = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ТипПодразделения() 
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.CW_ДоговорыПоГрузоперевозкам")
...
Процедура ИзменитьСхемуДляОтборовПоДопСвойствамСубконто(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Для каждого ЗапросПакета Из ПакетЗапросов Цикл
		...
		Если ЗапросПакета.ТаблицаДляПомещения = "CW_МСФООстаткиИОбороты" Тогда
		...
		Если ЗапросПакета.ТаблицаДляПомещения = "ОстаткиНаНачало"
		 ...
		 Или ЗапросПакета.ТаблицаДляПомещения = "CW_МСФООстаткиИОбороты" Тогда
...
Процедура ИнициализироватьКомпоновщикНастроекПриЗагрузкеПользовательскихНастроек(Форма, Настройки, ИмяВариантаНастроек = "") Экспорт
	...
	Если CW_БухгалтерскиеОтчетыКлиентСерверМСФО.КомпоновщикНастроекИнициализирован(Форма) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ОрганизацияИзменилась = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПроверитьОтложеннаяИнициализацияКомпоновщикаВызванаИзменениемОрганизации(Форма);
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.ИнициализацияКомпоновщикаНастроек(Форма, ОрганизацияИзменилась, ИмяВариантаНастроек);
	КонецЕсли;
...
Процедура ЗаполнитьПризнакиУчета(Форма) Экспорт
	...
	Если МассивСубконто.Количество() = 0 Тогда
		...
		Форма.ЕстьУчетПоПодразделениям = CW_БухгалтерскийУчетПереопределяемыйМСФО.ИспользоватьПодразделения();
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ CW_МСФО.Ссылка) КАК КоличествоСчетов,
	|	МАКСИМУМ(CW_МСФО.Количественный) КАК Количественный,
	|	МАКСИМУМ(CW_МСФО.Валютный) КАК Валютный,
	|	МАКСИМУМ(CW_МСФО.НалоговыйУчет) КАК НалоговыйУчет,
	|	МАКСИМУМ(CW_МСФО.УчетПоПодразделениям) КАК УчетПоПодразделениям
	|ИЗ
	|	ПланСчетов.CW_МСФО КАК CW_МСФО&УсловиеСоединения
	|ГДЕ
	|	&УсловиеОтбора";
	...
	Для каждого ВидСубконто Из МассивСубконто Цикл
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.CW_МСФО.ВидыСубконто КАК CW_МСФОВидыСубконто" + НомерСубконто + "
		|		ПО CW_МСФО.Ссылка = CW_МСФОВидыСубконто" + НомерСубконто + ".Ссылка";
		УсловиеОтбора = УсловиеОтбора 
			+ ?(НомерСубконто = 1, "", "
		|	И ") + "CW_МСФОВидыСубконто" + НомерСубконто + ".ВидСубконто = &ВидСубконто" + НомерСубконто;
	...
	Пока ВыборкаСчета.Следующий() Цикл
		...
		Форма.ЕстьУчетПоПодразделениям = ?(ВыборкаСчета.УчетПоПодразделениям = Истина,
			CW_БухгалтерскийУчетПереопределяемыйМСФО.ИспользоватьПодразделения(), Ложь);
...
// Параметры:
//  Форма        - УправляемаяФорма - имеет основной реквизит "Отчет".
//  Разделитель  - ПланСчетовСсылка.CW_МСФО, ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО - разделитель
...
Процедура СохранитьОписаниеНастроек(Форма, Разделитель) Экспорт
	...
	Если ИмяРазделителя = "CW_ВидыСубконтоМСФО" Тогда
		...
		КоличествоСубконтоМаксимум = CW_БухгалтерскийУчетМСФО.МаксимальноеКоличествоСубконто();
	...
	Для каждого Показатель Из КоллекцияНастроек Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если ИмяРазделителя = "CW_ВидыСубконтоМСФО"
	...
	// Сортировка.
	Если ИмяРазделителя = "CW_МСФО" Тогда
		ДанныеСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(Форма.СчетИсходноеЗначение);
...
Процедура ПрочитатьОписаниеНастроек(Форма, Разделитель, ТолькоОбновление = Ложь) Экспорт
	...
	Иначе // прочитать ранее сохраненные
		...
		Если ОписаниеНастройки = Неопределено Тогда // нет ранее сохраненного описания
			...
			CW_БухгалтерскиеОтчетыВызовСервераМСФО.ОбновитьОписаниеОтборовОтчета(Форма);
			Возврат;
		...
		Иначе // используем ранее сохраненное описание настроек
			...
			ИначеЕсли ИмяРазделителя <> "CW_МСФО" Тогда
	...
	ИначеЕсли ИмяРазделителя = "CW_ВидыСубконтоМСФО" Тогда
		...
		ЕстьУчетПоПодразделениям = CW_БухгалтерскийУчетПереопределяемыйМСФО.ИспользоватьПодразделения();
	...
	Если ОписаниеНастройки.Свойство("Группировка", КоллекцияВключенныхНастроек) Тогда
		...
		Для НомерСубконто = 1 По КоличествоСубконто Цикл
			...
			Если ИспользоватьОднуНоменклатурнуюГруппу 
				И ДанныеСчета["ВидСубконто" + НомерСубконто] = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.НоменклатурныеГруппы Тогда
				ИсключаемыеНастройки.Добавить("Субконто" + НомерСубконто);
			ИначеЕсли НЕ ВестиУчетПоДоговорам 
				И ДанныеСчета["ВидСубконто" + НомерСубконто] = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры Тогда
				ИсключаемыеНастройки.Добавить("Субконто" + НомерСубконто);
		...
		КонецЦикла;
		Для НомерСубконто = КоличествоСубконто + 1 По CW_БухгалтерскийУчетМСФО.МаксимальноеКоличествоСубконто() Цикл
			ИсключаемыеНастройки.Добавить("Субконто" + НомерСубконто);
		КонецЦикла;
	...
	Если ОписаниеНастройки.Свойство("ДополнительныеПоля", КоллекцияВключенныхНастроек) Тогда
		...
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.ЗаполнитьДополнительныеПоляИзНастроек(Отчет.КомпоновщикНастроек, ШаблонСпискаПолей, Отчет.Группировка);
...
Процедура ПреобразоватьОтборыПоСчету(Отбор, ИсходныйСчет, НовыйСчет) Экспорт
	...
	ДанныеИсходногоСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(ИсходныйСчет);
	ДанныеНовогоСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(НовыйСчет);
	ИспользоватьОднуНоменклатурнуюГруппу = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ИспользоватьОднуНоменклатурнуюГруппу();
	ВестиУчетПоДоговорам = РаботаСДоговорамиКонтрагентовБПВызовСервера.ВестиУчетПоДоговорам();
	...
	// Проекция измерений.
	Если CW_БухгалтерскийУчетПереопределяемыйМСФО.ИспользоватьПодразделения() Тогда
		...
		Для НомерСубконто = 1 По ДанныеНовогоСчета.КоличествоСубконто Цикл
			Если ДанныеИсходногоСчета["ВидСубконто" + НомерСубконтоИсходного] = ДанныеНовогоСчета["ВидСубконто" + НомерСубконто] Тогда
				...
				Если (Не ИспользоватьОднуНоменклатурнуюГруппу 
					Или ДанныеНовогоСчета["ВидСубконто" + НомерСубконто] <> ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.НоменклатурныеГруппы)
				   И (ВестиУчетПоДоговорам 
					Или ДанныеНовогоСчета["ВидСубконто" + НомерСубконто] <> ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры) Тогда
...
//		* Счет - ПланСчетовСсылка.CW_МСФО - Счет учета, для которого заданы настройки.
...
Функция УникальныеНастройкиОтчетаПоСчетам(ГруппировкиОтчета) Экспорт
	...
	ГруппировкиПоСубсчетам.Колонки.Добавить("Субсчет", Новый ОписаниеТипов("ПланСчетовСсылка.CW_МСФО"));
	...
	Для каждого СтрокаГруппировки Из ГруппировкиОтчета Цикл
		...
		Субсчета = CW_БухгалтерскийУчетПовтИспМСФО.СчетаВИерархии(СтрокаГруппировки.Счет);
...
Функция ИмяОтчета(Форма) Экспорт
	...
	ИмяОтчета = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИмяОтчетаПоИмениФормы(Форма);
...
Процедура УдалитьАвтоотступИзУсловногоОформления(УсловноеОформление) Экспорт
	...
	Для Каждого ЭлементУсловногоОформления Из УсловноеОформление.Элементы Цикл
		ПараметрОформления = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(ЭлементУсловногоОформления.Оформление, "Автоотступ");
...
Функция ЗаписьВЖурналОРасхожденииИтогов(МестоРасхождения, ТипСобытия) Экспорт
	...
	ПредставлениеПериода = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПредставлениеПериода(
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия,
		УровеньВажности,
		Метаданные.РегистрыБухгалтерии.CW_МСФО,
		,
		КомментарийСобытия);
...
Процедура ДополнитьКонтрольноеСоотношение(РезультатОтчета, ОбластьИтогов, КонтрольноеСоотношение) Экспорт
	...
	Для НомерСтроки = ОбластьИтогов.ПерваяИзСтрокИтогов По ОбластьИтогов.ПоследняяИзСтрокИтогов Цикл
		...
		Для каждого КолонкаИтога Из ОбластьИтогов.ФлагиОбщегоИтога Цикл
			...
			СуммаИтога = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.СтрокаСФорматомВЧисло(ЯчейкаИтога.Текст, ОбластьИтогов.РазделительДробнойЧасти);
...
//               - ПланСчетовСсылка.CW_МСФО - счет, список субконто которого анализируется.
...
Функция ИспользуютсяОборотныеСубконто(ИсточникВидовСубконто, Отбор = Неопределено) Экспорт
	...
	Если ТипСпискаСубконто = Тип("ПланСчетовСсылка.CW_МСФО") Тогда
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	CW_МСФОВидыСубконто.Ссылка КАК Ссылка
	|ИЗ
	|	ПланСчетов.CW_МСФО.ВидыСубконто КАК CW_МСФОВидыСубконто
	|ГДЕ
	|	CW_МСФОВидыСубконто.ВидСубконто В(&ВидыСубконто)
	|	И CW_МСФОВидыСубконто.ТолькоОбороты";
...
Процедура ПересчитатьИтоги(МестоРасхождения, АдресРезультата) Экспорт
	...
	РегистрСИтогами = РегистрыБухгалтерии.CW_МСФО;
...
//
// Параметры:
//	ДанныеРасшифровки - Структура - Структура, формируемая CW_БухгалтерскиеОтчетыВызовСервераМСФО.СформироватьОтчет() с ключами:
...
Функция СчетаУчетаРасчетов(ИсключенныеСчета)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	CW_МСФОВидыСубконто_Контрагенты.Ссылка КАК Счет,
	               |	CW_МСФОВидыСубконто_Договоры.Ссылка ССЫЛКА ПланСчетов.CW_МСФО КАК ЕстьДоговор
	               |ПОМЕСТИТЬ СчетаКонтрагентыИДоговоры
	               |ИЗ
	               |	ПланСчетов.CW_МСФО.ВидыСубконто КАК CW_МСФОВидыСубконто_Контрагенты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.CW_МСФО.ВидыСубконто КАК CW_МСФОВидыСубконто_Договоры
	               |		ПО CW_МСФОВидыСубконто_Контрагенты.Ссылка = CW_МСФОВидыСубконто_Договоры.Ссылка
	               |ГДЕ
	               |	CW_МСФОВидыСубконто_Контрагенты.ВидСубконто = &ВидСубконтоКонтрагенты
	               |	И CW_МСФОВидыСубконто_Договоры.ВидСубконто = &ВидСубконтоДоговоры
	               |	И НЕ CW_МСФОВидыСубконто_Контрагенты.Ссылка В (&ИсключенныеСчета)
	               |	И НЕ CW_МСФОВидыСубконто_Контрагенты.Ссылка.Забалансовый
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	CW_МСФОВидыСубконто_Документы.Ссылка
	               |ПОМЕСТИТЬ СчетаДокументыРасчетов
	               |ИЗ
	               |	ПланСчетов.CW_МСФО.ВидыСубконто КАК CW_МСФОВидыСубконто_Документы
	               |ГДЕ
	               |	CW_МСФОВидыСубконто_Документы.ВидСубконто = &ВидСубконтоДокументРасчетов
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	СчетаКонтрагентыИДоговоры.Счет КАК Счет,
	               |	ЕСТЬNULL(СчетаДокументыРасчетов.Ссылка ССЫЛКА ПланСчетов.CW_МСФО, ЛОЖЬ) КАК ЕстьДокументРасчетов
	               |ПОМЕСТИТЬ Счета
	               |ИЗ
	...
	Запрос.УстановитьПараметр("ВидСубконтоКонтрагенты", 			ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Контрагенты);
	Запрос.УстановитьПараметр("ВидСубконтоДоговоры", 				ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры);
	Запрос.УстановитьПараметр("ВидСубконтоДокументРасчетов", 		ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.ДокументыРасчетовСКонтрагентами);
...
Функция ОписаниеСортировки(КоллекцияНастроек, ДанныеСчета = Неопределено)
	...
	КоллекцияВключенныхНастроек.Колонки.Добавить("ВидСубконто",
		Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО"));
...
Функция ИмяСобытияЖурнала(ПолучательСобытия, ТипСобытия)
	...
	Если ПолучательСобытия = "ЦентрКонтроляКачества" Тогда
		...
		ИмяСобытия = НСтр("ru = 'СтатистикаБП.Ошибки.РасхожденияИтогов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		// Тип события используется в ключе уникальности.
		
	ИначеЕсли ПолучательСобытия = "ТехнологическийЖурнал" Тогда
		
		Если ТипСобытия = "Ошибка" Тогда
			ИмяСобытия = НСтр("ru = 'РасхождениеИтогов.Ошибка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Иначе
			ИмяСобытия = НСтр("ru = 'РасхождениеИтогов.Пересчет'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
		
	Иначе
		
		Если ТипСобытия = "Ошибка" Тогда
			ИмяСобытия = НСтр("ru = 'Расхождение итогов.Ошибка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Иначе
			ИмяСобытия = НСтр("ru = 'Расхождение итогов.Пересчет'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
