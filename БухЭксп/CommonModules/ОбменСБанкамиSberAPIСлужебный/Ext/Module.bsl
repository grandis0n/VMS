...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтправитьДокументыПоСчету", Истина);
	...
	Методы.Вставить("ПроверитьНаличиеЗарплатныхДоговоровSberAPI", Истина);
...
// Отправляет электронные документы в банк
//
// Параметры:
...
//    * ПоказыватьОкноПодтвержденияПлатежей - Булево - признак необходимости вывода окна.
//    * ТребуетсяПовторнаяАутентификация - Булево - признак необходимости повторной аутентификации.
//
Функция ОтправитьДокументы(НастройкаОбмена, ТекущаяСессия, МассивСообщенийОбмена) Экспорт
	...
	Для Каждого КлючЗначение Из БанковскиеСчета Цикл
		...
		// Признак отклонения документа банком в момент отправки, т.е. неуспешная отправка
		ОтклонитьДокумент = Неопределено;
		
		ПараметрыЭД = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыОткрытияЭлектронногоДокумента(КлючЗначение.Ключ);
		Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение Тогда
		
			ОтправитьПлатежныйДокумент(НастройкаОбмена, ТокенДоступа, ДанныеЭД, Результат, ТребуетсяАутентификация);
			
		ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников Тогда
			
			ОтправитьЗарплатнуюВедомость(НастройкаОбмена, ТокенДоступа, ДанныеЭД, Результат, ТребуетсяАутентификация);
			
			// Для ведомостей, не принятых сразу в момент отправки, скажем неверный номер договора,
			//  нужно выдать сообщение об ошибке и отклонить документ внутри 1С и поменять статус для электронного документа
			//  и поменять состояние для учетного документа
			ОтклонитьДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ОтклонитьДокумент", Неопределено);
			Если Не ТребуетсяАутентификация И ОтклонитьДокумент = Истина Тогда
				
				// Если статус по каким-то причинам не может быть изменен,
				// то не надо приостанавливать весь цикл.
				НачатьТранзакцию();
				
				Попытка
				
					// Поменяем статус электронного документа
					СтруктураПараметров = Новый Структура;
					СтруктураПараметров.Вставить("Статус", Перечисления.СтатусыОбменСБанками.ОтклоненБанком);
					СтруктураПараметров.Вставить("ПричинаОтклонения", Результат.ПричинаОтклонения);
					ОбменСБанкамиСлужебныйВызовСервера.ИзменитьСообщениеОбмена(КлючЗначение.Ключ, СтруктураПараметров);
					
					// Поменяем состояние документа учета
					ДокументУчета = ОбменСБанкамиСлужебныйВызовСервера.ДокументУчета(КлючЗначение.Ключ);
					Если ЗначениеЗаполнено(ДокументУчета) Тогда
						
						МенеджерЗаписи = РегистрыСведений.СостоянияОбменСБанками.СоздатьМенеджерЗаписи();
						МенеджерЗаписи.СсылкаНаОбъект  = ДокументУчета;
						МенеджерЗаписи.СообщениеОбмена = КлючЗначение.Ключ;
						МенеджерЗаписи.Состояние = Перечисления.СостоянияОбменСБанками.Отклонен;
				...
				Исключение
					...
					ОтменитьТранзакцию();
					ШаблонСообщения = НСтр("ru = 'Ошибка смены статуса на Отклонен для %1'");
					КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
					ТекстДляСообщения = СтрШаблон(ШаблонСообщения, Строка(КлючЗначение.Ключ));
					ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "") + ТекстДляСообщения;
					ШаблонВидаОперации = НСтр("ru = 'Попытка смены статуса на Отклонен для %1'");
					ВидОперации = СтрШаблон(ШаблонВидаОперации, Строка(КлючЗначение.Ключ));
					ПодробноеПредставлениеОшибки = ТекстСообщения
						+ Символы.ПС
						+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, , КлючЗначение.Ключ);
		...
		Если ОтклонитьДокумент = Неопределено Тогда
			
			Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение Тогда
				УстановитьСтатусДокумента(КлючЗначение.Ключ, Результат);
			ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников Тогда
				УстановитьСтатусВедомостьНаЗачислениеЗП(КлючЗначение.Ключ, Результат);
			КонецЕсли;
			// Проверим статус документа, если Отклонен, то такой документ не будем считать отправленным
			// Например для з/п ведомости, отправка могла пройти успешно, но банк тем не менее, сразу вернул назад статус Отклонено
			Статус = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КлючЗначение.Ключ, "Статус");
			Если Статус <> Перечисления.СтатусыОбменСБанками.Отклонен
				И Статус <> Перечисления.СтатусыОбменСБанками.ОтклоненБанком Тогда
...
//
Процедура ОтправитьДокументыПоСчету(Параметры, Адрес) Экспорт
	
	Результат = ОтправитьДокументы(
...
Процедура ОтправитьПлатежныйДокумент(НастройкаОбмена,
	...
	Если Не РезультатВыполнения.Статус И РезультатВыполнения.КодСостояния <> 201 Тогда
		...
		ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация);
...
// Формирует дайджест из электронного документа.
//
// Параметры:
...
Функция ДайджестЭлектронногоДокумента(ВидЭД, АдресФайлаВоВременномХранилище) Экспорт
	...
	Если ТипЗнч(ДвоичныеДанныеФайла) <> Тип("ДвоичныеДанные") Тогда
		...
		ВидОперации = НСтр("ru = 'Получение дайджеста электронного документа SberAPI'");
		Шаблон = НСтр("ru = 'Не удалось получить дайджест электронного документа SberAPI для %1'");
		ТекстОшибки = СтрШаблон(Шаблон, ВидЭД);
		ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
				ВидОперации, ТекстОшибки, ТекстОшибки);
	...
	ИначеЕсли ВидЭД = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников Тогда
		...
		СтрокаВозврата = ДайджестЗарплатнойВедомости(ДвоичныеДанныеФайла);
...
// Заполняет данные дайджеста зарплатной ведомости.
//
// Параметры:
//   ДанныеВедомости - Структура - данные зарплатной ведомости, куда требуется добавить дайджест.
//   ДайджестBase64 - Строка - дайджест данных зарплатной ведомости в Base64.
//   ИдентификаторСертификата - Строка - идентификатор сертификата.
//
Процедура ЗаполнитьДанныеДайджестаЗарплатнойВедомости(ДанныеВедомости, ДайджестBase64, ИдентификаторСертификата) Экспорт
	...
	ДанныеВедомости.Вставить("digestSignatures", digestSignatures);
	
КонецПроцедуры

// Дополняет json-файл зарплатной ведомости данными дайджеста, само подписание дайджеста не происходит.
//
// Параметры:
//    СсылкаНаОбъект - ДокументСсылка - ссылка на документ, на основании которого сформирован ЭД.
//    ИмяФайла - Строка - имя сформированного файла.
//    АдресФайла - Строка - АдресВременногоХранилища, содержит двоичные данные файла.
//
Процедура ДополнитьПодписьюЗарплатнуюВедомость(СсылкаНаОбъект, ИмяФайла, АдресФайла) Экспорт
	
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресФайла);
	Если ТипЗнч(ДвоичныеДанные) <> Тип("ДвоичныеДанные") Тогда
		
		ВидОперации = НСтр("ru = 'Дополнение дайджеста подписью з/п ведомость SberAPI'");
		Шаблон = НСтр("ru = 'Не удалось получить дайджест з/п ведомости SberAPI для %1'");
		ТекстОшибки = СтрШаблон(Шаблон, СсылкаНаОбъект);
		ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
				ВидОперации, ТекстОшибки, ТекстОшибки, СсылкаНаОбъект);
		
		Возврат;
		
	КонецЕсли;
	ДанныеФайла = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанные);
	JSONСтруктура = ОбщегоНазначения.JSONВЗначение(ДанныеФайла, , Ложь);
	
	ДайджестЗарплатнойВедомости = ДайджестЗарплатнойВедомостиИзСтруктуры(JSONСтруктура);
	ДайджестЗарплатнойВедомостиBase64 = ОбменСБанкамиСлужебныйВызовСервера.СтрокаBase64БезBOM(
			ДайджестЗарплатнойВедомости);
	ДайджестЗарплатнойВедомостиBase64 = СтрЗаменить(
			СтрЗаменить(ДайджестЗарплатнойВедомостиBase64, Символы.ВК, ""), Символы.ПС, "");
			
	ЗаполнитьДанныеДайджестаЗарплатнойВедомости(JSONСтруктура, ДайджестЗарплатнойВедомостиBase64, "uuid");
	
	СтрокаJSON = ОбменСБанкамиСлужебный.ЗначениеВJSON(JSONСтруктура);
	// Необходимо передавать null как значение, а не строковой литерал
	СтрокаJSON = СтрЗаменить(СтрокаJSON, """null""", "null");
	НовыеДвоичныеДанные = ДвоичныеДанныеИзСтроки(СтрокаJSON);
	
	АдресФайла = ПоместитьВоВременноеХранилище(НовыеДвоичныеДанные);
...
//   * ЭтоСинхронизация - Булево - признак, что процедура вызывается на нажатие команды Синхронизации.
...
Процедура ПолучитьИУстановитьСтатусДокумента(Параметры, Адрес) Экспорт
	...
	// Так как алгоритмы получения статусов для платежного поручения и зарплатной ведомости сильно отличаются, то разделим
	//  этот процесс на отдельные процедуры
	ВидЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.СообщениеОбмена, "ВидЭД");
	Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников Тогда
		
		ПолучитьИУстановитьСтатусЗарплатнойВедомости(Параметры, Адрес);
		
	ИначеЕсли ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение Тогда
		
		ПолучитьИУстановитьСтатусПлатежногоПоручения(Параметры, Адрес);
		
	КонецЕсли;

КонецПроцедуры

// Получает из банка статус платежного поручения
//
// Параметры:
//   НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена, если она есть.
...
//   ТребуетсяАутентификация - Булево - если Истина, то необходимо пройти процесс аутентификации.
//   ЭтоСинхронизация - Булево - если Истина, то функция вызывается из Синхронизации.
//
// Возвращаемое значение:
//   Структура - данные, полученные из сервиса SberAPI.
//
Функция ПолучитьСтатусПлатежногоПоручения(НастройкаОбмена,
								ТокенДоступа,
								ИдентификаторДокумента,
								ТребуетсяАутентификация,
								ЭтоСинхронизация)
	
	ПараметрыПодключения = СлужебныеПараметрыПодключения(НастройкаОбмена, "ПолучитьСтатусПлатежногоПоручения");
	...
	ЕстьОшибкаПозволяющаяПропуститьДокумент = Ложь;
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			ВидОперации = НСтр("ru = 'Получение данных через сервис SberAPI. Получение статуса платежного поручения.'");
			ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация);
		...
		КонецЕсли;
			...
			// Для ошибки 404 не будем выводит сообщение пользователю
			Если РезультатВыполнения.КодСостояния = 404 Тогда
				ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
					ВидОперации, ТекстОшибки, , НастройкаОбмена);
			Иначе
				ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
		...
		// Исключим Ошибку 404, которая означает, что на стенде не найден документ с таким ИД - в этом случае вызывать
		//  исключение нет необходимости
		Если РезультатВыполнения.КодСостояния = 404 Тогда
			ЕстьОшибкаПозволяющаяПропуститьДокумент = Истина;
		КонецЕсли;
		Если Не ЭтоСинхронизация И Не ЕстьОшибкаПозволяющаяПропуститьДокумент Тогда
	...
	Если ЕстьОшибкаПозволяющаяПропуститьДокумент Тогда
		Результат.Вставить("ЕстьОшибкаПозволяющаяПропуститьДокумент", Истина);
	КонецЕсли;
...
Процедура ПолучитьСтатусыДокументов(Параметры, Адрес) Экспорт
	...
	ПолучитьСтатусыПлатежныхПорученийИПоследниеВыписки(Параметры, СтруктураВозврата);

	ПолучитьСтатусыЗарплатныхВедомостей(Параметры, СтруктураВозврата);
...
Процедура ПроверитьНаличиеДоступаКДанным(Параметры, Адрес) Экспорт
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			ТекстОшибки = СтрШаблон(Шаблон, РезультатВыполнения.СообщениеОбОшибке, ПодробноеПредставлениеОшибки);
			ВидОперации = НСтр("ru = 'Получение данных через сервис SberAPI. Проверка наличия доступа к данным.'");
			ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
				ВидОперации, РезультатВыполнения.СообщениеОбОшибке, ТекстОшибки, Параметры.НастройкаОбмена);
			
			ПроверитьНеобходимостьАутентификации(
				ВидОперации, РезультатВыполнения, Параметры.НастройкаОбмена, ТребуетсяАутентификация);
...
// Получает информацию по имеющимся зарплатным договорам через сервис SberAPI.
//
// Параметры:
//  Параметры - Структура - параметры выполнения процедуры в фоне:
//   * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//   * ТокенДоступа - Строка - токен доступа для обмена с банком по SberAPI.
//   * Организация - СправочникСсылка.Организации - текущая организация.
//   * Банк - СправочникСсылка.Банки - текущий банк.
//   * ИгнорироватьСохраненныеНастройки - Булево - параметр передается в случае, если метод вызывается в момент 
//     создания настройки обмена, когда нужно параметры создания взять из файла JSON, а не
//     из справочника НастройкиОбменСБанками.
//  АдресРезультата - Строка - адрес временного хранилища с результатом выполнения
//
Процедура ПроверитьНаличиеЗарплатныхДоговоровSberAPI(Параметры, АдресРезультата) Экспорт
	
	ПараметрыПодключения = СлужебныеПараметрыПодключения(Параметры, "ПроверитьНаличиеЗарплатныхДоговоровSberAPI");
	...
	ОтветСервера = ПолучитьЗарплатныеДоговоры(ПараметрыПодключения.НастройкаОбмена, ТокенДоступа, СертификатПодключения);
	
	ПоместитьВоВременноеХранилище(ОтветСервера, АдресРезультата);
...
Процедура ПолучитьСтатусыПлатежныхПорученийИПоследниеВыписки(Параметры, СтруктураВозврата)
	...
	ЭтоСинхронизация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоСинхронизация", Ложь);
	...
	Пока ВыборкаПоСчетам.Следующий() Цикл
		...
		Пока ВыборкаИдентификаторов.Следующий() Цикл
			...
			ТребуетсяАутентификация = Ложь;
			Результат = ПолучитьСтатусПлатежногоПоручения(Параметры.НастройкаОбмена, ТокенДоступа,
				ВыборкаИдентификаторов.Идентификатор, ТребуетсяАутентификация, ЭтоСинхронизация);
			Если ТребуетсяАутентификация Тогда
				СтруктураВозврата.СчетаТребующиеАутентификацию.Добавить(ВыборкаПоСчетам.НомерСчета);
				Прервать;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(Результат)
				...
				И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ЕстьОшибкаПозволяющаяПропуститьДокумент", Ложь) = Истина) Тогда
...
Процедура УстановитьСтатусДокумента(СообщениеОбмена, Результат)
	...
	bankStatus = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "bankStatus", "");
	Если Не ЗначениеЗаполнено(bankStatus) Тогда
		Возврат;
	КонецЕсли;
	
	Если bankStatus = "ACCEPTED" Или bankStatus = "ACCEPTED_BY_ABS"
		Или bankStatus = "DELIVERED"
		Или bankStatus = "EXPORTED" // неактуальный статус
		Или bankStatus = "DELIVERED_RZK"
		Или bankStatus = "FRAUDALLOW" Или bankStatus = "SUBMITTED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Принят);
	ИначеЕсли bankStatus = "FRAUDREVIEW" Или bankStatus = "FRAUDSENT"
		Или bankStatus = "SENDING_TO_RZK" Или bankStatus = "PROCESSING_RZK"
		Или bankStatus = "PROCESSING" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Доставлен);
	ИначеЕсли bankStatus = "CARD2" Или bankStatus = "DELAYED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Приостановлен);
	ИначеЕсли bankStatus = "CREATED" Или bankStatus = "FRAUDSMS" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.НеПодтвержден);
	ИначеЕсли bankStatus = "IMPLEMENTED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Исполнен);
	ИначеЕсли bankStatus = "PARTSIGNED" Тогда
	...
	ИначеЕсли bankStatus = "SIGNED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Подписан);
	ИначеЕсли bankStatus = "CHECKERROR" Или bankStatus = "CHECKERROR_BANK"
		Или bankStatus = "FRAUDDENY" Или bankStatus = "INVALIDEDS"
		Или bankStatus = "REFUSED_BY_RZK" Или bankStatus = "NOT_ACCEPTED_RZK"
		Или bankStatus = "REFUSEDBYBANK" Или bankStatus = "RECALL"
		Или bankStatus = "REFUSEDBYABS" Или bankStatus = "REQUISITEERROR" Тогда
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("Статус", Перечисления.СтатусыОбменСБанками.ОтклоненБанком);
		bankComment = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "bankComment", "");
		СтруктураПараметров.Вставить("ПричинаОтклонения", bankComment);
		ОбменСБанкамиСлужебныйВызовСервера.ИзменитьСообщениеОбмена(СообщениеОбмена, СтруктураПараметров);
	ИначеЕсли bankStatus = "DELETED" Или bankStatus = "REQUESTED_RECALL" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Аннулирован);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПолучитьИУстановитьСтатусПлатежногоПоручения(Параметры, Адрес)
	...
	Результат = ПолучитьСтатусПлатежногоПоручения(
...
Функция ДайджестПлатежногоПорученияИзСтруктуры(Структура)
	...
	ОбязательныеПоляДайджеста = ОбязательныеПоляДайджестаПлатежногоПоручения();
	...
	Для Каждого СтрокаДайджеста Из ОбязательныеПоляДайджеста Цикл
		...
		ДобавитьОтформатированноеПоле(ИмяПоля, ЗначениеПоля, СтрокаДайджеста, СтрокаВозврата);
...
Функция ОбязательныеПоляДайджестаПлатежногоПоручения()
	
	ОбязательныеПоля = ТаблицаОбязательныеПоляДайджеста();
...
#Область ЗарплатныеВедомости

// Отправляет в банк зарплатную ведомость.
//
// Параметры:
//   НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//   ТокенДоступа - Строка - токен аутентификации на сервисе SberAPI.
//   ДанныеЭД - ДвоичныеДанные - двоичные данные зарплатной ведомости.
//   ОтветБанка - Структура - данные ответа из JSON.
//   ТребуетсяАутентификация - Булево - если Истина, то требуется повторная аутентификация.
//
Процедура ОтправитьЗарплатнуюВедомость(НастройкаОбмена,
									 ТокенДоступа,
									 ДанныеЭД,
									 ОтветБанка,
									 ТребуетсяАутентификация)
	
	ПараметрыПодключения = СлужебныеПараметрыПодключения(НастройкаОбмена, "ОтправитьЗарплатнуюВедомость");
	
	АдресРесурса = "fintech/api/v1/payrolls";
	...
	Если Не РезультатВыполнения.Статус И РезультатВыполнения.КодСостояния <> 201 Тогда
		...
		ВидОперации = НСтр("ru = 'Отправка зарплатной ведомости через SberAPI'");
		СообщениеОбОшибке = РезультатВыполнения.СообщениеОбОшибке;
		ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация);
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			КраткоеПредставлениеОшибки = ОбменСБанкамиSberAPIСлужебныйКлиентСервер.КороткоеПредставлениеОшибки(
				РезультатВыполнения.Тело);
			Шаблон = НСтр("ru = 'При отправке зарплатной ведомости произошла ошибка:
								|Сообщение об ошибке %1
								|Тело ответа: %2'");
			ТекстОшибки = СтрШаблон(Шаблон, СообщениеОбОшибке, ПодробноеПредставлениеОшибки);
			ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
		...
		// Для ведомостей, не принятых сразу в момент отправки, скажем неверный номер договора,
		//  нужно выдать сообщение об ошибке, отклонить документ внутри 1С и поменять статус для электронного документа
		//  и поменять состояние для учетного документа
		Если РезультатВыполнения.КодСостояния = 400 Тогда
			
			ОтветБанка = ОбщегоНазначения.JSONВЗначение(РезультатВыполнения.Тело, , Ложь);
			ОтветБанка.Вставить("ОтклонитьДокумент", Истина);
			ОтветБанка.Вставить("ПричинаОтклонения", КраткоеПредставлениеОшибки);
			Возврат;
...
Процедура ПолучитьИУстановитьСтатусЗарплатнойВедомости(Параметры, Адрес)
	
	Результат = Неопределено;
	ТребуетсяАутентификация = Ложь;
		
	Ведомость = ОбменСБанкамиСлужебный.ОбъектПривязки(Параметры.СообщениеОбмена);
	Если Не ЗначениеЗаполнено(Ведомость) Тогда
		
		Шаблон = НСтр("ru = 'При запросе статуса зарплатной ведомости не найден учетный документ для %1'");
		Сообщение = СтрШаблон(Шаблон, Строка(Параметры.СообщениеОбмена));
		
		ВидОперации = НСтр("ru = 'Запросе статуса зарплатной ведомости через SberAPI'");
		ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
			ВидОперации, Сообщение, Сообщение, Параметры.НастройкаОбмена);
	...
	ИдентификаторДокумента = Строка(Ведомость.УникальныйИдентификатор());
	
	Результат = ПолучитьОбщийСтатусВедомостьНаЗачислениеЗП(
		Параметры.НастройкаОбмена,
		Параметры.ТокенДоступа,
		ИдентификаторДокумента,
		ТребуетсяАутентификация);
	...
	ЕстьОшибкаПозволяющаяПропуститьДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Результат, "ЕстьОшибкаПозволяющаяПропуститьДокумент", Ложь);
	Если ЕстьОшибкаПозволяющаяПропуститьДокумент = Истина Тогда
	...
	УстановитьСтатусВедомостьНаЗачислениеЗП(Параметры.СообщениеОбмена, Результат);
	
	bankStatus = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "bankStatus", Неопределено);
	Если ЗначениеЗаполнено(bankStatus) И СтатусЗарплатнойВедомостиСоответствуетСохранению(bankStatus) Тогда
		
		ПодробныйРезультат = ПолучитьПодробныйСтатусВедомостьНаЗачислениеЗП(
			Параметры.НастройкаОбмена,
			Параметры.ТокенДоступа,
			ИдентификаторДокумента,
			ТребуетсяАутентификация);
		...
		Если ПодробныйРезультат <> Неопределено Тогда
			
			ДвоичныеДанныеФайла = ПолучитьДвоичныеДанныеИзСтроки(ПодробныйРезультат.Json);
			НовоеСообщениеОбмена = Неопределено;
			ДанныеВозврата = Неопределено;
			СохранитьПодтверждениеЗачисленияДенежныхСредствНаСчетаСотрудников(
					Параметры,
					Параметры.СообщениеОбмена,
					ДвоичныеДанныеФайла,
					ИдентификаторДокумента,
...
Процедура СохранитьПодтверждениеЗачисленияДенежныхСредствНаСчетаСотрудников(
	Параметры,
	СообщениеРодитель,
	ДвоичныеДанныеФайла,
	ИдентификаторДокумента,
	...
	ЕстьПоддержкаВидаЭДВКонфигурации = ОбменСБанкамиСлужебныйВызовСервера.ЕстьПоддержкаВидаЭДВКонфигурации(
		Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников);
	Если Не ЕстьПоддержкаВидаЭДВКонфигурации Тогда
		// Для конфигурации без зарплатных ведомостей данный электронный документ не нужен.
		Возврат;
	КонецЕсли;
	...
	ИмяФайла = "New.json";	// Имя файла это временная переменная для ОбменСБанкамиПереопределяемый.ПриПолученииXMLФайла,
							//  не несущая в себе значение, используемое далее
	...
	Если НЕ ЗначениеЗаполнено(ОбъектВладелец) Тогда
		ОбъектВладелец = Параметры.НастройкаОбмена;
	КонецЕсли;
	...
	СтруктураРеквизитов.Вставить("ВнешнийИдентификатор", ИдентификаторДокумента);
	...
	Если ЗначениеЗаполнено(СообщениеРодитель) Тогда
		...
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеРодитель, Перечисления.СтатусыОбменСБанками.ПолученоПодтверждение);
		...
		Если ЗначениеЗаполнено(ДокументУчета) Тогда
			...
			// Заменяем на учетный документ для отображения в подчиненных документах
			СтруктураРеквизитов.СсылкаНаОбъект = ДокументУчета;
...
Функция СтатусЗарплатнойВедомостиСоответствуетСохранению(bankStatus)
	
	МассивСтатусов = Новый Массив();				// Статус Банка								Статус 1С
	МассивСтатусов.Добавить("PARTIMPLEMENTED");		// Частично исполнен						Исполнен 
	МассивСтатусов.Добавить("IMPLEMENTED");			// Исполнен									Исполнен 
	МассивСтатусов.Добавить("REQUISITEERROR");		// Ошибка реквизитов 						ОтклоненБанком
	МассивСтатусов.Добавить("REFUSEDBYABS");		// Отказан АБС 								ОтклоненБанком 
	
	Возврат МассивСтатусов.Найти(ВРЕГ(bankStatus)) <> Неопределено;
	
КонецФункции

Процедура ПолучитьСтатусыЗарплатныхВедомостей(Параметры, СтруктураВозврата)
	...
	ИначеЕсли Параметры.Свойство("ТекущаяСессия", ТекущаяСессия) И ЗначениеЗаполнено(ТекущаяСессия) Тогда
		
		Если ТекущаяСессия.Свойство("ТокенДоступа", ТокенДоступа) И ЗначениеЗаполнено(ТокенДоступа) Тогда
		
			ТокенДоступа = Параметры.ТекущаяСессия.ТокенДоступа;
		
		ИначеЕсли ТекущаяСессия.Свойство("ДоступныеСчета", ДоступныеСчета) И ЗначениеЗаполнено(ДоступныеСчета) Тогда
			
			Для Каждого КлючИЗначение Из ТекущаяСессия.ДоступныеСчета Цикл
				ТокенДоступа = КлючИЗначение.Значение;
			КонецЦикла;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаДляПолученияСтатусовЗарплатныхВедомостей();
	Запрос.УстановитьПараметр("НастройкаОбмена", Параметры.НастройкаОбмена);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Параметры.Вставить("СообщениеОбмена", Выборка.Ссылка);
		
		ПолучитьИУстановитьСтатусЗарплатнойВедомости(Параметры, Неопределено);
		
		СтруктураВозврата.КоличествоПолученных = СтруктураВозврата.КоличествоПолученных + 1;
		
	КонецЦикла;
	
КонецПроцедуры

// Получает из банка общий статус электронного документа Ведомость на зачисление ЗП
//
// Параметры:
//   НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена, если она есть.
//   ТокенДоступа - Строка - токен аутентификации в сервисе SberAPI
//   ИдентификаторДокумента - Строка - идентификатор ведомости
//   ТребуетсяАутентификация - Булево - если Истина, то необходимо пройти процесс аутентификации.
//
// Возвращаемое значение:
//   Структура - данные, полученные из сервиса SberAPI.
//
Функция ПолучитьОбщийСтатусВедомостьНаЗачислениеЗП(НастройкаОбмена,
								ТокенДоступа,
								ИдентификаторДокумента,
								ТребуетсяАутентификация,
								ЭтоСинхронизация = Ложь)
	
	ПараметрыПодключения = СлужебныеПараметрыПодключения(НастройкаОбмена, "ПолучитьОбщийСтатусВедомостьНаЗачислениеЗП");
	
	АдресРесурса = "fintech/api/v1/payrolls/" + ИдентификаторДокумента + "/state";
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			ВидОперации = НСтр("ru = 'Получение данных через сервис SberAPI. Получение общего статуса з/п ведомости.'");
			ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация);
			...
			Шаблон = НСтр("ru = 'При получении общего статуса зарплатной ведомости через сервис SberAPI произошла ошибка.
								|Код ошибки: %1
								|Сообщение об ошибке: %2
								|Тело ответа: %3'");
			ТекстОшибки = СтрШаблон(Шаблон, РезультатВыполнения.КодСостояния, СообщениеОбОшибке, РезультатВыполнения.Тело);
...
// Получает из банка подробный статус электронного документа Ведомость на зачисление ЗП
//
// Параметры:
//   НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена, если она есть.
//   ТокенДоступа - Строка - токен аутентификации в сервисе SberAPI
...
//   Структура - данные, полученные из сервиса SberAPI.
//
Функция ПолучитьПодробныйСтатусВедомостьНаЗачислениеЗП(НастройкаОбмена,
								ТокенДоступа,
								ИдентификаторДокумента,
								ТребуетсяАутентификация)
	
	ПараметрыПодключения = СлужебныеПараметрыПодключения(НастройкаОбмена, "ПолучитьПодробныйСтатусВедомостьНаЗачислениеЗП");
	
	АдресРесурса = "fintech/api/v1/payrolls/" + ИдентификаторДокумента;
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			ВидОперации = НСтр("ru = 'Получение данных через сервис SberAPI. Получение подробного статуса з/п ведомости.'");
			ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация);
			...
			Шаблон = НСтр("ru = 'При получении подробного статуса документа через сервис SberAPI произошла ошибка.
								|Код ошибки: %1
								|Сообщение об ошибке: %2
								|Тело ответа: %3'");
			ТекстОшибки = СтрШаблон(Шаблон, РезультатВыполнения.КодСостояния, СообщениеОбОшибке, РезультатВыполнения.Тело);
	...
	Результат = Новый Структура();
	Результат.Вставить("Json", РезультатВыполнения.Тело);
	Результат.Вставить("Структура", ОбщегоНазначения.JSONВЗначение(РезультатВыполнения.Тело, , Ложь));
	
	Возврат Результат;
	
КонецФункции

Функция ТаблицаОбязательныеПоляДайджеста()
	
	ТаблицаОбязательныеПоляДайджеста = Новый ТаблицаЗначений();
	ТаблицаОбязательныеПоляДайджеста.Колонки.Добавить("ИмяПоляДайджеста");
	ТаблицаОбязательныеПоляДайджеста.Колонки.Добавить("ИмяПоляПлатежки");
	ТаблицаОбязательныеПоляДайджеста.Колонки.Добавить("ФорматнаяСтрока");
	
	Возврат ТаблицаОбязательныеПоляДайджеста;
	
КонецФункции

Функция ОбязательныеПоляДайджестаЗарплатнойВедомости()
	
	ОбязательныеПоля = ТаблицаОбязательныеПоляДайджеста();
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "account", "account");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "admissionValue", "admissionValue");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "amount.amount", "amount.amount");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "amount.currencyName", "amount.currencyName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "authPersonName", "authPersonName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "authPersonTelfax", "authPersonTelfax");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "bic", "bic");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "contractDate", "contractDate", "ДФ=yyyy-MM-dd");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "contractNumber", "contractNumber");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "date", "date", "ДФ=yyyy-MM-dd");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "employeesNumber", "employeesNumber");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "externalId", "externalId");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "incomeTypeCode", "incomeTypeCode");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "loanAmount", "loanAmount");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "loanDate", "loanDate", "ДФ=dd.MM.yyyy");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "loanNumber", "loanNumber");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "month", "month");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "orgName", "orgName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "orgTaxNumber", "orgTaxNumber");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "year", "year");
	
	Возврат ОбязательныеПоля;
	
КонецФункции

Функция ОбязательныеПоляДайджестаСотрудники()
	...
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "amount.currencyName", "amount.currencyName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "bic", "bic");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "firstName", "firstName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "lastName", "lastName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "middleName", "middleName");
	ДобавитьОбязательноеПолеДайджеста(ОбязательныеПоля, "withheldAmount", "withheldAmount");
	
	Возврат ОбязательныеПоля;
	
КонецФункции

Функция ДайджестЗарплатнойВедомости(ДвоичныеДанныеФайла)
...
Функция ДайджестЗарплатнойВедомостиИзСтруктуры(Структура)
	
	СтрокаВозврата = "";
	
	ОбязательныеПоляДайджестаЗарплатнойВедомости = ОбязательныеПоляДайджестаЗарплатнойВедомости();
	
	Для Каждого СтрокаДайджеста Из ОбязательныеПоляДайджестаЗарплатнойВедомости Цикл
		
		ИмяПоля = "";
		ЗначениеПоля = "";
		
		Если СтрНайти(СтрокаДайджеста.ИмяПоляДайджеста, "employeeSalaries") Тогда
			
			Продолжить;
		
		ИначеЕсли СтрНайти(СтрокаДайджеста.ИмяПоляДайджеста, "amount") Тогда
			
			amount = Неопределено;
			ИмяПодПоля = СтрЗаменить(СтрокаДайджеста.ИмяПоляДайджеста, "amount.", "");
			
			Если Структура.Свойство("amount", amount)
				И ЗначениеЗаполнено(amount)
				И amount.Свойство(ИмяПодПоля, ЗначениеПоля)
				И ЗначениеЗаполнено(ЗначениеПоля)
				И ЗначениеПоля <> "currencyCode" Тогда
	...
	СтрокаВозврата = СтрокаВозврата + "TABLES" + Символ(10);
	СтрокаВозврата = СтрокаВозврата + "Table=EmployeeSalaries" + Символ(10);
	
	ОбязательныеПоляДайджестаСотрудники = ОбязательныеПоляДайджестаСотрудники();
	МассивСтруктураСотрудники = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Структура, "employeeSalaries", Новый Массив);
	
	Для Каждого СтруктураСотрудники Из МассивСтруктураСотрудники Цикл
	
		Для Каждого СтрокаДайджеста Из ОбязательныеПоляДайджестаСотрудники Цикл
			
			ИмяПоля = "";
			ЗначениеПоля = "";
			
			Если СтрНайти(СтрокаДайджеста.ИмяПоляДайджеста, "amount") Тогда
				
				amount = Неопределено;
				ИмяПодПоля = СтрЗаменить(СтрокаДайджеста.ИмяПоляДайджеста, "amount.", "");
				
				Если СтруктураСотрудники.Свойство("amount", amount)
					И ЗначениеЗаполнено(amount)
					И amount.Свойство(ИмяПодПоля, ЗначениеПоля)
					И ЗначениеЗаполнено(ЗначениеПоля)
					И ЗначениеПоля <> "currencyCode" Тогда
			...
			ИначеЕсли СтруктураСотрудники.Свойство(СтрокаДайджеста.ИмяПоляДайджеста, ЗначениеПоля)
				И ЗначениеЗаполнено(ЗначениеПоля)
				И НРег(Строка(ЗначениеПоля)) <> "null" Тогда
...
Процедура ДобавитьОтформатированноеПоле(ИмяПоля, ЗначениеПоля, СтрокаДайджеста, СтрокаВозврата)
...
// Устанавливает статус ведомости на зачисление ЗП в соответствии со статусом в банке.
// Формат ответа банка для ЗП ведомости отличается от формата ответа для платежки, поэтому выделена отдельная процедура.
//
// Параметры:
//  СообщениеОбмена - ДокументСсылка.СообщениеОбменСБанками - текущий платежный документ.
//  Результат - Структура - ответ банка из JSON.
//
Процедура УстановитьСтатусВедомостьНаЗачислениеЗП(СообщениеОбмена, Результат)
	...
	Если bankStatus = "ACCEPTED" Или bankStatus = "ACCEPTED_BY_ABS"
		Или bankStatus = "DELIVERED"
		Или bankStatus = "EXPORTED" // неактуальный статус
		Или bankStatus = "FRAUDALLOW" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Принят);
		
	ИначеЕсли bankStatus = "FRAUDREVIEW" Или bankStatus = "FRAUDSENT"
		Или bankStatus = "SENDING_TO_RZK" Или bankStatus = "SENT_TO_RZK"
		Или bankStatus = "WAITING_FOR_RZK" Или bankStatus = "PROCESSING" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Доставлен);
	...
	ИначеЕсли bankStatus = "IMPLEMENTED" ИЛИ bankStatus = "PARTIMPLEMENTED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Исполнен);
		
	ИначеЕсли bankStatus = "PARTSIGNED" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.ЧастичноПодписан);
		
	ИначеЕсли bankStatus = "SIGNED_BANK" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Подписан);

	ИначеЕсли bankStatus = "INCONSISTENT_DATA" Или bankStatus = "UNABLE_TO_RECEIVE"
		Или bankStatus = "FRAUDDENY" Или bankStatus = "CHECKERROR"
		Или bankStatus = "INVALIDEDS" Или bankStatus = "REFUSEDBYBANK"
		Или bankStatus = "REFUSEDBYABS" Или bankStatus = "REQUISITEERROR"
		Или bankStatus = "REFUSED_BY_RZK" Тогда
		
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("Статус", Перечисления.СтатусыОбменСБанками.ОтклоненБанком);
		bankComment = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "bankComment");
		Если ЗначениеЗаполнено(bankComment) Тогда
			СтруктураПараметров.Вставить("ПричинаОтклонения", bankComment);
		КонецЕсли;
		// При получении статуса "INVALIDEDS", поле "bankComment" не возвращается банком,
		//  поэтому принудительно укажем причину отклонения
		Если bankStatus = "INVALIDEDS" Тогда
			СтруктураПараметров.Вставить("ПричинаОтклонения", НСтр("ru = 'Подпись неверна'"));
		КонецЕсли;
		ОбменСБанкамиСлужебныйВызовСервера.ИзменитьСообщениеОбмена(СообщениеОбмена, СтруктураПараметров);
		
	ИначеЕсли bankStatus = "TEMPLATE" Тогда
		ОбменСБанкамиСлужебный.УстановитьСтатусСообщенияОбмена(СообщениеОбмена, Перечисления.СтатусыОбменСБанками.Аннулирован);
	КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьЗарплатныеДоговоры(НастройкаОбмена, ТокенДоступа, СертификатПодключения)
	
	ТекстЗапроса = "";
	АдресРесурса = "fintech/api/v1/salary-agreements";
	...
	УдалитьВременныйФайлСертификата(СертификатПодключения.ФайлСертификата);
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			Шаблон = НСтр("ru = 'При получении данных об зарплатных договорах через сервис SberAPI произошла ошибка.
								|Сообщение об ошибке %1
								|Тело ответа: %2'");
			...
			ТекстОшибки = СтрШаблон(Шаблон, РезультатВыполнения.СообщениеОбОшибке, ПодробноеПредставлениеОшибки);
			ВидОперации = НСтр("ru = 'Получение данных через сервис SberAPI. Получение информации по з/п договорам.'");
			ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ОбработатьОшибку(
				ВидОперации, РезультатВыполнения.СообщениеОбОшибке, ТекстОшибки, НастройкаОбмена);
	...
	Возврат ОбщегоНазначения.JSONВЗначение(РезультатВыполнения.Тело, , Ложь);
	
КонецФункции

Функция ТекстЗапросаДляПолученияСтатусовЗарплатныхВедомостей()
	...
	|	И СообщениеОбменСБанками.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников)
...
Процедура ПолучитьИтогиПоВыписке(Параметры,
	...
	Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
		...
		ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, Параметры.НастройкаОбмена, ТребуетсяАутентификация);
...
Процедура ПолучитьСтраницуВыписки(Параметры,
	...
	Если Не РезультатВыполнения.Статус Тогда
		...
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			...
			ВидОперации = НСтр("ru = 'Получение страницы выписки через сервис SberAPI'");
			ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, Параметры.НастройкаОбмена, ТребуетсяАутентификация);
			...
			Шаблон = НСтр("ru = 'При получении страницы выписки произошла ошибка.
								|Код ошибки: %1
								|Сообщение об ошибке %2
								|Тело ответа: %3'");
			ТекстОшибки = СтрШаблон(Шаблон, РезультатВыполнения.КодСостояния, СообщениеОбОшибке, РезультатВыполнения.Тело);
...
Процедура ПроверитьНеобходимостьАутентификации(ВидОперации, РезультатВыполнения, НастройкаОбмена, ТребуетсяАутентификация)
