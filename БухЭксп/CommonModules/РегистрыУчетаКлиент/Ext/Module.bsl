...
Процедура СохранитьРегистрУчета(Форма, Знач Подписать = Ложь) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из ДанныеСохранения.СвойстваПрисоединенныхФайлов Цикл
		...
		ПараметрыОповещения = РаботаСФайламиСлужебныйКлиент.ПараметрыОповещенияЗаписиФайла();
		ПараметрыОповещения.Владелец = ДанныеСохранения.РегистрУчета;
		ПараметрыОповещения.ВладелецФайла = ДанныеСохранения.РегистрУчета;
		ПараметрыОповещения.Файл = ПрисоединенныйФайл.Ключ;
		ПараметрыОповещения.ЭтоНовый = Истина;
		Оповестить("Запись_Файл", ПараметрыОповещения, ПрисоединенныйФайл.Ключ);
