...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта, Знач СчетаУчетаКЗаполнению) Экспорт
	...
	Если СведенияОНоменклатуре <> Неопределено Тогда
		...
		Если ДанныеОбъекта.ИспользуютсяУпаковки Тогда
			...
			Если Не СведенияОНоменклатуре.ЕдиницаИзмерения = Неопределено Тогда
...
Процедура ЗаполнитьРассчитатьСуммы(Форма, 
	...
	Если Объект.Свойство("ИспользуютсяУпаковки") И Объект.Свойство("Товары") Тогда
		Если Форма.ИспользуютсяУпаковки Тогда
...
Функция ЭлектронныеДокументыПоОснованию(Объект) Экспорт
	...
	Возврат ОбменСГИСЭПДБП.ЭлектронныеДокументыПоОснованию(СписокОснования);
КонецФункции 
...
Процедура ПроверитьИзменитьАдресДоставки(Объект, ПредыдущаяДата) Экспорт
	...
	ЗначенияРеквизитов = Новый Структура("Контрагент, Грузополучатель");
	ЗаполнитьЗначенияСвойств(ЗначенияРеквизитов, Объект);
	
	СписокАдресов = СписокАдресовДоставки(ЗначенияРеквизитов.Контрагент, ЗначенияРеквизитов.Грузополучатель, ПредыдущаяДата);
	...
	СписокАдресов = СписокАдресовДоставки(ЗначенияРеквизитов.Контрагент, ЗначенияРеквизитов.Грузополучатель, Объект.Дата);
...
Процедура КонтрагентОбработатьИзменение(Форма, ПричиныИзмененияСчетовУчета)
	...
	Объект.ПередачаДляСобственныхНуждИСМП = НЕ РегистрыСведений.СтатусыКонтрагентовИСМП.УчастникСистемыМаркировки(Объект.Контрагент);
