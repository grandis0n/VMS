...
// Параметры:
//  СтандартнаяОбработка - Булево - выходной, переопределеяемый параметр.
...
Процедура ПриВыполненииКомандыСоздатьДокументПоЧерновику(СтандартнаяОбработка,
	Знач ЭлектронныйДокумент, Знач ЧерновикДокумента, Знач ОбработчикЗавершения) Экспорт

 	Данные = ОбменСГИСЭПДВызовСервера.ПолучитьДанныеДляТитулаНаОснованииЧерновика(ЭлектронныйДокумент,
		ЧерновикДокумента);
		
	Если Данные = Неопределено 
		Или Не Данные.ЭтоЭПДЧерновик Тогда
		Возврат;
	КонецЕсли;
	
	СтандартнаяОбработка = Ложь;
	
	Если Данные.Свойство("ДанныеДляЗаполнения") Тогда
	
		ПараметрыОткрытия = Новый Структура("ЗначенияЗаполнения", Данные.ДанныеДляЗаполнения);
		ОткрытьФорму(Данные.ИмяФормы, ПараметрыОткрытия);
		
		ИтогВыполненияДействийПоЭДО = ИнтерфейсДокументовЭДОКлиентСервер.НовыйИтогВыполненияДействийПоЭДО();
		ИтогВыполненияДействийПоЭДО.ОбработанныеДокументы.Вставить(ЭлектронныйДокумент, Истина);
		ВыполнитьОбработкуОповещения(ОбработчикЗавершения, ИтогВыполненияДействийПоЭДО);
...
Процедура ДокументыОснование_Закрытие(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		ИначеЕсли ДополнительныеПараметры.ТипДокумента = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
			ОбменСГИСЭПДКлиентПереопределяемый.ОбработкаЗаполненияНаФормеЭПЭ(ЭтотОбъект, Результат);
		ИначеЕсли ДополнительныеПараметры.ТипДокумента = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
			ОбменСГИСЭПДКлиентПереопределяемый.ОбработкаЗаполненияНаФормеЭЭР(ЭтотОбъект, Результат);
		ИначеЕсли ДополнительныеПараметры.ТипДокумента = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда
			ОбменСГИСЭПДКлиентПереопределяемый.ОбработкаЗаполненияНаФормеЭСР(ЭтотОбъект, Результат);
		КонецЕсли;
	КонецЕсли;
...
// Данные заполнения ЭТрН
// 
// Возвращаемое значение:
//  Структура - Данные для заполнения ЭТрН:
// * Организация               - ОпределяемыйТип.Организация                - Организация в ЭТрН
//                             - Неопределено                               - Не задан
// * Грузоотправитель          - ОпределяемыйТип.УчастникЭДО_ЭПД            - Грузоотправитель в ЭТрН
//                             - Неопределено                               - Не задан
// * Составитель               - ОпределяемыйТип.УчастникЭДО_ЭПД            - Составитель в ЭТрН
//                             - Неопределено                               - Не задан
// * Заказчик                  - ОпределяемыйТип.КонтрагентЭПД              - Заказчик в ЭТрН
//                             - Неопределено                               - Не задан
// * Грузополучатель           - ОпределяемыйТип.УчастникЭДО_ЭПД            - Грузополучатель в ЭТрН
//                             - Неопределено                               - Не задан
// * Перевозчик                - ОпределяемыйТип.УчастникЭДО_ЭПД            - Перевозчик в ЭТрН
//                             - Неопределено                               - Не задан
// * ДокументОснование         - ОпределяемыйТип.ДокументыОснованияЭТрН_ЭПД - Документ основание ЭТрН
//                             - Неопределено                               - Не задан
// * ТаблицаИнформационныеПоля - Строка - Адрес временного хранилища Таблицы инфополей ЭТрН
//                             - Неопределено                               - Не задан
Функция НовыеДанныеЗаполненияЭТрН() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("Организация"              , Неопределено);
	Результат.Вставить("Грузоотправитель"         , Неопределено);
	Результат.Вставить("Составитель"              , Неопределено);
	Результат.Вставить("Заказчик"                 , Неопределено);
	...
	Результат.Вставить("ТаблицаИнформационныеПоля", Неопределено);
	
	Возврат Результат;
	
КонецФункции

// Открывает форму создания документа ЭТрН
// 
// Параметры:
//  ДанныеЗаполнения - См. НовыеДанныеЗаполненияЭТрН 
//                   - Неопределено 
//  ФормаВладелец    - ФормаКлиентскогоПриложения - Форма, из которой открывается форма документа ЭТрН
//                   - Неопределено               - Форма не задана
//  Оповещение       - ОписаниеОповещения         - Обработчик оповещения о завершении работы с формой документа ЭТрН
//                   - Неопределено               - Обработчик не требуется
Процедура СоздатьЭТрН(
	Знач ДанныеЗаполнения = Неопределено,
	...
	Если ДанныеЗаполнения <> Неопределено Тогда  
		ЗначенияЗаполнения.Вставить("СоздатьЭТрН", Истина);
		ЗначенияЗаполнения.Вставить("Организация", ДанныеЗаполнения.Организация);
		ЗначенияЗаполнения.Вставить("ДокументОснование", ДанныеЗаполнения.ДокументОснование);
		ЗначенияЗаполнения.Вставить("СсылкаТитулГрузоотправителяГрузоотправитель", ДанныеЗаполнения.Грузоотправитель);
		ЗначенияЗаполнения.Вставить("СсылкаТитулГрузоотправителяСоставитель", ДанныеЗаполнения.Составитель);
		ЗначенияЗаполнения.Вставить("СсылкаТитулГрузоотправителяЗаказчик", ДанныеЗаполнения.Заказчик);
		ЗначенияЗаполнения.Вставить("СсылкаТитулГрузоотправителяГрузополучатель", ДанныеЗаполнения.Грузополучатель);
		ЗначенияЗаполнения.Вставить("СсылкаТитулГрузоотправителяПеревозчик", ДанныеЗаполнения.Перевозчик);
		ЗначенияЗаполнения.Вставить("ТаблицаИнформационныеПоля", ДанныеЗаполнения.ТаблицаИнформационныеПоля);
	КонецЕсли;
...
// Создать документ Электронный заказ (заявка) (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектронныйЗаказЗаявкаКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОткрытьФорму("Документ.ЭлектронныйЗаказЗаявка.ФормаОбъекта");
	
КонецПроцедуры

// Создать документ Электронная транспортная накладная (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектроннаяТранспортнаяНакладнаяКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОткрытьФорму("Документ.ЭлектроннаяТранспортнаяНакладная.ФормаОбъекта");
		
КонецПроцедуры

// Создать документ Электронный путевой лист (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектронныйПутевойЛистКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОткрытьФорму("Документ.ЭлектронныйПутевойЛист.ФормаОбъекта");
		
КонецПроцедуры

// Создать документ Электронный заказ-наряд (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектронныйЗаказНарядКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОткрытьФорму("Документ.ЭлектронныйЗаказНаряд.ФормаОбъекта");
		
КонецПроцедуры

// Создать документ Электронная сопроводительная ведомость (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектроннаяСопроводительнаяВедомостьКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОткрытьФорму("Документ.ЭлектроннаяСопроводительнаяВедомость.ФормаОбъекта");
		
КонецПроцедуры

// Создать документ Электронный договор фрахтования (команда)
// 
// Параметры:
//  Результат - Неопределено
//  ДополнительныеПараметры - Неопределено
Процедура СоздатьЭлектронныйДоговорФрахтованияКоманда(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура ВводАдреса(ИмяЭлемента, Форма, ВладелецАдреса = Неопределено, ТекущиеДанные = Неопределено, Параметры = Неопределено) Экспорт
...
Процедура ВводАдреса_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ДополнительныеПараметры.Свойство("Параметры", Параметры);
			
		ОбменСГИСЭПДКлиентСервер.ЗаполнитьРеквизитыАдреса(ДополнительныеПараметры.Форма, Результат, 
			ДополнительныеПараметры.ИмяЭлемента, ТекущиеДанные, Параметры); 
...
Функция ИдентификаторОтправителя(Объект)
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяСопроводительнаяВедомость") Тогда	
		...
		Иначе
			Результат = Объект.ИдентификаторГрузополучателя;	
		КонецЕсли; 
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда	
		Если ТипЗнч(Объект.СсылкаТитулЗаказчикаЗаказчик) = ТипОрганизация
			Или ТипЗнч(Объект.СсылкаТитулЗаказчикаСоставитель) = ТипОрганизация Тогда
			Результат = Объект.ИдентификаторЗаказчика;
		Иначе
			Результат = Объект.ИдентификаторЭкспедитора;	
		КонецЕсли;
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка")
		Или ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда	
		Если ТипЗнч(Объект.СсылкаТитулЭкспедитораЭкспедитор) = ТипОрганизация
			Или ТипЗнч(Объект.СсылкаТитулЭкспедитораСоставитель) = ТипОрганизация Тогда
			Результат = Объект.ИдентификаторЭкспедитора;	
		КонецЕсли;
...
Процедура ЗаполнитьИдентификациюФайлаОбмена_ПослеПолученияУИД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТребуетсяЗаполнить = Истина Тогда
		...
		//ЭПЭ	
		ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1") 
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3") Тогда
			
			ЗаполнитьИдентификациюФайлаОбменаЭПЭ(Форма, Объект, КонтейнерОтображаемойВерсии); 
			
		//ЭЭР	
		ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1") Тогда
			
			ЗаполнитьИдентификациюФайлаОбменаЭЭР(Форма, Объект, КонтейнерОтображаемойВерсии);
			
		//ЭСР	
		ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1") Тогда
			
			ЗаполнитьИдентификациюФайлаОбменаЭСР(Форма, Объект, КонтейнерОтображаемойВерсии);
		...
		ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
			ИдФайл = ПолучитьИмяФайла(ИнформацияПоПрефиксамТитула.ШаблонИмениФайла, 
																			Объект.ИдентификаторЗаказчика,
																			,
																			Объект.ИдентификаторЭкспедитора,
																			,
																			НаличиеИныхПолучателей);																
		ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
			ИдФайл = ПолучитьИмяФайла(ИнформацияПоПрефиксамТитула.ШаблонИмениФайла, 
																			Объект.ИдентификаторЗаказчика,
																			,
																			Объект.ИдентификаторЭкспедитора,
																			,
																			НаличиеИныхПолучателей);
		ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда
			ИдФайл = ПолучитьИмяФайла(ИнформацияПоПрефиксамТитула.ШаблонИмениФайла, 
																			Объект.ИдентификаторЗаказчика,
																			,
																			Объект.ИдентификаторЭкспедитора,
...
Процедура ЗаполнитьИдентификациюФайлаОбменаЭСР(Форма, Объект, КонтейнерОтображаемойВерсии) 
	
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораДата",
		Объект.Дата, Ложь,
		КонтейнерОтображаемойВерсии);
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораНомер",
		Объект.Номер, Ложь,
		КонтейнерОтображаемойВерсии);
				
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораНаименованиеДокумента",
		"электронная складская расписка", ,
		КонтейнерОтображаемойВерсии);  
			
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораСодержаниеОперации",
		"выдача электронной складской расписки", ,
		КонтейнерОтображаемойВерсии);
			
	СчетчикЦикла = 1;	
	КлючСтруктуры = "ТитулЭкспедитораОписаниеГруза__" + СчетчикЦикла + "__ИдентификаторСтроки";
	ИдентификаторСтроки = Неопределено;
	СтруктураРеквизитовТитула = КонтейнерОтображаемойВерсии.СтруктураРеквизитовЗаполнение[КонтейнерОтображаемойВерсии.ПрефиксТитула][КонтейнерОтображаемойВерсии.НомерВерсии];
	Пока СтруктураРеквизитовТитула.Свойство(КлючСтруктуры, ИдентификаторСтроки) Цикл
		
		ЗначенияСтроки = Новый Структура("Маркировка", "Отсутствует");
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеПодчиненнойСтрокиВСтруктуреФормы(
			Форма, "ТитулЭкспедитораОписаниеГрузаМарк", ИдентификаторСтроки, ЗначенияСтроки, Ложь, КонтейнерОтображаемойВерсии); 
			
		СтГрузСтоимость = 0;	
		КлючСвойства = "ТитулЭкспедитораОписаниеГруза__" + СчетчикЦикла + "__ОбъявлСтГрузСтоимость";
		Если Не СтруктураРеквизитовТитула.Свойство(КлючСвойства, СтГрузСтоимость) Тогда
			КлючСвойства = "ТитулЭкспедитораОписаниеГруза__" + СчетчикЦикла + "__ДейстСтГрузСтоимость";
			СтруктураРеквизитовТитула.Свойство(КлючСвойства, СтГрузСтоимость);	
		КонецЕсли;		
		КлючСвойства = "ТитулЭкспедитораОписаниеГруза__" + СчетчикЦикла + "__ВозмУкСтГруз";
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
					Форма, КлючСвойства, ЗначениеЗаполнено(СтГрузСтоимость), Истина, КонтейнерОтображаемойВерсии);
	...
	СчетчикЦикла = 1;	
	КлючСтруктуры = "ТитулЭкспедитораГрузы__" + СчетчикЦикла + "__ИдентификаторСтроки";
	ИдентификаторСтроки = Неопределено;
	СтруктураРеквизитовТитула = КонтейнерОтображаемойВерсии.СтруктураРеквизитовЗаполнение[КонтейнерОтображаемойВерсии.ПрефиксТитула][КонтейнерОтображаемойВерсии.НомерВерсии];
	Пока СтруктураРеквизитовТитула.Свойство(КлючСтруктуры, ИдентификаторСтроки) Цикл
					
		ОбъявлСтГруз = 0;	
		КлючСвойства = "ТитулЭкспедитораГрузы__" + СчетчикЦикла + "__ОбъявлСтГрузПартСтГр";
		Если Не СтруктураРеквизитовТитула.Свойство(КлючСвойства, ОбъявлСтГруз) Тогда
			КлючСвойства = "ТитулЭкспедитораГрузы__" + СчетчикЦикла + "__ДейстСтГрузПартСтГр";
			СтруктураРеквизитовТитула.Свойство(КлючСвойства, ОбъявлСтГруз);	
		КонецЕсли;		
		КлючСвойства = "ТитулЭкспедитораГрузы__" + СчетчикЦикла + "__ВозмУкСтПарт";
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
					Форма, КлючСвойства, ЗначениеЗаполнено(ОбъявлСтГруз), Истина, КонтейнерОтображаемойВерсии);
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораВозмУкОбщСтГруз",
		Ложь, Ложь,	КонтейнерОтображаемойВерсии);

	
КонецПроцедуры 

Процедура ЗаполнитьИдентификациюФайлаОбменаЭЭР(Форма, Объект, КонтейнерОтображаемойВерсии) 
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораСодержаниеОперации",
		"груз принят во владение экспедитором", ,
		КонтейнерОтображаемойВерсии);
...
Процедура ЗаполнитьИдентификациюФайлаОбменаЭПЭ(Форма, Объект, КонтейнерОтображаемойВерсии) 
	
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1") Тогда
		ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул1(Форма, Объект, КонтейнерОтображаемойВерсии);
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2") Тогда
		ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул2(Форма, Объект, КонтейнерОтображаемойВерсии);		
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3") Тогда
		ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул3(Форма, Объект, КонтейнерОтображаемойВерсии);
	КонецЕсли;

КонецПроцедуры    

Процедура ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул1(Форма, Объект, КонтейнерОтображаемойВерсии)
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЗаказчикаНаименованиеДокумента",
		"электронное поручение экспедитору, информация клиента", ,
		КонтейнерОтображаемойВерсии);  
			
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЗаказчикаСодержаниеОперации",
		"представление электронного поручения экспедитору", ,
		КонтейнерОтображаемойВерсии);
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		...
		СтруктураРеквизитовТитула.Свойство("ТитулЗаказчикаСопроводительныеДокументы__1__ИдентификаторСтроки"), ,
		КонтейнерОтображаемойВерсии);
		
			
	СчетчикЦикла = 1;	
	КлючСтруктуры = "ТитулЗаказчикаОписаниеГруза__" + СчетчикЦикла + "__ИдентификаторСтроки";
	ИдентификаторСтроки = Неопределено;
	Пока СтруктураРеквизитовТитула.Свойство(КлючСтруктуры, ИдентификаторСтроки) Цикл
		ЗначенияСтроки = Новый Структура("Маркировка", "Отсутствует");
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеПодчиненнойСтрокиВСтруктуреФормы(
			Форма, "ТитулЗаказчикаОписаниеГрузаМарк", ИдентификаторСтроки, ЗначенияСтроки, Ложь, КонтейнерОтображаемойВерсии);		
		СчетчикЦикла = СчетчикЦикла + 1;
		КлючСтруктуры = "ТитулЗаказчикаОписаниеГруза__" + СчетчикЦикла + "__ИдентификаторСтроки";
	КонецЦикла;
...
Процедура ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул2(Форма, Объект, КонтейнерОтображаемойВерсии)
	
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораНаименованиеДокумента",
		"электронное поручение экспедитору, информация экспедитора", ,
		КонтейнерОтображаемойВерсии); 
	
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЭкспедитораСодержаниеОперации",
		"согласование подлежащих оказанию транспортно-экспедиционных услуг и условий", ,
		КонтейнерОтображаемойВерсии);  
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЭкспедитораФайлЗаказчикаИдентификатор",
			Объект.ТитулЗаказчикаИдентификаторФайла, , КонтейнерОтображаемойВерсии);   
					
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЭкспедитораФайлЗаказчикаДатаФормирования", Объект.ТитулЗаказчикаДатаФормированияФайла, , КонтейнерОтображаемойВерсии);
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЭкспедитораФайлЗаказчикаВремяФормирования", Объект.ТитулЗаказчикаВремяФормированияФайла, , КонтейнерОтображаемойВерсии);  
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЭкспедитораФайлЗаказчикаЭлектроннаяПодпись",
			ОбменСГИСЭПДВызовСервера.ПолучитьПодписьТитулаBase64(Объект.Ссылка,
			ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1")), , КонтейнерОтображаемойВерсии);

КонецПроцедуры 	

Процедура ЗаполнитьИдентификациюФайлаОбменаЭПЭТитул3(Форма, Объект, КонтейнерОтображаемойВерсии)
	
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЗаказчикаОтзывНаименованиеДокумента",
		"электронное поручение экспедитору, информация клиента об отзыве поручения экспедитору", ,
		КонтейнерОтображаемойВерсии); 
	
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
		"ТитулЗаказчикаОтзывСодержаниеОперации",
		"отзыв поручения экспедитору и обязательство возместить экспедитору фактические расходы", ,
		КонтейнерОтображаемойВерсии);  
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЗаказчикаОтзывФайлЗаказчикаИдентификатор",
			Объект.ТитулЗаказчикаИдентификаторФайла, , КонтейнерОтображаемойВерсии);   
					
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЗаказчикаОтзывФайлЗаказчикаДатаФормирования", Объект.ТитулЗаказчикаДатаФормированияФайла, , КонтейнерОтображаемойВерсии);
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЗаказчикаОтзывФайлЗаказчикаВремяФормирования", Объект.ТитулЗаказчикаВремяФормированияФайла, , КонтейнерОтображаемойВерсии);  
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
		Форма, "ТитулЗаказчикаОтзывФайлЗаказчикаЭлектроннаяПодпись",
			ОбменСГИСЭПДВызовСервера.ПолучитьПодписьТитулаBase64(Объект.Ссылка,
			ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1")), , КонтейнерОтображаемойВерсии);
...
Процедура ОтправитьЧерезЭДОПослеЗаписи_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтрЗаканчиваетсяНа(Форма.ИмяФормы, "ФормаГрупповойОтправки") = Ложь Тогда
		...
		СсылкаНаЧерновик = Неопределено;
		
		Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗЗ_Титул1") Тогда
			
			СсылкаНаЧерновик = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(
				Форма, "СсылкаТитулГрузоотправителяЧерновик", Неопределено);
	
		КонецЕсли;

		Если ЗначениеЗаполнено(СсылкаНаЧерновик) Тогда
			ДопПараметрЧерновик = ЭлектронныеДокументыЭДОКлиентСервер.НовыеДополнительныеПараметрыДействия();
			ДопПараметрЧерновик.Черновик = СсылкаНаЧерновик; 
			ПараметрыВыполненияДействийПоЭДО.ДополнительныеПараметрыДействий[Объект.Ссылка] = ДопПараметрЧерновик;
		КонецЕсли;
		...
		ДополнительныеПараметрыВыполнения = Новый Структура;
		ДополнительныеПараметрыВыполнения.Вставить("ЭтоИсправление", ЭтоИсправление);
		ДополнительныеПараметрыВыполнения.Вставить("ПараметрыВыполненияДействийПоЭДО", ПараметрыВыполненияДействийПоЭДО);
