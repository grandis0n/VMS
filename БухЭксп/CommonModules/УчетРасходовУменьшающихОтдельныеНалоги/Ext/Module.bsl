...
Процедура ДобавитьРасходыПоВзносамСФОТ(РасходыПоВзносам, Реквизиты, ТаблицаРасходов)
	...
	ПериодыЕНП = ОбщегоНазначения.ВыгрузитьКолонку(РасходыПоВзносам, "ДатаПризнанияЕНП", Истина);
	...
	Если ЗначениеЗаполнено(ПроизводственныйКалендарь) Тогда
		ПараметрыПолученияДатОплаты = КалендарныеГрафики.ПараметрыПолученияБлижайшихРабочихДат();
		ПараметрыПолученияДатОплаты.ВызыватьИсключение = Ложь;
		ДатыОплаты = КалендарныеГрафики.БлижайшиеРабочиеДаты(
			ПроизводственныйКалендарь, ПериодыЕНП, ПараметрыПолученияДатОплаты);
	Иначе
	...
	ДополнитьРасходыПоВзносамПризнакомПримененияУСН(Реквизиты.Организация, РасходыПоВзносам);
	...
	Для каждого СтрокаВзноса Из РасходыПоВзносам Цикл
		...
		УчитыватьРасходыНаПатенте = Реквизиты.ПрименяетсяПСН И УчетПСН.ИмеютсяДействующиеПатентыЗаПериод(
			Реквизиты.Организация, НачалоМесяца(СтрокаВзноса.ПериодНачисления), КонецМесяца(СтрокаВзноса.ПериодНачисления));
		
		// Учтем распределение между основной системой и специальным режимом налогообложения
		Если УчитыватьРасходыНаПатенте И СтрокаВзноса.НачисленоСпецРежим > 0 Тогда
		...
		// Если на дату расхода не применялся УСН, то расход не отражаем
		Если Не СтрокаВзноса.ПрименяетсяУСН Тогда
			СуммаРасхода = 0;
		КонецЕсли;
...
Процедура ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыВоВнешнейПрограмме(РасходыПоБольничным, Реквизиты, ТаблицаРасходов)
	...
	Для Каждого СтрокаНачисления Из НачислениеБольничных Цикл
		...
		УчитыватьРасходыНаПатенте = Реквизиты.ПрименяетсяПСН И УчетПСН.ИмеютсяДействующиеПатентыЗаПериод(
			Реквизиты.Организация, НачалоМесяца(СтрокаНачисления.ПериодНачисления), КонецМесяца(СтрокаНачисления.ПериодНачисления));
		...
		Для Каждого СтрокаБольничного Из СтрокиРасходаПоБольничным Цикл
			...
			НачисленоОсновнаяСистема = СтрокаБольничного.БольничныйЗаСчетРаботодателя;
			НачисленоСпецРежим = СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим;
			
			РанееПризнаноОсновнаяСистема = СтрокаБольничного.РанееПризнано;
			РанееПризнаноСпецРежим = СтрокаБольничного.РанееПризнаноСпецРежим;
			
			Если Не УчитыватьРасходыНаПатенте И НачисленоСпецРежим > 0 Тогда
				НачисленоОсновнаяСистема = НачисленоОсновнаяСистема + НачисленоСпецРежим;
				РанееПризнаноОсновнаяСистема = РанееПризнаноОсновнаяСистема + РанееПризнаноСпецРежим;
				НачисленоСпецРежим = 0;
				РанееПризнаноСпецРежим = 0;
			КонецЕсли;
			
			СуммаРасхода = НачисленоОсновнаяСистема - НачисленоСпецРежим;
			СуммаРасходаСпецРежим = НачисленоСпецРежим - РанееПризнаноСпецРежим;
			...
			Если СуммаРасхода > 0 Или СуммаРасходаСпецРежим > 0 Тогда
				...
				СтрокаРасхода.СуммаОплаты           = СуммаВыплаты;
				СтрокаРасхода.Задолженность         = СтрокаБольничного.ВсегоКВыплате;
				СтрокаРасхода.НачисленоВсего        = НачисленоОсновнаяСистема;
				СтрокаРасхода.НачисленоСпецРежим    = НачисленоСпецРежим;
				СтрокаРасхода.СуммаРасхода          = СуммаРасхода;
				СтрокаРасхода.СуммаРасходаСпецРежим = СуммаРасходаСпецРежим;
...
Процедура ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыСредствамиБухгалтерии(РасходыПоБольничным, Реквизиты, ТаблицаРасходов)
	...
	Для Каждого СтрокаБольничного Из РасходыПоБольничным Цикл
		...
		УчитыватьРасходыНаПатенте = Реквизиты.ПрименяетсяПСН И УчетПСН.ИмеютсяДействующиеПатентыЗаПериод(
			Реквизиты.Организация, НачалоМесяца(СтрокаБольничного.ПериодНачисления), КонецМесяца(СтрокаБольничного.ПериодНачисления));
		...
		Если ВсегоНачислено <> 0 Тогда
			СуммаРасхода = СуммаВыплаты * НачисленоОсновнаяСистема / ВсегоНачислено;
			СуммаРасходаСпецРежим = СуммаВыплаты * НачисленоСпецРежим / ВсегоНачислено;
		Иначе
			СуммаРасхода = 0;
			СуммаРасходаСпецРежим = 0;
		КонецЕсли;
...
Процедура ДобавитьРасходыПоВзносамИП(Реквизиты, ТаблицаРасходов)
	...
	ДатыПринятияЕНП = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаРасходовНаУплатуСтраховыхВзносов, "ДатаПринятияЕНП", Истина);
	ПроизводственныйКалендарь = КалендарныеГрафики.ОсновнойПроизводственныйКалендарь();
	Если ЗначениеЗаполнено(ПроизводственныйКалендарь) Тогда
		ПараметрыПолученияДатОплаты = КалендарныеГрафики.ПараметрыПолученияБлижайшихРабочихДат();
		ПараметрыПолученияДатОплаты.ВызыватьИсключение = Ложь;
		ДатыОплаты = КалендарныеГрафики.БлижайшиеРабочиеДаты(
			ПроизводственныйКалендарь, ДатыПринятияЕНП, ПараметрыПолученияДатОплаты);
	Иначе
		ДатыОплаты = Новый Соответствие;
...
Функция ТекстЗапросаРасходыПоБольничным(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	Если УпрощенныйУчетБольничных Тогда
		...
		Иначе
			...
			НомераТаблиц.Вставить("ВременнаяТаблицаУдержанныйПриВыплатеНДФЛ", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("РасходыПоБольничным", НомераТаблиц.Количество());
...
Функция СрокиУплатыВзносовПоМесяцам(НачалоПериода, КонецПериода)
	...
	Если ЗначениеЗаполнено(ПроизводственныйКалендарь) Тогда
		...
		ПараметрыПолученияСроковУплат = КалендарныеГрафики.ПараметрыПолученияБлижайшихРабочихДат();
		ПараметрыПолученияСроковУплат.ВызыватьИсключение = Ложь;
		СрокиУплаты = КалендарныеГрафики.БлижайшиеРабочиеДаты(
			ПроизводственныйКалендарь, ИсходныеСроки, ПараметрыПолученияСроковУплат);
	КонецЕсли;
...
Процедура ДополнитьРасходыПоВзносамПризнакомПримененияУСН(Организация, РасходыПоВзносам)
	
	ТаблицаПериодов = РасходыПоВзносам.Скопировать(, "ПериодНачисления");
	ТаблицаПериодов.Свернуть("ПериодНачисления");
	РасходыПоВзносам.Колонки.Добавить("ПрименяетсяУСН", Новый ОписаниеТипов("Булево"));
	...
	Для Каждого СтрокаПериода Из ТаблицаПериодов Цикл
		...
		ПрименяетсяУСН = УчетнаяПолитика.ПрименяетсяУСН(Организация, СтрокаПериода.ПериодНачисления);
		Если ПрименяетсяУСН Тогда
			СтрокиПоОтбору = РасходыПоВзносам.НайтиСтроки(Отбор);
			Для Каждого СтрокаПоОтбору Из СтрокиПоОтбору Цикл
				СтрокаПоОтбору.ПрименяетсяУСН = Истина;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
