...
Процедура ПослеЗапускаСистемы() Экспорт
	...
	Если ТолькоОбработкаПараметровЗапускаПереносаСправочников() Тогда
		Возврат;
	КонецЕсли;
	...
	ИмяПодсистемыИнтеграцияСОблачнойОтчетностью = "РегламентированнаяОтчетность.ИнтеграцияСОтчетность24";
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует(ИмяПодсистемыИнтеграцияСОблачнойОтчетностью) Тогда
		МодульИнтеграцияСОтчетность24Клиент =
			ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияСОтчетность24Клиент");
		МодульИнтеграцияСОтчетность24Клиент.ПолучитьСообщенияПоHttp();
	КонецЕсли;

	ИмяПодсистемыОтчетность1С = "Отчетность1С";
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует(ИмяПодсистемыИнтеграцияСОблачнойОтчетностью) 
		Или ОбщегоНазначенияКлиент.ПодсистемаСуществует(ИмяПодсистемыОтчетность1С) Тогда
		МодульОтчетность24Клиент = 
			ОбщегоНазначенияКлиент.ОбщийМодуль("Отчетность24БРОКлиент");
		МодульОтчетность24Клиент.ИнициализацияШиныУведомлений();
	КонецЕсли;
...
Функция ТолькоОбработкаПараметровЗапускаПереносаСправочников()

	Если СтрНайти(ПараметрЗапуска, "ПереносСправочниковБРО") = 0 Тогда
		Возврат Ложь;
	КонецЕсли;

	МассивПараметров = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПараметрЗапуска, ";");
	Количество = МассивПараметров.Количество();
	Разделитель = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(МассивПараметров[Количество - 3]);

	ЭтоТолькоПодключение = СтрНайти(ПараметрЗапуска, "ТолькоПодключение") > 0;

	Если Не ИнтеграцияСОтчетность24ВызовСервера.ЕстьПраваНаПереносСправочников() Тогда

		ТекстОтказа = НСтр("ru = 'Данные не выгружены: у пользователя, под которым запущена учетная программа, недостаточно прав.
			|Перенос сведений доступен пользователю с полными правами.
			|Повторите перенос под таким пользователем или обратитесь к администратору учетной программы.'");

		НачатьЗаписьРезультатаПереносаСправочников(Разделитель,
			?(ЭтоТолькоПодключение, "PIN_ERROR.txt", "Ошибки.txt"), ТекстОтказа);

		Возврат Истина;

	КонецЕсли;

	Если ЭтоТолькоПодключение Тогда

		ТекстОшибки = ИнтеграцияСОтчетность24ВызовСервера.ПодключитьУчетнуюБазуВФоне(ПараметрЗапуска);

		НачатьЗаписьРезультатаПереносаСправочников(Разделитель,
			?(ЗначениеЗаполнено(ТекстОшибки), "PIN_ERROR.txt", "PIN_READY.txt"), ТекстОшибки);

	Иначе

		ОткрытьФорму("Обработка.ПереносСправочниковБРО.Форма.ФормаВыгрузки",
...
Процедура НачатьЗаписьРезультатаПереносаСправочников(Разделитель, ИмяФайла, ТекстСообщения)

	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("Разделитель", Разделитель);
	ПараметрыЗаписи.Вставить("ИмяФайла", ИмяФайла);
	ПараметрыЗаписи.Вставить("ТекстСообщения", ТекстСообщения);

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПереносСправочниковПослеПодключенияРасширения",
...
Процедура ПереносСправочниковПослеПодключенияРасширения(Подключено, ПараметрыЗаписи) Экспорт
	...
	ДокументРезультата.УстановитьТекст(ПараметрыЗаписи.ТекстСообщения);

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПереносСправочниковПослеЗаписиРезультата",
		ДокументооборотСКОКлиент,
		ПараметрыЗаписи.Разделитель);

	ДокументРезультата.НачатьЗапись(ОписаниеОповещения,
		ПараметрыЗаписи.Разделитель + ПараметрыЗаписи.ИмяФайла);

КонецПроцедуры

Процедура ПереносСправочниковПослеЗаписиРезультата(Результат, Разделитель) Экспорт

	ДокументГотовности = Новый ТекстовыйДокумент;

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПереносСправочниковПослеЗаписиГотовности",
		ДокументооборотСКОКлиент);

	ДокументГотовности.НачатьЗапись(ОписаниеОповещения, Разделитель + "READY.txt");

КонецПроцедуры

Процедура ПереносСправочниковПослеЗаписиГотовности(Результат, ДополнительныеПараметры) Экспорт
