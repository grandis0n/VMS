...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено,
	...
	// Команды создания на основании могут появиться на форме разными способами:
	//   1. Автоматически: платформа отобразит общую команду на форме, если в командном интерфейсе
	...
	//      если конкретный объект конфигурации был добавлен в методе
	...
	// Если отработали оба способа – команда может дублироваться на форме. В таком случае скроем лишнюю команду.
	МассивКомандСоздатьБизнесПроцесс = Новый Массив;
	ИмяКомандыСоздатьБизнесПроцесс = Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотСоздатьБизнесПроцесс.Имя;
	МассивКомандСоздатьПисьмо = Новый Массив;
	ИмяКомандыСоздатьПисьмо = Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотСоздатьПисьмо.Имя;
	Для Каждого ЭлементФормы Из Форма.Элементы Цикл
		ТипЭлементы = ТипЗнч(ЭлементФормы);
		Если ТипЭлементы = Тип("КнопкаФормы")
				И ЭлементФормы.Видимость
				И СтрЗаканчиваетсяНа(ЭлементФормы.Имя, ИмяКомандыСоздатьБизнесПроцесс)
				И СтрНайти(ЭлементФормы.Имя, "КонтекстноеМеню") = 0 Тогда
			МассивКомандСоздатьБизнесПроцесс.Добавить(ЭлементФормы);
		ИначеЕсли ТипЭлементы = Тип("КнопкаФормы")
				И ЭлементФормы.Видимость
				И СтрЗаканчиваетсяНа(ЭлементФормы.Имя, ИмяКомандыСоздатьПисьмо)
				И СтрНайти(ЭлементФормы.Имя, "КонтекстноеМеню") = 0 Тогда
			МассивКомандСоздатьПисьмо.Добавить(ЭлементФормы);
		КонецЕсли;
	КонецЦикла;
	Если МассивКомандСоздатьБизнесПроцесс.Количество() > 1 Тогда
		Для Индекс = 1 По МассивКомандСоздатьБизнесПроцесс.ВГраница() Цикл
			МассивКомандСоздатьБизнесПроцесс[Индекс].Видимость = Ложь;
		КонецЦикла;
	КонецЕсли;
	Если МассивКомандСоздатьПисьмо.Количество() > 1 Тогда
		Для Индекс = 1 По МассивКомандСоздатьПисьмо.ВГраница() Цикл
			МассивКомандСоздатьПисьмо[Индекс].Видимость = Ложь;
		КонецЦикла;
	КонецЕсли;
