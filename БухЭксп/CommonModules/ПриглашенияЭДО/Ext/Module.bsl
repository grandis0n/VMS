...
// Создает автоматически принимаемое приглашение
//
// Параметры:
//  Карточка - см. ТранспортныеКонтейнерыЭДО.НовоеОписаниеКарточкиЭлектронногоДокумента 
//  Организация - ОпределяемыйТип.Организация - Организация для создания приглашения
//  Контрагент - ОпределяемыйТип.УчастникЭДО, Неопределено - контрагент для создания приглашения.
//  
//	Возвращаемое значение:
//		Неопределено,
//		ДанныеПриглашения - см. ПриглашенияЭДОСлужебный.НовыеДанныеПриглашения
//
Функция СоздатьПриглашениеПоДаннымКарточки(Карточка, Организация, Контрагент) Экспорт
	...
	КлючПриглашения.ИдентификаторОрганизации = Карточка.ИдентификаторПолучателя;
	
	ТекстПриглашения = НСтр("ru = 'Связь настроена автоматически. Отключено требование приглашений.'");
	
	СтруктураРеквизитов = Новый Структура("ИНН, КПП, Наименование");
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, СтруктураРеквизитов);
	
	МенеджерЗаписи = РегистрыСведений.ПриглашенияКОбменуЭлектроннымиДокументами.СоздатьМенеджерЗаписи();
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, РеквизитыКонтрагента);
	МенеджерЗаписи.Ключ = КлючПриглашения.Ключ;
	МенеджерЗаписи.ИдентификаторОрганизации = Карточка.ИдентификаторПолучателя;
	
	МенеджерЗаписи.ИдентификаторКонтрагента = Карточка.ИдентификаторОтправителя;
	МенеджерЗаписи.Контрагент = Контрагент;
	МенеджерЗаписи.Организация = Организация;
	МенеджерЗаписи.Тип = Перечисления.ТипыПриглашений.НаИдентификатор;
	МенеджерЗаписи.Статус = Перечисления.СтатусыПриглашений.Принято;
	МенеджерЗаписи.ТекстПриглашения = ТекстПриглашения;
	МенеджерЗаписи.ДатаИзмененияСтатуса = ТекущаяДатаСеанса();
	МенеджерЗаписи.Автоматическое = Истина;
	...
	СведенияОПриглашении.Организация = Организация;
	ПриглашенияЭДОСобытия.ПослеИзмененияПриглашения(СведенияОПриглашении);
	
	ДанныеПриглашения = ПриглашенияЭДОСлужебный.НовыеДанныеПриглашения();
	ДанныеПриглашения.КлючПриглашения = КлючПриглашения.Ключ;
	ДанныеПриглашения.Организация = Организация;
	ДанныеПриглашения.Контрагент = Контрагент;
	
	Возврат ДанныеПриглашения;
	
КонецФункции

// Создает автоматически отправляемое приглашение
//
// Параметры:
//  ПараметрыПриглашения - Структура:
...
//  Отказ - Булево
Процедура СоздатьАвтоматическоеПриглашенияПоКлючевымРеквизитам(ПараметрыПриглашения, Отказ) Экспорт
	...
	КлючПриглашения.ИдентификаторКонтрагента = ПараметрыПриглашения.ИдентификаторКонтрагента;
	
	ДанныеПриглашения = ДанныеПриглашенияПоПараметрам(КлючПриглашения);
	
	ШаблонПриглашения = 
		НСтр("ru = 'Связь настроена автоматически. Контрагент не требует приглашений.'");
	
	Если ДанныеПриглашения = Неопределено Тогда
		
		Результат = 
			СоздатьПриглашениеКОтправкеОператоруЭДО(КлючПриглашения, ПараметрыПриглашения.Организация,
				ПараметрыПриглашения.Контрагент, ШаблонПриглашения);

		Если Не Результат.Успешно Тогда
			ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Создание автоматического приглашения'"),
	...
	Если Не ЗначениеЗаполнено(ДанныеПриглашения.КонтрагентИНН) 
		Или Не ЗначениеЗаполнено(ДанныеПриглашения.КонтрагентКПП) Тогда
		...
		ПараметрыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПриглашения.Контрагент,
	...
	Иначе
		...
		ИНН = ДанныеПриглашения.КонтрагентИНН;
		КПП = ДанныеПриглашения.КонтрагентКПП;
	
	КонецЕсли;
	
	СвойстваПриглашения = НовыеЗначенияСвойствПриглашенияДляОбновления();
	ЗаполнитьЗначенияСвойств(СвойстваПриглашения, ДанныеПриглашения);
	СвойстваПриглашения.Ключ = ДанныеПриглашения.КлючПриглашения;
	СвойстваПриглашения.Автоматическое = Истина;
	СвойстваПриглашения.Статус = Перечисления.СтатусыПриглашений.Принято;
	СвойстваПриглашения.ДатаИзмененияСтатуса = ТекущаяДатаСеанса();
	СвойстваПриглашения.ИНН = ИНН;
	СвойстваПриглашения.КПП = КПП;
	СвойстваПриглашения.Тип = Перечисления.ТипыПриглашений.НаИдентификатор;
	ПриглашенияЭДОСлужебный.ОбновитьСвойстваПриглашения(СвойстваПриглашения);
	
	ПараметрыПриглашения.КлючПриглашения = ДанныеПриглашения.КлючПриглашения;
...
Функция НоваяТаблицаПриглашений() Экспорт
	...
	КС2 = Новый КвалификаторыСтроки(2);
	...
	ОписаниеТиповС2 = Новый ОписаниеТипов("Строка", , КС2);
	...
	ТЗ.Колонки.Добавить("ТипПользователя", ОписаниеТиповС2);
...
Функция УчастникЭДОПоКлючуИПриглашению(Знач ПараметрыПриглашения, Знач КлючУчастникаЭДО,
	...
	Если Не ЗначениеЗаполнено(УчастникЭДО) Тогда
		...
		РеквизитыКонтрагента.ИНН = КлючУчастникаЭДО.ИНН;
		РеквизитыКонтрагента.КПП = КлючУчастникаЭДО.КПП;
		GLN = ТрансграничныйЭДО.ПолучитьGLN(ПараметрыПриглашения.ИдентификаторКонтрагента);
		РеквизитыКонтрагента.GLN = GLN;
		ВидКонтрагента = ОбменСКонтрагентамиКлиентСервер.ВидКонтрагента(КлючУчастникаЭДО.ИНН, КлючУчастникаЭДО.ТипПользователя);
...
//  * ТипПользователя - Строка - ФЛ, ЮЛ, ИП
Функция НовыйКлючУчастникаЭДО() Экспорт
	...
	КлючУчастникаЭДО.Вставить("ТипПользователя", "");
...
Функция ОбработатьТаблицуПриглашений(ТаблицаПриглашений, КонтекстДиагностики, Сохранить = Истина)
	...
	|	Приглашения.ТипПользователя КАК ТипПользователя
	...
	Для Каждого НовыеПриглашения Из ТЗПоследнихИзменений Цикл
		...
		Попытка
			...
			МенеджерЗаписи.ТипПользователя = НовыеПриглашения.ТипПользователя;
			
			Если ТребуетсяОпределитьКонтрагента(МенеджерЗаписи)
				И НЕ ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(МенеджерЗаписи.ИдентификаторКонтрагента) Тогда
				...
				РеквизитыКонтрагента.Вставить("ВидКонтрагента", МенеджерЗаписи.ТипПользователя);
				Если Не ЗначениеЗаполнено(МенеджерЗаписи.ТипПользователя)
						И СтрДлина(МенеджерЗаписи.ИНН) = 12 Тогда
					РеквизитыКонтрагента.ВидКонтрагента = "ИП";
				КонецЕсли;
			...
			Если ТребуетсяОпределитьКонтрагента(МенеджерЗаписи)
				...
				СоздаватьКонтрагентовАвтоматически = СоздаватьКонтрагентовАвтоматическиБЭД
					И НЕ ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(МенеджерЗаписи.ИдентификаторКонтрагента);
					
				Если (СоздатьКонтрагентаБезусловно Или СоздаватьКонтрагентовАвтоматически)
					И Не ОбменМеждуСобственнымиОрганизациями Тогда
						
					РеквизитыКонтрагента = ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента();
					РеквизитыКонтрагента.ИНН = МенеджерЗаписи.ИНН;
					РеквизитыКонтрагента.КПП = МенеджерЗаписи.КПП;
					РеквизитыКонтрагента.Наименование = МенеджерЗаписи.Наименование;
					ВидКонтрагента = ОбменСКонтрагентамиКлиентСервер.ВидКонтрагента(МенеджерЗаписи.ИНН, МенеджерЗаписи.ТипПользователя);
					РеквизитыКонтрагента.ВидКонтрагента = ВидКонтрагента;
					GLN = ТрансграничныйЭДО.ПолучитьGLN(МенеджерЗаписи.ИдентификаторКонтрагента);
					РеквизитыКонтрагента.Вставить("GLN", GLN);
			...
			// Автоматически принятые приглашения имеют наивысший приоритет, не перезаписываем их
			ЗаписатьНовыеДанныеПриглашения = Истина;
			Если МенеджерЗаписи.Автоматическое Тогда
				ЗаписатьНовыеДанныеПриглашения = Не ПриглашениеПринято(МенеджерЗаписи.Статус);
			КонецЕсли;
			...
			Если ЗаписатьНовыеДанныеПриглашения Тогда
				МенеджерЗаписи.Записать();
			КонецЕсли;
