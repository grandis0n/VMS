...
// Возрашает текущий Epoch Unix Timestamp 
// вычисляется как разница между текущим временем в UTC и 1-го января 1970 UTC, измеренная в миллисекундах
//
// Возвращаемое значение:
//  Число - 
//
Функция ТекущаяВременнаяМетка() Экспорт
	
	ТекущаяВМС = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	Дата1970 = '19700101'; 
	РазницаСекунд = (Дата1970 - '00010101') * 1000;  
	// Вычитаем смещение: получаем миллисекунды с 01.01.1970
	Возврат ТекущаяВМС - РазницаСекунд;
...
#Если ВебКлиент Тогда
	Возврат ОбщегоНазначенияБПОСлужебныйВызовСервера.ПрочитатьПараметрыИзXML(СтрокаXML);
#Иначе
...
Функция ИспользуетсяБСП() Экспорт
	...
	// Вызов БСП
	Возврат (ПараметрПриложения("СтандартныеПодсистемы.ОценкаПроизводительности.ВремяНачалаЗапуска") <> Неопределено) 
		Или (ПараметрПриложения("СтандартныеПодсистемы.ПараметрыКлиента") <> Неопределено);
	// Конец Вызов БСП
