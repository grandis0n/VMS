...
Функция ЗапускЗаданияПолученияДокументов(Знач НастройкаОбмена, Знач НомераСчетов = Неопределено) Экспорт
	...
	ПараметрыПроцедуры.Вставить("ЭтоСинхронизация", Истина);
...
Функция ЗапускЗаданияОтправкиДокументовПоСчету(Знач НастройкаОбмена, Знач ДанныеДляОтправки) Экспорт
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиSberAPIСлужебный.ОтправитьДокументыПоСчету", ПараметрыПроцедуры, ПараметрыВыполнения);
...
// Заполняет и возвращает данные дайджеста электронного документа его подписью.
//
// Параметры:
//  СообщениеОбмена - ДокументСсылка.СообщениеОбменСБанками - электронный документ.
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//  МассивПодписей - Массив из Структура - массив данных подписей:
...
// Возвращаемое значение:
//  Строка - данные дайджеста электронного документа с его подписью.
//
Функция ДанныеПодписанногоСообщения(СообщениеОбмена, НастройкаОбмена, МассивПодписей) Экспорт
	...
	Если ДанныеСертификата = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не найден сертификат установленной подписи при отправке платежного поручения через токен
			| SberAPI для %1'");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиОбменСБанкамиСертификатыПодписей.Идентификатор КАК Идентификатор
	|ИЗ
	|	Справочник.НастройкиОбменСБанками.СертификатыПодписейОрганизации КАК НастройкиОбменСБанкамиСертификатыПодписей
	|ГДЕ
	|	НастройкиОбменСБанкамиСертификатыПодписей.СерийныйНомер = &СерийныйНомер
	|	И НастройкиОбменСБанкамиСертификатыПодписей.Ссылка = &Ссылка";
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru = 'Не найден идентификатор установленной подписи при отправке платежного поручения через
			| токен SberAPI для сертификата с SN %1'");
	...
	ИначеЕсли ВидЭД = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников Тогда
		...
		ОбменСБанкамиSberAPIСлужебный.ЗаполнитьДанныеДайджестаЗарплатнойВедомости(
			ДанныеПлатежа, Подпись, ИдентификаторСертификата);
...
Функция ПолучитьСмещениеЧасовогоПоясаВЧасах() Экспорт
	Возврат Окр(СмещениеСтандартногоВремени(ЧасовойПоясСеанса())/3600,1, РежимОкругления.Окр15как20);
КонецФункции
