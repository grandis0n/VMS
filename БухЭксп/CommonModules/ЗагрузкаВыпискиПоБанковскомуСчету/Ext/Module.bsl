...
Функция РаспознанныеДанныеИзБанка(ИмяФайла,
	...
	Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ИмяФайла) Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			...
			,
			,
			СтрШаблон(НСтр("ru = 'Распознавать данные из банка можно только из файла во временном каталоге.
			|Для распознания данных банка передан каталог %1'"), ИмяФайла));
...
//
Функция УточнитьХозяйственнуюОперациюДаннымиВведеннымиПользователем(ДанныеВведенныеПользователем,
	...
	Иначе
		ХозяйственнаяОперация = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(
	...
	Если СведенияОВладельцеБанковскогоСчетаИзКонтекста.БанковскийСчетОрганизации.Количество() = 1 Тогда
		...
		Если ЗначениеЗаполнено(БанковскийСчет) И ТипЗнч(БанковскийСчет) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			РеквизитыСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				БанковскийСчет, "НомерСчета, ИдентификаторЦифровогоСчета");
			РеквизитыОрганизации.Вставить("НомерСчета",                  РеквизитыСчета.НомерСчета);
			РеквизитыОрганизации.Вставить("ИдентификаторЦифровогоСчета", РеквизитыСчета.ИдентификаторЦифровогоСчета);
		Иначе
...
Функция ВыделитьСуммуКомиссииВозвратаИзНазначенияПлатежа(ИсходноеНазначениеПлатежа, НаборШаблонов) Экспорт
	...
	НазначениеПлатежа = ПривестиТекстНазначения(ИсходноеНазначениеПлатежа, Истина);
...
Функция НовыйВыпискаПоБанковскомуСчету(Идентификатор) Экспорт
	...
	Выписка.Операции.Колонки.Добавить("MCC",               ОбщегоНазначения.ОписаниеТипаСтрока(4));
	Выписка.Операции.Колонки.Добавить("КодНазПлатежа",     ОбщегоНазначения.ОписаниеТипаСтрока(1));
	Выписка.Операции.Колонки.Добавить("ПлатежнаяСсылкаЦР", Новый ОписаниеТипов("Строка"));
	...
	Выписка.Операции.Колонки.Добавить("ИдентификаторЦифровогоСчета", БанковскиеПравила.ТипЦифровойНомерСчета()); // TransactionDetails.References.InstructionIdentification Для идентификации операции в цепочке "банк-владелец счета" используется часть номера и дата документа, которым идентифицирована транзакция, а также номер банковского счета контрагента
...
Функция ВыпискиПоБанковскимСчетам(ДанныеИзБанка, УчастникиОпераций, ИдентификаторыВсехДокументов, Протокол = Неопределено)
	...
	Для Каждого Документ Из ДанныеИзБанка.Документы Цикл
		...
		Для Каждого Сторона Из Стороны Цикл
			...
			ИдентификаторЦифровогоСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				Документ[ИмяСтороны + "Реквизиты"], "ИдентификаторЦифровогоСчета", "");
			Если ЗначениеЗаполнено(ИдентификаторЦифровогоСчета) Тогда
				Счет = ИдентификаторЦифровогоСчета;
			Иначе
		...
		Для Каждого КлючИЗначение Из Стороны Цикл
			...
			ИдентификаторЦРПротивоположнойСтороны = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				РеквизитыПротивоположнойСтороны, "ИдентификаторЦифровогоСчета");
			Если ЗначениеЗаполнено(ИдентификаторЦРПротивоположнойСтороны) Тогда
				НомерСчетаПротивоположнойСтороны = ИдентификаторЦРПротивоположнойСтороны;
			КонецЕсли;
			...
			ЗаполнитьЗначенияСвойств(Операция, Документ,
				...
				|ВидОперацииЦР, MCC, ПлатежнаяСсылкаЦР, КодНазПлатежа,
	...
	Для Каждого Остаток Из ДанныеИзБанка.Остатки Цикл
		...
		Если Не ЗначениеЗаполнено(ИдентификаторыСчета)
			И (Остаток.НачальныйОстаток <> 0 Или Остаток.КонечныйОстаток <> 0
				Или Остаток.ВсегоПоступило <> 0 Или Остаток.ВсегоСписано <> 0) Тогда
	...
	Для Каждого Документ Из ДанныеИзБанка.Документы Цикл
		...
		Для Каждого Сторона Из Стороны Цикл
			...
			ЭтоСчетПереводовЧерезСБП = БанковскиеПравила.ЭтоСчетНезавершенныхРасчетов3023Актив(БалансовыйСчет)
				Или БанковскиеПравила.ЭтоСчетНезавершенныхРасчетов3023Пассив(БалансовыйСчет);
			...
			Если ЭтоСчетПереводовЧерезСБП Тогда
				...
				Если ЭтоСБП Тогда
					ЭтоСБПКакЭквайринг = ЭтоНазваниеБанка(Наименование, СоставныеЧастиНазванияБанка());
				Иначе
					СчетКонтрагента = "";
				КонецЕсли;
			...
			ИначеЕсли Документ.ВидОперацииЦР = ЦифровойРубль.ОперацияC2B() Тогда
				Если ЗначениеЗаполнено(Документ.ПлатежнаяСсылкаЦР) Тогда
					РеквизитыЦБ = ЦифровойРубль.РеквизитыОператораПлатформыЦифровогоРубля();
					Идентификатор = ИдентификацияУчастниковБанковскихОпераций.ИдентификаторПоИНН(
						УчастникиОпераций, РеквизитыЦБ.ИНН, РеквизитыЦБ.КПП);
					Если Идентификатор <> Неопределено Тогда
						ИдентификаторыУчастника.Добавить(Идентификатор);
					КонецЕсли;
				КонецЕсли;
			...
			ИдентификаторыФизическогоЛица = ИдентификаторыФизическогоЛицаИзНаименования(
				Наименование, ИНН, СчетКонтрагента, Документ.ВидОперацииЦР, Сторона);
			Если ИдентификаторыФизическогоЛица.ЭтоФизическоеЛицо Тогда
				Если Документ.ВидОперацииЦР = ЦифровойРубль.ОперацияC2B()
					Или Документ.ВидОперацииЦР = ЦифровойРубль.ОперацияB2C() Тогда
				...
				Если Идентификатор <> Неопределено Тогда
					ИдентификаторыУчастника.Добавить(Идентификатор);
					Если ЭтоСБП И Не ЭтоСБПКакЭквайринг Тогда
			...
			ИдентификаторЦифровогоСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ДругиеРеквизиты, "ИдентификаторЦифровогоСчета", "");
			Если ЗначениеЗаполнено(Счет) Или ЗначениеЗаполнено(ИдентификаторЦифровогоСчета) Тогда
				Если БанковскиеСчетаСервер.ЭтоИдентификаторЦифровогоСчета(ИдентификаторЦифровогоСчета) Тогда
					...
					Идентификатор = ИдентификацияУчастниковБанковскихОпераций.ИдентификаторПоИдентификаторуЦифровогоРубля(
						УчастникиОпераций, ИдентификаторЦифровогоСчета, Счет, БИКЦР);
				Иначе
			...
			// Здесь Идентификатор может быть как BBAN, так и IBAN или DBAN
			Если Идентификатор <> Неопределено
				И (СчетПозволяетИдентифицироватьКонтрагента
					Или БанковскиеПравила.ЭтоКазначейскийСчет(Счет)
					Или БанковскиеСчетаСервер.ЭтоИдентификаторЦифровогоСчета(ИдентификаторЦифровогоСчета)) Тогда
				ИдентификаторыУчастника.Добавить(Идентификатор);
...
Функция ИдентификаторыФизическогоЛицаИзНаименования(Наименование, ИНН, Счет, ВидОперацииЦР, Сторона = "")
	...
	Если ПустаяСтрока(ИНН)
		И (ВидОперацииЦР = ЦифровойРубль.ОперацияB2C()
			Или ВидОперацииЦР = ЦифровойРубль.ОперацияC2B()) Тогда
	...
	ЭтоВыплатаЧерезСБПНаПустыеРеквизиты = Сторона = "Получатель" И ПустаяСтрока(Счет) И ПустаяСтрока(ИНН)
		И Не ЭтоНазваниеБанка(Наименование, СоставныеЧастиНазванияБанка);
	...
	Если СтрНайти(Наименование, Разделитель) = 0 Тогда
		...
		Если СтрНайти(Наименование, Разделитель) = 0 Тогда
			...
			ИначеЕсли ЭтоВыплатаЧерезСБПНаПустыеРеквизиты Тогда
				Результат.ЭтоФизическоеЛицо = Истина;
				Результат.ИННПустой = Истина;
				Возврат Результат;
			Иначе
...
Функция НовыйОписаниеВладельцаСчета(ИдентификаторВыписки, УчастникиОпераций)
	...
	ОписаниеВладельцаСчета.Вставить("ВидОплатыЦР", Справочники.ВидыОплатОрганизаций.ПустаяСсылка());
...
Функция ХозяйственнаяОперацияНалог(Параметры)
	...
	Если ВидОперацииЦР = ЦифровойРубль.ОперацияB2G() Тогда
...
Функция ХозяйственнаяОперацияЗаработнаяПлата(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		КодВидаДохода = "";
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.Операция, "КодНазПлатежа") Тогда
			КодВидаДохода = Параметры.Операция.КодНазПлатежа;
		КонецЕсли;
		
		ЭтоПеречислениеФизлицуПоРееструЦР = ВидОперацииЦР = ЦифровойРубль.ОперацияB2C() И ЗначениеЗаполнено(КодВидаДохода);
		...
		Если Не БанковскиеПравила.ЭтоСчетПереводовФизическимЛицам(Параметры.Операция.БалансовыйСчет)
			...
			И Не БанковскиеПравила.ЭтоСчетСредствПереводаЭДС(Параметры.Операция.БалансовыйСчет)
			И Не ЭтоПеречислениеФизлицуПоРееструЦР Тогда
			Возврат НовыйРезультатОпределенияХозяйственнойОперации();
		КонецЕсли;
		...
		Если Не ФизлицоУстановлено(Параметры) Тогда // физ.лицо не найдено
			...
			Если КонтрагентУстановлен(Параметры) И Не ЭтоПеречислениеФизлицуПоРееструЦР Тогда
				Возврат НовыйРезультатОпределенияХозяйственнойОперации();
			КонецЕсли;
		КонецЕсли;
...
Функция ХозяйственнаяОперацияВозвратНовомуПокупателю(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		Если Не НазначениеПлатежаУказываетНаВозврат(Параметры.Операция)
			И ВидОперацииЦР <> ЦифровойРубль.ОперацияB2C() Тогда
			Возврат НовыйРезультатОпределенияХозяйственнойОперации();
		КонецЕсли;
	КонецЕсли;
...
Функция ХозяйственнаяОперацияВыручкаРозничнойТорговлиЧерезПосредника(Параметры)
	...
	ОрганизацияИП =
		Параметры.ВладелецСчета.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
	
	НазначениеПлатежаУказываетНаСБП = Ложь;
	НазначениеПлатежаУказываетНаЭквайринг = Ложь;
	ОплатаПоQRКодуЦР = Ложь;
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		НазначениеПлатежаУказываетНаЭквайрингФНС = НазначениеПлатежаУказываетНаЭквайрингФНС(Параметры.Операция);
		...
		ОплатаПоQRКодуЦР = ЗначениеЗаполнено(Параметры.Операция.ПлатежнаяСсылкаЦР)
			И ЗначениеЗаполнено(Параметры.ВладелецСчета.ВидОплатыЦР);
		Если Не ОплатаПоQRКодуЦР И ЗначениеЗаполнено(Параметры.Операция.ПлатежнаяСсылкаЦР) Тогда
			ОчиститьИдентификаторыTXID(Параметры, "Плательщик");
		КонецЕсли;
		
		Если Не ЭтоСчетНезавершенныхРасчетов
			И Не ЭтоСчетРасчетовФилиалаБанкаСКлиентамиБанка
			И Не НазначениеПлатежаУказываетНаЭквайрингФНС
			И Не ОплатаПоQRКодуЦР
	...
	Если ОплатаПоQRКодуЦР Тогда
		КонтрагентЦР = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ВладелецСчета.ВидОплатыЦР, "Контрагент");
		Результат.ХозяйственнаяОперация.Вставить("Контрагент", КонтрагентЦР);
		Результат.ХозяйственнаяОперация.Вставить("ВидОплаты", Параметры.ВладелецСчета.ВидОплатыЦР);
		ОчиститьИдентификаторыЦРНеСоответствующиеЦБ(Параметры, "Плательщик");
		
		Если Параметры.ПлательщикКешИдентификаторов <> Неопределено Тогда
			Параметры.ПлательщикКешИдентификаторов.ВсеСсылкиПоТипам.Очистить();
		КонецЕсли;
	...
	Если ОплатаПоQRКодуЦР Тогда
		ДобавитьОтбор(ОтборДоговоров, "ВидОплаты", Параметры.ВладелецСчета.ВидОплатыЦР);
		Параметры.Операция.ДополнительныеСвойства.Вставить("ЭтоУслугиБанкаПоЭквайрингу", Истина);
	Иначе
		ДобавитьОтбор(ОтборДоговоров, "ВидОплаты");
	КонецЕсли;
	...
	ДоговорНайден = НайтиДоговорПоВидуОперации(
		...
		Результат.ХозяйственнаяОперация.ВидОперацииДокумента,
		ОтборДоговоров);
	Если ДоговорНайден И Не ОплатаПоQRКодуЦР Тогда
	...
	ЭтоСБП = Не ОплатаПоQRКодуЦР И (НазначениеПлатежаУказываетНаСБП
...
Функция ХозяйственнаяОперацияОплатаОтИзвестногоПокупателя(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		Если НазначениеПлатежаУказываетНаВозврат(Параметры.Операция)
			Или ВидОперацииЦР = ЦифровойРубль.ОперацияB2C() Тогда
			Возврат НовыйРезультатОпределенияХозяйственнойОперации();
		КонецЕсли;
	КонецЕсли;
...
Функция ХозяйственнаяОперацияПереводМеждуСобственнымиСчетами(Параметры)
	...
	ЭтоПереводСредствЦР = ВидОперацииЦР = ЦифровойРубль.ОперацияDCBU()  // Пополнение ЦР (банковский счет -> ЦР)
					  Или ВидОперацииЦР = ЦифровойРубль.ОперацияDCSE(); // Вывод средств (банковский счет <- ЦР)
...
Функция ОперацияЦРПоддерживается(ВидОперации)
	...
	МассивНеподерживаемыхОпераций.Добавить(ЦифровойРубль.ОперацияDCRF());
	МассивНеподерживаемыхОпераций.Добавить(ЦифровойРубль.ОперацияOOPN());
	МассивНеподерживаемыхОпераций.Добавить(ЦифровойРубль.ОперацияOPTR());
...
Процедура ОчиститьИдентификаторыЦРНеСоответствующиеЦБ(Параметры, СторонаКонтрагента)
	
	МассивСхем = Новый Массив;
	МассивСхем.Добавить("PAM");
	МассивСхем.Добавить("DBAN");
	
	МассивКУдалению = Новый Массив;
	МассивИдентификаторов = Параметры.Операция[СторонаКонтрагента + "Идентификаторы"];
	Для Индекс = 0 По МассивИдентификаторов.ВГраница() Цикл
		Идентификатор = МассивИдентификаторов[Индекс];
		Для каждого Схема Из МассивСхем Цикл
			Если СтрНачинаетсяС(Идентификатор, Схема) Тогда
				МассивКУдалению.Добавить(Индекс);
			КонецЕсли;
		КонецЦикла;
...
Процедура НайтиПлатежныеПоручения(РаспознаннаяВыписка, Выписка, УчастникиОпераций)
	...
	Для Каждого Операция Из Выписка.Операции Цикл
		...
		ИдентификаторЦифровогоСчетаКонтрагента = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			Операция.ПлательщикРеквизиты, "ИдентификаторЦифровогоСчета", "");
		ЭтоЦифровойСчет = БанковскиеСчетаСервер.ЭтоИдентификаторЦифровогоСчета(ИдентификаторЦифровогоСчетаКонтрагента);
		...
		Если ЭтоЦифровойСчет Тогда
			КлючПоиска = КлючПоискаПлатежногоПоручения(
				Операция.ДатаДокумента,
				Операция.НомерДокумента,
				ИдентификаторЦифровогоСчетаКонтрагента,
				ЭтоЦифровойСчет);
		Иначе
			КлючПоиска = КлючПоискаПлатежногоПоручения(
				Операция.ДатаДокумента,
		...
		// Потому что контрагент в созданных документах - это недостаточно надежный признак для поиска документов.
...
Процедура НайтиРанееСозданныеДокументы(РаспознаннаяВыписка, Выписка, УчастникиОпераций)
	...
	Для Каждого ПоступилоСписано Из ВариантыПоступилоСписано() Цикл
		...
		Если ПоступилоСписано = "Поступило" Тогда
			...
			|	ЕСТЬNULL(Документы.СчетКонтрагента.ИдентификаторЦифровогоСчета, """") КАК ИдентификаторЦифровогоСчетаКонтрагента,
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого Операция Из ОперацииПоКлючуБанк Цикл
				...
				ОценитьСоответствиеПоКритерию(Оценщик, Не ЗначениеЗаполнено(Выборка.НомерСчетаКонтрагента)
					И Не ЗначениеЗаполнено(Выборка.ИдентификаторЦифровогоСчетаКонтрагента));
...
Функция НазначениеПлатежаУказываетНаЭквайрингФНС(Операция)
	
	КлючевыеФразы = Новый Массив;
	
	КлючевыеФразы.Добавить(НСтр("ru = 'зачисление средств по терминалам эквайр'"));
	КлючевыеФразы.Добавить(НСтр("ru = 'возмещение средств по услуге эквайринга по реестру опер'"));
	КлючевыеФразы.Добавить(НСтр("ru = 'удержание средств по услуге интернет-эквайринга по реестру опер'"));
	КлючевыеФразы.Добавить(
		НСтр("ru = 'зачисление денежных средств по договору об обслуживании держателей платежных карт по терминал'"));
	
	// "перевод средств по договору № ИЭ-" и "по реестру операций от" и "сумма комиссии"
	Фраза = Новый Массив; // исключил указание комиссии, т.к. в качестве эквайера может выступать не только банк
	Фраза.Добавить(НСтр("ru = 'перевод средств по договору'"));
	Фраза.Добавить(НСтр("ru = 'иэ'"));
	Фраза.Добавить(НСтр("ru = 'по реестру операций'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "перевод средств по договору" и "(FOLDS) по реестру операций от" и "сумма комиссии"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'перевод средств по договору'"));
	Фраза.Добавить(НСтр("ru = 'folds'"));
	Фраза.Добавить(НСтр("ru = 'по реестру опер'"));
	Фраза.Добавить(НСтр("ru = 'сумма комис'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "зачисление средств по операциям эквайринга. мерчант №" и "дата реестра" и "комиссия" и "возврат покупки"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'зачисление средств по операциям эквайринга'"));
	Фраза.Добавить(НСтр("ru = 'мерчант'"));
	Фраза.Добавить(НСтр("ru = 'дата реестра'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'возврат покупки'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "зачисление средств по операциям с МБК (на основании реестров платежей). мерчант №" и "дата реестра" и "комиссия"
	// и "возврат покупки"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'зачисление средств по операциям'"));
	Фраза.Добавить(НСтр("ru = 'МБК'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'на основании реестров платежей'"));
	Фраза.Добавить(НСтр("ru = 'мерчант'"));
	Фраза.Добавить(НСтр("ru = 'дата реестра'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'возврат покупки'"));
	...
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение по торговому эквайрингу мерчант'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'терминал'"));
	Фраза.Добавить(НСтр("ru = 'возврат покупки'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "оплата товаров и услуг по ТСТ" и "реестр от" и "комиссия" и "возврат покупки по ТСТ"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'оплата товаров и услуг'"));
	Фраза.Добавить(НСтр("ru = 'реестр'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'ТСТ'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'возврат покупки'"));
	...
	// "оплата товаров и услуг по банковским картам (МПК) согласно реестра торг. операций. кол-во операций:"
	// и "удержано по операциям возврата:" и "сумма комиссии"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'оплата товаров и услуг по банковским картам'"));
	Фраза.Добавить(НСтр("ru = 'согласно реестра торг'"));
	Фраза.Добавить(НСтр("ru = 'кол-во'"));
	Фраза.Добавить(НСтр("ru = 'операций'"));
	Фраза.Добавить(НСтр("ru = 'МПК'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'удержано по операциям возврата'"));
	Фраза.Добавить(НСтр("ru = 'сумма комиссии'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// эквайринг за" и "комиссия" и "возврат:" и "возврат др.ТСТ:" и "реестр"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'эквайринг'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'возврат'"));
	Фраза.Добавить(НСтр("ru = 'ТСТ'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'реестр'"));
	...
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'эквайринг'"));
	Фраза.Добавить(НСтр("ru = 'по мерчанту'"));
	Фраза.Добавить(НСтр("ru = 'удержано ком'"));
	Фраза.Добавить(НСтр("ru = 'возвр'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "по принятым платежам с" и "в т.ч.усл.банка:" и "согласно эл.реестру"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'по принятым платежам'"));
	Фраза.Добавить(НСтр("ru = 'усл'"));
	Фраза.Добавить(НСтр("ru = 'банка'"));
	Фраза.Добавить(НСтр("ru = 'согласно'"));
	Фраза.Добавить(НСтр("ru = 'реестру'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "терминал №" и "дата операций" и "комиссия банка"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'терминал'"));
	Фраза.Добавить(НСтр("ru = 'дата операций'"));
	Фраза.Добавить(НСтр("ru = 'комиссия банка'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "возмещение ден.ср-в ТСП за" и "комиссия банка"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение'"));
	Фраза.Добавить(НСтр("ru = 'ден'"));
	Фраза.Добавить(НСтр("ru = 'ср-в'"));
	Фраза.Добавить(НСтр("ru = 'ТСП'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'комиссия банка'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "сумма возмещения" и "по договору №" и "(TTR за" и "комиссия"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'сумма возмещения'"));
	Фраза.Добавить(НСтр("ru = 'по договору'"));
	Фраза.Добавить(НСтр("ru = 'ttr'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "возмещение ср-в по операциям эквайринга за" и "согласно договору" и "комиссия по операциям"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение ср-в по операциям эквайринга'"));
	Фраза.Добавить(НСтр("ru = 'согласно договору'"));
	Фраза.Добавить(НСтр("ru = 'комиссия по операциям'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "возмещение ден.ср-в по дог.экв." и "оборот:" и "комиссия"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение ден'"));
	Фраза.Добавить(НСтр("ru = 'ср-в по дог'"));
	Фраза.Добавить(НСтр("ru = 'экв'"));
	Фраза.Добавить(НСтр("ru = 'оборот'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "общая сумма к возмещению" и "комиссия за осущ.расч." и "RURвозмещение ден.ср-в по дог.экв." и "оборот:"
	// и "комиссия:"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'общая сумма к возмещению'"));
	Фраза.Добавить(НСтр("ru = 'комиссия за осущ'"));
	Фраза.Добавить(НСтр("ru = 'расч'"));
	Фраза.Добавить(НСтр("ru = 'RURвозмещение ден'"));
	Фраза.Добавить(НСтр("ru = 'ср-в по дог'"));
	Фраза.Добавить(НСтр("ru = 'экв'"));
	Фраза.Добавить(НСтр("ru = 'оборот'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	...
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возм'"));
	Фраза.Добавить(НСтр("ru = 'по дог'"));
	Фраза.Добавить(НСтр("ru = 'экв'"));
	Фраза.Добавить(НСтр("ru = 'об'"));
	Фраза.Добавить(НСтр("ru = 'ком'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "перечисление по договору №" и "с ТСП ИНН" и "комиссия банка"
	// "перечисление по договору с ТСП" и "комиссия банка"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'перечисление по договору'"));
	Фраза.Добавить(НСтр("ru = 'ТСП'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'комиссия банка'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	...
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'расчеты по дог'"));
	Фраза.Добавить(НСтр("ru = 'за период'"));
	Фраза.Добавить(НСтр("ru = 'сумма'"));
	Фраза.Добавить(НСтр("ru = 'оплат'"));
	Фраза.Добавить(НСтр("ru = 'отмены'"));
	Фраза.Добавить(НСтр("ru = 'возвратов'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "возмещение по терминалам:" и "в т.ч. комиссия"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение по терминалам'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаПоследовательно");
	
	// "расч по дог. б/н опер." и "ком банка"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'расч по дог'"));
	Фраза.Добавить(НСтр("ru = 'б/н опер'"));
	Фраза.Добавить(НСтр("ru = 'ком банка'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "возмещение по ТСО по сумме" и "сумма комиссии" и "сумма возмещения" и "дата расчета" и "договор"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возмещение'"));
	Фраза.Добавить(НСтр("ru = 'ТСО'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'сумм'"));
	Фраза.Добавить(НСтр("ru = 'комиссии'"));
	Фраза.Добавить(НСтр("ru = 'возмещения'"));
	Фраза.Добавить(НСтр("ru = 'дата расчета'"));
	Фраза.Добавить(НСтр("ru = 'договор'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "перечисление денежных средств по договору MRC/" и "комиссия" и "выплата за"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'перечисление денежных средств по договору'"));
	Фраза.Добавить(НСтр("ru = 'MRC/'")); // аббревиатуру указываем заглавными, чтобы проверять на точное соответствие
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'выплата'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "платежи по дог." и "принято" и "комиссия" и "удержана"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'платежи по дог'"));
	Фраза.Добавить(НСтр("ru = 'принято'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'удержана'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	...
	Фраза.Добавить(НСтр("ru = 'перечисление денежных средств по договору'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
	Фраза.Добавить(НСтр("ru = 'по реестру'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	...
	Фраза.Добавить(НСтр("ru = 'перечисл'"));
	Фраза.Добавить(НСтр("ru = 'ден'"));
	Фраза.Добавить(НСтр("ru = 'ср-в с уч'"));
	Фраза.Добавить(НСтр("ru = 'упл'"));
	Фраза.Добавить(НСтр("ru = 'комис'"));
	Фраза.Добавить(НСтр("ru = 'по дог'"));
	Фраза.Добавить(НСтр("ru = 'от'"));
	Фраза.Добавить(НСтр("ru = 'за пер'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаПоследовательно");
	
	// "возврат денежных средств по эквайринговым операциям от" и "согласно договору"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'возврат денежных средств по эквайринговым операциям'"));
	Фраза.Добавить(НСтр("ru = 'согласно договору'"));
	ДобавитьПравилоПроверкиФразы(КлючевыеФразы, Фраза, "СловаВЛюбомПорядке");
	
	// "расчеты по договору №" и "по операции" и "по мерчанту"
	Фраза = Новый Массив;
	Фраза.Добавить(НСтр("ru = 'платежи по дог'"));
	Фраза.Добавить(НСтр("ru = 'принято'"));
	Фраза.Добавить(НСтр("ru = 'комиссия'"));
...
Функция НайтиКлючевыеФразыВТекстеНазначения(КлючевыеФразы, Операция, КромеРазделителей = "")
	...
	Для Каждого КлючеваяФраза Из КлючевыеФразы Цикл
		...
		Если ТипЗнч(КлючеваяФраза) = Тип("Строка") Тогда
			Правило = "СловаПодряд";
			Если СтрНайти(КлючеваяФраза, " ") > 0 Тогда
				СловаФразы = СтрРазделить(КлючеваяФраза, " ");
			Иначе
...
Функция НовыйШаблоныКомиссия() Экспорт
	...
	ТаблицаШаблонов.Добавить(НСтр("ru = 'комиссия-оплата'"));
	ТаблицаШаблонов.Добавить(НСтр("ru = 'комиссия'"));
	ТаблицаШаблонов.Добавить(НСтр("ru = 'комиссии'"));
	ТаблицаШаблонов.Добавить(НСтр("ru = 'комиссии за прием платежей'"));
	...
	ТаблицаШаблонов.Добавить(НСтр("ru = 'в соответствии с тарифами по продукту:'"));
...
Функция НовыйШаблоныТекстаНДС()
	...
	СтрокаШаблона = ТаблицаШаблонов.Добавить();
	СтрокаШаблона.ТекстШаблона = НСтр("ru = 'кроме того ндс'"); // НДС "сверху"
	СтрокаШаблона.ОблагаетсяНДС = Истина;
...
Функция ВыделитьИнформациюОНДСВНазначении(Знач ТекстНазначениеПлатежа, ЗначениеСтавкиНДС, СпецСимволыЧисла)
	Перем ИнформацияОНДС, Результат;
...
Функция ПривестиТекстНазначения(Знач ИсходноеНазначениеПлатежа, УбратьСкобку = Ложь)
	
	ЗаменяемыеСимволы = ?(УбратьСкобку, "", "(") + ")"
		+ Символы.НПП
		+ Символы.ПС
		+ Символы.ВК
		+ Символы.ПФ
...
Функция ПоискШаблонаВТекстеНазначения(Текст, Шаблоны)
	...
	Если Результат <> Неопределено И ЗначениеЗаполнено(Результат.ТекстПосле) Тогда
		ФорматЧисло   = Новый ОписаниеТипов("Число");
		ТекстПосле    = Результат.ТекстПосле;
		ПозицияСкобки = СтрНайти(ТекстПосле, "(");
		ОбщаяКомиссия = "0";
		Если ПозицияСкобки > 0 Тогда
			ОбщаяКомиссия = Лев(ТекстПосле,  ПозицияСкобки - 1);
			ТекстПосле    = Сред(ТекстПосле, ПозицияСкобки + 1);
		КонецЕсли;
		
		Если ОбщаяКомиссия <> "0" Тогда
			РезультатКомиссия = ВыделитьКомиссииНДСИБезНДС(ТекстПосле);
			Если ЗначениеЗаполнено(РезультатКомиссия.СуммаСНДС) И ЗначениеЗаполнено(РезультатКомиссия.СуммаБезНДС) Тогда
				СуммаКомиссии = ФорматЧисло.ПривестиЗначение(СтрЗаменить(ОбщаяКомиссия, " ", ""));
				СуммаСНДС     = ФорматЧисло.ПривестиЗначение(СтрЗаменить(РезультатКомиссия.СуммаСНДС,   " ", ""));
				СуммаБезНДС   = ФорматЧисло.ПривестиЗначение(СтрЗаменить(РезультатКомиссия.СуммаБезНДС, " ", ""));
				Если СуммаКомиссии = СуммаСНДС + СуммаБезНДС Тогда
					Результат.ТекстПосле = ОбщаяКомиссия;
					Возврат Результат;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Результат.ТекстПосле = СтрЗаменить(Результат.ТекстПосле, "(", " ");
		РезультатКомиссия = ВыделитьКомиссииНДСИБезНДС(Результат.ТекстПосле);
		Если ЗначениеЗаполнено(РезультатКомиссия.СуммаСНДС) И ЗначениеЗаполнено(РезультатКомиссия.СуммаБезНДС) Тогда
			Результат.ТекстПосле = СтрЗаменить(Результат.ТекстПосле,
											   СтрШаблон(НСтр("ru = 'и %1'"),          РезультатКомиссия.СуммаБезНДС),
											   СтрШаблон(НСтр("ru = 'и комиссия %1'"), РезультатКомиссия.СуммаБезНДС));
		КонецЕсли;
	КонецЕсли;
...
Функция ВыделитьКомиссииНДСИБезНДС(ТекстПосле)
	
	Результат = Новый Структура("СуммаСНДС, СуммаБезНДС");
	
	// Проверим, что в тексте переданы 2 комиссии - с НДС и без НДС и поправим текст для дальнейшей обработки.
	РегулярноеВыражениеВТЧНДС = "(\d+(?:[.,]\d+)?)(?=[\s(]*в т\.ч\.)";
	КомиссияВТЧНДС = СтрНайтиПоРегулярномуВыражению(ТекстПосле, РегулярноеВыражениеВТЧНДС);
	Если Не ЗначениеЗаполнено(КомиссияВТЧНДС.Значение) Тогда
		РегулярноеВыражениеВТЧНДС = "(\d+(?:[.,]\d+)?)(?=\s*\.\s*возврат покупки.*?в т\.ч\.)";
		КомиссияВТЧНДС = СтрНайтиПоРегулярномуВыражению(ТекстПосле, РегулярноеВыражениеВТЧНДС);
	КонецЕсли;
	
	КомиссияБезНДС = СтрНайтиПоРегулярномуВыражению(ТекстПосле, "(\d+(?:[.,]\d+)?)(?=[\s(]*ндс не)");
	Если ЗначениеЗаполнено(КомиссияВТЧНДС.Значение) И ЗначениеЗаполнено(КомиссияБезНДС.Значение) Тогда
		// Комиссия может быть с пробелами - разделителями групп.
		
		РегулярноеВыражениеВТЧНДС = СтрЗаменить(РегулярноеВыражениеВТЧНДС,
			"(\d+(?:[.,]\d+)", "(\d{1,3}(?:\s\d{3})*(?:[.,]\d+)");
		КомиссияВТЧНДССПробелами = СтрНайтиПоРегулярномуВыражению(ТекстПосле, РегулярноеВыражениеВТЧНДС);
		Если ЗначениеЗаполнено(КомиссияВТЧНДССПробелами.Значение)
			И СтрДлина(СтрЗаменить(КомиссияВТЧНДССПробелами.Значение, " ", "")) > СтрДлина(КомиссияВТЧНДС.Значение) Тогда
			Результат.СуммаСНДС = КомиссияВТЧНДССПробелами.Значение;
		Иначе
			Результат.СуммаСНДС = КомиссияВТЧНДС.Значение;
		КонецЕсли;
		
		КомиссияСПробелами = СтрНайтиПоРегулярномуВыражению(ТекстПосле,
			"(\d{1,3}(?:\s\d{3})*(?:[.,]\d+)?)\s*(?=[\s(]*ндс не)");
		Если ЗначениеЗаполнено(КомиссияСПробелами.Значение)
			И СтрДлина(СтрЗаменить(КомиссияСПробелами.Значение, " ", "")) > СтрДлина(КомиссияБезНДС.Значение) Тогда
			Результат.СуммаБезНДС = КомиссияСПробелами.Значение;
		Иначе
			Результат.СуммаБезНДС = КомиссияБезНДС.Значение;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОписаниеСчета <> Неопределено Тогда
		...
		Если ТипЗнч(ОписаниеСчета.ОписаниеСсылки.Владелец) = Тип("СправочникСсылка.Организации") Тогда
			...
			Если БанковскиеСчетаЦифровые.Количество() > 0 Тогда
				...
				ОписаниеВладельцаСчета.ВидОплатыЦР =
					Справочники.ВидыОплатОрганизаций.ВидОплатыЦР(ОписаниеВладельцаСчета.Организация);
			КонецЕсли;
