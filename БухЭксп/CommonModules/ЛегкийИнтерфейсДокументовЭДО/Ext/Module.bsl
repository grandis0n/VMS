...
// 
// Возвращаемое значение:
//  См. ЛегкийИнтерфейсДокументовЭДОСлужебный.НовыеДанныеКомпонентаПанельКомандВходящегоДокумента
...
//  * ЧерновикВладелец - ДокументСсылка.ЧерновикЭлектронногоДокументаЭДО
//  * ПредставлениеЧерновика - Строка
//
Функция НовыеСвойстваДокументаДляПолученияДанныхКомпонентовПросмотра() Экспорт
	...
	СвойстваДокумента.Вставить("ЧерновикВладелец",
		ПредопределенноеЗначение("Документ.ЧерновикЭлектронногоДокументаЭДО.ПустаяСсылка"));
	СвойстваДокумента.Вставить("ПредставлениеЧерновика", "");
...
Функция СвойстваДокументаДляПолученияДанныхКомпонентовПросмотраКарточкиДокумента(Форма)
	...
	Если СвойстваДокумента.ЭтоВходящийЭДО Тогда
		СвойстваДокумента.ЧерновикВладелец = Объект.ЧерновикВладелец;
		СвойстваДокумента.ПредставлениеЧерновика = Объект.ПредставлениеЧерновика;
	КонецЕсли;
	Возврат СвойстваДокумента;
...
Процедура НастроитьПанельКомандКарточкиВходящегоДокумента(Форма, ДанныеКомпонента)
	...
	НастроитьПанельКомандСозданиеЧерновикаКарточкиВходящегоДокумента(Форма, ДанныеКомпонента);
...
Функция ПредставлениеСостоянияПоДаннымСостоянияЭДО(СостояниеЭДО)
	...
	ПредставленияПоСостояниямЭДО.Вставить(Перечисления.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент,
		ВариантыПредставлений.СоздатьДокументПоЧерновику);
	ПредставленияПоСостояниямЭДО.Вставить(Перечисления.СостоянияДокументовЭДО.ЗакрытСОтзывом,
		ВариантыПредставлений.ЧерновикОтозван);
...
Функция ЦветПанелиСостоянияПоПредставлениям(ПредставлениеПоСостояниюЭДО, ПредставлениеПоОтражениюВУчете)
	...
	ЦветаПоСостояниямЭДО.Вставить(ВариантыПредставленийПоСостояниямЭДО.Создать,
		ВариантыЦвета.Желтый);
	ЦветаПоСостояниямЭДО.Вставить(ВариантыПредставленийПоСостояниямЭДО.СоздатьДокументПоЧерновику,
		ВариантыЦвета.Желтый);
	ЦветаПоСостояниямЭДО.Вставить(ВариантыПредставленийПоСостояниямЭДО.ЧерновикОтозван,
		ВариантыЦвета.Красный);
...
// * СоздатьДокументПоЧерновику - Строка
// * ЧерновикОтозван - Строка
//
Функция ПредставленияСостоянияПоДаннымСостоянияЭДО()
	...
	Представления.Вставить("СоздатьДокументПоЧерновику",
		НСтр("ru = 'Создать документ по черновику'"));
	Представления.Вставить("ЧерновикОтозван", НСтр("ru = 'Черновик отозван'"));
	Возврат Представления;
...
Процедура НастроитьПанельОтраженияВУчетеКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	Если ЭтоВходящийЭДО И ДанныеКомпонента.ЭтоФормализованныйДокумент И Не МногоОбъектовУчета Тогда
		Форма.Элементы.КнопкаПоказатьВсеДокументыУчета.Заголовок = НСтр("ru = 'Еще...'");
	КонецЕсли;
	...
	Форма.Элементы.ГруппаВсеДокументыУчета.Видимость = Не ЭтоВнутреннийДокумент И МногоОбъектовУчета
		...
		Или (ЭтоВходящийЭДО И ЕстьПравоОбработкиДокументов)
...
//  ДанныеКомпонента - Структура:
//  * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
//  * ТипДокументаЧерновика - ПеречислениеСсылка.ТипыДокументовЭДО
Процедура НастроитьПанельКомандСозданиеЧерновикаКарточкиВходящегоДокумента(Форма, ДанныеКомпонента)
	
	Форма.Элементы.КнопкаСоздатьДокументПоЧерновику.Видимость = Ложь;
	
	Если ДанныеКомпонента.ТипДокумента <> Перечисления.ТипыДокументовЭДО.Черновик Тогда
		Возврат;
	КонецЕсли;
	
	КомандаВидна = Ложь;
	ЧерновикиЭДО.ПриОпределенииВидимостиКомандыСозданияДокументаНаОснованииЧерновика(КомандаВидна,
		ДанныеКомпонента.ТипДокументаЧерновика);
		
	Форма.Элементы.КнопкаСоздатьДокументПоЧерновику.Видимость = КомандаВидна;
