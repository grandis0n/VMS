...
Асинх Процедура ВывестиPDFДокументНаФорму(Форма, СведенияОPdfДокументе, ДанныеPdfДокумента) Экспорт
	Если Не ЗначениеЗаполнено(СведенияОPdfДокументе)
		...
		Возврат;
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Элементы, СведенияОPdfДокументе.ИмяЭлемента) Тогда
		ЭлементПоляPDF = Форма.Элементы[СведенияОPdfДокументе.ИмяЭлемента];
	Иначе
		Возврат;
	КонецЕсли;
	...
	#Если ВебКлиент Тогда
		...
		Попытка
			...
			ЭлементПоляPDF.Видимость = Истина;
		Исключение
			РеквизитПоляPdf = Новый ДокументPDF;
			ЭлементПоляPDF.Видимость = Ложь;
...
Процедура ОткрытьЭлектронныйДокументПослеОбработкиПроблемПриФормировании(Результат, Контекст) Экспорт
	...
	Если ПараметрыВыполненияДействийПоЭДО.ОбъектыДействий.ОбъектыУчета.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ДанныеПечатныхФорм(Форма)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПечатныеФормы") Тогда
		...
		ОднаЗаполненнаяПечатнаяФорма =  НастройкиПечатныхФорм.Количество() = 1 
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ТекущаяПечатнаяФорма")
			И Форма.ТекущаяПечатнаяФорма.ВысотаТаблицы <> 0;
		...
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ОднаЗаполненнаяПечатнаяФорма И Форма.ТекущаяПечатнаяФорма <> ТабличныйДокумент Тогда
				ТабличныйДокумент = Форма.ТекущаяПечатнаяФорма;
			КонецЕсли;
