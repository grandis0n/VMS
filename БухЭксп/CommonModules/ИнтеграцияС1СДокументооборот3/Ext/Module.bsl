...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено) Экспорт
	...
	МассивКомандНачатьОбработку = Новый Массив;
	ИмяКоманды = Метаданные.ОбщиеКоманды.ИнтеграцияС1СДокументооборотНачатьОбработку.Имя;
	Для Каждого ЭлементФормы Из Форма.Элементы Цикл
		Если ТипЗнч(ЭлементФормы) = Тип("КнопкаФормы")
			...
			МассивКомандНачатьОбработку.Добавить(ЭлементФормы);
		КонецЕсли;
	КонецЦикла;
	Если МассивКомандНачатьОбработку.Количество() > 1 Тогда
		Для Индекс = 1 По МассивКомандНачатьОбработку.ВГраница() Цикл
			МассивКомандНачатьОбработку[Индекс].Видимость = Ложь;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьЗакладкуОбзорВФорме(Форма, ОбъектДОПредставлениеXDTO, ОбъектXDTO) Экспорт
	...
	СтрокиДереваФайлов = Форма.ДеревоФайлов.ПолучитьЭлементы();
