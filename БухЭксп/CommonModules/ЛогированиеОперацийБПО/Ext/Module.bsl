...
// Выполняет полную очистку истории операций с оборудованием
//
Процедура ОчиститьПолностьюИсториюОперацийСОборудованием() Экспорт

	Если ОбщегоНазначенияБПО.ИспользуетсяПодсистемаЛогирования() Тогда

		МодульОперацииСПодключаемымОборудованием =
			ОбщегоНазначенияБПО.ОбщийМодуль("РегистрыСведений.ОперацииСПодключаемымОборудованием");
		МодульОперацииСПодключаемымОборудованием.ОчиститьРегистрПолностью();
...
// Выполняет полную очистку истории платежных операций
//
Процедура ОчиститьПолностьюИсториюПлатежныхОпераций() Экспорт

	Если ОбщегоНазначенияБПО.ИспользуетсяПлатежныеСистемы() Тогда

		МодульОборудованиеПлатежныеСистемы = ОбщегоНазначенияБПО.ОбщийМодуль("ОборудованиеПлатежныеСистемы");
		МодульОборудованиеПлатежныеСистемы.ОчиститьПолностьюИсториюПлатежныхОпераций();
...
Процедура ОчиститьПолностьюИсториюФискальныхОпераций() Экспорт
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяЧекопечатающиеУстройства() Тогда
		...
		МодульОборудованиеЧекопечатающиеУстройства = ОбщегоНазначенияБПО.ОбщийМодуль("ОборудованиеЧекопечатающиеУстройства");
		МодульОборудованиеЧекопечатающиеУстройства.ОчиститьПолностьюИсториюФискальныхОпераций();
...
// Выполняет полную очистку истории операций очереди чеков
//
Процедура ОчиститьПолностьюИсториюОперацийОчередиЧеков() Экспорт

	Если ОбщегоНазначенияБПО.ИспользуетсяРаспределеннаяФискализация() Тогда

		МодульРаспределеннаяФискализация = ОбщегоНазначенияБПО.ОбщийМодуль("РаспределеннаяФискализация");
		МодульРаспределеннаяФискализация.ОчиститьПолностьюИсториюОперацийОчередиЧеков();
...
// Выполняет полную очистку истории операций проверки КМ
//
Процедура ОчиститьПолностьюИсториюОперацийПроверкиКМ() Экспорт

	Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда

		МодульМенеджерОборудованияМаркировка = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОборудованияМаркировка");
		МодульМенеджерОборудованияМаркировка.ОчиститьПолностьюИсториюОперацийПроверкиКМ();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчиститьПолностьюИсториюОперацийСОборудованием", Истина);
	Методы.Вставить("ОчиститьПолностьюИсториюОперацийПроверкиКМ", Истина);
	Методы.Вставить("ОчиститьПолностьюИсториюОперацийОчередиЧеков", Истина);
	Методы.Вставить("ОчиститьПолностьюИсториюФискальныхОпераций", Истина);
	Методы.Вставить("ОчиститьПолностьюИсториюПлатежныхОпераций", Истина);
