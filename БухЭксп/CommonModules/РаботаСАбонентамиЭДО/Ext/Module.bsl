...
Функция АбонентНеТребуетПриглашений(ИдентификаторУчетнойЗаписи) Экспорт
	...
	СписокИдентификаторов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторУчетнойЗаписи);
	АбонентИТребованиеПриглашения = АбонентыИТребованияПриглашений(СписокИдентификаторов);
	
	Если АбонентИТребованиеПриглашения[ИдентификаторУчетнойЗаписи] = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат АбонентИТребованиеПриглашения[ИдентификаторУчетнойЗаписи];
...
// 	Отбор - см. НовыйОтборАбонентовЭДО
// Возвращаемое значение:
...
Функция ЗапросАбонентовЭДО(ИмяВременнойТаблицы, Отбор = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	АбонентыЭДО.НеТребуетПриглашений КАК НеТребуетПриглашений
...
//
// Параметры:
//  СписокИдентификаторовУчетныхЗаписей - Массив из Строка
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - Строка - Идентификатор ЭДО
//  * Значение - Булево - если Истина - абонент не требует приглашения перед началом ЭДО
Функция АбонентыИТребованияПриглашений(СписокИдентификаторовУчетныхЗаписей) Экспорт
	
	АбонентыИТребованияПриглашений = Новый Соответствие();
	...
	|	РегистрСведений.АбонентыЭДО КАК АбонентыЭДО
	|ГДЕ
	|	АбонентыЭДО.ИдентификаторЭДО В(&ИдентификаторЭДО)";
	
	Запрос.УстановитьПараметр("ИдентификаторЭДО", СписокИдентификаторовУчетныхЗаписей);

	РезультатЗапроса = Запрос.Выполнить();

	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		АбонентыИТребованияПриглашений.Вставить(ВыборкаДетальныеЗаписи.ИдентификаторЭДО, 
			ВыборкаДетальныеЗаписи.НеТребуетПриглашений);
	КонецЦикла;
	
	Возврат АбонентыИТребованияПриглашений;
