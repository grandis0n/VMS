...
Процедура ВыполнитьАутентификациюПоЛогину(ПараметрыАутентификацииПоЛогину) Экспорт
	...
	Если ОбменСБанкамиСлужебныйКлиент.ЭтоМобильныйКлиент() Тогда
		ПолучитьФродПараметрыПоЛогинуДляМК(ДополнительныеПараметры);
...
// Выполняет обработку результата синхронизации
//
// Параметры:
//  РезультатЗадания - Структура - См. ДлительныеОперации.ВыполнитьВФоне
...
Процедура ПослеСинхронизации(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		// Если при запросе статусов зарплатных ведомостей обнаружится, что требуется авторизация, то обработаем эту ситуацию
		Если РезультатОперации.Свойство("ТребуетсяАутентификация") Тогда
			
			ОбменСБанкамиСлужебныйВызовСервера.УдалитьСессиюНаСервереСбербанк(ДополнительныеПараметры.НастройкаОбмена);
			ПолучитьДокументы(ДополнительныеПараметры);
			Возврат;
...
Процедура ПолучитьПараметрыПКПослеПолученияАдресаКомпьютера(РезультатВызова,
	...
	ПараметрыФродМониторинга = НовыйПараметрыФродМониторинга();
	ПараметрыФродМониторинга.IP = ПараметрыВызова[0];
	ПараметрыФродМониторинга.MAC = ПараметрыВызова[1];
...
Процедура ПолучитьПараметрыDevicePrintПослеПолученияПараметровПК(РезультатВызова,
	...
	ДополнительныеПараметры.ПараметрыФродМониторинга.ИдентификаторКомпьютера = ПараметрыВызова.Получить(0);
	ДополнительныеПараметры.ПараметрыФродМониторинга.ИдентификаторПроцессора = ПараметрыВызова.Получить(1);
	ДополнительныеПараметры.ПараметрыФродМониторинга.ИдентификаторBIOS = ПараметрыВызова.Получить(2);
	ДополнительныеПараметры.ПараметрыФродМониторинга.ИдентификаторДиска = ПараметрыВызова.Получить(3);
...
Процедура ПослеПолученияПараметровDevicePrint(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФродМониторинга = ДополнительныеПараметры.ПараметрыФродМониторинга;
	ПараметрыФродМониторинга.РазрядностьОС           = ПараметрыВызова.Получить(0);
	ПараметрыФродМониторинга.РазрядностьПроцессора   = ПараметрыВызова.Получить(1);
	ПараметрыФродМониторинга.ГлубинаЦвета            = ПараметрыВызова.Получить(2);
	ПараметрыФродМониторинга.ШиринаМонитора          = ПараметрыВызова.Получить(3);
	ПараметрыФродМониторинга.ВысотаМонитора          = ПараметрыВызова.Получить(4);
	ПараметрыФродМониторинга.ПлотностьПикселяX       = ПараметрыВызова.Получить(5);
	ПараметрыФродМониторинга.ПлотностьПикселяY       = ПараметрыВызова.Получить(6);
	ПараметрыФродМониторинга.ШиринаРабочегоСтола     = ПараметрыВызова.Получить(7);
	ПараметрыФродМониторинга.СмещениеЧасовогоПояса   = ПараметрыВызова.Получить(8);
	ПараметрыФродМониторинга.УстановленныеКомпоненты = ПараметрыВызова.Получить(9);
	ПараметрыФродМониторинга.Онлайн                  = ПараметрыВызова.Получить(10);
	ПараметрыФродМониторинга.СглаживаниеШрифтов      = ПараметрыВызова.Получить(11);
	ПараметрыФродМониторинга.ЯзыкСистемы             = ПараметрыВызова.Получить(12);
	ПараметрыФродМониторинга.ЯзыкПользователя        = ПараметрыВызова.Получить(13);
	
	ПараметрыФродМониторинга.КонфигурационнаяСтрока = "";
	
	СтруктураНормализованная = ФродПараметровНормализоватьЗначения(ПараметрыФродМониторинга);
...
Процедура ПолучитьФродПараметрыПоЛогинуДляМК(ДополнительныеПараметры)
	
	Если ОбменСБанкамиСлужебныйКлиент.ЭтоМобильныйКлиент() Тогда
		
		ПараметрыФродМониторинга = ПараметрыФродМониторингаДляМК();
		ВыполнитьОповещениеПослеПолученияФродПараметров(ПараметрыФродМониторинга, ДополнительныеПараметры);
	Иначе
		
		ОписаниеОшибки = НСтр("ru = 'Функциональность определения фрод параметров через внешнюю компоненту не поддерживается'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОписаниеОшибки);
		
		// При неуспешном выполнении в обработку оповещения передаем СтруктураВозврата, которая нужна для дальнейшей обработки
		ВыполнитьОбработкуОповещения(
			ДополнительныеПараметры.ОповещениеПослеБазовойАутентификации, ДополнительныеПараметры.СтруктураВозврата);
	КонецЕсли;
...
Функция НовыйПараметрыФродМониторинга()
	
	ПараметрыФродМониторинга = Новый Структура;
	ПараметрыФродМониторинга.Вставить("IP", "");
	ПараметрыФродМониторинга.Вставить("MAC", "");
	ПараметрыФродМониторинга.Вставить("ИдентификаторКомпьютера", "");
	ПараметрыФродМониторинга.Вставить("ИдентификаторПроцессора", "");
	ПараметрыФродМониторинга.Вставить("ИдентификаторBIOS", "");
	ПараметрыФродМониторинга.Вставить("ИдентификаторДиска", "");
	ПараметрыФродМониторинга.Вставить("РазрядностьОС", "");
	ПараметрыФродМониторинга.Вставить("РазрядностьПроцессора", "");
	ПараметрыФродМониторинга.Вставить("ГлубинаЦвета", "");
	ПараметрыФродМониторинга.Вставить("ШиринаМонитора", "");
	ПараметрыФродМониторинга.Вставить("ВысотаМонитора", "");
	ПараметрыФродМониторинга.Вставить("ПлотностьПикселяX", "");
	ПараметрыФродМониторинга.Вставить("ПлотностьПикселяY", "");
	ПараметрыФродМониторинга.Вставить("ШиринаРабочегоСтола", "");
	ПараметрыФродМониторинга.Вставить("СмещениеЧасовогоПояса", "");
	ПараметрыФродМониторинга.Вставить("УстановленныеКомпоненты", "");
	ПараметрыФродМониторинга.Вставить("Онлайн", "");
	ПараметрыФродМониторинга.Вставить("СглаживаниеШрифтов", "");
	ПараметрыФродМониторинга.Вставить("ЯзыкСистемы", "");
	ПараметрыФродМониторинга.Вставить("ЯзыкПользователя", "");
	ПараметрыФродМониторинга.Вставить("КонфигурационнаяСтрока", "");
	
	Возврат ПараметрыФродМониторинга;
	
КонецФункции

Функция ФродПараметровНормализоватьЗначения(ПараметрыФродМониторинга)
	
	СтруктураНормализованная = Новый Структура;
	Для Каждого ЭлементКоллекции Из ПараметрыФродМониторинга Цикл
	...
	Возврат СтруктураНормализованная;
	
КонецФункции

Функция ПараметрыФродМониторингаДляМК()
	
	ИнформацияЭкранаКлиента = ПолучитьИнформациюЭкрановКлиента();
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если СтрНайти(СистемнаяИнформация.ТипПлатформы, "64") > 0 Тогда
		РазрядностьПроцессора = "x64";
	Иначе
		РазрядностьПроцессора = "x86";
	КонецЕсли;
	
	Если СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Android_ARM
		Или СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Android_ARM_64
		Или СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Android_x86
		Или СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Android_x86_64 Тогда
		ИмяОС = "Android";
	ИначеЕсли СистемнаяИнформация.ТипПлатформы = ТипПлатформы.iOS_ARM
		Или СистемнаяИнформация.ТипПлатформы = ТипПлатформы.iOS_ARM_64 Тогда
		ИмяОС = "iOS";
	Иначе
		ИмяОС = "Win"
	КонецЕсли;
	...
	ТекущееВремяСкорректированное = ОбщегоНазначенияКлиент.ДатаСеанса();
	
	СмещениеЧасовогоПояса = ОбменСБанкамиSberAPIСлужебныйВызовСервера.ПолучитьСмещениеЧасовогоПоясаВЧасах();
	
	ПараметрыФродМониторинга = НовыйПараметрыФродМониторинга();
	ПараметрыФродМониторинга.IP                      = "127.0.0.1";
	ПараметрыФродМониторинга.MAC                     = "";
	ПараметрыФродМониторинга.ИдентификаторКомпьютера = Строка(СистемнаяИнформация.ИдентификаторКлиента);
	ПараметрыФродМониторинга.ИдентификаторПроцессора = СистемнаяИнформация.Процессор;
	ПараметрыФродМониторинга.ИдентификаторBIOS       = "";
	ПараметрыФродМониторинга.ИдентификаторДиска      = "";
	ПараметрыФродМониторинга.РазрядностьОС           = ИмяОС;
	ПараметрыФродМониторинга.РазрядностьПроцессора   = РазрядностьПроцессора;
	ПараметрыФродМониторинга.ГлубинаЦвета            = "1024"; //10-bit
	ПараметрыФродМониторинга.ШиринаМонитора          = Формат(ИнформацияЭкранаКлиента[0].Ширина, "ЧГ=");
	ПараметрыФродМониторинга.ВысотаМонитора          = Формат(ИнформацияЭкранаКлиента[0].Высота, "ЧГ=");
	ПараметрыФродМониторинга.ПлотностьПикселяX       = Формат(ИнформацияЭкранаКлиента[0].DPI, "ЧГ=");
	ПараметрыФродМониторинга.ПлотностьПикселяY       = Формат(ИнформацияЭкранаКлиента[0].DPI, "ЧГ=");
	ПараметрыФродМониторинга.ШиринаРабочегоСтола     = Формат(ИнформацияЭкранаКлиента[0].Ширина, "ЧГ=");
	ПараметрыФродМониторинга.СмещениеЧасовогоПояса   = Формат(СмещениеЧасовогоПояса, "ЧГ=");
	ПараметрыФродМониторинга.УстановленныеКомпоненты = "";
	ПараметрыФродМониторинга.Онлайн                  = "TRUE";
	ПараметрыФродМониторинга.СглаживаниеШрифтов      = "FALSE";
	ПараметрыФродМониторинга.ЯзыкСистемы             = Сред(ТекущийЯзыкСистемы(), 1, 2);
	ПараметрыФродМониторинга.ЯзыкПользователя        = Сред(ТекущийЯзык(), 1, 2);
	ПараметрыФродМониторинга.КонфигурационнаяСтрока  = "";
	
	СтруктураНормализованная = ФродПараметровНормализоватьЗначения(ПараметрыФродМониторинга);
	Возврат Новый ФиксированнаяСтруктура(СтруктураНормализованная);
...
Процедура ВыполнитьСинхронизациюПоСчетам(МассивСчетов, ДополнительныеПараметры)
	...
	ЗаполнитьЗначенияСвойств(
		ПараметрыАутентификацииПоЛогину,
		РеквизитыНастройкиОбмена,
		"Организация,Банк,ИмяВнешнегоМодуля");
	ПараметрыАутентификацииПоЛогину.Вставить("Оповещение", ОповещениеПослеАутентификации);
