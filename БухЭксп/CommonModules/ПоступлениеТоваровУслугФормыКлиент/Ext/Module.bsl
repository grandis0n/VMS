...
Процедура ПорядокУчетаРасчетовНажатие(Форма, Элемент, СтандартнаяОбработка, АдресХранилищаЗачетАвансов) Экспорт
	...
	ИспользоватьОплатуЧерезАгентаПоставщика = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.Товары")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия");
	...
	Иначе
		...
		Если ИспользоватьОплатуЧерезАгентаПоставщика Тогда
			ПараметрыФормы.Вставить("ИспользоватьОплатуЧерезАгентаПоставщика", Истина);
			ПараметрыФормы.Вставить("ОплатаЧерезАгентаПоставщика", Объект.ОплатаЧерезАгентаПоставщика);
			ПараметрыФормы.Вставить("АгентПоставщика", Объект.АгентПоставщика);
			ПараметрыФормы.Вставить("ДоговорАгентаПоставщика", Объект.ДоговорАгентаПоставщика);
			ПараметрыФормы.Вставить("СчетУчетаРасчетовСАгентомПоставщика", Объект.СчетУчетаРасчетовСАгентомПоставщика);
			ПараметрыФормы.Вставить("СчетУчетаРасчетовСАгентомПоставщикаПоАвансам", Объект.СчетУчетаРасчетовСАгентомПоставщикаПоАвансам);
		КонецЕсли;
...
Процедура ОбработкаВыбораГрузоотправителяГрузополучателя(Форма, ВыбранноеЗначение) Экспорт
	...
	Объект.ДОПП              = ВыбранноеЗначение.ДОПП;
...
Процедура ОткрытьФормуВыбораЭТРН(Форма) Экспорт
	Объект = Форма.Объект;
	...
	ОткрытьФорму("Документ.ЭлектроннаяТранспортнаяНакладная.ФормаВыбора", СтруктураПараметров, Форма); 
КонецПроцедуры
