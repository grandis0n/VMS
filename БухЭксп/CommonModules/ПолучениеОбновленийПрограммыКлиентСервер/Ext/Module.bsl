		...
		Если ВРег(Лев(ПрочитаннаяСтрока, 17)) = "INSTALLEDLOCATION" Тогда
			...
			Если Не ПустаяСтрока(ПутьКаталогаУстановки) Тогда
				...
				Если ОписательФайла.Существует() Тогда
					...
					Если ПутьККаталогуИзменен(ПутьКаталогаВерсииПлатформы, ПутьКаталогаУстановки) Тогда
						ВызватьИсключение НСтр("ru = 'Некорректный путь к каталогу установки платформы.'");
					КонецЕсли;
