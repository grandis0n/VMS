...
// Возвращает реквизиты оператора платформы цифрового рубля
//
// Возвращаемое значение:
//  Структура - реквизиты оператора платформы цифрового рубля. Содержит поля:
//   * Наименование - Строка - Наименование оператора платформы цифрового рубля
//   * ИНН - Строка - ИНН Банка России
//   * КПП - Строка - КПП Банка России
//   * ОГРН - Строка - ОГРН Банка России
//   * ПлатежныеРеквизиты - Структура - платежные реквизиты оператора. Содержит поля:
//                          ** Наименование - Строка - Наименование банка
//                          ** БИК - Строка - БИК банка
//                          ** КоррСчет - Строка - КоррСчет банка
//
Функция РеквизитыОператораПлатформыЦифровогоРубля() Экспорт
	
	ПлатежныеРеквизиты = Новый Структура;
	ПлатежныеРеквизиты.Вставить("Наименование", НаименованиеПлатформыЦифровогоРубля());
	ПлатежныеРеквизиты.Вставить("БИК", БИКПлатформыЦифровогоРубля());
	ПлатежныеРеквизиты.Вставить("КоррСчет", НомерСчетаПлатформыЦифровогоРубля());
	
	Реквизиты = Новый Структура;
	Реквизиты.Вставить("Наименование", НаименованиеКонтрагентаЦБ());
	Реквизиты.Вставить("ИНН",  ИННПлатформыЦифровогоРубля());
	Реквизиты.Вставить("КПП",  КПППлатформыЦифровогоРубля());
	Реквизиты.Вставить("ОГРН", ОГРНПлатформыЦифровогоРубля());
	Реквизиты.Вставить("ПлатежныеРеквизиты", ПлатежныеРеквизиты);
...
Процедура ПрочитатьДанныеОперацияC2B(ДеталиОперации, НовСтрока, Сторона)
	...
	MCC = ЗначениеСвойстваXDTO(ДеталиОперации.C2B, "MCC");
	Если MCC <> Неопределено Тогда
		НовСтрока.MCC = MCC;
	КонецЕсли;
	
	ПлатежнаяСсылка = ЗначениеСвойстваXDTO(ДеталиОперации.C2B, "PmtLkData.PmtLkID");
	Если ПлатежнаяСсылка <> Неопределено Тогда
		НовСтрока.ПлатежнаяСсылкаЦР = ПлатежнаяСсылка;
	КонецЕсли;
...
Процедура ПрочитатьДанныеОперацияB2C(ДеталиОперации, НовСтрока, Сторона)
	...
	КодВидаДохода = ЗначениеСвойстваXDTO(ДеталиОперации.B2C, "IncmCd");
	Если КодВидаДохода <> Неопределено Тогда
		НовСтрока.КодВидаДохода = КодВидаДохода;
	КонецЕсли;
...
// Возвращает наименование Банка России
//
// Возвращаемое значение:
//  Строка
//
Функция НаименованиеКонтрагентаЦБ()
	
	Наименование = НСтр("ru = 'БАНК РОССИИ'");
	
	Возврат Наименование;
	
КонецФункции

// Возвращает ИНН оператора  платформы цифрового рубля
//
// Возвращаемое значение:
//  Строка
//
Функция ИННПлатформыЦифровогоРубля()
...
// Возвращает КПП оператора  платформы цифрового рубля
//
// Возвращаемое значение:
//  Строка
//
Функция КПППлатформыЦифровогоРубля()
...
// Возвращает ОГРН оператора платформы цифрового рубля
//
// Возвращаемое значение:
//  Строка
//
Функция ОГРНПлатформыЦифровогоРубля()
