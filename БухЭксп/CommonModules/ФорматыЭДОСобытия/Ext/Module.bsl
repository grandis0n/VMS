...
Процедура ЗаполнитьДанные_ИзвещениеОПолучении(Данные, ДанныеДляФормирования, Отказ) Экспорт
	...
	Иначе
		...
		ТипУчастникаЭДО = ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(Абонент.Ссылка);
		ДанныеЮрФизЛица = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(Абонент.Ссылка);
		ПолноеНаименованиеИмя = ?(ТипУчастникаЭДО.ЭтоЮрЛицо, "ОфициальноеНаименование", "ПолноеНаименование");
		ПолноеНаименование = "";
		...
		Если ТипУчастникаЭДО.ЭтоЮрЛицо Тогда
			...
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь + ".НаименованиеОрганизации",
				ПолноеНаименование);
		ИначеЕсли ТипУчастникаЭДО.ЭтоИностранноеЛицо
				И НЕ ЗначениеЗаполнено(СтрЗаменить(ДанныеЮрФизЛица.ИНН, "0", "")) Тогда
			...
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".ИдСтат", "ЮЛ", ПараметрыОбработкиОшибок);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".СтрРег", ДанныеЮрФизЛица.КодСтраныНерезидента, ПараметрыОбработкиОшибок);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".Наим", ПолноеНаименование, ПараметрыОбработкиОшибок);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".КодНПРег", ДанныеЮрФизЛица.ИдентификаторЮридическогоЛица, ПараметрыОбработкиОшибок);
		Иначе
			Путь = ?(ТипУчастникаЭДО.ЭтоФизЛицо,
				"Получатель.ТипУчастника.ФЛ",
				"Получатель.ТипУчастника.ИП");
	...
	ТипУчастникаЭДО = ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(ДанныеДляФормирования.Участники.Отправитель.Ссылка);
	...
	Иначе
		Путь = ?(ТипУчастникаЭДО.ЭтоФизЛицо,
			"Отправитель.ТипУчастника.ФЛ",
			"Отправитель.ТипУчастника.ИП");
...
Функция ИнформацияОбУчастникеКоммерческихПредложенийПоДаннымЭлектронногоДокумента(ДанныеУчастника)
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "PrivatePerson") <> Неопределено Тогда
		
		ФизическоеЛицо = Новый Структура;
		ФизическоеЛицо.Вставить("ИНН",
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "PrivatePerson.inn"));
		ФизическоеЛицо.Вставить("Имя",
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "PrivatePerson.name"));
		ФизическоеЛицо.Вставить("Фамилия",
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "PrivatePerson.surname"));
		ФизическоеЛицо.Вставить("Отчество",
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "PrivatePerson.patronymic"));
		
		Организация.Вид.Вставить("ФизическоеЛицо", ФизическоеЛицо);
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеУчастника, "individualEntrepreneur") <> Неопределено Тогда
