...
//   ** ПодписаниеВЭтомПриложении - Булево
...
Процедура ПолучитьДолжностьПослеОпределенияСертификатаПодписи(РезультатВыбораСертификата, ПараметрыОповещения) Экспорт
	...
	ПодписаниеВЭтомПриложении = 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатВыбораСертификата.ОписаниеДанных, 
			"ПодписаниеВЭтомПриложении", Истина);

	Если СведенияОДолжности = Неопределено И Не ПодписаниеВЭтомПриложении Тогда
			
			// Так как сертификат госключа сейчас не попадает в список доступных сертификатов,
			// получим данные об идентификаторе ЭДО из отбора. Отбор всегда должен быть заполнен, иначе процедура
			// подписания документа не перейдет непосредственно к подписанию с ошибкой: "Нет доступных сертификатов".
			ИдентификаторЭДО = "";
			Для Каждого КлючИЗначение Из ДолжностиПоСертификатам Цикл
				ИдентификаторЭДО = КлючИЗначение.Значение.ИдентификаторОрганизации;
			КонецЦикла;
			
			Должность = 
				УчетныеЗаписиЭДОКлиент.ДолжностьПоСертификатуУчетнойЗаписи(ИдентификаторЭДО, ВыбранныйСертификат);
					
			СведенияОДолжности = Новый Структура("Должность, ИдентификаторОрганизации", Должность, ИдентификаторЭДО);
	...
	Если Не ЗначениеЗаполнено(Должность) Тогда
		...
		СвойстваСертификата =
			КриптографияБЭДВызовСервера.СвойстваСубъектаИСвойстваИздателяСертификатаПоСсылке(ВыбранныйСертификат);
		ЭтоСертификатИП = ЗначениеЗаполнено(СвойстваСертификата.СвойстваСубъекта.ОГРНИП) 
			Или СтрДлина(СвойстваСертификата.СвойстваСубъекта.ИННЮЛ) = 12;
		ЭтоСертификатФЛ = КриптографияБЭДКлиент.ЭтоВидСертификатаФизическогоЛица(СвойстваСертификата.СвойстваСубъекта,
			СвойстваСертификата.СвойстваИздателя);
		...
		ЭтоПодписаниеСамозанятым = ЭтоСертификатФЛ И Не ТребуетсяДоверенность;
...
Процедура ПродолжитьПодписаниеПослеУказанияДолжностиПодписанта(РезультатВвода, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		УчетныеЗаписиЭДОКлиент.ЗаписатьДолжностьСертификатаУчетнойЗаписи(РезультатВвода,
