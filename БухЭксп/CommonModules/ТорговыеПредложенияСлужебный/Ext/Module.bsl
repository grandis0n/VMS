...
Функция ЗапросСервисаСозданиеОбновлениеТорговогоПредложения(
	...
	Если ПараметрыЗапроса.Наличие = 0 Тогда
		Наличие = "OUT_OF_STOCK";
	ИначеЕсли ПараметрыЗапроса.Наличие = 1 Тогда
		Наличие = "LOW";
...
Функция ПредставлениеОтносительногоОстатка(КодНаличия)
	...
	Если КодНаличия = 0 Тогда
		Результат = НСтр("ru = 'Нет на складе'");
	ИначеЕсли КодНаличия = 1 Тогда
...
Процедура КонвертироватьЗначениеСервиса(Значение, Свойство, Правило)
	...
	ИначеЕсли Правило = "ДесериализацияНаличие" Тогда
		...
		Для каждого ЭлементКоллекции Из Значение Цикл
			...
			Если ЗначениеСвойства = "OUT_OF_STOCK" Тогда
				НовоеЗначение = 0;
			ИначеЕсли ЗначениеСвойства = "LOW" Тогда
				НовоеЗначение = 1;
...
Функция ДопустимыеЗначенияСтавокНДСДляПубликации()
	...
	Результат.Добавить("VAT_0");
	Результат.Добавить("VAT_5");
	Результат.Добавить("VAT_7");
