...
Процедура ПолучитьНастройкиВидимостиЭлементовФормы(ИДОтчета, ИДРедакцииОтчета, НастройкиВидимости) Экспорт
	...
	Если ИДОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		НастройкиВидимости.ПостояннаяВидимость = ТарификацияБПВызовСервераПовтИсп.РазрешенУчетРегулярнойДеятельности();
		НастройкиВидимости.ПоказыватьВФорме = Истина;
		НастройкиВидимости.ТекстПредупреждения = ПомощникЗаполнения3НДФЛ.ТекстПредупрежденияРедакцииФормы(ИДРедакцииОтчета);
...
Функция ПолучитьСведенияОПоказателяхОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетКосвенныеНалогиТамСоюз" Тогда
		...
		УчетНДСФормированиеОтчетности.ПолучитьСведенияОПоказателяхОтчетаКосвенныеНалогиПриИмпорте(ИДРедакцииОтчета, ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетАкцизыАлкоголь" Тогда
		...
		УчетАкцизов.ПолучитьСведенияОПоказателяхОтчетаАкцизыАлкоголь(ПоказателиОтчета, ИДРедакцииОтчета);
...
// Если не разрешен учет регулярной деятельности заполняется только ограниченный набор отчетов.
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетКосвенныеНалогиТамСоюз" Тогда
		...
		УчетНДСФормированиеОтчетности.ЗаполнитьОтчетКосвенныеНалогиПриИмпорте(ИДРедакцииОтчета, ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетАкцизыАлкоголь" Тогда
			...
			УчетАкцизов.ЗаполнитьРегламентированныйОтчетАкцизыАлкоголь(ИДРедакцииОтчета, ПараметрыОтчета, Контейнер);
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетОценочнаяКартаУБ" Тогда
		
		ЗаполнениеОценочнойКарты.ЗаполнитьОтчетОценочнаяКарта(ПараметрыОтчета, Контейнер);
		
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеУСН" Тогда
		
		Если ИДРедакцииОтчета = "Форма2025_1" Тогда
			УчетУСН.ЗаполнитьЕдиноеУведомлениеУСН(ПараметрыОтчета, Контейнер);
		КонецЕсли;
