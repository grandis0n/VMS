...
Процедура ИнициализацияКомпоновщикаНастроек(Форма, ОрганизацияИзменилась = Ложь, ИмяВариантаНастроек = "") Экспорт
	...
	ИмяОтчета = CW_БухгалтерскиеОтчетыМСФО.ИмяОтчета(Форма);
	...
	Если ОрганизацияИзменилась Тогда
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ОрганизацияПриИзменении(Форма, Форма.Отчет.Организация);
	КонецЕсли;
...
Процедура УстановитьНастройкиПоУмолчанию(ФормаОтчета) Экспорт
	...
	ИдентификаторОбъекта = CW_БухгалтерскиеОтчетыМСФО.ИмяОтчета(ФормаОтчета);
	...
	ЗначенияПоУмолчанию = CW_БухгалтерскиеОтчетыМСФО.БазовыеРеквизитыЗначенияПоУмолчанию();
	...
	Если Отчет.Свойство("Подразделение") Тогда
		Если CW_БухгалтерскийУчетПереопределяемыйМСФО.ВестиУчетПоПодразделениям() Тогда
			Отчет.Подразделение = CW_БухгалтерскийУчетПереопределяемыйМСФО.ПолучитьЗначениеПоУмолчанию("ОсновноеПодразделениеОрганизации");
		КонецЕсли;
	ИначеЕсли Отчет.Свойство("ПодразделениеОрганизации") Тогда
		Если CW_БухгалтерскийУчетПереопределяемыйМСФО.ВестиУчетПоПодразделениям() Тогда
			Отчет.ПодразделениеОрганизации = CW_БухгалтерскийУчетПереопределяемыйМСФО.ПолучитьЗначениеПоУмолчанию("ОсновноеПодразделениеОрганизации");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьДополнительныеПоля(ПараметрыОтчета, КомпоновщикНастроек) Экспорт
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(КомпоновщикНастроек, "РасположениеРеквизитов", РасположениеРеквизитов); 
	
	Для Каждого ПолеГруппировки Из ПараметрыОтчета.ДополнительныеПоля Цикл 
		Если ПолеГруппировки.Использование Тогда
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьВыбранноеПоле(КомпоновщикНастроек, ПолеГруппировки.Поле);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьГруппировкуПоПериоду(ПараметрыОтчета, Структура) Экспорт
	...
	Если ПараметрыОтчета.Периодичность = 2 Тогда
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Структура.Отбор, "Регистратор", 0, ВидСравненияКомпоновкиДанных.Заполнено);
...
//
Процедура ДобавитьОтборПоОрганизации(ПараметрыОтчета, КомпоновщикНастроек, ИмяОбъектаДанных = "РегистрБухгалтерии.CW_МСФО", ВПользовательскиеНастройки = Истина) Экспорт
	
	// Получим список организаций, доступных текущему пользователю.
	Если ВРег(ИмяОбъектаДанных) = ВРег("РегистрБухгалтерии.CW_МСФО") Тогда
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		...
		Если ПараметрыОтчета.ВключатьОбособленныеПодразделения Тогда
			...
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаОтборОрганизация, "Организация"                    , ПараметрыОтчета.Организация);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ГруппаОтборОрганизация, "Организация.ГоловнаяОрганизация", ПараметрыОтчета.Организация);
		Иначе
			НовыйОтбор = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(КомпоновщикНастроек, "Организация", ПараметрыОтчета.Организация,,, ВПользовательскиеНастройки);
...
Процедура УстановитьПараметрОрганизация(КомпоновщикНастроек, Знач Организация) Экспорт
	...
	Если ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(КомпоновщикНастроек, "Организация", ЗначениеПараметра);
	...
	Для Каждого ВыбранноеПоле Из КомпоновщикНастроек.Настройки.Выбор.Элементы Цикл
		Если ТипЗнч(ВыбранноеПоле) = Тип("ВыбранноеПолеКомпоновкиДанных") 
			...
			Если Группировка.НайтиСтроки(Новый Структура("Поле", МассивПолей[0])).Количество() > 0 Тогда
				...
				НоваяСтрока.Представление = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьРазмещениеДополнительныхПолей(КомпоновщикНастроек) 
	...
	РасположениеРеквизитов = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметрВывода(КомпоновщикНастроек, "РасположениеРеквизитов");	
...
Функция ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(Тип = 1) Экспорт
	...
	Возврат ХранилищеОбщихНастроек.Загрузить(CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьКлючНастройкиПоТипуЗадолженности(Тип));
...
Процедура СохранитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(СписокСчетов, Тип = 1) Экспорт
	...
	ОписаниеНастроек.Представление = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьОписаниеНастройкиПоТипуЗадолженности(Тип);
	
	ХранилищеОбщихНастроек.Сохранить(CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьКлючНастройкиПоТипуЗадолженности(Тип),, СписокСчетов, ОписаниеНастроек);
...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	CW_Отчеты.ЗаменитьТекстВТаблицеРезультата(Результат, "Итого"		, "Total:");	
	CW_Отчеты.ЗаменитьТекстВТаблицеРезультата(Результат, "Отбор:"		, "Selection:");	
	CW_Отчеты.ЗаменитьТекстВТаблицеРезультата(Результат, "Сортировка:"	, "Sorting:");	
	CW_Отчеты.ЗаменитьТекстВТаблицеРезультата(Результат, "Обороты за"	, "Turnovers for");	
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(ОтчетОбъект, Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	ПараметрНастройкиОтчета = 
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(НовыеНастройкиКД, "НастройкиОтчета");
	...
	Если ПараметрНастройкиОтчета <> Неопределено
		И CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(НовыеПользовательскиеНастройкиКД, "НастройкиОтчета") = Неопределено Тогда
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(
...
Процедура УстановитьМакетОформленияОтчета(ПараметрыОтчета, НастройкаКомпоновкиДанных) Экспорт
	...
	Если ПараметрыОтчета.Свойство("МакетОформления", МакетОформления) Тогда
		Если НЕ ЗначениеЗаполнено(МакетОформления) ИЛИ МакетОформления = "МакетОформленияОтчетовЗеленый" Тогда
			МакетОформления = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИмяМакетаОформленияПоУмолчанию();
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(НастройкаКомпоновкиДанных, "МакетОформления", МакетОформления);
...
Процедура ВывестиЗаголовокОтчета(ПараметрыОтчета, КомпоновщикНастроек, Результат) Экспорт
	...
	Макет = ПолучитьОбщийМакет("CW_ОбщиеОбластиСтандартногоОтчетаМСФО");
	...
	// Организация
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		ТекстОрганизация = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьТекстОрганизация(ПараметрыОтчета.Организация, ПараметрыОтчета.ВключатьОбособленныеПодразделения);
	...
	Если ПоказыватьВыводимыеДанные Тогда 
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru = 'Output data:'");
	...
	Если Не ПустаяСтрока(ТекстОтбор) Тогда
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru = 'Selection:'");
		ОбластьОписаниеНастроек.Параметры.ОписаниеНастроекОтчета = ТекстОтбор;
		Результат.Вывести(ОбластьОписаниеНастроек);
	КонецЕсли;
	
	Если ИдентификаторОбъекта <>  "ОтчетОбъект.CW_ОборотноСальдоваяВедомостьМСФО" 
		И ИдентификаторОбъекта <> "ОтчетОбъект.CW_КарточкаСчетаМСФО"
		И ИдентификаторОбъекта <> "ОтчетОбъект.CW_ОтчетПоПроводкамМСФО"  Тогда
		// Сортировка
		ТекстСортировка = "";
		Для Каждого СтрокаПорядок Из КомпоновщикНастроек.Настройки.Порядок.Элементы Цикл
			Если СтрокаПорядок.Использование Тогда
				ТекстСортировка = ТекстСортировка + CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьСвойствоПоля(КомпоновщикНастроек, СтрокаПорядок.Поле);
		...
		Если Не ПустаяСтрока(ТекстСортировка) Тогда
			ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru = 'Sort:'");
...
Функция ВычислитьСуммуВыделенныхЯчеекТабличногоДокумента(Знач Результат, КэшВыделеннойОбласти) Экспорт
	...
	Для Каждого КлючИЗначение Из КэшВыделеннойОбласти Цикл
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ВычислитьСуммуВЯчейках(Сумма, СтруктураАдресВыделеннойОбласти, Результат);
	КонецЦикла;
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, СохранятьТолькоРеквизиты = Ложь) Экспорт
	...
	Если НЕ СохранятьТолькоРеквизиты Тогда
		...
		CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметр(Настройки, "НастройкиОтчета", Новый ХранилищеЗначения(Неопределено), Истина);
	КонецЕсли;
	...
	Иначе 
		...
		Если Настройки.ДополнительныеСвойства.Свойство("ДанныеОтчета") Тогда
			...
			Если ДополнительныеСвойства.Свойство("МакетОформления") Тогда
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УстановитьПараметрВывода(
			...
			Для Каждого ЭлементСтруктуры Из ДополнительныеСвойства Цикл
				...
				Если ФормаОтчета.Отчет.Свойство(ЭлементСтруктуры.Ключ) Тогда
					...
					ИначеЕсли ЭлементСтруктуры.Ключ = "Подразделение"
						И Не CW_БухгалтерскийУчетПереопределяемыйМСФО.ВестиУчетПоПодразделениям() Тогда
						Продолжить;
			...
			Если ДополнительныеСвойства.Свойство("ВидимостьПанелиНастроек") 
				...
				Если ЕстьЭлементФормы(ФормаОтчета, "ПанельНастроек") Тогда
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИзменитьЗаголовокКнопкиПанельНастроек(
...
Процедура ВывестиОписаниеОтбора(ФормаОтчета, ОписаниеОтбора)
	...
	// Какое-то исключение
	Если CW_БухгалтерскиеОтчетыМСФО.ИмяОтчета(ФормаОтчета) = "ОборотноСальдоваяВедомость" Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, ИдентификаторОбъекта, Расшифровка) Экспорт
	...
	Если ИдентификаторОбъекта = "АнализСубконто"
		...
		Если ИдентификаторОбъекта = "ГлавнаяКнига" Тогда
			Если ТипЗнч(Расшифровка) = Тип("ПланСчетовСсылка.CW_МСФО") Тогда
		...
		Если ПервыйЭлемент <> Неопределено Тогда
			Если ИдентификаторОбъекта = "ОборотноСальдоваяВедомость" Тогда
				Если Не ЕстьПоказатель И ЗначениеЗаполнено(ПервыйЭлемент.Значение)  И Не CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПростойТип(ПервыйЭлемент.Значение) Тогда
			...
			Иначе
				...
				Иначе
					Если ИдентификаторОбъекта = "СводныеПроводки"
						...
						УстановитьВсеПоказатели = Истина;
					Иначе 
						Если ЗначениеЗаполнено(ПервыйЭлемент.Значение) И Не CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПростойТип(ПервыйЭлемент.Значение) Тогда
							Представление = СтрЗаменить(ПредставлениеОткрытьОбъект, "%Значение%", ПервыйЭлемент.Значение);
							СписокПунктовМеню.Добавить(ПервыйЭлемент.Значение, Представление);
						КонецЕсли;
						Если ИдентификаторОбъекта = "АнализСубконто" Тогда
	...
	Если ПоляТекущейРасшифровки.Количество() > 0
		...
		ИЛИ ПоляТекущейРасшифровки.Количество() = 1) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеПоля)) Тогда
				ВозможныеРасшифровки.Добавить(ПоляТекущейРасшифровки[0].Значение, СтрШаблон(НСтр("ru = 'Open""%1""'"), ПоляТекущейРасшифровки[0].Значение));
			КонецЕсли;
			
	КонецЕсли;
	
	РасшифровочныеОтчеты = CW_БухгалтерскиеОтчетыРасшифровкаМСФО.РасшифровочныеОтчеты(ДанныеРасшифровки, ИдентификаторРасшифровки, ИмяОтчета);
...
Функция ЕстьПравоПересчитатьИтоги() Экспорт
	...
	Возврат ПравоДоступа("УправлениеИтогами", Метаданные.РегистрыБухгалтерии.CW_МСФО);
...
//
// Параметры:
//  МестоРасхождения - Структура - см. CW_БухгалтерскиеОтчетыКлиентСерверМСФО.МестоРасхожденияИтогов()
//
// Возвращаемое значение:
...
Функция ПересчитатьИтоги(МестоРасхождения) Экспорт
	...
	КлючФоновогоЗадания = "ПересчетИтоговCW_МСФО";
	...
	Если Отказ Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Расхождение итогов.Параллельный запуск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Предупреждение, , , ДлительнаяОперация.ПодробноеПредставлениеОшибки);

	Иначе

		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне("CW_БухгалтерскиеОтчетыМСФО.ПересчитатьИтоги",
...
Функция ТипыСвязанныеСОрганизацией() Экспорт
	...
	Возврат CW_БухгалтерскийУчетПереопределяемыйМСФО.ТипыСвязанныеСОрганизацией();
...
Процедура ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма)
	...
	Если РеквизитыФормы.Свойство("ОписанияТиповВидовСубконто") Тогда
		...
		Форма.ОписанияТиповВидовСубконто.Вставить("Контрагент"                    , ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Контрагенты.ТипЗначения);
		Форма.ОписанияТиповВидовСубконто.Вставить("ДоговорКонтрагента"            , ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры.ТипЗначения);
		Форма.ОписанияТиповВидовСубконто.Вставить("ДокументРасчетовСКонтрагентами", ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.ДокументыРасчетовСКонтрагентами.ТипЗначения);
		Форма.ОписанияТиповВидовСубконто.Вставить("Номенклатура"                  , ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Номенклатура.ТипЗначения);
		Форма.ОписанияТиповВидовСубконто.Вставить("Склад"                  		, ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Склады.ТипЗначения);
	КонецЕсли;
	...
	МенеджерОтчета = Отчеты[CW_БухгалтерскиеОтчетыМСФО.ИмяОтчета(Форма)];
	...
	ПараметрМакетОформления = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметрВывода(Отчет.КомпоновщикНастроек.Настройки, "МакетОформления");
	...
	Если ЕстьЭлементФормы(Форма, "ГруппировкаТипГруппировки") Тогда
		CW_БухгалтерскиеОтчетыМСФО.УстановитьУсловноеОформлениеТипГруппировки(Форма);
	КонецЕсли;
...
// (см. CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйУниверсальныеНастройки),
...
Процедура ИнициализацияРасшифровкиПоНастройкам(Отчет, Параметры, РеквизитыФормы, Форма)
	...
	Если НЕ Параметры.Свойство("УниверсальныеНастройки", НастройкиРасшифровки) Тогда
		...
		НастройкиРасшифровки = CW_БухгалтерскиеОтчетыРасшифровкаМСФО.НастройкиРасшифровки(
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.УдалитьНеиспользуемыйОтбор(Отчет.КомпоновщикНастроек);
	...
	Если CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ОтчетПоддерживаетУниверсальныеНастройки(ИмяОтчета) Тогда
		
		Форма.ИзменениеСхемыКомпоновкиДанныхНаСервере();
		
		НастройкиКоторыеНужноЗаполнитьПоУмолчанию = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.РазделыНастроекСтандартногоОтчета();
...
Функция ПолучитьНастройкиДляРасшифровки(ИДРасшифровки, ИдентификаторОбъекта, МассивПолей, ОтчетОбъект, УстановитьВсеПоказатели)
	...
	Если ИдентификаторОбъекта = "ГлавнаяКнига" Тогда
		...
		Если ИДРасшифровки = "ОборотыСчета" Тогда
			ДополнительныеСвойства.Вставить("Счет", Счет);
			Если ЕстьКорЗначение Тогда
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "КорСчет", КорСчет, ВидСравненияКомпоновкиДанных.ВИерархии);	
			КонецЕсли;
		ИначеЕсли ИДРасшифровки = "ОтчетПоПроводкам" Тогда
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "СчетДт", Счет, ВидСравненияКомпоновкиДанных.ВИерархии);
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "СчетКт", КорСчет, ВидСравненияКомпоновкиДанных.ВИерархии);
		КонецЕсли;
	Иначе 
			...
			Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
				...
				ИначеЕсли ПолеОтбора = "Счет" И (ИдентификаторОбъекта = "АнализСчета" Или ИдентификаторОбъекта = "ОборотыСчета") Тогда
					...
					Если ЕстьСчет И ЗначениеЗаполнено(БухТипРесурса) ИЛИ Не ЕстьСчет Тогда
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора + БухТипРесурса, ЗначениеСчет, ВидСравненияКомпоновкиДанных.ВИерархии);
					КонецЕсли;
				...
				ИначеЕсли СтрНайти(ПолеОтбора, "Субконто") = 1 И (ИдентификаторОбъекта = "АнализСчета" ИЛИ ИдентификаторОбъекта = "ОборотыСчета") Тогда
					...
					// Устанавливаем отбор.
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеСубконто, ЗначениеОтбора, ВидСравненияОтбора);
				...
				ИначеЕсли ПолеОтбора = "Подразделение" И (ИдентификаторОбъекта = "АнализСчета" ИЛИ ИдентификаторОбъекта = "ОборотыСчета") Тогда
					...
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеПодразделение, ЗначениеОтбора, ?(Отбор.Иерархия, ВидСравненияКомпоновкиДанных.ВИерархии, ВидСравненияКомпоновкиДанных.Равно));						
				ИначеЕсли (ПолеОтбора = "Счет" Или ПолеОтбора = "КорСчет") И Не ЕстьСчет Тогда
					Если ПолеОтбора = "КорСчет" И Не ПустаяСтрока(БухТипРесурса) Тогда      
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "Счет" + ?(БухТипРесурса = "Дт", "Кт", "Дт"), ЗначениеОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);	
					Иначе
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);		
					КонецЕсли;
				ИначеЕсли СтрНайти(ПолеОтбора, "Субконто") = 1 Тогда
					Если ИдентификаторОбъекта = "ОборотыМеждуСубконто" Тогда
						...
						Иначе
							...
							CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора, ВидСравненияОтбора);
							...
							CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеВидСубконто, ОтчетОбъект.СписокВидовСубконто[ИндексСубконто - 1].Значение);
					...
					ИначеЕсли ИдентификаторОбъекта = "ОборотноСальдоваяВедомость" Тогда
						...
						СвойстваИсходногоСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(Счет);
						...
						Для НомерСубконто = 1 По СвойстваИсходногоСчета.КоличествоСубконто Цикл
							...
							Если СвойстваИсходногоСчета["ВидСубконто" + НомерСубконто + "ТипЗначения"].СодержитТип(ТипЗначенияОтбора) Тогда
								...
								CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "Субконто" + НомерСубконто, ЗначениеОтбора, ВидСравненияОтбора);
					...
					Иначе
						...
						СвойстваИсходногоСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(ОтчетОбъект.Счет);
						
						ОтборУстановлен = Ложь;
						ИмяПоляСубконто = "Субконто" + Сред(Отбор.Поле, 9, 1);
						ВидСубконтоОтбора = СвойстваИсходногоСчета["Вид" + ИмяПоляСубконто];
						Для ИндексСубконто = 1 По CW_БухгалтерскийУчетМСФО.МаксимальноеКоличествоСубконто() Цикл
							Если СвойстваСчета["ВидСубконто" + ИндексСубконто] = ВидСубконтоОтбора Тогда
								...
								CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, НовоеПолеОтбора, ЗначениеОтбора, ВидСравненияОтбора);
					...
					Если ИдентификаторОбъекта = "ОборотыМеждуСубконто" Тогда
						Если КорСчет <> Неопределено Тогда
							...
							Если СтрокаСубконтоКорСчета <> Неопределено Тогда
								...
								// Устанавливаем отбор.
								CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеКорСубконто, ЗначениеОтбора, ВидСравненияОтбора);	
							КонецЕсли;
						...
						Иначе
							...
							// Устанавливаем отбор по виду субконто.
							CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеВидСубконто, ОтчетОбъект.СписокВидовСубконто[ИндексКорСубконто - 1].Значение);
				...
				ИначеЕсли ПолеОтбора = "Подразделение" тогда
					...
					Если Отбор.Иерархия Тогда
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);
					Иначе
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора);
					КонецЕсли;
				...
				Если СписокПолейОтборов.Найти(Строка(Отбор.ЛевоеЗначение)) = Неопределено Тогда
					Если СтрНайти(Отбор.ЛевоеЗначение, "Субконто") = 1 И ИдентификаторОбъекта = "АнализСубконто" И ИДРасшифровки = "КарточкаСчета" Тогда
						...
						Если НайденнаяСтрока <> Неопределено Тогда
							...
							CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, СтрЗаменить(Строка(ПолеОтбора), "Субконто" + ИндексСубконто, "Субконто" + ИндексСубконтоОтчет), ЗначениеОтбора, Отбор.ВидСравнения);
						КонецЕсли;
					...
					ИначеЕсли ПолеОтбора = Новый ПолеКомпоновкиДанных("Счет")
						И (ИДРасшифровки = "КарточкаСубконто" 
							...
							CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора, Отбор.ВидСравнения);
					КонецЕсли;
		...
		Если ИдентификаторОбъекта = "ОборотыМеждуСубконто" Тогда
			Если ДобавитьОтборПоВидСубконто Тогда
				...
				Для Каждого ВидСубконто Из ОтчетОбъект.СписокВидовСубконто Цикл
					Если ЗначениеЗаполнено(ВидСубконто.Значение) Тогда
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "ВидСубконто" + Индекс, ВидСубконто.Значение);
						Индекс = Индекс + 1;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			Если ДобавитьОтборПоВидКорСубконто Тогда
				...
				Для Каждого ВидСубконто Из ОтчетОбъект.СписокВидовКорСубконто Цикл
					Если ЗначениеЗаполнено(ВидСубконто.Значение) Тогда
						CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(ПользовательскиеОтборы, "ВидКорСубконто" + Индекс, ВидСубконто.Значение);
						Индекс = Индекс + 1;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Если Период <> Неопределено Тогда
		...
		ДополнительныеСвойства.Вставить("КонецПериода" , CW_БухгалтерскиеОтчетыКлиентСерверМСФО.КонецПериода(Период, ?(Периодичность = Неопределено, ОтчетОбъект.Периодичность, Периодичность)));
	Иначе
...
Процедура ЗаполнитьГруппировкиИзНастроек(КомпоновщикНастроек, Структура, Группировка)
	...
	Если (НРег(Поле) <> "подразделение" ИЛИ CW_БухгалтерскийУчетПереопределяемыйМСФО.ВестиУчетПоПодразделениям())
...
Процедура УстановитьОрганизацию(Форма)
	...
	ОсновнаяОрганизация = CW_БухгалтерскийУчетПереопределяемыйМСФО.ПолучитьЗначениеПоУмолчанию("ОсновнаяОрганизация");
...
Процедура ПодставитьИменаСубконто(ФормаОтчета, ТекстОтбор)
	...
	СвойстваСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(ФормаОтчета.Отчет.Счет);
...
Функция ЗаполнитьПараметрыОтчетаИзНастроекКомпоновщика(ОтчетОбъект, КомпоновщикНастроекКД)
	...
	ПараметрНастройкиОтчета = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(КомпоновщикНастроекКД, "НастройкиОтчета");
	...
	ПараметрПериод = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПолучитьПараметр(КомпоновщикНастроекКД, "ПериодОтчета");
