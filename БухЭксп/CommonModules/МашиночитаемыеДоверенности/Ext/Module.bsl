...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьДанныеДляПереходаНаНовуюВерсию_КонвертацияПравилПроверки");
	
	Методы.Вставить("ВыполнитьПроверкуПодписиДокументовПоМЧД", Истина);
...
// Возвращает сведения доверителя
// 
// Параметры:
//  Доверенность - СправочникСсылка.МЧД003
...
//   * ОГРН - Строка
//   * Наименование - Строка
Функция СведенияОсновногоДоверителя(Доверенность) Экспорт
	
	СведенияДоверителя = Новый Структура;
	СведенияДоверителя.Вставить("ИНН", "");
	СведенияДоверителя.Вставить("КПП", "");
	СведенияДоверителя.Вставить("ОГРН", "");
	СведенияДоверителя.Вставить("Наименование", "");
	
	ДвоичныеДанные = ПолныеДанныеДоверенностиНаСервереМЧД(Доверенность);
	Результат = ДанныеИзФайлаОбмена(ДвоичныеДанные);
	Если Не Результат.Успех Тогда
		Возврат СведенияДоверителя;
	КонецЕсли;
	
	ПутиКДаннымДоверителя = Новый Массив;
	// Первичная доверенность
	ПутиКДаннымДоверителя.Добавить("Довер.СвДоверит.Доверит.РосОргДовер.СвРосОрг");
	ПутиКДаннымДоверителя.Добавить("Довер.СвДоверит.Доверит.ИнОргДовер.СвИнОрг");
	ПутиКДаннымДоверителя.Добавить("Довер.СвДоверит.Доверит.ИПДовер");
	// 003 Передоверие
	ПутиКДаннымДоверителя.Добавить("Передов.СвПервДовер.СвДоверПерв.ДоверитПерв.РосОргДовер");
	ПутиКДаннымДоверителя.Добавить("Передов.СвПервДовер.СвДоверПерв.ДоверитПерв.ИнОргДовер");
	ПутиКДаннымДоверителя.Добавить("Передов.СвПервДовер.СвДоверПерв.ДоверитПерв.ИПДовер");
	// 002 Передоверие
	ПутиКДаннымДоверителя.Добавить("Передов.СвДовПер.СвОснДовер.СвДовер0.РосОргДовер");
	ПутиКДаннымДоверителя.Добавить("Передов.СвДовПер.СвОснДовер.СвДовер0.ИнОргДовер");
	ПутиКДаннымДоверителя.Добавить("Передов.СвДовПер.СвОснДовер.СвДовер0.ИПДовер");
	
	ПредварительныеСведения = Новый Структура;
	ПредварительныеСведения.Вставить("ИННЮЛ", "");
	ПредварительныеСведения.Вставить("ИННФЛ", "");
	ПредварительныеСведения.Вставить("КПП", "");
	ПредварительныеСведения.Вставить("ОГРН", "");
	ПредварительныеСведения.Вставить("ОГРНИП", "");
	ПредварительныеСведения.Вставить("НаимОрг", "");
	ПредварительныеСведения.Вставить("НаимИО", "");
	ПредварительныеСведения.Вставить("НаимИП", "");
	
	Для Каждого Путь Из ПутиКДаннымДоверителя Цикл
		ЗначениеСвойстваXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Результат.ДанныеДоверенности.Документ, Путь);
		Если ЗначениеСвойстваXDTO = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ПредварительныеСведения, ЗначениеСвойстваXDTO); 
		
		Если ЗначениеЗаполнено(ПредварительныеСведения.ИННЮЛ) Тогда
			СведенияДоверителя.ИНН = ПредварительныеСведения.ИННЮЛ;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.ИННФЛ) Тогда
			СведенияДоверителя.ИНН = ПредварительныеСведения.ИННФЛ;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.КПП) Тогда
			СведенияДоверителя.КПП = ПредварительныеСведения.КПП;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.ОГРН) Тогда
			СведенияДоверителя.ОГРН = ПредварительныеСведения.ОГРН;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.ОГРНИП) Тогда
			СведенияДоверителя.ОГРН = ПредварительныеСведения.ОГРНИП;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.НаимОрг) Тогда
			СведенияДоверителя.Наименование = ПредварительныеСведения.НаимОрг;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.НаимИО) Тогда
			СведенияДоверителя.Наименование = ПредварительныеСведения.НаимИО;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПредварительныеСведения.НаимИП) Тогда
			СведенияДоверителя.Наименование = ПредварительныеСведения.НаимИП;
		КонецЕсли;
...
Функция НовыйОтборМЧД() Экспорт
	...
	Отбор.Вставить("ИННДоверителя", Неопределено);
...
Функция ПолучитьДоверенностиОрганизации(Отбор) Экспорт
	...
	Если Не ЗначениеЗаполнено(Отбор.Доверитель)
			И ЗначениеЗаполнено(Отбор.ИННДоверителя) Тогда
		РеквизитыОрганизации.ИНН = Отбор.ИННДоверителя;
	КонецЕсли;
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	ДоверенностьВыданаЦентральнымБанком =
		МашиночитаемыеДоверенностиКлиентСервер.ЭтоИННЦентральногоБанка(ИННДоверителяЮЛ);
...
Процедура ЗаполнитьТаблицуТипаСведФЛ(СведФЛ, СтруктураСведений) Экспорт
	...
	СтруктураСведений.Свойство("ДатаРождения", СведФЛ.ДатаРожд);
	...
	Если РаспределенныйРеестрФНСПоддерживаетИспользованиеКодаВидаДУЛФЛ(СведенияУдостоверения.КодФНС) Тогда
		...
		Если МашиночитаемыеДоверенностиКлиентСервер.ЭтоВидДокументаЗагранпаспортГражданинаРФ(СведФЛ.УдЛичнФЛ[0].КодВидДок) Тогда
			СтруктураСведений.Свойство("СрокДействия", СведФЛ.УдЛичнФЛ[0].expDate);
		КонецЕсли;
	КонецЕсли;
