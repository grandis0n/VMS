...
Функция НовыйКонтекстРасчетаНалога(Организация, ПериодРасчета) Экспорт
	...
	КонтекстРасчета.Вставить("ДатаИзмененияСНО", ДатаИзменения);
...
Процедура ЗаполнитьЕдиноеУведомлениеУСН(ПараметрыОтчета, Контейнер) Экспорт
	...
	ТитульныйЛист = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контейнер, "Титульная");
	Если Не ЗначениеЗаполнено(ТитульныйЛист) Тогда
		ТитульныйЛист = Новый Структура;
	КонецЕсли;
	
	ОснованияУведомления = ОснованияПредставленияЕдиногоУведомления();
	ПризнакиОбъектаНалогообложения = ПризнакиОбъектаНалогообложения();
	ПризнакиДатыПереходаНаУСН = ПризнакиДатыПереходаНаУСН();
	КодыЛицПодписывающихДокумент = КодыЛицПодписывающихДокумент();
	
	ДатаСоставления = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыЗаполнения, "ДатаСоставления", ТекущаяДатаСеанса());
	
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		
		СтрокаРеквизитыОрганизации = "ИННЮЛ,КППЮЛ,НаимЮЛПол,ТелСлуж,КодНО";
		РеквизитыОрганизации = СтрРазделить(СтрокаРеквизитыОрганизации, ",", Ложь);
		
		СведенияОрганизации = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОбОрганизации(
			Организация, ДатаСоставления, РеквизитыОрганизации);
		
		ТитульныйЛист.Вставить("Тлф",
			РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(СведенияОрганизации.ТелСлуж));
		ТитульныйЛист.Вставить("НаимОрг", СведенияОрганизации.НаимЮЛПол);
		ТитульныйЛист.Вставить("ИНН", СведенияОрганизации.ИННЮЛ);
		ТитульныйЛист.Вставить("КПП", СведенияОрганизации.КППЮЛ);
		ТитульныйЛист.Вставить("КодНО", СведенияОрганизации.КодНО);
		
	Иначе
		
		СтрокаРеквизитыИП = "ФамилияИП,ИмяИП,ОтчествоИП,ИННФЛ,ДатаРожд,КодУдЛичн,СерияУдЛичн,НомерУдЛичн,ТелСлуж,КодНО";
		...
		СведенияИП = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОбОрганизации(
			Организация, ДатаСоставления, РеквизитыИП);
		
		ТитульныйЛист.Вставить("Тлф",
			РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(СведенияИП.ТелСлуж));
		ТитульныйЛист.Вставить("Фамилия", СведенияИП.ФамилияИП);
		ТитульныйЛист.Вставить("Имя", СведенияИП.ИмяИП);
		ТитульныйЛист.Вставить("Отчество", СведенияИП.ОтчествоИП);
		ТитульныйЛист.Вставить("ИНН", СведенияИП.ИННФЛ);
		ТитульныйЛист.Вставить("КодНО", СведенияИП.КодНО);
		
		Если Не ЗначениеЗаполнено(СведенияИП.ИННФЛ) Тогда
			ТитульныйЛист.Вставить("ДатаРожд", СведенияИП.ДатаРожд);
			ТитульныйЛист.Вставить("КодВидДок", СведенияИП.КодУдЛичн);
			ТитульныйЛист.Вставить("СерНомДок", СтрШаблон("%1 %2", СведенияИП.СерияУдЛичн, СведенияИП.НомерУдЛичн));
		КонецЕсли;
		
	КонецЕсли;
	
	КодНО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "КодНО");
	Если ЗначениеЗаполнено(КодНО) Тогда
		ТитульныйЛист.Вставить("КодНО", КодНО);
	КонецЕсли;
	
	ПодписываетНалогоплательщик = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыЗаполнения, "ПодписываетНалогоплательщик", Истина);
	
	Если ПодписываетНалогоплательщик Тогда
		ПризнакПодписанта = КодыЛицПодписывающихДокумент.Налогоплательщик;
	Иначе
		ПризнакПодписанта = КодыЛицПодписывающихДокумент.ПредставительНалогоплательщика;
	КонецЕсли;
	
	ТитульныйЛист.Вставить("ПРИЗНАК_НП_ПОДВАЛ", ПризнакПодписанта);
	
	ПодписантФИО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыЗаполнения, "ПодписантФИО", "");
	
	Если ЗначениеЗаполнено(ПодписантФИО) Тогда
		ТитульныйЛист.Вставить("ФИО_РУКОВОДИТЕЛЯ_ПРЕДСТАВИТЕЛЯ", ПодписантФИО);
	КонецЕсли;
	
	ДокументПредставителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыЗаполнения, "ДокументПредставителя", "");
	
	Если ЗначениеЗаполнено(ДокументПредставителя) Тогда
		ТитульныйЛист.Вставить("НаимДок", ДокументПредставителя);
	КонецЕсли;
	
	ТелефонПредставителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыЗаполнения, "ТелефонПредставителя", "");
	
	Если ЗначениеЗаполнено(ТелефонПредставителя) Тогда
		ТитульныйЛист.Вставить("Тлф",
			РегистрацияОрганизацииСервер.НомерТелефонаВРазрешенномФормате(ТелефонПредставителя));
	КонецЕсли;
	
	ТитульныйЛист.Вставить("ДАТА_ПОДПИСИ", ДатаСоставления);
	
	ЭтоПереходНаОСНО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПереходНаОСНО", Ложь);
	ЭтоПереходНаУСН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПереходНаУСН", Ложь);
	ЭтоПереходНаУСНсНПД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПереходНаУСНсНПД", Ложь);
	
	Если ЭтоПереходНаУСН Тогда
		
		ТитульныйЛист.Вставить("ПрОснПрУвед", ОснованияУведомления.ПереходНаУСН);
		ТитульныйЛист.Вставить("ПрДатаПер", ПризнакиДатыПереходаНаУСН.СДатыПостановкиНаУчет);
		
		ОбъектНалогообложения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗаполнения, "ОбъектНалогообложения");
		
		КодОбъектаНалогообложения = "";
		Если ЗначениеЗаполнено(ОбъектНалогообложения) Тогда
			Если ОбъектНалогообложения = Перечисления.ОбъектыНалогообложенияПоУСН.Доходы Тогда
				КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.Доходы;
			ИначеЕсли ОбъектНалогообложения = Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы Тогда
				КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.ДоходыМинусРасходы;
			КонецЕсли;
		Иначе
			Если УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, ДатаСоставления) Тогда
				КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.Доходы;
			ИначеЕсли УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Организация, ДатаСоставления) Тогда
				КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.ДоходыМинусРасходы;
			КонецЕсли;
		КонецЕсли;
		
		ТитульныйЛист.Вставить("ПрОбНал1", КодОбъектаНалогообложения);
		
	ИначеЕсли ЭтоПереходНаУСНсНПД Тогда
		
		ВыбранныйРежим = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ВыбранныйРежим");
		
		ТитульныйЛист.Вставить("ПрОснПрУвед", ОснованияУведомления.ПереходНаУСН);
		ТитульныйЛист.Вставить("ПрДатаПер", ПризнакиДатыПереходаНаУСН.СДатыУтратыПраваНаНПД);
		
		КодОбъектаНалогообложения = "";
		Если ВыбранныйРежим = "УСНДоходыИП" Тогда
			КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.Доходы;
		Иначе
			КодОбъектаНалогообложения = ПризнакиОбъектаНалогообложения.ДоходыМинусРасходы;
		КонецЕсли;
	...
	ИначеЕсли ЭтоПереходНаОСНО Тогда
		
		ЭтоВынужденныйПереход = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыЗаполнения, "ЭтоВынужденныйПереход", Ложь);
		ДатаПерехода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ДатаПерехода", '00010101');
		НачалоПримененияОСНО = ДатаПерехода + 1;
		
		Если ЭтоВынужденныйПереход Тогда
			
			ПризнакиУтратыПрава = ПризнакиУтратыПраваНаУСН();
			
			ТитульныйЛист.Вставить("ПрОснПрУвед", ОснованияУведомления.УтратаПраваНаУСН);
			ТитульныйЛист.Вставить("ДатаПер", НачалоПримененияОСНО);
			
			ГраницаДоходаУСН =
				КонтрольПраваПримененияСпецрежимаКлиентСервер.ГраницаДоходовДляПримененияОсновнойСтавкиУСН(ДатаПерехода);
			ДоходыУСН = СуммаДохода(Организация, НачалоГода(ДатаПерехода), КонецГода(ДатаПерехода));
			
			Если ДоходыУСН > ГраницаДоходаУСН Тогда
				ТитульныйЛист.Вставить("ПрУтрПримУСН", ПризнакиУтратыПрава.ПревышенаГраницаДохода);
				
				КодыПериода = КодыПериодаПревышенияДоходаУСН();
				КварталПерехода = Число(Формат(ДатаПерехода, "ДФ=к"));
				КодПериода = "";
				Если КварталПерехода = 1 Тогда
					КодПериода = КодыПериода.ПервыйКвартал;
				ИначеЕсли КварталПерехода = 2 Тогда
					КодПериода = КодыПериода.Полугодие;
				ИначеЕсли КварталПерехода = 3 Тогда
					КодПериода = КодыПериода.ДевятьМесяцев;
				Иначе
					КодПериода = КодыПериода.Год;
				КонецЕсли;
				
				ТитульныйЛист.Вставить("Период1", КодПериода);
				ТитульныйЛист.Вставить("ОтчетГод1", Формат(ДатаПерехода, "ДФ=yyyy"));
			КонецЕсли;
			
		Иначе
			
			ТитульныйЛист.Вставить("ПрОснПрУвед", ОснованияУведомления.ОтказОтУСН);
			ТитульныйЛист.Вставить("ГодПерех3", Формат(НачалоПримененияОСНО, "ДФ=yyyy"));
			
		КонецЕсли;
		
	КонецЕсли;
	
	Контейнер.Вставить("Титульная", ТитульныйЛист);
...
Функция ОснованияПредставленияЕдиногоУведомления()
	
	ОснованияУведомления = Новый Структура;
	ОснованияУведомления.Вставить("ПереходНаУСН", "1");
	ОснованияУведомления.Вставить("ИзменениеОбъектаНалогообложения", "2");
	ОснованияУведомления.Вставить("ОтказОтУСН", "3");
	ОснованияУведомления.Вставить("ПрекращениеДеятельности", "4");
	ОснованияУведомления.Вставить("ПереходНаНПД", "5");
	ОснованияУведомления.Вставить("УтратаПраваНаУСН", "6");
	
	Возврат ОснованияУведомления;
	
КонецФункции

Функция ПризнакиОбъектаНалогообложения()
	
	ПризнакиОбъектаНалогообложения = Новый Структура;
	ПризнакиОбъектаНалогообложения.Вставить("Доходы", "1");
	ПризнакиОбъектаНалогообложения.Вставить("ДоходыМинусРасходы", "2");
	
	Возврат ПризнакиОбъектаНалогообложения;
	
КонецФункции

Функция ПризнакиДатыПереходаНаУСН()
	
	ПризнакиПереходаНаУСН = Новый Структура;
	ПризнакиПереходаНаУСН.Вставить("С1Января", "1");
	ПризнакиПереходаНаУСН.Вставить("СДатыПостановкиНаУчет", "2");
	ПризнакиПереходаНаУСН.Вставить("СДатыУтратыПраваНаАУСН", "3");
	ПризнакиПереходаНаУСН.Вставить("СДатыУтратыПраваНаНПД", "4");
	
	Возврат ПризнакиПереходаНаУСН;
	
КонецФункции

Функция ПризнакиУтратыПраваНаУСН()
	
	ПризнакиУтратыПрава = Новый Структура;
	ПризнакиУтратыПрава.Вставить("ПревышенаГраницаДохода", "1");
	ПризнакиУтратыПрава.Вставить("НарушеныУсловия", "2");
	ПризнакиУтратыПрава.Вставить("УчастиВДоговореТоварищества", "3");
	
	Возврат ПризнакиУтратыПрава;
	
КонецФункции

Функция КодыЛицПодписывающихДокумент()
	
	КодыЛиц = Новый Структура;
	КодыЛиц.Вставить("Налогоплательщик", "1");
	КодыЛиц.Вставить("ПредставительНалогоплательщика", "2");
	
	Возврат КодыЛиц;
	
КонецФункции

Функция КодыПериодаПревышенияДоходаУСН()
	
	КодыПериода = Новый Структура;
	КодыПериода.Вставить("ПервыйКвартал", "21");
	КодыПериода.Вставить("Полугодие", "31");
	КодыПериода.Вставить("ДевятьМесяцев", "33");
	КодыПериода.Вставить("Год", "34");
...
Функция РаботникиЗадействованыВДеятельностиНаУСН(Организация, НачалоПериода, КонецПериода)
	...
	Если ВзносыЗаРаботников.ЕстьРаботникиУСН Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если ВзносыЗаРаботников.ЕстьРаботникиПСН
		И ЕстьМесяцыСВзносамиНаПатентеБезДействующихПатентов(Организация, НачалоПериода, КонецПериода) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ЕстьМесяцыСВзносамиНаПатентеБезДействующихПатентов(Организация, НачалоПериода, КонецПериода)
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("СчетаВзносовСФОТ", СчетаУчетаВзносовСФОТ());
	...
	|ПОМЕСТИТЬ МесяцыНачисленияВзносов
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(СУММА(ВложенныйЗапрос.Сумма), 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ДействующиеПатенты
	|ИЗ
	|	Документ.ОперацияСПатентом КАК ОперацияСПатентом
	|ГДЕ
	|	ОперацияСПатентом.Организация = &Организация
	|	И ОперацияСПатентом.ДатаНачала <= &КонецПериода
	|	И ОперацияСПатентом.ДатаОкончания >= &НачалоПериода
	|	И ОперацияСПатентом.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьМесяцСВзносами
	|ИЗ
	|	МесяцыНачисленияВзносов КАК МесяцыНачисленияВзносов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДействующиеПатенты КАК ДействующиеПатенты
	|		ПО ДействующиеПатенты.ДатаНачала <= КОНЕЦПЕРИОДА(МесяцыНачисленияВзносов.Месяц, МЕСЯЦ)
	|		И ДействующиеПатенты.ДатаОкончания >= НАЧАЛОПЕРИОДА(МесяцыНачисленияВзносов.Месяц, МЕСЯЦ)
	|ГДЕ
	|	ДействующиеПатенты.Ссылка ЕСТЬ NULL";
...
Процедура РассчитатьНалогУСНОбъектДоходы(КонтекстРасчета, ТаблицаПроводок, ТаблицаСправкиРасчета, ТаблицаУменьшениеНалогаНаСтраховыеВзносыИП, Ошибки)
	...
	// Проверка перехода с другой СНО не с начала года
	НачалоПроверки = Макс(КонтекстРасчета.НачалоНалоговогоПериода, КонтекстРасчета.ДатаИзмененияСНО);
...
Функция НалоговаяБазаУменьшилась(Организация, Период) Экспорт
	...
	БазаТекущийПериод = СуммаДоходаДляРасчетаСтраховыхВзносовИП(Организация, НачалоГода(Период), КонецКвартала(Период));
