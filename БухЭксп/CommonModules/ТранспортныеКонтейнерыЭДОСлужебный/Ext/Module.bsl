...
Функция ДвоичныеДанныеТранспортногоКонтейнера(ДанныеОбъектов, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если СинхронизацияЭДО.ПоддерживаетсяПередачаМЧДВТранспортномКонтейнере(СпособОбмена)
		// ГИС ЭПД не поддерживает вложенные в пакет МЧД. Это не соответствует нормативке, но поддерживать не собираются
		И Не ЭтоЭПД Тогда
...
Процедура СформироватьКарточку(ЭлектронныйДокумент, АдресКаталога, Ошибки, ОписаниеОбъекта, КонтекстДиагностики = Неопределено)
	...
	ДеревоКарточки = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента("ОбменСКонтрагентами.КарточкаПередаваемыхДанных");
	
	// Готовим дерево данных для карточки контейнера.
	ПодготовитьДанныеПоКарточке(ОписаниеОбъекта, ДеревоКарточки);
	
	// Выполним проверку на заполненность обязательных полей.
	Если Не ЗначениеЗаполнено(Ошибки) Тогда
		ДеревоЭлектронногоДокументаБЭД.ПроверитьЗаполнениеРеквизитовДереваДанныхРекурсивно(ДеревоКарточки, Ошибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Ошибки) Тогда
		Карточка = СформироватьКарточкуCML(ДеревоКарточки, Ошибки, ЭлектронныйДокумент,
...
Процедура ПодготовитьДанныеПоКарточке(ОписаниеОбъекта, ДеревоКарточки)
	...
	Если ЗначениеЗаполнено(ОписаниеОбъекта.ИдентификаторЧерновика) Тогда
		НоваяСтрока = ДополнительныеДанные.Добавить();
		НоваяСтрока.Имя = "LinkedTemplate";
		НоваяСтрока.Значение = ОписаниеОбъекта.ИдентификаторЧерновика;
	КонецЕсли;
	...
	Если Не ОписаниеОбъекта.СпособОбмена = Перечисления.СпособыОбменаЭД.БыстрыйОбмен Тогда
		...
		Если РегламентыЭДО.ЭтоЭлементРегламентаОтправителя(ОписаниеОбъекта.ТипЭлементаРегламента)
			...
			Если Не ОписаниеОбъекта.ТребуетсяИзвещение 
				Или СообщениеНеТребуетИзвещениеОПолучении(ОписаниеОбъекта.ТипЭлементаРегламента) Тогда
				НоваяСтрока = ДополнительныеДанные.Добавить();
	...
	Если ЗначениеЗаполнено(ДанныеУчетнойЗаписи)
		И ЗначениеЗаполнено(ДанныеУчетнойЗаписи.ВидУчетнойЗаписи) Тогда
		
		ТегUserType = Перечисления.ВидУчетнойЗаписиЭДО.ТегUserTypeДляКарточки(ДанныеУчетнойЗаписи.ВидУчетнойЗаписи);
		Если ЗначениеЗаполнено(ТегUserType) Тогда
			НоваяСтрока = ДополнительныеДанные.Добавить();
			НоваяСтрока.Имя = "UserType";
			НоваяСтрока.Значение = ТегUserType;
		КонецЕсли;
...
Функция ТаблицаТиповЭлементовРегламента()
	...
	ТранспортныеКонтейнерыЭДО.ПриФормированииТаблицыТиповЭлементовРегламента(Таблица);
...
Функция ТаблицаТиповРегламентов()
	...
	ТранспортныеКонтейнерыЭДО.ПриФормированииТаблицыТиповРегламентов(Таблица);
...
Функция СообщениеНеТребуетИзвещениеОПолучении(ТипЭлементаРегламента)
