...
Процедура ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты) Экспорт
	...
	Если Не ПараметрыОжидания.ИспользоватьУведомленияКлиента Тогда
		ОповеститьОПолученииЭДОПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результат);
	КонецЕсли;
	
	Если Результат.Включено = ПараметрыОжидания.Включено Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыОжидания.ИспользоватьУведомленияКлиента
		И Не Результат.Включено Тогда
		...
		ПараметрыОжидания.ИспользоватьУведомленияКлиента = Ложь;
		
	ИначеЕсли Не ПараметрыОжидания.ИспользоватьУведомленияКлиента
		И Результат.Включено
...
// Параметры:
//  Результат - См. АвтоматическоеПолучениеЭДО.НовыйРезультатПериодическойОтправкиДанных
Процедура ОповеститьОПолученииЭДОПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результат)
	...
	ИначеЕсли ЗначениеЗаполнено(Результат.ДлительнаяОперация) Тогда
		...
		ПараметрыОжиданияДлительныеОперации = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжиданияДлительныеОперации.ВыводитьОкноОжидания = Ложь;
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Оповещение,
			ПараметрыОжиданияДлительныеОперации);
	КонецЕсли;
