...
Функция ОбработатьСобытиеОтУстройства(ОписаниеСобытия, ОписаниеОшибки = "") Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	Для Каждого Подключение Из ПодключаемоеОборудование.ПараметрыПодключенияПО Цикл
		Если (ПустаяСтрока(Подключение.ИДУстройства) И Подключение.ИменаСобытий.Количество() > 0)
			Или Подключение.ИДУстройства = ОписаниеСобытия.Источник 
			Или Подключение.ИдентификаторЭкземпляра = ОписаниеСобытия.Источник Тогда
				Если Не Подключение.Несертифицированное Тогда
					...
					Если Событие <> Неопределено Тогда
						...
						// Оповещаем 
						Если РезультатОбработки.Результат Тогда 
							Если РезультатОбработки.РассылкаОповещения Тогда
...
//   *СкидкаЭлектроннымиСертификатами - Булево.
...
//   *УстройствоРаспознаванияПолучениеИзображения - Булево. 
//   *УстройствоРаспознаванияАппаратноеРаспознавание - Булево.
...
//   * ПолученияСтатусаОперации - Булево. 
//   * СтолбцовНаДисплее - Число.
//   * СтрокНаДисплее - Число. 
//   * ДисплейОтображаетТекст - Булево.
//   * ДисплейОтображаетГрафику - Булево.
...
Функция ПараметрыПодключенияУстройства(ДанныеУстройства) Экспорт
	...
	Подключение.Вставить("ТСПИоТИдентификатор"     , ДанныеУстройства.ТСПИоТИдентификатор);  
	Подключение.Вставить("ТСПИоТПлатформа"         , ДанныеУстройства.ТСПИоТПлатформа);  
	Подключение.Вставить("ТСПИоТПутьДанных"        , ДанныеУстройства.ТСПИоТПутьДанных);
	// -- Локализация     
	...
	Подключение.Вставить("ИдентификаторЭкземпляра");
	Возврат Подключение;
...
Функция ПодключаемоеОборудование() Экспорт
	...
	Если ПодключаемоеОборудование = Неопределено Тогда
		...
		ПодключаемоеОборудование.Вставить("ИнициализацияПИоТ", Новый Массив());   
		// -- Локализация
...
Функция ПолучитьОписаниеДрайвера(СтрокаXML) Экспорт
	...
	Параметры.ОсновнойДрайверПустьГлавнойDLL = ?(ПараметрыXML.Свойство("DriverMainDLLPath"), ПараметрыXML.DriverMainDLLPath, "");
...
// Выбрать устройство из доступных, привязанных к текущему рабочему месту.
//
// Параметры:
...
Процедура ОткрытьОперацииСОборудованием(ПодключаемоеОборудование) Экспорт
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяПодсистемаЛогирования() Тогда
		...
		ОткрытьФорму("РегистрСведений.ОперацииСПодключаемымОборудованием.ФормаСписка", ПараметрыФормы, , ПодключаемоеОборудование);
	КонецЕсли;
	// -- НеМобильноеПриложение
...
Асинх Функция ЗагрузитьАрхивЛоговДрайвераАсинх(ПодключаемоеОборудование, ДрайверОборудования, УстановитьРасширениеРаботыСфайлами = Ложь) Экспорт
	...
	Если ЭтоКаталог Тогда
		...
		КонецЕсли;
		ПутиКФайлам.Добавить(НайденныеФайлы);
	Иначе
		// Лог файлы хранятся в одном файле
		ПутиКФайлам.Добавить(Файл.ПолноеИмя);
	КонецЕсли;
...
// ++ Локализация
	Если ОбщегоНазначенияБПОклиент.ИспользуетсяТСПИоТ() Тогда
		...
		ТСПИоТ = МодульМенеджерОборудованияТСПИоТВызовСервера.ПолучитьПараметрыТСПИоТ(ПодключаемоеОборудование);  
		Если ЗначениеЗаполнено(ТСПИоТ) Тогда  
			НайденныеФайлы  = СокрЛП(ТСПИоТ.ПутьДанныхЭкземпляра);
			Если Прав(НайденныеФайлы, 1) <> РазделительПути Тогда
				НайденныеФайлы = НайденныеФайлы + РазделительПути;
			КонецЕсли;
			ПутиКФайлам.Добавить(НайденныеФайлы);
...
// -- НеМобильноеПриложение 
	...
	Попытка
		Если ОбщегоНазначенияБПОКлиент.ЭтоВебКлиент() Тогда
			РезультатПередачи = Ждать ПередатьФайлыЛоговНаСервер(ПутиКФайлам);
			...
			КонецЕсли;
		Иначе
			Данные = СоздатьАрхивЛоговНаКлиенте(ПутиКФайлам);
		КонецЕсли;
...
// Функция возвращает признак инициализации модуля ТС ПИоТ
// 
// Параметры:
//  ИдентификаторУстройства - УникальныйИдентификатор -Идентификатор устройства
// 
// Возвращаемое значение:
//  Булево
Функция ПроверкаИнициализацияПИоТПройдена(ИдентификаторУстройства) Экспорт
	
	Результат = Ложь;
	
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	ИнициализацияПИоТ = ПодключаемоеОборудование.ИнициализацияПИоТ;      
	
	Для Каждого ФискальноеУстройство Из ИнициализацияПИоТ Цикл
		Если ФискальноеУстройство.ИдентификаторУстройства = ИдентификаторУстройства Тогда
				Результат = ФискальноеУстройство.Свойство("ИнициализацияПИоТ") И ФискальноеУстройство.ИнициализацияПИоТ;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
//  Булево  
//
Процедура УстановитьИнициализацияПИоТПройдена(ИдентификаторУстройства, Признак = Истина) Экспорт
	
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	ИнициализацияПИоТ = ПодключаемоеОборудование.ИнициализацияПИоТ;
	
	Для Каждого ФискальноеУстройство Из ИнициализацияПИоТ Цикл
		Если ФискальноеУстройство.ИдентификаторУстройства = ИдентификаторУстройства И ФискальноеУстройство.Свойство("ИнициализацияПИоТ") Тогда           
			ФискальноеУстройство.ИнициализацияПИоТ = Признак;
			Возврат;
		КонецЕсли;
	КонецЦикла;   
	...
	Если Признак Тогда
		ФискальноеУстройство = Новый Структура();
		ФискальноеУстройство.Вставить("ИдентификаторУстройства", ИдентификаторУстройства);
		ФискальноеУстройство.Вставить("ИнициализацияПИоТ", Признак);
		ИнициализацияПИоТ.Добавить(ФискальноеУстройство);
	КонецЕсли;
...
//
// Параметры:
//   ПутиКФайлам - Массив - Пути к файлам логов для архивации
//
// Возвращаемое значение:
//   ДвоичныеДанные, Неопределено - Архив логов, либо Неопределено в случае 
//    ошибки или при работе в веб-клиенте
//
Функция СоздатьАрхивЛоговНаКлиенте(ПутиКФайлам)
	
#Если Не ВебКлиент Тогда
	
	Попытка
		Архив = Новый ЗаписьZipФайла();
		Для Каждого ПутьКФайлам Из ПутиКФайлам Цикл
...
//   
//
Асинх Функция ПередатьФайлыЛоговНаСервер(ПутиКФайлам)
	...
	Для Каждого ПутьКФайлам Из ПутиКФайлам Цикл           
		ФайлЭлемент = Новый Файл(ПутьКФайлам);
		ЭтоКаталог = Ждать ФайлЭлемент.ЭтоКаталогАсинх();
		Если ЭтоКаталог Тогда
	...
	КонецЦикла;
		Иначе
			ОписанияПередаваемыхФайлов.Добавить(Новый ОписаниеПередаваемогоФайла(ПутьКФайлам));
		КонецЕсли;
	КонецЦикла;
...
Асинх Функция ВызватьHTTPМетодКлиентАсинх(Метод, ПараметрыСоединения)
	...
	ПараметрыСоединения.Заголовки.Вставить("Content-Type", "application/json");
