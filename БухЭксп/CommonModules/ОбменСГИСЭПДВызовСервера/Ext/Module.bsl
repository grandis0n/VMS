...
Функция ОпределенДоговорЭПД() Экспорт
	Возврат ОбменСГИСЭПД.ОпределенДоговорЭПД();
КонецФункции


Функция ПолучитьРеквизитыДоговора(СсылкаНаДоговор) Экспорт   
	Возврат ОбменСГИСЭПД.ПолучитьРеквизитыДоговора(СсылкаНаДоговор);
КонецФункции

Функция ДемоРежимВключен() Экспорт
	
	Возврат ОбменСГИСЭПД.ДемоРежимВключен();
	
КонецФункции

Функция ПолучитьДанныеДляТитулаНаОснованииЧерновика(ЭлектронныйДокумент, ЧерновикДокумента) Экспорт
	 Возврат ОбменСГИСЭПД.ПолучитьДанныеДляТитулаНаОснованииЧерновика(ЭлектронныйДокумент, ЧерновикДокумента);
КонецФункции
...
Функция СведенияОбАдресе(РезультатЗначение, ДополнительныеПараметры) Экспорт
	...
	СтруктураАдреса = РаботаСАдресами.СведенияОбАдресе(РезультатЗначение, ДополнительныеПараметры);
	
	Если ЗначениеЗаполнено(СтруктураАдреса)
		И СтруктураАдреса.ТипАдреса <> "ВСвободнойФорме" Тогда
			
			Город = СтруктураАдреса.Город + ?(СтруктураАдреса.ГородТипКраткий = "", "", " ") + СтруктураАдреса.ГородТипКраткий;
			НаселенныйПункт = СтруктураАдреса.НаселенныйПункт + ?(СтруктураАдреса.НаселенныйПунктТипКраткий = "", "", " ") + СтруктураАдреса.НаселенныйПунктТипКраткий;
			
			Если ВРЕГ(Город) = ВРЕГ(НаселенныйПункт) Тогда
				СтруктураАдреса.НаселенныйПункт = "";
				СтруктураАдреса.НаселенныйПунктТипКраткий = "";
			КонецЕсли;
...
Функция ПолучитьРеквизитыУчастника(Участник, ДобавитьПлатежныеРеквизиты = Ложь, Параметры = Неопределено) Экспорт
	...
	Возврат ОбменСГИСЭПД.ПолучитьРеквизитыУчастника(Участник, ДобавитьПлатежныеРеквизиты, Параметры);
...
Функция ИмяТипаСтраны() Экспорт
	...
	Возврат ОбменСГИСЭПДПереопределяемый.ИмяТипаСтраны();
	...
	Если ЭтоВторойТитул = Истина И ДокументСсылка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЭД) Тогда
			...
			ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЭлектронныйДоговорФрахтования") Тогда
				ПерваяТранзакция = "SendOrder";
			ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
				ПерваяТранзакция = "EPE_T1";
			ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
				ПерваяТранзакция = "EER_T1";
			ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда
				ПерваяТранзакция = "ESR_T1";
			КонецЕсли;
