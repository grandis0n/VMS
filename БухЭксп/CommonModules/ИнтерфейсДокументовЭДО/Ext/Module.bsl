...
Функция СформироватьФайлыВыгрузкиЭДВФорматеPDF(Знач ЭлектронныеДокументы, Знач АдресДанныхВХранилище) Экспорт
	...
	КоличествоФайловДокументов = Новый Соответствие;
	ЗанятыеИменаФайлов = Новый Соответствие;
...
Функция СформироватьФайлыВыгрузкиЭДДокументооборотЦеликом(Знач ЭлектронныеДокументы, Знач АдресДанныхВХранилище) Экспорт
	...
	Для каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
		...
		КаталогДокумента = РаботаСФайламиБЭДКлиентСервер.ОпределитьУникальноеИмяФайла(КаталогДокумента,
			ЗанятыеИменаФайлов, Истина) + ПолучитьРазделительПути();
		...
		Для каждого ФайлДокумента Из ФайлыДокумента Цикл
			...
			КаталогСообщения = РаботаСФайламиБЭДКлиентСервер.ОпределитьУникальноеИмяФайла(КаталогСообщения,
				ЗанятыеИменаФайлов, Истина) + ПолучитьРазделительПути();
...
Функция ЗапросКоличестваТекущихЭлектронныхДокументов(Параметры) Экспорт
	...
	ИначеЕсли Раздел = "НаКонтроле" Тогда
		...
		|					ЗНАЧЕНИЕ(Перечисление.СостоянияДокументовЭДО.ТребуетсяСоздатьДокумент)
		|					)
		|	ГДЕ
		|		&УсловиеОтбора
		|		И НЕ ЭлектронныйДокументВходящийЭДО.ПометкаУдаления
...
Функция НовыеСвойстваПредставленияДокумента() Экспорт
	...
	Свойства.Вставить("ПредставлениеЧерновика", "");
	Возврат Свойства;
КонецФункции
...
// * АбонентНеТребуетПриглашений - Булево
...
Функция НовыеПараметрыСвязиИдентификаторов() Экспорт
	...
	ПараметрыСвязи.Колонки.Добавить("АбонентНеТребуетПриглашений", ОписаниеТипаБулево);
	ПараметрыСвязи.Колонки.Добавить("КодыТиповДокументовКОбмену", ОбщегоНазначения.ОписаниеТипаСтрока(150));
...
Процедура ЗаполнитьСтатусыПоНастройкам(СтатусыЭлектронныхДокументов, РезультатыПоискаНастроек)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
			...
			ДанныеОбъекта.Вставить("ОбъектУчета", Неопределено);
			ДанныеОбъекта.Вставить("ТипДокумента", Неопределено);
			ЗаполнитьЗначенияСвойств(ДанныеОбъекта, СтрокаТаблицы);
			Если ЗначениеЗаполнено(СтрокаТаблицы.ВидДокумента) Тогда
				ДанныеОбъекта.ТипДокумента = ЭлектронныеДокументыЭДО.ОписаниеВидаДокумента(СтрокаТаблицы.ВидДокумента).ТипДокумента;
			КонецЕсли;
			ОбменСКонтрагентамиПереопределяемый.ПредставлениеНеСформированногоДокумента(ПредставлениеДокумента, ДанныеОбъекта, СтандартнаяОбработка);
			Если СтандартнаяОбработка Тогда
				ПредставлениеДокумента = ПредставлениеНеСформированногоДокументаПоВиду(
