...
Процедура ПодобратьВариантыСопоставленияПоИдентификаторамМаркетплейсов(ТаблицаНоменклатурыКонтрагентов, Маркетплейс)
	...
	|	&ТаблицаНоменклатурыКонтрагентов КАК ТаблицаНоменклатурыКонтрагентов
	|ГДЕ
	|	ТаблицаНоменклатурыКонтрагентов.КодМагазина <> """"
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ИдентификаторТовара
	|;
	...
	|			И (ВЫБОР
	|				КОГДА ТаблицаИдентификаторовМаркетплейсов.ШтрихкодКомбинации <> """"
	|					ТОГДА ТаблицаИдентификаторовМаркетплейсов.ШтрихкодКомбинации = НоменклатураМаркетплейсов.Штрихкод
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Штрихкод) Тогда
			Отбор.Вставить("ШтрихкодКомбинации", Выборка.Штрихкод);
		КонецЕсли;
