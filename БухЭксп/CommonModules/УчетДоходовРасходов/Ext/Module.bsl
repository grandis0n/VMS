...
Процедура СформироватьДвиженияСторноРеализацияСобственныхТоваровУслугКорректировка(ТаблицаВыручкиОтРеализации, ПараметрыРеализации, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из ТаблицаВыручкиОтРеализации Цикл
		...
		Если ПараметрыРеализации.ПрошлыйПериод Тогда
			...
			//Движение по признанию дохода в прошлом периоде в налоговом учете
			Если ПараметрыРеализации.СтавкаНалогаУвеличилась Тогда
				Проводка = Движения.Хозрасчетный.Добавить();
			...
			Если ПараметрыРеализации.СтавкаНалогаУвеличилась И ФинРезультатНУ <> 0 И ОтражатьВНалоговомУчете Тогда
				СформироватьДвиженияПризнанияФинРезультата(Движения, Организация, 
				ПараметрыРеализации.ДатаДокументаРеализации, 0, ФинРезультатНУ, ОтражатьВНалоговомУчете, ЭтоСчетПрочихДоходов);
			КонецЕсли; 
			...
			Если ПараметрыРеализации.СтавкаНалогаУвеличилась 
				ИЛИ НЕ ПараметрыРеализации.ОтчетностьПодписана Тогда
				
				//Движение по признанию задолженности в бухгалтерском учете
				Проводка = Движения.Хозрасчетный.Добавить();
				Проводка.Период      = ПараметрыРеализации.Период;
				Проводка.Организация = Организация;
				Проводка.Сумма       = ?(НЕ ПараметрыРеализации.ОтчетностьПодписана, СтрокаТаблицы.СуммаБУ, 0);
				Проводка.Содержание  = ТекстСодержания;
					
				Проводка.СчетДт = СтрокаТаблицы.КорСчет;
				ЗаполнитьАналитикуСчетаРасчетов(Проводка, СтрокаТаблицы, "Дт", ПараметрыРеализации.СтавкаНалогаУвеличилась);

				Проводка.СчетКт =  ПланыСчетов.Хозрасчетный.КорректировкаРасчетовПрошлогоПериода;
				ЗаполнитьАналитикуСчетаРасчетов(Проводка, СтрокаТаблицы, "Кт", ПараметрыРеализации.СтавкаНалогаУвеличилась);
				
			КонецЕсли;

			Если НЕ ПараметрыРеализации.СтавкаНалогаУвеличилась 
				ИЛИ ПараметрыРеализации.ОтчетностьПодписана Тогда
				...
				ЗаполнитьАналитикуСчетаРасчетов(Проводка, СтрокаТаблицы, "Кт", НЕ ПараметрыРеализации.СтавкаНалогаУвеличилась,, Истина);
				Если НЕ ПараметрыРеализации.ОтчетностьПодписана Тогда
				...
				Если НЕ ПараметрыРеализации.СтавкаНалогаУвеличилась
					И ОтражатьВНалоговомУчете И СвойстваСчетаДт.НалоговыйУчет Тогда
