...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьДатуПоследнейАктивностиАбонента", Истина);
...
Функция ТарифныеОграниченияБизнесСтарт(Организация = Неопределено)
	...
	ИначеЕсли ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНедоступнаяСистемаНалогообложения()
		Или ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНедоступныйВидОрганизации()
		Или ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНеразрешенныйРаздельныйУчетНДС() Тогда
...
Процедура ЗапуститьДлительнуюОперациюУстановкиДатыПоследнейАктивности(ДатаПоследнейАктивности) Экспорт
	
	ПараметрыПроцедуры = Новый Структура("ДатаПоследнейАктивности", ДатаПоследнейАктивности);
	
	// Параметры длительной операции
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне();
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Установка даты последней активности абонента'");
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	
	ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, "МобильнаяБухгалтерия.УстановитьДатуПоследнейАктивностиАбонента",
		ПараметрыПроцедуры);
	
КонецПроцедуры

Процедура УстановитьДатуПоследнейАктивностиАбонента(ПараметрыПроцедуры) Экспорт
	...
	Отбор.Вставить("Ключ", "ДатаАктивностиМобильнойБухгалтерии");
	
	НайденныеСтроки = Сведения.Свойства.НайтиСтроки(Отбор);
	
	Если НайденныеСтроки.Количество() = 0 Тогда
		ЗаписьЖурналаРегистрации("УстановкаДатыПоследнейАктивностиМБ", УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ru = 'Не найдено доп. сведение ДатаАктивностиМобильнойБухгалтерии'"));
	...
	НоваяСтрока.Ключ = "ДатаАктивностиМобильнойБухгалтерии";
	НоваяСтрока.Значение = XMLСтрока(ПараметрыПроцедуры.ДатаПоследнейАктивности);
	...
	ИначеЕсли ИмяМетода = "ОСТАТОК_РАСЧЕТОВ_БУ" Тогда
		...
		Если ВводНачальныхДанных Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов Тогда
				...
				Исключения.Добавить(ПланыСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате);
