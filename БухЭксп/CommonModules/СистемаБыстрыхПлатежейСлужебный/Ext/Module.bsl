...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОжидатьТерминальныйСтатусОплаты", Истина);
...
Функция ЗаголовокАвторизацииУчастникаСБП(
	...
	Если ДанныеАутентификации = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Неопределенны данные аутентификации.'");
	КонецЕсли;
...
// Выполняет ожидание терминального статуса оплаты.
//
// Параметры:
//  ИдентификаторОплаты - Строка - идентификатор оплаты;
//  ИдентификаторУчастника - Строка - идентификатор участника СБП;
//  ИдентификаторОперации - Строка - идентификатор операции оплаты.
//
// Возвращаемое значение:
//  Структура - данные сверке взаиморасчетов:
//    * ТерминальныйСтатусПолучен - Булево - признак получения терминального статуса операции.
//    * КодОшибки - Строка - строковый код возникшей ошибки, который
//     может быть обработан вызывающим методом;
//    * СообщениеОбОшибке  - Строка, ФорматированнаяСтрока - сообщение об ошибке для пользователя;
//    * ИнформацияОбОшибке - Строка, ФорматированнаяСтрока - сообщение об ошибке для администратора.
//
Функция ОжидатьТерминальныйСтатусОплаты(
	ИдентификаторОплаты,
	ИдентификаторУчастника,
	ИдентификаторОперации) Экспорт
	
	РезультатОперации = НовыйРезультатОперации();
	РезультатОперации.Вставить("ТерминальныйСтатусПолучен", Ложь);
	...
	Пока ВозможенВызовОперации(НастройкиВызова) Цикл
		...
		Если РезультатПроверки.Идентификаторы.Найти(ИдентификаторОплаты) <> Неопределено Тогда
			РезультатОперации.ТерминальныйСтатусПолучен = Истина;
			Возврат РезультатОперации;
		КонецЕсли;
...
Процедура УстановитьИспользованиеШаблоновСообщений() Экспорт
	...
	ИнтернетПоддержкаПользователей.УстановитьИспользованиеОтправкиШаблоновСообщений(
		НастройкиПодключенияПрограммы.ОбщиеНастройки.ИспользоватьПодсистемуВзаимодействия);
...
Функция ОбновитьКэшМакетов()
	...
	ОбновляемыеМакеты = МакетыДляОбновления(
		РезультатВызова.Описание);
	
	Если ОбновляемыеМакеты.Количество() = 0 Тогда
		Возврат РезультатОперации;
	КонецЕсли;
