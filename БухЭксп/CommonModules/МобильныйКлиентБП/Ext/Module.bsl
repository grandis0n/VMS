...
Процедура ОткрытьДоступнуюНаМобильномФорму(Источник, ВидФормы, Параметры, ВыбраннаяФорма, СтандартнаяОбработка)
	...
	Если ИмяОбъекта = "РегламентированныйОтчет" ИЛИ ИмяОбъекта = "УведомлениеОСпецрежимахНалогообложения"
		ИЛИ ИмяОбъекта = "УведомлениеОбИсчисленныхСуммахНалогов" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьДоступнуюФормуРегламентированнойОтчетности(Источник, ВидФормы, Параметры, ВыбраннаяФорма,
	...
	Если ВидФормы = "ФормаОбъекта"
		И Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ОткрытьФормуОбъектаРегламентированнойОтчетности(Источник, ВидФормы, Параметры, ВыбраннаяФорма,
			СтандартнаяОбработка);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОткрытьФормуОбъектаРегламентированнойОтчетности(Источник, ВидФормы, Параметры, ВыбраннаяФорма,
	СтандартнаяОбработка)
	...
	Если ИмяОбъекта = "РегламентированныйОтчет" Тогда
		...
		Иначе
			ИмяФормы = "ОбщаяФорма.МобильныйКлиентПросмотрРегОтчетности";
			
			Параметры.Вставить("ДокументРегОтчетности", ПараметрыОткрываемойФормы.Ключ);
		КонецЕсли;
	ИначеЕсли ИмяОбъекта = "УведомлениеОСпецрежимахНалогообложения" Тогда
		ИмяФормы = "ОбщаяФорма.МобильныйКлиентПросмотрРегОтчетности";
