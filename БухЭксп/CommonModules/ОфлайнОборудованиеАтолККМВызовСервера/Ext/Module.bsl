			...
			Если ЗначениеЗаполнено(Источник.ЭтоМаркированнаяПродукция) И Источник.ЭтоМаркированнаяПродукция Тогда
				Если ФорматОбмена = 8 Тогда
					...
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 37);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.СладостиИКондитерскиеИзделия") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 39);
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.СтроительныеМатериалы") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 40);
					Иначе
