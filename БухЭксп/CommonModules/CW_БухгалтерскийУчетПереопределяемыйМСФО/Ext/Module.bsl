...
Функция ДокументПроведен(ДокументСсылка) Экспорт
	...
	Если CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ЭтоРегламентнаяОперация(ДокументСсылка) Тогда
...
Функция ПолучитьКонтрагентаПоДоговору(Договор) Экспорт
	...
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ПолучитьИмяРеквизитаКонтрагентДоговора());
...
Функция ТекстЗапросаВидДоговора(ИмяПоля) Экспорт
	...
	ИмяРеквизитаВидДоговора = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ПолучитьИмяРеквизитаВидДоговора();
...
Функция ПолучитьСведенияОСпискеНоменклатуры(Знач СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	ЗаполнятьСчетаУчета = ОпределятьСчета И ПравоДоступа("Чтение", Метаданные.ПланыСчетов.CW_МСФО);
	...
	Если Реализация И 
		...
		Если НЕ ПравоДоступа("Чтение", Метаданные.ПланыСчетов.CW_МСФО) ИЛИ 
	...
	ОпределятьСчета = ОпределятьСчета И ПравоДоступа("Чтение", Метаданные.ПланыСчетов.CW_МСФО);
	...
	ВидСубконтоНоменклатурныеГруппы = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.НоменклатурныеГруппы;
	ВидСубконтоПрочиеДоходыИРасходы = ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.ПрочиеДоходыИРасходы;
	
	// Определим основную номенклатурную группу
	ОсновнаяНоменклатурнаяГруппа = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ОсновнаяНоменклатурнаяГруппа();
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		СведенияОНоменклатуре.Вставить("СтавкаНДС", 
			Перечисления.СтавкиНДС.СтавкаНДС(СведенияОНоменклатуре.ВидСтавкиНДС, Дата));
		...
		Если СчетаУчета <> Неопределено Тогда
			...
			СвойстваСчетаУчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(СчетаУчета.СчетУчета);
			Комиссионный = СвойстваСчетаУчета.Забалансовый;
			
			ДоходЕНВД = CW_БухгалтерскийУчетПовтИспМСФО.СчетОтноситсяКДеятельностиЕНВД(СчетаУчета.СчетДоходов);
			
			СвойстваСчетаДоходов = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(СчетаУчета.СчетДоходов);
		...
		Если ОпределятьЦены И (ЗначениеЗаполнено(ТипЦен) ИЛИ ЗначениеЗаполнено(СпособЗаполненияЦены)
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Цена = CW_Финансы.ПересчитатьИзВалютыВВалюту(
		...
		Если ОпределятьЦены И (ЗначениеЗаполнено(ТипЦенРозничнойТорговли) ИЛИ ЗначениеЗаполнено(СпособЗаполненияРозничнойЦены)) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				ЦенаВРознице = CW_Финансы.ПересчитатьИзВалютыВВалюту(
					НайденнаяСтрока.Цена,
		...
		Если ОпределятьЦены И (ЗначениеЗаполнено(ТипЦенПлановойСебестоимости) ИЛИ ЗначениеЗаполнено(СпособЗаполненияПлановойСебестоимости)) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				ПлановаяСтоимость = CW_Финансы.ПересчитатьИзВалютыВВалюту(
					НайденнаяСтрока.Цена,
					НайденнаяСтрока.Валюта, ВалютаРегламентированногоУчета,
					НайденнаяСтрока.Курс, 1,
					НайденнаяСтрока.Кратность, 1);
...
Функция ПодразделениеПринадлежитОрганизации(Подразделение, Организация) Экспорт
	...
	Если ЗначениеЗаполнено(Подразделение) И ЗначениеЗаполнено(Организация) Тогда
		...
		ИмяРеквизитаОрганизации = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ИмяРеквизитаОрганизацияПодразделения();
...
Функция ПодготовитьСоответствиеНоменклатураЦена(СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ЦенаНоменклатуры = CW_Финансы.ПересчитатьИзВалютыВВалюту(Выборка.Цена,
...
Функция ТаблицаСуммСписанияАктивов(МассивРегистраторов, ДатаНач, ДатаКон) 
	...
	Запрос.УстановитьПараметр("ВидСубконтоНоменклатура", ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Номенклатура);
	Запрос.УстановитьПараметр("ВидСубконтоПартия", ПланыВидовХарактеристик.CW_ВидыСубконтоМСФО.Партии);
	Запрос.УстановитьПараметр("СчетГТД", ПланыСчетов.CW_МСФО.ГТД);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	CW_МСФОДвиженияССубконто.Регистратор КАК Регистратор,
	|	ВЫБОР
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт1 = &ВидСубконтоНоменклатура
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт1
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт2 = &ВидСубконтоНоменклатура
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт2
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт3 = &ВидСубконтоНоменклатура
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт3
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт1 = &ВидСубконтоПартия
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт1
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт2 = &ВидСубконтоПартия
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт2
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт3 = &ВидСубконтоПартия
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт3
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Партия,
	|	СУММА(CW_МСФОДвиженияССубконто.КоличествоКт) КАК Количество,
	|	СУММА(CW_МСФОДвиженияССубконто.Сумма) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.CW_МСФО.ДвиженияССубконто(
	|			&ДатаНач,
	|			&ДатаКон,
	|			Регистратор В (&МассивРегистраторов)
	|				И НЕ СчетКт = &СчетГТД,
	|			,
	|			) КАК CW_МСФОДвиженияССубконто
	|
	|СГРУППИРОВАТЬ ПО
	|	CW_МСФОДвиженияССубконто.Регистратор,
	|	ВЫБОР
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт1 = &ВидСубконтоНоменклатура
	|			ТОГДА CW_МСФОДвиженияССубконто.СубконтоКт1
	|		КОГДА CW_МСФОДвиженияССубконто.ВидСубконтоКт2 = &ВидСубконтоНоменклатура
...
Функция ПолучитьСчетаУчетаВНТТ(Организация, Склад, Дата, УчетнаяПолитикаНУ) Экспорт
	...
	Если СрезПоследних.Количество() > 0 Тогда
		Если СрезПоследних[0].ОсобыйПорядокНалогообложения Тогда
			СтруктураВозврата.СчетДоходовОтРеализации  = ПланыСчетов.CW_МСФО.ВыручкаЕНВД;
			СтруктураВозврата.СчетРасходовОтРеализации = ПланыСчетов.CW_МСФО.СебестоимостьПродажЕНВД;
		Иначе
			СтруктураВозврата.СчетДоходовОтРеализации  = ПланыСчетов.CW_МСФО.ВыручкаНеЕНВД;
			СтруктураВозврата.СчетРасходовОтРеализации = ПланыСчетов.CW_МСФО.СебестоимостьПродажНеЕНВД;
		КонецЕсли;
	Иначе
...
Функция ПолучитьСчетаУчетаСпискаНоменклатуры(Организация, СписокНоменклатуры, Склад = Неопределено, Знач Дата = Неопределено, Знач ОсобыйПорядокНалогообложения = Ложь) Экспорт
	...
	ПустойСчет = ПланыСчетов.CW_МСФО.ПустаяСсылка();
	...
	Запрос.УстановитьПараметр("ПустойСчет",			ПланыСчетов.CW_МСФО.ПустаяСсылка());
	Если ОсобыйПорядокНалогообложения = Неопределено Тогда
		// Возвращаем данные "как есть". ОсобыйПорядокНалогообложения будет учтен в СчетаУчетаВДокументах.
		Запрос.УстановитьПараметр("Счет_Выручка",				ПланыСчетов.CW_МСФО.ПустаяСсылка());
		Запрос.УстановитьПараметр("Счет_СебестоимостьПродаж",	ПланыСчетов.CW_МСФО.ПустаяСсылка());
	Иначе
		// Для совместимости с предыдущей версией
		Запрос.УстановитьПараметр("Счет_Выручка",				?(ОсобыйПорядокНалогообложения, ПланыСчетов.CW_МСФО.ВыручкаЕНВД,             ПланыСчетов.CW_МСФО.ВыручкаНеЕНВД));
		Запрос.УстановитьПараметр("Счет_СебестоимостьПродаж",	?(ОсобыйПорядокНалогообложения, ПланыСчетов.CW_МСФО.СебестоимостьПродажЕНВД, ПланыСчетов.CW_МСФО.СебестоимостьПродажНеЕНВД));
	КонецЕсли;
...
Функция СчетаУчетаОбъектовСтроительства(Организация, Объект) Экспорт
	...
	Результат = Новый Структура("СчетУчета, СчетУчетаНДС, СпособУчетаНДС", Неопределено, Неопределено, Неопределено);
...
Функция СчетаПодлежащиеПереоценке() Экспорт
	...
	|ГДЕ
	|	CW_МСФО.Валютный
	|	И (НЕ CW_МСФО.Забалансовый)
	|	И (НЕ CW_МСФО.Ссылка В
	|				(ВЫБРАТЬ
	|					СчетаСОсобымПорядкомПереоценки.Счет
	|				ИЗ
	|					РегистрСведений.CW_СчетаСОсобымПорядкомПереоценкиБУМСФО КАК СчетаСОсобымПорядкомПереоценки))";
...
Процедура УстановитьТипыСвязанныхСубконто(СвязанныеСубконто) Экспорт
	...
	ТипыСубконто = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ПолучитьОписаниеТиповДоговора();
...
Функция СчетаВыручкиНеЕНВД() Экспорт
	...
	Запрос.УстановитьПараметр("Счета90_01", 
		CW_БухгалтерскийУчетПовтИспМСФО.СчетаВИерархии(ПланыСчетов.CW_МСФО.Выручка));
	...
	|		ПО (СчетаДоходовИРасходовЕНВД.Счет = CW_МСФО.Ссылка)
	|ГДЕ
	|	CW_МСФО.Ссылка В(&Счета90_01)
...
Процедура ОбработатьРезультатПроверкиИтогов(Форма, РезультатВыполнения) Экспорт
	...
	МестоРасхождения = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.МестоРасхожденияИтогов(Форма);
	ЗарегистрированныеСобытия = CW_БухгалтерскиеОтчетыМСФО.ЗаписьВЖурналОРасхожденииИтогов(МестоРасхождения, "Ошибка");
