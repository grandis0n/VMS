...
Процедура НастроитьФормуПодписанияИШифрования(Форма) Экспорт
	...
	ДоступныеСертификаты = Форма.ОтборСертификатов.ВыгрузитьЗначения();
	Если ДоступныеСертификаты.Количество() > 0 Тогда
		Форма.Сертификат = ДоступныеСертификаты[0];
	КонецЕсли;
	
	ЭлементыСтиля = ИнтерфейсМобильногоЭДОПовтИсп.ЭлементыСтиля();
	Элементы.Сертификат.СписокВыбора.ЗагрузитьЗначения(ДоступныеСертификаты);
	
	Форма.ПодключитьБиометрию = НастройкаРутокен("ПодключитьБиометрию", Ложь);
	Элементы.ФормаПодключитьБиометрию.Пометка = Форма.ПодключитьБиометрию;
	СвойстваДоступныхСертификатов = КриптографияБЭД.СвойстваСертификатов(ДоступныеСертификаты);
	Форма.СвойстваСертификатовАдрес = ПоместитьВоВременноеХранилище(СвойстваДоступныхСертификатов);
	
	Если ЭтоФормаПодписания Тогда
		ИнтерфейсМобильногоЭДОКлиентСервер.ОформитьАкцентнуюКнопку(ЭлементыСтиля, Элементы.Подписать);
	Иначе
		ИнтерфейсМобильногоЭДОКлиентСервер.ОформитьАкцентнуюКнопку(ЭлементыСтиля, Элементы.ФормаРасшифровать);
	КонецЕсли;

КонецПроцедуры

// Сохраняет настройку Рутокен.
//
// Параметры:
//  Имя - Строка - Имя настройки.
//  ЗначениеНастройки - Произвольный - Значение настройки.
//
Процедура СохранитьНастройкуРутокен(Имя, ЗначениеНастройки) Экспорт
	
	КлючОбъекта = "ПодписаниеИШифрованиеВМобильномЭДО";

	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъекта, Имя, ЗначениеНастройки);

КонецПроцедуры

// Возвращает сохраненную ранее настройку Рутокен.
//
// Параметры:
//  Имя - Строка - Имя настройки.
//  ЗначениеНастройкиПоУмолчанию - Произвольный - Значение настройки по умолчанию.
// Возвращаемое значение:
//  Произвольный - Значение настройки.
//
Функция НастройкаРутокен(Имя, ЗначениеНастройкиПоУмолчанию = Неопределено) Экспорт
	
	КлючОбъекта = "ПодписаниеИШифрованиеВМобильномЭДО";

	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, Имя, ЗначениеНастройкиПоУмолчанию);
