	...
	Для Каждого Документ Из СоздаваемыеДокументы Цикл
		...
		Попытка
			Если Документ.ВидДокумента = ВидДокументаWBУведомлениеВыкупа() 
				ИЛИ Документ.ВидДокумента = ВидДокументаОтчетOzonВыкуп()
				Или Документ.ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
			...
			ИначеЕсли Документ.ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
				Если Документ.Свойство("СоздатьКорректировкуРеализации") Тогда
					СоздатьНовыйДокумент = Документ.СоздатьКорректировкуРеализации;
					СозданДокумент = Документы.КорректировкаРеализации.СоздатьКорректировкуПередачиТовараНаРеализацию(
						Документ, СоздатьНовыйДокумент, РезультатПоСтроке);
				Иначе
					СозданДокумент = Документы.РеализацияТоваровУслуг.СоздатьПередачуТовараНаРеализацию(Документ, РезультатПоСтроке);
				КонецЕсли;
			Иначе
...
Процедура ПровестиСозданныеДокументы(ДокументыДляПроведения, АдресХранилища = Неопределено) Экспорт
	...
	Для Каждого ДокументДляПроведения Из ДокументыДляПроведения Цикл
		...
		ТекстОшибкиПроведения = НСтр("ru = 'Не удалось провести документ'");
		...
		Исключение
			Результат.Вставить(ДокументОбъект.Ссылка, ТекстОшибкиПроведения);
		КонецПопытки;
	КонецЦикла;
...
Процедура НайтиСоздатьНоменклатуруПоДокументу(ДокументСсылка)
	...
	ВидСтавкиПоУмолчанию = Перечисления.ВидыСтавокНДС.ВидСтавкиДляНоменклатуры(СтавкаНДСПоУмолчанию);
	...
	Для Каждого СтрокаТовара Из ТаблицаНовыхЭлементов Цикл
		...
		НоменклатураОбъект.ВидСтавкиНДС         = ВидСтавкиПоУмолчанию;
...
Процедура ПолучитьДанныеВФорматеБЭД(ДвоичныеДанныеФайла, Расширение, АдресХранилища) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		ДвоичныеДанныеФайлаЭД = ЗаписатьДанныеВXML(ДанныеДокумента);
		...
		Иначе
			...
			Результат.ДвоичныеДанные = ДвоичныеДанныеФайлаЭД;
		КонецЕсли;
	КонецЕсли;
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
...
Процедура ДобавитьОписаниеПрочитанногоФайлаWBДетальный(РезультатЧтения, ДанныеДокумента, ПолноеИмяФайла)
	...
	Для Каждого СтрокаТовара Из ДанныеДокумента.ТаблицаТоваров Цикл
		...
		ИначеЕсли ИнтеграцияWildberries.ВозвратТовараМаркетплейсом(СтрокаТовара.ВидДвижения,СтрокаТовара.ТипДокумента) Тогда
			...
			Если Не ИнтеграцияWildberries.ПродажаЧерезВыкупТовара(СтрокаТовара.КодСтраны, СтрокаТовара.НазваниеСтраны) Тогда
	...
	Для Каждого СтрокаТовара Из ДанныеДокумента.ТаблицаТоваров Цикл
		СтрокаТовара.НаимТов = ИнтеграцияWildberries.СформироватьНаименованиеТовара(СтрокаТовара.НаимТов, СтрокаТовара.Размер);
	КонецЦикла;
...
Процедура ДобавитьОписаниеПрочитанногоФайлаРеестрПродажЮрлицамWB(РезультатЧтения, ДанныеДокумента, ПолноеИмяФайла)
	...
	НачалоПериода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеДокумента, "НачалоПериода", Дата(1, 1, 1));
	ОкончаниеПериода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеДокумента, "ОкончаниеПериода", Дата(1, 1, 1));
	
	Описание = СтрШаблон(НСтр("ru='Реестр продаж юридическим лицам WB за период с %1 по %2
		|Продажи: %3 Возвраты: %4
		|Всего документов: %5, покупателей: %6'"),
		Формат(НачалоПериода, "ДФ=dd.MM.yyyy"),
		Формат(ОкончаниеПериода, "ДФ=dd.MM.yyyy"),
...
Функция ПолучитьДанныеФайлов(ЗагруженныеФайлы) Экспорт
	...
	Для Каждого ЗагруженныйФайл Из ЗагруженныеФайлы Цикл
		...
		Если ЗагруженныйФайл.РасширениеФайла = "zip"
			...
			СодержитПолезныеФайлы = Ложь;
			ВременныйКаталог = ФайловаяСистема.СоздатьВременныйКаталог();
			
			ДанныеВПотоке = ЗагруженныйФайл.ДвоичныеДанные.ОткрытьПотокДляЧтения();
			ЧтениеФайлаАрхива = Новый ЧтениеФайлаАрхива(ДанныеВПотоке);
			
			Для Каждого ФайлАрхива Из ЧтениеФайлаАрхива.Элементы Цикл
				Если НРег(ФайлАрхива.Расширение) = "xlsx"
					или НРег(ФайлАрхива.Расширение) = "csv" Тогда
					ОписаниеФайла = Новый Структура;
					Если НРег(ФайлАрхива.Расширение) = "csv" Тогда
						ОписаниеФайла.Вставить("ИмяФайла", ФайлАрхива.Имя);
					Иначе
						//Имя файла нужно сохранить для постобработки в форме загрузки
						ОписаниеФайла.Вставить("ИмяФайла", ЗагруженныйФайл.ИмяФайла);
					КонецЕсли;
					ОписаниеФайла.Вставить("РасширениеФайла", ФайлАрхива.Расширение);
					...
					Результат = Новый ДвоичныеДанные(
						ВременныйКаталог + ПолучитьРазделительПути() + ФайлАрхива.Имя);
				
					ОписаниеФайла.Вставить("ДвоичныеДанные", Результат);
					
					ЗагруженныеФайлы.Добавить(ОписаниеФайла);
					СодержитПолезныеФайлы = Истина;
				КонецЕсли;
			КонецЦикла;
			ФайловаяСистема.УдалитьВременныйКаталог(ВременныйКаталог);
			
			Если СодержитПолезныеФайлы Тогда
	...
	Для Каждого ЗагруженныйФайл Из ЗагруженныеФайлы Цикл
		...
		Иначе
			РезультатЧтенияФайла = ЗагрузитьФайлВТаблицу(
				ЗагруженныйФайл.ДвоичныеДанные, 
...
Функция ДанныеТабличногоДокумента(ДвоичныеДанныеФайла, РасширениеФайла)
	...
	ДанныеФайла = ЗагрузитьФайлВТаблицу(ДвоичныеДанныеФайла,, РасширениеФайла);
...
Функция ПолучитьДанные(ТаблицаДанных, МаксИндексКолонок, КоличествоСтрок, ВидДокумента, ТекстПрогресса, ДелительПрогресса = 1)
	...
	ИначеЕсли ВидДокумента = ВидДокументаОтчетOzon() Тогда
		...
		КоличествоСовпаденийНумерацииКолонок = 4;
		ТаблицаТоваров = ПодготовитьТаблицуТоваровМаркетплейс();
	ИначеЕсли ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		КоличествоСовпаденийЗаголовков = 4;
		КоличествоСовпаденийВторойСтрокиЗаголовков = 1;
	...
	ИначеЕсли ВидДокумента = ВидДокументаWBУведомлениеВыкупа() Тогда
		...
		КоличествоСовпаденийНумерацииКолонок = 0;
		ТаблицаТоваров = ПодготовитьТаблицуТоваровУведомлениеОВыкупе();
	ИначеЕсли ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
		КоличествоСовпаденийЗаголовков = 1;
		КоличествоСовпаденийВторойСтрокиЗаголовков = 3;
		КоличествоСовпаденийНумерацииКолонок = 0;
		ТаблицаТоваров = ПодготовитьТаблицуТоваровАктПриемки();
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Если Не ДанныеШапкиПолучены
			И ВидДокумента = ВидДокументаОтчетWBДетальный() Тогда
			ДанныеШапкиПолучены = Истина;
		КонецЕсли;
		
		Если Не ДанныеШапкиПолучены И ВидДокумента = ВидДокументаРеестрПродажЮрлицамWB() Тогда
			ПериодРеестраПродажЮрлицамWB(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента);
			Если ДанныеДокумента.Свойство("НачалоПериода")
				И ДанныеДокумента.Свойство("ОкончаниеПериода") Тогда
				ДанныеШапкиПолучены = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если Не ДанныеШапкиПолучены Тогда
			...
			ИначеЕсли ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
				...
				Если Не ДанныеДокумента.Свойство("НачалоПериода") Тогда
					ПериодДетализацииПоВыкупленнымТоварамOzon(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента);
				КонецЕсли;
				...
				Если Не ДанныеДокумента.Свойство("Плательщик") Тогда
					ДанныеКонтрагента = ПолучитьДанныеКонтрагентаОтчетОзон(СтрокаДанных, МаксИндексКолонок, ТаблицаДанных, "Покупатель");
					Если ДанныеКонтрагента.Количество() > 0 Тогда
						ДанныеДокумента.Вставить("Плательщик", ДанныеКонтрагента);
					КонецЕсли;
				КонецЕсли;
			...
			ИначеЕсли ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
				...
				ДанныеШапкиАктПриемкиТовараWB(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента);
				Если ДанныеДокумента.Свойство("Дата")
					И ДанныеДокумента.Свойство("Номер")
					И ДанныеДокумента.Свойство("Плательщик")
					И ДанныеДокумента.Свойство("Получатель") Тогда
			...
			Если НЕ НайденаСтрокаЗаголовкаТаблицы Тогда
				...
				Если ТаблицаЗаголовков <> Неопределено Тогда
					...
					Если МассивЗаголовков.Количество() >= КоличествоСовпаденийЗаголовков Тогда
						...
						Если ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
							НайденаВтораяСтрокаЗаголовкаТаблицы = Ложь;
							НайденаСтрокаНумерацииКолонокТаблицы = Истина;
						КонецЕсли;
	...
	Если ДанныеДокумента.ВидДокумента = ВидДокументаПриложение1Мегамаркет()
		...
		КонецЦикла;
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			СтрокаТовара.НомерДокумента = СокрЛП(СтрокаТовара.НомерДокумента);
		КонецЦикла;
	КонецЕсли;
	...
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
	...
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаОтчетWB() 
		...
		ИЛИ ДанныеДокумента.ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon()
...
Функция ПодготовитьТаблицуЗаголовковТабличнойЧасти(СтрокаДанных, МаксИндексКолонок, ВидДокумента, НомерТаблицы)
	...
	Для Н = 0 По МаксИндексКолонок Цикл
		...
		ИначеЕсли ВидДокумента = ВидДокументаОтчетOzon()
			...
			Или ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		...
		ИначеЕсли ВидДокумента = ВидДокументаРеестрПродажЮрлицамOzon() Тогда
			...
			КонецЕсли;
		ИначеЕсли ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
			Если СтрНайти(ПодготовленноеЗначение, "фактическипринято") > 0 Тогда
				ИскатьЗаголовки = Истина;
				Прервать;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		...
		ДобавитьСтрокиВТаблицуЗаголовковДетализацияПоВыкупленнымТоварамOzon(ТаблицаЗаголовков);
	...
	ИначеЕсли ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
		...
		ДобавитьСтрокиВТаблицуЗаголовковWBАктПриемки(ТаблицаЗаголовков);
...
Процедура НайтиВторуюСтрокуЗаголовкаТабличнойЧасти(СтрокаДанных, ТаблицаЗаголовков, МаксИндексКолонок, ВидДокумента)
	...
	Для Н = 0 По МаксИндексКолонок Цикл
		...
		Если ПодготовленноеЗначение = "код" 
			Или СтрНайти(ПодготовленноеЗначение, "баркод") > 0
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Для Н = 0 По МаксИндексКолонок Цикл
			...
			ИначеЕсли СтрНайти(ПодготовленноеЗначение, "дляпродавцовнаплатформеozon") > 0 Тогда
				...
				ИначеЕсли Ключ1 = "детализацияповыкупленнымтоварам" Тогда
					ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon();
				Иначе
			...
			ИначеЕсли СтрНайти(ПодготовленноеЗначение, "актприемкитовара") > 0 Тогда
				
				ВидДокумента = ВидДокументаАктПриемкиТовараWB();
				Прервать;
			...
			ИначеЕсли СтрНайти(ПодготовленноеЗначение, "детализацияповыкупленнымтоварам") > 0 Тогда
				
				Ключ1 = "детализацияповыкупленнымтоварам";
...
Процедура ПериодРеестраПродажЮрлицамWB(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента)
	
	ТекстЯчеекСтроки = Новый Массив;
	Для Сч = 0 По МаксИндексКолонок Цикл
		ЗначениеЯчейки = УдалитьПереносыСтроки(СтрокаДанных[Сч]);
		ЗначениеЯчейки = ПодготовитьСтроку(ЗначениеЯчейки, Ложь);
		Если ЗначениеЗаполнено(ЗначениеЯчейки) Тогда
			ТекстЯчеекСтроки.Добавить(ЗначениеЯчейки);
		КонецЕсли;
	КонецЦикла;
	Если Не ЗначениеЗаполнено(ТекстЯчеекСтроки) Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаПоиска = СтрСоединить(ТекстЯчеекСтроки, " ");
	
	// период с 27072026 по 02082026
	РезультатПоиска = СтрНайтиВсеПоРегулярномуВыражению(СтрокаПоиска, "период\s+с\s+(\d{2})(\d{2})(\d{4})\s+по\s+(\d{2})(\d{2})(\d{4})");
	Если ЗначениеЗаполнено(РезультатПоиска) Тогда
		Фрагменты = РезультатПоиска[0].ПолучитьГруппы();
		Если Фрагменты.Количество() = 6 Тогда
			НачалоПериода    = СтрШаблон("%1.%2.%3", Фрагменты[0].Значение, Фрагменты[1].Значение, Фрагменты[2].Значение);
			ОкончаниеПериода = СтрШаблон("%1.%2.%3", Фрагменты[3].Значение, Фрагменты[4].Значение, Фрагменты[5].Значение);
			ДанныеДокумента.Вставить("НачалоПериода", СтроковыеФункцииКлиентСервер.СтрокаВДату(НачалоПериода));
			ДанныеДокумента.Вставить("ОкончаниеПериода", СтроковыеФункцииКлиентСервер.СтрокаВДату(ОкончаниеПериода));
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаписатьДанныеВXML(ДанныеДокумента)
	...
	Иначе
		...
		ДвоичныеДанныеФайлаЭД = Новый ДвоичныеДанные(ФайлАрхива);
		ФайловаяСистема.УдалитьВременныйФайл(ФайлАрхива);
...
Функция ЗагрузитьФайлВТаблицу(ДвоичныеДанныеФайла, ПоОбластям = Ложь, РасширениеФайла)
	
	//Платформа не умеет читать табличный документ из потока с типом xls
...
Функция СоздатьЭлементСправочникаНоменклатура(НоменклатураКонтрагента, ЗначенияРеквизитов, Ошибки)
	...
	Наименование = НаименованиеНоменклатурыПоДаннымКонтрагента(НоменклатураКонтрагента);
	...
	НоменклатураОбъект.ВидСтавкиНДС = Перечисления.ВидыСтавокНДС.ВидСтавкиДляНоменклатуры(СтавкаНДС);
...
Функция СоздатьЭлементСправочникаОсновныеСредства(НоменклатураКонтрагента, ЗначенияРеквизитов, Ошибки)
	...
	ОсновноеСредствоОбъект.Наименование        = Наименование;
	ОсновноеСредствоОбъект.НаименованиеПолное  = Наименование;
...
Функция СоздатьЭлементСправочникаОбъектыСтроительства(НоменклатураКонтрагента, ЗначенияРеквизитов, Ошибки)
	...
	ОбъектСтроительстваОбъект.Наименование = Наименование;
...
Функция ЗначенияЗаполненияНовогоЭлементаСправочника(ДанныеКонтрагента) Экспорт
	...
	Наименование = НаименованиеНоменклатурыПоДаннымКонтрагента(ДанныеКонтрагента);
	
	ЗначенияЗаполнения.Вставить("Наименование", Наименование);
	ЗначенияЗаполнения.Вставить("НаименованиеПолное", Наименование);
	...
	Если ЗначениеЗаполнено(СтавкаНДС) Тогда
		ЗначенияЗаполнения.Вставить("ВидСтавкиНДС", Перечисления.ВидыСтавокНДС.ВидСтавкиДляНоменклатуры(СтавкаНДС));
	КонецЕсли;
...
Функция НаименованиеНоменклатурыПоДаннымКонтрагента(ДанныеКонтрагента)
	
	Характеристика = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеКонтрагента, "Характеристика");
	Если ЗначениеЗаполнено(Характеристика) Тогда
		Наименование = СтрШаблон("%1 (%2)", ДанныеКонтрагента.Наименование, Характеристика);
	Иначе
...
Процедура ДобавитьОписаниеПрочитанногоФайла(РезультатЧтения, ДанныеДокумента, ПолноеИмяФайла)
	...
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаАктПриемкиТовараWB() Тогда
		...
		Если ДанныеДокумента.Свойство("Получатель") Тогда
			...
			Организация = СсылкаНаОбъектПоИННКПП("Организации", ИНН, КПП);
			Если Организация = Неопределено Тогда
				Организация = СсылкаНаОбъектПоИННКПП("Организации", ИНН);
			КонецЕсли;
		КонецЕсли;
		...
		Если ДанныеДокумента.Свойство("Плательщик") Тогда
			ИНН = "";
			КПП = "";
			ДанныеДокумента.Плательщик.Свойство("ИНН", ИНН);
			ДанныеДокумента.Плательщик.Свойство("КПП", КПП);
		...
		Описание = СтрШаблон(НСтр("ru='Акт приемки товара Wildberries № %1 от %2'"),
			ДанныеДокумента.Номер, Формат(ДанныеДокумента.Дата, "ДФ=dd.MM.yyyy"));
		...
		Результат.Вставить("Контрагент",      Контрагент);
		Результат.Вставить("Дата",            ДанныеДокумента.Дата);
		Результат.Вставить("СуммаКоличество", ДанныеДокумента.ТаблицаТоваров.Итог("Количество"));
		Результат.Вставить("Загружать",       Истина);
		Результат.Вставить("Описание",        Описание);
		ДобавитьКонтрагентаВОписаниеПрочитанногоФайла(Результат);
		ДобавитьНоменклатуруВОписаниеПрочитанногоФайла(Результат);
	...
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		...
		Результат.Вставить("Организация",     Организация);
		Результат.Вставить("Дата",            ДанныеДокумента.Дата);
		Результат.Вставить("НомерВходящегоДокумента", ДанныеДокумента.Номер);
		Результат.Вставить("СуммаВыкупа",     СуммаВыкупа);
		Результат.Вставить("Загружать",       СуммаВыкупа > 0);
		ДобавитьКонтрагентаВОписаниеПрочитанногоФайла(Результат);
		ДобавитьНоменклатуруВОписаниеПрочитанногоФайла(Результат);
		
		Описание = СтрШаблон(НСтр("ru='Детализация по выкупленным товарам Ozon за период с %1 по %2
			|Сумма выкупа: %3'"),
			Формат(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеДокумента, "НачалоПериода", Дата(1,1,1)), "ДФ=dd.MM.yyyy"),
			Формат(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеДокумента, "КонецПериода", Дата(1,1,1)), "ДФ=dd.MM.yyyy"),
			Формат(СуммаВыкупа, "ЧДЦ=2; ЧН="));
		Результат.Вставить("Описание", Описание);
...
Процедура ДобавитьНоменклатуруВОписаниеПрочитанногоФайла(СтруктураОписания)
	...
	Если СтруктураОписания.Свойство("ВидДокумента") Тогда
		ЕстьСтавкаНДС = (СтруктураОписания.ВидДокумента = ВидДокументаОтчетOzonВыкуп()
			Или СтруктураОписания.ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon()
...
Функция ЭтоЗагрузкаИзМаркетплейса(ВидДокумента)
	...
	Возврат (ВидДокумента = ВидДокументаОтчетWB()
		...
		Или ВидДокумента = ВидДокументаОтчетOzonВыкуп()
		Или ВидДокумента = ВидДокументаДетализацияПоВыкупленнымТоварамOzon()
...
Функция ПодготовитьТаблицуТоваровМаркетплейс() Экспорт
	...
	ТаблицаТоваров.Колонки.Добавить("НомТов", ТипСтрока);
	ТаблицаТоваров.Колонки.Добавить("НаимТов", ТипСтрока);
	ТаблицаТоваров.Колонки.Добавить("Размер", ТипСтрока);
	ТаблицаТоваров.Колонки.Добавить("Артикул", ТипСтрока);
...
Функция ПодготовитьТаблицуТоваровАктПриемки()
...
Функция ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Экспорт
	
	Возврат "Детализация_по_выкупленным_товарам_Ozon";
...
Процедура ПериодДетализацииПоВыкупленнымТоварамOzon(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента)
	
	// "Детализация по выкупленным товарам за период с 01.07.2026 по 15.07.2026";
	ВыражениеПоиска = "за период с";
	
	Для Сч = 0 По МаксИндексКолонок Цикл
		РезультатПоиска = СтрНайти(СтрокаДанных[Сч], ВыражениеПоиска);
		Если РезультатПоиска > 0 Тогда
			СтрокаПериод = СтрЗаменить(Сред(СтрокаДанных[Сч], РезультатПоиска), ВыражениеПоиска, "");
			ЭлементыПериода = СтрРазделить(СтрокаПериод, "по", Ложь);
			Если ЭлементыПериода.Количество() = 2 Тогда
				ДанныеДокумента.Вставить("НачалоПериода", СтроковыеФункцииКлиентСервер.СтрокаВДату(СокрЛП(ЭлементыПериода[0])));
				ДанныеДокумента.Вставить("КонецПериода",  СтроковыеФункцииКлиентСервер.СтрокаВДату(СокрЛП(ЭлементыПериода[1])));
				ДанныеДокумента.Вставить("Дата", ДанныеДокумента.КонецПериода);
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковДетализацияПоВыкупленнымТоварамOzon(ТаблицаЗаголовков)
	...
	// Отчет о выкупленных товарах номер
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
	...
	// Отчет о выкупленных товарах дата
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
...
Функция ВидДокументаАктПриемкиТовараWB() Экспорт
	
	Возврат "Акт_приемки_товара_WB";
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковОтчетWBДетальный(ТаблицаЗаголовков)
	...
	// размер
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
	НоваяСтрокаЗаголовков.НазваниеКолонки 	= "Размер";
	НоваяСтрокаЗаголовков.СтрокаПоиска 		= "размер";
	НоваяСтрокаЗаголовков.НомерКолонки 		= " ";
	НоваяСтрокаЗаголовков.СтрокаЗаголовка 	= 1;
	НоваяСтрокаЗаголовков.ТипДанных 		= "Строка";
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковWBУведомлениеВыкупа(ТаблицаЗаголовков)
	...
	НоваяСтрокаЗаголовков.СтрокаЗаголовка 	= 1;
	НоваяСтрокаЗаголовков.ТипДанных 		= "Число";
	// заголовок колонки в файле "Сумма НДС*,Руб."
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
	НоваяСтрокаЗаголовков.НазваниеКолонки 	= "СуммаНДС";
	НоваяСтрокаЗаголовков.СтрокаПоиска 		= "суммандс";	
	НоваяСтрокаЗаголовков.НомерКолонки 		= "7";
	НоваяСтрокаЗаголовков.СтрокаЗаголовка 	= 1;
	НоваяСтрокаЗаголовков.ТипДанных 		= "Число";
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковРеестрПродажЮрлицамWB(ТаблицаЗаголовков)
	...
	// Номер счета-фактуры продавца (корректировка к счету-фактуре)
	Заголовок = ТаблицаЗаголовков.Добавить();
	Заголовок.НазваниеКолонки 	= "НомерСФ";
	Заголовок.СтрокаПоиска 		= "номерсчетафактурыпродавцакорректировкаксчетуфактуре";
	Заголовок.СтрокаЗаголовка 	= 1;
	Заголовок.ТипДанных 		= "Строка";
	...
	Заголовок = ТаблицаЗаголовков.Добавить();
	Заголовок.НазваниеКолонки 	= "ДатаСФ";
	Заголовок.СтрокаПоиска 		= "датасчетафактурыпродавца";
	Заголовок.СтрокаЗаголовка 	= 1;
	Заголовок.ТипДанных 		= "Дата";
	...
	Заголовок.СтрокаПоиска 		= "номеркорректировочногосчетафактурыпродавца";
	Заголовок.СтрокаЗаголовка 	= 1;
	Заголовок.ТипДанных 		= "Строка";
	...
	Заголовок.СтрокаПоиска 		= "датакорректировочногосчетафактурыпродавца";
	Заголовок.СтрокаЗаголовка 	= 1;
	Заголовок.ТипДанных 		= "Дата";
...
Процедура ДобавитьСтрокиВТаблицуЗаголовковWBАктПриемки(ТаблицаЗаголовков)
	...
	НоваяСтрокаЗаголовков = ТаблицаЗаголовков.Добавить();
	НоваяСтрокаЗаголовков.НазваниеКолонки 	= "НаимТов";
	НоваяСтрокаЗаголовков.СтрокаПоиска 		= "товарнаименование";
	НоваяСтрокаЗаголовков.НомерКолонки 		= "2";
	НоваяСтрокаЗаголовков.СтрокаЗаголовка 	= 1;
	НоваяСтрокаЗаголовков.ТипДанных 		= "Строка";
...
Процедура ДанныеШапкиАктПриемкиТовараWB(СтрокаДанных, МаксИндексКолонок, ДанныеДокумента)
	...
	ИскатьНомер = Не ДанныеДокумента.Свойство("Номер");
	ИскатьДату  = Не ДанныеДокумента.Свойство("Дата");
	ИскатьПолучателя = Не ДанныеДокумента.Свойство("Получатель");
	ИскатьПлательщика = Не ДанныеДокумента.Свойство("Плательщик");
	
	Если ИскатьНомер Тогда
		// акт приемки товара  29808496
		Если СтрНайти(СтрокаПоиска, "акт приемки товара") Тогда
			Номер = СокрЛП(СтрЗаменить(СтрокаПоиска, "акт приемки товара", ""));
			Если ЗначениеЗаполнено(Номер) Тогда
				ДанныеДокумента.Вставить("Номер", Номер);
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ИскатьДату Тогда
		// 03 06 2025г
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(СтрокаПоиска, "\d\d \d\d \d\d\d\dг");
		Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
			ДатаСтрокой = Сред(СтрокаПоиска, РезультатПоиска.НачальнаяПозиция, РезультатПоиска.Длина - 1);
			ДатаДокумента = СтроковыеФункцииКлиентСервер.СтрокаВДату(ДатаСтрокой);
			ДанныеДокумента.Вставить("Дата", ДатаДокумента);
		КонецЕсли;
	ИначеЕсли ИскатьПолучателя Тогда
		// продавец индивидуальный предприниматель инн 526312147937
		ДанныеКонтрагента = Новый Структура;
		
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(СтрокаПоиска, " инн\s+\d{10}(\d{2})?");
		Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
			ИНН = Сред(СтрокаПоиска, РезультатПоиска.НачальнаяПозиция, РезультатПоиска.Длина);
			ИНН = СокрЛП(СтрЗаменить(ИНН, "инн", ""));
			ДанныеКонтрагента.Вставить("ИНН", ИНН);
		КонецЕсли;
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(СтрокаПоиска, " кпп\s+\d{9}");
		Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
			КПП = Сред(СтрокаПоиска, РезультатПоиска.НачальнаяПозиция, РезультатПоиска.Длина);
			КПП = СокрЛП(СтрЗаменить(КПП, "кпп", ""));
			ДанныеКонтрагента.Вставить("КПП", КПП);
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеКонтрагента) Тогда
			ДанныеДокумента.Вставить("Получатель", ДанныеКонтрагента);
		КонецЕсли
	ИначеЕсли ИскатьПлательщика Тогда
		// вайлдберриз ооо рвб инн 9714053621
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(СтрокаПоиска, " инн\s+\d{10}");
		Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
			ИНН = Сред(СтрокаПоиска, РезультатПоиска.НачальнаяПозиция, РезультатПоиска.Длина);
			ИНН = СокрЛП(СтрЗаменить(ИНН, "инн", ""));
			ДанныеКонтрагента = Новый Структура("ИНН", ИНН);
			ДанныеДокумента.Вставить("Плательщик", ДанныеКонтрагента);
		КонецЕсли;
	КонецЕсли;
