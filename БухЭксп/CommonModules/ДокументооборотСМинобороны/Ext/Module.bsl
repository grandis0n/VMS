...
// Формирует архив пакета документов с отчетом, подписью и файлами доверенностей для отправки в систему Минобороны.
//
// Параметры:
//   ПараметрыАрхивации - Структура:
//     * ИмяФайлаПодписи - Строка - имя файла подписи
//     * СтрокаДанныхПодписи - Строка - строковые данные подписи
//     * ИмяФайлаВыгрузки - Строка - имя файла выгрузки
//     * АдресФайлаВыгрузки - Строка - адрес файла выгрузки во временном хранилище
//     * ОтчетСсылка - ОтчетСсылка - ссылка на отчет
//     * Доверенность - СправочникСсылка.Доверенности - ссылка на доверенность
//     * ТипФайлаВыгрузки - Строка - тип файла выгрузки (см. ДокументооборотСМинобороныКлиентСервер.ТипФайлаВыгрузки)
//     * ИНН - Строка - ИНН организации
//
// Возвращаемое значение:
//   Структура:
//     * Выполнено - Булево - признак успешного выполнения
//     * АдресАрхива - Строка - адрес созданного архива во временном хранилище
//     * ОписаниеОшибки - Строка - текст ошибки при неудачном выполнении
//     * ДатаСоставления - Дата - дата составления отчета
Функция СформироватьАрхивПакета(ПараметрыАрхивации) Экспорт
	...
	ПолноеИмяФайлаПодписи = КаталогВременныхФайловНаСервере
		+ РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ИмяФайлаПодписи);
	ПолноеИмяФайлаВыгрузки = КаталогВременныхФайловНаСервере
		+ РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ИмяФайлаВыгрузки);
	
	// записываем файл без BOM
	Попытка
		// Файл проверяется на наличие недопустимых символов.
...
// Сохраняет ответ от системы Минобороны в запись отправки
//
// Параметры:
//   Параметры - Структура:
//     * ДанныеОтвета - ДвоичныеДанные - текст протокола ответа
//     * ИмяФайла - Строка
//     * Идентификатор - Строка
//     * АдресАрхива - Строка - адрес во временном хранилище архива уведомления
//
// Возвращаемое значение:
//   Структура:
//     * Выполнено - Булево
//     * ОписаниеОшибки - Строка
//     * ОтчетСсылка - СправочникСсылка
//     * ТекстПротокола - Строка
//     * ОтправкаСсылка - СправочникСсылка.ОтправкиМинобороны
...
// Возвращает адрес во временном хранилище, содержащий дерево протокола сдачи для указанного источника
//
// Параметры:
//   ИсточникСсылка - СправочникСсылка.ОтправкиМинобороны
//                  - ДокументСсылка.РегламентированныйОтчет
//                  - СправочникСсылка.ЭлектронноеПредставление - отчет или отправка, для которой требуется получить протокол сдачи
//
// Возвращаемое значение:
//   Строка - адрес во временном хранилище, содержащий ДеревоЗначений с протоколом сдачи
//   - Неопределено - если протокол отсутствует или не удалось загрузить
Функция ПолучитьАдресДереваПротоколаСдачи(ИсточникСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(Отправка) Тогда 
		...
		Если Протокол <> Неопределено Тогда
			// Протокол извлекается из хранилища значения.
			//@skip-check 1c-fresh-audit
...
// Загружает сертификаты Министерства обороны с сервера справочной информации
//
// Возвращаемое значение:
//   Структура:
//     * Сертификаты - Массив из Структура - массив сертификатов, каждый сертификат содержит:
//         ** Описание - Строка - описание сертификата
//         ** Отпечаток - Строка - отпечаток сертификата
//         ** Хранилище - Строка - наименование хранилища сертификата
//         ** СодержимоеСертификата - ДвоичныеДанные - содержимое сертификата в двоичном виде
//         ** СодержимоеСертификатаBase64 - Строка - содержимое сертификата в формате Base64
//         ** СертификатНайденВХранилище - Булево - признак нахождения сертификата в хранилище
//     * Хранилища - Соответствие - ключ - наименование хранилища (Строка), значение - Истина
//     * ОписаниеОшибки - Строка - текст ошибки, если выполнение завершилось с ошибкой
Функция ЗагрузитьСертификатыМинобороны() Экспорт
	...
	// Имя файла возвращется с сервера.
	РегламентированнаяотчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(ИмяФайлаСертификатов,
		"ИмяФайлаСертификатов", "ДокументооборотСМинобороны.ЗагрузитьСертификатыМинобороны");
	//@skip-check 1c-fresh-audit
