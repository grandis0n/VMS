...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Если ОплатаПлатежнойКартойКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		...
		Элементы.РасшифровкаПлатежаСчетНаОплатуБезДокументов.Видимость = Не ЭтоВозвратПокупателю И Не ЭтоРозничнаяВыручка;
	...
	Если ОплатаПлатежнойКартойКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		Элементы.ГруппаБезЗакрывающихДокументов.Видимость = Объект.БезЗакрывающихДокументов;
	КонецЕсли;
...
// Возвращает результат сравнения вида операции:
// Истина - вид операции относится к оплате от покупателя, иначе ЛОЖЬ.
//
// Возвращаемое значение:
//   Булево.
//
Функция ЭтоРасчетыСКонтрагентом(ВидОперации) Экспорт
	...
	Возврат ВидыОперацийОплатаПокупателя().Найти(ВидОперации) <> Неопределено
		Или ЭтоВозвратПокупателю(ВидОперации);
	
КонецФункции

// Возвращает виды операций, относящихся к оплате от покупателя
//
// Возвращаемое значение:
//  ФиксированныйМассив - массив из ПеречислениеСсылка.ВидыОперацийОплатаПлатежнойКартой
//
Функция ВидыОперацийОплатаПокупателя() Экспорт
	
	ВидыОпераций = Новый Массив;
	ВидыОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя"));
	ВидыОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ПлатежПоРеестру"));
	ВидыОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаСчетаЧерезСБП"));
...
Функция ВидимостьУслугНПД(Форма) Экспорт
	...
	ЧекСформирован       = БанкИКассаФормыКлиентСервер.ЧекНПДСформирован(Форма);
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	Если ФормаДокументаОднострочная(Форма) Тогда
		СтрокаРасшифровки = ДанныеРасшифровкиПлатежаОднострочнойФормы(Форма);
		ЕстьУслугаНПД = Не ЗначениеЗаполнено(СтрокаРасшифровки.СчетНаОплату)
			Или ЧекСформирован И ЗначениеЗаполнено(СтрокаРасшифровки.УслугаНПД);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"УслугаНПД",
			"Видимость",
			ВидимостьУслугНПД И Не НесколькоСтрокВРасшифровке И ЕстьУслугаНПД);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "РасшифровкаПлатежаСчетНаОплатуНПД", "Видимость", ВидимостьУслугНПД);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "РасшифровкаПлатежаСчетНаОплату", "Видимость", Не ВидимостьУслугНПД);
	КонецЕсли;
...
Процедура УстановитьНаличиеУслугНПД(Форма) Экспорт
	
	РасшифровкаПлатежа = РеквизитДокумента(Форма, НаименованиеТабличнойЧасти());
	
	Форма.ЕстьУслугиНПД = Ложь;
	Для Каждого СтрокаПлатеж Из РасшифровкаПлатежа Цикл
		Если Не ЗначениеЗаполнено(СтрокаПлатеж.СчетНаОплату)
			Или БанкИКассаФормыКлиентСервер.ЧекНПДСформирован(Форма) И ЗначениеЗаполнено(СтрокаПлатеж.УслугаНПД) Тогда
			Форма.ЕстьУслугиНПД = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
