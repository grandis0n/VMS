...
Процедура ФормаРегламентированнойОтчетности_ПриСозданииНаСервере(Форма) Экспорт
	...
	ИменаДоступныхГруппОтборов = Новый Массив;
	
	ИменаДоступныхГруппОтборов.Добавить("ГруппаОтборовОтчетов");
	ИменаДоступныхГруппОтборов.Добавить("ГруппаОтборовУведомлений");
	
	НастроитьГруппуОтбораФормыРегОтчетности(Элементы, ИменаДоступныхГруппОтборов);
	
	СформироватьПредупрежденияФормыРегОтчетности(Форма);
...
Процедура СформироватьПредупрежденияФормыРегОтчетности(Форма)
	
	Элементы = Форма.Элементы;
	
	ИменаНедоступныхРазделов = Новый Массив;
	
	ИменаНедоступныхРазделов.Добавить("СтраницаПисьма");
	ИменаНедоступныхРазделов.Добавить("СтраницаСверки");
	ИменаНедоступныхРазделов.Добавить("СтраницаЕГРЮЛ");
	ИменаНедоступныхРазделов.Добавить("СтраницаВходящие");
	
	ПорядковыйНомер = 0;
	
	Для каждого ИмяРаздела Из ИменаНедоступныхРазделов Цикл
		ПорядковыйНомер = ПорядковыйНомер + 1;
		Раздел = Элементы.Найти(ИмяРаздела);
		
		Если Раздел = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПервыйЭлементРаздела = Раздел.ПодчиненныеЭлементы.Получить(0);
		
		Предупреждение = МобильныйКлиентБПФормаОбъекта.СоздатьПредупреждениеОбОграничениях(Форма,
			"Сформировать отчетность и прочесть письма от контролирующих органов можно в версии для компьютера", ПорядковыйНомер);
		
		Элементы.Переместить(Предупреждение, Раздел, ПервыйЭлементРаздела);
	КонецЦикла;
	
КонецПроцедуры

Процедура НастроитьГруппуОтбораФормыРегОтчетности(Элементы, ИменаДоступныхГруппОтборов)
	
	Для каждого ИмяГруппыОтборов Из ИменаДоступныхГруппОтборов Цикл
		ГруппаОтборов = Элементы.Найти(ИмяГруппыОтборов);
		
		Если ГруппаОтборов = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ГруппаОтборов.ОтображатьЗаголовок = Истина;
		ГруппаОтборов.Заголовок = "Отборы";
		ГруппаОтборов.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
		ГруппаОтборов.ВажностьПриОтображении = ВажностьПриОтображении.ОченьВысокая;
		ГруппаОтборов.РастягиватьПоГоризонтали = Истина;
		ГруппаОтборов.Скрыть();
	КонецЦикла;
