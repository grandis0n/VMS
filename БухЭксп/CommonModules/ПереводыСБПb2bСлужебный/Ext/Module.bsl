...
// Проверяет права на чтение данных операций Системы быстрых платежей
// в случае отсутствия прав, вызывается исключение.
//
Процедура ПроверитьПраваНаЧтениеОпераций() Экспорт
	
	Если Не ПереводыСБПb2b.ЧтениеПереводовСБПДоступно() Тогда
		ВызватьИсключение НСтр("ru = 'Нарушение прав доступа. Пользователю запрещено чтение данных операций в Системе быстрых платежей.
			|Обратитесь к администратору.'");
	КонецЕсли;
