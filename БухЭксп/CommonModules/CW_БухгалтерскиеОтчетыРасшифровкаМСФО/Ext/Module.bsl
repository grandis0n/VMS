...
//   - Структура см. CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйУниверсальныеНастройки - настройки расшифровки пригодные для чтения стандартным отчетом.
//
Функция НастройкиРасшифровки(АдресРасшифровки, ИдентификаторРасшифровки, ИмяИсходногоОтчета, РеквизитыРасшифровки) Экспорт
	...
	Объект 				= Неопределено;
	Настройки			= CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйУниверсальныеНастройки();
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений см. НовыйПравилаРасшифровки() - Содержит имена отчетов и реквизиты, требуемые для этих отчетов, правилами расшифровки (см. CW_БухгалтерскиеОтчетыРасшифровкаМСФО.НовыйПравилаРасшифровки()).
//
Функция РасшифровочныеОтчеты(ДанныеРасшифровки, ИдентификаторРасшифровки, ИмяОтчета) Экспорт
	
	ПравилаРасшифровки = CW_БухгалтерскиеОтчетыРасшифровкаМСФО.НовыйПравилаРасшифровки();
...
Процедура ДобавитьВНастройкиДанныеРасшифровки(ДанныеРасшифровки, ИдентификаторРасшифровки, Настройки, РеквизитыРасшифровки) Экспорт
	...
	Для Каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
		...
		Если ПолеРасшифровки.Ключ = "Период" Тогда
			...
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПреобразоватьПериодичностьОтчетаВПериод(Настройки, ЗначениеПоляРасшифровки);
			Продолжить;
		...
		Если ОтборПоПолюИмеетСмысл(ПолеРасшифровки.Ключ, ПрименяемыйВидСравнения, Настройки.Отбор) Тогда
			CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(Настройки.Отбор, ПолеРасшифровки.Ключ, ПолеРасшифровки.Значение.Значение, ПрименяемыйВидСравнения);
		КонецЕсли;
...
Процедура ДобавитьВНастройкиДанныеОтчета(Настройки, Отчет) Экспорт
	...
	Если Отчет.Свойство("Группировка", Группировка) Тогда
		...
		Для Каждого СтрокаГруппировки Из Группировка Цикл
			...
			НоваяСтрока = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйСтрокаГруппировки();
	...
	Если Отчет.Свойство("ДополнительныеПоля", ДополнительныеПоля) Тогда
		...
		Для Каждого СтрокаДополнительныеПоля Из ДополнительныеПоля Цикл
			...
			НоваяСтрока = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйСтрокаДополнительныеПоля();
...
//  НовыйСчет			 - ПланыСчетов.CW_МСФО	 - Новый счет.
...
Процедура ПоменятьНумерациюСубконтоВОтборе(Настройки, НовыйСчет, КартаВидовСубконто) Экспорт
	...
	Если ЭлементыОтбораПодЗамену.Количество() > 0 Тогда
		...
		СвойстваСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(НовыйСчет);
		...
		Для Каждого ЭлементЗамены Из ЭлементыОтбораПодЗамену Цикл
			...
			Иначе
				...
				Для Каждого Поле Из ПоляПодходящегоТипа Цикл
					...
					CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ДобавитьОтбор(НоваяГруппа, Поле, ЭлементЗамены.Ключ.ПравоеЗначение,  ЭлементЗамены.Ключ.ВидСравнения);
...
Функция КартаПереходаСубконтоМеждуСчетами(ВидыСубконтоИсходногоСчета, ВидыСубконтоНовогоСчета)
	...
	Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ВидыСубконтоИсходногоСчета, ВидыСубконтоНовогоСчета) Тогда
		...
		МаксимальноеКоличествоСубконто = CW_БухгалтерскийУчетМСФО.МаксимальноеКоличествоСубконто();
...
//
// Параметры:
//  Счет			 - ПланыСчетов.CW_МСФО	 - счет, коллекцию видов субконто которого нужно получить.
//
// Возвращаемое значение:
...
//  ИсходныйСчет		 - ПланыСчетов.CW_МСФО	 - Исходный счет.
//  НовыйСчет			 - ПланыСчетов.CW_МСФО	 - Новый счет.
//  КартаВидовСубконто	 - Соответствие				 - Содержит соответствие старых и новых номеров субконто см. КартаПереходаСубконтоМеждуСчетами().
//
Процедура ПоменятьНумерациюСубконтоВТабличнойЧасти(ИмяТабличнойЧасти, Настройки, ИсходныйСчет, НовыйСчет, КартаВидовСубконто = Неопределено) Экспорт
	...
	Для ИндексСтроки = 0 По ТабличнаяЧасть.ВГраница() Цикл
		...
		Если СтрНачинаетсяС(Путь[0],"Субконто") Тогда
			...
			НомерЗамены = КартаВидовСубконто.Получить(CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НомерСубконто(Путь[0]));
...
//
// Параметры:
//  Настройки		 - Структура	 - Настройки в которых нужно установить новое значение см. CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйУниверсальныеНастройки().
...
Процедура ОбработатьИзменениеНастройки(Настройки, ИмяНастройки, СтароеЗначение, НовоеЗначение)
	...
	Если ИмяНастройки = "Счет" тогда
		...
		Если ТипЗнч(НовоеЗначение) = Тип("ПланСчетовСсылка.CW_МСФО") И СтароеЗначение <> НовоеЗначение Тогда
...
//
// Параметры:
//  Счет	 - ПланСчетовСсылка.CW_МСФО	 - Счет, по свойствам которого нужно сбросить показатели.
//  Настройки - Структура						 - Настройки, в которых нужно установить свойства см. CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйУниверсальныеНастройки().
//
	...
	Для Каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		...
		Иначе
			...
			Если СтрНачинаетсяС(ЛевоеЗначениеПуть[0],"Субконто") Тогда
				...
				НомерЗамены = КартаВидовСубконто.Получить(CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НомерСубконто(ЛевоеЗначениеПуть[0]));
