...
Процедура УстановитьРежимКомандногоИнтерфейса(Режим) Экспорт
	...
	Если Не РежимСоответствуетТекущему Тогда
		УведомленияКлиента.ОтправитьУведомление("ИзменилсяКомандныйИнтерфейс", Неопределено);
	КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ПоказатьОбучение = Не ЭтоНачалоРаботы 
		И Обработки.ОбучениеРаботеВБизнесСтарт.ПоказыватьПриЗапуске();
	Параметры.Вставить("ПоказатьОбучение", ПоказатьОбучение);
	...
	Если ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНеразрешенныйРаздельныйУчетНДС() Тогда
		Параметры.Вставить("ИспользуетсяНеразрешенныйРаздельныйУчетНДС", Истина);
	КонецЕсли;
...
// Проверяет наличие у текущего пользователя указанного права доступа к объекту метаданных.
//
// Параметры:
//   ИмяОбъектаМетаданных - Строка - полное имя объекта метаданных, например "Справочник.Номенклатура"
//   Право - Строка - имя права доступа, например "Чтение", "Просмотр", "ИнтерактивноеДобавление"
//
// Возвращаемое значение:
//   Булево - Истина если право доступно, Ложь если право недоступно или объект не найден
Функция ЕстьПравоДоступаКОбъекту(ИмяОбъектаМетаданных, Право) Экспорт
	
	ЕстьПравоДоступаКОбъекту = Ложь;
	
	ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяОбъектаМетаданных);
	Если ОбъектМетаданных <> Неопределено Тогда
		ЕстьПравоДоступаКОбъекту = ПравоДоступа(Право, ОбъектМетаданных);
	КонецЕсли;
	
	Возврат ЕстьПравоДоступаКОбъекту;
...
// Возвращает массив имен конфигураций, относящихся к Бухгалтерии предприятия и ее производным.
// Может использоваться для проверки принадлежности приложения к данному виду.
//
// Возвращаемое значение:
//  Массив - массив строк с именами конфигураций.
//
Функция ИменаКонфигурацийБП() Экспорт
	
	Имена = Новый Массив;
	Имена.Добавить("БухгалтерияПредприятия");
	Имена.Добавить("БухгалтерияПредприятияКОРП");
	Имена.Добавить("БухгалтерияПредприятияУчебная");
	Имена.Добавить("БухгалтерияПредприятияБазовая");
	Имена.Добавить("БухгалтерияПредприятияБазовая1");
	Имена.Добавить("БухгалтерияНекоммерческойОрганизации");
	Имена.Добавить("БухгалтерияНекоммерческойОрганизацииКОРП");
	Имена.Добавить("БухгалтерияНекоммерческойОрганизацииБазовая");
	Имена.Добавить("Садовод");
	Имена.Добавить("СадоводБазовая");
	Имена.Добавить("Гаражи");
