...
Процедура ДатаПриИзмененииСервер(Форма) Экспорт
	...
	Если (Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
		...
		Если ПриходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
			...
			ИначеЕсли ЗначениеЗаполнено(Форма.СвойстваПлатежа.СтавкаНДС) Тогда
				СтрокаПлатеж.СтавкаНДС = Форма.СвойстваПлатежа.СтавкаНДС;
			КонецЕсли;
			ПриходныйКассовыйОрдерФормыКлиентСервер.ПересчитатьСуммуНДС(СтрокаПлатеж);
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	Если ПриходныйКассовыйОрдерФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		...
		Элементы.РасшифровкаПлатежаСчетНаОплатуНПД.Видимость = ВидимостьУслугиНПД;
		Элементы.РасшифровкаПлатежаСчетНаОплату.Видимость = Не ВидимостьУслугиНПД;
	...
	Иначе
		...
		Если ПриходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
			...
			ЕстьУслугаНПД = Не ЗначениеЗаполнено(СтрокаПлатеж.СчетНаОплату)
				Или ЧекСформирован И ЗначениеЗаполнено(СтрокаПлатеж.УслугаНПД);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, "УслугаНПД", "Видимость",
				ВидимостьУслугиНПД И Не НесколькоСтрокВРасшифровке И ЕстьУслугаНПД);
		Иначе
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, "РасшифровкаПлатежаСчетНаОплатуНПД", "Видимость", ВидимостьУслугиНПД);
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость(Форма, ФормаДокументаОднострочная)
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаСчетНаОплатуНПД");
	...
	ЭлементУО = Форма.УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаУслугаНПД");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
		"ЕстьУслугиНПД",
		ВидСравненияКомпоновкиДанных.Равно,
...
Процедура УстановитьУсловноеОформлениеРасшифровкаПлатежа(Форма, НачалоПутиКДаннымТЧ) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ГруппаОтбораИ,
		НачалоПутиКДаннымТЧ + "РасшифровкаПлатежа.УслугаНПД",
		ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ГруппаОтбораИ,
		НачалоПутиКДаннымТЧ + "РасшифровкаПлатежа.СчетНаОплату",
