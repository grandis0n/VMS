...
Функция СоответствиеТитуловИШагов(Документ = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору")
		Или Документ = Неопределено Тогда
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1"), НСтр("ru='Оформление'"));
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2"), НСтр("ru='Подтверждение'"));
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3"), НСтр("ru='Отзыв'"));
	КонецЕсли; 
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка")
		Или Документ = Неопределено Тогда
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1"), НСтр("ru='Оформление'"));
	КонецЕсли;
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка")
		Или Документ = Неопределено Тогда
		СоответствиеТитулов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1"), НСтр("ru='Оформление'"));
	КонецЕсли;
...
Функция ПрефиксыТитуловДокумента(ИмяДокумента = Неопределено) Экспорт
	...
	Если ИмяДокумента = "Документ.ЭлектронноеПоручениеЭкспедитору"
	Или Не ЗначениеЗаполнено(ИмяДокумента) Тогда 
		МассивПрефиксов.Добавить("ТитулЗаказчикаОтзыв");
		МассивПрефиксов.Добавить("ТитулЗаказчика");
		МассивПрефиксов.Добавить("ТитулЭкспедитора");  
	КонецЕсли;
	
	Если ИмяДокумента = "Документ.ЭлектроннаяЭкспедиторскаяРасписка"
	Или Не ЗначениеЗаполнено(ИмяДокумента) Тогда 
		МассивПрефиксов.Добавить("ТитулЭкспедитора");  
	КонецЕсли;
	
	Если ИмяДокумента = "Документ.ЭлектроннаяCкладскаяРасписка"
	Или Не ЗначениеЗаполнено(ИмяДокумента) Тогда 
		МассивПрефиксов.Добавить("ТитулЭкспедитора");  
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовЭСВ(Форма) Экспорт
	...
	Элементы.ГруппаДемоРежим.Видимость = ОбменСГИСЭПДВызовСервера.ДемоРежимВключен();
...
Процедура УстановитьДоступностьЭлементовЭПЭ(Форма) Экспорт
	...
	ВводимЛюбойДопТитул = Форма.ВводимТитулОтзывПоручения;
	...
	Если ДанныеСостоянияЭДО.ПредставлениеСостояния = "Закрыт с отклонением" Тогда
		Объект.ТекущийТитул = Неопределено;
	Иначе
		УстановитьТекущийДоступныйТитулЭПЭ(Объект, ТитулыПоДокументу, ВводимЛюбойДопТитул,ТребуетсяПовторно);
	КонецЕсли;
	
	// По имеющимся титулам
	Элементы.СтраницаТитулЗаказчика.ТолькоПросмотр = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1")) <> Неопределено;	
	Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2")) <> Неопределено;   
	Элементы.СтраницаТитулЗаказчикаОтзыв.ТолькоПросмотр = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3")) <> Неопределено;

	// Документ еще не отправлялся - дадим редактировать
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1") Тогда
		Элементы.СтраницаТитулЗаказчика.ТолькоПросмотр = Ложь;
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2") Тогда
		Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = Ложь;
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3") Тогда
		Элементы.СтраницаТитулЗаказчикаОтзыв.ТолькоПросмотр = Ложь;
	КонецЕсли;
	
	Элементы.ФормаСоздатьТитулОтзыва.Видимость = Ложь; 
	
	Если Объект.РольУчастника = 3
		И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2")) <> Неопределено
		И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3")) = Неопределено 
		И ВводимЛюбойДопТитул = Ложь
		Тогда
		Элементы.ФормаСоздатьТитулОтзыва.Видимость = Истина;	
	КонецЕсли;
	...
	Элементы.ФормаОтправитьЧерезЭДО.Доступность = ЗначениеЗаполнено(Объект.ТекущийТитул) И ДоступноИзменение;
	Элементы.ФормаОтправитьЧерезЭДОПовторно.Доступность = ДоступноИзменение;
	Элементы.ФормаОтправитьЧерезЭДОПовторно.Видимость = Ложь;
		
КонецПроцедуры 

Процедура УстановитьТекущийДоступныйТитулЭПЭ(Объект, ТитулыПоДокументу = Неопределено, ВводимЛюбойДопТитул = Ложь, ТребуетсяПовторно = Ложь) Экспорт
	...
	// Экспедитор
	Если Объект.РольУчастника = 1 Тогда	
		Если ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1")))
			И (ТребуетсяПовторно Или ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2"))) = Ложь) Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2");
		КонецЕсли;					
	// Заказчик
	ИначеЕсли Объект.РольУчастника = 3 Тогда
	
		Если ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1"))) = Ложь Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1");	
		ИначеЕсли ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1")))
				И ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2")))
				И ВводимЛюбойДопТитул Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3");
		КонецЕсли;
	КонецЕсли;	 
...
Процедура УстановитьДоступностьЭлементовЭЭР(Форма) Экспорт
	...
	Если ДанныеСостоянияЭДО.ПредставлениеСостояния = "Закрыт с отклонением" Тогда
		Объект.ТекущийТитул = Неопределено;
	Иначе
		УстановитьТекущийДоступныйТитулЭЭР(Объект, ТитулыПоДокументу, ТребуетсяПовторно);
	КонецЕсли;
	
	// По имеющимся титулам
	Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1")) <> Неопределено;	

	// Документ еще не отправлялся - дадим редактировать
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1") Тогда
		Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура УстановитьТекущийДоступныйТитулЭЭР(Объект, ТитулыПоДокументу = Неопределено, ТребуетсяПовторно = Ложь) Экспорт
	...
	Если Объект.РольУчастника = 3 Тогда
		...
		Если ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1"))) = Ложь Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1");	
		КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовЭСР(Форма) Экспорт
	...
	Если ДанныеСостоянияЭДО.ПредставлениеСостояния = "Закрыт с отклонением" Тогда
		Объект.ТекущийТитул = Неопределено;
	Иначе
		УстановитьТекущийДоступныйТитулЭСР(Объект, ТитулыПоДокументу, ТребуетсяПовторно);
	КонецЕсли;
	
	// По имеющимся титулам
	Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1")) <> Неопределено;	

	// Документ еще не отправлялся - дадим редактировать
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1") Тогда
		Элементы.СтраницаТитулЭкспедитора.ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура УстановитьТекущийДоступныйТитулЭСР(Объект, ТитулыПоДокументу = Неопределено, ТребуетсяПовторно = Ложь) Экспорт
	...
	Если Объект.РольУчастника = 3 Тогда
		...
		Если ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1"))) = Ложь Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1");	
		КонецЕсли;
...
Функция АдресатыПоИдФайл(ИдФайл) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ИдФайл, "ON_POREXPKLT") Тогда
		Результат.Грузоотправитель = ВРег(МассивЧастейИдФайла[3]);
		Результат.Грузоперевозчик = ВРег(МассивЧастейИдФайла[2]);
		Результат.ЕстьИные = МассивЧастейИдФайла[4] = "1";
	ИначеЕсли СтрНачинаетсяС(ИдФайл, "ON_POREXPEXP") Тогда
		Результат.Грузоотправитель = ВРег(МассивЧастейИдФайла[2]);
		Результат.Грузоперевозчик = ВРег(МассивЧастейИдФайла[3]);
	ИначеЕсли СтрНачинаетсяС(ИдФайл, "ON_POREXPOTZ") Тогда
		Результат.Грузоотправитель = ВРег(МассивЧастейИдФайла[3]);
		Результат.Грузоперевозчик = ВРег(МассивЧастейИдФайла[2]);
		
	// Электронная экспедиторская расписка
	ИначеЕсли СтрНачинаетсяС(ИдФайл, "ON_EXPRASP") Тогда
		Результат.Грузоотправитель = ВРег(МассивЧастейИдФайла[3]);
		Результат.Грузоперевозчик = ВРег(МассивЧастейИдФайла[2]);
		Результат.ЕстьИные = МассивЧастейИдФайла[4] = "1";
		
	// Электронная складская расписка
	ИначеЕсли СтрНачинаетсяС(ИдФайл, "ON_SKLADRASP") Тогда
		Результат.Грузоотправитель = ВРег(МассивЧастейИдФайла[3]);
		Результат.Грузоперевозчик = ВРег(МассивЧастейИдФайла[2]);
		Результат.ЕстьИные = МассивЧастейИдФайла[4] = "1";
...
Функция ПрефиксТитулаПоЭлементуРегламентаЭДО(ЭлементРегламента) Экспорт
	...
	Если ЗначениеЗаполнено(ЭлементРегламента) Тогда
		...
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1") Тогда
			Результат.ВПрограмме = "ТитулЗаказчика";
			Результат.ДляФайла = "ON_POREXPKLT";
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдПЕР_ИдГО_НалДр_GGGGMMDD_ГУИДФайла";
			Результат.КНД = "1110486";
			Результат.ПоФактХЖ = "";
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2") Тогда
			Результат.ВПрограмме = "ТитулЭкспедитора";
			Результат.ДляФайла = "ON_POREXPEXP";
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдГО_ИдПЕР_НалДр_GGGGMMDD_ГУИДФайла";
			Результат.КНД = "1110487";
			Результат.ПоФактХЖ = "";
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3") Тогда
			Результат.ВПрограмме = "ТитулЗаказчикаОтзыв";
			Результат.ДляФайла = "ON_POREXPOTZ";
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдПЕР_ИдГО_НалДр_GGGGMMDD_ГУИДФайла";
			Результат.КНД = "1110488";
			Результат.ПоФактХЖ = "";
			
		//Экспедиторская расписка
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1") Тогда
			Результат.ВПрограмме = "ТитулЭкспедитора";
			Результат.ДляФайла = "ON_EXPRASP";
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдГО_ИдПЕР_НалДр_GGGGMMDD_ГУИДФайла";
			Результат.КНД = "1110489";
			Результат.ПоФактХЖ = "";
			
		//Складская расписка
		ИначеЕсли ЭлементРегламента = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1") Тогда
			Результат.ВПрограмме = "ТитулЭкспедитора";
			Результат.ДляФайла = "ON_SKLADRASP";
			Результат.ПолеУИД = Результат.ВПрограмме + "УИДМинтранс";
			Результат.ШаблонИмениФайла = Результат.ДляФайла + "_ИдГО_ИдПЕР_НалДр_GGGGMMDD_ГУИДФайла";
			Результат.КНД = "1110490";
			Результат.ПоФактХЖ = "";
...
Функция МассивСтрокТаблицыИзСтруктурыРеквизитов(СтруктураРеквизитов, ИмяТаблицы) Экспорт
	...
	Для Каждого КиЗ Из СтруктураРеквизитов Цикл
		Если СтрНачинаетсяС(КиЗ.Ключ, НачалоКлюча) Тогда
			...
			СтруктураСтроки = СоответствиеСтрок.Получить(НомерСтрокиТаблицы); 
			Если СтруктураСтроки = Неопределено Тогда
				СтруктураСтроки = Новый Структура;
				СоответствиеСтрок.Вставить(НомерСтрокиТаблицы, СтруктураСтроки);	
			КонецЕсли;					
	...
	Для каждого Ключ Из СоответствиеСтрок Цикл
		МассивКлючей.Добавить(Ключ.Ключ);
	КонецЦикла;
	
	СписокЗначений = Новый СписокЗначений;
	СписокЗначений.ЗагрузитьЗначения(МассивКлючей);
	СписокЗначений.СортироватьПоЗначению(); 
	МассивКлючей = СписокЗначений.ВыгрузитьЗначения(); 
	
	Результат = Новый Массив;
	Для каждого НомерСтроки Из МассивКлючей Цикл
		Результат.Добавить(СоответствиеСтрок.Получить(НомерСтроки));
	КонецЦикла;
...
Процедура ИзменитьОформлениеФормы(Форма, ИмяЭлемента = Неопределено) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектронноеПоручениеЭкспедитору.Форма.ОсновнаяФорма" Тогда
		// Страница "Оформление"
		Если ЗначениеЗаполнено(Форма.Объект.СсылкаТитулЗаказчикаСоставитель) Тогда
			Форма.ВариантПоДоверенностиЗаЗаказчика = 1;	
		ИначеЕсли ИмяЭлемента <> "ВариантПоДоверенностиЗаЗаказчика" Тогда
			Форма.ВариантПоДоверенностиЗаЗаказчика = 0;
		КонецЕсли;
		
		Если Форма.ВариантПоДоверенностиЗаЗаказчика = 1 Тогда
			Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаСоставитель.Видимость = Истина;
			Элементы.ГруппаРеквизитаЗаполнитьТитулЗаказчикаСоставительДоверенность.Видимость = Истина;
			Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаЗаказчик.ЦветФона = Новый Цвет;
			Если Форма.Объект.РольУчастника = 3 Тогда
				Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаСоставитель.ЦветФона = ЦветВыделенияУчастникаЭПД;
			КонецЕсли;
		Иначе
			Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаСоставитель.Видимость = Ложь;
			Элементы.ГруппаРеквизитаЗаполнитьТитулЗаказчикаСоставительДоверенность.Видимость = Ложь;
			Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаСоставитель.ЦветФона = Новый Цвет;
			Если Форма.Объект.РольУчастника = 3 Тогда
				Элементы.ГруппаРеквизитаСсылкаТитулЗаказчикаЗаказчик.ЦветФона = ЦветВыделенияУчастникаЭПД;	
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектронноеПоручениеЭкспедитору.Форма.ТитулЭкспедитораДопИнфЭкспСвОплата" Тогда
		
		СвОплатаНалСт = Форма.СвОплатаНалСт;
		
		Если СвОплатаНалСт = "без НДС" Тогда
			
			Форма.СвОплатаБезНДС = "без НДС";
			Форма.СвОплатаСумНал = Неопределено;
			
		Иначе 
			
			Форма.СвОплатаБезНДС = Неопределено;
		
			СвОплатаОбщСтУсл = Форма.СвОплатаОбщСтУсл;
						
			Форма.СвОплатаСумНал =  ИзвлечьНДСИзСуммы(СвОплатаОбщСтУсл, СвОплатаНалСт);
				
		КонецЕсли; 
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектроннаяЭкспедиторскаяРасписка.Форма.ОсновнаяФорма"
		Или Форма.ИмяФормы = "Документ.ЭлектроннаяCкладскаяРасписка.Форма.ОсновнаяФорма" Тогда
		
		// Страница "Оформление"
		Если ЗначениеЗаполнено(Форма.Объект.СсылкаТитулЭкспедитораСоставитель) Тогда
			Форма.ВариантПоДоверенностиЗаЭкспедитора = 1;	
		ИначеЕсли ИмяЭлемента <> "ВариантПоДоверенностиЗаЭкспедитора" Тогда
			Форма.ВариантПоДоверенностиЗаЭкспедитора = 0;
		КонецЕсли;
		
		Если Форма.ВариантПоДоверенностиЗаЭкспедитора = 1 Тогда
			Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораСоставитель.Видимость = Истина;
			Элементы.ГруппаРеквизитаЗаполнитьТитулЭкспедитораСоставительДоверенность.Видимость = Истина;
			Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораЭкспедитор.ЦветФона = Новый Цвет;
			Если Форма.Объект.РольУчастника = 3 Тогда
				Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораСоставитель.ЦветФона = ЦветВыделенияУчастникаЭПД;
			КонецЕсли;
		Иначе
			Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораСоставитель.Видимость = Ложь;
			Элементы.ГруппаРеквизитаЗаполнитьТитулЭкспедитораСоставительДоверенность.Видимость = Ложь;
			Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораСоставитель.ЦветФона = Новый Цвет;
			Если Форма.Объект.РольУчастника = 3 Тогда
				Элементы.ГруппаРеквизитаСсылкаТитулЭкспедитораЭкспедитор.ЦветФона = ЦветВыделенияУчастникаЭПД;	
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектроннаяЭкспедиторскаяРасписка.Форма.ТитулЭкспедитораВладИнфр" Тогда
		Если ЗначениеЗаполнено(Форма.ТитулЭкспедитораВладИнфрСовпГОВ) = Ложь Тогда
			Форма.ТитулЭкспедитораВладИнфрСовпГОВ = "1";
		КонецЕсли;
		
		Элементы.СсылкаТитулЭкспедитораВладИнфр.Доступность = Форма.ТитулЭкспедитораВладИнфрСовпГОВ = "2";		
		Элементы.ТитулЭкспедитораВладИнфрОбНетИнфОВлад.Доступность = Форма.ТитулЭкспедитораВладИнфрСовпГОВ = "3";
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектроннаяЭкспедиторскаяРасписка.Форма.ТитулЭкспедитораСвЛицПогрГр" Тогда
		
		Если ЗначениеЗаполнено(Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз) = Ложь Тогда
			Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз = "1";
		КонецЕсли;
		
		Элементы.ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоДата.Доступность = Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз <> "1";		
		Элементы.ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоДополнительныеСведения.Доступность = Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз  <> "1"; 
		Элементы.ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоНаименование.Доступность = Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз  <> "1";
		Элементы.ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоНомер.Доступность = Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз  <> "1"; 
		Элементы.ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоРеквизитыСторон.Доступность = Форма.ТитулЭкспедитораСвЛицПогрГрЛицоПогруз  <> "1";
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектроннаяЭкспедиторскаяРасписка.Форма.ТитулЭкспедитораСвЛицПриемГр" Тогда
		
		СсылкаЗаполнена = ЗначениеЗаполнено(Форма.СсылкаТитулЭкспедитораСвЛицПриемГр);
		
		Элементы.ТитулЭкспедитораОснДовПриемГрНаименование.Доступность = СсылкаЗаполнена;		
		Элементы.ТитулЭкспедитораОснДовПриемГрНомер.Доступность = СсылкаЗаполнена; 
		Элементы.ТитулЭкспедитораОснДовПриемГрДата.Доступность = СсылкаЗаполнена;
		Элементы.ТитулЭкспедитораОснДовПриемГрДополнительныеСведения.Доступность = СсылкаЗаполнена; 
		Элементы.ТитулЭкспедитораОснДовПриемГрРеквизитыСторон.Доступность = СсылкаЗаполнена;
		
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектроннаяCкладскаяРасписка.Форма.ТитулЭкспедитораСведСклад" Тогда
		
		Если ЗначениеЗаполнено(Форма.ТитулЭкспедитораСведСкладВладСкладЭксп) = Ложь Тогда
			Форма.ТитулЭкспедитораСведСкладВладСкладЭксп = "1";
		КонецЕсли;
				
		Элементы.СсылкаТитулЭкспедитораСведСкладСвВладСклад.Доступность = Форма.ТитулЭкспедитораСведСкладВладСкладЭксп = "0";		

	КонецЕсли;
	 
КонецПроцедуры

Функция ИзвлечьНДСИзСуммы(СуммаСНДС, СтавкаСтрокой)
	...
	ИначеЕсли СтавкаСтрокой = "5%" Тогда
		...
		Результат = СуммаСНДС * 5 / 105;
		
	ИначеЕсли СтавкаСтрокой = "7%" Тогда
		
		Результат = СуммаСНДС * 7 / 107;
		
	ИначеЕсли СтавкаСтрокой = "22%" Тогда
		
		Результат = СуммаСНДС * 22 / 122;
...
Функция ТитулПоПрефиксу(ТипДокумента, ПрефиксТитула, ИндексТитула = 1) Экспорт
	...
	// ЭПЭ
	ИначеЕсли ТипДокумента = "Документ.ЭлектронноеПоручениеЭкспедитору" Тогда
		Если ПрефиксТитула = "ТитулЗаказчика" Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1");
		ИначеЕсли ПрефиксТитула = "ТитулЭкспедитора" Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2");
		ИначеЕсли ПрефиксТитула = "ТитулЗаказчикаОтзыв" Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3");
		КонецЕсли;
		
	//ЭЭР	
	ИначеЕсли ТипДокумента = "Документ.ЭлектроннаяЭкспедиторскаяРасписка" Тогда
		Если ПрефиксТитула = "ТитулЭкспедитора" Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1");
		КонецЕсли; 
		
	//ЭСР	
	ИначеЕсли ТипДокумента = "Документ.ЭлектроннаяCкладскаяРасписка" Тогда
		Если ПрефиксТитула = "ТитулЭкспедитора" Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1");
		КонецЕсли;
...
Процедура ЗаполнитьРеквизитыУчастника(Участник, ПрефиксРеквизитов, КонтейнерРеквизитов, Форма, ПрефиксТитула = Неопределено, Параметры = Неопределено) Экспорт
	...
	РеквизитыУчастника = МодульОбменСГИСЭПД.ПолучитьРеквизитыУчастника(Участник, Ложь, Параметры);
...
Процедура ЗаполнитьРеквизитыСтраны(Страна, ПрефиксРеквизитов, КонтейнерРеквизитов) Экспорт

	РеквизитыСтраны = ОбменСГИСЭПДВызовСервера.ЗначенияРеквизитовОбъекта(Страна, "Код, Наименование");
	
	ИмяКодСтраны = СтрЗаменить(ПрефиксРеквизитов, "Страна", "КодСтраны");
	ИмяНаименованиеСтраны = СтрЗаменить(ПрефиксРеквизитов, "Страна", "НаименованиеСтраны");
	Если ТипЗнч(КонтейнерРеквизитов) = Тип("Структура") Тогда
		КонтейнерРеквизитов.Вставить(ИмяКодСтраны, РеквизитыСтраны.Код);
		КонтейнерРеквизитов.Вставить(ИмяНаименованиеСтраны, РеквизитыСтраны.Наименование);
	Иначе
		КонтейнерРеквизитов[ИмяКодСтраны] = РеквизитыСтраны.Код;			
		КонтейнерРеквизитов[ИмяНаименованиеСтраны] = РеквизитыСтраны.Наименование;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьРеквизитыАдреса(Форма, Результат, ИмяЭлемента, ТекущиеДанные = Неопределено, Параметры = Неопределено) Экспорт
	...
	Если КонтейнерРеквизитов <> Неопределено Тогда
		...
		ЭтоАдресЭПДВСвободнойФорме = ЗначениеЗаполнено(СтруктураАдреса.Территория)
			Или (Не РаботаСАдресамиКлиентСервер.ЭтоАдминистративноТерриториальныйАдрес(СтруктураАдреса.ТипАдреса)
				 И Не РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(СтруктураАдреса.ТипАдреса))
			Или (СтруктураАдреса.Здание.ТипЗдания <> "Дом")
			Или (СтруктураАдреса.Корпуса.Количество() > 0
				 И СтруктураАдреса.Корпуса[0].ТипКорпуса <> "Корпус")
			Или (СтруктураАдреса.Помещения.Количество() > 0
				 И СтруктураАдреса.Помещения[0].ТипПомещения <> "Квартира"); 
		...
		Если ЭтоАдресЭПДВСвободнойФорме Тогда
			Если СтруктураВерсииТитула <> Неопределено Тогда
				...
				СтруктураВерсииТитула.Вставить(Префикс + "Страна", СтруктураАдреса.Страна);
			Иначе
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтейнерРеквизитов, Префикс + "Страна") Тогда
					КонтейнерРеквизитов[Префикс + "Страна"] = СтруктураАдреса.Страна;
				КонецЕсли;
			КонецЕсли;
		Иначе	
			Если СтруктураВерсииТитула <> Неопределено Тогда
				...
				СтруктураВерсииТитула.Вставить(Префикс + "Регион", СтруктураАдреса.Регион);
				...
				КонецЕсли;
				Если СтруктураАдреса.Помещения.Количество() > 0 Тогда
					СтруктураВерсииТитула.Вставить(Префикс + "Квартира", СтруктураАдреса.Помещения[0].Номер);
				КонецЕсли;
			...
			Иначе
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтейнерРеквизитов, Префикс + "Регион") Тогда
					КонтейнерРеквизитов[Префикс + "Регион"] = СтруктураАдреса.Регион;
				КонецЕсли;
				...
				КонецЕсли;
				Если СтруктураАдреса.Помещения.Количество() > 0 Тогда
					КонтейнерРеквизитов[Префикс + "Квартира"] = СтруктураАдреса.Помещения[0].Номер;
				КонецЕсли;
...
Процедура ЗаполнитьРеквизитыДоговора(ДоговорСсылка, ПрефиксРеквизитов, КонтейнерРеквизитов) Экспорт 
	...
	РеквизитыДоговора = МодульОбменСГИСЭПД.ПолучитьРеквизитыДоговора(ДоговорСсылка); 
	
	Если ТипЗнч(КонтейнерРеквизитов) = Тип("Структура") Тогда
		КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Наименование", РеквизитыДоговора.Наименование);
		КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Номер", РеквизитыДоговора.Номер);
		КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Дата", РеквизитыДоговора.Дата);
	Иначе
		КонтейнерРеквизитов[ПрефиксРеквизитов + "Наименование"] = РеквизитыДоговора.Наименование;			
		КонтейнерРеквизитов[ПрефиксРеквизитов + "Номер"] = РеквизитыДоговора.Номер;
		КонтейнерРеквизитов[ПрефиксРеквизитов + "Дата"] = РеквизитыДоговора.Дата;
	КонецЕсли;
...
Процедура ОчиститьАдрес(КонтейнерРеквизитов, Префикс)
	...
	Если ТипЗнч(КонтейнерРеквизитов) = Тип("Структура") Тогда
		...
		КонтейнерРеквизитов.Удалить(Префикс + "Регион");
		...
		КонтейнерРеквизитов.Удалить(Префикс + "Квартира");
		КонтейнерРеквизитов.Удалить(Префикс + "ИныеСвед");
	Иначе
		КонтейнерРеквизитов[Префикс + "Индекс"] = Неопределено;
		КонтейнерРеквизитов[Префикс + "КодРегиона"] = Неопределено;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтейнерРеквизитов, Префикс + "Регион") Тогда
			КонтейнерРеквизитов[Префикс + "Регион"] = Неопределено; 
		КонецЕсли;
		...
		КонтейнерРеквизитов[Префикс + "Квартира"] = Неопределено;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтейнерРеквизитов, Префикс + "ИныеСведения") Тогда
			КонтейнерРеквизитов[Префикс + "ИныеСведения"] = Неопределено; 
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоДоговорЭПД(ТипСсылки)
	
	#Если Сервер Или ВнешнееСоединение Тогда
		Возврат ОбменСГИСЭПД.ЭтоДоговорЭПД(ТипСсылки);	
	#Иначе
		Возврат ОбменСГИСЭПДВызовСервера.ЭтоДоговорЭПД(ТипСсылки);
	#КонецЕсли
	
КонецФункции

Процедура ЗаполнитьРеквизитыПоСсылке(Элемент, Форма, ТекущиеДанные = Неопределено, ПрефиксТитула = Неопределено, Параметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ЗначениеРеквизита) = ПолучитьТипОрганизация()
		...
		ЗаполнитьРеквизитыУчастника(ЗначениеРеквизита, ПрефиксРеквизитов, КонтейнерРеквизитов, Форма, ПрефиксТитула, Параметры);
	...
	ИначеЕсли ТипЗнч(ЗначениеРеквизита) = Тип(ОбменСГИСЭПДВызовСервера.ИмяТипаСтраны()) Тогда
		ЗаполнитьРеквизитыСтраны(ЗначениеРеквизита, ГруппаДанных, КонтейнерРеквизитов);
	...
	ИначеЕсли ТипЗнч(ЗначениеРеквизита) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		...
		КонецЕсли;	
		ЗаполнитьРеквизитыФизическогоЛица(ЗначениеРеквизита, ГруппаДанных, КонтейнерРеквизитов, Организация);
		
	ИначеЕсли ЭтоДоговорЭПД(ТипЗнч(ЗначениеРеквизита)) Тогда
        ЗаполнитьРеквизитыДоговора(ЗначениеРеквизита, ГруппаДанных, КонтейнерРеквизитов);
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ЗначениеРеквизита) = Тип("СправочникСсылка.ФизическиеЛица") Тогда 
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Объект") Тогда
			Организация = Форма.Объект.Организация
		КонецЕсли;
		ПоказатьОшибкиПоФизЛицу(Форма, Элемент, ЗначениеРеквизита, Организация);	
	КонецЕсли;
...
Функция ОписаниеГруппыДанных(Знач ГруппаДанных, Знач ИмяОбъекта) Экспорт
	...
	ИначеЕсли ГруппаДанных = "СторонаПодписавшаяДокумент" Тогда
		...
		Если ИмяОбъектаДляФормы = "Документ.ЭлектронноеПоручениеЭкспедитору"
		Или ЗначениеЗаполнено(ИмяОбъектаДляФормы) = Ложь Тогда
			СписокТаблиц = ?(СписокТаблиц = "", "", ",
							|") + "ТитулЗаказчикаСоставительДоверенностьРеквизитыСторон,
							|ТитулЗаказчикаДоговорРеквизитыСторон,
							|ТитулЗаказчикаРеквизитыСторонЗаявокНаТранспортировку,
							|ТитулЗаказчикаРеквизитыСторонСопроводительныхДокументов,
							|ТитулЗаказчикаГрузыРеквизитыСторонСопрДокПарт,
							|ТитулЗаказчикаГрузыРеквизитыСторонРеквЗакПост,
							|ТитулЗаказчикаГрузыРеквизитыСторонСвСтрахПолис,
							|ТитулЭкспедитораДопИнфЭкспОснОкУслИнымРеквИдРекСост,
							|ТитулЭкспедитораДопИнфЭкспРеквЗакПостРеквИдРекСост,
							|ТитулЭкспедитораДопИнфЭкспСвОплатаДокОплУслРеквИдРекСост,
							|ТитулЭкспедитораДопИнфЭкспСвТрсГрузСвТСДостСвВодитПутевойЛистРеквИдРекСост";
			
		КонецЕсли;
		
		Если ИмяОбъектаДляФормы = "Документ.ЭлектроннаяЭкспедиторскаяРасписка"
		Или ЗначениеЗаполнено(ИмяОбъектаДляФормы) = Ложь Тогда
			СписокТаблиц = ?(СписокТаблиц = "", "", ",
							|") + "ТитулЭкспедитораСоставительДоверенностьРеквизитыСторон,
							|ТитулЭкспедитораДоговорРеквизитыСторон,
							|ТитулЭкспедитораГрузыРеквизитыСторонСопрДокПарт,
							|ТитулЭкспедитораГрузыРеквизитыСторонРеквЗакПост,
							|ТитулЭкспедитораРеквизитыСторонСопроводительныхДокументов,
							|ТитулЭкспедитораОснДовПриемГрРеквизитыСторон,
							|ТитулЭкспедитораСвЛицПогрГрРеквДокЛицоРеквизитыСторон";
		КонецЕсли;
		
		Если ИмяОбъектаДляФормы = "Документ.ЭлектроннаяCкладскаяРасписка"
		Или ЗначениеЗаполнено(ИмяОбъектаДляФормы) = Ложь Тогда
			СписокТаблиц = ?(СписокТаблиц = "", "", ",
							|") + "ТитулЭкспедитораСоставительДоверенностьРеквизитыСторон,
							|ТитулЭкспедитораДоговорРеквизитыСторон,
							|ТитулЭкспедитораГрузУслПриемГрОснДовПриемГрХранРеквизитыСторон,
							|ТитулЭкспедитораГрузРеквЗаказПокРеквизитыСторон,
							|ТитулЭкспедитораГрузРеквизитыСторонСопрДокПарт,
							|ТитулЭкспедитораГрузРеквизитыСторонСвСтрахПолис,
							|ТитулЭкспедитораРеквизитыСторонСопроводительныхДокументов";
	...
	ИначеЕсли ГруппаДанных = "Водитель"
		...
		Если ИмяОбъектаДляФормы = "Документ.ЭлектроннаяТранспортнаяНакладная"
		Или ИмяОбъектаДляФормы = "Документ.ЭлектроннаяСопроводительнаяВедомость"
		Или ИмяОбъектаДляФормы = "Документ.ЭлектронныйЗаказНаряд" 
		Или ИмяОбъектаДляФормы = "Документ.ЭлектронноеПоручениеЭкспедитору" Тогда
			СписокРеквизитов = СписокРеквизитов + "
		...
		Иначе 
			...
			Или ЗначениеЗаполнено(ИмяОбъектаДляФормы) = Ложь Тогда
				СписокТаблиц = СписокТаблиц + ?(СписокТаблиц = "", "", ",
								|") + "ТитулЭкспедитораДопИнфЭкспСвТрсГрузСвТСДостСвВодит";
			КонецЕсли;
...
Функция ПолучитьПрефиксТитулаПоСтраницеОсновнойФормы(ИмяТекущейСтраницы, ТипДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = ТипДокументаПоручениеЭкспедитору() Тогда 
		...
		Если ИмяТекущейСтраницы = "СтраницаТитулЗаказчика" Тогда
			Возврат "ТитулЗаказчика";
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаТитулЭкспедитора" Тогда
			Возврат "ТитулЭкспедитора";
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаТитулЗаказчикаОтзыв" Тогда
			Возврат "ТитулЗаказчикаОтзыв";
		КонецЕсли;
		
	ИначеЕсли ТипДокумента = ТипДокументаЭкспедиторскаяРасписка() Тогда 
	
		Если ИмяТекущейСтраницы = "СтраницаТитулЭкспедитора" Тогда
			Возврат "ТитулЭкспедитора";
		КонецЕсли;
...
// Возвращает текстовое представление типа документа складская расписка.
// 
// Возвращаемое значение:
//  Строка - текстовое представление типа документа.
Функция ТипДокументаСкладскаяРасписка() Экспорт
	
	Возврат "ЭлектроннаяCкладскаяРасписка";
	
КонецФункции

// Возвращает текстовое представление типа документа экспедиторская расписка.
// 
// Возвращаемое значение:
//  Строка - текстовое представление типа документа.
Функция ТипДокументаЭкспедиторскаяРасписка() Экспорт
	
	Возврат "ЭлектроннаяЭкспедиторскаяРасписка";
	
КонецФункции


// Возвращает текстовое представление типа документа поручение экспедитору.
// 
// Возвращаемое значение:
//  Строка - текстовое представление типа документа.
Функция ТипДокументаПоручениеЭкспедитору() Экспорт
