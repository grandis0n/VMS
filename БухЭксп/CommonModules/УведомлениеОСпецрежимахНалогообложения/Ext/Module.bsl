...
Процедура ЗапретитьРедактированиеРеквизитовСданногоОтчета(Форма)
	...
	Если Не ЗапретитьРедактированиеПользователем 
		И Форма.Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ДокументОПредстоящейПоставке Тогда 
			
		ЗапретитьРедактированиеПользователем = (СтрНачинаетсяС(СтатусОтправки, НСтр("ru = 'Зарегистрирован'")))
			Или (СтатусОтправки = НСтр("ru = 'Обеспечен'"))
			Или (СтатусОтправки = НСтр("ru = 'Погашен'"));
	КонецЕсли;
