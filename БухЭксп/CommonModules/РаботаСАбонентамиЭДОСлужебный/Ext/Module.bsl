...
Функция НайтиУчетныеЗаписиАбонентов(Знач Идентификаторы, Знач КПП) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПриПоискеУчетныхЗаписейАбонентов(Идентификаторы, КПП, СписокИдентификаторов);
	КонецЕсли;
...
Функция ПараметрыСвязиИдентификаторов(ПараметрыПолучения, КонтекстДиагностики = Неопределено,
	...
	СоставнойЗапрос = Новый Запрос(
		...
		|	ЕСТЬNULL(АбонентыЭДО.НеТребуетПриглашений, ЛОЖЬ) КАК АбонентНеТребуетПриглашений,
		|	ЕСТЬNULL(АбонентыЭДО.КодыТиповДокументовКОбмену, """") КАК КодыТиповДокументовКОбмену,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВключатьВСписок
			...
			Если ФормироватьСписокВыбора И ТипЗнч(НоваяСтрока.СписокВыбора) = Тип("СписокЗначений") Тогда
				...
				ДанныеИдентификатора.Вставить("АбонентНеТребуетПриглашений", Выборка.АбонентНеТребуетПриглашений);
				ДанныеИдентификатора.Вставить("КодыТиповДокументовКОбмену", Выборка.КодыТиповДокументовКОбмену);
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка,, "АбонентНеТребуетПриглашений");
		НоваяСтрока.ИдентификаторКонтрагентаПредставление = ИдентификаторКонтрагентаПредставление;
		
		АбонентНеТребуетПриглашений = Ложь;
		
		Если Выборка.АбонентНеТребуетПриглашений Тогда
			
			АбонентНеТребуетПриглашений = Истина;
			
			Если ТрансграничныйЭДО.ЭтоИностранныйОператор(Выборка.ИдентификаторКонтрагента) Тогда
				АбонентНеТребуетПриглашений = 
						ТрансграничныйЭДО.ИностранномуКонтрагентуДоступенОбменБезПриглашений(
								Выборка.ИдентификаторКонтрагента);
			ИначеЕсли НоваяСтрока.ТребуетсяСоглашениеПриПервомПодключении 
		...
		НоваяСтрока.АбонентНеТребуетПриглашений = АбонентНеТребуетПриглашений;
		...
		Если ФормироватьПредставлениеСтатусаПриглашения Тогда
			...
			ИначеЕсли АбонентНеТребуетПриглашений Тогда
				
				НоваяСтрока.СтатусПриглашенияПредставление = 
					СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<b>Не требуется</b>'"));
				
			Иначе
				
				НоваяСтрока.СтатусПриглашенияПредставление = 
					СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<b>Не отправлено</b>'"));
