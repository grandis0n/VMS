...
Функция ВидОшибкиНеНастроенаСвязьСКонтрагентом(СтатусПриглашения, ВидОперации) Экспорт
	...
	ИначеЕсли СтатусПриглашения = ПредопределенноеЗначение("Перечисление.СтатусыПриглашений.ТребуетсяОтправить") Тогда 
		...
		Иначе
			...
			ОписаниеРешения = СтрШаблон(ШаблонОписаниеРешения, "приглашение");
		КонецЕсли;
	ИначеЕсли СтатусПриглашения = ПредопределенноеЗначение("Перечисление.СтатусыПриглашений.Отклонено") Тогда
		Идентификатор = "ТребуетсяПовторнаяОтправкаПриглашения" + ВидОперации;
		ОписаниеПроблемы = НСтр("ru = 'Приглашение к обмену электронными документами было отклонено'");
		Если ВидОперации = "Распаковка" Тогда
			ШаблонОписаниеРешения = НСтр("ru = 'Чтобы возобновить обмен электронными документами с контрагентом, отправьте <a href = ""%1"">приглашение</a> повторно.
					   |После того как оно будет принято, транспортный контейнер можно будет распаковать.'");
			ОписаниеРешения = СтрШаблон(ШаблонОписаниеРешения, "приглашение");
		Иначе
			ШаблонОписаниеРешения = НСтр("ru = 'Чтобы возобновить обмен электронными документами с контрагентом, отправьте <a href = ""%1"">приглашение</a> повторно.
