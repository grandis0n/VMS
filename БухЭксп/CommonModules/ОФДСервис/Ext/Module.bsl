...
Функция ПрочитатьДанные_items(Знач ДанныеТоваровИзСервиса)
	...
	ДанныеПоТоварам = Новый Массив;
	НечитаемыеСимволы = НечитаемыеСимволы();
...
Функция УдалитьНечитаемыеСимволы(Знач ИсходнаяСтрока, Знач НечитаемыеСимволы) Экспорт
...
Функция НечитаемыеСимволы()
	...
	Для КодСимвола = 0 По 31 Цикл
		Если КодСимвола = 9 Или КодСимвола = 10 Или КодСимвола = 13 Тогда
			Продолжить;
		КонецЕсли;
		НечитаемыеСимволы.Добавить(Символ(КодСимвола));
	КонецЦикла;
	
	Возврат НечитаемыеСимволы;
