...
//       0 - как файл во временном каталоге.
//       1 - как строка.
//       2 - как двоичные данные.
//       Значение по умолчанию: 0
				...
				Если НЕ ТелоУстановлено Тогда
					...
					Если ПараметрыПолучения.ДанныеДляОбработки <> Неопределено Тогда
						...
						Если ПараметрыПолучения.ФорматДанныхДляОбработки = 0 Тогда
							...
							Если Не ЭтоИмяВременногоФайла(ПараметрыПолучения.ДанныеДляОбработки) Тогда
								ВызватьИсключение НСтр("ru = 'Ошибка в адресе файла для http-запроса.'");
							КонецЕсли;
