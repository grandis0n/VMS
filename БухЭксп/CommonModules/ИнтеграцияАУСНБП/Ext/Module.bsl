...
Процедура ОбработатьОтветНаОтправленныйРасчетНДФЛ(ДокументЗапрос, ДокументОтвет, ДанныеДокумента, БанковскиеОперации) Экспорт
	...
	Если ТипЗнч(ОтветФНС) <> Тип("ОбъектXDTO") Тогда
		Возврат;
	КонецЕсли;
	Для Каждого СтатусОперации Из ОтветФНС.EmployeeInfoResponse Цикл
		СтрокаОперации = БанковскиеОперации.Найти(СтатусОперации.Id, "Идентификатор");
		Если СтрокаОперации = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтрокаОперации.РеквизитыДокумента.ИдентификаторИсходящегоЗапроса = ДокументЗапрос.Идентификатор;
		СтрокаОперации.РеквизитыДокумента.ИдентификаторВходящегоЗапроса = ДокументОтвет.Идентификатор;
		Если ВРег(СтатусОперации.StatusCode) = ВРег(ИнтеграцияАУСНКлиентСервер.ИмяСтатусаПринятФНС()) Тогда
			СтрокаОперации.РеквизитыДокумента.Статус = Перечисления.СтатусыРасчетовНДФЛАУСН.Принят;
		Иначе
			СтрокиОшибок = Новый Массив;
			Для Каждого ОписаниеОшибки Из СтатусОперации.RefusalToAccept Цикл
				СтрокиОшибок.Добавить(ОписаниеОшибки.ReasonDescription);
			КонецЦикла;
			СтрокаОперации.РеквизитыДокумента.Статус = Перечисления.СтатусыРасчетовНДФЛАУСН.Отклонен;
			СтрокаОперации.РеквизитыДокумента.ТекстОшибки = СтрСоединить(СтрокиОшибок, ", ");
		КонецЕсли;
	КонецЦикла;
