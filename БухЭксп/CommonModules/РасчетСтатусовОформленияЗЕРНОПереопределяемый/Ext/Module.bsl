...
Процедура ПриОпределенииТекстаЗапросаДляРасчетаСтатусаОформления(
	...
	ОтборНоменклатуры = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
	
	ОтборНоменклатурыТолькоЗерно = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукцииЗЕРНО)";
	
	ОтборНоменклатурыТолькоЗернопереработка = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО В (&ВидыПродукцииППЗ)";
	...
	Если МетаданныеДокументаОснования = Метаданные.Документы.ОтчетПроизводстваЗаСмену Тогда
		...
		Если МетаданныеДокументаЗЕРНО = Метаданные.Документы.ВнесениеСведенийОСобранномУрожаеЗЕРНО Тогда
			...
			ДополнительныеПараметрыЗапроса.Вставить("ВидыПродукцииЗЕРНО", ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(Перечисления.РазделыУчетаЗЕРНО.Зерно));
		...
		ИначеЕсли  МетаданныеДокументаЗЕРНО = Метаданные.Документы.ФормированиеПартийПриПроизводствеЗЕРНО Тогда
			...
			ДополнительныеПараметрыЗапроса.Вставить("ВидыПродукцииППЗ", ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(Перечисления.РазделыУчетаЗЕРНО.ПродуктыПереработкиЗерна)); 
		КонецЕсли;
...
Функция ТекстЗапросаТоварыДокументаОснования(МетаданныеДокументаОснования,ОписаниеТабличныхЧастей,ОтборНоменклатуры)
	...
	|	ПО %ИмяПоляНоменклатураТЧТовары% = ОписаниеНоменклатурыИС.Номенклатура
	|
	|ГДЕ
