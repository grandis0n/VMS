...
//  ПараметрыУчетныхЗаписей - Неопределено, Соответствие из см. УчетныеЗаписиЭДО.ПараметрыУчетнойЗаписи -
//   параметры учетных записей, которые используются при распаковке контейнеров: Организация, НеИспользоватьПриглашения.
// 	
//  
// Возвращаемое значение:
// 	Структура:
...
Функция СохранитьКонтейнер(ОписаниеКонтейнера, КлючСинхронизации, КонтекстДиагностики, ИмяБезРасширения = "",
	ПараметрыУчетныхЗаписей = Неопределено) Экспорт
	...
	РезультатСохранения.ТранспортныйКонтейнер = СохранитьКонтейнерПоПараметрам(ПараметрыКонтейнера, 
		ОписаниеКонтейнера.ДвоичныеДанные, КонтекстДиагностики, ПараметрыУчетныхЗаписей);
...
Функция СохранитьКонтейнерПоПараметрам(ПараметрыКонтейнера, ДвоичныеДанныеФайла, КонтекстДиагностики,
	ПараметрыУчетныхЗаписей = Неопределено) Экспорт
	...
	Попытка
		...
		ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанныеФайла,
			Неопределено, КонтекстДиагностики,, ПараметрыУчетныхЗаписей);
...
// Возвращаемое значение:
//  Структура:
//  * Карточка - см. НовоеОписаниеКарточкиЭлектронногоДокумента
...
Функция РаспаковатьФайлКонтейнера(Файл, КонтекстДиагностики = Неопределено) Экспорт
	...
	СервисEDIСлужебный.ВыполнитьПоискИдентификаторовДокументооборотаПоИмениФайла(ОписаниеКонтейнера);
...
Процедура ПриФормированииТаблицыТиповЭлементовРегламента(Таблица) Экспорт
	...
	ЧерновикиЭДО.ПриФормированииТаблицыТиповЭлементовРегламента(Таблица);
КонецПроцедуры
...
Процедура ПриФормированииТаблицыТиповРегламентов(Таблица) Экспорт
	...
	ЧерновикиЭДО.ПриФормированииТаблицыТиповРегламентов(Таблица);
КонецПроцедуры

#Область ДляВызоваИзДругихПодсистем

// Возвращает карточку транспортного контейнера оператора ЭДО (card.xml).
// Для обмена через сервер ЕИС используется собственная структура карточки,
// см. ЭлектронноеАктированиеЕИСПолучениеВходящих.НовоеОписаниеКарточкиПакета.
// 
// Возвращаемое значение:
//  Структура:
// * ТипДокумента - Строка
...
// * КППОрганизацииУточненный - Неопределено
//                            - Строка
// * КППКонтрагентаУточненный - Неопределено
//                            - Строка
// * НаименованиеКонтрагентаУточненное - Неопределено
//                                     - Строка
...
// * ФайлНазначения - Строка
// * ДанныеЧерновика - Неопределено
//                   - См. ЧерновикиЭДО.НовыеДанныеЧерновика
// * ДополнительныеПараметры - Структура
Функция НовоеОписаниеКарточкиЭлектронногоДокумента() Экспорт
	...
	Карточка.Вставить("ТипПользователя", "");
	Карточка.Вставить("ФайлНазначения", "");
	Карточка.Вставить("ДанныеЧерновика", Неопределено);
...
Процедура ПриЗагрузкеОбъекта(Объект, ДанныеОбъекта, КонтекстДиагностики) Экспорт
	...
	Если Не ЗначениеЗаполнено(КонтейнерОбъект.Организация) Тогда
		КонтейнерОбъект.Организация = ДанныеОбъекта.Организация;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(КонтейнерОбъект.Контрагент) Тогда
		КонтейнерОбъект.Контрагент = ДанныеОбъекта.Контрагент;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(КонтейнерОбъект.ДоговорКонтрагента) Тогда
		КонтейнерОбъект.ДоговорКонтрагента = ДанныеОбъекта.ДоговорКонтрагента;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(КонтейнерОбъект.Отправитель) Тогда
...
// * Карточка - см. НовоеОписаниеКарточкиЭлектронногоДокумента - карточка оператора ЭДО.
//              Для обмена через сервер ЕИС состав полей свой,
//              см. ЭлектронноеАктированиеЕИСПолучениеВходящих.НовоеОписаниеКарточкиПакета.
// * Ошибка - см. ПрочитатьФайлОшибки
// * Доверенности - см. ПрочитатьКарточкуМЧД
// * ДанныеТЭДО - см. ТрансграничныйЭДО.ДанныеКонтейнера
...
Процедура ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанные, РезультатРаспаковки, КонтекстДиагностики,
	КэшЦиклов = Неопределено, ПараметрыУчетныхЗаписей = Неопределено)
	...
	Иначе
		...
		СоздаватьКонтрагентовАвтоматическиБЭД = НастройкиЭДО.СоздаватьКонтрагентовАвтоматически()
			И НЕ ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(РезультатРаспаковки.Карточка.ИдентификаторОтправителя);
		УстановитьПривилегированныйРежим(Ложь);
	...
	ЗаполнитьКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки, 
		СоздаватьКонтрагентовАвтоматическиБЭД, ПараметрыУчетныхЗаписей);
...
// Читает карточку транспортного контейнера оператора ЭДО из card.xml.
// 
// Параметры:
// 	ИмяФайла - Строка,
//           - ПотокВПамяти
// Возвращаемое значение:
// 	см. НовоеОписаниеКарточкиЭлектронногоДокумента
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	
	ВидОперации = НСтр("ru = 'Чтение карточки электронного документа.'");
	ПодсистемаСобытия = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
	
	Карточка = НовоеОписаниеКарточкиЭлектронногоДокумента();
	...
	Карточка.ДополнительныеПараметры = ДополнительныеПараметры;
	...
	КонецЕсли;
	Если ДополнительныеПараметры.Свойство("UserType") Тогда
		Карточка.ТипПользователя = ДополнительныеПараметры.UserType;
	КонецЕсли;
	
	Если Карточка.ТипДокумента = "Template" Тогда
		ЧерновикиЭДО.ЗаполнитьДанныеЧерновикаПоДаннымДополнительныхПараметровКарточкиДокумента(Карточка.ДанныеЧерновика,
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТранспортныйКонтейнер) Тогда
			Если СинхронизацияЭДОКлиентСервер.ЭтоОбменЧерезОператора(ПараметрыКонтейнера.СпособОбменаЭД)
				Или СинхронизацияЭДО.ЭтоПрямойОбмен(ПараметрыКонтейнера.СпособОбменаЭД)
				Или СервисEDIСлужебный.ЭтоОбменЧерезСервис1СEDI(ПараметрыКонтейнера.СпособОбменаЭД) Тогда
...
// Получить контрагента по данным контейнера.
// 
// Параметры:
//  ПараметрыПриглашения - см. ПриглашенияЭДОКлиентСервер.КлючПриглашения()
//  КлючКонтрагента - см. ПриглашенияЭДО.НовыйКлючУчастникаЭДО()
//  ОбменЧерезОператора - Булево - Обмен через оператора
//  НаименованиеКонтрагента - Строка - Наименование контрагента
//  ИдентификаторОтправителя - Строка - Идентификатор отправителя
//  СоздаватьКонтрагентовАвтоматически - Булево - Создавать контрагентов автоматически
// 
// Возвращаемое значение:
//  Неопределено, ОпределяемыйТип.УчастникЭДО - Получить контрагента по данным контейнера
//
Функция КонтрагентПоДаннымКонтейнера(ПараметрыПриглашения, КлючКонтрагента, ОбменЧерезОператора, 
	НаименованиеКонтрагента, ИдентификаторОтправителя, СоздаватьКонтрагентовАвтоматически)
	...
	Иначе
		ОбменМеждуСобственнымиОрганизациями = СинхронизацияЭДО.ЭтоИнтеркампани(ИдентификаторОтправителя);
	КонецЕсли;
	...
	ТипОбъектаОтправителя = ?(Не ОбменМеждуСобственнымиОрганизациями = Истина, "Контрагенты", "Организации");
	Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП(ТипОбъектаОтправителя, КлючКонтрагента.ИНН, КлючКонтрагента.КПП, 
		КлючКонтрагента.ДатаСведений, КлючКонтрагента.ТипПользователя);
	...
	Если Не ЗначениеЗаполнено(Контрагент)
		...
		ЗаполнитьЗначенияСвойств(РеквизитыКонтрагента, КлючКонтрагента);
		РеквизитыКонтрагента.Вставить("Наименование", НаименованиеКонтрагента);
		ВидКонтрагента = ОбменСКонтрагентамиКлиентСервер.ВидКонтрагента(КлючКонтрагента.ИНН, КлючКонтрагента.ТипПользователя);
...
//   параметры учетных записей, которые используются при распаковке контейнеров: Организация, НеИспользоватьПриглашения.
//
Процедура ЗаполнитьКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки, 
	СоздаватьКонтрагентовАвтоматически, ПараметрыУчетныхЗаписей)
	
	Карточка = РезультатРаспаковки.Карточка;
	Контрагент = Неопределено;
	ДанныеПриглашения = Неопределено;
	
	Если ПараметрыУчетныхЗаписей = Неопределено Тогда
		ПараметрыУчетныхЗаписей = УчетныеЗаписиЭДО.ПараметрыУчетныхЗаписей(ПараметрыКонтейнера.Получатель,
			"Организация, НеИспользоватьПриглашения");
	КонецЕсли;
	
	ПараметрыУчетнойЗаписи = ПараметрыУчетныхЗаписей[ПараметрыКонтейнера.Получатель];
	...
	КлючКонтрагента.ТипПользователя = Карточка.ТипПользователя;
	...
	ОбменЧерезОператора = СинхронизацияЭДОКлиентСервер.ЭтоОбменЧерезОператора(ПараметрыКонтейнера.СпособОбмена);
	
	СоздаватьПриглашениеДляЭПДАвтоматически = Ложь;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки,
			ПараметрыПриглашения, Контрагент, СоздаватьПриглашениеДляЭПДАвтоматически);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Контрагент)
		...
		Контрагент = КонтрагентПоДаннымКонтейнера(
			ПараметрыПриглашения,
			КлючКонтрагента,
			ОбменЧерезОператора,
			Карточка.НаименованиеКонтрагентаУточненное,
			ПараметрыПриглашения.ИдентификаторКонтрагента,
			СоздаватьКонтрагентовАвтоматически
				Или ПараметрыУчетнойЗаписи.НеИспользоватьПриглашения);
	...
	Если Не ЗначениеЗаполнено(Контрагент)
		...
		Контрагент = КонтрагентПоДаннымКонтейнера(
			...
			КлючКонтрагента,
			ОбменЧерезОператора, 
			Карточка.НаименованиеКонтрагента,
	...
	Если Не ПустаяСтрока(Карточка.ИдентификаторОтправителя) Тогда
		ДанныеПриглашения = ПриглашенияЭДО.ДанныеПриглашенияПоПараметрам(ПараметрыПриглашения, Ложь);
	КонецЕсли;
	
	Если ДанныеПриглашения = Неопределено
			И ОбменЧерезОператора 
			И (ПараметрыУчетнойЗаписи.НеИспользоватьПриглашения
				Или СоздаватьПриглашениеДляЭПДАвтоматически) Тогда
		ДанныеПриглашения = ПриглашенияЭДО.СоздатьПриглашениеПоДаннымКарточки(
			Карточка, ПараметрыУчетнойЗаписи.Организация, Контрагент);
	КонецЕсли;
