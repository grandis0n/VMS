...
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено, ПараметрыПодписи = Неопределено) Экспорт
	...
	Если ОписаниеДанных.Свойство("КонтекстОперации")
	   И ТипЗнч(ОписаниеДанных.КонтекстОперации) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		НачалоИмениФормы = "Обработка.ПодписаниеИШифрованиеДанныхВМобильномЭДО.Форма.";
...
//   Операция - Строка - Операция, которую необходимо выполнить.
//   Пароль - Строка - Значение пароля к контейнеру Рутокен в с 
//
Асинх Процедура НачатьОбработкуОперацииРутокен(ПараметрыКомпоненты, Операция, Пароль) Экспорт

#Если МобильныйКлиент Тогда
	УстановитьЗапретЗасыпанияКомпьютера(Истина);
#КонецЕсли

	КомпонентаПодключена = Ждать ПодключитьКомпонентуРутокенАсинх(ПараметрыКомпоненты);
	...
	Контекст.Вставить("ПараметрыКомпоненты", ПараметрыКомпоненты);
	...
	МобильныйЭДОСлужебныйКлиент.ПродолжитьОбработкуОперацииРутокен(ОписаниеДанных, Контекст);
...
//
Асинх Процедура ЗавершитьОбработкуОперацииРутокен(Результат, ПараметрыКомпоненты) Экспорт
...
#Если Не ВебКлиент Тогда
	...
	Если ЧтениеДанных["isError"] Тогда
		...
		ПараметрыКомпоненты.ИдентификаторТекущейОперации ="";
		ПараметрыКомпоненты.ТекущаяОперация ="";
	Иначе
		...
		Если ПараметрыКомпоненты.ИдентификаторТекущейОперации = ИдентификаторОперации Тогда
			...
			Иначе
				...
				ИначеЕсли Операция = "Расшифровка" Тогда
					...
					ОповещениеОбработатьРезультат = Новый ОписаниеОповещения("ОбработатьРезультатРасшифровкиДанных",
						МобильныйЭДОСлужебныйКлиент,
						ПараметрыКомпоненты);
...
//
Асинх Функция ПодключитьКомпонентуРутокенАсинх (ПараметрыКомпоненты)
	...
	Если КомпонентаПодключена Тогда
		ОбъектКомпоненты = Новый ("AddIn.BIT.EDMSRutoken");
		ПараметрыКомпоненты.ОбъектКомпоненты = ОбъектКомпоненты;
		ПараметрыКомпоненты.Версия = ОбъектКомпоненты.ВерсияКомпоненты();
	Иначе
