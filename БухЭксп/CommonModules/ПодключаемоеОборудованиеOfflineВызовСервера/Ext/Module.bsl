...
Функция ТаблицыДанныхДляВыгрузки(Устройство, Параметры, ПолнаяВыгрузка)
	...
	|		ПО КодыТоваровПодключаемогоОборудованияOffline.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
...
Функция СоздатьИЗаполнитьОтчетОПродажах(ПараметрыУстройства, ТаблицыДанных, Комментарий = "", ЭтоПатент)
	...
	// Чеки загруженные из торгового оборудования и ОФД всегда с НДС в сумме
	СтруктураПараметровДокумента.Вставить("СуммаВключаетНДС", Истина);
	...
	ОтчетОРозничныхПродажахОбъект.ДеятельностьНаПатенте =  
		ЭтоПатент И УчетнаяПолитика.ПрименяетсяУСНПатент(ПараметрыУстройства.Организация, ТаблицыДанных.ДатаЗакрытияСмены);
