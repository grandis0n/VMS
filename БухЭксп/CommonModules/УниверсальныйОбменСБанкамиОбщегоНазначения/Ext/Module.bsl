...
//
// Параметры:
//  ПотокКомпоновщика - ПотокВПамяти - поток, куда будет записан xml.
...
//
Функция НовыйКомпоновщикXML(ПотокКомпоновщика, Кодировка = "windows-1251") Экспорт
	
	КомпоновщикXML = Новый ЗаписьXML;
	КомпоновщикXML.ОткрытьПоток(ПотокКомпоновщика, Кодировка);
...
//  ПотокКомпоновщика - ПотокВПамяти - поток, куда записывался xml-текст.
//
// Возвращаемое значение:
//   ДвоичныеДанные - xml-текст из переданного объекта.
//
Функция ДвоичныеДанныеКомпоновщикаXML(КомпоновщикXML, ПотокКомпоновщика) Экспорт
	
	КомпоновщикXML.Закрыть();
	КомпоновщикXML = Неопределено;
	
	Возврат ПотокКомпоновщика.ЗакрытьИПолучитьДвоичныеДанные();
