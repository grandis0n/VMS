...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗапуститьУниверсальныйОбменСБанкамиПоСервису");
...
#КонецОбласти
		...
		ФайловаяСистема.УдалитьВременныйФайл(ПолноеИмяФайлаКонтейнера);
		ФайловаяСистема.УдалитьВременныйКаталог(КаталогВременныхФайлов);
...
Функция ПоместитьДокументыКонтейнераВЖурналОперацийСФайлами(Транзакция, ТранспортнаяИнформация, КаталогСДаннымиКонтейнера)
	...
	Для каждого Документ Из ЗначениеВКоллекцию(ТранспортнаяИнформация.Документы.Документ) Цикл
		...
		Если СвойстваДокумента.Подписать Тогда
			ИмяФайлаПодписи = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
				Документ.Подпись.ИмяФайла);
...
Функция ПоместитьДанныеТранзакцииВЖурналОперацийСФайлами(Транзакция) Экспорт
	...
	ФайловаяСистема.УдалитьВременныйКаталог(КаталогСДаннымиКонтейнера);
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ФайловаяСистема.УдалитьВременныйКаталог(КаталогСДаннымиКонтейнера);
		ФайловаяСистема.УдалитьВременныйФайл(ИмяФайлаКонтейнера);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОпределитьРазмер(Данные) Экспорт
	...
	Если ТипДанных = Тип("Строка") И ЭтоАдресВременногоХранилища(Данные) Тогда
...
Функция ПоместитьСодержимоеТранзакцииВБазуДанных(Транзакция,
	...
	Если ТипДанных = Тип("ХранилищеЗначения") Тогда // если хранилище значения
		ДвоичныеДанныеФайла = Данные.Получить();
	ИначеЕсли ТипДанных = Тип("Строка") И ЭтоАдресВременногоХранилища(Данные) Тогда
...
Процедура УдалитьСписокФайлов(МассивИменФайлов)
	...
	Для каждого Файл Из МассивИменФайлов Цикл
		ФайловаяСистема.УдалитьВременныйФайл(Файл);
	КонецЦикла;
...
Функция ВыполнитьЗапросКСерверу(ПараметрыЗапроса, ПараметрыСервера = Неопределено) Экспорт
	...
	Исключение
		...
		Если НЕ ПустаяСтрока(ИмяФайлаТела) Тогда
			ФайловаяСистема.УдалитьВременныйФайл(ИмяФайлаТела);
		КонецЕсли;
		Возврат Результат;
...
#Область РазборОписанияКонтейнера
	...
	Для каждого Документ Из ЗначениеВКоллекцию(ТранспортнаяИнформация.Документы.Документ) Цикл
		...
		ИмяФайлаСодержимого = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			Документ.Содержимое.ИмяФайла);
		
		ПолноеИмяФайлаСодержимого = КаталогСДаннымиКонтейнера + ИмяФайлаСодержимого;
		НайденныеФайлы = НайтиФайлы(ПолноеИмяФайлаСодержимого);
		Если НайденныеФайлы.Количество() = 0 Тогда
			ФайловаяСистема.УдалитьВременныйКаталог(КаталогСДаннымиКонтейнера);
			ФайловаяСистема.УдалитьВременныйФайл(ИмяФайлаКонтейнера);
		...
		Иначе
			...
			Если Сжат Тогда
				...
				ФайловаяСистема.УдалитьВременныйКаталог(КаталогСДаннымиДокумента);
				...
				ЧтениеZip.Закрыть();
				ФайловаяСистема.УдалитьВременныйФайл(ПолноеИмяФайлаСодержимого);
				ПолноеИмяФайлаСодержимого = КаталогСДаннымиДокумента + ИмяФайлаВАрхиве;
			КонецЕсли;
			
			ДвоичныеДанныеСодержимого = Новый ДвоичныеДанные(ПолноеИмяФайлаСодержимого);
...
Функция РасшифроватьИПроверитьПодписьЗаписиВЖурналеОперацийСФайлами(МенеджерЗаписи, СертификатыБанка)
	...
	Если ТипЗнч(РасшифрованныеДанные) = Тип("ДвоичныеДанные") Тогда
		Если МенеджерЗаписи.Сжать Тогда
			...
			ИмяФайлаВАрхиве = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
				Архиватор.Элементы[0].Имя);
			Архиватор.Закрыть();
...
Функция РаспаковатьДвоичныеДанные(Данные, ИмяФайла, Идентификатор = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяФайлаАрхива = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ЧтениеАрхива.Элементы[0].Имя);
		РаспакованныеДанные = Новый ДвоичныеДанные(КаталогВременныхФайлов + ИмяФайлаАрхива);
