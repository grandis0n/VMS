...
Функция ДанныеЗаполненияНастройкиОтправкиПриглашений(ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.ОбновитьКэшНастроек Тогда
		СинхронизацияЭДО.ОбновитьКешиОператоровЭДОИФорматовВФоне(КонтекстДиагностики);
	КонецЕсли;
...
Функция ОтправитьПриглашения(Приглашения, КлючиСинхронизации, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Для Каждого СтрокаПриглашенияНаИдОрганизации Из ПриглашенияНаИдентификаторОрганизации Цикл
			...
			НоваяСтрока.ВидУчетнойЗаписи = Приглашение.ВидУчетнойЗаписи;
		КонецЦикла;
...
Процедура СоздатьКонтрагентовДляПриглашений(Приглашения, КонтекстОперации)
	...
	Для Каждого СтрокаТЧ Из Приглашения Цикл
		...
		Если ТрансграничныйЭДО.ЗапрещеноАвтосозданиеКонтрагента(СтрокаТЧ.ИдентификаторКонтрагента) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Приглашение Из Приглашения Цикл
		...
		Попытка
			...
			Если МенеджерЗаписи.Выбран() Тогда
				...
				РеквизитыКонтрагента.Наименование = ВыборкаДетальныеЗаписи.Наименование;
				РеквизитыКонтрагента.ИНН = ВыборкаДетальныеЗаписи.ИНН;
				РеквизитыКонтрагента.КПП = ВыборкаДетальныеЗаписи.КПП;
				ВидКонтрагента = ОбменСКонтрагентамиКлиентСервер.ВидКонтрагента(ВыборкаДетальныеЗаписи.ИНН, ВыборкаДетальныеЗаписи.ТипПользователя);
				...
				Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП(ТипОбъектаОтправителя,
					РеквизитыКонтрагента.ИНН, РеквизитыКонтрагента.КПП, , ВидКонтрагента);
...
Функция НовыеЗначенияСвойствПриглашенияДляОбновления() Экспорт
	...
	Данные.Вставить("Автоматическое", Ложь);
	Возврат Данные;
КонецФункции
...
Функция НовыеДанныеПриглашения() Экспорт
	
	Данные = Новый Структура;
	Данные.Вставить("КлючПриглашения", "");
	Данные.Вставить("Статус", Перечисления.СтатусыПриглашений.ПустаяСсылка());
	Данные.Вставить("Контрагент", Неопределено);
	Данные.Вставить("КонтрагентНаименование", "");
	Данные.Вставить("КонтрагентПредставление", "");
	Данные.Вставить("КонтрагентИНН", "");
	Данные.Вставить("КонтрагентКПП", "");
	Данные.Вставить("ИдентификаторКонтрагента", "");
	Данные.Вставить("Организация", Неопределено);
	Данные.Вставить("СпособОбмена", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
	Данные.Вставить("ИдентификаторОрганизации", "");
	Данные.Вставить("ОписаниеОшибки", "");
	Данные.Вставить("ТекстПриглашения", "");
	Данные.Вставить("ЭлектроннаяПочта", "");
	Данные.Вставить("ОператорКонтрагента", "");
	Данные.Вставить("ОператорОрганизации", "");
	Данные.Вставить("ПричинаОтказа", "");
	Данные.Вставить("EmailОрганизации", "");
	Данные.Вставить("ОбменМеждуСобственнымиОрганизациями", Ложь);
	Данные.Вставить("ИспользоватьЦентрЭДО", Ложь);
	Данные.Вставить("Автоматическое", Ложь);
	Данные.Вставить("ТипПользователя", "");
...
Функция ДанныеПриглашения(КлючПриглашения, ЗаполнятьПредставлениеКонтрагента = Истина) Экспорт
	...
	ИтоговыйЗапрос = Новый Запрос(
		...
		|	ПриглашенияКОбменуЭлектроннымиДокументами.Автоматическое КАК Автоматическое,
		|	ПриглашенияКОбменуЭлектроннымиДокументами.ТипПользователя КАК ТипПользователя
		|ИЗ
		|	РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами КАК ПриглашенияКОбменуЭлектроннымиДокументами
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетныеЗаписиЭДО КАК УчетныеЗаписиЭДО
		|		ПО ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторОрганизации = УчетныеЗаписиЭДО.ИдентификаторЭДО
	...
	Данные = НовыеДанныеПриглашения();
	ЗаполнитьЗначенияСвойств(Данные, Выборка);
	Данные.КлючПриглашения = КлючПриглашения.Ключ;
	Данные.ТипПользователя = Выборка.ТипПользователя;
