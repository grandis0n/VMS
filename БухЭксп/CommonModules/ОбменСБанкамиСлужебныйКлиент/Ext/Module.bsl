...
Процедура ОтправитьЗапросыВыписокПослеПодписания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.РеквизитыНастройкиОбмена.АутентификацияПоСертификату Тогда
		...
		ИначеЕсли ДополнительныеПараметры.Свойство("ОповещениеПослеСинхронизации") Тогда
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеСинхронизации, Ложь);
		КонецЕсли;
...
// Проверяет возможность настройки обмена с банком по предопределенным настройкам
Функция ДоступнаРаботаНаМобильномКлиенте(ПрограммаБанка, ИспользуетсяКриптография) Экспорт
	
	Если Не (ИспользуетсяКриптография = Истина)
		И (ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI")
			Или ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен")
			// Для совместимости со старыми предопределенными настройками
...
Процедура ПослеПолученияДокументовАсинхронныйОбмен(РезультатПолучения, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОтправки.ТекстОшибки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(РезультатОтправки.ТекстОшибки);
	КонецЕсли;
	...
	Иначе
		...
		Если РезультатПолучения.Свойство("ДанныеЭП") Тогда
			ПараметрыОбработкиПодписей = Новый Структура;
			ПараметрыОбработкиПодписей.Вставить("ДанныеЭП", РезультатПолучения.ДанныеЭП);
			ПараметрыОбработкиПодписей.Вставить("ПараметрОповещения", РезультатПолучения.ПараметрОповещения);
			ПараметрыОбработкиПодписей.Вставить("Отправлено", Отправлено);
			ПараметрыОбработкиПодписей.Вставить("Получено", Получено);
			ПараметрыОбработкиПодписей.Вставить("ОповещениеПослеСинхронизации",
				ДополнительныеПараметры.ОповещениеПослеСинхронизации);
			ОбработатьДанныеЭП(ПараметрыОбработкиПодписей);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьДанныеЭП(ДополнительныеПараметры)

	Если ДополнительныеПараметры.ДанныеЭП.Количество() = 0 Тогда
		Оповестить("ОбновитьСостояниеОбменСБанками", ДополнительныеПараметры.ПараметрОповещения);
		ПоказатьРезультатСинхронизации(ДополнительныеПараметры.Отправлено, ДополнительныеПараметры.Получено);
		ЕстьОшибка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ЕстьОшибка", Ложь);
	...
	Оповещение = Новый ОписаниеОповещения("ПослеОбработкиОдногоСообщения", ЭтотОбъект, ДополнительныеПараметры);
	ДобавитьПодписиИОпределитьСтатусы(Оповещение, КлючЗначение.Ключ, КлючЗначение.Значение);

КонецПроцедуры

Процедура ПослеОбработкиОдногоСообщения(Результат, ДополнительныеПараметры) Экспорт

	Если Не Результат Тогда
		ДополнительныеПараметры.Вставить("ЕстьОшибка", Истина);
	КонецЕсли;

	ОбработатьДанныеЭП(ДополнительныеПараметры);
КонецПроцедуры
...
Процедура ОбработатьЭД(
	...
	Если ЭтоМобильныйКлиент()
		...
		Если ЗначениеЗаполнено(РеквизитыДокумента)
			И НЕ ДоступнаРаботаНаМобильномКлиенте(РеквизитыДокумента.ПрограммаБанка, РеквизитыДокумента.ИспользуетсяКриптография) Тогда
...
Процедура УстановитьРасширениеПослеОтвета(Ответ, ДополнительныеПараметры) Экспорт
	...
	#Если МобильныйКлиент Тогда
		// Метод НачатьУстановкуРасширенияПолученияИнформацииОКомпьютере недоступен в мобильном клиенте 8.3.27
		ВызватьИсключение НСтр("ru = 'Функциональность не поддерживается в мобильном клиенте'");
	#Иначе
...
Процедура ПодключитьРасширениеПослеУстановки(ДополнительныеПараметры) Экспорт
	...
	#Если МобильныйКлиент Тогда
		// Метод НачатьПодключениеРасширенияПолученияИнформацииОКомпьютере недоступен в мобильном клиенте 8.3.27
		ВызватьИсключение НСтр("ru = 'Функциональность не поддерживается в мобильном клиенте'");
	#Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения(
