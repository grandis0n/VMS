...
Процедура ОбработатьУведомлениеОтФНС(Документ, ДанныеДокумента, ПутьРаспаковкиФайлов) Экспорт
	...
	Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ПутьРаспаковкиФайлов) Тогда
		ВызватьИсключение "Обрабатывать уведомления от ФНС можно только из временного каталога";
	КонецЕсли;
...
Процедура ОбновитьПриложенныеФайлы(УведомлениеОбъект, ПроверитьРанееСозданныеФайлы, ПриложенныеФайлыXML, ПутьРаспаковкиФайлов)
	...
	Если Не ОбщегоНазначенияБП.ЭтоИмяВременногоФайла(ПутьРаспаковкиФайлов) Тогда
		ВызватьИсключение "Обновлять приложенные файлы можно только из временного каталога";
	КонецЕсли;
