...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьФлагОтправкиПустыхОтчетов");
...
// Обработчик обновления. Заполняет признак необходимости отправки пустых отчетов
//  для рассылок, созданных из форм отчетов.
//
// Параметры:
//   ПараметрыВыполнения - Структура - Параметры обработчика обновления в отложенном режиме
//
Процедура ЗаполнитьФлагОтправкиПустыхОтчетов(ПараметрыВыполнения = Неопределено) Экспорт
	...
	|ГДЕ
	|	РассылкиОтчетов.СозданаИзФормыОтчета";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		РассылкаОтчета = Выборка.Ссылка.ПолучитьОбъект();
		Для Каждого Отчет Из РассылкаОтчета.Отчеты Цикл
			Отчет.ОтправлятьЕслиПустой = Истина;
		КонецЦикла;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(РассылкаОтчета);
	КонецЦикла;
