...
Функция УплачиваетсяПлатежНаЕНСПоПериоду(Период) Экспорт

	// ИП на УСН (доходы) без сотрудников пополняет ЕНС только в эти месяцы
	МесяцыПополнения = Новый Массив;
	
	МесяцыПополнения.Добавить(4); // апрель
	МесяцыПополнения.Добавить(7); // июль
	МесяцыПополнения.Добавить(10); // октябрь
	
	Возврат МесяцыПополнения.Найти(Месяц(Период) + 1) <> Неопределено;
