...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ЗагрузкаСтатусовОперацийПлатежныхСистем");
	КонецЕсли;
...
//
Функция ИзображениеQRКода(ФункциональнаяСсылка, Размер, ПлатежнаяСистема, Знач ТипПечати) Экспорт
	
	Логотип = ЛоготипQRКода(ПлатежнаяСистема, ТипПечати);
	...
	Если ПлатежнаяСистема = НастройкиПлатежныхСистемКлиентСервер.ИмяПодсистемыСБП()
		И ПечатьУПКПоддержана() Тогда
		ПараметрыГенерации.ТипКода = 26;
		ПараметрыГенерации.ФорматВыходногоИзображения = 1;
		ПараметрыГенерации.ПрозрачныйФон = Ложь;
		Если ТипПечати = 1
			Или ТипПечати = 2 Тогда
			ПараметрыГенерации.ОтображениеУниверсальногоПлатежногоКода = 1;
		Иначе
			ПараметрыГенерации.ОтображениеУниверсальногоПлатежногоКода = 0;
		КонецЕсли;
	Иначе
...
//
// Возвращаемое значение:
//  Картинка - логотип платежной системы.
//
Функция ЛоготипQRКода(ПлатежнаяСистема, Знач ТипПечати)
...
// Выполняет проверку поддержки печати УПК.
//
// Возвращаемое значение:
//  Булево - Истина, если печать УПК поддержана.
//
Функция ПечатьУПКПоддержана()
	
	ТекущаяВерсияБСП = СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ТекущаяВерсияБСП, "3.2.1.1") >= 0 Тогда
		ТребуемаяВерсия = "3.2.1.499";
	Иначе
		ТребуемаяВерсия = "3.1.12.293";
	КонецЕсли;
