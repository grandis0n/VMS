...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	// Установка вида приложения
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ВидыПриложенийСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьУстановкуВидаПриложения";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(СвойстваВида.Процедура, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РезультатОбработки))";
	
	ИменаПроцедур = Новый Соответствие;
	ВидыПриложенийКонфигурации = ВидыПриложенийСервер.ВидыПриложенийКонфигурации();
	
	Для Каждого ВидыПриложенияКонфигурации Из ВидыПриложенийКонфигурации Цикл
		
		ИменаПроцедур.Вставить(ВидыПриложенияКонфигурации.Процедура);
		
	КонецЦикла;
	
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик установки вида приложения ""%1"".'");
	
	// Переход на вид приложения
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ВидыПриложенийСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПереходНаВидПриложения";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОбработчикПерехода.Процедура)";
	
	ИменаПроцедур = Новый Соответствие;
	ОбработчикиПереходов = ВидыПриложенийСервер.ОбработчикиПереходов();
	
	Для Каждого ОбработчикПерехода Из ОбработчикиПереходов Цикл
		
		ИменаПроцедур.Вставить(ОбработчикПерехода.Процедура);
		
	КонецЦикла;
	
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик перехода вида приложения ""%1"".'");
	
	// Очередь заданий
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОчередьЗаданийСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьМетодКонфигурации";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяМетода, Параметры)";
	
	Если Метаданные.ОбщиеМодули.Найти("ОчередьЗаданийСлужебныйПовтИсп") <> Неопределено Тогда
		
		МодульОчередьЗаданийСлужебныйПовтИсп = ОбщегоНазначения.ОбщийМодуль("ОчередьЗаданийСлужебныйПовтИсп");
		МетодыКонфигурации = МодульОчередьЗаданийСлужебныйПовтИсп.СоответствиеИменМетодовПсевдонимам(Ложь);
		
	Иначе
		
		МетодыКонфигурации = Новый Соответствие;
		ПриОпределенииПсевдонимовОбработчиков(МетодыКонфигурации);
	...
	Для Каждого КлючИЗначение Из МетодыКонфигурации Цикл
		...
		Иначе
			...
			ИменаПроцедур.Вставить(КлючИЗначение.Ключ);
	...
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик задания очереди ""%1"".'");
	
	// Инциденты ЦКК
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ИнцидентыЦККСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ПроверитьАктуальностьОткрытыхИнцидентов";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОпределениеТипа.ПроцедураПроверки, ПараметрыПроцедуры)";
	
	ИменаПроцедур = Новый Соответствие;
	СписокТиповИнцидентов = Новый Соответствие();
	ИнцидентыЦККПереопределяемый.СписокТиповИнцидентовПереопределяемый(СписокТиповИнцидентов);
	
	Для Каждого ТипИнцидента Из СписокТиповИнцидентов Цикл
		
		ИменаПроцедур.Вставить(ТипИнцидента.Значение.ПроцедураПроверки);
		
	КонецЦикла;
	
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Процедура проверки инцидента ЦКК ""%1"".'");
	
	Если Метаданные.ОбщиеМодули.Найти("ОТ_ПрограммныйИнтерфейсБТСПовтИсп") <> Неопределено Тогда
		
		МодульПрограммныйИнтерфейсБТСПовтИсп = ОбщегоНазначения.ОбщийМодуль("ОТ_ПрограммныйИнтерфейсБТСПовтИсп");

		// Программный интерфейс БТС (описания)
		ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
		ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОТ_ПрограммныйИнтерфейсБТСПовтИсп";
		ИсключениеПроверки.ИмяПроцедуры       = "ОписаниеМетодаИнтерфейса";
		ИсключениеПроверки.ФрагментСодержания = "Результат = ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИнфоМетода.ФункцияОписания,, Истина)";
		
		ИменаПроцедур = Новый Соответствие;
		ВсеМетоды = МодульПрограммныйИнтерфейсБТСПовтИсп.МетодыИнтерфейса();
		
		Для Каждого ИнфоМетода Из ВсеМетоды Цикл
			
			ИменаПроцедур.Вставить(ИнфоМетода.Значение.ФункцияОписания);
			
		КонецЦикла;
		
		ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
		ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
		ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Функция описания программного интерфейса БТС ""%1"".'");
		
		// Программный интерфейс БТС (обработчики)
		ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
		ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОТ_ПрограммныйИнтерфейсБТС";
		ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчикМетодаИнтерфейса";
		ИсключениеПроверки.ФрагментСодержания = "РезультатВыполнения = ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик, Параметры, Истина)";
		
		ИменаПроцедур = Новый Соответствие;
		
		Для Каждого ИнфоМетода Из ВсеМетоды Цикл
			
			ОписаниеМетодаИнтерфейса = МодульПрограммныйИнтерфейсБТСПовтИсп.ОписаниеМетодаИнтерфейса(ИнфоМетода.Ключ);
			ИменаПроцедур.Вставить(ОписаниеМетодаИнтерфейса.Обработчик);
			
		КонецЦикла;
		
		ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
		ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
		ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик программного интерфейса БТС ""%1"".'");
