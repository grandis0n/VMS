...
// Получает список характеров груза из сервиса поставщика. 
// Если запрос вернул необрабатываемый результат возвращает пустую структуру.
// 
// Параметры:
...
//  - Структура:
//		* Ошибки - Массив Из Строка - обязательное свойство, массив ошибок может быть пустым
//		* Список - Массив Из Структура - заполненный массив, если запрос выполнен без ошибок:
...
Процедура ОчиститьНезаполненныеПоляШаблона(ПараметрыЗаказа, ТипГрузоперевозки) Экспорт
	
	УдаляемыеКлючи = Новый Массив;
	
	Для Каждого Реквизит Из ПараметрыЗаказа Цикл
		Если Реквизит.Значение = Неопределено Или Не ЗначениеЗаполнено(Реквизит.Значение) Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли (ТипЗнч(Реквизит.Значение) = ТИП("ТаблицаЗначений") Или ТипЗнч(Реквизит.Значение) = ТИП("Массив")
			Или ТипЗнч(Реквизит.Значение) = ТИП("СписокЗначений")) И Реквизит.Значение.Количество() = 0 Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли ТипЗнч(Реквизит.Значение) = ТИП("Булево") И Не Реквизит.Значение Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли (Реквизит.Ключ = "СпособДоставки" Или Реквизит.Ключ = "СпособОтгрузки") И Реквизит.Значение = 1 Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли ТипЗнч(Реквизит.Значение) = ТИП("Структура") Тогда
			ОчиститьНезаполненнуюСтруктуруШаблона(Реквизит, УдаляемыеКлючи, ТипГрузоперевозки);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Ключ Из УдаляемыеКлючи Цикл
		ПараметрыЗаказа.Удалить(Ключ);
	КонецЦикла;
...
Функция ЗаполнитьДанныеДляСервисаСозданиеИзменениеЗаказаНаДоставку(ПараметрыЗапроса)
	...
	Для Каждого ТекущаяПозиция Из ПараметрыЗапроса.Товары Цикл
		...
		Если ЭтоСДЭК Тогда
			...
			ДобавитьЗначение(НоваяПозиция, "costDeclared", ТекущаяПозиция.Стоимость, Истина);
		...
		Если Не Позиции = Неопределено Тогда
			...
			Для Каждого ТекущаяПозиция Из Позиции Цикл
				...
				Если ЭтоСДЭК Тогда
					...
					НоваяПозиция.Стоимость = ПолучитьЗначение(ТекущаяПозиция, "costDeclared");
				КонецЕсли;
...
Процедура ОчиститьНезаполненнуюСтруктуруШаблона(Реквизит, УдаляемыеКлючи, ТипГрузоперевозки)
	
	Если Реквизит.Ключ = "Валюта" И Не ЗначениеЗаполнено(Реквизит.Значение.Ссылка) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ВремяДоставки" И ДатаНеЗаполнена(Реквизит.Значение) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ВремяОтгрузки" И ДатаНеЗаполнена(Реквизит.Значение) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "Отправитель" И ДанныеКонтрагентаНеЗаполнены(Реквизит.Значение) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "Плательщик" И ДанныеКонтрагентаНеЗаполнены(Реквизит.Значение) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "Получатель" И ДанныеКонтрагентаНеЗаполнены(Реквизит.Значение) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ОтправительНаселенныйПункт" И Не ЗначениеЗаполнено(Реквизит.Значение.Представление) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ПолучательНаселенныйПункт" И Не ЗначениеЗаполнено(Реквизит.Значение.Представление) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "Пропуск" И Не ЗначениеЗаполнено(Реквизит.Значение.Заказывать) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ПунктВыдачиГруза" И Не ЗначениеЗаполнено(Реквизит.Значение.Адрес) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ПунктВыдачиКлиента" И Не ЗначениеЗаполнено(Реквизит.Значение.Адрес) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "ПунктПриемаГруза" И Не ЗначениеЗаполнено(Реквизит.Значение.Адрес) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	ИначеЕсли Реквизит.Ключ = "Тариф" И Не ЗначениеЗаполнено(Реквизит.Значение.Идентификатор) Тогда
		УдаляемыеКлючи.Добавить(Реквизит.Ключ);
	КонецЕсли;

	Если ТипГрузоперевозки = Перечисления.ТипыГрузоперевозки.ДеловыеЛинии Тогда
		Если Реквизит.Ключ = "Груз" И ДанныеГрузаНеЗаполнены(Реквизит.Значение) Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		КонецЕсли;
	КонецЕсли; 
...
Функция ДатаНеЗаполнена(Дата)
	
	Возврат Не (ЗначениеЗаполнено(Дата.ОбедС) Или ЗначениеЗаполнено(Дата.ОбедПо)
		Или ЗначениеЗаполнено(Дата.С) Или ЗначениеЗаполнено(Дата.По) Или ЗначениеЗаполнено(Дата.Фиксированное));
	
КонецФункции
	
Функция ДанныеКонтрагентаНеЗаполнены(ДанныеКонтрагента)
	
	УдаляемыеКлючи = Новый Массив;
	
	Для Каждого Реквизит Из ДанныеКонтрагента Цикл
		Если Реквизит.Ключ = "Адрес" И Не ЗначениеЗаполнено(Реквизит.Значение.Представление) Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли Реквизит.Ключ = "КонтактноеЛицо" И Не ЗначениеЗаполнено(Реквизит.Значение.Наименование) Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		ИначеЕсли Реквизит.Ключ = "Контрагент" И Не ЗначениеЗаполнено(Реквизит.Значение.Наименование) Тогда
			УдаляемыеКлючи.Добавить(Реквизит.Ключ);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Ключ Из УдаляемыеКлючи Цикл
		ДанныеКонтрагента.Удалить(Ключ);
	КонецЦикла;
	
	Возврат Не ЗначениеЗаполнено(ДанныеКонтрагента);
	
КонецФункции

Функция ДанныеГрузаНеЗаполнены(ДанныеГруза)
	
	УдаляемыеКлючи = Новый Массив;
	
	Для Каждого Реквизит Из ДанныеГруза Цикл
	...
	Для Каждого Ключ Из УдаляемыеКлючи Цикл
		ДанныеГруза.Удалить(Ключ);
	КонецЦикла;
	
	Возврат Не ЗначениеЗаполнено(ДанныеГруза);
