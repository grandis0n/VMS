...
Процедура ОбработатьШтрихкоды(Форма, ДанныеДляОбработки, КэшированныеЗначения) Экспорт
	...
	Для каждого ДанныеШтрихкода Из ДанныеДляОбработки.Штрихкоды Цикл
		...
		Если ЗначениеЗаполнено(НоменклатураПоШтрихкоду) Тогда
			...
			Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(СведенияОНоменклатуре.ВидПродукцииИС) Тогда
			...
			Если НЕ КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(СведенияОНоменклатуре.ВидПродукцииИС) 
				И НЕ КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМОТП(СведенияОНоменклатуре.ВидПродукцииИС) 
...
//
Процедура ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыСканирования = Неопределено) Экспорт
	...
	Для каждого СтрокаТаблицы Из ДанныеПоШтрихкодамEAN Цикл
		...
		Если СтрокаСоответствия <> Неопределено Тогда
			...
			СтрокаТаблицы.ВидПродукции              = СтрокаСоответствия.КлючВидовПродукцииИС;
		Иначе
...
Процедура ПриОпределенииТекстаЗапросаСвойствМаркируемойПродукции(ТекстЗапросаСвойстваМаркируемойПродукции, ТаблицаИсточник = "ДанныеШтрихкодовУпаковок") Экспорт
	...
	ВидыМаркируемойПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииИСМП(Истина);
