...
Функция ОпределитьДокументОснованиеСверкиКодов(ДОкументСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АктОРасхождениях") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ДокументПоступления");
	Иначе
...
Процедура ЕстьМаркируемаяПродукция(Объект, ИмяТЧ, ЕстьМаркируемаяПродукция) Экспорт
	...
	|ИЗ
	|	ВремТаблТовары КАК ТаблицаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеИС
	|		ПО ТаблицаТовары.Номенклатура = ОписаниеИС.Номенклатура
	|		И ОписаниеИС.КлючВидовПродукции <> ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка)
	|";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	//При необходимости фильтровать здесь (в объекте есть ссылка)
	Запрос.УстановитьПараметр("ТаблицаТовары", Объект[ИмяТЧ].Выгрузить(, "Номенклатура"));
	Результат = Запрос.Выполнить();
...
//
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(Контекст, ТаблицаМаркируемойПродукции) Экспорт
	...
	ИначеЕсли ИнтеграцияИСБПКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ПередачаТоваров") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииПередачаТоваров(Контекст);
	Иначе
		Возврат;
	КонецЕсли;
	...
	Если ТаблицаМаркируемойПродукции.Колонки.Найти("ВидПродукции") <> Неопределено Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаМаркируемойПродукции Цикл
			СтрокаТаблицы.ВидПродукции = СтрокаТаблицы.ВидПродукции; 
		КонецЦикла;
...
Процедура ПриЗаполненииЗначенияШтрихкодовПродукцииПоДокументу(ДокументСсылка, ТаблицаТоваровЗначенийШтрихкодов) Экспорт
	...
	Если ТипПроверямогоДокумента = Тип("ДокументСсылка.АктОРасхождениях") Тогда
		...
		Запрос.УстановитьПараметр("ВидыМаркируемойПродукции", ФильтрВидПродукции);
...
Функция ШтрихкодыСодержащиеВидыПродукции(Знач ШтрихкодыДляПроверки, Знач ВидыПродукции) Экспорт
	...
	|	ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ, ЛОЖЬ) КАК ПодходящаяПродукция
	|ИЗ
	...
	|		ПО ШтрихкодыУпаковок.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура 
	|ГДЕ
...
Функция ЗапросМаркируемойПродукцииПоступлениеТоваровУслуг(Документ)
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ПоступлениеТоваровУслугТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
...
Процедура ПриОпеределенииТаблицыПродукцииПринятойБезРасхождений(ДокументОснование, НоменклатураПоДаннымЭДО, ТаблицаПродукции) Экспорт
	...
	Если ТипЗнч(ДОкументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииРеализацияТоваровУслуг(ДокументОснование).Выбрать();
...
Функция ЗапросМаркируемойПродукцииВозвратТоваровОтПокупателя(Документ)
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|   ПО ВозвратТоваровОтПокупателяТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура 
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
...
Функция ЗапросМаркируемойПродукцииАктОРасхождениях(Документ)
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|   ПО АктОРасхожденияхТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура 
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
...
Функция ЗапросМаркируемойПродукцииАктОРасхожденияхПолученный(Документ)
...
Функция ЗапросМаркируемойПродукцииПередачаТоваров(Документ)
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ПередачаТоваровТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
...
Функция ЗапросМаркируемойПродукцииПриемкаТоваровИСМП(Документ)
...
Процедура СформироватьТаблицуМаркируемойПродукцииДокумента(Документ, ТаблицаПродукции, ВидыМаркируемойПродукции) Экспорт
	...
	Запрос.УстановитьПараметр("ВидыМаркируемойПродукции", ВидыМаркируемойПродукции);
...
Процедура СформироватьТаблицуМаркированныхТоваровОснований(Запрос, ИсточникОснований) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ТоварыПредварительно.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
...
Процедура ОбработкаЗаполненияДокументаВыводИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.СписаниеТоваров") Тогда
		...
		ФильтрВидПродукции = ?(ЗначениеЗаполнено(ДокументОбъект.ТоварнаяГруппа), ДокументОбъект.ТоварнаяГруппа, Неопределено);
...
Процедура ОбработкаЗаполненияДокументаМаркировкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") Тогда
		//•	ВводВОборотСырьяПроизводствоРФ (если сырье)
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ДокументСсылка.Ссылка           КАК ДокументОснование,
		|	ДокументСсылка.Организация      КАК Организация,
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПоступлениеИзПереработки") Тогда
		//•	ВводВОборотСырьяПроизводствоРФ	
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ДокументСсылка.Ссылка           КАК ДокументОснование,
		|	ДокументСсылка.Организация      КАК Организация,
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС") Тогда
		...
		ФильтрВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииВетИС(); 
		ДокументОбъект.ТоварнаяГруппа = ФильтрВидПродукции;
...
Функция ВидОперацииОтгрузкаТоваровИСМППоВидуПродукцииИКонтрагенту(ВидПродукции, Контрагент)
	...
	Если ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаЕАЭССПризнаниемКИДоступна(ВидПродукции) Тогда
		ВидыПродукцииЭкспортЕАЭС.Вставить(ВидПродукции, Перечисления.ВидыОперацийИСМП.ОтгрузкаЕАЭССПризнаниемКИ);
	КонецЕсли;
	
	Если ОтгрузкаТоваровКлиентСервер.ОперацияОтгрузкаВЕАЭСПриОСУДоступна(ВидПродукции) Тогда
		ВидыПродукцииЭкспортЕАЭС.Вставить(ВидПродукции, Перечисления.ВидыОперацийИСМП.ОтгрузкаВЕАЭСПриОСУ);
	КонецЕсли;
	...
	ИначеЕсли (ВидыПродукцииЭкспортЕАЭС[ВидПродукции] <> Неопределено И Справочники.Контрагенты.КонтрагентРезидентТаможенногоСоюза(Контрагент)) Тогда
...
Процедура ОбработкаЗаполненияДокументаОтгрузкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		ДокументОбъект.Операция = ВидОперацииОтгрузкаТоваровИСМППоВидуПродукцииИКонтрагенту(ДокументОбъект.ТоварнаяГруппа, ДокументОбъект.Контрагент);
...
Процедура УстановитьУсловиеПоМаркируемойПродукции(ТекстЗапроса, Знач ВидыМаркируемойПродукции, ИмяПараметра =  "&УсловиеМаркируемаяПродукция") Экспорт
	...
	СтрокаУсловия = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ";
	Если ТипЗнч(ВидыМаркируемойПродукции) = Тип("СправочникСсылка.ВидыПродукцииИС") Тогда
		СтрокаУсловия = СтрокаУсловия + " = &ВидыМаркируемойПродукции";
	ИначеЕсли (ТипЗнч(ВидыМаркируемойПродукции) = Тип("Массив") Или ТипЗнч(ВидыМаркируемойПродукции) = Тип("ФиксированныйМассив"))Тогда  
		СтрокаУсловия = СтрокаУсловия + " В (&ВидыМаркируемойПродукции)"; 
	Иначе
		СтрокаУсловия = СтрокаУсловия + " = ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
	КонецЕсли;
...
Процедура УстановитьОпределениеВидаПродукции(ТекстЗапроса) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОпределениеВидаПродукции", "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ");
...
Функция УсловиеПоВидуПродукции(ВидПродукции = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидПродукции) Тогда
		Условие = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)";
	ИначеЕсли ТипЗнч(ВидПродукции) = Тип("СправочникСсылка.ТоварныеГруппыГИСМТ") Тогда
		Условие = "ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ТоварнаяГруппаГИСМТ)" 
	КонецЕсли;
...
Функция ТекстЗапросаМаркируемойПродукции(ДокументСсылка, ИмяТабЧастиТовары = Неопределено)
	...
	|	МАКСИМУМ(ОписаниеНоменклатурыИС.КлючВидовПродукции) КАК КлючВидовПродукции,
	|	МАКСИМУМ(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ) КАК ВидПродукцииГИСМТ
	|ИЗ
...
Процедура ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(
	...
	Запрос.УстановитьПараметр("ТоварнаяГруппаГИСМТ"	, КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииПоТоварнойГруппе(ФильтрВидПродукции));
...
Функция МаркируемаяПродукция(Номенклатура, ПараметрыСканирования = Неопределено) Экспорт
	
	ЭтоМаркируемаяПродукция = Ложь;
	
	ОписаниеНоменклатурыИС 	= ОбщегоНазначенияИС.ОписаниеНоменклатуры(Номенклатура);
	КлючВидаПродукции 		= ОписаниеНоменклатурыИС[Номенклатура].КлючВидовПродукции;
	Если ПараметрыСканирования = Неопределено Тогда
		ЭтоМаркируемаяПродукция = КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(КлючВидаПродукции, Истина);
	Иначе
		ЭтоМаркируемаяПродукция = 
			(ПараметрыСканирования.ЕГАИС И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(КлючВидаПродукции))
			Или (ПараметрыСканирования.ИСМП И КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(КлючВидаПродукции, Истина))
	КонецЕсли;
	
	Возврат ЭтоМаркируемаяПродукция;
...
//Проверяет добавление в таблицу товаров колонок ИСМП
//Параметры: Товары Тип("ДанныеФормыКоллекция")
//					или Тип("ТаблицаЗначений")
//
//Возвращаемое значение ЕстьКолонкиИСМП Тип("Булево")
//	
Функция ЕстьДобавленныеКолонкиИСМП(Товары) Экспорт
	
	ЕстьКолонкиИСМП = Ложь;
	...
	Иначе
		Возврат ЕстьКолонкиИСМП;
	КонецЕсли;
	
	Если ТаблицаТоваров.Колонки.Найти("МаркируемаяПродукция") <> Неопределено Тогда
		ЕстьКолонкиИСМП = Истина;
	КонецЕсли;
...
//Заполняет признак маркруемой продукции и вид маркируемой продукции
//Параметры: ТаблицаТовары Тип("ДанныеФормыКоллекция")
//
//Возвращаемое значение: нет
//
Процедура ЗаполнитьДобавленныеКолонкиИСМП(ТаблицаТовары, ИспользоватьКонтрольныеЗнакиГИСМ = Ложь) Экспорт
	
	МассивНоменклатуры = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаТовары, "Номенклатура", Истина);
	СоответствиеВидовПродукции = ОбщегоНазначенияИС.ОписаниеНоменклатуры(МассивНоменклатуры);
	
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		КлючВидаПродукцииИС = СоответствиеВидовПродукции[СтрокаТаблицы.Номенклатура].КлючВидовПродукции;
		Если КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияИСМП(КлючВидаПродукцииИС, Истина)
			ИЛИ КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоМаркируемаяПродукцияЕГАИС(КлючВидаПродукцииИС) Тогда
			СтрокаТаблицы.МаркируемаяПродукция = Истина;
		Иначе
			СтрокаТаблицы.МаркируемаяПродукция = Ложь;	
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ВидПродукцииИС") Тогда
			СтрокаТаблицы.ВидПродукцииИС = КлючВидаПродукцииИС;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ПродукцияМаркируемаяДляГИСМ") Тогда
			СтрокаТаблицы.ПродукцияМаркируемаяДляГИСМ = ИспользоватьКонтрольныеЗнакиГИСМ;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДоработатьЗапросПродукцииВЕТИС(Текст)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииВЕТИС В(&ВидыПродукции)
	|;
	|
	|";
...
Функция ТекстУсловияСоответствияНоменклатурыВидамПродукцииИС(Источник, Синоним = "", ВключаяАлкоголь = Ложь) Экспорт
	...
	Возврат ТекстУсловия = ТекстУсловия + ?(ЗначениеЗаполнено(ТекстУсловия), " ИЛИ ", "") +
				"ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)"
...
Функция ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения(ИмяРеквизита, ИсточникУпаковки, ИсточникНоменклатуры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИсточникНоменклатуры) Тогда
		...
		ТекстЗапроса = 
		"ВЫБОР
		|	КОГДА ЕСТЬNULL(ТаблицаТовары.Упаковка, ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
		|		ТОГДА NULL
		|	ИНАЧЕ ТаблицаТовары.Номенклатура.ЕдиницаИзмерения#ИмяРеквизита
		|КОНЕЦ";
	...
	Иначе
		...
		Иначе
			СтрокаЗамены = "ИнтеграцияИСМПБП.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения";
...
// Реквизиты единицы измерения.
// 
// Возвращаемое значение:
//  Массив из Строка - Реквизиты единицы измерения
Функция РеквизитыЕдиницыИзмерения() Экспорт
