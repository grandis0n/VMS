...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьКешиОператоровЭДОИФорматовВФонеСервер", Истина);
...
Функция ОтправитьОбъекты(ЗапросОбъектов, КонтекстДиагностики, ОтпечаткиСертификатов, ВыбранныеСертификаты = Неопределено,
	...
	ОбрабатыватьТолькоОбъектыAPIТакском = Ложь;
	ИспользоватьТокеныДоступа = Ложь;
	ПараметрыВыполнения = Новый Структура;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ПараметрыВыполнения", ПараметрыВыполнения) Тогда
		ОбрабатыватьТолькоОбъектыAPIТакском = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыВыполнения, "ВыполнитьТолькоПоТакском", Ложь);
		ИспользоватьТокеныДоступа = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыВыполнения, "ИспользоватьТокеныДоступа", Ложь);
	КонецЕсли;
	...
	Если ИспользоватьТокеныДоступа Тогда
		...
		КлючиСинхронизацииПоТокенам = КлючиСинхронизацииПоТокенамДоступаКСервисуЭДО(
			ИдентификаторыОтправителей, КонтекстДиагностики);
		
		Если ЗначениеЗаполнено(ДанныеДляСинхронизации.КлючиСинхронизации) Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
				ДанныеДляСинхронизации.КлючиСинхронизации, КлючиСинхронизацииПоТокенам, Ложь);
		Иначе
			ДанныеДляСинхронизации.КлючиСинхронизации = КлючиСинхронизацииПоТокенам;
		КонецЕсли;
...
// * Карточка - см. ТранспортныеКонтейнерыЭДО.НовоеОписаниеКарточкиЭлектронногоДокумента - карточка контейнера оператора ЭДО.
//            - см. ЭлектронноеАктированиеЕИСПолучениеВходящих.НовоеОписаниеКарточкиПакета - карточка контейнера ЕИС.
//              Состав полей свой: поле ДанныеЧерновика и служебные поля оператора
//              (ФайлНазначения, ДоверенностьВерна, КарточкаМЧД и др.) отсутствуют.
...
Функция НовыеДанныеОбъектов() Экспорт
	...
	ДанныеОбъектов.Колонки.Добавить("ИдентификаторЧерновика", Новый ОписаниеТипов("Строка",,,
		Новый КвалификаторыСтроки(32))); // LinkedTemplate
...
Функция ДоступныеИдентификаторыЭДО(Отправитель, Получатель) Экспорт
	...
	ИмяРеквизитаЮрФизЛицоОрганизации = ИнтеграцияЭДО.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЮрФизЛицоОрганизации");
	ПараметрыОрганизации = Новый Структура(ИмяРеквизитаИННОрганизации + ", " + ИмяРеквизитаКППОрганизации + ", " + ИмяРеквизитаЮрФизЛицоОрганизации);
	
	Если ЗначениеЗаполнено(Отправитель) Тогда
		ИменаРеквизитовОбъекта = ИмяРеквизитаИННОрганизации + ", " + ИмяРеквизитаКППОрганизации;
		Если ЗначениеЗаполнено(ИмяРеквизитаЮрФизЛицоОрганизации) Тогда
			ИменаРеквизитовОбъекта = ИменаРеквизитовОбъекта + ", " + ИмяРеквизитаЮрФизЛицоОрганизации;
		КонецЕсли;
		ПараметрыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Отправитель, ИменаРеквизитовОбъекта);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаЮрФизЛицоОрганизации) Тогда
		ОтборОрганизаций.ЮрФизЛицо = "&ЮрФизЛицо";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаЮрФизЛицоОрганизации) Тогда
		ИтоговыйЗапрос.УстановитьПараметр("ЮрФизЛицо"      , ПараметрыОрганизации[ИмяРеквизитаЮрФизЛицоОрганизации]);
	КонецЕсли;
...
Функция СпособыОбменаЧерезОператора() Экспорт
	...
	СпособыОбмена.Добавить(Перечисления.СпособыОбменаЭД.ЧерезСервис1СEDI);
...
// Серверная процедура для запуска синхронизации кеша ЭДО в фоновом режиме.
//
// Параметры:
//  Параметры - Структура - структура с параметрами:
//   *КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  АдресРезультата - Строка
Процедура ОбновитьКешиОператоровЭДОИФорматовВФонеСервер(Параметры, АдресРезультата = "") Экспорт
	
	КонтекстДиагностики = Неопределено;
	Если Параметры.Свойство("КонтекстДиагностики") Тогда
		КонтекстДиагностики = Параметры.КонтекстДиагностики;
	КонецЕсли;
	
	ОбновитьКешиОператоровЭДОИФорматов(КонтекстДиагностики);
	
	ПоместитьВоВременноеХранилище(КонтекстДиагностики, АдресРезультата);
КонецПроцедуры

// Запускает обновление кеша операторов ЭДО и форматов в фоновом задании.
// Не блокирует выполнение вызывающего кода, синхронизация выполняется асинхронно.
//
// Параметры:
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ОбновитьКешиОператоровЭДОИФорматовВФоне(КонтекстДиагностики = Неопределено) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне();
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	ПараметрыВыполнения.КлючФоновогоЗадания = "ОбновитьКешиОператоровЭДОИФорматов";
	
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("КонтекстДиагностики", КонтекстДиагностики);
	
	ДлительныеОперации.ВыполнитьВФоне("СинхронизацияЭДО.ОбновитьКешиОператоровЭДОИФорматовВФонеСервер",
