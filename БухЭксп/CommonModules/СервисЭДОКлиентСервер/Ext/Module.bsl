//@strict-types

#Область СлужебныйПрограммныйИнтерфейс

// Возвращает параметры уведомлений в сервисе 1С-ЭДО.
// 
// Возвращаемое значение:
// 	Структура:
// * АдресУведомлений - Строка
// * УведомлятьОСобытиях - Булево
// * УведомлятьОНовыхПриглашениях - Булево
// * УведомлятьОбОтветахНаПриглашения - Булево
// * УведомлятьОНовыхДокументах - Булево
// * УведомлятьОНеобработанныхДокументах - Булево
// * УведомлятьОбОкончанииСрокаДействияСертификата - Булево
Функция НовыеПараметрыУведомлений() Экспорт
	
	Возврат УчетныеЗаписиЭДОКлиентСервер.НовыеПараметрыУведомлений();
	
КонецФункции

// Возвращает ссылку на описание сервиса.
// 
// Возвращаемое значение:
// 	Строка - ссылка на описание.
Функция СсылкаНаОписаниеСервисаЭДО() Экспорт
	
	Возврат "https://portal.1c.ru/applications/30/#conditions";
	
КонецФункции

// Возвращает параметры регистрации в сервисе 1С-ЭДО.
// 
// Возвращаемое значение:
//  Структура - Описание:
//  * Идентификатор - Строка
//  * Организация - Неопределено,ОпределяемыйТип.Организация
//  * НаименованиеОрганизации - Строка - если не заполнено, данные по Наименованию, ИНН, КПП и ОГРН будут взяты из справочника Организации.
//  * ИННОрганизации - Строка
//  * КППОрганизации - Строка
//  * ОГРНОрганизации - Строка
//  * АдресОрганизацииЗначение - Строка
//  * АдресОрганизации - Строка
//  * ЭлектроннаяПочта - Строка
//  * КодНалоговогоОргана - Строка
//  * ИдентификаторОрганизации - Строка
//  * ОператорЭДО - Строка
//  * СертификатыПодписейОрганизации - Массив из СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//  * ОтборСертификатов - Массив из СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//  * Доверенности - Соответствие из КлючИЗначение:
//  ** Ключ - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//  ** Значение - СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  * СертификатПодписанта - Неопределено,СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - если не заполнен,
//                          формируется xml-файл, в котором вместо значений, зависящих от сертификата,
//                          указано [БудетЗаполненоПослеПодписания]. Такой файл предусмотрен для ознакомления 
//                          при подписании рег. пакета.
//  * ВидОперацииЭДО - Строка - см. УчетныеЗаписиЭДОКлиентСервер.ДействиеОбновлениеСертификата,
//                              см. УчетныеЗаписиЭДОКлиентСервер.ДействиеПодключениеЭДО
//  * НеИспользоватьПриглашения - Неопределено,Булево
//  * КодыТиповДокументовКОбмену - Массив из Строка
//  * Комментарий - Строка
//  * ВидУчетнойЗаписи - ПеречислениеСсылка.ВидУчетнойЗаписиЭДО
Функция НовыеПараметрыРегистрацииВСервисе1СЭДО() Экспорт
	
	ПараметрыРегистрации = Новый Структура;
	ПараметрыРегистрации.Вставить("Идентификатор", "");
	ПараметрыРегистрации.Вставить("Организация", Неопределено);
	ПараметрыРегистрации.Вставить("НаименованиеОрганизации", "");
	ПараметрыРегистрации.Вставить("ИННОрганизации", "");
	ПараметрыРегистрации.Вставить("КППОрганизации", "");
	ПараметрыРегистрации.Вставить("ОГРНОрганизации", "");
	ПараметрыРегистрации.Вставить("АдресОрганизации", "");
	ПараметрыРегистрации.Вставить("АдресОрганизацииЗначение", "");
	ПараметрыРегистрации.Вставить("ЭлектроннаяПочта", "");
	ПараметрыРегистрации.Вставить("КодНалоговогоОргана", "");
	ПараметрыРегистрации.Вставить("ИдентификаторОрганизации", "");
	ПараметрыРегистрации.Вставить("ОператорЭДО", "");
	ПараметрыРегистрации.Вставить("СертификатыПодписейОрганизации", Новый Массив);
	ПараметрыРегистрации.Вставить("ОтборСертификатов", Новый Массив);
	ПараметрыРегистрации.Вставить("Доверенности", Новый Соответствие);
	ПараметрыРегистрации.Вставить("СертификатПодписанта", Неопределено);
	ПараметрыРегистрации.Вставить("ВидОперацииЭДО", "");
	ПараметрыРегистрации.Вставить("НеИспользоватьПриглашения", Неопределено);
	ПараметрыРегистрации.Вставить("КодыТиповДокументовКОбмену", Новый Массив);
	ПараметрыРегистрации.Вставить("Комментарий", "");
	ПараметрыРегистрации.Вставить("ВидУчетнойЗаписи", ПредопределенноеЗначение("Перечисление.ВидУчетнойЗаписиЭДО.ПустаяСсылка"));
	
	Возврат ПараметрыРегистрации;
	
КонецФункции

// Сравнить версии форматов регистрации.
// 
// Параметры:
//  Версия1 - Строка - Версия1
//  Версия2 - Строка - Версия2
// 
// Возвращаемое значение:
//  Число - Числовая разница версий
//
Функция СравнитьВерсииФорматовРегистрации(Знач Версия1 = "0", Знач Версия2 = "0") Экспорт
	
	Версия1Числом = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрЗаменить(Версия1, ".", ""));
	Версия2Числом = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрЗаменить(Версия2, ".", ""));
	
	Возврат Версия1Числом - Версия2Числом;
	
КонецФункции

#КонецОбласти