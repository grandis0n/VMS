...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаменитьВладельцаКлючаИнтеграцииСЛК");
	Методы.Вставить("ОбработатьЗапускРегламентногоЗаданияПоОбменуССервисом");
...
Функция ВыполнитьОбменССервисомЛКАУСН() Экспорт
	...
	ЗаблокированныеДанные = ЗаблокированныеДанныеДляВыполненияОбмена(Истина);
	Если ЗаблокированныеДанные = Неопределено Тогда
		Возврат РезультатОбмена;
	КонецЕсли;
	...
	Для Каждого КлючЗаписи Из ЗаблокированныеДанные.КлючиСостоянийИнтеграции Цикл
		...
		Для Каждого ИмяМетода Из МассивМетодов Цикл
			...
			ПараметрыМетода.Вставить("Организация", КлючЗаписи.Организация);
		...
		КлючОбмена.Организация = КлючЗаписи.Организация;
...
Функция НовыйОписаниеОшибокОтправки() Экспорт
	...
	Ошибки.Колонки.Добавить("БанковскаяОперация", Новый ОписаниеТипов("Массив")); // Для статуса документа НДФЛ
...
Функция НовыеДанныеДляОтправкиНДФЛ() Экспорт
	...
	Данные.Колонки.Добавить("БанковскиеОперации", Новый ОписаниеТипов("ТаблицаЗначений")); // см. НовыйБанковскиеОперацииНДФЛ
	Данные.Колонки.Добавить("ДанныеВФорматеФНС", Новый ОписаниеТипов("Строка"));
	Возврат Данные;
...
//  ДокументыНДФЛАУСН - ТаблицаЗначений - см. ДокументыНДФЛАУСНОжидающиеОтправки
//
Процедура ПодготовитьРасчетыНДФЛАУСНВФорматеФНС(ДанныеДляОтправкиНДФЛ, Ошибки, ПрямаяВыгрузкаВФНС = Ложь, ДокументыНДФЛАУСН = Неопределено) Экспорт
	
	Если ДокументыНДФЛАУСН = Неопределено Тогда
		ДокументыНДФЛАУСН = ДокументыНДФЛАУСНОжидающиеОтправки(ПрямаяВыгрузкаВФНС);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ДокументыНДФЛАУСН) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ДокументыНДФЛАУСНКОтправке
		|ИЗ
		...
		|ГДЕ
		|	РасчетыНДФЛАУСН.Регистратор В
		|			(ВЫБРАТЬ
		|				ДокументыНДФЛАУСНКОтправке.Документ
		|			ИЗ
		|				ДокументыНДФЛАУСНКОтправке КАК ДокументыНДФЛАУСНКОтправке)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ДокументыНДФЛАУСН", ДокументыНДФЛАУСН);
	...
	НомерЗапросаОтчета       = 2;
	НомерЗапросаСуммыВычетов = 5;
	НомерЗапросаДанныеФЛ     = 6;
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Если ПрямаяВыгрузкаВФНС Тогда
			ИдентификаторБанка = "";
		Иначе
			Банк = РегистрыСведений.СостоянияИнтеграцииАУСН.БанкПередачиНДФЛИзПрограммы(Организация);
			ИдентификаторБанка = ИнтеграцияАУСНПовтИсп.ИдентификаторБанкаПоКлассификатору(Банк);
		КонецЕсли;
		
		ОтправленныеДокументы = НовыйБанковскиеОперацииНДФЛ();
		...
		Пока ВыборкаВедомость.Следующий() Цикл
			...
			Пока ВыборкаВыплата.Следующий() Цикл
				...
				Если Не ЗначениеЗаполнено(EmployeeInfo.id) Тогда
					...
					ПараметрыОшибки.БанковскаяОперация = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыборкаВыплата.Выплата);
		...
		Для Каждого ЭлементКОтправке Из КОтправке Цикл
			Если ВыплатыСОшибками.Найти(ЭлементКОтправке.Ключ) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			БанковскаяОперацияНДФЛ = ОтправленныеДокументы.Добавить();
			БанковскаяОперацияНДФЛ.Документ = ЭлементКОтправке.Ключ;
			БанковскаяОперацияНДФЛ.РеквизитыДокумента = РегистрыСведений.ДокументыНДФЛАУСН.НовыйРеквизитыДокумента(
				ЭлементКОтправке.Ключ, Организация);
			БанковскаяОперацияНДФЛ.РеквизитыДокумента.Статус = ?(ПрямаяВыгрузкаВФНС,
				Перечисления.СтатусыРасчетовНДФЛАУСН.ПереданВФНС, Перечисления.СтатусыРасчетовНДФЛАУСН.ПереданВБанк);
			Для Каждого ИнформацияВыплаты Из ЭлементКОтправке.Значение Цикл
				ОбъектЗапрос.EmployeeInfos.Добавить(ИнформацияВыплаты);
				БанковскаяОперацияНДФЛ.Идентификатор = ИнформацияВыплаты.id;
			КонецЦикла;
		КонецЦикла;
...
Функция ДокументыНДФЛАУСНОжидающиеОтправки(ПрямаяВыгрузкаВФНС)
	...
	|ГДЕ
	|	ДокументыНДФЛАУСН.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыРасчетовНДФЛАУСН.ОжидаетОтправки), ЗНАЧЕНИЕ(Перечисление.СтатусыРасчетовНДФЛАУСН.ОшибкаПриОтправке))
	|	И ДокументыНДФЛАУСН.Организация В
	|			(ВЫБРАТЬ
...
// Обработчик обновления. Заменяет владельца настройки, хранящей токен доступа к ЛК ФНС АУСН.
// Ранее владелец определялся наименованием организации, после замены определяется
// уникальным идентификатором организации
//
Процедура ЗаменитьВладельцаКлючаИнтеграцииСЛК() Экспорт
	...
	СхемаЗапроса.УстановитьТекстЗапроса(
		"ВЫБРАТЬ
		|	СостоянияИнтеграцииАУСН.Организация КАК Организация,
		|	ВРЕГ(СостоянияИнтеграцииАУСН.Организация.Наименование) КАК ПредставлениеОрганизации
		|ИЗ
		|	РегистрСведений.СостоянияИнтеграцииАУСН КАК СостоянияИнтеграцииАУСН
		|ГДЕ
		|	СостоянияИнтеграцииАУСН.Банк = ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)
		...
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(БезопасноеХранилищеДанных.Владелец) = ТИП(СТРОКА)
		|	И ВЫРАЗИТЬ(БезопасноеХранилищеДанных.Владелец КАК СТРОКА) ПОДОБНО &ШаблонИмениВладельца");
	
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ЗапросКХранилищуДанных = СхемаЗапроса.ПакетЗапросов[1];
		Оператор = ЗапросКХранилищуДанных.Операторы[0];
		ИмяРегистра = "БезопасноеХранилищеДанныхОбластейДанных";
		РегистрХранилищаОбластейДанных = ЗапросКХранилищуДанных.ДоступныеТаблицы.Найти(
			СтрШаблон("РегистрСведений.%1", ИмяРегистра));
		Оператор.Источники.Заменить(0, РегистрХранилищаОбластейДанных);
	КонецЕсли;
	Запрос = Новый Запрос;
	ПрефиксИмениВладельца = ВладелецДанныхБезопасногоХранилищаИнтеграцииСЛК("");
	ШаблонИмениВладельца = СтрШаблон("%1%%", ПрефиксИмениВладельца);
	Запрос.УстановитьПараметр("ШаблонИмениВладельца", ШаблонИмениВладельца);
	Запрос.Текст = СхемаЗапроса.ПолучитьТекстЗапроса();
	РезультатВыполнения = Запрос.ВыполнитьПакет();
	Организации = РезультатВыполнения[0].Выгрузить();
	Если Не ЗначениеЗаполнено(Организации) Тогда
		Возврат;
	КонецЕсли;
	ВладельцыДанных = РезультатВыполнения[1].Выгрузить();
	ВладельцыДанных.Колонки.Добавить("ПредставлениеОрганизации");
	Для Каждого СтрокаВладелец Из ВладельцыДанных Цикл
		ОбработаннаяСтрока = СтрЗаменить(СтрокаВладелец.Владелец, ПрефиксИмениВладельца, "");
		Если ЗначениеЗаполнено(ОбработаннаяСтрока) Тогда
			СтрокаВладелец.ПредставлениеОрганизации = ВРег(ОбработаннаяСтрока);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаОрганизация Из Организации Цикл
		
		НайденнаяСтрока = ВладельцыДанных.Найти(СтрокаОрганизация.ПредставлениеОрганизации, "ПредставлениеОрганизации");
		Если НайденнаяСтрока = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Токен = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
			НайденнаяСтрока.Владелец, КлючДанныхТокенБезопасногоХранилища());
		Если ЗначениеЗаполнено(Токен) Тогда
			ВладелецПоИдентификатору = ВладелецДанныхБезопасногоХранилищаИнтеграцииСЛК(
				СтрокаОрганизация.Организация);
			ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
				ВладелецПоИдентификатору, Токен, КлючДанныхТокенБезопасногоХранилища());
		КонецЕсли;
...
//  БанковскиеОперации - ТаблицаЗначений - см. НовыйБанковскиеОперацииНДФЛ
// 
// Возвращаемое значение:
//  Структура - НовыйОтвет()
//
Функция ОтправитьРасчетыНДФЛВФНС(ДокументКОтправке, БанковскиеОперации)
	...
	ДокументОтвет = ЗаписатьОтветВДокументыАУСН(ИмяМетода, ОтветСервиса.ТелоОтвета, ДокументКОтправке.Организация);
	...
	Если Выполнено Тогда
		...
		ИмяВременногоКаталога = ПолучитьИмяВременногоФайла();
		ДанныеДокумента = РасшифроватьДанныеДокумента(ДокументОтвет, ИмяВременногоКаталога);
		УдалитьФайлы(ИмяВременногоКаталога);
		ИнтеграцияАУСНБП.ОбработатьОтветНаОтправленныйРасчетНДФЛ(МенеджерЗаписи,
			ДокументОтвет, ДанныеДокумента, БанковскиеОперации);
	Иначе
...
Процедура ЗаписатьСостояниеИнтеграцииЛКАУСН(ОтветСервиса, Организация)
	...
	Если СтрНайти(ВРег(ОтветСервиса.ТелоОтвета), "TAXPAYER_ALREADY_BOUND") Тогда
		РегистрыСведений.СостоянияИнтеграцииАУСН.ЗаписатьСостояниеИнтеграцииЛКАУСН(
...
Функция ВладелецДанныхБезопасногоХранилищаИнтеграцииСЛК(Организация)
	
	ПрефиксВладельца = "БухгалтерияПредприятия.ИнтеграцияАУСНСЛК.";
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Возврат ПрефиксВладельца;
	КонецЕсли;
	ИдентификаторСтрокой = Строка(Организация.УникальныйИдентификатор());
	Возврат СтрШаблон("%1%2", ПрефиксВладельца, ИдентификаторСтрокой);
...
Процедура ОтправитьРасчетыНДФЛАУСНВБанк(ДополнительныеПараметры, ПрямаяВыгрузкаВФНС = Ложь)
	...
	ДокументыНДФЛАУСНКОтправке = ДокументыНДФЛАУСНОжидающиеОтправки(ПрямаяВыгрузкаВФНС);
	Если Не ЗначениеЗаполнено(ДокументыНДФЛАУСНКОтправке) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеДляОтправкиНДФЛ = НовыеДанныеДляОтправкиНДФЛ();
	
	// Блокируем записи в регистре "ДокументыНДФЛАУСН", по которым производится отправка расчета:
	// Отправка расчета приводит к изменению статуса.
	// В свою очередь, статус может поменяться при перепроведении банковской операции.
	...
	Попытка
		...
		ЭлементБлокировкиДанных.ИсточникДанных = ДокументыНДФЛАУСНКОтправке;
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Документ", "Документ");
		БлокировкаДанных.Заблокировать();
		
		ПодготовитьРасчетыНДФЛАУСНВФорматеФНС(ДанныеДляОтправкиНДФЛ,
			ДополнительныеПараметры.ОписаниеОшибокОтправки, ПрямаяВыгрузкаВФНС, ДокументыНДФЛАУСНКОтправке);
		...
		Для Каждого СтрокаДанных Из ДанныеДляОтправкиНДФЛ Цикл
			...
			Если ПрямаяВыгрузкаВФНС Тогда
				ОтветСервиса = ОтправитьРасчетыНДФЛВФНС(МенеджерЗаписи, ОтправленныеДокументы);
				Если ОтветСервиса.Статус = ИнтеграцияАУСНКлиентСервер.СтатусыЗапросов().Выполнено Тогда
					КлючОбмена = ДополнительныеПараметры.ОтправленныеДанные.Добавить();
			...
			Если ЗначениеЗаполнено(ОтправленныеДокументы) Тогда
				Для Каждого БанковскаяОперацияНДФЛ Из ОтправленныеДокументы Цикл
					БанковскаяОперацияНДФЛ.РеквизитыДокумента.Идентификатор = БанковскаяОперацияНДФЛ.Идентификатор;
					РегистрыСведений.ДокументыНДФЛАУСН.ИзменитьДокумент(БанковскаяОперацияНДФЛ.РеквизитыДокумента);
				КонецЦикла;
			КонецЕсли;
		
		КонецЦикла;
		
		Если ЗначениеЗаполнено(ДополнительныеПараметры.ОписаниеОшибокОтправки) Тогда
			ДокументыСОшибкамиОтправки = Новый ТаблицаЗначений;
			ДокументыСОшибкамиОтправки.Колонки.Добавить("Документ");
			ДокументыСОшибкамиОтправки.Колонки.Добавить("Организация");
			Для Каждого СтрокаОшибка Из ДополнительныеПараметры.ОписаниеОшибокОтправки Цикл
				Для Каждого БанковскаяОперация Из СтрокаОшибка.БанковскаяОперация Цикл
					Если ДокументыСОшибкамиОтправки.Найти(БанковскаяОперация, "Документ") = Неопределено Тогда
						ДокументСОшибкой = ДокументыСОшибкамиОтправки.Добавить();
						ДокументСОшибкой.Документ = БанковскаяОперация;
						ДокументСОшибкой.Организация = СтрокаОшибка.Организация;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			Для Каждого ДокументСОшибкой Из ДокументыСОшибкамиОтправки Цикл
				РеквизитыДокументаНДФЛ = РегистрыСведений.ДокументыНДФЛАУСН.НовыйРеквизитыДокумента(
					ДокументСОшибкой.Документ, ДокументСОшибкой.Организация);
				РеквизитыДокументаНДФЛ.Статус = Перечисления.СтатусыРасчетовНДФЛАУСН.ОшибкаПриОтправке;
				РегистрыСведений.ДокументыНДФЛАУСН.ИзменитьДокумент(РеквизитыДокументаНДФЛ);
			КонецЦикла;
		КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось отправить расчеты НДФЛ АУСН по причине:
			|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки);
	КонецПопытки;
...
Процедура ЗаписатьСостояниеОбмена(Параметры)
	...
	Для Каждого СтрокаПолученныйОтвет Из ПолучившиеОтветСервиса Цикл
		Если Не ЕстьОшибки(КлючиНастроекСОшибкамиОтправки, СтрокаПолученныйОтвет) Тогда
...
Функция ЗаблокированныеДанныеДляВыполненияОбмена(ПрямойОбменСФНС = Ложь)
	...
	|	СостоянияИнтеграцииАУСН.Организация КАК Организация,
	|	СостоянияИнтеграцииАУСН.Банк КАК Банк,
	|	ЕСТЬNULL(БанкиАУСН.Идентификатор, """") КАК Идентификатор
	|ИЗ
	|	РегистрСведений.СостоянияИнтеграцииАУСН КАК СостоянияИнтеграцииАУСН
	...
	|	И НЕ СостоянияИнтеграцииАУСН.Организация.ВАрхиве
	|ИТОГИ ПО
	|	Организация,
	|	Идентификатор");
	
	СхемаЗапросаОтбор = СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Отбор;
	Если ПрямойОбменСФНС Тогда
		УсловиеБанк = "СостоянияИнтеграцииАУСН.Банк = ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)";
		СхемаЗапросаОтбор.Добавить(УсловиеБанк);
	Иначе
		УсловиеБанк = "СостоянияИнтеграцииАУСН.Банк <> ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)";
		СхемаЗапросаОтбор.Добавить(УсловиеБанк);
		УсловиеБанкАУСН = "НЕ БанкиАУСН.Идентификатор ЕСТЬ NULL";
		СхемаЗапросаОтбор.Добавить(УсловиеБанкАУСН);
	КонецЕсли;
...
Процедура ЗаполнитьОшибкуОтправкиПоВыборке(Ошибки, Выборка, ИмяРеквизита, Синоним, ТекстОшибки, ЕстьОшибки)
	...
	ПараметрыОшибки.БанковскаяОперация = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Выборка.БанковскаяОперация);
	РеквизитОшибка = ПараметрыОшибки.Реквизиты.Добавить();
	РеквизитОшибка.ИмяРеквизита = СтрШаблон("%1%2",
...
Процедура ДобавитьОшибкуОтправки(Ошибки, ПараметрыЗаполнения)
	...
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(СтрокаОшибка, ПараметрыЗаполнения, , "Реквизиты, БанковскаяОперация");
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтрокаОшибка.БанковскаяОперация,
		ПараметрыЗаполнения.БанковскаяОперация, Истина);
...
Функция ВедомостьИДвиженияРасчетыНДФЛСоответствуютПротоколуОбмена(ВыборкаВыплата, Организация, ИдентификаторБанка, Ошибки)
	...
	Если Не ЗначениеЗаполнено(ВыборкаВыплата.ДатаПлатежа) Тогда
		...
		ПараметрыОшибки.БанковскаяОперация = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Выплата);
		РеквизитОшибка = ПараметрыОшибки.Реквизиты.Добавить();
		РеквизитОшибка.ИмяРеквизита = СтрШаблон(
...
Функция НовыйБанковскиеОперацииНДФЛ()
	
	БанковскиеОперацииНДФЛ = Новый ТаблицаЗначений;
	БанковскиеОперацииНДФЛ.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.СписаниеСРасчетногоСчета"));
	БанковскиеОперацииНДФЛ.Колонки.Добавить("Идентификатор", ОбщегоНазначения.ОписаниеТипаСтрока(
		ОбщегоНазначенияБПКлиентСервер.ДлинаИдентификатора()));
	БанковскиеОперацииНДФЛ.Колонки.Добавить("РеквизитыДокумента", Новый ОписаниеТипов("Структура"));// см. РегистрСведений.ДокументыНДФЛАУСН.НовыйРеквизитыДокумента
	Возврат БанковскиеОперацииНДФЛ;
