...
//  ЗанятыеИменаФайлов - Соответствие Из КлючИЗначение - возвращаемый параметр, пополняется при каждом вызове:
//                       * Ключ - Строка - имя файла в верхнем регистре.
//                       * Значение - Булево - признак занятости имени файла.
//  ЭтоКаталог - Булево - Истина, если это каталог.
//  
//  Возвращаемое значение:
//   Строка - Уникальное имя файла.
//
Функция ОпределитьУникальноеИмяФайла(Знач ИсходноеИмяФайла, ЗанятыеИменаФайлов, Знач ЭтоКаталог = Ложь) Экспорт
	...
	Пока Истина Цикл
		...
		// Проверка существования файла на диске на регистрозависимых файловых системах (Linux/ext4)
		// не считает дублями имена, различающиеся только регистром, тогда как ЗаписьZIPФайла
		// сравнивает имена в архиве без учета регистра.
		Если ЗанятыеИменаФайлов[ВРег(ИмяФайла)] = Неопределено
			И Не Файл.Существует() Тогда
			Прервать;
		КонецЕсли;
	...
	ЗанятыеИменаФайлов.Вставить(ВРег(ИмяФайла), Истина);
	Возврат ИмяФайла;
