...
Процедура ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, ОстаткиАвансов, СтрокаДокумента, Реквизиты)
	...
	Для каждого СтрокаОстатка Из ОстаткиАвансов Цикл
		...
		ИначеЕсли СтрокаВзаиморасчетов.РасчетыВВалюте Тогда
...
Процедура ЗаполнитьПараметрыВзаимозачета(СтрокаВзаимозачета, СтрокаДебиторскойЗадолженности, СтрокаКредиторскойЗадолженности, Реквизиты)
	...
	СтрокаВзаимозачета.КорСуммаВзаиморасчетовКомитента =
		Окр(СтрокаКредиторскойЗадолженности.СуммаВзаиморасчетовКомитента * Коэффициент, 2);
	СтрокаВзаимозачета.КорСуммаБУ_Комитента = Окр(СтрокаКредиторскойЗадолженности.СуммаБУ_Комитента * Коэффициент, 2);
...
Процедура СформироватьДвиженияВзаимозачет(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаВзаиморасчетов из Параметры.Взаиморасчеты Цикл
		...
		Если Реквизиты.УчитыватьЗадолженностьУСН
			...
			// Проводка Дт УСН.02 (УСН.22) - если ДтКтКорСчета = "Дт"
			// или Кт УСН.02 (УСН.22) - если ДтКтКорСчета = "Кт"
			Если Реквизиты.УчитыватьЗадолженностьУСН
				И СтрокаВзаиморасчетов.КорСуммаВзаиморасчетовКомитента <> 0 Тогда
				
				Проводка = Движения.Хозрасчетный.Добавить();
				
				Проводка.Период      = Реквизиты.Период;
				Проводка.Организация = Реквизиты.Организация;
				Проводка.Содержание  = НСтр("ru = 'Расчеты за товары и услуги комитента'");
				...
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтКорСчета],
					Проводка["Субконто" + ДтКтКорСчета], "Контрагенты", СтрокаВзаиморасчетов.КорСубконто1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтКорСчета],
					Проводка["Субконто" + ДтКтКорСчета], "Договоры", СтрокаВзаиморасчетов.КорСубконто2);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтКорСчета],
					Проводка["Субконто" + ДтКтКорСчета],
					"ДокументыРасчетовСКонтрагентами",
					СтрокаВзаиморасчетов.КорСубконто3);
				
				СвойстваСчетаУСН = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(
					Проводка["Счет" + ДтКтКорСчета]);
				Если СвойстваСчетаУСН.Валютный Тогда
					Проводка["Валюта" + ДтКтКорСчета]        = СтрокаВзаиморасчетов.КорВалютаВзаиморасчетов;
					Проводка["ВалютнаяСумма" + ДтКтКорСчета] = СтрокаВзаиморасчетов.КорСуммаВзаиморасчетовКомитента;
				КонецЕсли;
				Если СвойстваСчетаУСН.УчетПоПодразделениям Тогда
					Проводка["Подразделение" + ДтКтКорСчета] = СтрокаВзаиморасчетов.КорПодразделение;
				КонецЕсли;
				
				Проводка.Сумма = СтрокаВзаиморасчетов.КорСуммаБУ_Комитента;
...
Функция ПодготовитьПараметрыВзаимозачета(ТаблицаВзаиморасчетов, ТаблицаРеквизиты)
	...
	+ "СуммаВзаиморасчетовТорговыйСбор," // <Число,15,2> - сумма по деятельности на торговом сборе в валюте взаиморасчетов
	+ "СуммаБУ_ТорговыйСбор,"            // <Число,15,2> - сумма по деятельности на торговом сборе в рублях
	+ "КорСуммаВзаиморасчетовКомитента," // <Число,15,2> - сумма за товары и услуги комитентов в валюте взаиморасчетов
	+ "КорСуммаБУ_Комитента,"            // <Число,15,2> - сумма за товары и услуги комитентов в рублях
	+ "Содержание";                      // <Строка,150>
...
Функция ПустаяТаблицаВзаимозачет() Экспорт
	...
	ПустаяТаблица.Колонки.Добавить("СуммаБУ_ТорговыйСбор",            ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ПустаяТаблица.Колонки.Добавить("Содержание",                      ОбщегоНазначения.ОписаниеТипаСтрока(150));
	ПустаяТаблица.Колонки.Добавить("КорСуммаВзаиморасчетовКомитента", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ПустаяТаблица.Колонки.Добавить("КорСуммаБУ_Комитента",            ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
