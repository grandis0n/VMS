...
Процедура ДобавитьВНастройкиДанныеРасшифровки(ДанныеРасшифровки, ИдентификаторРасшифровки, Настройки, РеквизитыРасшифровки) Экспорт
	...
	ИсключитьНедоступныеОтборыИзПолейРасшифровки(ПоляРасшифровки);
...
Процедура ИсключитьНедоступныеОтборыИзПолейРасшифровки(ПоляРасшифровки)
	
	ПолеКорСчет = ПоляРасшифровки["КорСчет"];
	ПолеКорПодразделение = ПоляРасшифровки["КорПодразделение"];
	ПолеСчет = ПоляРасшифровки["Счет"];
	ПолеПодразделение = ПоляРасшифровки["Подразделение"];

	Если ПолеКорСчет <> Неопределено И ПолеКорПодразделение <> Неопределено Тогда
		
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПолеКорСчет.Значение);
		Если Не СвойстваСчета.УчетПоПодразделениям Тогда
			ПоляРасшифровки.Удалить("КорПодразделение");
		КонецЕсли;
		
	КонецЕсли;
	
	Если ПолеСчет <> Неопределено И ПолеПодразделение <> Неопределено Тогда
		
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПолеСчет.Значение);
		Если Не СвойстваСчета.УчетПоПодразделениям Тогда
			ПоляРасшифровки.Удалить("Подразделение");
		КонецЕсли;
...
Процедура ПоменятьНумерациюСубконтоВОтборе(Настройки, НовыйСчет, КартаВидовСубконто) Экспорт
	...
	Если ЭлементыОтбораПодЗамену.Количество() > 0 Тогда
		...
		Для Каждого ЭлементЗамены Из ЭлементыОтбораПодЗамену Цикл
			...
			Если ЭлементЗамены.ПравоеЗначение = Неопределено Тогда
				// Тип правого значения определить нельзя - подбор невозможен.

			ИначеЕсли ЛевоеЗначениеПуть.Количество() = 1 Тогда
				// Отбор непосредственно по субконто - подбираем по типу значения.
			...
			Иначе
				// Отбор по реквизиту субконто: СубконтоN.<Реквизит>.
				// Подбираем по наличию реквизита с таким именем в виде субконто нового счёта.

				ИмяРеквизита = ЛевоеЗначениеПуть[1];

				Для Каждого ВидСубконто Из ВидыСубконто Цикл

					Если ВидСубконтоИмеетРеквизит(ВидСубконто.Ключ, ИмяРеквизита) Тогда
						ПоляПодходящегоТипа.Добавить(ПутьПоляСНовымНомеромСубконто(ЛевоеЗначениеПуть, ВидСубконто.Значение));
					КонецЕсли;
...
// Проверяет, существует ли в виде субконто реквизит (включая реквизиты табличных частей) с указанным именем.
//
// Параметры:
//  ВидСубконто	 - ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные	 - Вид субконто, в котором ищем реквизит.
//  ИмяРеквизита	 - Строка												 - Имя реквизита для поиска.
//
// Возвращаемое значение:
//   Булево - Истина, если реквизит существует.
//
Функция ВидСубконтоИмеетРеквизит(ВидСубконто, ИмяРеквизита)
	...
	Для Каждого Тип Из ВидСубконто.ТипЗначения.Типы() Цикл
		...
		Если МетаданныеОбъектаИмеетРеквизит(МетаданныеОбъекта, ИмяРеквизита) Тогда
			Возврат Истина;
		КонецЕсли;
...
// Проверяет, существует ли в объекте метаданных реквизит (включая стандартные реквизиты и реквизиты табличных частей).
//
// Параметры:
//  МетаданныеОбъекта	 - ОбъектМетаданных	 - Объект метаданных, в котором ищем реквизит.
//  ИмяРеквизита		 - Строка		 - Имя реквизита для поиска.
//
// Возвращаемое значение:
//   Булево - Истина, если реквизит существует.
//
Функция МетаданныеОбъектаИмеетРеквизит(МетаданныеОбъекта, ИмяРеквизита)
	...
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта(ИмяРеквизита, ТабличнаяЧасть) Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция ВидСравненияДляПоляРасшифровки(ПолеРасшифровки)
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ЗначениеПоляРасшифровки) Тогда
		Если ТипЗнч(ЗначениеПоляРасшифровки) = Тип("Неопределено")
			Или ТипЗнч(ЗначениеПоляРасшифровки) = Тип("Null") Тогда
