...
Процедура ПриОпределенииИспользуемыхТиповЭлектронныхДокументов(ИспользуемыеТипы) Экспорт
	...
	ОбменСКонтрагентами.ДобавитьИспользуемыйТипДокумента(ИспользуемыеТипы, ТипыДокументов.ЗаказТовара);
...
Процедура СписокТиповДокументовПоТипуЭлектронногоДокумента(ТипДокумента, ОписанияОбъектовУчета) Экспорт
	...
	ИначеЕсли ТипДокумента = ТипыДокументов.ЗаказТовара Тогда
		...
		ОписанияОбъектовУчета.Добавить(Документы.СчетНаОплатуПокупателю.ПустаяСсылка(),
...
Процедура ПослеФормированияЭлектронногоДокумента(ЭлектронныйДокумент) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП")
		...
		Если СчетаФактуры <> Неопределено Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Элемент = Неопределено Тогда
					Элемент = Новый Массив;
				КонецЕсли;
				Если Элемент.Найти(Выборка.ДокументОснование) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыКПроверке, Метаданные.ОпределяемыеТипы.ОснованиеОтгрузкаТоваровИСМП.Тип.Типы()		, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыКПроверке, Метаданные.ОпределяемыеТипы.ОснованиеВыводаИзОборотаИСМП.Тип.Типы()		, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыКПроверке, Метаданные.ОпределяемыеТипы.ОснованиеРазрешениеНаОтгрузкуИСМП.Тип.Типы(), Истина);
...
// Обновляет сведения организации
// 
// Параметры:
//  ДанныеОрганизации - см. ОбменСКонтрагентамиКлиентСервер.НовыйДанныеОрганизации
Процедура ОбновитьДанныеОрганизации(ДанныеОрганизации) Экспорт
...
Процедура СсылкаНаОбъектПоИННКПП(ТипОбъекта, ИНН, КПП, Ссылка, ДатаСведений = Неопределено, ЮрФизЛицо = "") Экспорт
...
// Устанавливает признак того что в прикладном решении поддержана работа с физическими лицами
//
// Параметры:
//  ПоддержанаРабота - Булево
Процедура ПоддержанаРаботаСФизическимиЛицами(ПоддержанаРабота) Экспорт
	
	ПоддержанаРабота = Истина;
...
Процедура ПредставлениеНеСформированногоДокумента(ПредставлениеДокумента, ДанныеОбъекта, СтандартнаяОбработка) Экспорт
	
	Если ТипЗнч(ДанныеОбъекта.ОбъектУчета) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах")
			И ДанныеОбъекта.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД Тогда
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Продавец", Справочники.Контрагенты.ПустаяСсылка());
		СчетФактура = УчетНДСПереопределяемый.НайтиПодчиненныйСчетФактуруВыданныйНаРеализацию(ДанныеОбъекта.ОбъектУчета,,, СтруктураОтбора);
		Если ЗначениеЗаполнено(СчетФактура) Тогда
			СтандартнаяОбработка = Ложь;
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СчетФактура, "Номер, Дата");
			ПредставлениеДокумента = СтрШаблон(НСтр("ru = 'УПД № %1 от %2'"),
				ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ЗначенияРеквизитов.Номер, Истина, Ложь), Формат(ЗначенияРеквизитов.Дата, "ДЛФ=D"));
		КонецЕсли;
	КонецЕсли;
