...
Функция ЗаполнитьПараметрыОтчетаИзНастроекКомпоновщика(ОтчетОбъект, КомпоновщикНастроекКД)
	...
	МенеджерОтчета = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОтчетОбъект.Метаданные().ПолноеИмя());
	НастройкиКомпоновкиДанных = КомпоновщикНастроекКД.ПолучитьНастройки();
	...
	ЭтоСтандартнаяРассылкаОтчетов = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		КомпоновщикНастроекКД.ПользовательскиеНастройки.ДополнительныеСвойства,
		"СтандартнаяРассылкаОтчетов",
		Ложь);
	Если ЭтоСтандартнаяРассылкаОтчетов Тогда
		ПараметрыОтчета.Вставить("НастройкиКомпоновкиДанных", НастройкиКомпоновкиДанных);
	КонецЕсли;
