...
Процедура ЗаписатьСобытиеДокументооборотаСКО(Запись) Экспорт
	...
	Если МенеджерЗаписи.Выбран() 
		...
		МенеджерЗаписи.ПредставлениеПериода = Запись.ПредставлениеПериода;
...
Функция ПолучитьОтправленоПолученоНовогоСобытияДокументооборотаСКО(Запись)
	...
	Если Запись.Страница = Перечисления.СтраницыЖурналаОтчетность.Входящие
		...
		ИначеЕсли ТипЗнч(Запись.Объект) = Тип("ДокументСсылка.ОбращениеСФРКСтрахователю") Тогда
			Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Объект, "ДатаЗагрузки");
...
Функция НовоеДеревоНовыхСобытий() Экспорт
	...
	Результат.Колонки.Добавить("ИсходнаяГруппа",
		Новый ОписаниеТипов("ПеречислениеСсылка.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами"));
	Результат.Колонки.Добавить("ПредставлениеПериода", ТипСтрока);
...
Процедура ЗаполнитьДеревоНовое(
	...
	Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.НовыеСобытияПоСтраницамЖурнала()
		...
		Запрос.Текст =
			...
			|			ТОГДА 6
			|		КОГДА ЖурналНовыхСобытий.СтраницаЖурнала = ЗНАЧЕНИЕ(Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ)
			|				И (НЕ ЖурналНовыхСобытий.Просмотрено ИЛИ &ВключатьПросмотренные)
			|			ТОГДА 7
			...
			|	ЖурналНовыхСобытий.ПредставлениеПериода КАК ПредставлениеПериода
			|ИЗ
			|	РегистрСведений.ЖурналНовыхСобытий КАК ЖурналНовыхСобытий
		...
		Если ИнициаторСеанса <> Перечисления.ИнициаторыСеансовСвязиСКонтролирующимиОрганами.Автообмен Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "УПОРЯДОЧИТЬ ПО",
				...
				|	NULL,
				|	NULL,
				|	ЗНАЧЕНИЕ(Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ),
				|	NULL,
				|	NULL,
				|	NULL,
				|	NULL
	...
	Иначе
		Запрос.Текст =
			...
			|	ЖурналНовыхСобытий.ЕстьКритическиеОшибкиОтправки,
			|	ЗНАЧЕНИЕ(Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения),
			|	ЖурналНовыхСобытий.ПредставлениеПериода
			|ИЗ
			|	РегистрСведений.ЖурналНовыхСобытий КАК ЖурналНовыхСобытий
			|ГДЕ
			|	(&ИспользоватьОтборИнициаторСеанса
...
Процедура ЗаполнитьСтрокиДерева(
	...
	Пока ВыборкаДетали.Следующий() Цикл
		Если ЗначениеЗаполнено(ВыборкаДетали.Объект) Тогда
			...
			ЭлементДерева.ИсходнаяГруппа = ВыборкаДетали.ИсходнаяГруппа;
			ЭлементДерева.ПредставлениеПериода = ВыборкаДетали.ПредставлениеПериода;
...
Функция ЭлементДерева()
	...
	КлючиСтруктуры = "Дата, Наименование, Ссылка, КонтролирующийОрган, Статус, Организация,
		...
		|ЕстьКритическиеОшибкиОтправки, ИсходнаяГруппа, ПредставлениеПериода";
