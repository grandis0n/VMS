	...
	Если ОписаниеТаблиц.Свойство("Раздел2Доходы") Или ОписаниеТаблиц.Свойство("Раздел2Налоги2024") Или ОписаниеТаблиц.Свойство("КоличествоПолучателейДохода") Или ОписаниеТаблиц.Свойство("Раздел2Вычеты") Или ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			Если ИскатьВычеты И ТаблицаВычетов.Количество() > 100 Тогда
				ТаблицаВычетов.Индексы.Добавить("ФизическоеЛицо,НалоговаяБаза");
			КонецЕсли;
			ПоискВычетов = Новый Структура("ФизическоеЛицо,НалоговаяБаза");
			...
			Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
				Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Тогда
					...
					КонецЕсли;
				КонецЕсли;
				// Стандартные, имущественные вычеты учитываем общей суммой нарастающим итогом.
				ОблагаемыйДоходСледующегоМесяца = Макс(ОблагаемыйДоходСНачалаГода + СтрокаТаблицы.ОблагаемыйДоход - ПредоставленныеВычеты, 0);
				ПредоставленныеВычеты = Макс(ПредоставленныеВычеты - СтрокаТаблицы.ОблагаемыйДоход, 0);
				СтрокаТаблицы.ЭтоДеньПревышенияПредела1 = ОблагаемыйДоходСНачалаГода < Предел1 И ОблагаемыйДоходСледующегоМесяца > Предел1;
				Если Предел1 <> Предел2 Тогда
					СтрокаТаблицы.ЭтоДеньПревышенияПредела2 = ОблагаемыйДоходСНачалаГода < Предел2 И ОблагаемыйДоходСледующегоМесяца > Предел2;
					Если Предел2 <> Предел3 Тогда
						СтрокаТаблицы.ЭтоДеньПревышенияПредела3 = ОблагаемыйДоходСНачалаГода < Предел3 И ОблагаемыйДоходСледующегоМесяца > Предел3;
						Если Предел3 <> Предел4 Тогда
							СтрокаТаблицы.ЭтоДеньПревышенияПредела4 = ОблагаемыйДоходСНачалаГода < Предел4 И ОблагаемыйДоходСледующегоМесяца > Предел4;
						Иначе 
				...
				КонецЕсли;
				ОблагаемыйДоходСНачалаГода = ОблагаемыйДоходСледующегоМесяца;
				СтрокаТаблицы.ОблагаемыйДоходСНачалаГода = ОблагаемыйДоходСледующегоМесяца;
				Если Не (СтрокаТаблицы.ЭтоДеньПревышенияПредела1 Или СтрокаТаблицы.ЭтоДеньПревышенияПредела2 Или СтрокаТаблицы.ЭтоДеньПревышенияПредела3 Или СтрокаТаблицы.ЭтоДеньПревышенияПредела4) Тогда
					СтрокаТаблицы.Предел1Превышен = ОблагаемыйДоходСледующегоМесяца > Предел1;
					Если Предел1 <> Предел2 Тогда
						СтрокаТаблицы.Предел2Превышен = ОблагаемыйДоходСледующегоМесяца > Предел2;
						Если Предел2 <> Предел3 Тогда
							СтрокаТаблицы.Предел3Превышен = ОблагаемыйДоходСледующегоМесяца > Предел3;
							Если Предел3 <> Предел4 Тогда
								СтрокаТаблицы.Предел4Превышен = ОблагаемыйДоходСледующегоМесяца > Предел4;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
