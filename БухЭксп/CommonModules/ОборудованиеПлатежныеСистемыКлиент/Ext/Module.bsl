...
// Начать получение статус операции.
//
// Параметры:
//   ОповещениеПриЗавершении - ОписаниеОповещения - оповещение при завершении.
//   ИдентификаторКлиента    - ФормаКлиентскогоПриложения -идентификатор формы.
...
//   ДополнительныеПараметры - Структура - дополнительные команды.
//
Асинх Процедура НачатьПолучитьСтатусОперации(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства = Неопределено, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	КонтекстОперации.ПараметрыОперации       = ПараметрыОперации;
	КонтекстОперации.ДополнительныеПараметры = ДополнительныеПараметры; 
	КонтекстОперации.ТипТранзакции           = "OperationStatus"; 
	КонтекстОперации.ПодготовитьДанные       = Ложь; 
	КонтекстОперации.ОбработатьДанные        = Истина;
	КонтекстОперации.ПодготовитьДанныеКлиент = Истина;
	КонтекстОперации.ОбработатьДанныеКлиент  = Ложь;
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации, ДанныеОперации) Экспорт
	...
	Если Команда <> "OperationStatus" Тогда
		Если Команда = "PayElectronicCertificate" Или Команда = "ReturnElectronicCertificate" Тогда
			Если ПустаяСтрока(ПараметрыОперации.ИдентификаторКорзины) 
				Или (ПараметрыОперации.СуммаЭлектронногоСертификата = 0 И ПараметрыОперации.СкидкаЭлектроннымиСертификатами = 0)  Тогда
	...
	Если Команда <> "OperationStatus" Тогда  
		ПараметрыОперации.ИдентификаторОперацииНаОборудовании = ИдентификаторОперацииНаОборудовании();    
	КонецЕсли;
...
// Генеририровать идентификатор операции на Оборудовании
// Целое число размерностью 4 байта, представлено как HEX-строка длинной 8 символом ASCII без «0x»
//          
// Возвращаемое значение:
//   Строка - 
//
Функция ИдентификаторОперацииНаОборудовании() 
	Генератор = Новый ГенераторСлучайныхЧисел();   
	ЗначенияЧисла = Генератор.СлучайноеЧисло(100);
	Возврат МенеджерОборудованияКлиентСервер.ЧислоВСтрокуHEX(ЗначенияЧисла);
