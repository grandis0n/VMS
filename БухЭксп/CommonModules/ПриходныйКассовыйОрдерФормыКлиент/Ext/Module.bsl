...
Процедура РасшифровкаПлатежаПриИзменении(Форма, Элемент) Экспорт
	...
	ПриходныйКассовыйОрдерФормыКлиентСервер.УстановитьНаличиеУслугНПД(Форма);
	ПриходныйКассовыйОрдерФормыКлиентСервер.ПоказатьПредупреждениеПечатьЧека(Форма);
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		Если Форма.ОтражениеДоходаВидимость Тогда
			СтрокаПлатеж.ОтражениеДоходаПредставление = БанкИКассаФормыКлиентСервер.ВариантОтраженияДоходовПредставление(
				...
				БанкИКассаФормыКлиентСервер.ВариантыОтраженияДоходов(Форма));
		КонецЕсли;
		Форма.ЕстьУслугиНПД = Форма.ЕстьУслугиНПД Или Не ЗначениеЗаполнено(СтрокаПлатеж.СчетНаОплату)
			Или БанкИКассаФормыКлиентСервер.ЧекНПДСформирован(Форма) И ЗначениеЗаполнено(СтрокаПлатеж.УслугаНПД);
	КонецЕсли;
...
Процедура РасшифровкаПлатежаСчетНаОплатуПриИзменении(Форма, Элемент) Экспорт
	
	СтрокаПлатеж = ПриходныйКассовыйОрдерФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма);
	Если ЗначениеЗаполнено(СтрокаПлатеж.СчетНаОплату) И Не БанкИКассаФормыКлиентСервер.ЧекНПДСформирован(Форма) Тогда
		СтрокаПлатеж.УслугаНПД = Неопределено;
	КонецЕсли;
...
Функция ПроверитьЗаполнениеДляФормированияЧекаНПД(Форма) Экспорт
	...
	Если ОднострочнаяФорма И КоличествоСтрок = 1 Тогда
		Если ЗначениеЗаполнено(Форма.РасшифровкаПлатежаСчетНаОплату) Тогда
			СообщениеОбОшибке = ЧекиНПДВызовСервера.ПроверитьСчетНаОплатуДляФормированияЧекаНПД(
				Форма.РасшифровкаПлатежаСчетНаОплату);
			Если Не ПустаяСтрока(СообщениеОбОшибке) Тогда
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Поле", "Корректность", НСтр("ru = 'Счет на оплату'"), , , СообщениеОбОшибке);
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "РасшифровкаПлатежаСчетНаОплату");
				УслугаНПДЗаполнена = Ложь;
			КонецЕсли;
		ИначеЕсли Не ЗначениеЗаполнено(Форма.РасшифровкаПлатежаУслугаНПД) Тогда
	...
	Иначе
		Для Индекс = 0 По Объект.РасшифровкаПлатежа.Количество() - 1 Цикл
			...
			Если ЗначениеЗаполнено(СтрокаПлатежа.СчетНаОплату) Тогда
				СообщениеОбОшибке = ЧекиНПДВызовСервера.ПроверитьСчетНаОплатуДляФормированияЧекаНПД(
					СтрокаПлатежа.СчетНаОплату);
				Если Не ПустаяСтрока(СообщениеОбОшибке) Тогда
					УслугаНПДЗаполнена = Ложь;
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка", "Корректность", НСтр("ru = 'Счет на оплату'"), Индекс + 1, НСтр("ru = 'Расшифровка платежа'"),
						СообщениеОбОшибке);
					Если ОднострочнаяФорма Тогда
						ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "НадписьСуммаДокумента");
					Иначе
						ПутьКСтроке = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасшифровкаПлатежа", Индекс + 1, "СчетНаОплату");
						ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , ПутьКСтроке, "Объект");
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаПлатежа.УслугаНПД) Тогда
				УслугаНПДЗаполнена = Ложь;
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
