...
Функция ПолучитьНовыеЭлектронныеДокументы(Знач КлючиСинхронизации, КонтекстДиагностики,
	...
	СервисEDIСлужебный.ПолучитьНовыеЭлектронныеДокументыЧерезСервис1СEDI(РезультатПолученияДокументов,
		КлючиСинхронизации, КонтекстДиагностики, РезультатыОтправкиПолучения);
...
Процедура ПолучитьНовыеЭлектронныеДокументыЧерезОператораЭДО(
	...
	ПараметрыУчетныхЗаписей = УчетныеЗаписиЭДО.ПараметрыУчетныхЗаписей(ИдентификаторыУчетныхЗаписей,
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		ПараметрыПолучения.Вставить("ПараметрыУчетныхЗаписей", ПараметрыУчетныхЗаписей);
...
//  * ПараметрыУчетныхЗаписей - Соответствие из КлючИЗначение:
...
Функция ОтправитьТранспортныеКонтейнеры(ТранспортныеКонтейнеры, КлючиСинхронизации, КонтекстДиагностики,
	...
	Для Каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			ИначеЕсли СервисEDIСлужебный.ЭтоОбменЧерезСервис1СEDI(ТипОтправки) Тогда
				РезультатОтправкиКонтейнера = СервисEDIСлужебный.ОтправитьКонтейнер(
					ПараметрыОтправки, КонтекстДиагностики);
			КонецЕсли;
...
Процедура ПроверитьВозможностьОтправки(ДанныеОбъектов, КонтекстДиагностики) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектов Цикл
		Если Не ЭтоПрямойОбмен(ДанныеОбъекта.СпособОбмена)
			...
			И Не ПриглашенияЭДО.ПриглашениеПринято(ДанныеОбъекта.СтатусПриглашения)
			И Не СервисEDIСлужебный.ЭтоОбменЧерезСервис1СEDI(ДанныеОбъекта.СпособОбмена) Тогда
...
Функция НовыйРезультатПолученияДокументов() Экспорт
	...
	Для Каждого ДокументДляЗагрузки Из ЗагружаемыеДокументы Цикл
		...
		Попытка
			РезультатСохраненияКонтейнера = ТранспортныеКонтейнерыЭДО.СохранитьКонтейнер(ДокументДляЗагрузки,
				КлючСинхронизации, КонтекстДиагностики,, ПараметрыПолучения.ПараметрыУчетныхЗаписей);
...
Функция ТребуетсяПрерватьОтправку(ОшибкаПередачи)
	...
	Возврат СервисЭДО.ЭтоКодНеизвестнойОшибки(ОшибкаПередачи.Код)
		...
		Или СервисEDIСлужебный.ЭтоКодОшибкиТранспорта(ОшибкаПередачи.Код);
...
//  Структура:
// * ТранспортныйКонтейнер - ДокументСсылка.ТранспортныйКонтейнерЭДО
// * РеквизитыКонтейнера - см. ТранспортныеКонтейнерыЭДО.ДанныеДляОтправки
//                       - Неопределено
// * ДанныеДляОтправки - см. СформироватьДанныеДляОтправки
//                     - Неопределено
// * КлючиСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыеКлючиСинхронизации
// * КэшFTPСоединений - Соответствие из КлючИЗначение:
//    ** Ключ - Строка - идентификатор отправителя
//    ** Значение - FTPСоединение
Функция НовыеПараметрыОтправкиТранспортногоКонтейнера() Экспорт
