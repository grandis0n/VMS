...
Функция ДанныеАбонентовИзСервисаНастроек(Отбор, Отказ, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПриЗаполненииДанныхАбонентовИзСервисаНастроек(Отбор, Отказ, КонтекстДиагностики, Результат);
	КонецЕсли;
...
//    * КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ОтправитьДанныеАбонентаВСервисНастроек(ДанныеУчетнойЗаписи) Экспорт
	
	Отказ = Ложь;
	ЭтоНовыеДанные = ТипЗнч(ДанныеУчетнойЗаписи) = Тип("Структура");
	ИдентификаторУчетнойЗаписи = ?(ЭтоНовыеДанные, ДанныеУчетнойЗаписи.ИдентификаторУчетнойЗаписи, ДанныеУчетнойЗаписи);
	...
	|	УчетныеЗаписиЭДО.НеИспользоватьПриглашения КАК НеИспользоватьПриглашения,
	|	УчетныеЗаписиЭДО.КодыТиповДокументовКОбмену КАК КодыТиповДокументовКОбмену,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СертификатыКлючей.Отпечаток КАК imprint,
	|	СертификатыКлючей.ДействителенДо КАК expireDate
	|ИЗ
	|	СертификатыУчетныхЗаписейЭДО КАК СертификатыУчетныхЗаписейЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования КАК СертификатыКлючей
	|		ПО СертификатыУчетныхЗаписейЭДО.Сертификат = СертификатыКлючей.Ссылка
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	СертификатыКлючей.Отпечаток,
	|	СертификатыКлючей.ДействителенДо
	|ИЗ
	|	Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования КАК СертификатыКлючей
	|ГДЕ
	|	СертификатыКлючей.Ссылка В (&СписокСертификатов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ИтоговыйЗапрос.УстановитьПараметр("СписокСертификатов", 
		?(ЭтоНовыеДанные, ДанныеУчетнойЗаписи.СертификатыАбонента, Неопределено));
	...
	ЗаполнитьЗначенияСвойств(ОписаниеДанных, ?(ЭтоНовыеДанные, ДанныеУчетнойЗаписи, ДанныеПоУчетнойЗаписи), 
		"НазначениеУчетнойЗаписи, ПодробноеОписаниеУчетнойЗаписи, НеИспользоватьПриглашения");
	
	Если Не ЭтоНовыеДанные И ЗначениеЗаполнено(ДанныеПоУчетнойЗаписи.КодыТиповДокументовКОбмену) Тогда
		ОписаниеДанных.КодыТиповДокументовКОбмену = 
			ОбщегоНазначения.JSONВЗначение(ДанныеПоУчетнойЗаписи.КодыТиповДокументовКОбмену);
	ИначеЕсли ЭтоНовыеДанные Тогда
		ОписаниеДанных.КодыТиповДокументовКОбмену = ДанныеУчетнойЗаписи.КодыТиповДокументовКОбмену;
	КонецЕсли;
	...
	Если Не ЭтоНовыеДанные И Не Отказ Тогда
...
// 
// Возвращаемое значение:
//  Структура - Новое описание отправки данных абонента в сервисе настроек:
// * ИНН - Строка
// * КПП - Строка
// * ИдентификаторЭДО - Строка
...
// * СертификатыАбонента - Массив из Структура:
// 		** imprint - Строка
// 		** expireDate - Дата  
// * ВариантыЗаполненияФорматов - Массив из Структура см. ТаблицуФорматов 
...
Функция НовоеОписаниеОтправкиДанныхАбонентаВСервисеНастроек()
	...
	Результат.Вставить("НеИспользоватьПриглашения", Ложь);
	Результат.Вставить("КодыТиповДокументовКОбмену", Новый Массив);
...
Функция ПараметрыКомандыОтправкаДанныхАбонентаВСервисНастроек(ПараметрыЗапроса, Отказ,
	...
	Если СервисНастроекЭДОПовтИсп.ОператорПоддерживаетВерсиюФорматаРегистрации(Лев(Идентификатор, 3), "5.04") Тогда
		ОписаниеИдентификатора.Вставить("noInvitations", ПараметрыЗапроса.НеИспользоватьПриглашения);
		ОписаниеИдентификатора.Вставить("abonentDocTypes", ПараметрыЗапроса.КодыТиповДокументовКОбмену);
	КонецЕсли;
...
Функция ОбработкаРезультатаПолученияДанныхАбонентов(Знач ПолученныеДанные, Отказ)
	...
	Для Каждого Абонент Из ПолученныеДанные.identifierInfos Цикл
		...
		// Если оператор еще официально не готов к схеме работы без приглашений, или наблюдаются проблемы в работе
		// без приглашений, убираем признак по абонентам.
		Если СервисНастроекЭДОПовтИсп.ОператорПоддерживаетОбменБезПриглашений(НоваяСтрока.ОператорЭДО) Тогда
			Если Абонент.Свойство("noInvitations") Тогда
				НоваяСтрока.НеТребуетПриглашений = Абонент.noInvitations;
			КонецЕсли;
		Иначе
			НоваяСтрока.НеТребуетПриглашений = Ложь;
		КонецЕсли;
		
		Если Абонент.Свойство("abonentDocTypes") И ЗначениеЗаполнено(Абонент.abonentDocTypes) Тогда
			НоваяСтрока.КодыТиповДокументовКОбмену = ОбщегоНазначения.ЗначениеВJSON(Абонент.abonentDocTypes);
		КонецЕсли;
...
Функция ОбработкаРезультатаКаталогОператоров(ПолученныеДанные, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ОператорЭДО Из ПолученныеДанные.operators Цикл
		...
		ВерсияПоддерживаемогоРегистрационногоПакета = ОператорЭДО.regPackageVersion;
		Если ЗначениеЗаполнено(ВерсияПоддерживаемогоРегистрационногоПакета) Тогда
			ВерсияЧислом = 
				СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрЗаменить(ВерсияПоддерживаемогоРегистрационногоПакета, ".", ""));
			МаксимальнаяВерсияРегПакета = "5.03";
			МаксимальнаяВерсияЧислом =
				СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрЗаменить(МаксимальнаяВерсияРегПакета, ".", ""));
			Если ВерсияЧислом - МаксимальнаяВерсияЧислом > 0 Тогда
				ВерсияПоддерживаемогоРегистрационногоПакета = МаксимальнаяВерсияРегПакета;
			КонецЕсли;
		КонецЕсли;
		НоваяСтрока.ВерсияПоддерживаемогоРегистрационногоПакета = ВерсияПоддерживаемогоРегистрационногоПакета;
		// Отключено обновление признака ПоддерживаетсяОбменБезПриглашений
		// ПоддерживаетсяОбменБезПриглашений = ОператорЭДО.isNoInvitationsEnabled
		НоваяСтрока.ПоддерживаетсяОбменБезПриглашений = Ложь;
	КонецЦикла;
...
// * НеТребуетПриглашений - Булево
// * КодыТиповДокументовКОбмену - Строка
Функция НоваяТаблицаАбонентов()
	...
	Таблица.Колонки.Добавить("НеТребуетПриглашений", Типы().Булево);
	Таблица.Колонки.Добавить("КодыТиповДокументовКОбмену", Типы().Строка);
