...
Процедура ИзменитьРасчетыПриОплате(Форма, УстановитьБезЗакрывающихДокументов) Экспорт
	...
	ЭтоУплатаНалога = УчетДенежныхСредствКлиентСервер.ЭтоОперацияУплатаНалога(Объект.ВидОперации);
...
Функция ТекстБаннераПредупреждениеОЗаполненииРеквизитовПлатежаНПД()
	...
	ЧастиТекста.Добавить(НСтр("ru = 'Оплатить налог можно в Личном кабинете сервиса'"));
...
Процедура НастроитьФормуЖурналаДляРежимаИнтеграции(Форма, ИнтеграцияОрганизацииВключена) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ЗагрузитьИзКлиентБанк",
		"Заголовок",
		НСтр("ru = 'Загрузить из другого банка'"));
	
	Если Форма.ИмяФормы = "ЖурналДокументов.Деньги.Форма.ФормаСписка" Тогда
		Элементы.ОбщаяКомандаПоказатьДвиженияДокумента.Видимость = Ложь;
		Элементы.ПодменюОтчеты.Видимость = Ложь;
		Элементы.ФормаПодобратьНеоплаченныеПлатежныеПоручения.Видимость = Ложь;
	КонецЕсли;
