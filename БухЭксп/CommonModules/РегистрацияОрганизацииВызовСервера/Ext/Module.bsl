...
Функция ДанныеЗаявления(Заявление) Экспорт
	
	Если Не ЗначениеЗаполнено(Заявление) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заявление, "Организация, ИмяОтчета, ИмяФормы");
	
КонецФункции

// Проверяет доступность печати заявления (уведомления) в PDF417
//
// Параметры:
...
//
// Возвращаемое значение:
//   Булево - Истина, если печать PDF417 доступна для указанного отчета и формы
//
Функция ДоступнаПечатьPDF417(ИмяОтчета, ИмяФормы) Экспорт
	
	Если Не ЗначениеЗаполнено(ИмяОтчета) Или Не ЗначениеЗаполнено(ИмяФормы) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат Отчеты[ИмяОтчета].ДоступнаПечатьPDF417(ИмяФормы);
...
Функция СписокПечатаемыхЛистов(Объект, ИмяОтчета) Экспорт
	...
	Если ИмяОтчета = "РегламентированноеУведомлениеФормаР26001"
		Или ИмяОтчета = "РегламентированноеУведомлениеУСН" Тогда
		// Печать завязывается на документ-объект
		Возврат Отчеты[ИмяОтчета].СформироватьСписокЛистов(Объект.Ссылка);
	Иначе
		Возврат Отчеты[ИмяОтчета].СформироватьСписокЛистов(Объект);
	КонецЕсли;
