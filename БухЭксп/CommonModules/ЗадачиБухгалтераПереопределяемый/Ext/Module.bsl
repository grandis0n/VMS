...
Процедура ПриОпределенииЗапросаИзменений(Запрос, ОтборДействие) Экспорт
	...
	|	И НЕ ЗадачиБухгалтера.Правило ССЫЛКА Перечисление.РазовыеЗадачи
...
Процедура СкорректироватьРасписаниеСУчетомСменыСистемыНалогообложения(Организация, Расписание, ТекущаяДата) Экспорт
	...
	ДатаПереходаНаОСНО = БухгалтерскийУчетПовтИсп.ДатаПереходаНаОСНОЗаПериод(Организация, НачалоПериода, КонецПериода);
	Если Не ЗначениеЗаполнено(ДатаПереходаНаОСНО) Тогда
		Возврат;
	КонецЕсли;
	
	НаименованиеЗадачиНалогНаПрибыль = "Налог на прибыль";
	КоличествоСтрок = Расписание.Количество();
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		Индекс = КоличествоСтрок - НомерСтроки;
		ЭлементРасписания = Расписание[Индекс];
		Если ЭлементРасписания.НаименованиеСокращенное = НаименованиеЗадачиНалогНаПрибыль
			И ЭлементРасписания.ПериодСобытия >= НачалоПериода
			И ЭлементРасписания.ПериодСобытия <= ДатаПереходаНаОСНО Тогда
...
Процедура ПриОпределенииТекстаЗапросаСпискаЗадач(СписокЗадач) Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.ДокументСЭДО)
	|		КОГДА ЗадачиБухгалтера.Правило ССЫЛКА Перечисление.РазовыеЗадачи
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.РазовыеЗадачи)
	...
	|			ТОГДА 10
	|		КОГДА ЗадачиБухгалтера.Действие = ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.РазовыеЗадачи)
	|			ТОГДА 11
	...
	|				И ЗадачиБухгалтера.Действие <> ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.РазовыеЗадачи)
	|			ТОГДА 999
	...
	|		ПО ЗадачиБухгалтера.Организация = Организации.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Перечисление.РазовыеЗадачи КАК РазовыеЗадачи
	|		ПО ЗадачиБухгалтера.Правило = РазовыеЗадачи.Ссылка
	|ГДЕ
	|	НЕ ЗадачиБухгалтера.ВАрхиве
