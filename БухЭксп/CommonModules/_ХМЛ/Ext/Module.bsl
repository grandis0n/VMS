Функция ПараметрыДОМ(Документ,Разыменователь=Неопределено) Экспорт 
	// --- 
	
	Стр = _сф.ст_Создать("ДокументДОМ,Разыменователь");
	Стр.ДокументДОМ = Документ;
	Если Разыменователь = Неопределено Тогда
		
		Стр.Разыменователь = Новый РазыменовательПространствИменDOM(Документ);
		
	Иначе
		
		Стр.Разыменователь = Разыменователь;
...
Функция ПолучитьИтераторДОМ(ПараметрыДОМ,Выражение,Узел=Неопределено) Экспорт 
	
	Возврат(ПараметрыДОМ.ДокументДОМ.ВычислитьВыражениеXPath(Выражение,?(Узел <> Неопределено,Узел,ПараметрыДОМ.ДокументДОМ),ПараметрыДОМ.Разыменователь));
	
КонецФункции

Функция ПолучитьУзелДОМ(ПараметрыДОМ,Выражение,Узел=Неопределено) Экспорт 
	// --- 
	
	Итератор = ПолучитьИтераторДОМ(ПараметрыДОМ,Выражение,Узел);
	
	Возврат(Итератор.ПолучитьСледующий());
...
Функция ПолучитьВыражениеДляУзла(ПараметрыДОМ,Выражение,Узел=Неопределено) Экспорт
	
	Значение = ПолучитьИтераторДОМ(ПараметрыДОМ,Выражение,Узел).ПолучитьСледующий();
	Возврат(Значение); 
	
КонецФункции


Функция ПолучитьЗначениеУзла(ПараметрыДОМ,Выражение,Узел=Неопределено,ТипРезультата=Неопределено) Экспорт
	
	УзелЗначения = ПолучитьИтераторДОМ(ПараметрыДОМ,Выражение,Узел).ПолучитьСледующий();
	Если (УзелЗначения <> Неопределено) Тогда
		Значение = УзелЗначения.ТекстовоеСодержимое;
		...
		Иначе
			...
			Возврат(XMLЗначение(Тип(ТипРезультата),Значение));
...
Функция ЗначениеУзла(Узел,ТипРезультата=Неопределено) Экспорт 
	// --- 
	
	Значение = Узел.ТекстовоеСодержимое;
	Если ТипРезультата=Неопределено Тогда
...
Функция ЗначениеАтрибута(Узел,Атрибут,ТипРезультата=Неопределено) Экспорт 
	// --- 
	
	Значение = Узел.Атрибуты.ПолучитьИменованныйЭлемент(Атрибут).ТекстовоеСодержимое;
	Если ТипРезультата=Неопределено Тогда
...
Функция Значение(Элемент,Тип) Экспорт 
	// --- 
	
	Возврат(XMLЗначение(Тип(Тип),Элемент));
...
Функция ТипСтрока(Элемент) Экспорт 
	// --- 
	
	Возврат(Значение(Элемент,"Строка"));
	
	// --- 
КонецФункции

Функция ТипДата(Элемент) Экспорт 
	// --- 
	
	Возврат(Значение(Элемент,"Дата"));
	
	// --- 
КонецФункции

Функция ТипБулево(Элемент) Экспорт 
	// --- 
	
	Возврат(Значение(Элемент,"Булево"));
	
	// --- 
КонецФункции

Функция ТипЧисло(Элемент) Экспорт 
	// --- 
	
	Возврат(Значение(Элемент,"Число"));
...
Процедура Элемент(ОбъектХМЛ,ИмяЭлемента,Значение=Неопределено,Атрибуты=Неопределено) Экспорт 
	
	// атрибуты - структура
	
	ОбъектХМЛ.ЗаписатьНачалоЭлемента(ИмяЭлемента);
	// --- 
	Если Атрибуты <> Неопределено Тогда
		
		Для каждого Элемент Из Атрибуты Цикл
			
			ОбъектХМЛ.ЗаписатьАтрибут(Строка(Элемент.Ключ),XMLСтрока(Элемент.Значение));
			
		КонецЦикла; 
		
	КонецЕсли; 
	// --- 
	ОбъектХМЛ.ЗаписатьТекст(Строка(XMLСтрока(Значение)));
	// --- 
	ОбъектХМЛ.ЗаписатьКонецЭлемента();
...
//
Функция Прочитать_XLSX(Знач ФайлEXCEL, Знач ИмяНомерЛиста, Знач СтрокаЗаголовка = 1, Знач НачСтрока = 0, Знач КонСтрока = 0, КолвоСтрокExcel = 0, Идентификатор="") Экспорт 
    ...
    ZIPКаталог = КаталогВременныхФайлов() +"\_1cv8_xlsx_"+Формат(ТекущаяДата(),"ДФ=ddMMyyyyHHmmss")+"_"+Строка(Идентификатор)+"\";
	...
	ИмяФайлаЛиста = ZIPКаталог + НРег("XL\WorkSheets\Sheet") + ФайлНомерЛиста + ".xml";
    SheetX.ОткрытьФайл(ИмяФайлаЛиста);
