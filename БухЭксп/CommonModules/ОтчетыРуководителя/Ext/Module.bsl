...
Функция ВыручкаИСебестоимость(ПараметрыОтчета, ПрошлыйПериод = Ложь) Экспорт
	...
	|	СУММА(ВЫБОР
	|			КОГДА СкорректированныеПродажи.Счет В (&СчетаВыручкиЕНВД)
	|				ТОГДА ЕСТЬNULL(Себестоимость.Стоимость, 0)
	|			КОГДА ЕСТЬNULL(Себестоимость.Количество, 0) <> 0
	|					И ЕСТЬNULL(СкорректированныеПродажи.Количество, 0) <> 0
	|				ТОГДА ЕСТЬNULL(Себестоимость.Стоимость, 0) / Себестоимость.Количество * СкорректированныеПродажи.Количество
	|			ИНАЧЕ 0
