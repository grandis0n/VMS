...
Процедура ДополнительноОбработатьТаблицуКонтекстныхНовостей(
	...
	ИначеЕсли ИдентификаторМетаданных = "БП.РазделУчета.Обновления" И НЕ ПравоДоступа("Просмотр",Метаданные.Общиеформы.ОписаниеИзмененийПрограммы) Тогда
		...
		НовостьВводаОстатков = Неопределено;
		Если ТипЗнч(ТаблицаКонтекстныхНовостей) = Тип("ТаблицаЗначений") И ТаблицаКонтекстныхНовостей.Колонки.Найти("УИННовости") <> Неопределено Тогда
...
Процедура ПослеИзмененияСостоянияНовостей(
	...
	ОбработкаНовостейБП.ПослеИзмененияСостоянияНовостей(ВыбранныеПользователи, ВыбранныеНовости, ПредыдущееСостояние, НовоеСостояние);
