...
Функция Сайт1СКредит()
	...
	Возврат "https://buh.ru/video/1s-bukhgalteriya-8-red-3-0-kak-sformirovat-i-otpravit-zayavku-na-kredit-napryamuyu-iz-programmy-vide.html";
...
Функция ТекстБаннераЭТрН(НавигационнаяСсылка)
	...
	ТекстЗаголовка = НовыйЗаголовок(НСтр("ru = 'Электронные перевозочные документы c 01.09.2026'"));
	
	ТекстПодзаголовка = Новый ФорматированнаяСтрока(
		НовыйСтрокаБаннера(НСтр("ru = 'Узнайте,'"), НавигационнаяСсылка),
		НовыйСтрокаБаннера(НСтр("ru = ' как отправлять ЭТрН и другие документы напрямую из программы.'")));
...
Функция ТекстЗапросаУведомленияЕНС(Запрос, СтруктураБаннеров, НомераТаблиц) Экспорт
	...
	|	И ЗадачиБухгалтера.Правило В(&ПравилаЕдиногоНалоговогоПлатежа)
	|
	|УПОРЯДОЧИТЬ ПО
