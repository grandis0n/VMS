...
Функция ПараметрыФискализацииЧека() Экспорт
	...
	ПараметрыФискализации.Вставить("КодыМаркировки", Новый Массив);
...
Функция ПараметрыРегистрацииУстройства(Идентификатор) Экспорт
	...
	Если ЕстьДанные Тогда
		...
		Если ДанныеХранения.Свойство("ИдентификаторМоделиККТ") Тогда
			РезультатОперации.ИдентификаторМоделиККТ  = ДанныеХранения.ИдентификаторМоделиККТ;
		Иначе
			РезультатОперации.ИдентификаторМоделиККТ = ""
		КонецЕсли;
...
Процедура ОбновитьПараметрыРегистрацииККТ(ПараметрыРегистрацииТаблица, ПараметрыРегистрации) Экспорт
	...
	Для Каждого ПараметрРегистрации Из ПараметрыРегистрации Цикл
		...
		ИначеЕсли НаименованиеПараметра = "ИдентификаторМоделиККТ" Тогда
			НаименованиеПараметра = НСтр("ru='Идентификатор модели ККТ'")
...
Функция ПараметрыИзXMLПакетаККТ(Данные, РевизияИнтерфейса = 0) Экспорт
	...
	Если РевизияИнтерфейса >= 3000 Тогда  // Версия стандарта 3.0 (ОФД 1.1)
		...
		Если (ЧтениеXML.Имя = "Parameters" Или ЧтениеXML.Имя = "TableParametersKKT") И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
			...
			Параметры.ИдентификаторМоделиККТ     = СокрЛП(ЧтениеXML.ЗначениеАтрибута("KKTModelID"));     
...
Процедура СформироватьXMLПакетДляФискализацияЧека(ОбщиеПараметры, ПараметрыФискализации, ВключатьПерсональныеДанные = Ложь, 
	...
	Если ПараметрыФискализации.РевизияИнтерфейса >= 3000 Тогда  // Версия стандарта 3.0 (ОФД 1.1)
		...
		КонецЕсли;
		Если НЕ ПустаяСтрока(ОбщиеПараметры.МестоРасчетов) Тогда  
		...
		Иначе
			Если НЕ ПустаяСтрока(ОбщиеПараметры.НомерАвтомата) Тогда
		...
		Если НЕ ПустаяСтрока(ОбщиеПараметры.ДополнительныйРеквизит) Тогда  
		...
		Если ОбщиеПараметры.ПозицииЧека <> Неопределено Тогда
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда // Фискальная строка
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.СуммаНДС) И ОбработкаНДС.ЗначениеСтавкаНДС <> "none" Тогда 
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.ПризнакСпособаРасчета) Тогда
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.ПризнакПредметаРасчета) Тогда
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.ПризнакАгентаПоПредметуРасчета) Тогда
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.КодСтраныПроисхожденияТовара) Тогда
						ЗаписьXML.ЗаписатьАтрибут("CountryOfOrigin", XMLСтрока(ТекущаяПозиция.КодСтраныПроисхожденияТовара));
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ТекущаяПозиция.НомерТаможеннойДекларации) Тогда
		...
		// Cекция "ElectronicPaymentInfo" содержащая сведения об оплате безналичными.
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "CheckFiscalization" Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыОперации.УстройствоПечати) 
			И (ПараметрыПодключения.ТипыОборудования.Свойство("ПринтерЧеков") 
			И НЕ ПараметрыПодключения.ТипыОборудования.ПринтерЧеков) 
		...
		Для Каждого Позиция Из ПараметрыОперации.ПозицииЧека Цикл
			Если Позиция.Свойство("ФискальнаяСтрока") Тогда
				Если ЗначениеЗаполнено(Позиция.КонтрольнаяМарка) Тогда       
					КодМаркировки = Новый Структура();
					КодМаркировки.Вставить("КодМаркировки", Позиция.КонтрольнаяМарка);
					КодМаркировки.Вставить("Наименование",  Позиция.Наименование);
					ПараметрыФискализации.КодыМаркировки.Добавить(КодМаркировки);
				КонецЕсли;
	...
	ИначеЕсли Команда = "OpenShift"  Тогда
		...
		КонецЕсли;
		ДанныеОперации.Кассир = ПараметрыОперации.Кассир;
	...
	ИначеЕсли Команда = "PiotAuthentication" Тогда
		
		// ++ Локализация
		ПараметрыXML = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПолучитьПараметрыАутентификацииXML(ПараметрыОперации);    
		ДанныеОперации.Вставить("ПараметрыXML", ПараметрыXML);       
		// -- Локализация
		Возврат ДанныеОперации;   
		
	ИначеЕсли Команда = "PiotCreateEvent" Тогда
		
		// ++ Локализация
		ПараметрыXML = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПолучитьПараметрыСобытияЭксплуатацииXML(ПараметрыОперации);    
		ДанныеОперации.Вставить("ПараметрыXML", ПараметрыXML);       
		// -- Локализация
		Возврат ДанныеОперации;   
	...
	ИначеЕсли Команда = "PiotStart" Или Команда = "PiotCheckKM" Тогда  
		...
		ПараметрыАутентификации.Вставить("KKTModelID", ?(ПараметрыРегистрации.Свойство("ИдентификаторМоделиККТ"), ПараметрыРегистрации.ИдентификаторМоделиККТ, ""));
		...
		Если ПараметрыОперации.Свойство("ЗапросJSON") Тогда
			...
			Иначе
				КодМаркировки = Новый Структура();    
				КодМаркировки.Вставить("cis", ПараметрыОперации.КодМаркировки);  
				Если Не ПустаяСтрока(ПараметрыОперации.ТоварнаяГруппа) Тогда
					КодМаркировки.Вставить("pg", ПараметрыОперации.ТоварнаяГруппа);   
				КонецЕсли;
				ЗапросКодМаркировки = Новый Массив();
				ЗапросКодМаркировки.Добавить(КодМаркировки);
				ПараметрыЗапросJSON = Новый Структура();
				ПараметрыЗапросJSON.Вставить("codes", ЗапросКодМаркировки);
		...
		Если ОбщегоНазначенияБПО.ИспользуетсяТСПИоТ() Тогда                   
			...
			ЗаписиАутентификацииПользователя = МодульМенеджерОборудованияТСПИоТ.ЗаписиАутентификацииПользователяПИоТ(ПараметрыПодключения.ИдентификаторУстройства);
			АутентификацииПользователя = Новый Массив();
			Для Каждого ЗаписьАутентификацииПользователя Из ЗаписиАутентификацииПользователя Цикл
				ПараметрыXML = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПолучитьПараметрыАутентификацииXML(ЗаписьАутентификацииПользователя);    
				АутентификацииПользователя.Добавить(ПараметрыXML);
			КонецЦикла;
			ДанныеОперации.Вставить("ЗаписиАутентификацииПользователя", АутентификацииПользователя); 
		Иначе
			ДанныеОперации.Вставить("ЗаписиАутентификацииПользователя", Неопределено);
		КонецЕсли;
		// -- НеМобильноеПриложение
		// -- Локализация
		Возврат ДанныеОперации;
...
Процедура ОбработатьДанныеОперации(ПараметрыПодключения, Команда, РезультатВыполнения, ДанныеОперации) Экспорт
	...
	ИначеЕсли Команда = "PiotInformation"  Тогда  
		
		// ++ Локализация          
		Если РезультатВыполнения.Свойство("ИнформацияJSON") Тогда
			ИнформацияСтруктура = ПолучитьСтруктуруИнформацииЛМЧЗ(РезультатВыполнения.ИнформацияJSON);
		Иначе
			ИнформацияСтруктура = Неопределено;
		КонецЕсли;
		РезультатВыполнения.Вставить("Информация", ИнформацияСтруктура); 
		// -- Локализация  
...
Функция ПолучитьСтруктуруИнформацииЛМЧЗ(Знач ИнформацияJSON)
	
	ИнформацияСтруктура = Новый Структура();

	ЧтениеJSON = Новый ЧтениеJSON();
	ЧтениеJSON.УстановитьСтроку(ИнформацияJSON);
	Данные = ПрочитатьJSON(ЧтениеJSON, Истина);     
	Для Каждого ЭлементДаннных ИЗ Данные Цикл   
		Если ЭлементДаннных.Ключ = "lm" Тогда    
			ЛМЧЗ = Новый Структура(); 
			Для Каждого ЛМЧЗЭлемент Из ЭлементДаннных.Значение Цикл  
				ЭлементЗначение = ЛМЧЗЭлемент.Значение;
				Если ЛМЧЗЭлемент.Ключ = "expDate" И НЕ ПустаяСтрока(ЭлементЗначение) Тогда
					ЭлементЗначение = ОбщегоНазначенияБПОКлиентСервер.ПреобразоватьДатуJSON(ЭлементЗначение);
				КонецЕсли; 
				ЛМЧЗ.Вставить(ЛМЧЗЭлемент.Ключ, ЭлементЗначение);
			КонецЦикла;    
			ИнформацияСтруктура.Вставить("LM", ЛМЧЗ);
		Иначе
			ИнформацияСтруктура.Вставить(ЭлементДаннных.Ключ, ЭлементДаннных.Значение);
		КонецЕсли; 
	КонецЦикла;
...
Процедура ОбработатьЗакрытиеКассовойСмены(РевизияИнтерфейса, ДанныеОперации, ПараметрыПодключения, РезультатВыполнения)
	...
	КонецЕсли;
	Если ДанныеОперации.Свойство("Кассир") Тогда
		РезультатВыполнения.Вставить("Кассир", ДанныеОперации.Кассир);
	КонецЕсли;
...
// Выполняет полную очистку истории фискальных операций
//
Процедура ОчиститьПолностьюИсториюФискальныхОпераций() Экспорт

	РегистрыСведений.ФискальныеОперации.ОчиститьРегистрПолностью();
