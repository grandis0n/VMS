...
// Преобразовывает Epoch Unix Timestamp в дату
//
// Возвращаемое значение:
//  Дата - 
//
Функция ВременнаяМеткаВДату(ВременнаяМетка) Экспорт
	
	Возврат Дата(1970, 1, 1) + ВременнаяМетка / 1000 + СмещениеСтандартногоВремени(ЧасовойПояс(), );
