...
Процедура ПовторноОбработатьВходящиеСообщенияСЭДО(ПараметрыОбновления = Неопределено, ТипыЧислом = Неопределено, Идентификаторы = Неопределено, Страхователи = Неопределено, МинимальнаяДата = Неопределено) Экспорт
	Если МинимальнаяДата = Неопределено Тогда
		МинимальнаяДата = ДатаНачалаПроактивныхВыплат();
	КонецЕсли;
	Количество = ?(ПараметрыОбновления = Неопределено, Неопределено, 1000);
	Запрос = ЗапросПоВходящимСообщениямСЭДО(ТипыЧислом, Идентификаторы, Страхователи, МинимальнаяДата, Количество);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПовторноОбработатьВходящееСообщениеСЭДО(Выборка, Выборка.Содержимое.Получить());
	КонецЦикла;
	Если ПараметрыОбновления <> Неопределено Тогда
		ПараметрыОбновления.ОбработкаЗавершена = Выборка.Количество() < 1000;
	КонецЕсли;
КонецПроцедуры
...
Функция ЗапросПоВходящимСообщениямСЭДО(ТипыЧислом = Неопределено, Идентификаторы = Неопределено, Страхователи = Неопределено, МинимальнаяДата = Неопределено, Количество = Неопределено) Экспорт
	...
	Если Количество <> 1000 Тогда
		Если ТипЗнч(Количество) = Тип("Число") Тогда
			ПодстрокаЗамены = "ВЫБРАТЬ ПЕРВЫЕ " + Формат(Количество, "ЧН=; ЧГ=");
		Иначе
			ПодстрокаЗамены = "ВЫБРАТЬ";
		КонецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ ПЕРВЫЕ 1000", ПодстрокаЗамены);
	КонецЕсли;
