...
Функция НовыйРеквизитыПечатиЧека()
	...
	РеквизитыПечатиЧека.Вставить("ПередачаДляСобственныхНуждИСМП",     Ложь);
...
Функция ПараметрыОперацииФискализацииЧека(Реквизиты, ДанныеПечатиЧека, ВерсияФормата, АдресУстановкиККТ, СообщениеОбОшибке) Экспорт
	...
	Для каждого ОплачиваемыйДокумент Из ОплачиваемыеДокументы Цикл
		...
		ПризнакПредметаРасчетаПереданВТаблице = ОплачиваемыеПозицииСчета.Колонки.Найти("ПризнакПредметаРасчета") <> Неопределено;
		...
		Иначе
			...
			Для каждого СтрокаСчета Из ОплачиваемыеПозицииСчета Цикл
				...
				ИначеЕсли ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ПередачаСПолнойОплатой 
					...
					КонецЕсли; 
				ИначеЕсли ПризнакПредметаРасчетаПереданВТаблице И ЗначениеЗаполнено(СтрокаСчета.ПризнакПредметаРасчета) Тогда
					ПризнакПредметаРасчета = СтрокаСчета.ПризнакПредметаРасчета;
				Иначе
...
Функция ОбработатьМаркированнуюПродукцию(ТаблицаДокумента, ТаблицаШтрихкодов, РеквизитыПечатиЧека, ИдентификаторФормы, СообщениеОбОшибке)
	...
	Иначе
		Для каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			Для каждого СтрокаДокумента Из СтрокиДокумента Цикл
				Если КоличествоТовара(ШтрихкодыПоТовару) >= СтрокаДокумента.Количество И НЕ ЗначениеЗаполнено(СтрокаДокумента.Штрихкод) Тогда
					...
					СоответствиеТовар = Новый Соответствие;
					СоответствиеТовар.Вставить(СтрокаТовары.Номенклатура, ШтрихкодыПоСтроке);
					...
					Если ИдентификаторФормы = Неопределено Тогда
						СтрокаДокумента.Штрихкод = ПоместитьВоВременноеХранилище(СоответствиеТовар, Новый УникальныйИдентификатор);
					Иначе
						СтрокаДокумента.Штрихкод = ПоместитьВоВременноеХранилище(СоответствиеТовар, ИдентификаторФормы);
					КонецЕсли; 
				Иначе
					СообщениеОбОшибке = СтрШаблон(
		...
		Если НЕ РеквизитыПечатиЧека.ПередачаДляСобственныхНуждИСМП Тогда
