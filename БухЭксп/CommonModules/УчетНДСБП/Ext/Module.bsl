...
Функция ТекстЗапросаДанныеОРНПТДляОтчетности(ПоискРНПТПоОснованию)
	...
	Иначе
		...
		|	ОперацииСПрослеживаемымиТоварами.Организация В(&Организация)
		|	И ОперацииСПрослеживаемымиТоварами.ОтражениеВОтчетности = &ОтражениеВОтчетности
		|	И НЕ СписокСчетовФактур.НаАванс
		|
		|СГРУППИРОВАТЬ ПО
		|	СписокСчетовФактур.СчетФактура,
		|	ОперацииСПрослеживаемымиТоварами.РНПТ.Код,
...
#Область СведенияИзДОПП

Процедура ТекстЗапросаКосвенныеНалогиСведенияИзДОПП(Запрос) Экспорт

	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КосвенныеНалогиСведенияИзДОПП.Организация КАК Организация,
	|	КосвенныеНалогиСведенияИзДОПП.КБК КАК КБК,
	|	КосвенныеНалогиСведенияИзДОПП.ДОПП КАК ДОПП,
	|	КосвенныеНалогиСведенияИзДОПП.СуммаНалогаАкциза КАК СуммаНалогаАкциза
	|ИЗ
