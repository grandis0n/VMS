...
Функция СчетаРасчетовСКонтрагентами(Тип) Экспорт
	...
	ИсключенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате);	//68.43
...
Функция ЗапросКДаннымРазделаМонитора(ПолучатьПрошлыйПериод, ОтбиратьПоОрганизации, ПолучатьДанныеПодразделов)
	...
	|			ТОГДА 0
	|		ИНАЧЕ (ДанныеМонитораРуководителя.Сумма - ДанныеМонитораРуководителя.СуммаПрошлыйПериод) / ВЫБОР
	|				КОГДА ДанныеМонитораРуководителя.СуммаПрошлыйПериод >= 0
	|					ТОГДА ДанныеМонитораРуководителя.СуммаПрошлыйПериод
	|				ИНАЧЕ -ДанныеМонитораРуководителя.СуммаПрошлыйПериод
	|			КОНЕЦ * 100
