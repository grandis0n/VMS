...
// Присваивает задаче по оплате налога АУСН финальный статус, когда проходит платеж на ЕНС
//
// Параметры:
//  РеквизитыПлатежа - Структура - см. ВыполнениеЗадачБухгалтера.РеквизитыПлатежаПоДаннымОбъекта
//  ДополнительныеСвойства - Структура - дополнительные свойства объекта (проводимого документа)
//
Процедура ОбновитьСтатусЗадачиАУСН(РеквизитыПлатежа, ДополнительныеСвойства) Экспорт
	
	ПрименяетсяАУСН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "ПрименяетсяАУСН", Ложь);
	
	Если Не ПрименяетсяАУСН Тогда
		Возврат;
	КонецЕсли;
	
	Если РегистрыСведений.СостоянияИнтеграцииАУСН.СервисЛКАУСНПодключен()
		Или РегистрыСведений.СостоянияИнтеграцииАУСН.СервисПодключен() Тогда
		// Когда сервис подключен, статусы задач меняются автоматически, при загрузке данных ФНС
		Возврат;
	КонецЕсли;
	...
	Если ВыполнениеЗадачБухгалтераБП.ЭтоУплатаНалога(РеквизитыПлатежа.ВидОперации)
		...
		Если Не ЗначениеЗаполнено(ПравилоУплатыНалогов) Или Не ЗначениеЗаполнено(ПериодСобытияКалендаря) Тогда
			Возврат;
		КонецЕсли;
		
		КлючиЗаписи = РегистрыСведений.ЗадачиБухгалтера.КлючиЗадачСтруктурой();
		КлючиЗаписи.Организация = РеквизитыПлатежа.Организация;
		КлючиЗаписи.Правило = ПравилоУплатыНалогов;
		КлючиЗаписи.ПериодСобытия = ПериодСобытияКалендаря;
		КлючиЗаписи.Действие = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПравилоУплатыНалогов, "Действие");
		КлючиЗаписи.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане;
		...
		Если МенеджерЗаписи.Выбран() Тогда
			ВыполнениеЗадачБухгалтера.ЗаписатьНовоеСостояниеЗадачи(
				КлючиЗаписи, Ложь, ВыполнениеЗадачБухгалтера.ФинальныйСтатусЗадачи(КлючиЗаписи));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодготовитьУведомленияПоЗадачамОрганизаций(Уведомления) Экспорт
	...
	ПодготовитьУведомленияПоОбщимЗадачам(Уведомления, Получатели);
	
	ПодготовитьУведомленияПоЗадачамСЭДО(Уведомления, Получатели);
...
Функция ТекстЗапросаЗадачиОрганизаций()
...
Процедура ПодготовитьУведомленияПоОбщимЗадачам(Уведомления, Получатели)
	...
	ПараметрыФормирования = ПараметрыФормированияУведомления();
	
	ПараметрыФормирования.СрокВыполнения = СрокВыполнения;
	ПараметрыФормирования.Уведомления = Уведомления;
	ПараметрыФормирования.СЭДО = Ложь;
	ПараметрыФормирования.ТекстЗапроса = ТекстЗапросаЗадачиОрганизаций();
	ПараметрыФормирования.Получатели = Получатели;
	
	СформироватьУведомления(ПараметрыФормирования);
	
КонецПроцедуры

Функция ТекстPushУведомления(КоличествоЗадач, Представление, СрокВыполнения)
...
Процедура ПодготовитьУведомленияПоЗадачамСЭДО(Уведомления, Получатели)
	
	КоличествоСекундВСутках = 86400;
	
	//Отправка пуш осуществляется на след. день после создания задачи.
	//В 5 дневный срок уже не попадает
	КоличествоДней = 4;
	Интервал = КоличествоДней * КоличествоСекундВСутках;
	...
	ПараметрыФормирования.СрокВыполнения = СрокВыполнения;
	ПараметрыФормирования.Уведомления = Уведомления;
	ПараметрыФормирования.СЭДО = Истина;
	ПараметрыФормирования.ТекстЗапроса = ТекстЗапросаЗадачиСЭДО();
	ПараметрыФормирования.Получатели = Получатели;
...
Функция ТекстЗапросаЗадачиСЭДО()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиБухгалтера КАК ЗадачиБухгалтера
	|		ПО ВТ_Получатели.Организация = ЗадачиБухгалтера.Организация
	|			И (ЗадачиБухгалтера.Статус = ""Расшифровать"")
	|			И (НЕ ЗадачиБухгалтера.ВАрхиве)
	|			И (ЗадачиБухгалтера.Срок = &Срок)
	|			И (ЗадачиБухгалтера.Правило ССЫЛКА Документ.ВходящийЗапросФССДляРасчетаПособия)
	|ИТОГИ ПО
	|	Пользователь";
...
Функция ТекстPushУведомленияСЭДО(КоличествоЗадач, Представление, СрокВыполнения)
	
	ТекстУведомления = "";
	
	Если КоличествоЗадач > 1 Тогда
		СклоняемаяЧастьСтроки = НСтр("ru='запрос'");
		...
		ТекстУведомления = СтрШаблон(НСтр("ru='У вас %1 от СФР. Срок до %2'"),
			СклоняемаяЧастьСтроки, Формат(СрокВыполнения, "ДФ='d MMMM'"));
...
Функция ПараметрыФормированияУведомления()
	
	ПараметрыФормирования = Новый Структура;
	
	ПараметрыФормирования.Вставить("СрокВыполнения");
	ПараметрыФормирования.Вставить("ТекстЗапроса");
	ПараметрыФормирования.Вставить("Уведомления");
	ПараметрыФормирования.Вставить("СЭДО");
	ПараметрыФормирования.Вставить("Получатели");
	
	Возврат ПараметрыФормирования;
	
КонецФункции

Процедура СформироватьУведомления(ПараметрыФормирования)
	
	Запрос = Новый Запрос;
	
	Запрос.Текст = ПараметрыФормирования.ТекстЗапроса;
	
	Запрос.УстановитьПараметр("Срок", ПараметрыФормирования.СрокВыполнения);
	Запрос.УстановитьПараметр("Получатели", ПараметрыФормирования.Получатели);
	
	Если НЕ ПараметрыФормирования.СЭДО Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			...
			PushУведомление = PushУведомление(Представление, ПараметрыФормирования.СрокВыполнения,
					ВыборкаДетальныеЗаписи.ИдентификаторПолучателя, КоличествоЗадач, ПараметрыФормирования.СЭДО);
			
			ПараметрыФормирования.Уведомления.Добавить(PushУведомление);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Функция PushУведомление(Представление, СрокВыполнения, ИдентификаторПолучателяХранилище, КоличествоЗадач, СЭДО)
	
	Если НЕ СЭДО Тогда
		ТекстУведомления = ТекстPushУведомления(КоличествоЗадач, Представление, СрокВыполнения);
		ЗаголовокУведомления = НСтр("ru='Подходит срок выполнения задач'");
	Иначе
		ТекстУведомления = ТекстPushУведомленияСЭДО(КоличествоЗадач, Представление, СрокВыполнения);
		ЗаголовокУведомления = НСтр("ru='Поступил запрос от СФР'");
	КонецЕсли;
	
	ИдентификаторПолучателя = ИдентификаторПолучателяХранилище.Получить();
	
	PushУведомление = НовоеPushУведомление(ЗаголовокУведомления, ТекстУведомления);
...
Функция НовоеPushУведомление(ЗаголовокУведомления, ТекстУведомления)
