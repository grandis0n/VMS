...
Функция ДвоичныеДанныеРегламентированногоОтчета(РегламентированныйОтчет, Ошибки = Неопределено) Экспорт
	...
	ИмяФайлаВыгрузки = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
		РезультатВыгрузки.ИмяФайлаВыгрузки);
	
	ИмяВременногоФайлаОтчета = ИмяВременногоКаталога + ПолучитьРазделительПути() + ИмяФайлаВыгрузки;
