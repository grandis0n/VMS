...
Функция ПрочитатьJSONРезультатОперации(СтрокаJSON, СписокОшибок = Неопределено, ОснованиеФискальнойОперации = Неопределено) Экспорт 
	...
	Если Не ПустаяСтрока(СтрокаJSON) 
		...
		Если РезультатВыполнения.Результат И РезультатОперации.Свойство("basket") Тогда
			...
			Если РезультатОперации.basket.Свойство("article") Тогда
				Для Каждого Позиция Из РезультатОперации.basket.article Цикл
					...
					КонецЕсли;                                                    
					Если Позиция.Свойство("discountPrice") И Не ПустаяСтрока(Позиция.discountPrice) Тогда
						ТоварнаяПозиция.ЦенаСоСкидкой = Число(Позиция.discountPrice) / 100;
					Иначе 
						ТоварнаяПозиция.ЦенаСоСкидкой = ТоварнаяПозиция.Цена;
					КонецЕсли;   
					
					Если Позиция.Свойство("discount") И Позиция.discount <> Неопределено  Тогда
						Для Каждого Скидка Из Позиция.discount Цикл  
							СкидкаСтрока = ЭлектронныеСертификатыНСПККлиентСервер.ПараметрыСкидки();
							Если Скидка.Свойство("percent") Тогда
								СкидкаСтрока.Процент = Скидка.percent;
							КонецЕсли;            
							Если Скидка.Свойство("amount") Тогда   
								Если Не ПустаяСтрока(Скидка.amount) Тогда
									СкидкаСтрока.Сумма = Число(Скидка.amount) / 100;
								КонецЕсли;            
							КонецЕсли;            
							Если Скидка.Свойство("priority") Тогда
								СкидкаСтрока.Приоритет = Скидка.priority;
							КонецЕсли;   
							ТоварнаяПозиция.Скидки.Добавить(СкидкаСтрока);
						КонецЦикла; 
					КонецЕсли;   
					...
					Если Позиция.Свойство("certificate") И Позиция.certificate <> Неопределено  Тогда
		...
		Если РезультатВыполнения.Результат И РезультатОперации.Свойство("discounts") Тогда
			Для Каждого Скидка Из РезультатОперации.discounts Цикл    
				
				СкидкаСтрока = ЭлектронныеСертификатыНСПККлиентСервер.ПараметрыДоступнойСкидки();
				Если Скидка.Свойство("startDate") И ЗначениеЗаполнено(Скидка.startDate)Тогда
					СкидкаСтрока.ДатаНачалаДействия = XMLЗначение(Тип("Дата"), Скидка.startDate);
				КонецЕсли;                     
				Если Скидка.Свойство("expiryDate") И ЗначениеЗаполнено(Скидка.expiryDate) Тогда
					СкидкаСтрока.ДатаЗавершенияДействия = XMLЗначение(Тип("Дата"), Скидка.expiryDate);
				КонецЕсли;                     
				Если Скидка.Свойство("percent") Тогда
					СкидкаСтрока.Процент = Скидка.percent;
				КонецЕсли;            
				...
				Если Скидка.Свойство("tru") Тогда
					Для Каждого КодТовараТРУ Из Скидка.tru Цикл       
						Если КодТовараТРУ.Свойство("Code") Тогда
							СкидкаСтрока.КодыТоваровТРУ.Добавить(КодТовараТРУ.Code);
						КонецЕсли;                     
					КонецЦикла;
				КонецЕсли;
				
				РезультатВыполнения.ДоступныеСкидки.Добавить(СкидкаСтрока);
			КонецЦикла;
		КонецЕсли;  
...
Функция ТипПлатежнойОперацииПоКоманде(Команда)
	...
	ТипыОпераций.Вставить(ЭлектронныеСертификатыНСПККлиентСервер.ТипПлатежнойОперацииПолучениеСпискаСкидокЭС(),
		Перечисления.ТипыПлатежныхОпераций.ПолучениеСпискаСкидокЭС);              
