...
Процедура ПриЗакрытии(Форма, ЗавершениеРаботы = Истина) Экспорт
	...
	ОтменяемыеЗадания = CW_БухгалтерскийУчетКлиентПереопределяемыйМСФО.ЗаданияОтменяемыеПриЗакрытииОтчета();
	...
	Если Не ЗавершениеРаботы Тогда
		CW_БухгалтерскиеОтчетыВызовСервераМСФО.ОтменитьВыполнениеЗаданий(ИдентификаторыОтменяемыхЗаданий);
	КонецЕсли;
...
Процедура ПодразделениеПриИзменении(Форма, Элемент) Экспорт
	...
	CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ПодразделениеПриИзменении(Форма);
...
Процедура ОтборОбработатьВыборЗначения(Форма, Элемент, СтандартнаяОбработка, Значение, СписокПараметров, ТипПоля, МножественныйВыбор = Ложь) Экспорт
	...
	ИначеЕсли ТипПоля.Типы().Количество() > 0 Тогда
		...
		Если ?(ТипЭлемента = Тип("Неопределено"), Истина, Не ТипПоля.СодержитТип(ТипЭлемента)) Тогда
			...
			Если ТипПоля.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов")) Тогда
				...
				ИмяРеквизитаКонтрагент = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ПолучитьИмяРеквизитаКонтрагентДоговора();
	...
	Если ТипЭлемента = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ТипЗначенияБанковскогоСчетаОрганизации() Тогда
		...
		CW_БухгалтерскийУчетКлиентПереопределяемыйМСФО.ОткрытьФормуВыбораБанковскогоСчетОрганизации(ПараметрыФормы, Элемент);
		
	ИначеЕсли ТипЭлемента = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ТипПодразделения() Тогда
		...
		CW_БухгалтерскийУчетКлиентПереопределяемыйМСФО.ОткрытьФормуВыбораПодразделения(ПараметрыФормы, Элемент);
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		КонецЕсли;
		ИмяРеквизитаКонтрагента = CW_БухгалтерскийУчетКлиентСерверПереопределяемыйМСФО.ПолучитьИмяРеквизитаКонтрагентДоговора();
		...
		CW_БухгалтерскийУчетКлиентПереопределяемыйМСФО.ОткрытьФормуВыбораДоговора(ПараметрыФормы, Элемент, ФормаВыбора);
		
	ИначеЕсли ТипПоля = ОписанияТиповВидовСубконто.ДокументРасчетовСКонтрагентами
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.CW_ДокументРасчетовСКонтрагентом") Тогда
		СтандартнаяОбработка = Ложь;
		...
		ОткрытьФорму("Документ.CW_ДокументРасчетовСКонтрагентом.ФормаВыбора", ПараметрыФормы, Элемент);
...
Процедура ОтборыПравоеЗначениеНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка, СписокПараметров, КомпоновщикНастроек = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если СтрокаОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
			...
			ИначеЕсли ОписанияТиповВидовСубконто.Склад Тогда
...
Процедура ТабличноеПолеПоСчетамСчетПриИзменении(ФормаОтчета, ИмяЭлемента, Элемент) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Счет) Тогда
			ДанныеСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(ТекущиеДанные.Счет);
			...
			//	И CW_БухгалтерскиеОтчетыВызовСервераМСФО.ИспользоватьПодразделения() Тогда
			...
			Если ДанныеСчета.КоличествоСубконто > 0 Тогда
				Для Индекс = 1 По ДанныеСчета.КоличествоСубконто Цикл
					...
					Если ИспользоватьОднуНоменклатурнуюГруппу 
						И ДанныеСчета["ВидСубконто" + Индекс] = ПредопределенноеЗначение("ПланВидовХарактеристик.CW_ВидыСубконтоМСФО.НоменклатурныеГруппы") Тогда
						СтрокаПоСубконто    = СтрокаПоСубконто + "-" + Индекс;
					ИначеЕсли НЕ ВестиУчетПоДоговорам 
						И ДанныеСчета["ВидСубконто" + Индекс] = ПредопределенноеЗначение("ПланВидовХарактеристик.CW_ВидыСубконтоМСФО.Договоры") Тогда
						СтрокаПоСубконто    = СтрокаПоСубконто + "-" + Индекс;
					Иначе
						СтрокаПоСубконто    = СтрокаПоСубконто + "+" + Индекс;
...
Процедура ТабличноеПолеПоСчетамПредставлениеНачалоВыбора(ФормаОтчета, ИмяЭлемента, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	ДанныеСчета = CW_БухгалтерскийУчетВызовСервераПовтИспМСФО.ПолучитьСвойстваСчета(Счет);
...
Процедура ТабличноеПолеПоСчетамПредставлениеОбработкаВыбора(ФормаОтчета, ИмяЭлемента, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СписокЗначений") Тогда 
		...
		Для Каждого ЭлементСписка Из ВыбранноеЗначение Цикл
			...
			Если ТипЗнч(ЭлементСписка.Значение) = Тип("ПланВидовХарактеристикСсылка.CW_ВидыСубконтоМСФО") Тогда
				Для Индекс = 1 По ДанныеСчета.КоличествоСубконто Цикл 
...
Процедура РасшифровкаСтандартногоОтчета(ФормаОтчета, Элемент, ИдентификаторРасшифровки) Экспорт
	...
	ИмяОтчета = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИмяОтчетаПоИмениФормы(ФормаОтчета);

	ВозможныеРасшифровки = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ВозможныеРасшифровки(ИмяОтчета, ФормаОтчета.ДанныеРасшифровки, ИдентификаторРасшифровки);
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ИдентификаторОбъекта = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ИмяОтчетаПоИмениФормы(ФормаОтчета);
	СтандартнаяОбработка = Ложь;
	
	Если CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ОтчетПоддерживаетУниверсальныеНастройки(ИдентификаторОбъекта) Тогда
		
		РасшифровкаСтандартногоОтчета(ФормаОтчета, Элемент, Расшифровка);
		
	Иначе
		
		ПараметрыРасшифровки = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПолучитьПараметрыРасшифровкиОтчета(ФормаОтчета.ДанныеРасшифровки, ИдентификаторОбъекта, Расшифровка);
...
Процедура УстановитьВидимостьПанелиПересчетаИтогов(Форма, ВариантОтображения = "") Экспорт
	...
	Если ВариантОтображения = "ТребуетсяПересчет" Тогда
		...
		Если ЭлементыФормы.ПересчитатьИтоги.Видимость
		   И Не CW_БухгалтерскиеОтчетыВызовСервераМСФО.ЕстьПравоПересчитатьИтоги() Тогда
...
Процедура ПересчитатьИтогиЗавершение(Результат, Форма) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ДлительнаяОперация = CW_БухгалтерскиеОтчетыВызовСервераМСФО.ПересчитатьИтоги(МестоРасхождения);
...
Процедура ПересчитатьИтогиЗавершениеВФоне(РезультатЗадания, Форма) Экспорт
	...
	Если РезультатЗадания.Статус = "Выполнено" Тогда
		...
		Если РезультатПересчета.ПересчетУспешноЗавершен Тогда
			...
			CW_БухгалтерскиеОтчетыКлиентМСФО.УстановитьВидимостьПанелиПересчетаИтогов(Форма, "ИтогиПересчитаны");
...
//                CW_БухгалтерскиеОтчетыВызовСервераМСФО.ВычислитьСуммуВыделенныхЯчеекТабличногоДокумента.
//
	...
	Если НеобходимоОбновитьСумму(Результат, КэшВыделеннойОбласти) Тогда
		...
		ИначеЕсли КоличествоВыделенныхОбластей = 1 Тогда
			...
			Если ВычислитьНаКлиенте Тогда
				СуммаВЯчейках = 0;
				CW_БухгалтерскиеОтчетыКлиентСерверМСФО.ВычислитьСуммуВЯчейках(СуммаВЯчейках, СтруктураАдресВыделеннойОбласти, Результат);
...
Процедура ОткрытьРасшифровку(ЗначениеРасшифровки, ДанныеРасшифровки, ИдентификаторРасшифровки, ИмяИсходногоОтчета)
	...
	Если ЭтоОтчет(ЗначениеРасшифровки) Тогда
		...
		ПараметрыФормы = CW_БухгалтерскиеОтчетыКлиентСерверМСФО.НовыйПараметрыФормыСтандартногоОтчета();
