...
Функция ИмяСобытияИзмененоСостояние() Экспорт
	
	Возврат "ИнтеграцияАУСН_ИзмененоСостояние";
...
Процедура ОткрытьОбращениеВТехподдержку1С(Форма)
	...
	Если Форма.ИмяФормы = "Документ.СписаниеСРасчетногоСчета.Форма.ФормаРасчетНДФЛАУСНПоДокументу" Тогда
		ИнтеграцияАУСНВызовСервера.ЗаполнитьПараметрыДокументаДляОбращенияВТехПоддержку1С(ПараметрыДокумента,
			Форма.ПлатежныйДокумент);
	Иначе
		ЗаполнитьЗначенияСвойств(ПараметрыДокумента, Форма.Объект);
	КонецЕсли;
