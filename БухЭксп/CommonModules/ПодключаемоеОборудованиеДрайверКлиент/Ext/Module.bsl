...
Асинх Функция ВыполнениеКоманды(ПараметрыПодключения, Команда, ДанныеОперации) Экспорт
	...
	ИначеЕсли Команда = "PiotAuthentication" Тогда // Аутентификация пользователя ПИоТ.
		РезультатВыполнения = Ждать АутентификацияПользователяПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);       
	ИначеЕсли Команда = "PiotCreateEvent" Тогда // Сформировать событие эксплуатации ПИоТ
		РезультатВыполнения = Ждать СформироватьСобытиеЭксплуатацииПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);       
	...
	ИначеЕсли Команда = "PiotServiceInformation" Тогда // Служебная информация ПИоТ.
		РезультатВыполнения = Ждать СлужебнаяИнформацияПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);   
	ИначеЕсли Команда = "PiotInformation" Тогда // Получить информацию о внешних системах ПИоТ.
		РезультатВыполнения = Ждать ПолучитьИнформациюОВнешнихСистемахПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);   
	...
	ИначеЕсли Команда = "AuthorizePreSales" Или Команда = "AuthorizeCompletion" Или Команда = "AuthorizeVoidPreSales" Тогда
		РезультатВыполнения = Ждать ЭквайринговаяОперацияXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда);
	ИначеЕсли Команда = "OperationStatus" Тогда // Получить статус операции
		РезультатВыполнения = Ждать СтатусОперацииXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	ИначеЕсли Команда = "AddMediaFile" Тогда // Добавить медиа файл на дисплей покупателя
		РезультатВыполнения = Ждать ДобавитьМедиафайлНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации); 
...
//  см. РезультатОперацииНаОборудовании 
//
Асинх Функция ОписаниеДрайвера50Асинх(ПараметрыПодключения) Экспорт
	...
	Иначе                                                
		ОписаниеДрайвераXML = "";
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения,  "GetDescription");
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьОписаниеАсинх(ОписаниеДрайвераXML); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
...
// ++ Локализация 
	...
	Если ИспользоватьТСПИоТ Тогда         
		// Проверка идентичности типа платформы клиента для запуска ТС ПИоТ
		Если Строка(ОбщегоНазначенияБПОКлиент.ТипПлатформыКлиента()) <> ПараметрыПодключения.ТСПИоТПлатформа Тогда    
			ОписаниеОшибки = СтрШаблон(НСтр("ru='Данный экземпляр ""ТС ПИоТ"" был создан для другого типа клиента (%1).'"), ПараметрыПодключения.ТСПИоТПлатформа);
			РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			Возврат РезультатВыполнения;
		КонецЕсли;
		// Создание данных экземпляр ТС ПИоТ
		РезультатВыполнения = Ждать СозданиеДанныхЭкземпляраТСПИоТ(ПараметрыПодключения); 
		Если НЕ РезультатВыполнения.Результат Тогда
			Возврат РезультатВыполнения;
		КонецЕсли;
		// Подключение основной ВК драйвера
		РезультатОписание = Ждать ОписаниеДрайвера50Асинх(ПараметрыПодключения);
		Если РезультатОписание.Результат Тогда
			ОписаниеДрайвера = РезультатОписание.ОписаниеДрайвера;
		Иначе
			Возврат РезультатОписание;
		КонецЕсли;    
		// Подключить ТС ПИоТ
		РезультатПодключения = Ждать ПодключитьТСПИоТАсинх(ПараметрыПодключения, ОписаниеДрайвера);  
		...
		КонецЕсли;    
		ОбъектДрайвера = РезультатПодключения.ПодключаемыйМодуль;
		ПараметрыПодключения.ИдентификаторЭкземпляра = ПараметрыПодключения.ТСПИоТИдентификатор;
	Иначе
// -- Локализация
// -- НеМобильноеПриложение 
		ОбъектДрайвера = Ждать ОбъектДрайвераАсинх(ПараметрыПодключения);
...
// -- НеМобильноеПриложение 
	...
	Иначе
		...
		Попытка
			...
			ПараметрыПодключения.ИДУстройства = РезультатВызова.Параметры[0]; // ИДУстройства 
			Результат = РезультатВызова.Значение;
		Исключение
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
	...
	Иначе
		...
		Если ОбщегоНазначенияБПОКлиент.ИспользуетсяЧекопечатающиеУстройства()
			...
			Или (ТипыОборудования.Свойство("ПринтерЧеков") И ТипыОборудования.ПринтерЧеков)) Тогда      
				Результат = Ждать ШиринаСтроки(ОбъектДрайвера, ПараметрыПодключения, Неопределено);
		// ++ НеМобильноеПриложение
		// ++ Локализация       
			Если ТипыОборудования.ККТ И ИспользоватьТСПИоТ Тогда
				ПараметрыПодключения.ИменаСобытий.Добавить("PIOT_Event");
			КонецЕсли;
		// -- Локализация
		// -- НеМобильноеПриложение 
		КонецЕсли;
...
Функция ПараметрыСобытия(Результат, ИмяСобытия, ДанныеСобытия, РассылкаОповещения = Истина)
	...
	РезультатОбработки.Вставить("РассылкаОповещения", РассылкаОповещения); 
...
Функция ОбработатьСобытиеШтрихкод(Данные) Экспорт
	...
	РезультатОбработки = ПараметрыСобытия(Истина, ИмяСобытия, ДанныеСобытия); 
	Возврат РезультатОбработки;
...
Функция ОбработатьНажатиеНаДисплее(Данные) Экспорт
	...
	РезультатОбработки = ПараметрыСобытия(Истина, ИмяСобытия, Данные); 
	Возврат РезультатОбработки;
...
// Обработать cобытие ТС ПИоТ
Функция ОбработатьСобытиеПИоТ(ПараметрыПодключения, Событие, Данные) Экспорт
	
	Если ПараметрыПодключения.ИспользоватьТСПИоТ Тогда
		МодульМенеджерОборудованияТСПИоТКлиент = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("МенеджерОборудованияТСПИоТКлиент"); 
		МодульМенеджерОборудованияТСПИоТКлиент.ЗаписатьСобытиеЭксплуатацииТСПИоТ(ПараметрыПодключения.ИдентификаторУстройства, 
			ОбщегоНазначенияБПОКлиент.ТекущаяВременнаяМетка(), Данные);
	КонецЕсли;
	
	РезультатОбработки = ПараметрыСобытия(Истина, Событие, Данные, Ложь); 
	Возврат РезультатОбработки;
...
Функция ОбработатьСобытиеRFID(ПараметрыПодключения, Событие, Данные) Экспорт
	...
	РезультатОбработки = ПараметрыСобытия(Результат, Событие, ДанныеСобытия); 
	Возврат РезультатОбработки;
...
Функция ОбработатьСобытиеВеса(ПараметрыПодключения, Данные) Экспорт
	...
	РезультатОбработки = ПараметрыСобытия(Результат, ИмяСобытия, ДанныеСобытия); 
	Возврат РезультатОбработки;
...
Функция ОбработатьСобытие(ОбъектДрайвера, ПараметрыПодключения, Событие, Данные) Экспорт
	...
	// ++ НеМобильноеПриложение
	// ++ Локализация       
	ИначеЕсли Событие = "PIOT_Event" Тогда
		РезультатОбработки = ОбработатьСобытиеПИоТ(ПараметрыПодключения, Событие, Данные);                         
	// -- Локализация       
	...
	Иначе
		РезультатОбработки = ПараметрыСобытия(Ложь, "", Данные); 
		Возврат РезультатОбработки;
	КонецЕсли;            
...
Функция РезультатОперацииНаОборудовании(Результат = Ложь, ОписаниеОшибки = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	РезультатВыполнения.Вставить("ИдентификаторОперацииНаОборудовании");
	Возврат РезультатВыполнения;
...
// Функция добавляет медиафайл на дисплей покупателя.
//
// Параметры:
//   ОбъектДрайвера - ОбъектВнешнейКомпоненты
//   ПараметрыПодключения - Структура
...
//   см. РезультатОперацииНаОборудовании
//
Асинх Функция ДобавитьМедиафайлНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) Экспорт
	
	Если ПараметрыПодключения.РевизияИнтерфейса < 4008  Тогда
		ОписаниеОшибки = НСтр("ru='Устройство не поддерживает данную операцию.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "AddMediaFile", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);   
		Возврат РезультатВыполнения;
	КонецЕсли;
	
	Для Каждого МедиаФайл Из ДанныеОперации.МедиаФайлы Цикл
		
		ДанныеBase64 = МедиаФайл.Base64;
		Если Не ЗначениеЗаполнено(ДанныеBase64) 
			И ТипЗнч(МедиаФайл.Данные) = Тип("ДвоичныеДанные") Тогда
			ДанныеBase64 = Base64Строка(МедиаФайл.Данные);
		КонецЕсли;
		
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "AddMediaFile", ВМассив(Неопределено, МедиаФайл.ИмяФайла, "MEDIAFILE"));
			РезультатВызова = Ждать ОбъектДрайвера.ДобавитьМедиафайлАсинх(ПараметрыПодключения.ИДУстройства, МедиаФайл.ИмяФайла, ДанныеBase64); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
			Если РезультатВызова.Значение Тогда
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
			Иначе
		...
		Исключение    
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
			РезультатВыполнения = ОшибкаВызоваДрайвера("ДобавитьМедиафайл", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Прервать;
		КонецПопытки;
...
Асинх Функция РезультатыЗапросаКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		ДанныеОперации.СтатусРезультатаКод = Число(ДанныеОперации.СтатусРезультатаКод);
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Некорректно передан статус запроса от драйвера оборудования, ожидается число, а передано значение %1.'"), 
			ТипЗнч(ДанныеОперации.СтатусРезультатаКод));
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ТекстОшибки);;
	КонецПопытки;
...
Асинх Функция НачатьРаботуПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	ПроверкаИнициализацияПИоТ = МенеджерОборудованияКлиент.ПроверкаИнициализацияПИоТПройдена(ПараметрыПодключения.ИдентификаторУстройства);
	Если НЕ ПроверкаИнициализацияПИоТ Тогда 
		Попытка                                
			СтатусСостоянияXML = "";          
		...
		Если РезультатВызова.Значение Тогда              
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);         
			МенеджерОборудованияКлиент.УстановитьИнициализацияПИоТПройдена(ПараметрыПодключения.ИдентификаторУстройства);
		Иначе                                                       
			ТекстОшибки = СтрШаблон(НСтр("ru = '%1 не инициализирован.'"), НаименованиеТСПиОТ());
		...
		АутентификацииПередана = Ложь;
		Если ДанныеОперации.Свойство("ЗаписиАутентификацииПользователя") Тогда      
			ЗаписиАутентификацииПользователя = ДанныеОперации.ЗаписиАутентификацииПользователя;
			Для Каждого ЗаписьАутентификацииПользователя Из ЗаписиАутентификацииПользователя Цикл
				Попытка                                
					ПараметрыОперацииXML = ДанныеОперации.ПараметрыXML;
					ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PiotAuthentication", ВМассив(, ЗаписьАутентификацииПользователя));
						РезультатВызова = Ждать ОбъектДрайвера.АутентификацияПользователяПИоТАсинх(ПараметрыПодключения.ИДУстройства, ЗаписьАутентификацииПользователя); // Структура
					ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
				Исключение
					ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
				КонецПопытки;                                               
				АутентификацииПередана = Истина;
			КонецЦикла;
		КонецЕсли;  
		РезультатВыполнения.Вставить("АутентификацииПередана", АутентификацииПередана); 
...
Асинх Функция ПроверкаКМПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	ИспользоватьТСПИоТ = ПараметрыПодключения.Свойство("ИспользоватьТСПИоТ") И ПараметрыПодключения.ИспользоватьТСПИоТ;
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = НСтр("ru='Данная команда доступна только при использовании'") + Символы.НПП  + 
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotCheckKM", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
...
Асинх Функция СтатусПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	ПроверкаИнициализацияПИоТ = МенеджерОборудованияКлиент.ПроверкаИнициализацияПИоТПройдена(ПараметрыПодключения.ИдентификаторУстройства);
	Если НЕ ПроверкаИнициализацияПИоТ Тогда      
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Процедура инициализации ""%1"" ранее не была запущена.'"), Наименование1СТСПиОТ());
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotStatus", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
...
// Аутентификация пользователя ПИоТ
//
Асинх Функция АутентификацияПользователяПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	
	Если НЕ ПараметрыПодключения.ТипыОборудования.ККТ Или ПараметрыПодключения.РевизияИнтерфейса < 4005 Тогда
		ОписаниеОшибки = НСтр("ru='Драйвер не поддерживает операцию.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotAuthentication", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;   
	...
	Попытка                                
		ПараметрыОперацииXML = ДанныеОперации.ПараметрыXML;
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PiotAuthentication", ВМассив(, ПараметрыОперацииXML));
		РезультатВызова = Ждать ОбъектДрайвера.АутентификацияПользователяПИоТАсинх(ПараметрыПодключения.ИДУстройства, ПараметрыОперацииXML); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
		РезультатВыполнения = ОшибкаВызоваДрайвера("АутентификацияПользователяПИоТ", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
		Возврат РезультатВыполнения;
	КонецПопытки;
	...
	Если РезультатВызова.Значение Тогда              
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);     
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка аутентификаци пользователя в %1.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;                                  
...
// Сформировать событие эксплуатации ПИоТ
//
Асинх Функция СформироватьСобытиеЭксплуатацииПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	
	Если НЕ ПараметрыПодключения.ТипыОборудования.ККТ Или ПараметрыПодключения.РевизияИнтерфейса < 4005 Тогда
		ОписаниеОшибки = НСтр("ru='Драйвер не поддерживает операцию.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotCreateEvent", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;   
	...
	Попытка                                
		ПараметрыОперацииXML = ДанныеОперации.ПараметрыXML;
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PiotCreateEvent", ВМассив(, ПараметрыОперацииXML));
		РезультатВызова = Ждать ОбъектДрайвера.СформироватьСобытиеЭксплуатацииПИоТАсинх(ПараметрыПодключения.ИДУстройства, ПараметрыОперацииXML); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
		РезультатВыполнения = ОшибкаВызоваДрайвера("СформироватьСобытиеЭксплуатацииПИоТ", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
		Возврат РезультатВыполнения;
	КонецПопытки;
	...
	Если РезультатВызова.Значение Тогда              
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);     
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка формирования событие эксплуатации в %1.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;                                  
...
Асинх Функция СлужебнаяИнформацияПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) Экспорт
	...
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Данная команда доступна только при использовании ""%1""'"), Наименование1СТСПиОТ());
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotServiceInformation", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Информация %1 не получена.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
// Получить информацию о внешних системах ТС ПИоТ 
// 
// Параметры:
//  ОбъектДрайвера - ОбъектВнешнейКомпоненты 
//  ОписаниеДрайвера - Структура
...
//  Булево      
//
Асинх Функция ПолучитьИнформациюОВнешнихСистемахПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) Экспорт
	...
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Данная команда доступна только при использовании ""%1""'"), Наименование1СТСПиОТ());
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotInformation", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Попытка
		ИнформацияJSON = "";   
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ПолучитьИнформациюОВнешнихСистемах", ВМассив(, ИнформацияJSON));
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьИнформациюОВнешнихСистемахАсинх(ПараметрыПодключения.ИДУстройства, ИнформацияJSON); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
	...
	Если РезультатВызова.Значение Тогда              
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);     
		РезультатВыполнения.Вставить("ИнформацияJSON", РезультатВызова.Параметры[1]);
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Информация %1 не получена.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
Функция РезультатЭквайринговойОперации(Результат, ОписаниеОшибки, ПараметрыПодключения)
	...
	РезультатВыполнения.Вставить("СкидкаЭлектроннымиСертификатами");
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 3004 Тогда
		Попытка
			...
			Если РезультатВызова.Значение Тогда
				...
				ПараметрыПодключения.СкидкаЭлектроннымиСертификатами = ПараметрыТерминала.Свойство("ElectronicCertificateDiscount") И ВРег(ПараметрыТерминала.ElectronicCertificateDiscount) = "TRUE";
				...
				ПараметрыПодключения.ПолученияСтатусаОперации = ПараметрыТерминала.Свойство("OperationStatus") И ВРег(ПараметрыТерминала.OperationStatus) = "TRUE";
...
Асинх Функция ЭквайринговаяОперация(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Попытка        
		...
		Если РезультатВызова.Значение Тогда        
			...
			РезультатВыполнения.ИдентификаторОперацииНаОборудовании = ДанныеОперации.ИдентификаторОперацииНаОборудовании;
...
// Функция получает статус операции.
//
Асинх Функция СтатусОперацииXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	
	Если ПараметрыПодключения.РевизияИнтерфейса < 4008 Или НЕ ПараметрыПодключения.СписокОперацийПоКартам Тогда
		ОписаниеОшибки = НСтр("ru='Команда не поддерживается драйвером.'");
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;   
	КонецЕсли; 
	
	ИдентификаторОперацииНаОборудовании = ДанныеОперации.ИдентификаторОперацииНаОборудовании;
	ИсходящиеПараметрыXML = ""; 
	Статус = 1;
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "OperationStatus", ВМассив(Неопределено, ИдентификаторОперацииНаОборудовании, ИсходящиеПараметрыXML, Статус));
		РезультатВызова = Ждать ОбъектДрайвера.СтатусОперацииАсинх(ПараметрыПодключения.ИДУстройства, ИдентификаторОперацииНаОборудовании, ИсходящиеПараметрыXML, Статус); // Структура
		ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
		
		Если РезультатВызова.Значение Тогда        
			Статус = РезультатВызова.Параметры[3];
			РезультатВыполнения = РезультатЭквайринговойОперации(Истина, Неопределено, ПараметрыПодключения); 
			РезультатВыполнения.ИдентификаторОперацииНаОборудовании = ДанныеОперации.ИдентификаторОперацииНаОборудовании;
			РезультатВыполнения.Вставить("ИдентификаторОперацииНаОборудовании", ИдентификаторОперацииНаОборудовании);
			РезультатВыполнения.Вставить("ИсходящиеПараметрыXML", РезультатВызова.Параметры[2]);   
			РезультатВыполнения.Вставить("ОперацияВыполнена", Статус = 1);   
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера, ЗаписьЛога);
		КонецЕсли;      
	...
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗавершитьЗаписьЛогаИсключением(ЗаписьЛога, ТекстОшибки);
		РезультатВыполнения = ОшибкаВызоваДрайвера("OperationStatus", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Асинх Функция ЭквайринговаяОперацияСЭлектроннымСертификатом(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	СкидкаЭлектроннымиСертификатами = ?(ДанныеОперации.СкидкаЭлектроннымиСертификатами <> Неопределено, ДанныеОперации.СкидкаЭлектроннымиСертификатами, 0);
	...
	Если (СкидкаЭлектроннымиСертификатами > 0) Тогда
		ОписаниеОшибки = НСтр("ru='Терминал не поддерживает операцию cкидки по ЭС.'"); 
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, Команда, ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;
	КонецЕсли;  
...
Асинх Функция ЭквайринговаяОперацияСЭлектроннымСертификатомXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Если НЕ ПараметрыПодключения.ОплатаЭлектроннымиСертификатами Тогда
		...
		Возврат РезультатВыполнения;
	КонецЕсли;
	Если (СкидкаЭлектроннымиСертификатами > 0) И НЕ ПараметрыПодключения.СкидкаЭлектроннымиСертификатами Тогда
		ОписаниеОшибки = НСтр("ru='Терминал не поддерживает операцию cкидки по ЭС.'"); 
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, Команда, ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;
	...
	Попытка        
		...
		Если РезультатВызова.Значение Тогда                         
			...
			РезультатВыполнения.ИдентификаторКорзины = ИдентификаторКорзины;
			РезультатВыполнения.СуммаЭлектронногоСертификата = СуммаЭлектронногоСертификата;
			РезультатВыполнения.СкидкаЭлектроннымиСертификатами = СкидкаЭлектроннымиСертификатами;
			РезультатВыполнения.СуммаСобственныхСредств = СуммаСобственныхСредств;
...
//
Асинх Функция ПодключитьТСПИоТАсинх(ПараметрыПодключения, ОписаниеДрайвера) 
	...
	Попытка  
		ИДПИоТ = "";  
		ОбъектДрайвера = РезультатПодключения.ПодключаемыйМодуль;
		ТСПИоТПараметрыXML = ПараметрыПодключения.ТСПИоТПараметрыXML;  
	#Если НЕ ВебКлиент Тогда 
		ТСПИоТПараметрыXML = СтрЗаменить(ТСПИоТПараметрыXML, "%Path%", XMLСтрока(ОписаниеДрайвера.ОсновнойДрайверПустьГлавнойDLL));
	#КонецЕсли
		РезультатВызова = Ждать ОбъектДрайвера.ПодключитьПИоТАсинх(ИДПИоТ, ТСПИоТПараметрыXML); // Структура  
		Если РезультатВызова.Значение Тогда
...
Асинх Функция СозданиеДанныхЭкземпляраТСПИоТ(ПараметрыПодключения)  Экспорт
	
	РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
#Если НЕ ВебКлиент Тогда 
	Если Не ПустаяСтрока(ПараметрыПодключения.ТСПИоТПутьДанных) Тогда
		
		КаталогНаДиске = Новый Файл(ПараметрыПодключения.ТСПИоТПутьДанных);
		Если КаталогНаДиске.Существует() Тогда
			Возврат РезультатВыполнения;
		КонецЕсли;                   
		
		Попытка
			Ждать СоздатьКаталогАсинх(ПараметрыПодключения.ТСПИоТПутьДанных);
		Исключение
			РезультатВыполнения.Результат = Ложь;   
			РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Ошибка создания данных экземпляра ""ТС ПИоТ"".'");
		КонецПопытки;
		
		РазделительПути = ПолучитьРазделительПути();          
		МодульМенеджерОборудованияТСПИоТВызовСервера = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("МенеджерОборудованияТСПИоТВызовСервера");
		Сертификаты = МодульМенеджерОборудованияТСПИоТВызовСервера.ПолучитьСертификатыИзМакета(ПараметрыПодключения.ИдентификаторОбъекта, ПараметрыПодключения.ТСПИоТПлатформа);
		Для Каждого Сертификат Из Сертификаты Цикл       
			ИмяФайла = ПараметрыПодключения.ТСПИоТПутьДанных + РазделительПути + Сертификат.ИмяФайла;  
			Запись = Новый ЗаписьТекста(ИмяФайла, КодировкаТекста.ANSI); 
			Запись.Записать(Сертификат.Содержимое);
			Запись.Закрыть();
		КонецЦикла;
	КонецЕсли;                   
#КонецЕсли
