...
Процедура НапечататьРегламентированныйОтчетPDF(СсылкаНаОтчет, ПараметрыПечати) Экспорт
	
	СписокОтчетов = Новый СписокЗначений;
	
	СписокОтчетов.Добавить(СсылкаНаОтчет);
	
	РегламентированнаяОтчетностьКлиент.НапечататьОтчеты(СписокОтчетов, "СформироватьБланкPDF", ПараметрыПечати);
