...
Процедура ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, НеОтражатьДанныеЭлектронногоДокумента) Экспорт
	
	Если Не ЭтоДокументЭПД(ЗагруженныйДокумент.ТипДокумента) Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрНачинаетсяС(ВРег(ЗагруженныйДокумент.ОписаниеДанных.ИмяФайла), "DP_UVUTOCH") Тогда
		НеОтражатьДанныеЭлектронногоДокумента = Истина;
		Возврат;
	КонецЕсли;
	
	ТипРегламента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ЗагруженныйДокумент.ЭлектронныйДокумент, "ТипРегламента");
		
	Если ТипРегламента = Перечисления.ТипыРегламентовЭДО.Черновик Тогда
		НеОтражатьДанныеЭлектронногоДокумента = Истина;
	КонецЕсли;
...
Процедура ПриОпределенииВидимостиКомандыСозданияДокументаНаОснованииЧерновика(КомандаВидна,
	...
	КомандаВидна = ЭтоДокументЭПД(ТипДокументаЧерновика);
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Документы.ЭлектронноеПоручениеЭкспедитору, Истина);
	Списки.Вставить(Метаданные.Документы.ЭлектроннаяЭкспедиторскаяРасписка, Истина);
	Списки.Вставить(Метаданные.Документы.ЭлектроннаяCкладскаяРасписка, Истина);
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Описание = Описание + "
	|Документ.ЭлектроннаяCкладскаяРасписка.Чтение.Организации
	|Документ.ЭлектроннаяCкладскаяРасписка.Изменение.Организации
	|Документ.ЭлектроннаяЭкспедиторскаяРасписка.Чтение.Организации
	|Документ.ЭлектроннаяЭкспедиторскаяРасписка.Изменение.Организации
	|Документ.ЭлектронноеПоручениеЭкспедитору.Чтение.Организации
	|Документ.ЭлектронноеПоручениеЭкспедитору.Изменение.Организации
...
Процедура ПриОпределенииПриоритетаРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой, Приоритет) Экспорт
	...
	Если КодТранзакции = "ShipperInformation"
		...
		Или КодТранзакции = "CarrierInformation_1"
		Или КодТранзакции = "EPE_T1" Тогда
...
Процедура ПриОпределенииСтандартныхТиповДокументов(ТипыДокументов) Экспорт
	...
	ТипыДокументов.Вставить("ЭПЭ", Перечисления.ТипыДокументовЭДО.ЭПЭ);
	ТипыДокументов.Вставить("ЭЭР", Перечисления.ТипыДокументовЭДО.ЭЭР);
	ТипыДокументов.Вставить("ЭСР", Перечисления.ТипыДокументовЭДО.ЭСР);
	
КонецПроцедуры

// Возвращает, включен демонстрационный режим или нет
// 
// Возвращаемое значение:
//  Булево - Демо режим включен
Функция ДемоРежимВключен() Экспорт
	УстановитьПривилегированныйРежим(Истина);
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЭПДДемоРежим", "ДемоРежимВключен", Ложь, , "");
КонецФункции    

// Включить или выключить демонстрационный режим
// 
// Параметры:
//  Включить - Булево - Включить или выключить демонстрационный режим
Процедура ДемоРежимВключить( Включить ) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ЭПДДемоРежим", "ДемоРежимВключен", Включить, , "");
КонецПроцедуры

// Возвращает, статус приема экспедиторских документов в ГИС
// 
// Возвращаемое значение:
//  Булево - Истина, если документы принимаются
Функция СтатусПриемаЭкспедиторскихДокументовВГисЭПД() Экспорт  
	
	Результата = Истина;
	
	ПолныйАдрес =  СервисНастроекЭДО.АдресОблачногоХранилищаНастроек() + "/epd/EpdExpDocNotSend2Gis.json";
...
// При определении параметров сервиса 1С-ЭДО.
// 
// Параметры:
//  ПараметрыСервиса Параметры сервиса
Процедура ПриОпределенииПараметровСервиса1СЭДО(ПараметрыСервиса) Экспорт
	
	Если ДемоРежимВключен() Тогда
		
		ПараметрыСервиса.АдресВебСервиса = "https://1ctest.ru/API";
		ПараметрыСервиса.АдресДляПроверкиАктивности = "https://1ctest.ru/";
		
	КонецЕсли;
	
КонецПроцедуры 

// При заполнении параметров карточки регистрации.
// 
// Параметры:
//  ЭДКарточки ЭДКарточки
Процедура ПриЗаполненииПараметровКарточкиРегистрации(ЭДКарточки) Экспорт
	
	Если ДемоРежимВключен() Тогда
		
		ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
		ЭДКарточки.ЗаписатьАтрибут("Name", "ЭПДДемо");
		ЭДКарточки.ЗаписатьАтрибут("Value", "true");
...
// Перед отправкой регистрационного пакета1 СЭДО.
// 
// Параметры:
//  ПараметрыВыполненияОперации Параметры выполнения операции
Процедура ПередОтправкойРегистрационногоПакета1СЭДО(ПараметрыВыполненияОперации) Экспорт
	
	Если ДемоРежимВключен() Тогда
		
		ПараметрыВыполненияОперации.Заголовки.Вставить("Epd-Demo", "true");
		
	КонецЕсли;
	
КонецПроцедуры

// При поиске учетных записей абонентов.
// 
// Параметры:
//  Идентификаторы Идентификаторы
//  КПП КПП
//  Результат Результат
Процедура ПриПоискеУчетныхЗаписейАбонентов(Знач Идентификаторы, Знач КПП, Результат) Экспорт
...
// При заполнении данных абонентов из сервиса настроек.
// 
// Параметры:
//  Отбор Отбор
//  Отказ Отказ
//  КонтекстДиагностики Контекст диагностики
//  Результат Результат
Процедура ПриЗаполненииДанныхАбонентовИзСервисаНастроек(Отбор, Отказ, КонтекстДиагностики, Результат) Экспорт
...
Процедура ПриЗаполненииОписанияФайла(Данные, ОписаниеФайла) Экспорт
	
	Если Не ЭтоТитулЭПД(ОписаниеФайла.ИмяФайла) Тогда
		Возврат;
	КонецЕсли;
	
	ЭлектронныйДокумент = ЭлектронныеДокументыЭДО.ЭлектронныйДокументСообщенияЭДО(Данные.Сообщение);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ЭлектронныйДокумент", ЭлектронныйДокумент);
	Запрос.Текст = "ВЫБРАТЬ
	|	ОбъектыУчетаДокументовЭДО.ОбъектУчета КАК ДокументЭПД,
	...
	Если Выборка.Следующий() Тогда
		ОписаниеФайла.Вставить("ДокументЭПД", Выборка.ДокументЭПД);
		ОписаниеФайла.Вставить("ТипРегламента", Выборка.ТипРегламента);
	КонецЕсли;
		
КонецПроцедуры

Функция ПриФормированияДанныхФайлаЭД(ОписаниеФайла) Экспорт
	...
	Попытка
		...
		СоответствиеПрефикса = ПолучитьСоответствиеПрефиксаПоИдФайла(ОписаниеФайла.ИмяФайла);
	
		МенеджерОбъекта = СоответствиеПрефикса.Менеджер;
		ИмяСхемы = СоответствиеПрефикса.ИмяСхемы;
		
		МакетСхема = МенеджерОбъекта.ПолучитьМакет("Схема" + ИмяСхемы);
	
		ДанныеПоXsdСхеме = СоздатьФабрикуПоСхемеXSD(МакетСхема.ПолучитьТекст());
		
		Пакет = Неопределено;
		Если ДанныеПоXsdСхеме.targetNamespace <> Неопределено Тогда
			Пакет = ДанныеПоXsdСхеме.Фабрика.Пакеты.Получить(ДанныеПоXsdСхеме.targetNamespace);
		ИначеЕсли ДанныеПоXsdСхеме.xmlns <> Неопределено Тогда
			Пакет = ДанныеПоXsdСхеме.Фабрика.Пакеты.Получить(ДанныеПоXsdСхеме.xmlns);
		Иначе
			Пакет = ДанныеПоXsdСхеме.Фабрика.Пакеты[0];
		КонецЕсли;  
		...
		ПотокДанныхФайла = ОписаниеФайла.ДвоичныеДанные.ОткрытьПотокДляЧтения();
		ЧтениеXML.ОткрытьПоток(ПотокДанныхФайла);

		ДанныеФайлаЭД = ДанныеПоXsdСхеме.Фабрика.ПрочитатьXML(ЧтениеXML, СвойствоФайл.Тип);
	...
	Исключение
		...
		|%1'", КодОсновногоЯзыка);
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ОписаниеФайла.ИмяФайла);
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
		
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИмяСобытия = НСтр("ru = 'ЭПД.ПриФормированияДанныхФайлаЭД'", КодОсновногоЯзыка);
...
Функция ПриЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	
	Возврат ЭтоТипРегламентаЭПД(СодержаниеДокумента.ТипРегламента);
	
КонецФункции

Процедура ПриЗаполненииДанныхПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта, ЗаполнениеДанныхПодписантаДляЭПД, Ошибки) Экспорт
	
	Если Не ОписаниеФайла.Свойство("ДокументЭПД") Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДанныеПодписанта.СертификатПодписи) Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось заполнить подписанта. Не заполнен сертификат подписи'");
		ОбщегоНазначенияБЭД.ДобавитьОшибку(Ошибки, ТекстОшибки);
		Возврат;
	КонецЕсли;
	
	ЗаполнениеДанныхПодписантаДляЭПД = Истина;
	
	Если ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭТрН Тогда
		
		ЗаполнитьДанныеПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЗН Тогда
		
		ЗаполнитьДанныеПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСВ Тогда
		
		ЗаполнитьДанныеПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЗЗ Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЛ Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭДФ Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта); 
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	ИначеЕсли ОписаниеФайла.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР Тогда
		
		ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта);
		
	Иначе
		
		ЗаполнениеДанныхПодписантаДляЭПД = Ложь;
		
		ТекстОшибки = НСтр("ru = 'Тип документа для заполнения информации о подписанте не поддерживается!'");
		ОбщегоНазначенияБЭД.ДобавитьОшибку(Ошибки, ТекстОшибки);
...
Функция ПриУдаленииПространстваИмен(ДанныеФайлаЭД, УдалениеПространстваИменДляЭПД) Экспорт
	
	Если Не ЭтоТитулЭПД(ДанныеФайлаЭД.ИдФайл) Тогда
		Возврат Неопределено;
	КонецЕсли; 
	
	Кодировка = "windows-1251";
	
	Если ТитулДолженБытьВКодировкеUtf8(ДанныеФайлаЭД.ИдФайл) Тогда
		Кодировка = "utf-8";	
	КонецЕсли;
	
	ДанныеФайлаЭДФабрикаXDTO = ДанныеФайлаЭД.Фабрика();

	ЗаписьОбъекта = Новый ЗаписьXML;
	ЗаписьОбъекта.УстановитьСтроку(Кодировка);
	ЗаписьОбъекта.ЗаписатьОбъявлениеXML();
	
	ДанныеФайлаЭДФабрикаXDTO.ЗаписатьXML(ЗаписьОбъекта, ДанныеФайлаЭД, "Файл");
	...
	ДвоичныеДанные = ПолучитьДвоичныеДанныеИзСтроки(ТекстОбъекта, Кодировка);
	
	УдалениеПространстваИменДляЭПД = Истина;
...
Функция ЭтоТипРегламентаЭПД(ТипРегламента) Экспорт
	...
	Возврат ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭТрН
			...
			Или ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ
			Или ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР
			Или ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР;
...
Функция РегламентЭПД(ТипРегламента) Экспорт
	...
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ Тогда
		Возврат ОбщегоНазначения.ОбщийМодуль("РегламентыЭПД_ЭПЭ");
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР Тогда
		Возврат ОбщегоНазначения.ОбщийМодуль("РегламентыЭПД_ЭЭР");
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР Тогда
		Возврат ОбщегоНазначения.ОбщийМодуль("РегламентыЭПД_ЭСР");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ДвоичныеДанныеДокументаЭПД(Объект, Данные = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		...
		МассивВозможныхПустыхЗначений.Добавить("//Файл/Документ/СодИнфГО/СвГруз/ОбЦеннГр/СтЦеннГр");
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		МенеджерОбъекта = Документы.ЭлектронноеПоручениеЭкспедитору;
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяЭкспедиторскаяРасписка;
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяCкладскаяРасписка;
	КонецЕсли;
	...
	Для НомерСтроки = 1 По КоличествоСтрокМакета Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеРеквизита) 
			Или МассивВозможныхПустыхЗначений.Найти(Узел) <> Неопределено Тогда	
			...
			Для Каждого СтрТЧ Из ДанныеФормирования[КиЗ.Ключ] Цикл
			    ...
			    Для Каждого Колонка Из СтрТЧ Цикл
					...
					Если ОбластьКолонкаНайдено <> Неопределено Тогда
						...
						Если ЗначениеЗаполнено(Колонка.Значение) 
							Или МассивВозможныхПустыхЗначений.Найти(УзелКолонка) <> Неопределено Тогда
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ 
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		УстановитьПоследовательностьУзловРекурсивно(ОбъектXDTO);	
	КонецЕсли;
...
//	СоздаватьПриглашениеДляЭПДАвтоматически - Булево - Признак который необходимо заполнить
Процедура ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки,
	ПараметрыПриглашения, Контрагент, СоздаватьПриглашениеДляЭПДАвтоматически = Ложь) Экспорт
	...
	Если Таблица.Найти(РезультатРаспаковки.Карточка.ТипДокумента, 
	"ТипДокументаДляДополнительныхДанныхСтрокой") <> Неопределено Тогда
		СоздаватьПриглашениеДляЭПДАвтоматически = Истина;
	КонецЕсли;
...
// 
// Параметры:
//  ДанныеДокумента - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,
//  ДокументСсылка.ЭлектронныйДокументИсходящийЭДО,СправочникСсылка.ВидыДокументовЭДО
...
// @skip-check function-return-types
// @skip-check doc-comment-ref-link
Функция ПриРаспознаванииДокумента(ДанныеДокумента, УспешностьРаспознования) Экспорт
	
	ТипДокумента = Неопределено;

	Если ТипЗнч(ДанныеДокумента) = Тип("СправочникСсылка.ВидыДокументовЭДО") Тогда 
	
		УспешностьРаспознования = ЭтоДокументЭПД(ДанныеДокумента);
		ТипДокумента = ДанныеДокумента;
		
	ИначеЕсли ТипЗнч(ДанныеДокумента) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО")
		Или ТипЗнч(ДанныеДокумента) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящийЭДО") Тогда
			
			ТипДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДокумента, "ВидДокумента.ТипДокумента");
			
			Если ТипДокумента = Перечисления.ТипыДокументовЭДО.Черновик Тогда
				
				ТипДокументаЧерновикаЗначение = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеДокумента,
				"ЧерновикВладелец.ВидЧерновика.ТипДокумента");
				
				Если ЗначениеЗаполнено(ТипДокументаЧерновикаЗначение) Тогда
					ТипДокументаЧерновика = ТипДокументаЧерновикаЗначение.ЧерновикВладелецВидЧерновикаТипДокумента;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ТипДокументаЧерновика)
				 И (ТипДокументаЧерновика = Перечисления.ТипыДокументовЭДО.ЭТрН 
				 	Или ТипДокументаЧерновика = Перечисления.ТипыДокументовЭДО.ЭЗЗ) Тогда
			...
			Иначе
				...
				УспешностьРаспознования = ЭтоДокументЭПД(ТипДокумента);

			КонецЕсли;
			
	КонецЕсли;
	
	Если УспешностьРаспознования Тогда
...
Функция ПредставлениеФайлаПрикладногоЭлектронногоДокумента(ДанныеФайла) Экспорт
	...
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ Тогда
		МенеджерОбъекта = Документы.ЭлектронныйДоговорФрахтования;
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		МенеджерОбъекта = Документы.ЭлектронноеПоручениеЭкспедитору;
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяЭкспедиторскаяРасписка;
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяCкладскаяРасписка;
	КонецЕсли;
...
Функция ЭтоДокументЭПД(ТипДокумента) Экспорт
	...
	Возврат ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН 
				...
				Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ
				Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР
				Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР;
...
Процедура ЗаполнитьПараметрыЭлектронногоДокумента(ИсточникДанныхСсылка, ПараметрыИсточника) Экспорт
	...
	ИначеЕсли ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументСсылка.ЭлектронныйДоговорФрахтования")
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору")
		Или ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументОбъект.ЭлектронноеПоручениеЭкспедитору") Тогда
		ПараметрыИсточника.Тип = Перечисления.ТипыДокументовЭДО.ЭПЭ;
		ПараметрыИсточника.Направление = ?(ИсточникДанныхСсылка.ЭтоВходящий, Перечисления.НаправленияЭДО.Входящий, Перечисления.НаправленияЭДО.Исходящий);
		
		Если ЗначениеЗаполнено(ИсточникДанныхСсылка.СсылкаТитулЗаказчикаСоставитель) Тогда
			СоставительПервогоТитула = ИсточникДанныхСсылка.СсылкаТитулЗаказчикаСоставитель;
		Иначе
			СоставительПервогоТитула = ИсточникДанныхСсылка.СсылкаТитулЗаказчикаЗаказчик;	
		КонецЕсли;
		
		Если ИсточникДанныхСсылка.ЭтоВходящий = Истина Тогда
			ПараметрыИсточника.Организация = ИсточникДанныхСсылка.СсылкаТитулЗаказчикаЭкспедитор; 
			ПараметрыИсточника.Контрагент = СоставительПервогоТитула;
		Иначе
			ПараметрыИсточника.Организация = СоставительПервогоТитула; 
			ПараметрыИсточника.Контрагент = ИсточникДанныхСсылка.СсылкаТитулЗаказчикаЭкспедитор; 
		КонецЕсли;
	ИначеЕсли ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка")
		Или ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументОбъект.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
		ПараметрыИсточника.Тип = Перечисления.ТипыДокументовЭДО.ЭЭР;
		ПараметрыИсточника.Направление = ?(ИсточникДанныхСсылка.ЭтоВходящий, Перечисления.НаправленияЭДО.Входящий, Перечисления.НаправленияЭДО.Исходящий);
		
		Если ЗначениеЗаполнено(ИсточникДанныхСсылка.СсылкаТитулЭкспедитораСоставитель) Тогда
			СоставительПервогоТитула = ИсточникДанныхСсылка.СсылкаТитулЭкспедитораСоставитель;
		Иначе
			СоставительПервогоТитула = ИсточникДанныхСсылка.СсылкаТитулЭкспедитораЭкспедитор;	
		КонецЕсли;
		
		Если ИсточникДанныхСсылка.ЭтоВходящий = Истина Тогда
			ПараметрыИсточника.Организация = ИсточникДанныхСсылка.СсылкаТитулЭкспедитораЗаказчик; 
			ПараметрыИсточника.Контрагент = СоставительПервогоТитула;
		Иначе
			ПараметрыИсточника.Организация = СоставительПервогоТитула; 
			ПараметрыИсточника.Контрагент = ИсточникДанныхСсылка.СсылкаТитулЭкспедитораЗаказчик; 
		КонецЕсли;
	ИначеЕсли ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка")
		Или ТипЗнч(ИсточникДанныхСсылка) = Тип("ДокументОбъект.ЭлектроннаяCкладскаяРасписка") Тогда
		ПараметрыИсточника.Тип = Перечисления.ТипыДокументовЭДО.ЭСР;
		ПараметрыИсточника.Направление = ?(ИсточникДанныхСсылка.ЭтоВходящий, Перечисления.НаправленияЭДО.Входящий, Перечисления.НаправленияЭДО.Исходящий);
...
Функция ЭтоТитулЭПД(ИдФайл) Экспорт
	...
	Возврат СтрНачинаетсяС(ИдФайл, "ON_TRNACLGROT")
			...
			// ЭПЭ
			Или СтрНачинаетсяС(ИдФайл, "ON_POREXPKLT")
			Или СтрНачинаетсяС(ИдФайл, "ON_POREXPEXP")
			Или СтрНачинаетсяС(ИдФайл, "ON_POREXPOTZ")
			
			// ЭЭР
			Или СтрНачинаетсяС(ИдФайл, "ON_EXPRASP") 
			
			// ЭСР
			Или СтрНачинаетсяС(ИдФайл, "ON_SKLADRASP")
		;
...
Функция ПрочитатьСодержаниеДокумента(ОписаниеФайла) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_DOGFRA") Тогда
		...
		Если СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_DOGFRAKHTEL_") Тогда
			...
			Содержание.НомерДокумента = КлючеваяИнформацияПоТитулу.Номер;
		КонецЕсли; 
	ИначеЕсли СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_POREXP") Тогда
		Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
		Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ;
		Содержание.Формат = "EPE";
		// Дата и номер есть только в первом титуле
		Если СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_POREXPKLT_") Тогда
			Поток = ОписаниеФайла.ДвоичныеДанные.ОткрытьПотокДляЧтения();
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.ОткрытьПоток(Поток);		
			ОбъектXDTO = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
			...
			Содержание.НомерДокумента = КлючеваяИнформацияПоТитулу.Номер;
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_EXPRASP") Тогда
		Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР;
		Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР;
		Содержание.Формат = "EER";
		// Дата и номер есть только в первом титуле
		Если СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_EXPRASP_") Тогда
			Поток = ОписаниеФайла.ДвоичныеДанные.ОткрытьПотокДляЧтения();
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.ОткрытьПоток(Поток);		
			ОбъектXDTO = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
			...
			Содержание.НомерДокумента = КлючеваяИнформацияПоТитулу.Номер;
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_SKLADRASP") Тогда
		Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР;
		Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР;
		Содержание.Формат = "ESR";
		// Дата и номер есть только в первом титуле
		Если СтрНачинаетсяС(ОписаниеФайла.ИмяФайла, "ON_SKLADRASP_") Тогда
			Поток = ОписаниеФайла.ДвоичныеДанные.ОткрытьПотокДляЧтения();
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.ОткрытьПоток(Поток);		
			ОбъектXDTO = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
...
Функция ЭтоТитулОтправителя(ТипЭлементаРегламента) Экспорт
	...
	Возврат ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1
		...
		Или ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1
		Или ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1
		Или ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1;
...
Функция ЭтоТитулПолучателя(ТипЭлементаРегламента) Экспорт
	...
	Возврат ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2
		...
		Или ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2;
...
Функция ПервыйТитулДокумента(ТипДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		Возврат Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1;
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		Возврат Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1;
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		Возврат Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1;
	КонецЕсли;
...
Процедура ДобавитьТипыДокументовЭПД(Таблица) Экспорт
	...
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.ТипДокументаСтрокой = "Other";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
	НоваяСтрока.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1;
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ;
	НоваяСтрока.ТипДокументаДляДополнительныхДанныхСтрокой = "EPE_T1";
	НоваяСтрока.ЭтоОтветнаяПодпись = Неопределено;
	
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.ТипДокументаСтрокой = "Other";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
	НоваяСтрока.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2;
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ;
	НоваяСтрока.ТипДокументаДляДополнительныхДанныхСтрокой = "EPE_T2";
	НоваяСтрока.ЭтоОтветнаяПодпись = Неопределено;
	
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.ТипДокументаСтрокой = "Other";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
	НоваяСтрока.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3;
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ;
	НоваяСтрока.ТипДокументаДляДополнительныхДанныхСтрокой = "EPE_T3";
	НоваяСтрока.ЭтоОтветнаяПодпись = Неопределено;
	
	//Электронная экспедиторская расписка
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.ТипДокументаСтрокой = "Other";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР;
	НоваяСтрока.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1;
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР;
	НоваяСтрока.ТипДокументаДляДополнительныхДанныхСтрокой = "EER_T1";
	НоваяСтрока.ЭтоОтветнаяПодпись = Неопределено;
	
	//Электронная экспедиторская расписка
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.ТипДокументаСтрокой = "Other";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР;
	НоваяСтрока.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1;
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР;
	НоваяСтрока.ТипДокументаДляДополнительныхДанныхСтрокой = "ESR_T1";
	НоваяСтрока.ЭтоОтветнаяПодпись = Неопределено;
...
Процедура ПриРаспознанииДокумента(ДанныеДокумента, ЭлементСхемы) Экспорт
	
	ЭтоЭПДДокумент = Ложь;
	
	Если ТипЗнч(ДанныеДокумента) = Тип("СправочникСсылка.ВидыДокументовЭДО") Тогда
	
		ТипДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДокумента, "ТипДокумента");
		
		ЭтоЭПДДокумент = ЭтоДокументЭПД(ТипДокумента);
	...
	ИначеЕсли ТипЗнч(ДанныеДокумента) = Тип("Структура") Тогда
		...
		Если ТипДокумента = Перечисления.ТипыДокументовЭДО.Черновик Тогда
			...
			ТипДокументаЧерновикаЗначение = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеДокумента.Ссылка,
				"ЧерновикВладелец.ВидЧерновика.ТипДокумента");
...
Функция ПредставлениеИнформацииОтправителя(ТипДокумента, НомерДокумента, ДатаДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ Тогда
		ТитулГрузоотправителяСтрока = НСтр("ru='Информация фрахтователя (Титул 1)'"); 
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		ТитулГрузоотправителяСтрока = НСтр("ru='Информация заказчика (Титул 1)'");
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		ТитулГрузоотправителяСтрока = НСтр("ru='Информация экспедитора (Титул 1)'");
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		ТитулГрузоотправителяСтрока = НСтр("ru='Информация экспедитора (Титул 1)'");
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныхДанныеКарточкиЭПД(СообщениеЭДО, ДополнительныеДанные, ТипДокументаСтрокой) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЭлектронноеПоручениеЭкспедитору.ВидДокумента,
	|	ЭлектронноеПоручениеЭкспедитору.ИдентификаторЗаказчика КАК Грузоотправитель,
	|	ЭлектронноеПоручениеЭкспедитору.ИдентификаторЭкспедитора КАК Грузоперевозчик,
	|	НЕОПРЕДЕЛЕНО КАК Грузополучатель,
	|	НЕОПРЕДЕЛЕНО КАК Оформитель,
	|	НЕОПРЕДЕЛЕНО КАК Медосмотр,
	|	НЕОПРЕДЕЛЕНО КАК Техконтроль,
	|	НЕОПРЕДЕЛЕНО КАК Одометр,
	|	ЭлектронноеПоручениеЭкспедитору.ЭтоВходящий КАК ЭтоВходящий,
	|	ЭлектронноеПоручениеЭкспедитору.ТекущийТитул КАК ТекущийТитул,
	|	ЭлектронноеПоручениеЭкспедитору.УИДМинтранс КАК МинтрансИД,
	|	""""
	|ИЗ
	|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектронноеПоручениеЭкспедитору КАК ЭлектронноеПоручениеЭкспедитору
	|		ПО ОбъектыУчетаДокументовЭДО.ОбъектУчета = ЭлектронноеПоручениеЭкспедитору.Ссылка
	|ГДЕ
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = &ЭлектронныйДокумент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЭлектроннаяЭкспедиторскаяРасписка.ВидДокумента,
	|	ЭлектроннаяЭкспедиторскаяРасписка.ИдентификаторЗаказчика КАК Грузоотправитель,
	|	ЭлектроннаяЭкспедиторскаяРасписка.ИдентификаторЭкспедитора КАК Грузоперевозчик,
	|	НЕОПРЕДЕЛЕНО КАК Грузополучатель,
	|	НЕОПРЕДЕЛЕНО КАК Оформитель,
	|	НЕОПРЕДЕЛЕНО КАК Медосмотр,
	|	НЕОПРЕДЕЛЕНО КАК Техконтроль,
	|	НЕОПРЕДЕЛЕНО КАК Одометр,
	|	ЭлектроннаяЭкспедиторскаяРасписка.ЭтоВходящий КАК ЭтоВходящий,
	|	ЭлектроннаяЭкспедиторскаяРасписка.ТекущийТитул КАК ТекущийТитул,
	|	ЭлектроннаяЭкспедиторскаяРасписка.УИДМинтранс КАК МинтрансИД,
	|	""""
	|ИЗ
	|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяЭкспедиторскаяРасписка КАК ЭлектроннаяЭкспедиторскаяРасписка
	|		ПО ОбъектыУчетаДокументовЭДО.ОбъектУчета = ЭлектроннаяЭкспедиторскаяРасписка.Ссылка
	|ГДЕ
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = &ЭлектронныйДокумент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЭлектроннаяCкладскаяРасписка.ВидДокумента,
	|	ЭлектроннаяCкладскаяРасписка.ИдентификаторЗаказчика КАК Грузоотправитель,
	|	ЭлектроннаяCкладскаяРасписка.ИдентификаторЭкспедитора КАК Грузоперевозчик,
	|	НЕОПРЕДЕЛЕНО КАК Грузополучатель,
	|	НЕОПРЕДЕЛЕНО КАК Оформитель,
	|	НЕОПРЕДЕЛЕНО КАК Медосмотр,
	|	НЕОПРЕДЕЛЕНО КАК Техконтроль,
	|	НЕОПРЕДЕЛЕНО КАК Одометр,
	|	ЭлектроннаяCкладскаяРасписка.ЭтоВходящий КАК ЭтоВходящий,
	|	ЭлектроннаяCкладскаяРасписка.ТекущийТитул КАК ТекущийТитул,
	|	ЭлектроннаяCкладскаяРасписка.УИДМинтранс КАК МинтрансИД,
	|	""""
	|ИЗ
	|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяCкладскаяРасписка КАК ЭлектроннаяCкладскаяРасписка
	|		ПО ОбъектыУчетаДокументовЭДО.ОбъектУчета = ЭлектроннаяCкладскаяРасписка.Ссылка
	|ГДЕ
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = &ЭлектронныйДокумент";
...
Процедура ПриФормированииТаблицыТиповЭлементовРегламента(Таблица) Экспорт
	...
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
	"EPE_T1", Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
	"EPE_T2", Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
	"EPE_T3", Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3);
	
	//Экспедиторская расписка
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
	"EER_T1", Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1);
	
	//Складская расписка
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
	"ESR_T1", Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1);
...
Процедура ПриФормированииТаблицыТиповРегламентов(Таблица) Экспорт
	...
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.КодРегламента = "EPE";
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ;
	
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.КодРегламента = "EER";
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР;
	
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.КодРегламента = "ESR";
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР;
...
Процедура ПриОпределенииИнформацииОтправителя(Знач ТипРегламента, Знач ДанныеЭлементовСхемы, ИнформацияОтправителя) Экспорт
	...
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ Тогда
		ИнформацияОтправителя = ДанныеЭлементовСхемы.Найти(
			Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1, "ТипЭлементаРегламента");	
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР Тогда
		ИнформацияОтправителя = ДанныеЭлементовСхемы.Найти(
			Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1, "ТипЭлементаРегламента");
	ИначеЕсли ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР Тогда
		ИнформацияОтправителя = ДанныеЭлементовСхемы.Найти(
			Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1, "ТипЭлементаРегламента");
	КонецЕсли;
...
Процедура ПриОпределенииИспользуемыхТиповЭлектронныхДокументов(АктуальныеТипы) Экспорт
	...
	Если ИспользоватьЭПД() Тогда
		...
		ОбменСКонтрагентами.ДобавитьИспользуемыйТипДокумента(
			АктуальныеТипы, Перечисления.ТипыДокументовЭДО.ЭПЭ, Истина, Истина); 
		ОбменСКонтрагентами.ДобавитьИспользуемыйТипДокумента(
			АктуальныеТипы, Перечисления.ТипыДокументовЭДО.ЭЭР, Истина, Истина);
		ОбменСКонтрагентами.ДобавитьИспользуемыйТипДокумента(
			АктуальныеТипы, Перечисления.ТипыДокументовЭДО.ЭСР, Истина, Истина);
	КонецЕсли;
...
Процедура ПредставлениеИсходящихВидовЭлектронныхДокументов(ПредставленияЭПД) Экспорт
	...
	ПредставленияЭПД.Вставить(Перечисления.ТипыДокументовЭДО.ЭПЭ, НСтр("ru = 'Электронные поручения экспедиторам'"));
	ПредставленияЭПД.Вставить(Перечисления.ТипыДокументовЭДО.ЭЭР, НСтр("ru = 'Электронные экспедиторские расписки'"));
	ПредставленияЭПД.Вставить(Перечисления.ТипыДокументовЭДО.ЭСР, НСтр("ru = 'Электронные складские расписки'"));
...
Процедура СпособыОтраженияВУчетеТипаЭлектронногоДокумента(ТипДокумента, СпособыОтраженияВУчете) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектронноеПоручениеЭкспедитору", НСтр("ru = 'Электронное поручение экспедитору'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектроннаяЭкспедиторскаяРасписка", НСтр("ru = 'Электронная экспедиторская расписка'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектроннаяCкладскаяРасписка", НСтр("ru = 'Электронная складская расписка'"), Истина);
	КонецЕсли;
...
Процедура СохранитьДанныеОбъектаВБД(СтрокаДляЗагрузки, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект) Экспорт
	...
	Если ЭтоДокументЭПД(СтрокаДляЗагрузки.ВидЭД) Тогда		
...
Функция ИмяДокументаПоТипу(ТипДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		ИмяДокумента = "ЭлектронноеПоручениеЭкспедитору";
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		ИмяДокумента = "ЭлектроннаяЭкспедиторскаяРасписка";
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		ИмяДокумента = "ЭлектроннаяCкладскаяРасписка";
	КонецЕсли;
...
// Добавляет команды создания документов
// Параметры:
// Команды - См. ПодключаемыеКомандыПереопределяемый.ПриОпределенииКомандПодключенныхКОбъекту.Команды
// ВидКоманды - Строка
// РежимЗаписи - Строка
Процедура ЗаполнитьКомандыСозданияУчетныхДокументов(Команды, ВидКоманды, РежимЗаписи) Экспорт

	НоваяКоманда = Команды.Добавить();
	НоваяКоманда.Вид = ВидКоманды;
	НоваяКоманда.Важность = "СмТакже";
	НоваяКоманда.РежимЗаписи = РежимЗаписи;
	НоваяКоманда.Порядок = 1;
	НоваяКоманда.Идентификатор = "СоздатьЭлектронныйЗаказЗаявка";
	НоваяКоманда.Представление = НСтр("ru='Электронный заказ (заявка)'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектронныйЗаказЗаявкаКоманда";
	...
	НоваяКоманда.Идентификатор = "СоздатьЭлектроннаяТранспортнаяНакладная";
	НоваяКоманда.Представление = НСтр("ru='Электронная транспортная накладная'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектроннаяТранспортнаяНакладнаяКоманда";
	
	НоваяКоманда = Команды.Добавить();
	НоваяКоманда.Вид = ВидКоманды;
	НоваяКоманда.Важность = "СмТакже";
	НоваяКоманда.РежимЗаписи = РежимЗаписи;
	НоваяКоманда.Порядок = 3;
	НоваяКоманда.Идентификатор = "СоздатьЭлектронныйПутевойЛист";
	НоваяКоманда.Представление = НСтр("ru='Электронный путевой лист'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектронныйПутевойЛистКоманда";
				
	НоваяКоманда = Команды.Добавить();
	НоваяКоманда.Вид = ВидКоманды;
	НоваяКоманда.Важность = "СмТакже";
	НоваяКоманда.РежимЗаписи = РежимЗаписи;
	НоваяКоманда.Порядок = 4;
	НоваяКоманда.Идентификатор = "СоздатьЭлектронныйЗаказНаряд";
	НоваяКоманда.Представление = НСтр("ru='Электронный заказ-наряд'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектронныйЗаказНарядКоманда";
	...
	НоваяКоманда.Идентификатор = "СоздатьЭлектроннаяСопроводительнаяВедомость";
	НоваяКоманда.Представление = НСтр("ru='Электронная сопроводительная ведомость'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектроннаяСопроводительнаяВедомостьКоманда";
	
	НоваяКоманда = Команды.Добавить();
	НоваяКоманда.Вид = ВидКоманды;
	НоваяКоманда.Важность = "СмТакже";
	НоваяКоманда.РежимЗаписи = РежимЗаписи;
	НоваяКоманда.Порядок = 6;
	НоваяКоманда.Идентификатор = "СоздатьЭлектронныйДоговорФрахтования";
	НоваяКоманда.Представление = НСтр("ru='Электронный договор фрахтования'");
	НоваяКоманда.Обработчик = "ОбменСГИСЭПДКлиент.СоздатьЭлектронныйДоговорФрахтованияКоманда";	
...
// Возвращает истину если тип входит в опредееляемый тип ДоговорЭПД 
//
// Параметры:
//  ТипСсылки - Произвольный 
// 
// Возвращаемое значение:
//   Булево - Успешность обработки результата
//
Функция ЭтоДоговорЭПД(ТипСсылки) Экспорт
	
	Возврат Метаданные.ОпределяемыеТипы.ДоговорЭПД.Тип.СодержитТип(ТипСсылки);

КонецФункции  

// Возвращает истину если в определяемом типе ДоговорЭПД нет типа СправочникСсылка.ИдентификаторыОбъектовМетаданных 
// 
// Возвращаемое значение:
//  Булево - Успешность обработки результата
//
Функция ОпределенДоговорЭПД() Экспорт 
	
	Возврат Не Метаданные.ОпределяемыеТипы.ДоговорЭПД.Тип.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	
КонецФункции

Функция ПолучитьДанныеОЗаписиВРеестреУведомленийТЭДПоПоследнемуДокументу(Организация, ВидДокумента, Титул = Неопределено) Экспорт 
	...
	Если Титул = Неопределено Тогда	
		Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
			Титул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2");
		ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
			Титул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1");
		ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 2
		|	Т.ЗначениеРеквизитаСтрока КАК ЗначениеРеквизитаСтрока,
		|	Т.ЗначениеРеквизита КАК ЗначениеРеквизита,
		|	Т.ИмяРеквизита КАК ИмяРеквизита
		|ИЗ
		|	РегистрСведений.ЗначенияРеквизитовДокументовЭПД КАК Т
		|ГДЕ
		|	Т.Организация = &Организация
		|	И Т.Титул = &Титул
		|	И (Т.ИмяРеквизита = ""ТитулЭкспедитораНомРеестр""
		|			ИЛИ Т.ИмяРеквизита = ""ТитулЭкспедитораДатаРеестр"")
		|	И Т.Документ В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				Док.Ссылка
		|			ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ИмяРеквизита = "ТитулЭкспедитораНомРеестр" Тогда
			Результат.Номер = Выборка.ЗначениеРеквизитаСтрока; 	
		ИначеЕсли Выборка.ИмяРеквизита = "ТитулЭкспедитораДатаРеестр" Тогда
			Результат.Дата = Выборка.ЗначениеРеквизита;
		КонецЕсли;
...
Процедура ОбновитьДанныеОПодписантах(Объект, НомерВерсии, СтруктураРеквизитов) Экспорт
	
		Запрос = СформироватьЗапросДляПолученияДанныхОПодписантах(Объект, НомерВерсии);
		
		Если Запрос = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ВыборкаРеквизитовПоТитулу = Запрос.Выполнить().Выбрать();
		
		Пока ВыборкаРеквизитовПоТитулу.Следующий() Цикл
			
			ЗначениеРеквизита = Неопределено;
			
			Если ВыборкаРеквизитовПоТитулу.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.НеограниченнаяСтрока Тогда
				ЗначениеРеквизита = ВыборкаРеквизитовПоТитулу.ЗначениеРеквизитаСтрока;
			ИначеЕсли ВыборкаРеквизитовПоТитулу.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.ПростойКромеСтроки Тогда
				ЗначениеРеквизита = ВыборкаРеквизитовПоТитулу.ЗначениеРеквизита;
			ИначеЕсли ВыборкаРеквизитовПоТитулу.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.СсылочныйТип Тогда
				ЗначениеРеквизита = ВыборкаРеквизитовПоТитулу.ЗначениеРеквизитаСсылка;
			КонецЕсли;
			
			Если СтруктураРеквизитов.Свойство(ВыборкаРеквизитовПоТитулу.ИмяРеквизита) Тогда
				СтруктураРеквизитов[ВыборкаРеквизитовПоТитулу.ИмяРеквизита] = ЗначениеРеквизита;
			КонецЕсли
...
Процедура ОбновитьДанныеОПодписантахВСтруктуреФормы(Форма, НомерВерсии) Экспорт
		...
		Пока ВыборкаРеквизитовПоТитулу.Следующий() Цикл
			...
			ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма,
				ВыборкаРеквизитовПоТитулу.ИмяРеквизита, ЗначениеРеквизита
				, , , Истина);
...
Функция ПолучитьДанныеДляТитулаНаОснованииЧерновика(ЭлектронныйДокумент, ЧерновикДокумента) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЭтоЭПДЧерновик", Ложь);
	
	ТипДокументаЗначение = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЧерновикДокумента,
		"ВидЧерновика.ТипДокумента");
		
	ТипДокумента = Неопределено;
	
	Если ЗначениеЗаполнено(ТипДокументаЗначение) 
		И ТипДокументаЗначение.Свойство("ВидЧерновикаТипДокумента") Тогда
		ТипДокумента = ТипДокументаЗначение.ВидЧерновикаТипДокумента;
	КонецЕсли;
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		ИмяСхемы = "ТитулГрузоотправителя";
		МенеджерОбъекта = Документы.ЭлектроннаяТранспортнаяНакладная;
		МетаДанныеПолноеИмя = "Документ.ЭлектроннаяТранспортнаяНакладная";
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ Тогда
		ИмяСхемы = "ТитулГрузоотправителя";
		МенеджерОбъекта = Документы.ЭлектронныйЗаказЗаявка;
		МетаДанныеПолноеИмя = "Документ.ЭлектронныйЗаказЗаявка";
	Иначе
		Возврат Результат;
	КонецЕсли;
	
	Результат.ЭтоЭПДЧерновик = Истина;

	ДанныеПоФайлуЧерновика = ЭлектронныеДокументыЭДОСлужебный.ОсновнойФайлИнформацииОтправителя(ЭлектронныйДокумент);
	ДанныеЧерновика = РаботаСФайлами.ДанныеФайла(ДанныеПоФайлуЧерновика);
	ДвоичныеДанныеЧерновика = ПолучитьИзВременногоХранилища(ДанныеЧерновика.СсылкаНаДвоичныеДанныеФайла);
	
	МакетСхемы = МенеджерОбъекта.ПолучитьМакет("СхемаЧерновика" + ИмяСхемы);
	...
	ПотокДанныхФайла = ДвоичныеДанныеЧерновика.ОткрытьПотокДляЧтения();
	...
	Попытка
		...
		ОбъектXDTO = СтруктураСФабрикой.Фабрика.ПрочитатьXML(ЧтениеXML, ТипXDTOФайла);
	...
	ДокументОбъект = Новый Структура("ДополнительныеСвойства,ТипДокумента,ТекущийПолученныйТитул",
		Новый Структура, ТипДокумента, КлючеваяИнформацияПоТитулу.Титул);
	...
	Если КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		...
		ПолучитьДанныеНаОснованииЧерновикаЭТрН(ДокументОбъект, КлючеваяИнформацияПоТитулу);
		
		ИмяФормы = "Документ.ЭлектроннаяТранспортнаяНакладная.ФормаОбъекта";
		
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ Тогда
		
		ПолучитьДанныеНаОснованииЧерновикаЭЗЗ(ДокументОбъект, КлючеваяИнформацияПоТитулу);

		ИмяФормы = "Документ.ЭлектронныйЗаказЗаявка.ФормаОбъекта";
		
	КонецЕсли;
	
	ЗаполнитьПредставленияСсылочныхТипов(ДокументОбъект);
	ЗаполнитьПредставлениеХранимыхДанныхОбъекта(СтруктураРеквизитов, МетаДанныеПолноеИмя, ОбластьПоискаИмен);
	
	СтруктураРеквизитов.Вставить("Ссылка" + КлючеваяИнформацияПоТитулу.ПрефиксТитула + "Черновик", ЧерновикДокумента);
	
	Результат.Вставить("ДанныеДляЗаполнения", СтруктураРеквизитов);
...
Функция МакетСоответствийИменРеквизитов(ТипДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = ТипДокументаПоручениеЭкспедитору() Тогда
		Макет = Документы.ЭлектронноеПоручениеЭкспедитору.ПолучитьМакет("СоответствиеИменРеквизитов");
	ИначеЕсли ТипДокумента = ТипДокументаЭкспедиторскаяРасписка() Тогда
		Макет = Документы.ЭлектроннаяЭкспедиторскаяРасписка.ПолучитьМакет("СоответствиеИменРеквизитов");
	ИначеЕсли ТипДокумента = ТипДокументаСкладскаяРасписка() Тогда
		Макет = Документы.ЭлектроннаяCкладскаяРасписка.ПолучитьМакет("СоответствиеИменРеквизитов");
	КонецЕсли;
...
Функция ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи, ВидДокумента, ОтборПодписанта = Неопределено) Экспорт
	...
	Если ОтборПодписанта = Неопределено Тогда
		...
		//Реальное заполнение будет после выбора сертификата при отправке
		Результат.Вставить("Фамилия", "---");
		Результат.Вставить("Имя", "---");
		Результат.Вставить("Отчество", "---");
		Результат.Вставить("Должность", "---");
		Результат.Вставить("СоответствуетОтбору", Истина);
...
Функция ПроверитьЗаполнениеДокументаЭПД(ДанныеДляПроверки, ТекущийТитул, ВидДокумента) Экспорт
	...
	Если ТипЗнч(ВидДокумента) = Тип("Строка") Тогда
		...
		ИначеЕсли ВидДокумента = "Документ.ЭлектронноеПоручениеЭкспедитору" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
		ИначеЕсли ВидДокумента = "Документ.ЭлектроннаяЭкспедиторскаяРасписка" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР;
		ИначеЕсли ВидДокумента = "Документ.ЭлектроннаяCкладскаяРасписка" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР;
		КонецЕсли;	
	Иначе
...
Функция ТипыЭлементовРегламентаЭПД() Экспорт
	...
	Результат.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1); 
	Результат.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2);
	Результат.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3);
	Результат.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1);
	Результат.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1);
...
Процедура ЗаполнитьТитулыОтправителя(МассивТитулов) Экспорт
	...
	МассивТитулов.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1);
	МассивТитулов.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1);
	МассивТитулов.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1);
...
// Создает таблицу значений для хранения информационных полей обмена с ГИС ЭПД.
//
// Возвращаемое значение:
//   ТаблицаЗначений:
//     * ИдентификаторИнфоПолей - Строка - идентификатор строки родителя информационных полей
//     * ИдентификаторДопИнформации - Строка - идентификатор группы информационных полей
//     * Идентификатор - Строка - идентификатор информационного поля
//     * Значение - Строка - значение информационного поля
//     * ТипДанных - Строка - тип данных информационного поля
Функция НоваяТаблицаИнформационныеПоля() Экспорт
	
	ТипСтрока = Новый ОписаниеТипов("Строка");
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ИдентификаторИнфоПолей", ТипСтрока);
	Результат.Колонки.Добавить("ИдентификаторДопИнформации", ТипСтрока);
	Результат.Колонки.Добавить("Идентификатор", ТипСтрока);
	Результат.Колонки.Добавить("Значение", ТипСтрока);
	Результат.Колонки.Добавить("ТипДанных", ТипСтрока);
...
Функция ПолучитьРеквизитыУчастника(Участник, ДобавитьПлатежныеРеквизиты = Ложь, Параметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Участник) Тогда
		...
		Если ЮрФизЛицо = Неопределено Тогда
			...
			Если ЮрФизЛицо = Неопределено Тогда
				СведенияОбУчастнике.Свойство("ВидКонтрагента", ЮрФизЛицо);	
			КонецЕсли;	
		КонецЕсли;
		...
		Иначе
			Если Строка(ЮрФизЛицо) = "Физическое лицо"
				...
				Если СведенияОбУчастнике.Свойство("ОГРНИП") Тогда
					Результат.Вставить("ОГРНИП", СведенияОбУчастнике.ОГРНИП); 
				ИначеЕсли СведенияОбУчастнике.Свойство("ОГРН") Тогда
		...
		КонецЕсли;
		АдресУчастника = РаботаСАдресами.СведенияОбАдресе(АдресВоВнутреннемФормате, ДополнительныеПараметры);
		
		ЭтоАдресЭПДВСвободнойФорме = ЗначениеЗаполнено(АдресУчастника.Территория)
			Или (Не РаботаСАдресамиКлиентСервер.ЭтоАдминистративноТерриториальныйАдрес(АдресУчастника.ТипАдреса)
				 И Не РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(АдресУчастника.ТипАдреса))
			Или (АдресУчастника.Здание.ТипЗдания <> "Дом")
			Или (АдресУчастника.Корпуса.Количество() > 0
				 И АдресУчастника.Корпуса[0].ТипКорпуса <> "Корпус")
			Или (АдресУчастника.Помещения.Количество() > 0
				 И АдресУчастника.Помещения[0].ТипПомещения <> "Квартира");
				 
		Если Параметры <> Неопределено И Параметры.Свойство("АдресВСвободнойФорме")
			И Параметры.АдресВСвободнойФорме Тогда
			ЭтоАдресЭПДВСвободнойФорме = Истина;
		КонецЕсли;
		 
		Если ЭтоАдресЭПДВСвободнойФорме Тогда 
			
			Результат.Вставить("АдресКодСтраны", АдресУчастника.КодСтраны);
			Результат.Вставить("АдресПредставление", АдресУчастника.Представление);
			
			Если Параметры <> Неопределено И Параметры.Свойство("АдресВСвободнойФорме")
				И Параметры.АдресВСвободнойФорме Тогда
				Результат.Вставить("АдресСтрана", АдресУчастника.Страна);
			КонецЕсли;
			
		Иначе
			Результат.Вставить("АдресИндекс", АдресУчастника.Индекс);
			Результат.Вставить("АдресКодРегиона", АдресУчастника.КодРегиона); 
			Если АдресУчастника.КодРегиона = "77"
				Или АдресУчастника.КодРегиона = "78"
				Или АдресУчастника.КодРегиона = "92" Тогда
				Результат.Вставить("АдресГород", АдресУчастника.Регион
					+ ?(АдресУчастника.РегионТипКраткий = "", "", " ")
					+ АдресУчастника.РегионТипКраткий);	
			Иначе
				Результат.Вставить("АдресГород", АдресУчастника.Город
					+ ?(АдресУчастника.ГородТипКраткий = "", "", " ") + АдресУчастника.ГородТипКраткий);
			КонецЕсли;	
			Результат.Вставить("АдресРайон", АдресУчастника.Район
				+ ?(АдресУчастника.РайонТипКраткий = "", "", " ") + АдресУчастника.РайонТипКраткий);
			Результат.Вставить("АдресНаселенныйПункт", АдресУчастника.НаселенныйПункт
				+ ?(АдресУчастника.НаселенныйПунктТипКраткий = "", "", " ")
				+ АдресУчастника.НаселенныйПунктТипКраткий);
			Результат.Вставить("АдресУлица", АдресУчастника.Улица
				+ ?(АдресУчастника.УлицаТипКраткий = "", "", " ") + АдресУчастника.УлицаТипКраткий);
			Результат.Вставить("АдресДом", АдресУчастника.Здание.Номер);
			Если АдресУчастника.Корпуса.Количество() > 0 Тогда
				Результат.Вставить("АдресКорпус", АдресУчастника.Корпуса[0].Номер);
			КонецЕсли;
			Если АдресУчастника.Помещения.Количество() > 0 Тогда
				Результат.Вставить("АдресКвартира", АдресУчастника.Помещения[0].Номер);
			КонецЕсли;
		КонецЕсли;
...
// 
// Возвращаемое значение:
//  Структура - Получить реквизиты договора:
// * Наименование 
// * Номер 
// * Дата 
Функция ПолучитьРеквизитыДоговора(СсылкаНаДоговор) Экспорт
	
	Результат = Новый Структура("Наименование,Номер,Дата");
	
	РеквизитыДоговора = Неопределено;
	ОбменСГИСЭПДПереопределяемый.ПолучитьРеквизитыДоговора(СсылкаНаДоговор, РеквизитыДоговора);
	
	Если РеквизитыДоговора <> Неопределено Тогда
		
		ЗаполнитьЗначенияСвойств(Результат, РеквизитыДоговора);
			...
			Для Каждого НайденныйРеквизит Из НайденныеРеквизиты Цикл
				...
				Если ТипДанныхХранитсяВТаблице Тогда
					...
					ИначеЕсли ТипСтрокой = "Строка" Тогда
						ОграничениеТипа = ОписаниеТипаДопРеквизита(ТипСтрокой, КвалификаторыСтрокой);
						РазрешитьМногострочныйРежим = Истина;
						Если Форма[ИмяТаблицы][ИтераторЦикла].ИдентификаторИнформации = "ЕГАИС" Тогда
							РазрешитьМногострочныйРежим = Ложь;
						КонецЕсли;
...
Процедура ЗаполнитьТаблицуЗначенийРеквизитовПоПараметрамФормы(Параметры, СтруктураРеквизитов) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		ИначеЕсли ТипЗнч(Параметры.ЗначениеКопирования) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
			МассивИсключений.Добавить("ТитулЗаказчикаНомер");
			МассивИсключений.Добавить("ТитулЗаказчикаДата");
			МассивИсключений.Добавить("ТитулЗаказчикаИдентификаторФайла");
			МассивИсключений.Добавить("ТитулЗаказчикаДатаФормированияФайла");
			МассивИсключений.Добавить("ТитулЗаказчикаВремяФормированияФайла");
			МассивИсключений.Добавить("ТитулЗаказчикаУИДМинтранс");
			
			ПервыйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1;
			
		ИначеЕсли ТипЗнч(Параметры.ЗначениеКопирования) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
			МассивИсключений.Добавить("ТитулЭкспедитораНомер");
			МассивИсключений.Добавить("ТитулЭкспедитораДата");
			МассивИсключений.Добавить("ТитулЭкспедитораИдентификаторФайла");
			МассивИсключений.Добавить("ТитулЭкспедитораДатаФормированияФайла");
			МассивИсключений.Добавить("ТитулЭкспедитораВремяФормированияФайла");
			МассивИсключений.Добавить("ТитулЭкспедитораУИДМинтранс");
			
			ПервыйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1;
			
		ИначеЕсли ТипЗнч(Параметры.ЗначениеКопирования) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда
			МассивИсключений.Добавить("ТитулЭкспедитораНомер");
			МассивИсключений.Добавить("ТитулЭкспедитораДата");
			МассивИсключений.Добавить("ТитулЭкспедитораИдентификаторФайла");
			МассивИсключений.Добавить("ТитулЭкспедитораДатаФормированияФайла");
			...
			ПервыйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1;
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения) Тогда
		...
		Если Параметры.ЗначенияЗаполнения.Свойство("Основание") 
			И ТипЗнч(Параметры.ЗначенияЗаполнения.Основание) = Тип("ДокументСсылка.ЭлектронныйЗаказЗаявка")
			И ТипЗнч(Параметры.Ключ) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда 
		
			Если Параметры.ЗначенияЗаполнения.Основание.ЭтоВходящий = Истина Тогда
				СтруктураРеквизитов = Новый ФиксированнаяСтруктура(Новый Структура);
				Возврат;
			КонецЕсли;
			
			ЗаполнитьЭТрННаОснованииЭЗЗ(Параметры, Параметры.ЗначенияЗаполнения.Основание, СтруктураРеквизитов);
		...
		Иначе
			...
			Если ТипЗнч(Параметры.ЗначенияЗаполнения) = Тип("Структура")
				И Параметры.ЗначенияЗаполнения.Свойство("ТаблицаИнформационныеПоля") Тогда
				ЗаполнитьТаблицуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
					Параметры.ЗначенияЗаполнения.ТаблицаИнформационныеПоля);
			КонецЕсли;
			...
			ИначеЕсли ТипЗнч(Параметры.ЗначениеКопирования) = Тип("ДокументСсылка.ЭлектронныйДоговорФрахтования") Тогда
				ПервыйТитулПрефикс = "ТитулФрахтователя"; 
			ИначеЕсли ТипЗнч(Параметры.ЗначениеКопирования) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
				ПервыйТитулПрефикс = "ТитулЗаказчика";
			КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Основание) Тогда
		...
		Если ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.ЭлектронныйЗаказЗаявка")
			...
			ЗаполнитьЭТрННаОснованииЭЗЗ(Параметры, Параметры.Основание, СтруктураРеквизитов);
...
Процедура ЗаполнитьТаблицуЗначенийРеквизитов(Документ, Организация, СтруктураРеквизитов, Титул = Неопределено, МассивИсключений = Неопределено, ТолькоНовые = Ложь) Экспорт
	...
	Пока ВыборкаТитул.Следующий() Цикл
		...
		Если МассивВерсийЗаполнение = Неопределено Тогда
			МассивВерсийЗаполнение = Новый Массив; 
		ИначеЕсли ТипЗнч(МассивВерсийЗаполнение) = Тип("ФиксированныйМассив") Тогда
			МассивВерсийЗаполнение = ОбменСГИСЭПДКлиентСервер.СкопироватьИзФиксированногоЗначенияРекурсивно(МассивВерсийЗаполнение);
		КонецЕсли;     
...
Функция ТитулДолженБытьВКодировкеUtf8(ИмяФайла)
	
	Результат = Ложь;
	
	ПрефиксыИмениФайла = Новый Массив;
	ПрефиксыИмениФайла.Добавить("ON_POREXPKLT");
	ПрефиксыИмениФайла.Добавить("ON_POREXPEXP");
	ПрефиксыИмениФайла.Добавить("ON_POREXPOTZ");
	ПрефиксыИмениФайла.Добавить("ON_EXPRASP");
	ПрефиксыИмениФайла.Добавить("ON_SKLADRASP");
	
	ИмяФайлаВВерхнемРегистре = ВРЕГ(ИмяФайла);
	
	Для Каждого ПрефиксФайла Из ПрефиксыИмениФайла Цикл
		
		Если СтрНачинаетсяС(ИмяФайлаВВерхнемРегистре, ПрефиксФайла) Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
...
Функция СформироватьЗапросДляПолученияДанныхОПодписантах(Объект, НомерВерсии)
	...
	|	ЗначенияРеквизитовДокументовЭПД.НомерСтрокиРеквизита КАК НомерСтрокиРеквизита,
	|	ЗначенияРеквизитовДокументовЭПД.ИмяРеквизита КАК ИмяРеквизита,
	|	ЗначенияРеквизитовДокументовЭПД.ЗначениеРеквизита КАК ЗначениеРеквизита,
	|	ЗначенияРеквизитовДокументовЭПД.ЗначениеРеквизитаСсылка КАК ЗначениеРеквизитаСсылка,
	|	ЗначенияРеквизитовДокументовЭПД.ЗначениеРеквизитаСтрока КАК ЗначениеРеквизитаСтрока,
	|	ЗначенияРеквизитовДокументовЭПД.ТипРеквизита КАК ТипРеквизита
	|ИЗ
	|	РегистрСведений.ЗначенияРеквизитовДокументовЭПД КАК ЗначенияРеквизитовДокументовЭПД
	|ГДЕ
	|	ЗначенияРеквизитовДокументовЭПД.Документ = &Документ
	|	И ЗначенияРеквизитовДокументовЭПД.Организация = &Организация
	|	И ЗначенияРеквизитовДокументовЭПД.НомерВерсии = &НомерВерсии
	|	И ЗначенияРеквизитовДокументовЭПД.Титул = &Титул
	|	И ЗначенияРеквизитовДокументовЭПД.ИмяРеквизита ПОДОБНО &ИмяРеквизита";
	
	Запрос.УстановитьПараметр("Документ", Объект.Ссылка);
	Запрос.УстановитьПараметр("Организация", Объект.Организация);
	Запрос.УстановитьПараметр("НомерВерсии", НомерВерсии);
	Запрос.УстановитьПараметр("Титул", Объект.ТекущийТитул);
	Запрос.УстановитьПараметр("ИмяРеквизита", ИнформацияПоПрефиксамТитула.ВПрограмме + "Подписант%");
	
	Возврат Запрос;
	
КонецФункции

Функция НайтиСсылкуНаКонтрагента(СтруктураРеквизитов, ПрефиксРеквизита, ТипОбъекта = "Контрагенты")
	
	Инн = "";
	Кпп = "";
	
	СтруктураРеквизитов.Свойство(ПрефиксРеквизита + "ЮЛ_ИНН", Инн);
	СтруктураРеквизитов.Свойство(ПрефиксРеквизита + "ЮЛ_КПП", Кпп);
	...
	Если ЗначениеЗаполнено(Инн) Тогда
		Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП(ТипОбъекта, Инн, Кпп);
	КонецЕсли;
...
Процедура ЗадатьСсылкуНаКонтрагента(ДокументОбъект, ПрефиксРеквизита, ТипОбъекта = "Контрагенты")
	...
	ДокументОбъект.Вставить("Ссылка" + ПрефиксРеквизита, 
		НайтиСсылкуНаКонтрагента(СтруктураРеквизитов, ПрефиксРеквизита, ТипОбъекта));
		
	СтруктураРеквизитов.Вставить("Ссылка" + ПрефиксРеквизита,
		ДокументОбъект["Ссылка" + ПрефиксРеквизита]);
	
КонецПроцедуры

Процедура ПолучитьДанныеНаОснованииЧерновикаЭТрН(ДокументОбъект, КлючеваяИнформацияПоТитулу)
	...
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяУИДМинтранс");
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяВерсияПрограммы");
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяВерсияФормата");
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяИдентификаторФайла");
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяДатаФормированияФайла");
	РеквизитыДляУдаления.Добавить("ТитулГрузоотправителяВремяФормированияФайла");
	
	Для Каждого РеквизитДляУдаления Из РеквизитыДляУдаления Цикл
		СтруктураРеквизитов.Удалить(РеквизитДляУдаления);
	КонецЦикла;
	
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяГрузоотправитель", "Организации");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяПеревозчик");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяГрузополучатель");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяЗаказчик");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяВладелецПунктаПогрузки");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяРеквизитыГрузоотправителя");
	ЗадатьСсылкуНаКонтрагента(ДокументОбъект, "ТитулГрузоотправителяЛицоОсуществляющееПогрузку");

КонецПроцедуры

Процедура ПолучитьДанныеНаОснованииЧерновикаЭЗЗ(ДокументОбъект, КлючеваяИнформацияПоТитулу)
...
Процедура ЗаполнитьЭТрННаОснованииЭЗЗ(Параметры, Основание, СтруктураРеквизитов) 
	
	СтруктураРеквизитовОснования = Неопределено;
	ЗаполнитьТаблицуЗначенийРеквизитов(Основание, 
...
Функция ПолучитьСоответствиеПрефиксов()
	
	СоответствиеПрефиксов = Новый Соответствие;
	СоответствиеПрефиксов.Вставить("ON_TRNACLGROT",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулГрузоотправителя"));
	СоответствиеПрефиксов.Вставить("ON_TRNACLPPRIN", Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулПеревозчикаПриемка"));
	СоответствиеПрефиксов.Вставить("ON_TRNPEREADR",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулПереадресовка"));
	СоответствиеПрефиксов.Вставить("ON_TRNZAMEN",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулПеревозчикаЗамены"));
	СоответствиеПрефиксов.Вставить("ON_TRNACLGRPO",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулГрузополучателя"));
	СоответствиеПрефиксов.Вставить("ON_TRNACLPVYN",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулПеревозчикаВыдача"));
	СоответствиеПрефиксов.Вставить("ON_TRNPUDPER",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулПеревозчикаФХЖ"));
	СоответствиеПрефиксов.Вставить("ON_TRNPUDGO",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяТранспортнаяНакладная, "ТитулГрузоотправителяФХЖ"));
	СоответствиеПрефиксов.Вставить("ON_PTLSSOBTS",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулОформление"));
	СоответствиеПрефиксов.Вставить("ON_PTLSPRMO",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулМедосмотр"));
	СоответствиеПрефиксов.Вставить("ON_PTLSVIPTS",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулВыпуск"));
	СоответствиеПрефиксов.Вставить("ON_PTLSODVZD",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулВыезд"));
	СоответствиеПрефиксов.Вставить("ON_PTLSODPARK",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулЗаезд"));
	СоответствиеПрефиксов.Вставить("ON_PTLSPOSMO",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйПутевойЛист, "ТитулМедосмотрПосле"));
	СоответствиеПрефиксов.Вставить("ON_ZAKZVGO",     Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказЗаявка, "ТитулГрузоотправителя"));
	СоответствиеПрефиксов.Вставить("ON_ZAKZVPER",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказЗаявка, "ТитулПеревозчика"));
	СоответствиеПрефиксов.Вставить("ON_DOGFRAKHTEL", Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйДоговорФрахтования, "ТитулФрахтователя"));
	СоответствиеПрефиксов.Вставить("ON_DOGFRASHCH",  Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйДоговорФрахтования, "ТитулФрахтовщика"));
	СоответствиеПрефиксов.Вставить("ON_ZAKAZNARSOG", Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказНаряд, "ТитулФрахтовщика"));
	СоответствиеПрефиксов.Вставить("ON_ZAKAZNARPOD", Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказНаряд, "ТитулФрахтователяПодача"));
	СоответствиеПрефиксов.Вставить("ON_ZAKAZNARVOZ", Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказНаряд, "ТитулФрахтовщикаВозврат"));
	СоответствиеПрефиксов.Вставить("ON_ZAKAZNAR",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронныйЗаказНаряд, "ТитулФрахтователя"));
	СоответствиеПрефиксов.Вставить("ON_SOPVEDPER",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяСопроводительнаяВедомость, "ТитулПеревозчика"));
	СоответствиеПрефиксов.Вставить("ON_SOPVEDGO",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяСопроводительнаяВедомость, "ТитулГрузоотправителя"));
	СоответствиеПрефиксов.Вставить("ON_SOPVEDGP",    Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяСопроводительнаяВедомость, "ТитулГрузополучателя"));
	СоответствиеПрефиксов.Вставить("ON_POREXPKLT",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронноеПоручениеЭкспедитору, "ТитулЗаказчика"));
	СоответствиеПрефиксов.Вставить("ON_POREXPEXP",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронноеПоручениеЭкспедитору, "ТитулЭкспедитора"));
	СоответствиеПрефиксов.Вставить("ON_POREXPOTZ",   Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектронноеПоручениеЭкспедитору, "ТитулЗаказчикаОтзыв"));
	СоответствиеПрефиксов.Вставить("ON_EXPRASP",     Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяЭкспедиторскаяРасписка, "ТитулЭкспедитора"));
	СоответствиеПрефиксов.Вставить("ON_SKLADRASP",     Новый Структура("Менеджер, ИмяСхемы", Документы.ЭлектроннаяCкладскаяРасписка, "ТитулЭкспедитора"));
	
	Возврат СоответствиеПрефиксов;

КонецФункции

Функция ПолучитьСоответствиеПрефиксаПоИдФайла(ИдФайл)
	
	СоответствиеПрефиксов = ПолучитьСоответствиеПрефиксов();
	
	Для Каждого Элемент Из СоответствиеПрефиксов Цикл
		Если СтрНачинаетсяС(ИдФайл, Элемент.Ключ) Тогда
...
Функция ПолучитьДанныеПоСертификатуИДоверенностиДляЗаполнения(ТипРегламента, ДанныеПодписанта)
	
	Результат = Новый Структура;
	
	Результат.Вставить("Организация", ДанныеПодписанта.Организация);
	Результат.Вставить("Фамилия");
	Результат.Вставить("Имя");
	Результат.Вставить("Отчество");
	Результат.Вставить("Должность");
	...
	ПараметрыСертификатов = КриптографияБЭД.СвойстваСертификатов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеПодписанта.СертификатПодписи));

	ПараметрыСертификата = ПараметрыСертификатов.Получить(ДанныеПодписанта.СертификатПодписи);
	
	Результат.Фамилия = ПараметрыСертификата.Фамилия;
	Результат.Имя = ПараметрыСертификата.Имя;
	Результат.Отчество = ПараметрыСертификата.Отчество;
		
	Если ЗначениеЗаполнено(ПараметрыСертификата.Должность) Тогда
		Результат.Должность = ПараметрыСертификата.Должность;
	КонецЕсли;
	
	Если (ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЗЗ 
		  Или ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭДФ)
		  И Не ЗначениеЗаполнено(Результат.Должность) Тогда
		Результат.Должность = "-";
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ДанныеПодписанта.Доверенность) Тогда
	
		ЗапросПоДоверенности = Новый Запрос;
		ЗапросПоДоверенности.УстановитьПараметр("Доверенность", ДанныеПодписанта.Доверенность); 
		
		Если ТипЗнч(ДанныеПодписанта.Доверенность) = Тип("СправочникСсылка.МЧД003") Тогда
			
			ЗапросПоДоверенности.Текст = "ВЫБРАТЬ
			|	Доверенности.Ссылка КАК Ссылка,
			|	Доверенности.НомерДоверенности КАК НомерДоверенности,
			|	Доверенности.ДатаВыдачи КАК ДатаДоверенности,
			|	Доверенности.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
			...
			|	Справочник.МЧД003 КАК Доверенности
			|ГДЕ
			| Доверенности.Ссылка = &Доверенность";
		...
		Выборка = ЗапросПоДоверенности.Выполнить().Выбрать();
...
Процедура ЗаполнитьСвойствоXDTO(ОбъектXDTO, ИмяСвойства, Значение, Обязательное = Ложь, УстанавливатьПустыеЗначения = Ложь) Экспорт
...
Процедура ЗаполнитьДанныеПодписантаСОпределением(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта)
	
	ДанныеПоСертификатуИДоверенности = ПолучитьДанныеПоСертификатуИДоверенностиДляЗаполнения(ОписаниеФайла.ТипРегламента, ДанныеПодписанта);
	
	ПространствоИменСхемы = ДанныеФайлаЭД.Тип().URIПространстваИмен;
	ДанныеФайлаЭДФабрикаXDTO = ДанныеФайлаЭД.Фабрика();
	
	СвойстваВДокументе = ДанныеФайлаЭД.Документ.Тип().Свойства;

	ПодписантXDTO = Неопределено;
	
	Для Каждого СвойствоВДокументе Из СвойстваВДокументе Цикл
		
		Если СтрНачинаетсяС(СвойствоВДокументе.Имя, "Подп") Тогда
			ПодписантXDTO = СвойствоВДокументе;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ПодписантXDTO = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеДляЗаписиВРеквизитыДокумента = Новый Структура;
	
	ДанныеФайлаЭД.Документ.Сбросить(ПодписантXDTO.Имя);
	
	Подписант = ДанныеФайлаЭДФабрикаXDTO.Создать(ПодписантXDTO.Тип);
		
	ФИО = ДанныеФайлаЭДФабрикаXDTO.Создать(ДанныеФайлаЭДФабрикаXDTO.Тип(ПространствоИменСхемы, "ФИОТип")); 
	
	ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", ДанныеПоСертификатуИДоверенности.Фамилия, Истина);
	ЗаполнитьСвойствоXDTO(ФИО, "Имя", ДанныеПоСертификатуИДоверенности.Имя, Истина);
	ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ДанныеПоСертификатуИДоверенности.Отчество);
	
	ЗаполнитьСвойствоXDTO(Подписант, "ФИО", ФИО, Истина);
	ЗаполнитьСвойствоXDTO(Подписант, "Должн", ДанныеПоСертификатуИДоверенности.Должность); 

	ЗаполнитьСвойствоXDTO(Подписант, "ТипПодпис", "1", Истина); 
	
	ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантФамилия", ДанныеПоСертификатуИДоверенности.Фамилия);
	ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантИмя", ДанныеПоСертификатуИДоверенности.Имя);
	ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантОтчество", ДанныеПоСертификатуИДоверенности.Отчество);
	ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДолжность",  ДанныеПоСертификатуИДоверенности.Должность);
	
	Если ЗначениеЗаполнено(ДанныеПоСертификатуИДоверенности.НомерДоверенности) Тогда
		
		Если ЗначениеЗаполнено(ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме) Тогда 
			
			ЗаполнитьСвойствоXDTO(Подписант, "СпосПодтПолном", "3", Истина);
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантСпособПодтвержденияПолномочий", "3");
			
			СвДоверЭл = ДанныеФайлаЭДФабрикаXDTO.Создать(Подписант.Тип().Свойства.Получить("СвДоверЭл").Тип);

			ЗаполнитьСвойствоXDTO(СвДоверЭл, "НомДовер", ДанныеПоСертификатуИДоверенности.НомерДоверенности, Истина); 
			ЗаполнитьСвойствоXDTO(СвДоверЭл, "ДатаДовер",
				Формат(ДанныеПоСертификатуИДоверенности.ДатаДоверенности, "ДФ=dd.MM.yyyy"), Истина);
			ЗаполнитьСвойствоXDTO(СвДоверЭл, "ИдСистХран", ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме, Истина);
			ЗаполнитьСвойствоXDTO(СвДоверЭл, "ВнНомДовер", ДанныеПоСертификатуИДоверенности.ПорядковыйНомерДоверенности); 
			
			Подписант.СвДоверЭл.Добавить(СвДоверЭл);
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектронная__1__Номер",
				ДанныеПоСертификатуИДоверенности.НомерДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектронная__1__Дата",
				ДанныеПоСертификатуИДоверенности.ДатаДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектронная__1__ИнформационнаяСистема",
				ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме);
			
		Иначе 
			
			ЗаполнитьСвойствоXDTO(Подписант, "СпосПодтПолном", "6", Истина);
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантСпособПодтвержденияПолномочий", "6");

		КонецЕсли;
		
	Иначе
		
		ЗаполнитьСвойствоXDTO(Подписант, "СпосПодтПолном", "1", Истина);
		
		ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантСпособПодтвержденияПолномочий", "1");
		
	КонецЕсли;
		
	ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ, ПодписантXDTO.Имя, Подписант, Истина);
	
	ЗаписатьДанныеОПодписантеВРеквизитыДокумента(ОписаниеФайла.ДокументЭПД, ДанныеФайлаЭД,
		 ДанныеПодписанта.Организация, ДанныеДляЗаписиВРеквизитыДокумента);
	
КонецПроцедуры 

Процедура ЗаполнитьДанныеПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта)
	...
	ИдФайл = ВРЕГ(ДанныеФайлаЭД.ИдФайл);
	
	ПодписантМножественный = Ложь;
	
	Если СтрНачинаетсяС(ИдФайл, "ON_TRNPEREADR")
		Или СтрНачинаетсяС(ИдФайл, "ON_TRNPUDGO")
		Или СтрНачинаетсяС(ИдФайл, "ON_TRNPUDPER") Тогда 
		
		ПодписантМножественный = Истина;
		
	КонецЕсли;
	
	Если ПодписантМножественный Тогда
		
		ПодписантТипXDTO = ДанныеФайлаЭД.Документ.Подписант[0].Тип();
		ДанныеФайлаЭД.Документ.Подписант.Очистить();
		
	Иначе
		
		ПодписантТипXDTO = ДанныеФайлаЭД.Документ.Подписант.Тип();
		ДанныеФайлаЭД.Документ.Сбросить("Подписант");
		
	КонецЕсли;
	
	ДанныеДляЗаписиВРеквизитыДокумента = Новый Структура;
	
	Подписант = ДанныеФайлаЭДФабрикаXDTO.Создать(ПодписантТипXDTO);
	ФИО = ДанныеФайлаЭДФабрикаXDTO.Создать(ДанныеФайлаЭДФабрикаXDTO.Тип(ПространствоИменСхемы, "ФИОТип")); 
	...
	Если ПодписантМножественный Тогда
		...
		ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__Фамилия", ДанныеПоСертификатуИДоверенности.Фамилия);
		ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__Имя", ДанныеПоСертификатуИДоверенности.Имя);
		ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__Отчество", ДанныеПоСертификатуИДоверенности.Отчество);
		ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__Должность", ДанныеПоСертификатуИДоверенности.Должность);
	...
	Если ЗначениеЗаполнено(ДанныеПоСертификатуИДоверенности.НомерДоверенности) Тогда
		...
		ЗаполнитьСвойствоXDTO(Подписант, "СтатПодп", "2", Истина);
		
		ЗаполнитьСвойствоXDTO(Подписант, "ИдСистХран", ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме);
		
		СвДовер = ДанныеФайлаЭДФабрикаXDTO.Создать(Подписант.Тип().Свойства.Получить("СвДовер").Тип);
			 
		ЗаполнитьСвойствоXDTO(СвДовер, "ДатаДовер",
			Формат(ДанныеПоСертификатуИДоверенности.ДатаДоверенности, "ДФ=dd.MM.yyyy"), Истина);
		ЗаполнитьСвойствоXDTO(СвДовер, "ИдентДовер", ДанныеПоСертификатуИДоверенности.НомерДоверенности, Истина);
		ЗаполнитьСвойствоXDTO(СвДовер, "НомерДовер", ДанныеПоСертификатуИДоверенности.НомерДоверенности, Истина);
		
		ЗаполнитьСвойствоXDTO(Подписант, "СвДовер", СвДовер);
		
		Если ПодписантМножественный Тогда

			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__КодСтатусаПодписанта", "2");
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__ДоверенностьИдентификаторФайла",
				ДанныеПоСертификатуИДоверенности.НомерДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__ДоверенностьДата",
				ДанныеПоСертификатуИДоверенности.ДатаДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__СистемаХранящаяДоверенность",
				ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме);
			
		Иначе
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантКодСтатуса", "2");
			
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектроннаяНомер",
				ДанныеПоСертификатуИДоверенности.НомерДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектроннаяДата",
				ДанныеПоСертификатуИДоверенности.ДатаДоверенности);
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантДоверенностьЭлектроннаяИнформационнаяСистема",
				ДанныеПоСертификатуИДоверенности.СведенияОбИнформационнойСистеме);
	...
	Иначе
		...
		ЗаполнитьСвойствоXDTO(Подписант, "СтатПодп", "1", Истина);
		
		Если ПодписантМножественный Тогда
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("Подписанты__1__КодСтатусаПодписанта", "1");
		Иначе
			ДанныеДляЗаписиВРеквизитыДокумента.Вставить("ПодписантКодСтатуса", "1");
		КонецЕсли;
	...
	Иначе
		...
		ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ, "Подписант", Подписант, Истина); 
...
Процедура ЗаписатьДанныеОПодписантеВРеквизитыДокумента(ДокументЭПД, ДанныеФайлаЭД, Организация, ДанныеДляЗаписиВРеквизитыДокумента)
	
	КлючеваяИнформацияПоТитулу = КлючеваяИнформацияПоТитулу(ДанныеФайлаЭД); 
	
	ИдФайл = ДанныеФайлаЭД.ИдФайл;
	...
	ЗапросВерсии.Параметры.Вставить("Документ", ДокументЭПД);
	ЗапросВерсии.Параметры.Вставить("Организация", Организация);
	ЗапросВерсии.Параметры.Вставить("Титул", КлючеваяИнформацияПоТитулу.Титул);
	ЗапросВерсии.Параметры.Вставить("ИдентификаторФайла", ИдФайл);
	...
	Для Каждого КиЗ Из ДанныеДляЗаписиВРеквизитыДокумента Цикл
		...
		ИмяРеквизита = КлючеваяИнформацияПоТитулу.ПрефиксТитула + ИмяРеквизита;
		
		НаборЗаписейЗначений = РегистрыСведений.ЗначенияРеквизитовДокументовЭПД.СоздатьНаборЗаписей();
		
		НаборЗаписейЗначений.Отбор.Документ.Установить(ДокументЭПД);
		НаборЗаписейЗначений.Отбор.Организация.Установить(Организация);
		НаборЗаписейЗначений.Отбор.Титул.Установить(КлючеваяИнформацияПоТитулу.Титул);
		НаборЗаписейЗначений.Отбор.НомерВерсии.Установить(НомерВерсии); 
		
		НаборЗаписейЗначений.Отбор.ИмяРеквизита.Установить(ИмяРеквизита);
		НаборЗаписейЗначений.Отбор.ИмяТабличнойЧасти.Установить(ИмяТабличнойЧасти);
		НаборЗаписейЗначений.Отбор.НомерСтрокиРеквизита.Установить(НомерСтрокиРеквизита);
		
		НаборЗаписейЗначений.Прочитать();
		
		Если НаборЗаписейЗначений.Количество() > 0 Тогда
		
			Для Каждого НаборЗапись Из НаборЗаписейЗначений Цикл 
				
				ЗаполнитьРеквизит(НаборЗапись, КиЗ);
		...
		Иначе
			...
			НоваяЗапись.Документ = ДокументЭПД;
			НоваяЗапись.Организация = Организация;
			НоваяЗапись.Титул = КлючеваяИнформацияПоТитулу.Титул;
			...
			ЗаполнитьРеквизит(НоваяЗапись, КиЗ);
			
		КонецЕсли;
		
		НаборЗаписейЗначений.Записать(Истина);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьРеквизит(НаборЗапись, ДанныеРеквизита)
	
	Если ДанныеРеквизита.Значение = Неопределено Или ТипЗнч(ДанныеРеквизита.Значение) = Тип("Строка") Тогда 
			НаборЗапись.ЗначениеРеквизитаСтрока = ДанныеРеквизита.Значение;
			НаборЗапись.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.НеограниченнаяСтрока;
	ИначеЕсли ТипЗнч(ДанныеРеквизита.Значение) = Тип("Число")
		Или ТипЗнч(ДанныеРеквизита.Значение) = Тип("Дата")
		Или ТипЗнч(ДанныеРеквизита.Значение) = Тип("Булево") Тогда
			НаборЗапись.ЗначениеРеквизита = ДанныеРеквизита.Значение;
			НаборЗапись.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.ПростойКромеСтроки;
	Иначе
		НаборЗапись.ЗначениеРеквизитаСсылка = ДанныеРеквизита.Значение;
		НаборЗапись.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.СсылочныйТип;
	КонецЕсли;
...
Процедура ОбработатьПараметры(Форма)
	...
	Если ЗначениеЗаполнено(ЗначениеЗаполнения)
		...
		Если РеквизитыХранимыхДанных.ГруппаДанных = "Груз"
			И ОписаниеХранимыхДанныхЭПД.ДанныеЗаполненияРеквизитовФормы.Свойство("КодТоварнойНоменклатуры") Тогда
			
			СсылкаТоварнойНоменклатуры = НайтиСсылкуВОпределяемомТипеПоКоду("КлассификаторТНВЭДЭПД",
				ОписаниеХранимыхДанныхЭПД.ДанныеЗаполненияРеквизитовФормы.КодТоварнойНоменклатуры);
			Если ЗначениеЗаполнено(СсылкаТоварнойНоменклатуры) Тогда
				Форма.КодТоварнойНоменклатуры = СсылкаТоварнойНоменклатуры;
			КонецЕсли;
...
// Найти ссылку в определяемом типе по коду.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Неопределено - Найти ссылку в определяемом типе по коду
Функция НайтиСсылкуВОпределяемомТипеПоКоду(ИмяОпределяемогоТипа, Код)
    МассивТипов = Метаданные.ОпределяемыеТипы[ИмяОпределяемогоТипа].Тип.Типы();
    Для Каждого Тип Из МассивТипов Цикл
        Если Метаданные.НайтиПоТипу(Тип) <> Неопределено Тогда
            ...
            НайденнаяСсылка = Менеджер.НайтиПоКоду(Код);
...
Функция СтруктураУчастникаЭПД(ДобавитьПлатежныеРеквизиты = Ложь)
	...
	Результат.Вставить("АдресКвартира");
	
	Результат.Вставить("АдресКодСтраны"); 
	Результат.Вставить("АдресСтрана");
	Результат.Вставить("АдресПредставление");
...
Функция ТипыРегламентовЭПД()
	...
	Результат.Добавить(Перечисления.ТипыРегламентовЭДО.ЭПЭ);
	Результат.Добавить(Перечисления.ТипыРегламентовЭДО.ЭЭР);
	Результат.Добавить(Перечисления.ТипыРегламентовЭДО.ЭСР);
...
// Выполняет поиск объекта учета (например, ЭТрН) по имени файла исходящего сообщения ЭДО 
// 
// Параметры:
//  ИмяФайла - Строка - Имя файла сообщения ЭДО отправленного ЭД
// 
// Возвращаемое значение:
//  ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО - Найденный документ
Функция ДокументПоИмениФайла(Знач ИмяФайла)
	...
	ЭлектронныйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Сведения.ФайлСообщенияЭДО, "ВладелецФайла.ЭлектронныйДокумент");
	ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЭлектронныйДокумент);
	ОбъектыУчета = ИнтеграцияЭДО.АктуальныеОбъектыУчетаСДаннымиПроведения(ЭлектронныеДокументы);
	
	Если ОбъектыУчета.Количество() = 1 Тогда
		Возврат ОбъектыУчета[0].ОбъектУчета;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция НайтиСоздатьЭПД(ДеревоДляСовместимостиСБЭД, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект)
	...
	Иначе
		...
		ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР
			Или КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		...
		Иначе
			СсылкаПоИдФайла = ДокументПоИмениФайла(КлючеваяИнформацияПоТитулу.ЗначениеИДПоиска);
		КонецЕсли;
	...
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЛ Тогда
		...
		КонецЕсли;
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		ДокументОбъект.ИдентификаторЗаказчика = ИдентификаторГрузоотправителя;
		ДокументОбъект.ИдентификаторЭкспедитора = ИдентификаторГрузоперевозчика;
		ДокументОбъект.РольУчастника = РольУчастника;
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР
		Или КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		ДокументОбъект.ИдентификаторЭкспедитора = ИдентификаторГрузоотправителя;
		ДокументОбъект.ИдентификаторЗаказчика = ИдентификаторГрузоперевозчика;
		ДокументОбъект.РольУчастника = РольУчастника;
	Иначе
		ДокументОбъект.ИдентификаторГрузоотправителя = ИдентификаторГрузоотправителя;
		ДокументОбъект.ИдентификаторПеревозчика = ИдентификаторГрузоперевозчика;
	...
	ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1 Тогда
		ДокументОбъект.ТитулЗаказчикаНомер = КлючеваяИнформацияПоТитулу.Номер;
		ДокументОбъект.ТитулЗаказчикаДата = КлючеваяИнформацияПоТитулу.Дата;
		...
		Если ДокументОбъект.РольУчастника = 9 Тогда
			...
			Если НастройкиОбменаСТретьимУчастником <> Неопределено Тогда
				Если КлючеваяИнформацияПоТитулу.ПоДоверенности = Истина Тогда
					ДокументОбъект.СсылкаТитулЗаказчикаСоставитель = НастройкиОбменаСТретьимУчастником.Контрагент;	
				Иначе
					ДокументОбъект.СсылкаТитулЗаказчикаЗаказчик = НастройкиОбменаСТретьимУчастником.Контрагент;
				КонецЕсли;
			КонецЕсли;
			// Экспедитор
			КонтрагентТретьегоУчастника = Неопределено;
			НастройкиОбменаСТретьимУчастником = ОтражениеВУчетеЭДО.ПолучитьНастройкиПолученияПоИдентификаторам(																								
																									...
																									КонтрагентТретьегоУчастника, Истина);
			Если НастройкиОбменаСТретьимУчастником <> Неопределено Тогда
				ДокументОбъект.СсылкаТитулЗаказчикаЭкспедитор = НастройкиОбменаСТретьимУчастником.Контрагент;
			КонецЕсли;
		Иначе
			ДокументОбъект.СсылкаТитулЗаказчикаЗаказчик = ПараметрыОбработки.Отправитель;
			ДокументОбъект.СсылкаТитулЗаказчикаЭкспедитор = ПараметрыОбработки.Получатель;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЗаказчикаСоставитель) Тогда
			ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
						"СсылкаТитулЗаказчикаСоставитель", 
						ДокументОбъект.СсылкаТитулЗаказчикаСоставитель);	
		КонецЕсли;
		
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
			"СсылкаТитулЗаказчикаЗаказчик", 
			ДокументОбъект.СсылкаТитулЗаказчикаЗаказчик);
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
			"СсылкаТитулЗаказчикаЭкспедитор", 
			ДокументОбъект.СсылкаТитулЗаказчикаЭкспедитор);
			
	ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1
			Или КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭСР_Титул1 Тогда
		
		ДокументОбъект.ТитулЭкспедитораНомер = КлючеваяИнформацияПоТитулу.Номер;
		ДокументОбъект.ТитулЭкспедитораДата = КлючеваяИнформацияПоТитулу.Дата;
		...
		Если ДокументОбъект.РольУчастника = 9 Тогда
			...
			Если НастройкиОбменаСТретьимУчастником <> Неопределено Тогда
				Если КлючеваяИнформацияПоТитулу.ПоДоверенности = Истина Тогда
					ДокументОбъект.СсылкаТитулЭкспедитораСоставитель = НастройкиОбменаСТретьимУчастником.Контрагент;	
				Иначе
					ДокументОбъект.СсылкаТитулЭкспедитораЭкспедитор = НастройкиОбменаСТретьимУчастником.Контрагент;
				КонецЕсли;
			КонецЕсли;
			...
			НастройкиОбменаСТретьимУчастником = ОтражениеВУчетеЭДО.ПолучитьНастройкиПолученияПоИдентификаторам(																								
																									...
																									КонтрагентТретьегоУчастника, Истина);
			Если НастройкиОбменаСТретьимУчастником <> Неопределено Тогда
				ДокументОбъект.СсылкаТитулЭкспедитораЗаказчик = НастройкиОбменаСТретьимУчастником.Контрагент;
			КонецЕсли;
		Иначе
			ДокументОбъект.СсылкаТитулЭкспедитораЭкспедитор = ПараметрыОбработки.Отправитель;
			ДокументОбъект.СсылкаТитулЭкспедитораЗаказчик = ПараметрыОбработки.Получатель;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЭкспедитораСоставитель) Тогда
			ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
						"СсылкаТитулЭкспедитораСоставитель", 
						ДокументОбъект.СсылкаТитулЭкспедитораСоставитель);	
		КонецЕсли;
		
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
			"СсылкаТитулЭкспедитораЭкспедитор", 
			ДокументОбъект.СсылкаТитулЭкспедитораЭкспедитор);
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
			"СсылкаТитулЭкспедитораЗаказчик", 
			ДокументОбъект.СсылкаТитулЭкспедитораЗаказчик);
	...
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭПЭ(ДокументОбъект, ТитулыПоДокументу);
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭЭР(ДокументОбъект, ТитулыПоДокументу);
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭСР(ДокументОбъект, ТитулыПоДокументу);
	КонецЕсли;
...
Процедура ЗаполнитьПредставленияСсылочныхТипов(ДокументОбъект)
	...
	ТипДокумента = Неопределено;
	Если ТипЗнч(ДокументОбъект) = Тип("Структура") И ДокументОбъект.Свойство("ТипДокумента") Тогда
		ТипДокумента = ДокументОбъект.ТипДокумента;
	КонецЕсли;
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектроннаяТранспортнаяНакладная")
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектронныйЗаказЗаявка")
		Или ТипДокумента = Документы.ЭлектронныйЗаказЗаявка Тогда
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектронноеПоручениеЭкспедитору") Тогда
		
		Если ДокументОбъект.ТекущийПолученныйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул1 Тогда
			
			Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЗаказчикаЗаказчик) = Ложь Тогда
				ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЗаказчикаЗаказчик");
			КонецЕсли; 
			Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЗаказчикаСоставитель) = Ложь Тогда
				ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЗаказчикаСоставитель"); 
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЗаказчикаЭкспедитор) = Ложь Тогда
				ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЗаказчикаЭкспедитор");
			КонецЕсли;
					
			ПостфиксыДляЗаполнения = Новый Массив;
			ПостфиксыДляЗаполнения.Добавить("Грузоотправитель"); 
			ПостфиксыДляЗаполнения.Добавить("Грузополучатель");
			ПостфиксыДляЗаполнения.Добавить("Грузополучатель"); 
			ПостфиксыДляЗаполнения.Добавить("СвСтрахРекИдентСтрах");
			
			Для каждого ПостфиксЗаполнения  Из ПостфиксыДляЗаполнения  Цикл
			
				ИтераторЦикла = 1;
				КлючСтруктуры = "ТитулЗаказчикаГрузы__" + ИтераторЦикла + "__" + ПостфиксЗаполнения;
				Пока ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, КлючСтруктуры) Цикл
					ИтераторЦикла = ИтераторЦикла + 1;
					КлючСтруктуры = "ТитулЗаказчикаГрузы__" + ИтераторЦикла + "__" + ПостфиксЗаполнения;
				КонецЦикла;	
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЗаказчикаГрузыСвУведСторон__" + ИтераторЦикла + "__Сторона";
			Сторона = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, Сторона) Цикл
				Если Сторона = "3" Тогда
					ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЗаказчикаГрузыСвУведСторон__" + ИтераторЦикла + "__ИноеУвед");	
				КонецЕсли;	
				ИтераторЦикла = ИтераторЦикла + 1;
				КлючСтруктуры = "ТитулЗаказчикаГрузыСвУведСторон__" + ИтераторЦикла + "__Сторона";
			КонецЦикла; 
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__ДейстСтГрузКодВалюты";
			КодВалютыСтоимости = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодВалютыСтоимости) Цикл
				Если ЗначениеЗаполнено(КодВалютыСтоимости) Тогда
					ВалютаНайдено = Справочники.Валюты.НайтиПоКоду(КодВалютыСтоимости);
					КлючЗначенияСсылки = "ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__СсылкаДейстСтГрузВалюта";
					Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, ВалютаНайдено);
					Иначе
						НаименованиеВалютыСтоимости = Неопределено;
						СтруктураРеквизитов.Свойство("ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__ДейстСтГрузНаименованиеВалюты", НаименованиеВалютыСтоимости);
						Если ЗначениеЗаполнено(НаименованиеВалютыСтоимости) Тогда
							СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеВалютыСтоимости));
						КонецЕсли;	
					КонецЕсли;	
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЗаказчикаОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__КодСтраны";
			КодСтраны = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодСтраны) Цикл
				Если ЗначениеЗаполнено(КодСтраны) Тогда
					СтранаНайдено = Справочники.СтраныМира.НайтиПоКоду(КодСтраны);
					КлючЗначенияСсылки = "ТитулЗаказчикаОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__СсылкаСтрана";
					Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, СтранаНайдено);
					Иначе
						НаименованиеСтраны = Неопределено;
						СтруктураРеквизитов.Свойство("ТитулЗаказчикаОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__НаименованиеСтраны", НаименованиеСтраны);
						Если ЗначениеЗаполнено(НаименованиеСтраны) Тогда
							СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеСтраны));
						КонецЕсли;	
					КонецЕсли;	
				КонецЕсли;
				ИтераторЦикла = ИтераторЦикла + 1;
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__ОбъявлСтГрузКодВалюты";
			КодВалютыСтоимости = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодВалютыСтоимости) Цикл
				Если ЗначениеЗаполнено(КодВалютыСтоимости) Тогда
					ВалютаНайдено = Справочники.Валюты.НайтиПоКоду(КодВалютыСтоимости);
					КлючЗначенияСсылки = "ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__СсылкаОбъявлСтГрузВалюта";
					Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, ВалютаНайдено);
					Иначе
						НаименованиеВалютыСтоимости = Неопределено;
						СтруктураРеквизитов.Свойство("ТитулЗаказчикаОписаниеГруза__" + ИтераторЦикла + "__ОбъявлСтГрузНаименованиеВалюты", НаименованиеВалютыСтоимости);
						Если ЗначениеЗаполнено(НаименованиеВалютыСтоимости) Тогда
							СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеВалютыСтоимости));
						КонецЕсли;	
					КонецЕсли;	
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЗаказчикаОписаниеГрузаСвОпаснГруз__" + ИтераторЦикла + "__НадлежащееОтгрузочноеНаименование";
			НадлежащееОтгрузочноеНаименование = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, НадлежащееОтгрузочноеНаименование) Цикл
				Если ЗначениеЗаполнено(НадлежащееОтгрузочноеНаименование) Тогда
					Если СтрДлина(НадлежащееОтгрузочноеНаименование) <= 150 Тогда
					...
					КонецЕсли;
					КлассификаторДОПОГНайдено = Справочники.КлассификаторДОПОГЭПД.НайтиПоНаименованию(КраткоеНаименованиеДОПОГ);
					КлючЗначенияСсылки = "ТитулЗаказчикаОписаниеГрузаСвОпаснГруз__" + ИтераторЦикла + "__СсылкаДОПОГ";
					Если ЗначениеЗаполнено(КлассификаторДОПОГНайдено) Тогда		
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, КлассификаторДОПОГНайдено);
					Иначе
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(КраткоеНаименованиеДОПОГ));	
		...
		ИначеЕсли ДокументОбъект.ТекущийПолученныйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул2 Тогда
			
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЭкспедитораДопИнфЭксп__" + ИтераторЦикла + "__СвПредстЭксп";
			Пока ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, КлючСтруктуры) Цикл
				ИтераторЦикла = ИтераторЦикла + 1;
				КлючСтруктуры = "ТитулЭкспедитораДопИнфЭксп__" + ИтераторЦикла + "__СвПредстЭксп";
			КонецЦикла; 
			...
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЭкспедитораДопИнфЭксп__" + ИтераторЦикла + "__СвОплатаКодВалюты";
			КодВалютыСтоимости = Неопределено;
			Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодВалютыСтоимости) Цикл
				Если ЗначениеЗаполнено(КодВалютыСтоимости) Тогда
					ВалютаНайдено = Справочники.Валюты.НайтиПоКоду(КодВалютыСтоимости);
					КлючЗначенияСсылки = "ТитулЭкспедитораДопИнфЭксп__" + ИтераторЦикла + "__СсылкаСвОплатаВалюта";
					Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, ВалютаНайдено);
					Иначе
						НаименованиеВалютыСтоимости = Неопределено;
						СтруктураРеквизитов.Свойство("ТитулЭкспедитораДопИнфЭксп__" + ИтераторЦикла + "__СвОплатаНаименованиеВалюты", НаименованиеВалютыСтоимости);
						Если ЗначениеЗаполнено(НаименованиеВалютыСтоимости) Тогда
							СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеВалютыСтоимости));
						КонецЕсли;	
					КонецЕсли;	
		...
		ИначеЕсли ДокументОбъект.ТекущийПолученныйТитул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЭ_Титул3 Тогда 
			
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЗаказчикаОтзывСвВозврГрузКонтЛицо");
	
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектроннаяЭкспедиторскаяРасписка") Тогда  
			
		Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЭкспедитораЭкспедитор) = Ложь Тогда
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораЭкспедитор");
		КонецЕсли; 
		Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЭкспедитораСоставитель) = Ложь Тогда
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораСоставитель"); 
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДокументОбъект.СсылкаТитулЭкспедитораЗаказчик) = Ложь Тогда
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораЗаказчик");
		КонецЕсли; 
		
		ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораВладИнфр");
		ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораСвЛицПриемГр");
		
		ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораГрузоотправитель");

		ИтераторЦикла = 1;
		КлючСтруктуры = "ТитулЭкспедитораОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__КодСтраны";
		КодСтраны = Неопределено;
		Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодСтраны) Цикл
			Если ЗначениеЗаполнено(КодСтраны) Тогда
				СтранаНайдено = Справочники.СтраныМира.НайтиПоКоду(КодСтраны);
				КлючЗначенияСсылки = "ТитулЭкспедитораОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__СсылкаСтрана";
				Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, СтранаНайдено);
				Иначе
					НаименованиеСтраны = Неопределено;
					СтруктураРеквизитов.Свойство("ТитулЭкспедитораОписаниеГрузаСвСтрПроисх__" + ИтераторЦикла + "__НаименованиеСтраны", НаименованиеСтраны);
					Если ЗначениеЗаполнено(НаименованиеСтраны) Тогда
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеСтраны));
					КонецЕсли;	
				КонецЕсли;	
		...
		ИтераторЦикла = 1;
		КлючСтруктуры = "ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__ДейстСтГрузКодВалюты";
		КодВалютыСтоимости = Неопределено;
		Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодВалютыСтоимости) Цикл
			Если ЗначениеЗаполнено(КодВалютыСтоимости) Тогда
				ВалютаНайдено = Справочники.Валюты.НайтиПоКоду(КодВалютыСтоимости);
				КлючЗначенияСсылки = "ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__СсылкаДейстСтГрузВалюта";
				Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, ВалютаНайдено);
				Иначе
					НаименованиеВалютыСтоимости = Неопределено;
					СтруктураРеквизитов.Свойство("ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__ДейстСтГрузНаименованиеВалюты", НаименованиеВалютыСтоимости);
					Если ЗначениеЗаполнено(НаименованиеВалютыСтоимости) Тогда
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеВалютыСтоимости));
					КонецЕсли;	
				КонецЕсли;	
		...
		ИтераторЦикла = 1;
		КлючСтруктуры = "ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__ОбъявлСтГрузКодВалюты";
		КодВалютыСтоимости = Неопределено;
		Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, КодВалютыСтоимости) Цикл
			Если ЗначениеЗаполнено(КодВалютыСтоимости) Тогда
				ВалютаНайдено = Справочники.Валюты.НайтиПоКоду(КодВалютыСтоимости);
				КлючЗначенияСсылки = "ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__СсылкаОбъявлСтГрузВалюта";
				Если ЗначениеЗаполнено(ВалютаНайдено) Тогда		
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, ВалютаНайдено);
				Иначе
					НаименованиеВалютыСтоимости = Неопределено;
					СтруктураРеквизитов.Свойство("ТитулЭкспедитораОписаниеГруза__" + ИтераторЦикла + "__ОбъявлСтГрузНаименованиеВалюты", НаименованиеВалютыСтоимости);
					Если ЗначениеЗаполнено(НаименованиеВалютыСтоимости) Тогда
						СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(НаименованиеВалютыСтоимости));
					КонецЕсли;	
				КонецЕсли;	
		...
		ИтераторЦикла = 1;
		КлючСтруктуры = "ТитулЭкспедитораОписаниеГрузаСвОпаснГруз__" + ИтераторЦикла + "__НадлежащееОтгрузочноеНаименование";
		НадлежащееОтгрузочноеНаименование = Неопределено;
		Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, НадлежащееОтгрузочноеНаименование) Цикл
			Если ЗначениеЗаполнено(НадлежащееОтгрузочноеНаименование) Тогда
				Если СтрДлина(НадлежащееОтгрузочноеНаименование) <= 150 Тогда
				...
				КонецЕсли;
				КлассификаторДОПОГНайдено = Справочники.КлассификаторДОПОГЭПД.НайтиПоНаименованию(КраткоеНаименованиеДОПОГ);
				КлючЗначенияСсылки = "ТитулЭкспедитораОписаниеГрузаСвОпаснГруз__" + ИтераторЦикла + "__СсылкаДОПОГ";
				Если ЗначениеЗаполнено(КлассификаторДОПОГНайдено) Тогда		
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, КлассификаторДОПОГНайдено);
				Иначе
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(КраткоеНаименованиеДОПОГ));	
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектроннаяCкладскаяРасписка") Тогда
			...
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораСведСкладСвВладСклад");
			ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, "ТитулЭкспедитораГрузСвСтрахРекИдентСтрах");
			
			ИтераторЦикла = 1;
			КлючСтруктуры = "ТитулЭкспедитораГрузУслПриемГр__" + ИтераторЦикла + "__СвЛицПриемГрХран";
			Пока ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, КлючСтруктуры) Цикл
				ИтераторЦикла = ИтераторЦикла + 1;
				КлючСтруктуры = "ТитулЭкспедитораГрузУслПриемГр__" + ИтераторЦикла + "__СвЛицПриемГрХран";
			КонецЦикла;
			...
			Если ОбластьНайдено <> Неопределено Тогда
				...
				Если СтрНайти(ИмяРеквизита, ".") Тогда
					...
					Если ТипРеквизита = "Дата" Тогда
						...
						КонецЕсли;
					ИначеЕсли ТипРеквизита = "Время" Тогда
						ЗначениеРеквизита = ОбменСГИСЭПДКлиентСервер.ПреобразоватьВДату(ТекущийОбъектXDTO.Объект);
...
Функция СформироватьДанныеПрикладногоДокумента(ОбъектУчета, Данные)
	...
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектронноеПоручениеЭкспедитору") Тогда
		ДанныеСообщения.Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЭ;
		ДанныеСообщения.Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭПЭ; 
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектУчета, 
			"Номер,
			|Дата");
		ДанныеСообщения.Содержание.НомерДокумента = РеквизитыДокумента.Номер;
		ДанныеСообщения.Содержание.ДатаДокумента = РеквизитыДокумента.Дата;
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектроннаяЭкспедиторскаяРасписка") Тогда
		ДанныеСообщения.Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЭР;
		ДанныеСообщения.Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭЭР; 
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектУчета, 
			"Номер,
			|Дата");
		ДанныеСообщения.Содержание.НомерДокумента = РеквизитыДокумента.Номер;
		ДанныеСообщения.Содержание.ДатаДокумента = РеквизитыДокумента.Дата;
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектроннаяCкладскаяРасписка") Тогда
		ДанныеСообщения.Содержание.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСР;
		ДанныеСообщения.Содержание.ТипРегламента = Перечисления.ТипыРегламентовЭДО.ЭСР; 
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектУчета, 
			"Номер,
			|Дата");
		ДанныеСообщения.Содержание.НомерДокумента = РеквизитыДокумента.Номер;
...
Процедура УстановитьПоследовательностьУзловРекурсивно(ОбъектXDTO)
	
	Если ТипЗнч(ОбъектXDTO) = Тип("ОбъектXDTO") Тогда
			
		ТипОбъекта = ОбъектXDTO.Тип();
		
		КоллекцияСвойств = ТипОбъекта.Свойства;
		
		Если ТипОбъекта.Последовательный Тогда
			
			ДанныеПоследовательности = Новый Соответствие;
			
			Последовательность = ОбъектXDTO.Последовательность();
			
			Для Индекс = 0 По Последовательность.Количество() -1 Цикл
				
				СвойствоПоследовательности = Последовательность.ПолучитьСвойство(Индекс);
				
				ЗначенияПоследовательности = ДанныеПоследовательности.Получить(СвойствоПоследовательности);
				
				Если ЗначенияПоследовательности = Неопределено Тогда
					ЗначенияПоследовательности = Новый Массив;
					ДанныеПоследовательности.Вставить(СвойствоПоследовательности, ЗначенияПоследовательности); 	
				КонецЕсли;
				
				ЗначенияПоследовательности.Добавить(Последовательность.ПолучитьЗначение(Индекс)); 
			...
			Для Каждого Свойство Из КоллекцияСвойств Цикл
				...
				ЗначенияСвойств =  ДанныеПоследовательности.Получить(Свойство);
		...
		Для Каждого Свойство Из КоллекцияСвойств Цикл
			...
			Если ТипЗнч(ЗначениеСвойства) = Тип("СписокXDTO") Тогда  
				...
				Для Индекс = 0 По ЗначениеСвойства.Количество() -1 Цикл
					УстановитьПоследовательностьУзловРекурсивно(ЗначениеСвойства.Получить(Индекс));				
				КонецЦикла;
				
			Иначе
				
				УстановитьПоследовательностьУзловРекурсивно(ЗначениеСвойства);
	...
	ИначеЕсли  ТипЗнч(ОбъектXDTO) = Тип("СписокXDTO") Тогда
		...
		Для Индекс = 0 По ОбъектXDTO.Количество() -1 Цикл
			УстановитьПоследовательностьУзловРекурсивно(ОбъектXDTO.Получить(Индекс));				
		КонецЦикла;
...
Функция ПреобразоватьЗначениеДляXML(Значение, СвойствоТип)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Значение)) Тогда
		...
		Если СвойствоТип.Имя = "string" Или СвойствоТип.БазовыйТип.Имя = "string"
			Или СвойствоТип.БазовыйТип.Имя = "ОКВГУМТип" Тогда
...
// Заполняет структуру реквизитов титула данными из таблицы информационных полей.
//
// Параметры:
//   СтруктураРеквизитовТитулаЗаполнение - Структура - структура для заполнения реквизитов титула
//   АдресТаблицаИнформационныеПоля - Строка, Неопределено - адрес таблицы информационных полей во временном хранилище
Процедура ЗаполнитьТаблицуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение, АдресТаблицаИнформационныеПоля)
	
	Если АдресТаблицаИнформационныеПоля = Неопределено
		Или Не ЭтоАдресВременногоХранилища(АдресТаблицаИнформационныеПоля) Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаИнформационныеПоля = ПолучитьИзВременногоХранилища(АдресТаблицаИнформационныеПоля); // см. НоваяТаблицаИнформационныеПоля
	ИтераторЦикла = 1;
	ИдентификаторСтрокиИнфПолей = Строка(Новый УникальныйИдентификатор);
	Для Каждого СтрокаИнформационныхПолей Из ТаблицаИнформационныеПоля Цикл
		Если ИтераторЦикла = 1 Тогда
			// Добавляем строку-родителя информационных полей
			ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
				СтрокаИнформационныхПолей.ИдентификаторИнфоПолей,
				ИтераторЦикла,
				"ИдентификаторСтроки",
				ИдентификаторСтрокиИнфПолей);
		КонецЕсли;
		// Добавляем информационные поля
		ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
			СтрокаИнформационныхПолей.ИдентификаторДопИнформации,
			ИтераторЦикла,
			"ИдентификаторИнформации",
			СтрокаИнформационныхПолей.Идентификатор);
		ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
			СтрокаИнформационныхПолей.ИдентификаторДопИнформации,
			ИтераторЦикла,
			"ТипДанных",
			СтрокаИнформационныхПолей.ТипДанных);
		ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
			СтрокаИнформационныхПолей.ИдентификаторДопИнформации,
			ИтераторЦикла,
			"Значение",
			СтрокаИнформационныхПолей.Значение);
		ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитулаЗаполнение,
			СтрокаИнформационныхПолей.ИдентификаторДопИнформации,
			ИтераторЦикла,
...
// Добавляет строку информационного поля в структуру реквизитов титула.
//
// Параметры:
//   СтруктураРеквизитовТитула - Структура - структура для хранения реквизитов титула
//   ИдентификаторИнфоПолей - Строка - идентификатор группы информационных полей
//   Итератор - Число - номер итерации для уникальности ключа
//   Идентификатор - Строка - идентификатор информационного поля
//   Значение - Строка - значение информационного поля
Процедура ДобавитьСтрокуИнформационныхПолей(СтруктураРеквизитовТитула, ИдентификаторИнфоПолей, Итератор, Идентификатор,
	Значение)
	
	СтруктураРеквизитовТитула.Вставить(ИдентификаторИнфоПолей + "__" + Итератор + "__" + Идентификатор, Значение);
...
Функция УзлыСхемыПоНаименованиюФормы(Знач ИмяФормы, ТипДокумента)
	...
	ИначеЕсли ТипДокумента = ТипДокументаПоручениеЭкспедитору() Тогда
		Возврат УзлыСхемыФормыПоручениеЭкспедитору(ИмяФормы); 
	ИначеЕсли ТипДокумента = ТипДокументаЭкспедиторскаяРасписка() Тогда
		Возврат УзлыСхемыФормыЭкспедиторскаяРасписка(ИмяФормы);
	ИначеЕсли ТипДокумента = ТипДокументаСкладскаяРасписка() Тогда
		Возврат УзлыСхемыФормыСкладскаяРасписка(ИмяФормы);

	КонецЕсли;
	
КонецФункции 

Функция УзлыСхемыФормыСкладскаяРасписка(ИмяФормы)
	
	МассивУзлов = Новый Массив;
	
	Если ИмяФормы = "ОсновнаяФорма" Тогда
		МассивУзлов.Добавить("//Файл"); 
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораСоставительДоверенность" Тогда
		МассивУзлов.Добавить("//Файл/Документ/ОснДовОргСост"); 
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДоговор" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДогТрЭксп");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораСведСклад" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СведСклад/АдрСклад/Адрес/АдрИнф/АдрТекст");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СведСклад/ВладСкладЭксп");
	
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораСопроводительныеДокументы" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СопрДок");
				
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораРеквизитыСторонСопроводительныхДокументов" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СопрДок/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГруз" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз");	

	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузСвКонтейн" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузСвКонтейнНомПломб" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвКонтейн/НомПломб");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузСопрДокПарт" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СопрДок");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузРеквизитыСторонСопрДокПарт" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СопрДок/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузРеквЗаказПок" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/РеквЗаказПок");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузРеквЗаказПокРеквизитыСторон" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/РеквЗаказПок/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГруза" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузУслПриемГр" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/УслПриемГр");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузУслПриемГрОснДовПриемГрХран" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/УслПриемГр/ОснДовПриемГрХран/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/УслПриемГр/ОснДовПриемГрХран/РеквНомерДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/УслПриемГр/ОснДовПриемГрХран/РеквДатаДок");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузСвСтрахПолис" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвСтрах/Полис");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузСвСтрахУслСтрах" Тогда
		 МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвСтрах/УслСтрах"); 
		 
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодТовОКПД2" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/КодТовОКПД2");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодГрузГНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/КодГрузГНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодГрузЕТСНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/КодГрузЕТСНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаМарк" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/Марк");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаНомКонтейн" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/НомКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвСтрПроисх" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/СвСтрПроисх");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаИнфУпГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/ИнфУпГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвОпаснГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/СвОпаснГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвГосСист" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГрузХран/СвГосСист");
...
Функция УзлыСхемыФормыЭкспедиторскаяРасписка(ИмяФормы)
	...
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузы" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз");	

	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыСвКонтейн" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыСвКонтейнНомПломб" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвКонтейн/НомПломб");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыСопрДокПарт" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СопрДокПарт");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыРеквизитыСторонСопрДокПарт" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СопрДокПарт/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыРеквЗакПост" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/РеквЗаказПост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыРеквизитыСторонРеквЗакПост" Тогда   
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/РеквЗаказПост/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГруза" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз");

	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыСвСтрахУслСтрах" Тогда
		 МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/СвСтрах/УслСтрах"); 
		 
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодТовОКПД2" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/КодТовОКПД2");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодГрузГНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/КодГрузГНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаКодГрузЕТСНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/КодГрузЕТСНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаМарк" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/Марк");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаНомКонтейн" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/НомКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвСтрПроисх" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/СвСтрПроисх");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаИнфУпГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/ИнфУпГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвОпаснГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/СвОпаснГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораОписаниеГрузаСвГосСист" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/ОпГруз/СвГосСист");
			
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораВладИнфр" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ВладИнфр/СовпГОВ");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораСвЛицПриемГр" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ОснДовПриемГр/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ОснДовПриемГр/РеквНомерДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ОснДовПриемГр/РеквДатаДок");	
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораСвЛицПогрГр" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвЛицПогрГр/ЛицоПогруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораГрузыУслПриемОбязЭксп" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/СвГруз/УслПрием/ОбязЭксп");
...
Функция УзлыСхемыФормыПоручениеЭкспедитору(ИмяФормы)
	
	МассивУзлов = Новый Массив;
	
	Если ИмяФормы = "ОсновнаяФорма" Тогда
		МассивУзлов.Добавить("//Файл");
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаСоставительДоверенность" Тогда
		МассивУзлов.Добавить("//Файл/Документ/ОснДовОргСост");
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаДоговор" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/ДогТрЭксп");
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаЗаявкиНаТранспортировку" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/РеквЗаявТранс");
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаРеквизитыСторонЗаявокНаТранспортировку" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/РеквЗаявТранс/РеквИдРекСост");
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаСопроводительныеДокументы" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СопрДок");	
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаРеквизитыСторонСопроводительныхДокументов" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СопрДок/РеквИдРекСост");		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаИныеДанныеИОговоркиКлиента" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/ИныеДанКлнт");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузы" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз"); 
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвТС" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвТС");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГруза" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз"); 
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвСтрахПолис" Тогда	
	    МассивУзлов.Добавить("/Файл/Документ/СодИнфКлнт/СвГруз/СвСтрах/Полис/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвСтрах/Полис/РеквНомерДок"); 
		МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвСтрах/Полис/РеквДатаДок");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвСтрахУслСтрах" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвСтрах/УслСтрах");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвУведСторон" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвУведСторон");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвУсл" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвУсл");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыРеквЗакПост" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/РеквЗакПост");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыРеквизитыСторонРеквЗакПост" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/РеквЗакПост/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСопрДокПарт" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СопрДокПарт");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыРеквизитыСторонСопрДокПарт" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СопрДокПарт/РеквИдРекСост");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвКонтейн" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыСвКонтейнНомПломб" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/СвКонтейн/НомПломб");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаГрузыУказКлнт" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/УказКлнт/УкНормПрвз");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/УказКлнт/Отмет");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаКодТовОКПД2" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/КодТовОКПД2");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаКодГрузГНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/КодГрузГНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаКодГрузЕТСНГ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/КодГрузЕТСНГ");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаМарк" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/Марк");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаНомКонтейн" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/НомКонтейн");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаСвСтрПроисх" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/СвСтрПроисх");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаИнфУпГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/ИнфУпГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаСвОпаснГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/СвОпаснГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОписаниеГрузаСвГосСист" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфКлнт/СвГруз/ОпГруз/СвГосСист");
			
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭксп" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораИныеДанЭксп" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ИныеДанЭксп");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспОснОкУслИным" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвПредстЭксп/ОснОкУслИным/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвПредстЭксп/ОснОкУслИным/РеквНомерДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвПредстЭксп/ОснОкУслИным/РеквДатаДок");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспРеквЗакПост" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/РеквЗакПост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспРеквЗакПостРеквИдРекСост" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/РеквЗакПост/РеквИдРекСост"); 
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспИдентПартГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/ИдентПартГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспНомПартГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/НомПартГруз");
				
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвОплата" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвОплата/ОбщСтУсл");

	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвТрсГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвТрсГрузСвТСДост" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз/СвТСДост");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвОплатаДокОплУ" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвОплата/ДокОплУсл/РеквИдРекСост");	
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвТрсГрузСвТСДостСвВодит" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз/СвТСДост/СвВодит");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвТрсГрузСвТСДостСвВодитПутевойЛист" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз/СвТСДост/СвВодит/ПутевойЛист/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз/СвТСДост/СвВодит/ПутевойЛист/РеквНомерДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвТрсГруз/СвТСДост/СвВодит/ПутевойЛист/РеквДатаДок");
		
	ИначеЕсли ИмяФормы = "ТитулЭкспедитораДопИнфЭкспСвОплатаДокОплУсл" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвОплата/ДокОплУсл/РеквНаимДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвОплата/ДокОплУсл/РеквНомерДок");
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЭксп/ДопИнфЭксп/СвОплата/ДокОплУсл/РеквДатаДок");
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОтзывИныеДанКлнт" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфОтз/ИныеДанКлнт"); 
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОтзывИныеДанКлнт" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфОтз/ИныеДанКлнт"); 
		
	ИначеЕсли ИмяФормы = "ТитулЗаказчикаОтзывСвВозврГруз" Тогда	
	    МассивУзлов.Добавить("//Файл/Документ/СодИнфОтз/СвВозврГруз/УслВозврГруз");
...
Функция УзлыСхемыФормыЗаказЗаявки(ИмяФормы)
	...
	ИначеЕсли ИмяФормы = "ТитулГрузоотправителяСоставительДоверенность" Тогда
		МассивУзлов.Добавить("//Файл/Документ/ОснДовОргСост");
	КонецЕсли;
...
Функция ТипДокументаПоручениеЭкспедитору()
	
	Возврат ОбменСГИСЭПДКлиентСервер.ТипДокументаПоручениеЭкспедитору();
	
КонецФункции 

Функция ТипДокументаЭкспедиторскаяРасписка()
	
	Возврат ОбменСГИСЭПДКлиентСервер.ТипДокументаЭкспедиторскаяРасписка();
	
КонецФункции  

Функция ТипДокументаСкладскаяРасписка()
	
	Возврат ОбменСГИСЭПДКлиентСервер.ТипДокументаСкладскаяРасписка();
	...
	Если СтруктураРеквизитов = Неопределено Тогда
		...
		Если СтрокаДерева.СтруктураХраненияРеквизита.ЭтоРеквизитТабЧасти Тогда
			...
			Если Не ОбъектСДаннымиДокумента.Свойство(ТабличнаяЧасть) Тогда
				Возврат Ложь;
			КонецЕсли;
			Если ОбъектСДаннымиДокумента[ТабличнаяЧасть].Количество() > 0
				И ОбъектСДаннымиДокумента[ТабличнаяЧасть].Колонки.Найти(Реквизит) <> Неопределено Тогда
		...
		Иначе
			Если ОбъектСДаннымиДокумента.Свойство(Реквизит) Тогда 
...
Функция ОсобоеОформлениеДляЗаполненногоЭлемента(ПутьКЭлементуФормы,
	...
	ИначеЕсли ТипДокумента = ТипДокументаДоговорФрахтования() Тогда
		...
		Возврат Заголовок; 
	ИначеЕсли ТипДокумента = ТипДокументаПоручениеЭкспедитору() Тогда
		Заголовок = ОсобоеОформлениеДляПорученияЭкспедитору(ПутьКЭлементуФормы,
			ОбъектСДаннымиДокумента, СтруктураРеквизитов);
		Возврат Заголовок;
	ИначеЕсли ТипДокумента = ТипДокументаЭкспедиторскаяРасписка() Тогда
		Заголовок = ОсобоеОформлениеДляЭкспедиторскойРасписки(ПутьКЭлементуФормы,
			ОбъектСДаннымиДокумента, СтруктураРеквизитов);
		Возврат Заголовок;
	ИначеЕсли ТипДокумента = ТипДокументаСкладскаяРасписка() Тогда
		Заголовок = ОсобоеОформлениеДляСкладскойРасписки(ПутьКЭлементуФормы,
			ОбъектСДаннымиДокумента, СтруктураРеквизитов);
		Возврат Заголовок;
...
Функция ОсобоеОформлениеДляЗаказаЗаявки(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
	...
	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьТитулГрузоотправителяСоставительДоверенность" Тогда
		
		Наименование = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулГрузоотправителяСоставительДоверенностьНаименование");
		Номер = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулГрузоотправителяСоставительДоверенностьНомер");
		Дата = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулГрузоотправителяСоставительДоверенностьДата");
...
Функция ОсобоеОформлениеДляПорученияЭкспедитору(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
	
	Если ПутьКЭлементуФормы = "ЗаполнитьТитулЗаказчикаСоставительДоверенность" Тогда
		
		Наименование = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаСоставительДоверенностьНаименование");
		Номер = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаСоставительДоверенностьНомер");
		Дата = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаСоставительДоверенностьДата");
		
		Заголовок = СтрШаблон("%1 %2 от %3", Наименование, Номер, Формат(Дата, "ДЛФ=D;"));
		
		Возврат Заголовок;
		
	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьТитулЗаказчикаДоговор" Тогда 
		
		Наименование = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаДоговорНаименование");
		Номер = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаДоговорНомер");
		Дата = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЗаказчикаДоговорДата");
...
Функция ОсобоеОформлениеДляЭкспедиторскойРасписки(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
	
	Если ПутьКЭлементуФормы = "ЗаполнитьТитулЭкспедитораСоставительДоверенность" Тогда
		
		Наименование = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораСоставительДоверенностьНаименование");
		Номер = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораСоставительДоверенностьНомер");
		Дата = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораСоставительДоверенностьДата");
		
		Заголовок = СтрШаблон("%1 %2 от %3", Наименование, Номер, Формат(Дата, "ДЛФ=D;"));
		
		Возврат Заголовок;
		
	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьТитулЭкспедитораДоговор" Тогда 
		
		Наименование = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораДоговорНаименование");
		Номер = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораДоговорНомер");
		Дата = ЗначениеРеквизитаСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулЭкспедитораДоговорДата");
...
Функция ОсобоеОформлениеДляСкладскойРасписки(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
