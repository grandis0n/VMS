...
Функция ВидимостьЭлементовДиректБанк(Организация, ДанныеБанкаСчета) Экспорт
	...
	ЕстьИнтеграцияБанковскойВыписки = Ложь;
	ВыпискиТолькоЧерезИнтегратор = Ложь;
	...
	Иначе
		...
		ИнтеграторыБанковскойВыписки = ОбменСБанками.ИнтеграторыБанковскойВыписки(Организация, Банк);
		Если ЗначениеЗаполнено(ИнтеграторыБанковскойВыписки) Тогда
			...
			НастройкаОбмена = ОбменСБанками.НастройкаОбменаПоОтбору(ПараметрыОтбора);
			Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
				Если ОбменСБанками.ВозможенПрямойОбменСБанком(БИК, 1) Тогда
					ЕстьИнтеграцияБанковскойВыписки = Истина;
				Иначе
					ВыпискиТолькоЧерезИнтегратор = Истина;
				КонецЕсли;
			КонецЕсли;
		Иначе
	...
	Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
		Если ЕстьИнтеграцияБанковскойВыписки Тогда
			Результат = "ПоказатьИзменениеПодключения";
		ИначеЕсли ВыпискиТолькоЧерезИнтегратор Тогда
			Результат = "ПоказатьТолькоИнтегратора";
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработатьТабличныйДокументВСтруктуруСервер(ЗагруженныеФайлы, АдресХранилища) Экспорт
	...
	Для Каждого ЗагруженныйФайл Из ЗагруженныеФайлы Цикл
		...
		ЗагруженныйФайл.Вставить("ДвоичныеДанные", ДвоичныеДанные);
...
Процедура ЗаполнитьШапкуВыписки(Организация, ДанныеВыписки, СтрокаТаблицыВыписок, МассивСчетов)
	...
	Для ИндексВыписки = 0 По КоличествоВыписок - 1 Цикл
		...
		ИдентификаторЦифровогоСчета = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.ИдентификаторЦифровогоСчета");
		Если ЗначениеЗаполнено(ИдентификаторЦифровогоСчета) Тогда
			МассивНомеровСчетов.Добавить(ИдентификаторЦифровогоСчета);
		Иначе
	...
	БИК = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.Банк.БИК");
	Если ЗначениеЗаполнено(БИК) Тогда
		ДополнительныеРеквизиты.Вставить("БИК", БИК);
	КонецЕсли;
	...
	ТаблицаИтогов = ОбменСБанкамиСлужебный.ТаблицаВыписок();
	Для Каждого Выписка Из СтрокаТаблицыВыписок.Строки Цикл
		НомерСчетаВыписки = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.НомерСчета");
		ИдентификаторЦифровогоСчета = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.ИдентификаторЦифровогоСчета");
		НачальныйОстаток = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.НачальныйОстаток");
		ОборотВходящихПлатежей = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.ОборотВходящихПлатежей");
		ОборотИсходящихПлатежей = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.ОборотИсходящихПлатежей");
		КонечныйОстаток = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.КонечныйОстаток");
		НачалоПериода = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.НачалоПериода");
		КонецПериода  = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.КонецПериода");
		
		СтрокаИтогов = ТаблицаИтогов.Найти(НомерСчетаВыписки, "НомерСчета");
		Если СтрокаИтогов = Неопределено Тогда
			СтрокаИтогов = ТаблицаИтогов.Добавить();
			СтрокаИтогов.ИдентификаторЦифровогоСчета = ИдентификаторЦифровогоСчета;
			СтрокаИтогов.НомерСчета       = НомерСчетаВыписки;
			СтрокаИтогов.НачалоПериода    = НачалоПериода;
			СтрокаИтогов.НачальныйОстаток = НачальныйОстаток;
			СтрокаИтогов.ОборотВходящихПлатежей  = 0;
			СтрокаИтогов.ОборотИсходящихПлатежей = 0;
		КонецЕсли;
		
		Если НачалоПериода < СтрокаИтогов.НачалоПериода Тогда
			СтрокаИтогов.НачальныйОстаток = НачальныйОстаток;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаИтогов.КонецПериода) Или КонецПериода > СтрокаИтогов.КонецПериода Тогда
			СтрокаИтогов.КонечныйОстаток = КонечныйОстаток;
		КонецЕсли;
		
		СтрокаИтогов.НачалоПериода = Мин(НачалоПериода, СтрокаИтогов.НачалоПериода);
		СтрокаИтогов.КонецПериода =
			Макс(КонецПериода, ?(ЗначениеЗаполнено(СтрокаИтогов.КонецПериода), СтрокаИтогов.КонецПериода, '00010101'));
		
		СтрокаИтогов.ОборотВходящихПлатежей  = СтрокаИтогов.ОборотВходящихПлатежей
			+ ?(ЗначениеЗаполнено(ОборотВходящихПлатежей), ОборотВходящихПлатежей, 0);
		СтрокаИтогов.ОборотИсходящихПлатежей = СтрокаИтогов.ОборотИсходящихПлатежей
			+ ?(ЗначениеЗаполнено(ОборотИсходящихПлатежей), ОборотИсходящихПлатежей, 0);
	КонецЦикла;
	
	ЗаполнитьОстаткиИОбороты(ДанныеВыписки, ТаблицаИтогов);
	
КонецПроцедуры

Процедура ЗаполнитьОстаткиИОбороты(ДанныеВыписки, ТаблицаИтогов)
	...
	Для Каждого СтрокаИтогов Из ТаблицаИтогов Цикл
		...
		ЕстьОстатки = Ложь;
		Если ЗначениеЗаполнено(СтрокаИтогов.НачальныйОстаток) Тогда
			СтрокаОстатка.НачальныйОстаток = СтрокаИтогов.НачальныйОстаток;
			ЕстьОстатки = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаИтогов.ОборотВходящихПлатежей) Тогда
			СтрокаОстатка.ВсегоПоступило = СтрокаИтогов.ОборотВходящихПлатежей;
			ЕстьОстатки = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаИтогов.ОборотИсходящихПлатежей) Тогда
			СтрокаОстатка.ВсегоСписано = СтрокаИтогов.ОборотИсходящихПлатежей;
			ЕстьОстатки = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаИтогов.КонечныйОстаток) Тогда
			СтрокаОстатка.КонечныйОстаток = СтрокаИтогов.КонечныйОстаток;
			ЕстьОстатки = Истина;
		КонецЕсли;
		
		Если ЕстьОстатки Тогда
			Если ЗначениеЗаполнено(СтрокаИтогов.ИдентификаторЦифровогоСчета) Тогда
				СтрокаОстатка.РасчСчет = СтрокаИтогов.ИдентификаторЦифровогоСчета;
			Иначе
				СтрокаОстатка.РасчСчет = СтрокаИтогов.НомерСчета;
			КонецЕсли;
			
			СтрокаОстатка.ДатаНачала = СтрокаИтогов.НачалоПериода;
			СтрокаОстатка.ДатаКонца  = СтрокаИтогов.КонецПериода;
			
			Если ЗначениеЗаполнено(ДатаНачала) Тогда
				ДатаНачала = Мин(ДатаНачала, СтрокаОстатка.ДатаНачала);
			Иначе
				ДатаНачала = СтрокаОстатка.ДатаНачала;
			КонецЕсли;
			
			ДатаКонца = Макс(ДатаКонца, СтрокаОстатка.ДатаКонца);
			
			// СекцияРасчСчет
			ДанныеВыписки.Условия.РасчСчет.Добавить(СтрокаИтогов.НомерСчета);
		Иначе
			ДанныеВыписки.Остатки.Удалить(СтрокаОстатка);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ДанныеВыписки.Условия.РасчСчет)
		И ЗначениеЗаполнено(ДатаНачала)
		И ЗначениеЗаполнено(ДатаКонца) Тогда
		ДанныеВыписки.Условия.Вставить("ДатаНачала", ДатаНачала);
		ДанныеВыписки.Условия.Вставить("ДатаКонца",  ДатаКонца);
	КонецЕсли;
...
Процедура ДобавитьДокумент(ДанныеВыписки, Операция, ОписаниеОбменаВФормате1С, Индекс, РасСчет = "")
	...
	ДобавитьНеПустойПараметр(Документ, Операция, "MCC",
		"Выписки.НомерСтроки.Операции.НомерСтроки.MCC");
	ДобавитьНеПустойПараметр(Документ, Операция, "ПлатежнаяСсылкаЦР",
		"Выписки.НомерСтроки.Операции.НомерСтроки.ПлатежнаяСсылкаЦР");
...
Процедура ЗаполнитьОтсутствующиеРеквизиты(Документ, Операция, РасСчет)
	...
	Если ПустаяСтрока(РасСчет) Тогда
		СтрокаДерева = Операция;
		Пока СтрокаДерева <> Неопределено И Не ЗначениеЗаполнено(РасСчет) Цикл
			РасСчет = ЗначениеРеквизитаВДереве(СтрокаДерева, "Выписки.НомерСтроки.НомерСчета");
			СтрокаДерева = СтрокаДерева.Родитель;
		КонецЦикла;
		
		Если Не ЗначениеЗаполнено(РасСчет) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает код вид операции по ЦБ, соответствующий коду вида электронного документа ДиректБанк
//
// Параметры:
//  КодВидаЭД - Строка - код вида электронного документа 1С:ДиректБанк
//
// Возвращаемое значение:
//  Строка - код вида операции в соответствии с нормативными актами Банка России
//
Функция СоответствиеВидЭДВидОперации(КодВидаЭД) Экспорт
	...
	// из Описание стандарта взаимодействия между "1С:Предприятие 8" и банковским сервисом по технологии 1С:ДиректБанк
...
Процедура ДобавитьПлательщикаИПолучателя(Документ, Операция, ОписаниеОбменаВФормате1С)
	...
	ДобавитьНеПустойПараметр(ДанныеЭД, Операция, "ПлательщикРасчСчет",
		"Выписки.НомерСтроки.Операции.НомерСтроки.Плательщик.РасчСчет", , Истина);
	ДобавитьНеПустойПараметр(ДанныеЭД, Операция, "ПлательщикИдентификаторЦифровогоСчета",
		"Выписки.НомерСтроки.Операции.НомерСтроки.Плательщик.ИдентификаторЦифровогоСчета", , Истина);
	...
	ДобавитьНеПустойПараметр(ДанныеЭД, Операция, "ПолучательИдентификаторЦифровогоСчета",
		"Выписки.НомерСтроки.Операции.НомерСтроки.Получатель.ИдентификаторЦифровогоСчета", , Истина);
...
Функция ВыделитьРасСчет(ДанныеИзБанка)
	...
	Если ТипЗнч(РасСчет) = Тип("Массив") И РасСчет.Количество() > 1 Тогда
		РасСчет = "";
	КонецЕсли;
...
//
// Параметры:
//  ДеревоДанных - ДеревоЗначений - дерево данных с разобранной выпиской (см. ДеревоЭлектронногоДокументаБЭДСлужебный.ДеревоЭлектронногоДокумента)
//  Организация - СправочникСсылка.Организации - организация, по которой получена выписка
//  МассивСчетов - Массив из СправочникСсылка.БанковскиеСчета - содержит ссылки на банковские счета организации.
//
// Возвращаемое значение:
//  Структура - разобранные данные выписки, содержит значения:
...
//
Функция ПрочитанныеДанныеВыпискиИзДерева(Знач ДеревоДанных, Организация, МассивСчетов = Неопределено) Экспорт
	...
	Для Каждого Выписка Из СтрокаТаблицыВыписок.Строки Цикл
		...
		Если СтрокаОперацияВыписки.Значение = Неопределено
			Или СтрокаОперацияВыписки.Значение = 0
			Или СтрокаОперацияВыписки.Значение = "" Тогда
			Продолжить;
		КонецЕсли;
