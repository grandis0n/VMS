...
Процедура ПриОпределенииПараметровИнтеграцииФормыПрикладногоОбъекта(Форма, МодулиИнтеграции) Экспорт
	...
	Если ИмяФормы = "Документ.ПроизводственнаяОперацияВЕТИС.Форма.ФормаДокумента" 
		И Не ОбщегоНазначенияИС.СерииИспользуются() Тогда
		Форма.Элементы.ТоварыСгенерироватьСерии.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриСозданииНаСервереВФормеПрикладногоОбъекта(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаВыбора" Тогда
		...
		// Добавить характеристику отбора номенклатуры - вид продукции из регистра
		Если Форма.Параметры.Свойство("Отбор")
			И Форма.Параметры.Отбор.Свойство("ВидПродукцииИС") Тогда
			ПереопределитьТаблицуПараметровДинамическогоСпискаВыбораНоменклатуры(Форма);
		КонецЕсли;
...
Процедура ПереопределитьТаблицуПараметровДинамическогоСпискаВыбораНоменклатуры(Форма, ИмяСписка = "Список", ИсточникВидовПродукции = Неопределено) Экспорт
	...
	Если ИсточникВидовПродукции = Неопределено Тогда
		ИсточникВидовПродукции = Форма.Параметры.Отбор.ВидПродукцииИС; 
		Если ТипЗнч(ИсточникВидовПродукции) <> Тип("Массив") И ТипЗнч(ИсточникВидовПродукции) <> Тип("ФиксированныйМассив") Тогда
			Если ТипЗнч(ИсточникВидовПродукции) = Тип("СправочникСсылка.КлючиВидовПродукцииИС") Тогда
				ИсточникВидовПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииКлюча(ИсточникВидовПродукции);
			Иначе
				ИсточникВидовПродукции = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИсточникВидовПродукции);
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли;
	
	Для Каждого ВидПродукции Из ИсточникВидовПродукции Цикл
	...
	ТекстЗапроса = Форма[ИмяСписка].ТекстЗапроса;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Форма[ИмяСписка], "Ссылка.ВидПродукцииИС", ВидыПродукции, ВидСравненияКомпоновкиДанных.ВСписке);
...
Процедура ПриИзмененииНоменклатуры(Форма, ТекущаяСтрока, КэшированныеЗначения = Неопределено, ПараметрыУказанияСерий = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "Номенклатура")
		...
		ТекущаяСтрока.МаркируемаяПродукция = ИнтеграцияИСМПБП.МаркируемаяПродукция(ТекущаяСтрока.Номенклатура, ПараметрыСканирования);
		
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "ТребуетВзвешивания")
		Или ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "ПроизвольнаяЕдиницаУчета")
		Или ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "ВидПродукции") Тогда

			ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(ТекущаяСтрока.Номенклатура)[ТекущаяСтрока.Номенклатура];
			
			Реквизиты = Новый Структура;
			Реквизиты.Вставить("ТребуетВзвешивания",
			...
			Реквизиты.Вставить("ПроизвольнаяЕдиницаУчета",
			...
			Реквизиты.Вставить("ВидПродукции",
				ОписаниеНоменклатуры.КлючВидовПродукции);
