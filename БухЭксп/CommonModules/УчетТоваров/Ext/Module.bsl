...
Процедура СформироватьДвиженияКорректировкаПоступлениеТоваров(ТаблицаВозвращенныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаПартии Из Параметры.ТаблицаВозвращенныеТовары Цикл
		...
		Если Реквизиты.СтавкаНалогаУвеличилась Тогда
			...
			Проводка = Движения.Хозрасчетный.Добавить();
			Проводка.Период      = Реквизиты.ДатаДокументаРеализации;
			Проводка.Организация = Реквизиты.Организация;
			Проводка.Содержание  = СокрЛП(Реквизиты.Содержание);
			...
			ЗаполнитьАналитикуСписанияТовара(Проводка, Реквизиты, СтрокаПартии, "Кт", 1, ВедетсяУчетПоПартиям, Реквизиты.СтавкаНалогаУвеличилась, ПоддержкаПБУ18);
			...
			Если НЕ Реквизиты.ОтчетностьПодписана Тогда
				//Если увеличилась ставка налога и не подписана бухгалтерская отчетность, то будут сформированы 2 проводки на 41.К (по БУ и по НУ)
				//В этом случае в одной из проводок нужно очистить комичество по 41.К, иначе оно задвоится
		...
		Если НЕ Реквизиты.ОтчетностьПодписана
			ИЛИ Реквизиты.СтавкаНалогаУвеличилась Тогда
			...
			ЗаполнитьАналитикуСписанияТовара(Проводка, Реквизиты, СтрокаПартии, "Дт", 1, ВедетсяУчетПоПартиям, Реквизиты.СтавкаНалогаУвеличилась, ПоддержкаПБУ18);
			
			Проводка.СчетКт = СтрокаПартии.СчетУчета;
			ЗаполнитьАналитикуСписанияТовара(Проводка, Реквизиты, СтрокаПартии, "Кт", 1, ВедетсяУчетПоПартиям, Реквизиты.СтавкаНалогаУвеличилась, ПоддержкаПБУ18);
			
			Проводка.Сумма = ?(НЕ Реквизиты.ОтчетностьПодписана, СтрокаПартии.СуммаСписания, 0);
			
			Проводка.НеКорректироватьСтоимостьАвтоматически = СтрокаПартии.Себестоимость <> 0;
		КонецЕсли;
		
		Если Реквизиты.ОтчетностьПодписана 
			ИЛИ (НЕ Реквизиты.СтавкаНалогаУвеличилась) Тогда
			...
			СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетДт);
			ЗаполнитьАналитикуСписанияТовара(Проводка, Реквизиты, СтрокаПартии, "Дт", -1, ВедетсяУчетПоПартиям, НЕ Реквизиты.СтавкаНалогаУвеличилась, ПоддержкаПБУ18);
			...
			Если СвойстваСчетаДт.Забалансовый = СвойстваСчетаКт.Забалансовый Тогда
				...
				УстановитьПараметрыПроводки(Проводка, СвойстваСчетаКт, "Кт", СтрокаПартии.КорПодразделение, 0,
					?(Реквизиты.СтавкаНалогаУвеличилась, 0, -СтрокаПартии.СуммаКорСписанияНУ), 
					?(Реквизиты.СтавкаНалогаУвеличилась, 0, -СтрокаПартии.СуммаКорСписанияПР), 
					?(Реквизиты.СтавкаНалогаУвеличилась, 0, -СтрокаПартии.СуммаКорСписанияВР), 
					ОтражатьВНалоговомУчете, ПоддержкаПБУ18);
			КонецЕсли;
			
			Проводка.Сумма = ?(НЕ Реквизиты.ОтчетностьПодписана, 0, - СтрокаПартии.СуммаСписания);
			Проводка.КоличествоДт = ?(НЕ Реквизиты.ОтчетностьПодписана ИЛИ Реквизиты.СтавкаНалогаУвеличилась, 0, - СтрокаПартии.Количество);
			Проводка.НеКорректироватьСтоимостьАвтоматически = СтрокаПартии.Себестоимость <> 0;
		КонецЕсли;
...
Функция ПодготовитьПараметрыКорректировкаПоступлениеТоваров( ТаблицаВозвращенныеТовары, ТаблицаРеквизиты)
	...
	+ "СтавкаНалогаУвеличилась," // <Булево>
	+ "Регистратор,"       	 // <ДокументСсылка.*> - документ-регистратор движений
...
Процедура СформироватьДвиженияИзменениеСебестоимостиОстатковТоваров(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	НДСВключенВСтоимость = Не ПлательщикНДС Или (ОрганизацияОсвобожденаОтУплатыНДС И Не РаздельныйУчетНДС);
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТовары Цикл
		...
		Если НачислятьЕНП И (Не УчетНДС.НачислениеКосвенныхНалоговНаЕНСРегламентнымДокументом(ДобавитьМесяц(СтрокаТаблицы.ДатаПринятияНаУчет, 1))
			Или Не СтрокаТаблицы.ПрименяетсяДОПП) Тогда
			УчетНДСПереопределяемый.НачислитьНДСНаЕНППриВвозеТоваровИзЕАЭС(Реквизиты, СтрокаТаблицы, НачислениеНалоговНаЕНС);
		КонецЕсли;
...
Функция ПодготовитьПараметрыИзменениеСебестоимостиОстатковТоваров(ТаблицаТовары,
	...
	Если ЕстьКолонкаДатаПринятияНаУчет Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок
			...
			+ ",ПрименяетсяДОПП";    // <Булево> - признак применения ДОПП по текущей строке начисления НДС
	КонецЕсли;
			...
			Если Реквизиты.ОтражатьВНалоговомУчете 
				...
				Если СтрСуммаБУ < 0 
					...
					Если СвойстваСчетаДт.НалоговыйУчет Тогда
						Проводка.СуммаНУДт = 0;
						Если Реквизиты.ПоддержкаПБУ18 Тогда
							Проводка.СуммаПРДт = ?(СтрСуммаБУ < 0, 0, СтрСуммаБУ);
							Проводка.СуммаВРДт = 0;
						КонецЕсли;
					КонецЕсли;
					...
					Если СвойстваСчетаКт.НалоговыйУчет Тогда
						Проводка.СуммаНУКт = 0;
						Если Реквизиты.ПоддержкаПБУ18 Тогда
							Проводка.СуммаПРКт = ?(СтрСуммаБУ < 0,- СтрСуммаБУ, 0);
							Проводка.СуммаВРКт = 0;
						КонецЕсли;
					КонецЕсли;
