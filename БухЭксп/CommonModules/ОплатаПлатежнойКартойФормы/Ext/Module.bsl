...
Процедура УстановитьПараметрыВыбораВидаОплаты(Форма)
	...
	ТипыОплатБезналичныеОплаты.Добавить(Перечисления.ТипыОплат.ЦифровойРубль);
...
Процедура УстановитьУсловноеОформление(Форма) Экспорт
	...
	Для Каждого ВидОперацииОплатаПокупателя Из ОплатаПлатежнойКартойКлиентСервер.ВидыОперацийОплатаПокупателя() Цикл
		СписокОперацийОплатаПокупателя.Добавить(ВидОперацииОплатаПокупателя);
	КонецЦикла;
	...
	ДобавитьПолеВУсловноеОформление(СтрШаблон("%1СчетНаОплатуНПД", ИмяТабличнойЧасти),
		ЭлементУО, Элементы, УсловноеОформление);
	Если ЭлементУО <> Неопределено Тогда
	...
	ЭлементУО = Неопределено;
	ДобавитьПолеВУсловноеОформление(СтрШаблон("%1УслугаНПД", ИмяТабличнойЧасти),
		ЭлементУО, Элементы, УсловноеОформление);
	Если ЭлементУО <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
			ГруппаОтбораИ,
			СтрШаблон("%1ВидОперации", НачалоПутиКДаннымТЧ),
	...
	Если ЭлементУО <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
			ГруппаОтбораИ,
			СтрШаблон("%1%2.УслугаНПД", НачалоПутиКДаннымТЧ, ИмяТабличнойЧасти),
			ВидСравненияКомпоновкиДанных.НеЗаполнено);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
			ГруппаОтбораИ,
			СтрШаблон("%1%2.СчетНаОплату", НачалоПутиКДаннымТЧ, ИмяТабличнойЧасти),
