...
Функция ПараметрыФискальногоНакопителяОбщие() Экспорт
	...
	Параметры.Вставить("ИдентификаторМоделиККТ");  // Идентификатор модели ККТ
...
Функция ПараметрыВыполненияОперации() Экспорт;
	...
	ПараметрыОперации.Вставить("Кассир"     , НСтр("ru='Администратор'")); // Строка,Обязательно - ФИО лица и должность, осуществившего операцию.
...
Функция ПараметрыПроверкиКМПИоТ() Экспорт
	...
	Параметры.Вставить("ТоварнаяГруппа"); // Cтрока,Необязательно - Товарная группа   
	Возврат Параметры; 
...
Функция ПараметрыСтатусСостоянияПИоТ() Экспорт
	...
	Параметры.Вставить("АварийныйРежимДо"         ); // Дата,Обязательно - Дата действия аварийного режима
	...
	Параметры.Вставить("ЛМЧЗСтатусТекст");     // Строка,Необязательно - ЛМ ЧЗ Статуc в тестовом виде    
	...
	Параметры.Вставить("ТокенЛМЧЗ");              // Строка,Обязательно - Токен ЛМ ЧЗ
	Параметры.Вставить("ТокенЛМЧЗДатаДействия"); // Дата,Обязательно - Дата действия токена ЛМ ЧЗ
	Возврат Параметры; 
...
// Получить наименование статуса ЛМ ЧЗ.
// 
// Параметры:
//  КодСтатуса - Число.
// 
...
//  Строка - Наименование  
//
Функция НаименованиеСтатусаЛМЧЗ(КодСтатуса) Экспорт
	
	ТипыСтатуса = Новый Соответствие();
	ТипыСтатуса.Вставить(1, "Not connected");
	ТипыСтатуса.Вставить(2, "Connected but status unknown");
	ТипыСтатуса.Вставить(3, "Not configured");
	ТипыСтатуса.Вставить(4, "Initializing");
	ТипыСтатуса.Вставить(5, "Sync error");
	ТипыСтатуса.Вставить(6, "Ready");
	
	Возврат ТипыСтатуса.Получить(Число(КодСтатуса));
...
#Если ВебКлиент Тогда
	Параметры = ОбщегоНазначенияБПОСлужебныйВызовСервера.ПрочитатьПараметрыИзXML(ДанныеXML);
#Иначе
...
#КонецЕсли
	...
	Если ПараметрыСтатус.АварийныйРежим Тогда
		АварийныйРежимДо = ?(Параметры.Свойство("EmergencyModeDate"), Параметры.EmergencyModeDate, ""); 
		Если Не ПустаяСтрока(АварийныйРежимДо) Тогда
			ПараметрыСтатус.АварийныйРежимДо = ОбщегоНазначенияБПОКлиентСервер.ПреобразоватьДатуJSON(АварийныйРежимДо);
		КонецЕсли;
	КонецЕсли;
	...
	ЛМЧЗСтатус = ?(Параметры.Свойство("LMCZStatus"), Параметры.LMCZStatus, 0);         
	ПараметрыСтатус.ЛМЧЗСтатус = ЛМЧЗСтатус;   
	ПараметрыСтатус.ЛМЧЗСтатусТекст = НаименованиеСтатусаЛМЧЗ(ЛМЧЗСтатус);
	...
	ПараметрыСтатус.ТокенЛМЧЗ             = ?(Параметры.Свойство("LMCZToken"), Параметры.LMCZToken, "");         
	ТокенЛМЧЗДатаДействия = ?(Параметры.Свойство("LMCZTokenExpiredDate"), Параметры.LMCZTokenExpiredDate, ""); 
	Если Не ПустаяСтрока(ТокенЛМЧЗДатаДействия) Тогда
		ПараметрыСтатус.ТокенЛМЧЗДатаДействия = ОбщегоНазначенияБПОКлиентСервер.ПреобразоватьДатуJSON(ТокенЛМЧЗДатаДействия);
	КонецЕсли;
...
//
Функция ПолучитьПараметрыАутентификацииXML(ПараметрыОперации) Экспорт

	ПараметрыАутентификации = Новый Структура();    
	ПараметрыАутентификации.Вставить("DateMS"     , ПараметрыОперации.ВременнаяМетка);
	ПараметрыАутентификации.Вставить("CashierName", ПараметрыОперации.Кассир); 
	ПараметрыАутентификации.Вставить("CashierINN",  ПараметрыОперации.КассирИНН);
	ПараметрыАутентификации.Вставить("Success"   ,  ПараметрыОперации.Успешность);
		
	ПараметрыXML = МенеджерОборудованияВызовСервера.ПолучитьПараметрыXML(ПараметрыАутентификации, Неопределено, Ложь);    
	Возврат ПараметрыXML;
	
КонецФункции      

// Получить параметры события эксплуатации XML
//
Функция ПолучитьПараметрыСобытияЭксплуатацииXML(ПараметрыОперации) Экспорт
	
	ПараметрыСобытия = Новый Структура();    
	ПараметрыСобытия.Вставить("DateMS"     , ПараметрыОперации.ВременнаяМетка);
	ПараметрыСобытия.Вставить("EventName"  , ПараметрыОперации.ИмяСобытия); 
	ПараметрыСобытия.Вставить("KI"         , ПараметрыОперации.КодИдентификации);
	ПараметрыСобытия.Вставить("Status"     , ПараметрыОперации.Статус);
	ПараметрыСобытия.Вставить("UserMessage", ПараметрыОперации.СообщениеПользователю); 
	Если ПараметрыОперации.ИмяСобытия = "user_decision" Тогда                 
		// Принимает значение «true» в случае, если по совокупности проверок товар разрешен к продаже.
		ПараметрыСобытия.Вставить("Km_Chk_Ok", ПараметрыОперации.РазрешенКПродаже);                   
		// Принимает значение «true» в случае, если товар запрещенный к продаже был включен в чек.          
		ПараметрыСобытия.Вставить("Wrong_Op", НЕ ПараметрыОперации.РазрешенКПродаже);                   
	КонецЕсли;
	
	ПараметрыXML = МенеджерОборудованияВызовСервера.ПолучитьПараметрыXML(ПараметрыСобытия, Неопределено, Ложь);    
	Возврат ПараметрыXML;
