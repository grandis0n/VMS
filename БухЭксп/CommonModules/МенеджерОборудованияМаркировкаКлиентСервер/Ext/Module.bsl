...
Функция РазобратьШтриховойКодТовараСлужебная(Знач Штрихкод, Знач ШтрихкодВBase64 = Ложь, КодыGS1 = Неопределено) Экспорт;
	...
	Если ДанныеКодаТовара.ЕстьКриптоХвост Тогда
		...
		КодыКриптоХвоста = Новый Массив;
		КодыКриптоХвоста.Добавить("91");
		КодыКриптоХвоста.Добавить("92");
		КодыКриптоХвоста.Добавить("93");

		Для Каждого КодЧасти Из КодыКриптоХвоста Цикл
			...
			ФрагментКриптоХвоста = КодЧасти + ЭлементКода.Значение;
			ШтрихкодБезКриптоХвоста = СтрЗаменить(ШтрихкодБезКриптоХвоста,
				РазделительКода + ФрагментКриптоХвоста, "");
			ШтрихкодБезКриптоХвоста = СтрЗаменить(ШтрихкодБезКриптоХвоста,
				"(" + КодЧасти + ")" + ЭлементКода.Значение, "");

			Если Не ПустаяСтрока(КриптоХвост) Тогда
				КриптоХвост = КриптоХвост + РазделительКода;
			КонецЕсли;
			КриптоХвост = КриптоХвост + ФрагментКриптоХвоста;
		...
		Если ОписаниеКода.ПеременнаяДлина > 0 И Позиция < ДлинаШтрихкода Тогда
			...
			Пока ПозицияСледующегоИдентификатора > 0 И Не ПравильныйИдентификатор Цикл
				...
				Если ПозицияЗакрывающегоИдентификатора <> 0 Тогда
					...
					ДлинаПредполагаемогоИдентификатора = СтрДлина(ПредполагаемыйИдентификатор);
					ПравильныйИдентификатор = 
						ДлинаПредполагаемогоИдентификатора >= МинимальнаяДлинаИдентификатораПрименения
						И ДлинаПредполагаемогоИдентификатора <= МаксимальнаяДлинаИдентификатораПрименения
						И ТолькоЦифрыВСтроке(ПредполагаемыйИдентификатор)
						И ЭтоИзвестныйИдентификаторПримененияGS1(
							ПредполагаемыйИдентификатор,
							КодыGS1,
							МаксимальнаяДлинаИдентификатораПрименения);
				КонецЕсли;
...
Функция ЭтоИзвестныйИдентификаторПримененияGS1(
	ИдентификаторПрименения,
	КодыGS1,
	МаксимальнаяДлинаИдентификатораПрименения)

	Если КодыGS1[ИдентификаторПрименения] <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;

	Если СтрДлина(ИдентификаторПрименения) <> МаксимальнаяДлинаИдентификатораПрименения Тогда
		Возврат Ложь;
	КонецЕсли;

	ОписаниеКода = КодыGS1[Лев(ИдентификаторПрименения, МаксимальнаяДлинаИдентификатораПрименения - 1)];

	Возврат ОписаниеКода <> Неопределено И ОписаниеКода.ЕстьПоложениеДесятичнойТочки;
