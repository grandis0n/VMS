...
// Результат проверки операции подключения ЭДО.
// 
// Параметры:
//  ОперацияЭДО - Структура - Операция ЭДО
// 
// Возвращаемое значение:
//	Структура:
...
Функция РезультатПроверкиОперацииПодключенияЭДО(Знач ОперацияЭДО)
	...
	АктуальнаяВерсия =
		СервисНастроекЭДОВызовСервера.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(Параметры.ОператорЭДО);
	...
	Если ВключеноРазделениеДанных <> Истина Тогда
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Параметры.АдресОрганизации)
		...
		Если Параметры.СпособОбменаЭД <> ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском")
				И Не ЗначениеЗаполнено(Параметры.КодНалоговогоОргана)
				И СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальнаяВерсия, "5.03") < 0 Тогда
...
Функция СоздатьУчетнуюЗапись(ОперацияЭДО)
	...
	ОписаниеУчетнойЗаписи.НеИспользоватьПриглашения =
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОперацияЭДО.Параметры, "НеИспользоватьПриглашения", Ложь);
	ОписаниеУчетнойЗаписи.КодыТиповДокументовКОбмену = 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОперацияЭДО.Параметры, "КодыТиповДокументовКОбмену", Новый Массив);
	Если ЗначениеЗаполнено(ОперацияЭДО.Параметры.Сертификат) Тогда
		...
		ДолжностьПоСертификату = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОперацияЭДО.Параметры, "Должность", "");
		ОписаниеУчетнойЗаписи.Должности.Вставить(ОперацияЭДО.Параметры.Сертификат, ДолжностьПоСертификату);
	КонецЕсли;
...
Функция ВыполнитьПодключениеЭДО_1СЭДО_ОтправкаЗаявкиНаПодключение(ПроцессВыполнения)
	...
	Если ОтправкаЗаявки = Неопределено И Не ЗначениеЗаполнено(НомерЗаявки) Тогда
		...
		ПараметрыРегистрации.НеИспользоватьПриглашения      =
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "НеИспользоватьПриглашения",
				Не УчетныеЗаписиЭДОКлиентСервер.ТребоватьПриглашенияПередНачаломЭДОЗначениеПоУмолчанию());
		ПараметрыРегистрации.КодыТиповДокументовКОбмену = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "КодыТиповДокументовКОбмену");
		ПараметрыРегистрации.Комментарий = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "НазначениеУчетнойЗаписи", "");
		ПараметрыРегистрации.ВидУчетнойЗаписи = Параметры.ВидУчетнойЗаписи;
