...
//  КонтекстОтчета - см. БухгалтерскаяОтчетностьБРО.НовыйКонтекстЗаполненияОтчета
//  НастройкиОтчета - см. НовыйНастройкиЗаполненияТаблицОтчета
...
//
Процедура ЗаполнитьТаблицыОтчета(Контейнер, ОтборРазделов, КонтекстОтчета, НастройкиОтчета, Периоды, Организации) Экспорт
	...
	ЗаполнениеОтчета = НачатьЗаполнениеОтчета(Периоды, Организации, НастройкиОтчета, ТаблицыОтчетаКЗаполнению);
...
//
Функция ДанныеЗаполненияТаблицОтчета(Периоды, Организации, НастройкиОтчета, ТаблицыОтчетаКЗаполнению) Экспорт
...
Функция ПредставлениеСпискаСчетовПредложныйПадеж(ПредставлениеСписка) Экспорт
	
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	
	Шаблон = НСтр("ru = 'счете %1'", КодЯзыка);
	Если ПредставлениеСписка.Количество > 1 Тогда
		Шаблон = НСтр("ru = 'счетах %1'", КодЯзыка);
	КонецЕсли;
...
// Конструктор коллекции, описывающей настройки заполнения бухгалтерского баланса.
// 
// Возвращаемое значение:
//  Структура
//   * ЭтоБалансНекоммерческойОрганизации - Булево
//   * КлассифицироватьАктивыОбязательства - Булево
//   * РазделятьАвансыПоВидамАктивов - Булево
//
Функция НовыйНастройкиЗаполненияТаблицОтчета() Экспорт // за все периоды по всем таблицам
	
	Настройки = Новый Структура;

	Настройки.Вставить("ЭтоБалансНекоммерческойОрганизации", Ложь);
	Настройки.Вставить("КлассифицироватьАктивыОбязательства", Ложь);
	Настройки.Вставить("РазделятьАвансыПоВидамАктивов", Ложь);
...
Функция НовыйЗаполнениеОтчета() // за все периоды по всем таблицам
	...
	Процессор.Вставить("НастройкиОтчета",      НовыйНастройкиЗаполненияТаблицОтчета());
...
Функция НовыйПолучениеДанныхТаблицОтчета() // за один период по всем таблицам
	...
	Процессор.Вставить("НастройкиОтчета"); // см. НовыйНастройкиЗаполненияТаблицОтчета
...
Процедура ЗаполнитьОтчет(ЗаполнениеОтчета)
	...
	Для ИндексПериода = 0 По ЗаполнениеОтчета.Периоды.ВГраница() Цикл
		...
		ПолучениеДанныхТаблицОтчета.НастройкиОтчета          = ЗаполнениеОтчета.НастройкиОтчета;
...
Функция НачатьЗаполнениеОтчета(Периоды, Организации, НастройкиОтчета, ТаблицыОтчетаКЗаполнению)
	...
	ЗаполнитьЗначенияСвойств(ЗаполнениеОтчета.НастройкиОтчета, НастройкиОтчета);
