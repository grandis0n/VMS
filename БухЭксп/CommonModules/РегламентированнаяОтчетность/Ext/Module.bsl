...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	...
	ЭтоОтчетМинобороны = Реквизиты.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ"
		...
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма4д"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма5"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма5д"
...
Функция ВидКонтролирующегоОргана(Знач ИмяОтчета) Экспорт
	...
	Иначе
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ИсполнениеКонтрактовГОЗ") > 0 
			...
			Или СтрНайти(ИмяОтчета, "ГОЗФорма4д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма5") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма5д") > 0
...
#Область ИнтеграцияСОтчетность24
	Если ДекларацияНДС = Неопределено Тогда
		Возврат;
	КонецЕсли;
#КонецОбласти	
...
Функция АдресВJSON(КонтактнаяИнформация, ВидАдреса) Экспорт
	...
	Если Результат.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(Результат[0].Значение) Тогда 
