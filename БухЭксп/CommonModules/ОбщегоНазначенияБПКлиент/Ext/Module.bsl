...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ОбработчикУведомленияОСменеКомандногоИнтерфейса", ЭтотОбъект);
	УведомленияКлиента.ПодключитьОбработчик("ИзменилсяКомандныйИнтерфейс", Обработчик);
...
Процедура ОткрытьОбучение() Экспорт
	
	ОткрытьФорму("Обработка.ОбучениеРаботеВБизнесСтарт.Форма");
...
Асинх Процедура ОбработчикУведомленияОСменеКомандногоИнтерфейса(Данные, ДополнительныеПараметры) Экспорт
	
	ЗаголовокВопроса = НСтр("ru = 'Требуется перезапуск'");
	ТекстВопроса = НСтр("ru = 'Изменен командный интерфейс. Для корректной работы требуется перезапустить программу.
		|Перезапустить сейчас?'");
	Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60, , ЗаголовокВопроса, КодВозвратаДиалога.Да);
