...
Процедура ЗаполнитьСырьеЕГАИС(Источник,ДанныеЗаполнения, ИсключитьМаркируемые = Ложь)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК Фильтр
	|	ПО Фильтр.Номенклатура = ОтчетПроизводстваЗаСменуМатериалы.Номенклатура
	|	И Фильтр.КлючВидовПродукции.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
	|ГДЕ
