...
Процедура ОтразитьВУСН(ДокументСсылка, ВидОперации = Неопределено, Параметры = Неопределено, ЭтапПроведения = 0, СтруктураДопПараметров = Неопределено, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Если ВидОперации = "Поступление"
		...
		Иначе
			...
			Если СтруктураДопПараметров.Свойство("ТаблицаРасчетовАгентаПоставщика") Тогда
				ТаблицаОплатАгентаПоставщика = СтруктураДопПараметров.ТаблицаРасчетовАгентаПоставщика;
				Если ЗначениеЗаполнено(ТаблицаОплатАгентаПоставщика) Тогда
					РасчетыСКонтрагентами_ЗачетАвансаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаОплатАгентаПоставщика, ТаблицаПрихода, ДвиженияРегистров, Договор, СуммаВзаиморасчетов, (ЭтапПроведения > 0));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли (ВидОперации = "Списание")
		...
		ИначеЕсли ВидОперации = "ВозвратПоставщику" Тогда
			...
			КонецЕсли;
			Если Не БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомитентом(ВидДоговора)
				И Не БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомитентомНаЗакупку(ВидДоговора) Тогда
				ТаблицаОплат = СтруктураДопПараметров.ТаблицаРасчетов;
				Если ТаблицаОплат.Количество() > 0 Тогда
					РасчетыСКонтрагентамиУСН(
						СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, "Оплата");
				Иначе
	...
	ИначеЕсли ВидОперации = "ОтражениеДоходаОтРеализацииКомиссионером" Тогда
		...
		ДоходыВсего = (СуммаДоходов + СуммаНДС) - (СуммаВозврат + СуммаНДСВозврат);
		...
		Если ДоходыВсего >= 0 Тогда
			...
			Если ДоходыВсего > 0
				И СтруктураДопПараметров.Свойство("ДенежныеСредства")
		...
		Если ВидОперации = "ОплатаОтЭквайера" Тогда
			...
			Если ВыделятьНДССКомиссииБанка Тогда
				...
				Для Каждого СтрокаТаблицы Из ТаблицаОплат Цикл
					Если БухгалтерскийУчетПовтИсп.ЭтоСчетРасчетовСПоставщиками(СтрокаТаблицы.КорСчет) Тогда
	...
	ИначеЕсли (ВидОперации = "ПроведениеВзаимозачетаДебитор") Тогда
		...
		Если СтруктураДопПараметров.Свойство("ДолгПоставщику") Тогда
			ТаблицаОплат = СтруктураДопПараметров.ДолгПоставщику;
			Если ТаблицаОплат.Количество() > 0 Тогда
				ДоговорыКонтрагента = ТаблицаОплат.ВыгрузитьКолонку("ДоговорКонтрагента");
				ВидыДоговоров = Справочники.ДоговорыКонтрагентов.ВидыДоговоров(ДоговорыКонтрагента);
				Если ВидыДоговоров.Найти(Перечисления.ВидыДоговоровКонтрагентов.Прочее) <> Неопределено
					Или ВидыДоговоров.Найти(Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку) <> Неопределено Тогда
...
Процедура КорректировкаЗадолженностиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ВидДвижения, ВидОперации)
	...
	ДополнитьТаблицуПризнакамиДоговора(ТаблицаДвиженияВзаиморасчетов);
	...
	Для Каждого СтрокаПлатеж Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		Если ВидОперации = "ПереносЗадолженности" Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПлатеж[ИмяКолонкиКорДоговора])
				...
				И СтрокаПлатеж[ИмяКолонкиКорДоговора].Владелец = СтрокаПлатеж.ВладелецДоговора
				И СтрокаПлатеж[ИмяКолонкиКорДоговора].ВалютаВзаиморасчетов = СтрокаПлатеж.ВалютаДоговора Тогда
				Продолжить;
			КонецЕсли;
		...
		Если ВидОперации = "ПереносЗадолженности" Тогда
			Если ЗначениеЗаполнено(СтрокаПлатеж[ИмяКолонкиКорДоговора]) И
				...
				СтрокаПлатеж[ИмяКолонкиКорДоговора].Владелец = СтрокаПлатеж.ВладелецДоговора Тогда
...
Процедура РасчетыСКонтрагентамиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, ВидОперации = "Оплата", ТаблицаНДСПродажи = Неопределено)
	...
	МетаданныеДокументаОплаты = Документ.Метаданные();
	
	Если МетаданныеДокументаОплаты.ТабличныеЧасти.Найти(ИмяРеквизитаРасшифровкаПлатежа) <> Неопределено Тогда
	...
	Для Каждого СтрокаПлатеж Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		ИначеЕсли ВидОперации = "ВозвратПокупателю" Тогда
			...
			Если НЕ ТаблицаОплат.Колонки.Найти("ДоговорКонтрагента") = Неопределено
				И ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента)
				И СтрокаПлатеж.ЭтоВидДоговораСКомитентомНаЗакупку
		...
		ИначеЕсли ВидОперации = "Доход" Тогда
			...
			Если СтрокаПлатеж.ЭтоВидДоговораСКомитентомНаЗакупку И ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент)
			...
			Если ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				...
				Если НалоговыйУчетПовтИсп.ЭтоОплатаЗаПериодБезПатентаПриПримененииТолькоПатента(
						СтруктураШапкиДокумента.Организация, СтрокаПлатеж.ДатаДокументаРасчетов, СтруктураШапкиДокумента.Дата) Тогда
...
Функция ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, Сумма = 0, ВидОперации = "Оплата", КонтролироватьСумму = Истина, ДокументОплаты = Неопределено, ВходящаяСтруктура = Неопределено)
	...
	ВидДокумента = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураШапкиДокумента, "ВидДокумента");
	ЭтоТребованиеНакладная = ВидДокумента = "ТребованиеНакладная";
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		ЭтоМатериал = ЗначениеЗаполнено(Расход.СчетУчета)
			И Расход.СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.Материалы);
		
		ПризнаватьРасход = Ложь;
		НовыйСтатусРасхода = Расход.ОтражениеВУСН;
		...
		Если РаспределятьРасходыПоСпособамУчетаНДС Тогда
			...
			Если НДСВПартии > 0 И НДСРасход > 0 Тогда
				Для Каждого СтрокаСпособаУчетаНДС Из НайденныеСтрокиСпособУчетаНДС Цикл
					...
					Если НДСРасход <= СтрокаСпособаУчетаНДС.НДС * КоэффициентРасхода Тогда
						СтрокаСпособаУчетаНДС.НДС = СтрокаСпособаУчетаНДС.НДС - НДСРасход;
						НДСРасход = 0;
					Иначе
		...
		// Обработаем изменение способа учета НДС
		ОбработатьИзменениеСпособаУчетаНДС = ЗначениеЗаполнено(СписанныеПартииНДС)
			И (ЭтоТребованиеНакладная И ЭтоМатериал)
			И Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ТаможенныеПлатежи
			И Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НДС_ТС
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.ПустаяСсылка()
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.НеПринимаются
			И (ПризнаватьРасход Или Не ЗначениеЗаполнено(НовыйСтатусОплаты))
			И НовыйСтатусРасхода <> Перечисления.ОтражениеВУСН.НеПринимаются;
...
Процедура УчестьНДСВДоходахПриКорректировкеРеализации(СтруктураШапкиДокумента, КУДиР, РеквизитыДокумента, ТаблицаРасчетов)
	...
	Если ТипЗнч(СтруктураШапкиДокумента.Объект) = Тип("Структура") Тогда
		СписокРеквизитов =
			"Товары, Услуги, ДокументРеализации, СчетУчетаРасчетовСКонтрагентом, Контрагент, ДоговорКонтрагента";
		Объект = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураШапкиДокумента.Ссылка, СписокРеквизитов, Истина);
	Иначе
		Объект = СтруктураШапкиДокумента.Объект;
	КонецЕсли;
	
	// В качестве документа реализации может быть корректировка, необходимо найти первичную реализацию
	Реализация = Объект.ДокументРеализации;
	Пока ТипЗнч(Реализация) = Тип("ДокументСсылка.КорректировкаРеализации") Цикл
		Реализация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реализация, "ДокументРеализации");
	КонецЦикла;
	
	// При отгрузке без перехода права собственности, доходы не корректируются
	Если ТипЗнч(Реализация) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		ВидОперацииРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реализация, "ВидОперации");
	...
	СчетаТоваровКомитентов = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссию);
	Для Каждого СтрокаТовар Из Объект.Товары.Выгрузить() Цикл
		Если СчетаТоваровКомитентов.Найти(СтрокаТовар.СчетУчета) = Неопределено Тогда
		...
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаУслуга Из Объект.Услуги.Выгрузить() Цикл
		КоэффициентНДСДоИзменения =
	...
	Если СуммаКорректировкиДохода <> 0 Тогда
		...
		ЗаполнитьЗначенияСвойств(СтрокаДанныхРасшифровки, Объект);
		...
		СтрокаДанныхРасшифровки.СчетРасчетов = Объект.СчетУчетаРасчетовСКонтрагентом;
		...
		СтрокаРеализации = ТаблицаВзаиморасчеты.Найти(Объект.ДокументРеализации);
		СуммаПоРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументРеализации, "СуммаДокумента");
		...
		Если РасчетыВВалюте Тогда
			ДокументРеализацииСсылка = Объект.ДокументРеализации;
		...
		Если СуммаКорректировкиДохода <> 0 Тогда
			...
			// Добавляем расшифровку по договорам.
			СтрокаРасшифровки = КУДиР.РасшифровкаПоДоговорам.Добавить();
			СтрокаРасшифровки.Контрагент = Объект.Контрагент;
...
Процедура ДополнитьТаблицуПризнакамиДоговора(ТаблицаДвижений)
	
	ЗаполнятьТаблицу = Ложь;
	
	Если ТаблицаДвижений.Колонки.Найти("ЭтоВидДоговораСКомитентомНаЗакупку") = Неопределено Тогда
		ТаблицаДвижений.Колонки.Добавить("ЭтоВидДоговораСКомитентомНаЗакупку", Новый ОписаниеТипов("Булево"));
		ЗаполнятьТаблицу = Истина;
	КонецЕсли;
	Если ТаблицаДвижений.Колонки.Найти("ВладелецДоговора") = Неопределено Тогда
		ТаблицаДвижений.Колонки.Добавить("ВладелецДоговора", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
		ЗаполнятьТаблицу = Истина;
	КонецЕсли;
	Если ТаблицаДвижений.Колонки.Найти("ВалютаДоговора") = Неопределено Тогда
		ТаблицаДвижений.Колонки.Добавить("ВалютаДоговора", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
		ЗаполнятьТаблицу = Истина;
	КонецЕсли;
	
	Если Не ЗаполнятьТаблицу Тогда
		Возврат;
	КонецЕсли;
	
	Договоры = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДвижений, "ДоговорКонтрагента", Истина);
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДоговорыКонтрагентов.Ссылка КАК ДоговорКонтрагента,
		|	ДоговорыКонтрагентов.Владелец КАК ВладелецДоговора,
		|	ДоговорыКонтрагентов.ВалютаВзаиморасчетов КАК ВалютаДоговора,
		|	ДоговорыКонтрагентов.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку) КАК ЭтоВидДоговораСКомитентомНаЗакупку
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		СтрокиТаблицы = ТаблицаДвижений.НайтиСтроки(Новый Структура("ДоговорКонтрагента", Выборка.ДоговорКонтрагента));
