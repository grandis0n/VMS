...
Функция НовыйРезультатФормированияПриглашений() Экспорт
	Результат = Новый Структура;
	Результат.Вставить("Обработано", 0);
	Результат.Вставить("КонтекстДиагностики", Новый Структура);
	Результат.Вставить("СтатусыПриглашений", Новый Массив);
	Возврат Результат;
КонецФункции
...
// Формирует приглашения в центре ЭДО для контрагентов и возвращает результат действия.
//
// Параметры:
//  Приглашения - Массив из см. ПриглашенияЭДОКлиентСервер.НовоеИсходящееПриглашение
//
// Возвращаемое значение:
//  см. НовыйРезультатФормированияПриглашений
Функция СформироватьПриглашенияВЦентреЭДО(Приглашения) Экспорт
	
	Результат = НовыйРезультатФормированияПриглашений();
	Результат.КонтекстДиагностики = ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики();
	Результат.Обработано = 0;
	
	РезультатыДействияСформироватьПриглашения = Новый Массив();
	ИмяМетодаФормированияПриглашений = "СформироватьПриглашенияЭДО";
	
	ДанныеПриглашений = ДанныеПриглашенийДляОтправки(Приглашения);
	ПараметрыДействийПоУчетнымЗаписям = ПараметрыДействияСформироватьПриглашенияПоУчетнымЗаписямЭДО(ДанныеПриглашений,
		Результат.КонтекстДиагностики);
		
	ТекущийПользователь = Пользователи.ТекущийПользователь();
	
	Для Каждого ПараметрыДействияПоУчетнойЗаписи Из ПараметрыДействийПоУчетнымЗаписям Цикл
		...
		РезультатФормирования = ИнтеграцияЦентраЭДО.ВыполнитьМетодСервиса(ПараметрыВыполнения, ОписаниеМетода,
			Результат.КонтекстДиагностики); // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.СформироватьПриглашенияЭДО
		
		Если ЗначениеЗаполнено(РезультатФормирования) Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РезультатыДействияСформироватьПриглашения, 
	...
	Результат.Обработано = ОбновитьПриглашенияПоРезультатамДействия(Приглашения,
		РезультатыДействияСформироватьПриглашения);
		
	Результат.СтатусыПриглашений = 
		СтатусыПриглашенийИзФорматаСервиса(РезультатыДействияСформироватьПриглашения);
...
Функция ПараметрыДействияСформироватьПриглашенияПоУчетнымЗаписямЭДО(ДанныеПриглашений, КонтекстДиагностики)
	
	ВидОперации = "СформироватьПриглашенияЭДО";
	...
	УчетныеЗаписиЦентровЭДО = 
		РегистрыСведений.НастройкиИнтеграцииЦентраЭДО.УчетныеЗаписиПоИдентификаторамЭДО(ИдентификаторыЭДО);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьПриглашенияВЦентреЭДО", Истина);
