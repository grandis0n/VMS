...
Функция ДанныеПлатежаДокумента(ДанныеФайла, ВидКНД, ПараметрыОбъекта)
	...
	Попытка
		Если ВидКНД = "1160001" Тогда // Требование ФНС
			...
			Если ПустаяСтрока(БлокПоиска) Тогда
				ОкончаниеБлока = "настоящеетребованиенебудетисполнено";
				БлокПоиска = ТекстБлока(ВесьТекст, НачалоБлока, ОкончаниеБлока);
			КонецЕсли;
			...
			Если СтрНайти(Результат.ИНН, "КПП") > 0 Тогда
				
				МассивИННКПП = СтрРазделить(Результат.ИНН, "КПП", Ложь);
				
				Если МассивИННКПП.Количество() > 0 Тогда
					
					Результат.ИНН = МассивИННКПП[0];
					Если МассивИННКПП.Количество() = 2 Тогда
						Результат.КПП = МассивИННКПП[1];
					КонецЕсли; 
			...
			Если Третий = "1" Тогда
				Третий = ПолучитьЗначениеВыражения(ТекстБлока(БлокПоиска, Второй, "года"), "1");
			КонецЕсли; 
