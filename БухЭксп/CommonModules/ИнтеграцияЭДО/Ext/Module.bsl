...
//
// Параметры:
//   РеквизитыКонтрагента - см. ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента
//
//  Возвращаемое значение:
...
//   * ИНН - Строка
//   * КПП - Строка
//   * ЮрФизЛицо - Произвольный - Вид организации "ЮЛ", "ИП", "ФЛ"
Функция НовыйОтборОрганизаций() Экспорт
	
	Отбор = Новый Структура;
	Отбор.Вставить("ИНН", "");
	Отбор.Вставить("КПП", "");
	Отбор.Вставить("ЮрФизЛицо");
...
Функция ЗапросДоступныхОрганизаций(ИмяВременнойТаблицы, Отбор = Неопределено) Экспорт
	...
	ИмяРеквизитаИНН = ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННОрганизации");
	ИмяРеквизитаКПП = ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППОрганизации");
	ИмяРеквизитаЮрФизЛицо = ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЮрФизЛицоОрганизации");
	
	ПоляУсловия = Новый Массив;
	Если ЗначениеЗаполнено(Отбор.ИНН) Тогда
		ПоляУсловия.Добавить(СтрШаблон("Организации.%1 = %2", ИмяРеквизитаИНН, Отбор.ИНН));
	КонецЕсли;
	Если ЗначениеЗаполнено(Отбор.КПП) Тогда
		ПоляУсловия.Добавить(СтрШаблон("Организации.%1 = %2", ИмяРеквизитаКПП, Отбор.КПП));
	КонецЕсли;
	Если ЗначениеЗаполнено(Отбор.ЮрФизЛицо)
			И ЗначениеЗаполнено(ИмяРеквизитаЮрФизЛицо) Тогда
		ПоляУсловия.Добавить(СтрШаблон("Организации.%1 = %2", ИмяРеквизитаЮрФизЛицо, Отбор.ЮрФизЛицо));
	КонецЕсли;
...
// Параметры:
//  Вид - Строка - ЮЛ, ИП, ФЛ
//  КлючевыеРеквизиты - См. НовыеКлючевыеРеквизитыУчастникаЭДО
// 
// Возвращаемое значение:
//  Неопределено,ОпределяемыйТип.Организация
Функция ОрганизацияПоВидуИКлючевымРеквизитам(Вид, КлючевыеРеквизиты) Экспорт
	
	Организация = Неопределено;
	
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить("ЮрФизЛицо", Вид);
	ПараметрыПоиска.Вставить("ИНН", КлючевыеРеквизиты.ИНН);
	
	Если ЗначениеЗаполнено(КлючевыеРеквизиты.КПП) Тогда
		ПараметрыПоиска.Вставить("КПП", КлючевыеРеквизиты.КПП);
	КонецЕсли;
	
	ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект("Организации", Организация,, ПараметрыПоиска);
...
// Возвращаемое значение:
//  Булево
Функция ЭтоФизЛицо(ПроверяемоеЛицо) Экспорт
	
	ТипУчастникаЭДО = ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(ПроверяемоеЛицо);
	
	Возврат ТипУчастникаЭДО.ЭтоФизЛицо Или ТипУчастникаЭДО.ЭтоИП;
...
Функция ТипУчастникаЭДО(ПроверяемоеЛицо) Экспорт
	...
	ОписаниеТипаУчастник = Метаданные.ОпределяемыеТипы.УчастникЭДО.Тип;
	Если ЗначениеЗаполнено(ПроверяемоеЛицо)
			И ОписаниеТипаУчастник.СодержитТип(ТипЗнч(ПроверяемоеЛицо)) Тогда
...
//  ЮрФизЛицо - Строка - "", ЮЛ, ИП, ФЛ
//  
// Возвращаемое значение:
//  СправочникСсылка - ссылка на объект.
Функция СсылкаНаОбъектПоИННКПП(ТипОбъекта, ИНН, КПП, ДатаСведений = Неопределено, ЮрФизЛицо = "") Экспорт
	
	СсылкаНаОбъект = Неопределено;
	ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП(
		ТипОбъекта, ИНН, КПП, СсылкаНаОбъект, ДатаСведений, ЮрФизЛицо);
...
Функция НайтиКонтрагентаПоРеквизитам(ТипОбъекта, РеквизитыКонтрагента, ДатаСведений = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтрЗаменить(Реквизиты.ИНН, "0", ""))
			И Не ЗначениеЗаполнено(РеквизитыКонтрагента.ВидКонтрагента) Тогда
		Возврат СсылкаНаОбъектПоИННКПП(ТипОбъекта, Реквизиты.ИНН, Реквизиты.КПП, ДатаСведений);
	КонецЕсли;
	
	Контрагент = Неопределено;
	РеквизитыКонтрагентаДляПоиска = Новый Структура;
	РеквизитыКонтрагентаДляПоиска.Вставить("ИНН", Реквизиты.ИНН);
	РеквизитыКонтрагентаДляПоиска.Вставить("КПП", Реквизиты.КПП);
	РеквизитыКонтрагентаДляПоиска.Вставить("ВидКонтрагента", Реквизиты.ВидКонтрагента);
	РеквизитыКонтрагентаДляПоиска.Вставить("GLN", Реквизиты.GLN);
	Если ЗначениеЗаполнено(Реквизиты.Наименование) Тогда
		РеквизитыКонтрагентаДляПоиска.Вставить("Наименование", Реквизиты.Наименование);
	КонецЕсли;
	ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект(ТипОбъекта, Контрагент,,
		РеквизитыКонтрагентаДляПоиска);
	Возврат Контрагент;
...
// 
// Параметры:
//  Реквизиты - см. ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента
// 
// Возвращаемое значение:
//  см. ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента
//
Функция РеквизитыКонтрагента(Реквизиты)
	
	Результат = ОбменСКонтрагентамиКлиентСервер.НовыйДанныеКонтрагента();
...
Функция ПредставлениеРегиона(Наименование, ТипРегиона, КодРегиона)
	...
	ИначеЕсли КодРегиона = "94" Или КодРегиона = "93" Тогда
		Возврат СоединитьНаименованиеИТипРегиона(Наименование, "Республика", Истина);
