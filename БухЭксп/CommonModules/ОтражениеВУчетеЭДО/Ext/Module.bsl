...
Процедура ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если МодульОбменСГИСЭПД <> Неопределено Тогда
		НеОтражатьДанныеЭлектронногоДокумента = Ложь;
		МодульОбменСГИСЭПД.ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, НеОтражатьДанныеЭлектронногоДокумента);
		Если НеОтражатьДанныеЭлектронногоДокумента Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьДеревоДаннымиКомитента(ДеревоДанных, ДанныеЭлектронногоДокумента)
	...
	ТипУчастникаЭДО = ИнтеграцияЭДО.ТипУчастникаЭДО(Получатель);
	
	Если ТипУчастникаЭДО.ЭтоФизЛицо Тогда
		
		ПутьУчастникФИО = ВидУчастника + ".ТипУчастника.ФЛ.";
		ПоддерживаемыеФорматы = ФорматыЭДО.ПоддерживаемыеФорматы();
