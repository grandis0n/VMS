...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	НастроитьСтатусРасчетаНДФЛАУСН(Форма, Форма.Объект);
...
Процедура ПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	НастроитьСтатусРасчетаНДФЛАУСН(Форма, ТекущийОбъект);
...
Процедура НастроитьСтатусРасчетаНДФЛАУСН(Форма, Объект)
	
	Элементы = Форма.Элементы;
	
	Если СписаниеСРасчетногоСчетаФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма)
		Или БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма)
		Или Не Форма.ПрименяетсяАУСН
		Или Не Документы.СписаниеСРасчетногоСчета.ЭтоОперацияРасчетаНДФЛАУСН(Объект) Тогда
		Возврат;
	КонецЕсли;
	
	СтатусыРасчетовНДФЛ = Перечисления.СтатусыРасчетовНДФЛАУСН;
	СтатусОбменаАУСН = СтатусыРасчетовНДФЛ.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ДанныеОбменаАУСН = РегистрыСведений.ДокументыНДФЛАУСН.ДанныеДокумента(Объект.Ссылка);
		Если ДанныеОбменаАУСН <> Неопределено Тогда
			СтатусОбменаАУСН = ДанныеОбменаАУСН.Статус;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СтатусОбменаАУСН)
		Или СтатусОбменаАУСН = СтатусыРасчетовНДФЛ.ОжидаетОтправки Тогда
		ЦветТекста = Новый Цвет;
	ИначеЕсли СтатусОбменаАУСН = СтатусыРасчетовНДФЛ.ОшибкаПриОтправке
		Или СтатусОбменаАУСН = СтатусыРасчетовНДФЛ.Отклонен Тогда
		ЦветТекста = ЦветаСтиля.ЦветТекстаОтрицательногоЗначения;
	Иначе
		ЦветТекста = ЦветаСтиля.ЦветТекстаПоложительногоЗначения;
	КонецЕсли;
	
	Форма.СтатусРасчетаНДФЛАУСН = СтатусыРасчетовНДФЛ.ПредставлениеСтатуса(СтатусОбменаАУСН, Объект.Проведен);
	Элементы.СтатусРасчетаНДФЛАУСН.ЦветТекста = ЦветТекста;
...
Процедура УправлениеЭлементамиОтраженияВУСНСервер(Форма) Экспорт
	...
	Если Форма.ПрименяетсяАУСНДоходы Тогда
		УстановитьВидимостьРазметкиАУСНДоходы(Форма, ЭтоОднострочнаяФорма, Объект.БезЗакрывающихДокументов);
	КонецЕсли;
	
	Если Элементы.Найти("СтатусРасчетаНДФЛАУСН") <> Неопределено Тогда
		Элементы.СтатусРасчетаНДФЛАУСН.Видимость = ПрименяетсяАУСН
			И Документы.СписаниеСРасчетногоСчета.ЭтоОперацияРасчетаНДФЛАУСН(Объект);
	КонецЕсли;
