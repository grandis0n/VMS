...
Процедура СведенияКонтрагентаПоИНН_КПП(РеквизитыКонтрагента, СведенияОКонтрагенте) Экспорт
	...
	СведенияОКонтрагенте.Значение = Справочники.Контрагенты.ПустаяСсылка();
	
	ИНН = "";
	РеквизитыКонтрагента.Свойство("ИНН", ИНН);
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		Возврат;
	КонецЕсли;
	КПП = "";
	РеквизитыКонтрагента.Свойство("КПП", КПП);
	Если Не ЗначениеЗаполнено(КПП) Тогда
		КПП = "";
	КонецЕсли;

	РезультатПоиска = НайтиКонтрагентаДляСервисаСверки(ИНН, КПП);
	СведенияОКонтрагенте.Значение = РезультатПоиска.Значение;

	Если РезультатПоиска.КоличествоКонтрагентов > 1
		И РезультатПоиска.КоличествоКонтрагентовСДокументами <> 1 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось однозначно сопоставить контрагента по ИНН %1 и КПП %2. Найдено контрагентов: %3, из них с документами для сверки: %4. Выбран контрагент: %5.'"),
			ИНН,
			КПП,
			РезультатПоиска.КоличествоКонтрагентов,
			РезультатПоиска.КоличествоКонтрагентовСДокументами,
			СведенияОКонтрагенте.Значение);
	...
	Если Не ЗначениеЗаполнено(СведенияОКонтрагенте.Значение) Тогда
		СведенияОКонтрагенте.КППКонтрагента = КПП;
		...
		Иначе
			СведенияОКонтрагенте.ИННКонтрагента = ИНН;
		КонецЕсли;
	КонецЕсли;
...
// Ищет контрагента по связке ИНН/КПП с учетом документов, зарегистрированных для сверки.
//
// Параметры:
...
// Возвращаемое значение:
//  Структура:
//    Значение - СправочникСсылка.Контрагенты - найденный контрагент;
//    КоличествоКонтрагентов - Число - количество найденных контрагентов;
//    КоличествоКонтрагентовСДокументами - Число - количество контрагентов с документами в реестре.
//
Функция НайтиКонтрагентаДляСервисаСверки(ИНН, КПП)

	Результат = Новый Структура;
	Результат.Вставить("Значение", Справочники.Контрагенты.ПустаяСсылка());
	Результат.Вставить("КоличествоКонтрагентов", 0);
	Результат.Вставить("КоличествоКонтрагентовСДокументами", 0);
	...
	КонецЦикла;
	Результат.КоличествоКонтрагентов = Контрагенты.Количество();
	Если Результат.КоличествоКонтрагентов = 0 Тогда
		Возврат Результат;
	КонецЕсли;

	Результат.Значение = Контрагенты[0];
	Если Результат.КоличествоКонтрагентов = 1 Тогда
		Возврат Результат;
	КонецЕсли;
	
	// Значит есть дубли, найдем наиболее подходящего контрагента.
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СервисСверкиРасчетовРеестрДокументов.Контрагент КАК Контрагент,
	|	КОЛИЧЕСТВО(СервисСверкиРасчетовРеестрДокументов.Документ) КАК КоличествоДокументов,
	|	МАКСИМУМ(СервисСверкиРасчетовРеестрДокументов.ДатаДокумента) КАК ДатаПоследнегоДокумента
	|ИЗ
	|	РегистрСведений.СервисСверкиРасчетовРеестрДокументов КАК СервисСверкиРасчетовРеестрДокументов
	|ГДЕ
	|	СервисСверкиРасчетовРеестрДокументов.Контрагент В(&Контрагенты)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.КоличествоКонтрагентовСДокументами = Результат.КоличествоКонтрагентовСДокументами + 1;
		Если Результат.КоличествоКонтрагентовСДокументами = 1 Тогда
			Результат.Значение = Выборка.Контрагент;
		КонецЕсли;
	КонецЦикла;
