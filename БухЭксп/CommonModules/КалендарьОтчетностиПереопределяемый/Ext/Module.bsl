...
Процедура КалендарьОтчетностиПриСозданииНаСервере(Форма, Параметры, Отказ, СтандартнаяОбработка) Экспорт
	...
	Форма.Организация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "Организация");
...
Процедура ЗаполнитьДанныеПлатежей(ДанныеПлатежей, Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СчетчикСтрокой = Формат(Счетчик, "ЧГ=0");
		ТекстЗапроса = ТекстЗапроса + СтрЗаменить(ТекстШаблонЗапроса, "%1", СчетчикСтрокой);
		
		Запрос.УстановитьПараметр("Организация" + СчетчикСтрокой, Выборка.Организация);
		Запрос.УстановитьПараметр("Правило" + СчетчикСтрокой,     Выборка.Правило);
		Запрос.УстановитьПараметр("Срок" + СчетчикСтрокой,        Выборка.Срок);
		Запрос.УстановитьПараметр("ДатаНачала"+ СчетчикСтрокой,   НачалоДня(Выборка.Срок));
		Запрос.УстановитьПараметр("ДатаКонца" + СчетчикСтрокой,   КонецДня(Выборка.Срок));
		Запрос.УстановитьПараметр("СчетУчета" + СчетчикСтрокой,   Выборка.СчетУчета);
