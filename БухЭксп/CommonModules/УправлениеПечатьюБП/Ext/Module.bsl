...
Функция НастройкиПодписиИПечати(ИмяМакета = "") Экспорт
	...
	Если НастройкиВыводаФаксимиле <> Неопределено Тогда
		...
		Попытка
			ИмяМакетаДляСтруктуры = ИмяМакетаДляХранения(ИмяМакета);
			НастройкиВыводаФаксимиле.Свойство("ВыводитьФаксимиле" + ИмяМакетаДляСтруктуры, ВыводитьФаксимиле);
		Исключение
		КонецПопытки;
...
Процедура СохранитьНастройкиПодписиИПечати(ВыводитьФаксимиле, ИмяМакета = "") Экспорт
	...
	Если ВыводитьФаксимиле <> Неопределено
		...
		Если ЗначениеЗаполнено(ИмяМакетаДляСтруктуры)
			И ОбщегоНазначенияКлиентСервер.ИмяСоответствуетТребованиямИменованияСвойств(ИмяМакетаДляСтруктуры) Тогда
			НастройкиВыводаФаксимиле.Вставить("ВыводитьФаксимиле" + ИмяМакетаДляСтруктуры, ВыводитьФаксимиле);
		КонецЕсли;
...
Функция ИмяМакетаДляХранения(ИмяМакета)
	
	// Для пользовательских печатных форм, которые добавляются макетом,
	// имя макета содержит ГУИД. Например "ПФ_e26b0b34-eea1-4de4-aa2d-534217d32283"
	// Такое имя нельзя использовать для ключа структуры.
	// Поэтому заменим - его на допустимый символ _.
	Возврат СтрЗаменить(ИмяМакета, "-", "_");
