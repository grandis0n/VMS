...
Процедура ЗаполнитьТабличнуюЧастьКорректировочногоОформленияСДИЗЗЕРНО(ДокументОбъект, ДанныеЗаполнения, ИмяДокумента, ЕстьСерии = Ложь)
	...
	ОсобенностьУчета = ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(ДокументОбъект.РазделУчета);
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ИмяФильтра") И ДанныеЗаполнения.ИмяФильтра = "ЗаполнениеСВидомПродукции" Тогда
			ФильтрВидПродукции 			= ДанныеЗаполнения.ЗначениеФильтра;
			ДокументОбъект.РазделУчета 	= ФильтрВидПродукции;
			ОсобенностьУчета 			= ИнтеграцияЗЕРНОКлиентСервер.ВидыПродукции(ФильтрВидПродукции);
		КонецЕсли;
...
Функция ТекстЗапросаЗаполненияКорректировочногоОформленияСДИЗИзПрикладногоДокумента(ИмяДокумента)
	...
	|ИЗ
	|	Документ.ИмяПрикладногоДокумента.Товары КАК Товары
	|	НУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	|	Товары.Ссылка = &ДокументОснование
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
...
Функция ТекстЗапросаЗаполненияФормированиеПартииПриПроизводствеИзПрикладногоДокумента(ИмяДокумента, ЕстьСерии, ИмяТабличнойЧастиСырье = "Материалы")
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|   ПО Сырье.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
...
Процедура ЗаполнитьВидПродукцииПоТабличнойЧастиОснования(ДокументОбъект, ДокументОснование) Экспорт
	...
	|   ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО КАК ВидПродукцииИС
	|ИЗ
	|	Документ.%1.%2 КАК ТабличнаяЧастьДокумента
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|   ПО ТабличнаяЧастьДокумента.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
	...
	Если ТаблицаТоваров.Количество() > 0 Тогда
		ВидПродукции = ТаблицаТоваров[0].ВидПродукцииИС;
	Иначе
		ВидПродукции = Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВидПродукции) Тогда
		ДокументОбъект.ТоварнаяГруппа = ВидПродукции;
	КонецЕсли;
...
Функция УчитываемыеВидыПродукцииЗЕРНО() Экспорт
	...
	Возврат КонструкторИСОбщегоНазначенияКлиентСервер.ВидыПродукцииЗЕРНО();
