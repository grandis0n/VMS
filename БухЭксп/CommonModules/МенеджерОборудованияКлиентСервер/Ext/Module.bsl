...
//
// Параметры:
//  ПодключенноеОборудование - СправочникСсылка.ПодключаемоеОборудование, Неопределено -
//   подключенное оборудование, по которому определяется кассир.
//
// Возвращаемое значение:
//   Структура.
//   *Кассир - Строка, Неопределено - текст, используемый для заполнения документа
//   *КассирИНН - Строка, Неопределено - текст, используемый для заполнения документа
//
Функция ТекущийКассирДляФискальныхОпераций(ПодключенноеОборудование = Неопределено) Экспорт

	Кассир = "";
	ВыполненаСтандартнаяОбработка = Истина;

	МенеджерОборудованияКлиентСерверПереопределяемый.
		ОбработкаЗаполненияИмяКассираПоПодключенномуОборудованию(
			Кассир,
			ПодключенноеОборудование,
			ВыполненаСтандартнаяОбработка);

	Если ВыполненаСтандартнаяОбработка Или ПустаяСтрока(Кассир) Тогда
		ВыполненаСтандартнаяОбработка = Истина;
		МенеджерОборудованияКлиентСерверПереопределяемый.
			ОбработкаЗаполненияИмяКассира(Кассир, ВыполненаСтандартнаяОбработка);
	КонецЕсли;
	...
	МенеджерОборудованияКлиентСерверПереопределяемый.
		ОбработкаЗаполненияИННКассираПоПодключенномуОборудованию(
			КассирИНН,
			ПодключенноеОборудование,
			ВыполненаСтандартнаяОбработка);

	Если ВыполненаСтандартнаяОбработка Или ПустаяСтрока(КассирИНН) Тогда
		ВыполненаСтандартнаяОбработка = Истина;
		МенеджерОборудованияКлиентСерверПереопределяемый.
			ОбработкаЗаполненияИННКассира(КассирИНН, ВыполненаСтандартнаяОбработка);
	КонецЕсли;
...
//   *ОсновнойДрайверПустьГлавнойDLL - Строка -
...
Функция ПараметрыОписанияДрайвера() Экспорт
	...
	РевизииИнтерфейса = ОбщегоНазначенияБПОКлиентСервер.ПоддерживаемыеРевизииИнтерфейсаДрайверов();
	...
	Результат.Вставить("ОсновнойДрайверПустьГлавнойDLL"); 
	Результат.Вставить("РевизияИнтерфейса"        , РевизииИнтерфейса.Максимальная);
...
Функция ПараметрыПодключения(ДанныеДрайвера) Экспорт
	...
	Подключение.Вставить("СкидкаЭлектроннымиСертификатами", ?(ДанныеДрайвера.Свойство("СкидкаЭлектроннымиСертификатами"), ДанныеДрайвера.СкидкаЭлектроннымиСертификатами, Ложь));
	...
	Подключение.Вставить("ПолученияСтатусаОперации"       , ?(ДанныеДрайвера.Свойство("ПолученияСтатусаОперации"), ДанныеДрайвера.ПолученияСтатусаОперации, Ложь));  
	...
	Подключение.Вставить("ДисплейСостоянияОпераций", Новый Соответствие);
	
	Подключение.Вставить("УстройствоРаспознаванияПолучениеИзображения"   , ?(ДанныеДрайвера.Свойство("УстройствоРаспознаванияПолучениеИзображения"), ДанныеДрайвера.УстройствоРаспознаванияПолучениеИзображения, Ложь));
	Подключение.Вставить("УстройствоРаспознаванияАппаратноеРаспознавание", ?(ДанныеДрайвера.Свойство("УстройствоРаспознаванияАппаратноеРаспознавание"), ДанныеДрайвера.УстройствоРаспознаванияАппаратноеРаспознавание, Ложь));
...
// ++ Локализация
	Подключение.Вставить("ИспользоватьТСПИоТ", ?(ДанныеДрайвера.Свойство("ИспользоватьТСПИоТ"), ДанныеДрайвера.ИспользоватьТСПИоТ, Ложь)); 
// -- Локализация
	Возврат Подключение;
...
// Преобразовывает число HEX строку.
//
// Параметры:
//   ЗначенияЧисла - Число - Тестовая строка
// 
// Возвращаемое значение:
//   - Строка - Строка HEX содержащая символы.  
//
Функция ЧислоВСтрокуHEX(Знач ЗначенияЧисла) Экспорт
	
	Результат = "";
	Пока ЗначенияЧисла <> 0 Цикл
		Позиция = ЗначенияЧисла % 16;
		Результат = Сред("0123456789ABCDEF", Позиция + 1, 1) + Результат;
		ЗначенияЧисла = Цел(ЗначенияЧисла / 16);
	КонецЦикла;         
...
Функция НастройкиСоединенияПроверкиСтатусаMAX() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("Хост", "ext-api2.max.ru");
	Результат.Вставить("Порт", 443);
	Результат.Вставить("Таймаут", 5);
	Результат.Вставить("АдресРесурса", "/esia/age-verification?session_id=%1");
