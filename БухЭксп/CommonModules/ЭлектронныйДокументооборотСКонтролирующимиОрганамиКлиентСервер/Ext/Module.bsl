...
Функция ОбновитьКоличествоНовых(
	...
	Результат.Вставить("КоличествоНепрочитанныхЕГРЮЛ", 0);
	Результат.Вставить("КоличествоЕГРЮЛ", 0);
	...
	Для каждого СтрокиДереваМассива Из МассивДеревьев Цикл
		Для каждого СтрокаДерева Из СтрокиДереваМассива Цикл
			...
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокЕГРЮЛ = СтрокаДерева;
				Иначе
					Результат.КоличествоЕГРЮЛ = Результат.КоличествоЕГРЮЛ + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхЕГРЮЛ = Результат.КоличествоНепрочитанныхЕГРЮЛ + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
	...
	Результат.КоличествоНовых = Результат.КоличествоПолученныеСообщения + Результат.КоличествоОбработанныеЗапросы
		...
		+ Результат.КоличествоНепрочитанныхВходящие + Результат.КоличествоНепрочитанныхЕГРЮЛ
		+ Результат.КоличествоНепрочитанныхРассылки;
	...
	Если ТипЗнч(ФормаИлиДерево) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если СтрокаБлокЕГРЮЛ <> Неопределено Тогда
			Если Результат.КоличествоЕГРЮЛ < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокЕГРЮЛ.ЗаголовокБлока = НСтр("ru = 'ЕГРЮЛ'");
			Иначе
				СтрокаБлокЕГРЮЛ.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'ЕГРЮЛ (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоЕГРЮЛ, Результат.КоличествоНепрочитанныхЕГРЮЛ));
			КонецЕсли;
		КонецЕсли;
