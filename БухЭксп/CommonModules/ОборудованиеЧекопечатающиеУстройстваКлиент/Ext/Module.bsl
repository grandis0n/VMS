...
// Заполняет структуру параметров выполнения операции аутентификации пользователя.
//
// Возвращаемое значение:
//  Структура.
//
Функция ПараметрыАутентификацииПользователя(УстановитьВременнуюМетку = Истина) Экспорт;
	
	ВременнаяМетка = ?(УстановитьВременнуюМетку, ОбщегоНазначенияБПОКлиент.ТекущаяВременнаяМетка(), Неопределено);
	
	ПараметрыОперации = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыВыполненияОперации();   
	ПараметрыОперации.Вставить("ВременнаяМетка", ВременнаяМетка);
	ПараметрыОперации.Вставить("Успешность", Истина); // Успешность аутентификации пользователя.     
	Возврат ПараметрыОперации;
	
КонецФункции    

// Заполняет структуру параметров события пользователя.
//
// Возвращаемое значение:
//  Структура.
//
Функция ПараметрыСобытияЭксплуатации(УстановитьВременнуюМетку = Истина) Экспорт;                             
	
	ВременнаяМетка = ?(УстановитьВременнуюМетку, ОбщегоНазначенияБПОКлиент.ТекущаяВременнаяМетка(), Неопределено);
	
	ПараметрыОперации = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыВыполненияОперации();  
	ПараметрыОперации.Вставить("ВременнаяМетка", ВременнаяМетка);
	ПараметрыОперации.Вставить("ИмяСобытия");       // Строка,Обязательно - Имя события, одно из значения "km_user_msg", "user_decision".     
	ПараметрыОперации.Вставить("КодИдентификации"); // Строка,Обязательно - Значение кода идентификации     
	ПараметрыОперации.Вставить("Статус", 0);        // Число,Обязательно  - Успешное выполнение операции 0 или 1.    
	ПараметрыОперации.Вставить("РазрешенКПродаже", Истина); // Булево,Обязательно - Товар разрешен к продаже по совокупности проверок.
	ПараметрыОперации.Вставить("СообщениеПользователю"); // Строка,Необязательно - Сообщение пользователю.
	Возврат ПараметрыОперации;
...
Процедура НачатьРаботуПИоТ(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыВыполнениеКоманды.ОбработатьДанные  = Ложь;
...
Процедура НачатьПолучениеСлужебнойИнформацииПИоТ(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыВыполнениеКоманды = МенеджерОборудованияКлиентСервер.ПараметрыВыполнениеКоманды("PiotServiceInformation", 
		"ОборудованиеЧекопечатающиеУстройства", ДополнительныеПараметры);
	ПараметрыВыполнениеКоманды.ПодготовитьДанные = Ложь;   
	ПараметрыВыполнениеКоманды.ОбработатьДанные  = Ложь; 
	ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент = Ложь;  
...
//   ДополнительныеПараметры - Структура - дополнительные команды.
//
Процедура НачатьПолучениеИнформацииЛМЧЗПИоТ(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ПараметрыОперации = Неопределено Тогда
		ПараметрыОперации = Новый Структура();  
	КонецЕсли;
	
	ПараметрыВыполнениеКоманды = МенеджерОборудованияКлиентСервер.ПараметрыВыполнениеКоманды("PiotInformation", 
		"ОборудованиеЧекопечатающиеУстройства", ДополнительныеПараметры);
	ПараметрыВыполнениеКоманды.ПодготовитьДанные = Ложь;   
	ПараметрыВыполнениеКоманды.ОбработатьДанные  = Истина; 
	ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент = Ложь;  
	ПараметрыВыполнениеКоманды.ОбработатьДанныеКлиент  = Ложь; 
	ПараметрыОперации.Вставить("ИдентификаторКлиента", ИдентификаторКлиента);   
...
// Начать аутентификацию пользователя ТС ПИоТ
//
// Параметры:
//   ОповещениеПриЗавершении - ОписаниеОповещения - оповещение при завершении.
//   ИдентификаторКлиента    - ФормаКлиентскогоПриложения -идентификатор формы.
...
//   ДополнительныеПараметры - Структура - дополнительные команды.
//
Процедура НачатьАутентификациюПользователяПИоТ(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ПараметрыОперации = Неопределено Тогда
		ПараметрыОперации = Новый Структура();  
	КонецЕсли;
		
	ПараметрыВыполнениеКоманды = МенеджерОборудованияКлиентСервер.ПараметрыВыполнениеКоманды("PiotAuthentication", 
		"ОборудованиеЧекопечатающиеУстройства", ДополнительныеПараметры);
	ПараметрыВыполнениеКоманды.ПодготовитьДанные = Истина;   
	ПараметрыВыполнениеКоманды.ОбработатьДанные  = Ложь; 
	ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент = Ложь;  
...
// Аутентификация пользователя ТС ПИоТ
//
// Параметры:
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование - идентификатор устройства.
//   ПараметрыОперации       - Структура - параметры выполнения операции.
//
Процедура АутентификациюПользователяПИоТ(ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации) Экспорт;
	
	ПроверкаИнициализацияПИоТ = МенеджерОборудованияКлиент.ПроверкаИнициализацияПИоТПройдена(ИдентификаторУстройства);    
	Если ПроверкаИнициализацияПИоТ Тогда      
		// Если ПИоТ инициализирован, событие передается сразу ему
		НачатьАутентификациюПользователяПИоТ(Неопределено, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации); 
	Иначе                                                           
		// Если ПИоТ не инициализирован, событие записывается в регистр
		...
		МодульМенеджерОборудованияТСПИоТВызовСервера.ЗаписатьАутентификациюПользователяПИоТ(ИдентификаторУстройства, ПараметрыОперации);
	КонецЕсли;
...
//
// Параметры:
//   Клиент    - ФормаКлиентскогоПриложения -идентификатор формы.
//   Устройство - СправочникСсылка.ПодключаемоеОборудование - идентификатор устройства.
//   ПараметрыОперации       - Структура - параметры выполнения операции.
//   ДополнительныеПараметры - Структура - дополнительные команды.
//
Процедура НачатьСформироватьСобытиеЭксплуатацииПИоТ(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ПараметрыОперации = Неопределено Тогда
		ПараметрыОперации = Новый Структура();  
	КонецЕсли;
		
	ПараметрыВыполнениеКоманды = МенеджерОборудованияКлиентСервер.ПараметрыВыполнениеКоманды("PiotCreateEvent", 
		"ОборудованиеЧекопечатающиеУстройства", ДополнительныеПараметры);
	ПараметрыВыполнениеКоманды.ПодготовитьДанные = Истина;   
	ПараметрыВыполнениеКоманды.ОбработатьДанные  = Ложь; 
	ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент = Ложь;  
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации, ДанныеОперации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеОперации) Тогда
	...
	ИначеЕсли Команда = "RequestKM" Или Команда = "RequestKMKMNoWait" Тогда
		...
		Если ОбщегоНазначенияБПОКлиент.ИспользуетсяМаркировка() Тогда
			...
			Если Не ПустаяСтрока(ПараметрыОперации.КонтрольнаяМарка) Тогда
				...
				ДанныеОперации.Вставить("ПараметрыXML", МодульМенеджерОборудованияМаркировкаКлиент.СформироватьXMLДляЗапросаКМ(ПараметрыОперации, ПараметрыПодключения.РевизияИнтерфейса));
			Иначе
...
Процедура ОбработатьДанныеОперации(ПараметрыПодключения, Команда, РезультатВыполнения, ДанныеОперации) Экспорт
	...
	ИначеЕсли Команда = "PiotStart" Или Команда = "PiotCheckKM" Или Команда = "PiotStatus"  Тогда  
		...
		Если Команда = "PiotCheckKM" Тогда
			Если РезультатВыполнения.Свойство("РезультатПараметрыXML") Тогда
				РезультатПараметрыСтруктура = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПолучитьПараметрыРезультатаИзXMLПакета(РезультатВыполнения.РезультатПараметрыXML); 
			Иначе
		...
		Если Команда = "PiotStart" Тогда
			Если ПараметрыПодключения.Свойство("ИспользоватьТСПИоТ") И ПараметрыПодключения.ИспользоватьТСПИоТ Тогда
				Если РезультатВыполнения.Свойство("АутентификацииПередана") Тогда  
					МодульМенеджерОборудованияТСПИоТВызовСервера = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("МенеджерОборудованияТСПИоТВызовСервера"); 
					МодульМенеджерОборудованияТСПИоТВызовСервера.ОчиститьАутентификацииПользователяПИоТ(ПараметрыПодключения.ИдентификаторУстройства);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;              
	...
	ИначеЕсли Команда = "PiotServiceInformation"  Тогда  

		// ++ Локализация
		Если РезультатВыполнения.Свойство("СлужебнаяИнформацияXML") Тогда
			СлужебнаяИнформацияСтруктура = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПолучитьСлужебнуюИнформациюИзXMLПакета(РезультатВыполнения.СлужебнаяИнформацияXML); 
		Иначе
