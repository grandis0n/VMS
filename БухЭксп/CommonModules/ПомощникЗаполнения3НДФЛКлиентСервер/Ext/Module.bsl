...
Функция УпрощеннаяФормаДекларации(ВыбраннаяФорма) Экспорт
	...
	ДоступныеФормы.Добавить("ФормаОтчета2025Кв4");
	ДоступныеФормы.Добавить("ФормаОтчета2026Кв3");
	
	Возврат ДоступныеФормы.Найти(ВыбраннаяФорма) <> Неопределено;
...
// Возвращает дату начала действия вычета на накопительное страхование жизни
// пп. 5 п. 1 ст. 219.2 НК РФ
//
// Возвращаемое значение:
//   Дата
//
Функция НачалоДействияВычетаЗаДолгосрочноеСтрахование() Экспорт
	
	Возврат '20260901';
	
КонецФункции

// Возвращает признак того, что вычет на страхование жизни со сроком от 10 лет можно применить
// пп. 5 п. 1 ст. 219.2 НК РФ
//
// Параметры:
//   ВыбраннаяФорма - Строка - Редакция формы 3-НДФЛ
//   Период - Дата - Налоговый период
//   ДатаСоставленияДекларации - Дата - Дата составления декларации
//
// Возвращаемое значение:
//   Булево
//
Функция ПрименяетсяВычетЗаДолгосрочноеСтрахование(ВыбраннаяФорма, Период, ДатаСоставленияДекларации) Экспорт
	
	// Приказ ФНС России от 25.05.2026 N ЕД-1-11/333@
	// Если декларация подается с 01.09.2026 за периоды с 2025 г. можно применить вычет за долгосрочное страхование жизни
	
	Если Не ЗначениеЗаполнено(ДатаСоставленияДекларации)
		Или ДатаСоставленияДекларации < НачалоДействияВычетаЗаДолгосрочноеСтрахование() Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ВыбраннаяФорма = "ФормаОтчета2026Кв3" И Период >= '20250101';
	
КонецФункции

// Возвращает признак того, что договор страхования заключен на длительный срок
// пп. 5 п. 1 ст. 219.2 НК РФ
//
// Параметры:
//   СрокДействияДоговора - Число - срок действия договора в полных годах
//
// Возвращаемое значение:
//   Булево
//
Функция ЭтоДолгосрочноеСтрахование(СрокДействияДоговора) Экспорт
	
	Возврат СрокДействияДоговора >= 10;
...
Функция ЗастрахованныеЛицаНакопительноеСтрахование(Пол) Экспорт
	
	Результат = Новый СписокЗначений;
	
	ЗастрахованныеЛица = СвойстваЗастрахованныеЛицаНакопительноеСтрахование();
	
	Результат.Добавить(ЗастрахованныеЛица.Сам, "Заявитель");
	Результат.Добавить(ЗастрахованныеЛица.Дети, "Дети");
	
	Если Пол = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Женский") Тогда
		Результат.Добавить(ЗастрахованныеЛица.Родственник, "Супруг/Родитель");
	Иначе
		Результат.Добавить(ЗастрахованныеЛица.Родственник, "Супруга/Родитель");
	КонецЕсли;
...
Функция ЗастрахованыДети(ЗастрахованноеЛицо) Экспорт
	
	Возврат ЗастрахованноеЛицо = СвойстваЗастрахованныеЛицаНакопительноеСтрахование().Дети;
	
КонецФункции

Функция ЗастрахованноеЛицоПоУмолчанию() Экспорт
	
	Возврат СвойстваЗастрахованныеЛицаНакопительноеСтрахование().Сам;
	
КонецФункции

Функция ИдентификаторДолгосрочныеСбережения() Экспорт
	
	Возврат "ДолгосрочныеСбережения";
	
КонецФункции

Функция ИдентификаторДолгосрочноеСтрахование() Экспорт
	
	Возврат "ДолгосрочноеСтрахование";
	
КонецФункции

Функция ИдентификаторСтрахованиеЖизни() Экспорт
	
	Возврат "СтрахованиеЖизни";
	
КонецФункции

Функция ПараметрыРасчетаПредельнойСуммыВычета() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("ВыбраннаяФорма", ""); // Редакция формы декларации
	Результат.Вставить("ВидВычета", ""); // ключ из структуры см. ПомощникЗаполнения3НДФЛ.КодыВидовДоговоровДобровольныхВзносов()
	Результат.Вставить("Период", '00010101');
	Результат.Вставить("ДатаСоставленияДекларации", '00010101');
	Результат.Вставить("ДатаДоговора", '00010101');
	Результат.Вставить("ДатаОкончанияДоговора", '00010101');
	Результат.Вставить("Застрахованный", ""); // см. СвойстваЗастрахованныеЛицаНакопительноеСтрахование()
	
	Возврат Результат;
	
КонецФункции

Функция СрокДействияДоговора(ДатаНачала, ДатаОкончания) Экспорт
	
	Если Не ЗначениеЗаполнено(ДатаНачала) Или Не ЗначениеЗаполнено(ДатаОкончания) Тогда
		Возврат 0;
	КонецЕсли;
	
	Возврат ОбщегоНазначенияБПКлиентСервер.РазностьДат(ДатаНачала,
		ДатаОкончания,
		ПредопределенноеЗначение("Перечисление.Периодичность.Год"));
	
КонецФункции

Функция ВыбраноДолгосрочноеСбережение(ВидВычета) Экспорт
	
	Возврат ВидВычета = ИдентификаторДолгосрочныеСбережения();
	
КонецФункции

Функция ВыбраноСтрахованиеЖизни(ВидВычета) Экспорт
	
	Возврат ВидВычета = ИдентификаторСтрахованиеЖизни();
	
КонецФункции

Функция ВыбраноДолгосрочноеСтрахованиеЖизни(ПараметрыВычета) Экспорт
	
	Если Не ПрименяетсяВычетЗаДолгосрочноеСтрахование(ПараметрыВычета.ВыбраннаяФорма,
		ПараметрыВычета.Период,
		ПараметрыВычета.ДатаСоставленияДекларации) Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	СрокДействияДоговора = СрокДействияДоговора(ПараметрыВычета.ДатаДоговора, ПараметрыВычета.ДатаОкончанияДоговора);
	
	Возврат ЭтоДолгосрочноеСтрахование(СрокДействияДоговора);
...
Функция СвойстваЗастрахованныеЛицаНакопительноеСтрахование()
	
	Результат = Новый Структура;
	Результат.Вставить("Сам", "Заявитель");
	Результат.Вставить("Дети", "Дети");
	Результат.Вставить("Родственник", "Родственник");
