...
Функция АнализТекстаОшибкиОборудования(ИдентификаторУстройства, Знач ПараметрыПодключения, Знач ТекстОшибки) Экспорт
	...
	ПараметрыПоставкиДрайвера = ПараметрыПоставкиДрайвера(ИдентификаторУстройства);
	ДрайверПоставляетсяВКонфигурации = 
		ПараметрыПоставкиДрайвера.ПоставляетсяВКонфигурации
		И Не ПараметрыПоставкиДрайвера.СнятСПоддержки;
	...
	Если ДрайверПоставляетсяВКонфигурации И УстановленаАктуальнаяВерсияДрайвера Тогда
		...
		Результат.ОписаниеОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + КлассОшибки.ПодробноеОписание;
	...
	ИначеЕсли Не ДрайверПоставляетсяВКонфигурации Тогда
		...
		Строки.Добавить();
		Если ПараметрыПоставкиДрайвера.СнятСПоддержки Тогда
			Строки.Добавить(НСтр("ru = 'Используемый драйвер оборудования не сертифицирован.'"));
		Иначе
			Строки.Добавить(НСтр("ru = 'Используемый драйвер оборудования снят с поддержки.'"));
		КонецЕсли;
		Строки.Добавить(НСтр("ru = 'Обратитесь, пожалуйста, за помощью к поставщику драйвера.'"));
		Результат.ОписаниеОшибки = СтрСоединить(Строки, Символы.ПС);
	...
	ИначеЕсли Не УстановленаАктуальнаяВерсияДрайвера Тогда
		...
		Результат.ОписаниеОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + ПодробноеОписание;
	КонецЕсли;
...
Функция ПараметрыПоставкиДрайвера(ИдентификаторУстройства)
	
	Результат = Новый Структура("ПоставляетсяВКонфигурации, СнятСПоддержки", Истина, Ложь);
	...
	Если ЗначениеЗаполнено(ИдентификаторУстройства) Тогда
		...
		Если ЗначениеЗаполнено(Реквизиты.ДрайверОборудованияПредопределенный) Тогда
			Результат.ПоставляетсяВКонфигурации = Реквизиты.ДрайверОборудованияПредопределенный;
		Иначе
			Результат.ПоставляетсяВКонфигурации = Ложь;
		КонецЕсли;
		Если ЗначениеЗаполнено(Реквизиты.ДрайверОборудованияСнятСПоддержки) Тогда
			Результат.СнятСПоддержки = Реквизиты.ДрайверОборудованияСнятСПоддержки;
		Иначе
			Результат.СнятСПоддержки = Ложь;
		КонецЕсли;
