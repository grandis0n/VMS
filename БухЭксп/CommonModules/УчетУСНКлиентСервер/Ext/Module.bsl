...
Функция ГраницаПримененияНДС(Дата) Экспорт
	...
	Если Год(Дата) >= 2031 Тогда
		// Федеральный закон от 04.07.2026 № 228-ФЗ
		ГраницаДоходов = 10000000;
	ИначеЕсли Год(Дата) >= 2030 Тогда
		// Федеральный закон от 04.07.2026 № 228-ФЗ
...
// Возвращает дату, с которой применяется единое уведомление УСН
//
// Возвращаемое значение:
//  Дата
//
Функция ДатаНачалаПримененияЕдиногоУведомленияУСН() Экспорт
	
	// Приказ Федеральной налоговой службы от 30.04.2026 № ЕД-1-3/285@
	Возврат Дата(2026, 07, 31);
	
КонецФункции

// Возвращает имя актуального регламентированного отчета для уведомления о переходе на УСН
//
// Параметры:
//   ДатаУведомления - Дата - дата уведомления о переходе на УСН
//   ПереходСНПД - Булево - признак перехода на УСН в связи с утратой права на НПД
//
// Возвращаемое значение:
//   Строка - имя регламентированного отчета
//
Функция ИмяОтчетаУведомленияОПереходеНаУСН(ДатаУведомления, ПереходСНПД) Экспорт
	
	ПрименяетсяЕдиноеУведомлениеУСН = ДатаУведомления >= ДатаНачалаПримененияЕдиногоУведомленияУСН();
	
	Если ПрименяетсяЕдиноеУведомлениеУСН Тогда
		ИмяОтчета = "РегламентированноеУведомлениеУСН";
	Иначе
		Если ПереходСНПД Тогда
			ИмяОтчета = "РегламентированноеУведомлениеПереходСНПД";
		Иначе
