...
Процедура СтатусРасчетаНДФЛАУСННажатие(Форма, Элемент, СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПлатежныйДокумент", Форма.Объект.Ссылка);
	ПараметрыФормы.Вставить("ОткрываетсяИзДокумента", Истина);
	ОткрытьФорму("Документ.СписаниеСРасчетногоСчета.Форма.ФормаРасчетНДФЛАУСНПоДокументу", ПараметрыФормы, Форма);
