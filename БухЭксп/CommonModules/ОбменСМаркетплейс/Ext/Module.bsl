...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("АвтозагрузкаОтчетовМаркетплейсовРегламентноеЗадание");
...
// Определяет вариант обработки чисел для используемой локали
// 
// Возвращаемое значение:
//  Число - вариант обработки чисел
//
Функция ОпределитьВариантОбработкиЧисел() Экспорт
	
	ВариантОбработкиЧисел = 0;
	
	ТипЧисло             = Новый ОписаниеТипов("Число");
	РезультатЗапятая     = ТипЧисло.ПривестиЗначение("5,02");
	РезультатТочка       = ТипЧисло.ПривестиЗначение("5.02");
	Если РезультатЗапятая = 0 И РезультатТочка <> 0 Тогда
		ВариантОбработкиЧисел = 1;
	ИначеЕсли РезультатТочка = 0 И РезультатЗапятая <> 0 Тогда
		ВариантОбработкиЧисел = 2;
	КонецЕсли;
	
	Возврат ВариантОбработкиЧисел;
	
КонецФункции

// Преобразует строку в число, исходя из настроек локали
//
// Параметры:
//  Значение				 - Строка - преобразуемая строка
//  ВариантОбработкиЧисел	 - Число - см. функцию ОпределитьВариантОбработкиЧисел
// 
// Возвращаемое значение:
//  Число - преобразованная строка
//
Функция СтрокаВЧисло(Знач Значение, ВариантОбработкиЧисел = 0) Экспорт
	
	Если ВариантОбработкиЧисел = 1 Тогда
		Значение = СтрЗаменить(Значение, ",", ".");
	ИначеЕсли ВариантОбработкиЧисел = 2 Тогда
		Значение = СтрЗаменить(Значение, ".", ",");
	КонецЕсли;
...
Функция ПериодыАвтозагрузки(Интеграция)
	...
	Пока ДатаНачала < ДатаОкончания Цикл
		Если Не ДатыЗапретаИзмененияБП.ЗапрещеноРедактированиеПериода(ДатаНачала, Интеграция.Организация) Тогда
