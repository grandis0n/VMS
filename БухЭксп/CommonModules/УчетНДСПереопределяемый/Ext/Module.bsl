...
Процедура ФормаОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Отчет.Организация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.Параметры, "Организация");
...
Процедура НачислитьНДСНаЕНППриВвозеТоваровИзЕАЭС(Реквизиты, СтрокаТаблицыНачислений, НачислениеНалоговНаЕНС) Экспорт
	...
	ЕНПНеплательщикаНДС = Не ПлательщикНДС Или (ОрганизацияОсвобожденаОтУплатыНДС И Не РаздельныйУчетНДС);
	...
	НоваяСтрока.Содержание    = НСтр("ru = 'Единый налоговый счет: Начисление НДС за товары из ЕАЭСа'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецПроцедуры

Процедура НачислитьАкцизНаЕНППриВвозеТоваровИзЕАЭС(Реквизиты, СтрокаТаблицыНачислений, НачислениеНалоговНаЕНС) Экспорт
	
	НоваяСтрока = НачислениеНалоговНаЕНС.Добавить();
	
	НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате; // 68.43
	
	НоваяСтрока.СуммаНалога   = СтрокаТаблицыНачислений.СуммаАкциза;
	НоваяСтрока.Начислено     = СтрокаТаблицыНачислений.СуммаАкциза;
	НоваяСтрока.Сумма         = СтрокаТаблицыНачислений.СуммаАкциза;
	НоваяСтрока.Субконто1     = Реквизиты.Контрагент;
	НоваяСтрока.Субконто2     = Реквизиты.ДоговорКонтрагента;
	НоваяСтрока.Субконто3     = Реквизиты.Регистратор;
	...
	НоваяСтрока.НачалоПериода = НачалоМесяца(СтрокаТаблицыНачислений.ДатаПринятияНаУчет);
	НоваяСтрока.КонецПериода  = КонецМесяца(СтрокаТаблицыНачислений.ДатаПринятияНаУчет);
	НоваяСтрока.Содержание    = НСтр("ru = 'Единый налоговый счет: Начисление акциза за товары из ЕАЭС'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ТекстЗапросаКосвенныеНалогиСведенияИзДОПП(Запрос) Экспорт

	УчетНДСБП.ТекстЗапросаКосвенныеНалогиСведенияИзДОПП(Запрос);
