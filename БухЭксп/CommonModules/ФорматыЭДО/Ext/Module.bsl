...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		Форматы.ЭПД.Вставить("ЭПЭ", "EPE");
		Форматы.ЭПД.Вставить("ЭЭР", "EER");
		Форматы.ЭПД.Вставить("ЭСР", "ESR");
	КонецЕсли;
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		ДанныеФайлаЭД = МодульПодсистемы.ПриФормированияДанныхФайлаЭД(ОписаниеФайла);
	КонецЕсли;
	...
	ДолжностьПоСертификату = 
		УчетныеЗаписиЭДО.ДолжностьПоСертификатуУчетнойЗаписи(ИдентификаторОрганизации, СертификатПодписи);
	...
	ТипУчастникаЭДО = ИнтеграцияЭДО.ТипУчастникаЭДО(Организация);
	...
	Если (Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Если ПустаяСтрока(ОписаниеОшибки) Тогда
			...
			Если (ЕстьСвойствоXDTO(Подписант, "ИП") И Подписант.ИП <> Неопределено)
				...
				Или (ЕстьСвойствоXDTO(Подписант, "ФЛ") И Подписант.ФЛ <> Неопределено)
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		...
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "Должн", Должность, Истина, Ошибки);
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях Тогда
		...
		Иначе
			...
			ИначеЕсли ТипУчастникаЭДО.ЭтоИП Тогда
				...
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ИП, "ФИО", ФИО, Истина, Ошибки);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "ИП", ИП, Истина, Ошибки);
			ИначеЕсли ТипУчастникаЭДО.ЭтоФизЛицо Тогда
				ФЛ = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML("СвФлТип", ПространствоИменСхемы);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФЛ, "ИННФЛ", ДанныеЮрФизЛица.ИНН, Истина, Ошибки);

				ФИО = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML("СвФлТип.ФИО", ПространствоИменСхемы);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", ФамилияСертификата, Истина, Ошибки);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Имя", ИмяСертификата, Истина, Ошибки);
				Если ЗначениеЗаполнено(ОтчествоСертификата) Тогда
					РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, , Ошибки);
			...
			ИначеЕсли ТипУчастникаЭДО.ЭтоИП Тогда
				...
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта],
					"ОснПолн", ОснованиеПолномочий, Истина, Ошибки);
			ИначеЕсли ТипУчастникаЭДО.ЭтоФизЛицо Тогда
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта], "ФЛ", ФЛ,
					Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта],
				"Статус", "4", Истина, Ошибки);
			КонецЕсли;
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
			...
			МодульПодсистемы.ПриЗаполненииДанныхПодписанта(ОписаниеФайла, ДанныеФайлаЭД, ДанныеПодписанта,
				ЗаполнениеДанныхПодписантаДляЭПД, Ошибки);
		КонецЕсли;
		
		Попытка
			
			Если Не ЗаполнениеДанныхПодписантаДляЭПД Тогда
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		...
		УдалениеПространстваИменДляЭПД = Ложь;
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
			МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
			ДвоичныеДанные = МодульПодсистемы.ПриУдаленииПространстваИмен(ДанныеФайлаЭД, УдалениеПространстваИменДляЭПД);
		КонецЕсли;
		
		Если Не УдалениеПространстваИменДляЭПД Тогда
...
Функция ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		Если МодульПодсистемы.ПриЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ПодписантЗаполнен(Подписант, Организация, ОписаниеОшибки = "") Экспорт
	...
	Если ТипУчастникаЭДО.ЭтоИП
			И Подписант.Вид <> "ИП" Тогда
	...
	ИначеЕсли ТипУчастникаЭДО.ЭтоЮрЛицо
			И Подписант.Вид <> "ЮЛ" Тогда
		ОписаниеОшибки = СтрШаблон(
...
Функция СформироватьДанныеОсновногоТитулаПоОбъектуУчета(ОбъектУчета, Настройки, Данные) Экспорт
	...
	ЕстьОшибкиПоПроверкеФизическогоЛица = Ложь;
	Если ТипЗнч(ДанныеДокумента) = Тип("ДеревоЗначений") И ДанныеДокумента.Строки.Количество() > 0 Тогда
		ЕстьОшибкиПоПроверкеФизическогоЛица = ЕстьОграниченияДляФизическогоЛица(ОбъектУчета, ДанныеДокумента, Формат);
	КонецЕсли;
	
	Если ТипЗнч(ДанныеДокумента) = Тип("ДеревоЗначений") И ДанныеДокумента.Строки.Количество() = 0
		Или ЕстьОшибкиПоПроверкеФизическогоЛица Тогда
...
Функция ЗагружаемыйФайлСоответствуетЗагруженномуФайлу(ДанныеДокумента, ЗагружаемыйФайл, ЗагруженныйФайл) Экспорт
	...
	Если ИнтеграцияЭДОВызовСервера.ПоддержанаРаботаСФизическимиЛицами()
		И (ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_02.ИнформацияПродавца
			Или ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_03.ИнформацияПродавца) Тогда
		
		ЭтоФизическоеЛицо = Ложь;
		Если ЕстьСвойствоXDTO(ЗагружаемыеДанные, "Документ")
				И ЕстьСвойствоXDTO(ЗагружаемыеДанные.Документ, "СвСчФакт")
				И ЕстьСвойствоXDTO(ЗагружаемыеДанные.Документ.СвСчФакт, "СвПрод") Тогда
			ИННФЛ = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыеДанные.Документ.СвСчФакт.СвПрод[0], "ИдСв.СвФЛУч.ИННФЛ");
			ЭтоФизическоеЛицо = ЗначениеЗаполнено(ИННФЛ);
		КонецЕсли;
		
		ДокументФункция = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыеДанные, "Документ.Функция");
		Если ЭтоФизическоеЛицо
				И (ДокументФункция = "СЧФ" 
					Или ДокументФункция = "СЧФДОП") Тогда
			ТекстСообщения = НСтр("ru='Документ не загружен, так как продавец в лице физического лица не может использовать Счет-фактуру.
			|Выберите другой файл и повторите загрузку.'");
			РезультатЗагрузки.Ошибка = ТекстСообщения;
			Возврат РезультатЗагрузки;
			
		ИначеЕсли ЭтоФизическоеЛицо 
				И ЕстьСвойствоXDTO(ЗагружаемыеДанные, "Документ")
				И ЕстьСвойствоXDTO(ЗагружаемыеДанные.Документ, "ТаблСчФакт")
				И ЕстьСвойствоXDTO(ЗагружаемыеДанные.Документ.ТаблСчФакт, "СведТов") Тогда
			
			Для Каждого СтрокаСведений Из ЗагружаемыеДанные.Документ.ТаблСчФакт.СведТов Цикл
				
				Числа = "0123456789";
				Если СтрЧислоВхождений(Числа, Лев(СтрокаСведений.НалСт, 1)) И Не Лев(СтрокаСведений.НалСт, 1) = "0" Тогда
					ТекстСообщения = НСтр("ru='Документ не загружен, так как продавец в лице физического лица не может использовать НДС.
					|Выберите другой файл и повторите загрузку.'");
					РезультатЗагрузки.Ошибка = ТекстСообщения;
					Возврат РезультатЗагрузки;
				КонецЕсли;
				
				СумАкциз = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СтрокаСведений, "Акциз.СумАкциз");
				Если СумАкциз <> Неопределено И Не Лев(СумАкциз, 1) = "0" Тогда
					ТекстСообщения = НСтр("ru='Документ не загружен, так как продавец в лице физического лица не может реализовывать подакцизную продукцию.
					|Выберите другой файл и повторите загрузку.'");
					РезультатЗагрузки.Ошибка = ТекстСообщения;
					Возврат РезультатЗагрузки;
				КонецЕсли;
				
				ГТИН = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СтрокаСведений, "ДопСведТов.ГТИН");
				Если ГТИН <> Неопределено И ГТИН <> "" Тогда
					ТекстСообщения = НСтр("ru='Документ не загружен, так как продавец в лице физического лица не может реализовывать маркируемую продукцию.
					|Выберите другой файл и повторите загрузку.'");
					РезультатЗагрузки.Ошибка = ТекстСообщения;
					Возврат РезультатЗагрузки;
				КонецЕсли;
...
Процедура ПроверитьРегистрационныеДанныеДляОператораЭДО(ДеревоДанных, Ошибки) Экспорт
	...
	СтрокаКомментария = Неопределено;
	МинимальнаяДлиннаКомментария = УчетныеЗаписиЭДОКлиентСервер.МинимальнаяДопустимаяДлинаНазначенияУчетнойЗаписи();
	Комментарий = СокрЛП(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных,
			"УчастЭДО.Комментарий", Ложь, СтрокаКомментария));
	
	Если ЗначениеЗаполнено(Комментарий) И  СтрДлина(Комментарий) < МинимальнаяДлиннаКомментария Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Длина назначения должна быть не менее %1 символов'"), МинимальнаяДлиннаКомментария);
		ДеревоЭлектронногоДокументаБЭД.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтрокаКомментария, ТекстОшибки,
			Истина, , "Назначение учетной записи");
	КонецЕсли;
...
// Проверяет подходит ли документ для формирования ЭД для физического лица
//
// Параметры:
//  ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  ДанныеДокумента - См. ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента
//  Формат - Строка
//
// Возвращаемое значение:
//  Булево
Функция ЕстьОграниченияДляФизическогоЛица(ОбъектУчета, ДанныеДокумента, Формат)
	...
	Если Не (Формат = ПоддерживаемыеФорматы.ФНС.СчетФактура 
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.ТорговаяОперация.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.ДоговорныйДокумент101
				Или Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01
				Или Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101
				Или Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.CML208
				Или Формат = ПоддерживаемыеФорматы.БизнесСеть.КоммерческиеПредложения)
			Или Не ИнтеграцияЭДОВызовСервера.ПоддержанаРаботаСФизическимиЛицами() Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если Формат = ПоддерживаемыеФорматы.ФНС.ДоговорныйДокумент101
			И ДанныеДокумента.ИдентификационныеСведенияФизЛица.Количество() > 0 Тогда
		Если ДанныеДокумента.ИнформацияДокумента.Количество() > 0
				И ДанныеДокумента.ИнформацияДокумента[0].СведенияОбОбщейСтоимости.СуммаНДС > 0 Тогда
			ТекстСообщения = НСтр("ru='Физическое лицо не может использовать НДС.
			|Укажите в договоре ставку ""0%"", ""Без НДС"" или ""Облагается у покупателя"", после попробуйте снова сформировать электронный документ.'");
			Ошибки.Добавить(ТекстСообщения);
		КонецЕсли;
	
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101 Тогда
		
		ТипУчастникаПродавец = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Продавец.ТипУчастника", Ложь);
		ПродолжитьПроверку = ТипУчастникаПродавец = "ФЛ";
		...
		Путь = "ТаблицаТоваров";
		СтрокаТаблицаТоваров = ДанныеДокумента.Строки.Найти(Путь, "ПолныйПуть", Истина);
		Если СтрокаТаблицаТоваров <> Неопределено Тогда
			Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
				СтавкаНДС = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, Путь + ".НомерСтроки.СтавкаНДС", Ложь);
				Если СтрЧислоВхождений(Числа, Лев(СтавкаНДС, 1)) И Не Лев(СтавкаНДС, 1) = "0" Тогда
					ЕстьНДС = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
		...
		Если СтрокаТаблицаТоваров <> Неопределено Тогда
			Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
				СтавкаНДС = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, Путь + ".НомерСтроки.НалоговаяСтавка.СтавкаНДС", Ложь);
				Если СтрЧислоВхождений(Числа, Лев(СтавкаНДС, 1)) И Не Лев(СтавкаНДС, 1) = "0" Тогда
					ЕстьНДС = Истина;
					Прервать;
				КонецЕсли;
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.CML208
			Или Формат = ПоддерживаемыеФорматы.БизнесСеть.КоммерческиеПредложения Тогда
		
		ТипУчастникаПродавец = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Продавец.ТипУчастника", Ложь);
		ТипУчастникаВладелец = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Владелец.ТипУчастника", Ложь);
		ОрганизацияВид = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Организация.Вид", Ложь);
		ПродолжитьПроверку = ТипУчастникаПродавец = "ФЛ"
			Или ТипУчастникаВладелец = "ФЛ"
			Или (ОрганизацияВид = "ФизическоеЛицо"
					Или ОрганизацияВид = "ФЛ");
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца Тогда
		...
		ТипУчастникаПродавец = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "СведенияОПродавце.ТипУчастника", Ложь);
		ПродолжитьПроверку = ТипУчастникаПродавец = "ФЛ";
		...
		ФункцияУПД = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Функция");
		Если ФункцияУПД <> Неопределено
				И ФункцияУПД = "КСЧФ" Тогда
			ТекстСообщения = НСтр("ru='Физическое лицо не может использовать корректировочную Счет-фактуру.'");
			Ошибки.Добавить(ТекстСообщения);
		ИначеЕсли ФункцияУПД <> Неопределено
				И ФункцияУПД = "КСЧФДИС" Тогда
			ТекстСообщения = НСтр("ru='Физическое лицо не может использовать корректировочную Счет-фактуру.
			|Исключите Счет-фактуру из документа и попробуйте снова его сформировать.'");
			Ошибки.Добавить(ТекстСообщения);
		Иначе
			
			Путь = "СведенияОТоварах";
			СтрокаТаблицаТоваров = ДанныеДокумента.Строки.Найти(Путь, "ПолныйПуть", Истина);
			Если СтрокаТаблицаТоваров <> Неопределено Тогда
				Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
					СтавкаНДС = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, Путь + ".НомерСтроки.НалоговаяСтавка");
					Если СтавкаНДС <> Неопределено
							И СтрЧислоВхождений(Числа, Лев(СтавкаНДС, 1))
							И Не Лев(СтавкаНДС, 1) = "0" Тогда
						ЕстьНДС = Истина;
					КонецЕсли;
					СуммаАкциза = ДеревоЭлектронногоДокументаБЭД.ДанныеЭлементаДереваЭлектронногоДокумента(
						Товар, Путь + ".НомерСтроки.СуммаАкциза");
					Если СуммаАкциза <> Неопределено
							И СуммаАкциза > 0  Тогда
						ЕстьАкцизы = Истина;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Иначе 
		...
		Путь = "СведенияОСчетеФактуре.СведенияОПродавцах";
		СведенияОбУчастнике = ДанныеДокумента.Строки.Найти(Путь, "ПолныйПуть", Истина);
		Если СведенияОбУчастнике <> Неопределено Тогда
			Для Каждого СтрокаУчастника Из СведенияОбУчастнике.Строки Цикл
				ТипУчастника = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(СтрокаУчастника, Путь + ".НомерСтроки.Продавец.ТипУчастника");
				ПродолжитьПроверку = ТипУчастника = "ФЛ";
		...
		ФункцияУПД = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента, "Функция");
		Если ФункцияУПД <> Неопределено
				И ФункцияУПД = "СЧФ" Тогда
			ТекстСообщения = НСтр("ru='Физическое лицо не может использовать Счет-фактуру.'");
			Ошибки.Добавить(ТекстСообщения);
		ИначеЕсли ФункцияУПД <> Неопределено
				И ФункцияУПД = "СЧФДОП" Тогда
			ТекстСообщения = НСтр("ru='Физическое лицо не может использовать Счет-фактуру.
			|Исключите Счет-фактуру из документа и попробуйте снова его сформировать.'");
			Ошибки.Добавить(ТекстСообщения);
		Иначе
			...
			Если СтрокаТаблицаТоваров <> Неопределено Тогда
				Для Каждого Товар Из СтрокаТаблицаТоваров.Строки Цикл
					...
					СтавкаНДС = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Товар, Путь + ".НомерСтроки.НалоговаяСтавка");
					Если СтавкаНДС <> Неопределено
							И СтрЧислоВхождений(Числа, Лев(СтавкаНДС, 1)) И Не Лев(СтавкаНДС, 1) = "0" Тогда
						ЕстьНДС = Истина;
					КонецЕсли;
					
					СведенияПоАкцизам = ДеревоЭлектронногоДокументаБЭД.ДанныеЭлементаДереваЭлектронногоДокумента(
						Товар, Путь + ".НомерСтроки.СуммаАкциза");
					Если СведенияПоАкцизам <> Неопределено
							И СведенияПоАкцизам.Сумма > 0  Тогда
						ЕстьАкцизы = Истина;
					КонецЕсли;
					
					СведенияОМаркировке = ДеревоЭлектронногоДокументаБЭД.ДанныеЭлементаДереваЭлектронногоДокумента(
						Товар, Путь + ".НомерСтроки.СведенияОМаркировке");
					Если СведенияОМаркировке <> Неопределено
							И СведенияОМаркировке.Количество() > 0 Тогда
						ЕстьМаркируемаяПродукция = Истина;
					КонецЕсли;
					
				КонецЦикла;
			КонецЕсли;
			
			СтрокаТаблицаТоваров = ДанныеДокумента.Строки.Найти("ЕстьАлкогольнаяПродукция", "ПолныйПуть", Истина);
			ЕстьАлкогольнаяПродукция = СтрокаТаблицаТоваров <> Неопределено
				И СтрокаТаблицаТоваров.Строки.Количество() > 0;
	...
	Если ЕстьНДС Тогда
		ТекстСообщения = НСтр("ru='Физическое лицо не может использовать НДС.
		|Укажите в документе ставку ""0%"", ""Без НДС"" или ""Облагается у покупателя"", после попробуйте снова сформировать электронный документ.'");
		Ошибки.Добавить(ТекстСообщения);
	КонецЕсли;
	
	Если ЕстьАкцизы Тогда
		ТекстСообщения = НСтр("ru='Физическое лицо не может реализовывать подакцизную продукцию.
		|Удалите акцизную продукцию в документе и попробуйте снова его сформировать.'");
		Ошибки.Добавить(ТекстСообщения);
	КонецЕсли;
	
	Если ЕстьАлкогольнаяПродукция Тогда
		ТекстСообщения = НСтр("ru='Физическое лицо не может реализовывать алкогольную продукцию.
		|Удалите алкогольную продукцию в документе и попробуйте снова его сформировать.'");
		Ошибки.Добавить(ТекстСообщения);
	КонецЕсли;
	
	Если ЕстьМаркируемаяПродукция Тогда
		ТекстСообщения = НСтр("ru='Физическое лицо не может реализовывать маркируемую продукцию.
		|Удалите маркируемую продукцию в документе и попробуйте снова его сформировать.'");
		Ошибки.Добавить(ТекстСообщения);
	КонецЕсли;
	...
	Для Каждого Ошибка Из Ошибки Цикл
		...
		Сообщение.КлючДанных = ОбъектУчета;
		Сообщение.Сообщить();
	КонецЦикла;
