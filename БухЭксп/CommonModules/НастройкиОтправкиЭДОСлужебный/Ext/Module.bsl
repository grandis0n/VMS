...
Функция ИспользуетсяУПД_УКД(КлючНастроекОтправки) Экспорт
	...
	ОтправительФизЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(КлючНастроекОтправки.Отправитель).ЭтоФизЛицо;
	
	Результат = Новый Структура;
	Результат.Вставить("ИспользуетсяУПД", Не ОтправительФизЛицо);
	Результат.Вставить("ИспользуетсяУКД", Не ОтправительФизЛицо);
...
Функция УчастникиЭДОГотовыКОбмену(ИдентификаторОтправителя, ИдентификаторПолучателя, СпособОбмена)
	...
	ИначеЕсли СинхронизацияЭДО.ЭтоПрямойОбмен(СпособОбмена) Тогда
		Результат = Истина;	
	ИначеЕсли СервисEDIСлужебный.ЭтоОбменЧерезСервис1СEDI(СпособОбмена) Тогда
		Результат = Истина;	
	Иначе
