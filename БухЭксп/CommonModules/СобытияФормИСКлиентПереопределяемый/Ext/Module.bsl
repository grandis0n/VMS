...
Процедура ОбработкаНавигационнойСсылки(Форма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ГиперссылкаОткрытьФормуНастройкиПараметровНоменклатурыИС") Тогда
		...
		ВидПродукции = Форма.КлючВидовПродукции;
		...
		ПараметрыОбработкиДействия.КарточкаТовараСодержитВесовойПризнак 	= Не ИнтеграцияИСМПБПВызовСервера.ЭтоШтучнаяЕдиница(Форма.Объект.ЕдиницаИзмерения);
		ПараметрыОбработкиДействия.ДопустимаНастройкаЛогистическойЕдиницы 	= Ложь;
		ПараметрыОбработкиДействия.УпаковкиВключены                     	= Ложь;
		ПараметрыОбработкиДействия.АдресДанныхНастроекПараметровНоменклатуры= Форма.АдресДанныхНастроекПараметровНоменклатурыИС;	
...
Процедура ПриНачалеВыбораНоменклатуры(Владелец, ВидыПродукции, СтандартнаяОбработка, ОписаниеОповещения = Неопределено, Знач Реквизиты = Неопределено) Экспорт
	...
	ВидПродукцииИС = Новый Массив;
	
	Если ТипЗнч(ВидыПродукции) = Тип("Массив")
		ИЛИ ТипЗнч(ВидыПродукции) = Тип("ФиксированныйМассив") Тогда
		ВидПродукцииИС = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ВидыПродукции);
	Иначе
		ВидПродукцииИС.Добавить(ВидыПродукции);
	КонецЕсли;
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ВидПродукцииИС", ВидПродукцииИС);
	
	Если ДополнительныеПараметры.Свойство("ОКПД2") Тогда
		ПараметрыОтбора.Вставить("ОКПД2", ИнтеграцияИСМПБПВызовСервера.ИерархияЭлементовСправочникаОКПД2(ДополнительныеПараметры.ОКПД2));
	КонецЕсли;
	Если ДополнительныеПараметры.Свойство("КодТНВЭД") Тогда
		ДоступныеТНВЭД = Новый Массив;
		ДоступныеТНВЭД.Добавить("");
		ДоступныеТНВЭД.Добавить(ДополнительныеПараметры.КодТНВЭД);
		ПараметрыОтбора.Вставить("КодТНВЭД", ДоступныеТНВЭД);
	КонецЕсли;
	...
	Иначе
		Номенклатура = ИнтеграцияИСКлиентПереопределяемый.ЗначениеЭлементаФормыПоИмени(Владелец, "Номенклатура");
	КонецЕсли;
