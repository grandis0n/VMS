...
Функция РеквизитыДокументаПорядокУчетаРасчетов(ОсобенностиДокумента = Неопределено) Экспорт
	...
	ТребуетсяУчетРасчетовСАгентомПоставщика = Ложь;
	...
	Иначе
		...
		ТребуетсяУчетРасчетовСАгентомПоставщика = ОсобенностиДокумента.ТребуетсяУчетРасчетовСАгентомПоставщика;
	КонецЕсли;
	...
	Если ТребуетсяУчетРасчетов Тогда
		...
		Если СчетаУчетаВДокументахВызовСервераПовтИсп.ПользовательУправляетСчетамиУчета() Тогда
			...
			Если ТребуетсяУчетРасчетовСАгентомПоставщика Тогда
				Реквизиты.Вставить("СчетУчетаРасчетовСАгентомПоставщика");
				Реквизиты.Вставить("СчетУчетаРасчетовСАгентомПоставщикаПоАвансам");
			КонецЕсли;
		КонецЕсли;
...
Функция НовыйОсобенностиУчетаРасчетовДокумента() Экспорт
	...
	Особенности.Вставить("ТребуетсяУчетРасчетовСАгентомПоставщика", Ложь);
...
Функция СвойстваФормыПорядокУчетаРасчетов(Форма, ОсобенностиДокумента)
	...
	Если ОсобенностиДокумента.ТребуетсяУчетРасчетовСПлатежнымАгентом Тогда
		...
		КонецЕсли;
	ИначеЕсли ОсобенностиДокумента.ТребуетсяУчетРасчетовСАгентомПоставщика
		И Объект.ОплатаЧерезАгентаПоставщика Тогда
		Счета.Добавить("СчетУчетаРасчетовСАгентомПоставщика");
		Счета.Добавить("СчетУчетаРасчетовСАгентомПоставщикаПоАвансам");
		НаименованиеАгентаПоставщика = "<...>";
		Если ЗначениеЗаполнено(Объект.АгентПоставщика) Тогда
			НаименованиеАгентаПоставщика = СокрЛП(Объект.АгентПоставщика);
			МаксимальнаяДлина = 30;
			Если СтрДлина(НаименованиеАгентаПоставщика) > МаксимальнаяДлина Тогда
				НаименованиеАгентаПоставщика = Лев(НаименованиеАгентаПоставщика, МаксимальнаяДлина - 3) + "...";
			КонецЕсли;
		КонецЕсли;
		ЭлементыТекста.Добавить(СтрШаблон(НСтр("ru = 'оплата через %1'"), НаименованиеАгентаПоставщика));
	Иначе
		Счета.Добавить("СчетУчетаРасчетовСКонтрагентом");
		Счета.Добавить("СчетУчетаРасчетовПоАвансам");
	КонецЕсли;
	...
	ИначеЕсли ОсобенностиДокумента.ТребуетсяУчетРасчетовСАгентомПоставщика Тогда
		
		Если Не Объект.ОплатаЧерезАгентаПоставщика
			И РеквизитыФормы.Свойство("СрокОплаты") Тогда
			Если ЗначениеЗаполнено(Форма.СрокОплаты) Тогда
				ТекстСрокОплаты = СрокиОплатыДокументов.ПредставлениеСрокаОплаты(Форма.СрокОплаты);
				ЭлементыТекста.Добавить(ТекстСрокОплаты);
