...
Функция БронированияЗаПериод(Организация, Гостиница, Период, СообщениеОбОшибке) Экспорт
	...
	ЗначимыеРеквизиты = Новый Структура("Номенклатура,Содержание,Количество,Цена,Сумма,СтавкаНДС,СуммаНДС");
	...
	Для каждого СтрокаБронирование Из ТаблицаБронирования Цикл
		...
		Если ЗначениеЗаполнено(ДокументПоНомеруБрони) Тогда
			...
			ТаблицаТовары.Свернуть("Номенклатура,Содержание,Количество,Цена,Сумма,СтавкаНДС,СуммаНДС, ВидЛьготыПоТуристическомуНалогу");
			ДокументОбъект.Товары.Загрузить(ТаблицаТовары);
			
			ДокументВXML = ОбщегоНазначения.ЗначениеВСтрокуXML(ДокументОбъект);
		...
		ДокументИзменен = НЕ ЗначениеЗаполнено(ДокументВXML) 
			ИЛИ ДокументВXML <> ОбщегоНазначения.ЗначениеВСтрокуXML(ДокументОбъект);
...
Функция ПроживанияЗаПериод(Организация, Гостиница, Период, СообщениеОбОшибке) Экспорт
	...
	Для каждого СтрокаБронирование Из ТаблицаБронирования Цикл
		...
		Если ЗначениеЗаполнено(ДокументПоНомеруБрони) Тогда
			...
			ТаблицаТовары = ДокументОбъект.Товары.Выгрузить();
			ТаблицаТовары.Свернуть("Номенклатура,Количество,Цена,Сумма,СтавкаНДС,СуммаНДС");
			ДокументОбъект.Товары.Загрузить(ТаблицаТовары);
			
			ТаблицаУслуги = ДокументОбъект.Услуги.Выгрузить();
			ТаблицаУслуги.Свернуть("Номенклатура,Содержание,Количество,Цена,Сумма,СтавкаНДС,СуммаНДС");
			ДокументОбъект.Услуги.Загрузить(ТаблицаУслуги);
...
Функция ЗапросСоздатьОбновитьУчетнуюЗапись(ПараметрыМетода)
	...
	|	НастройкиИнтеграцииBnovo.ИдентификаторГостиницы КАК ИдентификаторГостиницы
	|ИЗ
	|	РегистрСведений.НастройкиИнтеграцииBnovo КАК НастройкиИнтеграцииBnovo
	|ГДЕ
	...
	Если Результат.Следующий() Тогда
		...
		ПараметрыЗапроса.Данные.Вставить("name", СтрШаблон("%1 (%2/%3)", Результат.Организация, Результат.ИдентификаторГостиницы, Результат.Гостиница));
	КонецЕсли;
