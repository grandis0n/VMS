...
//  Структура - Пример файла выгрузки:
//   * СодержаниеФайла - Строка - 
//   * ИмяФайла - Строка - 
//
Функция ФайлВыгрузкиДокументацииКС(ОсновныеСведенияВыгрузки, ДанныеДокументации, УникальныйИдентификатор) Экспорт
	...
	СсылкаНаХранилищеКомпоновщикаXML = СсылкаНаХранилищеКомпоновщикаXML(КомпоновщикXML, ИмяВременногоФайла, УникальныйИдентификатор);
...
Процедура ВывестиУзел_ДокУвКонтрСд(КомпоновщикXML, ПараметрыМетода)
	...
	Если ПараметрыМетода.КодМетодаЦенообразования = КонтролируемыеСделкиЦенообразование.КодМетодаКомбинацияМетодов() Тогда
...
Процедура ВывестиУзел_ИнфМетодЦен(КомпоновщикXML, Метод)
	...
	ВывестиАтрибут(КомпоновщикXML, "НаимКоррСопост", Метод.НаименованиеКорректировкиДляСопоставимостиСопоставимыхЦен, "Т,100");
...
Процедура ВывестиУзел_ОснСведКонтр(КомпоновщикXML, ОписаниеКонтрагента)
	...
	ВывестиНеобязательныйАтрибут(КомпоновщикXML, "НаимОргЛат", ОписаниеКонтрагента.НаименованиеОрганизацииЛатиницей, "Т,1000");
...
Процедура ДобавитьФайлыПриложенияКВыгрузке(Результат, ДанныеДокументации, УникальныйИдентификатор)
	...
	Для Каждого Приложение Из ДанныеДокументации.Приложения Цикл
		...
		ДанныеФайла = Новый Структура();
		ДанныеФайла.Вставить("АдресФайлаВыгрузки", ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, УникальныйИдентификатор));
