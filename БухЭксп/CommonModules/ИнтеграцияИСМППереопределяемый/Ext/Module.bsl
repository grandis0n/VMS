...
Процедура ПриОпределенииТекстаЗапросаЗначенияРеквизитаЕдиницыИзмерения(ТекстЗамены, ИмяРеквизита, ПутьКРеквизитуНоменклатура, ПутьКРеквизитуЕдиницыИзмерения) Экспорт
	...
	//++ НЕ ГОСИС
	ТекстЗамены = ИнтеграцияИСМПБП.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения(
		ИмяРеквизита,
...
Процедура ОбработкаЗаполненияДокумента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	//заполняем организцию если документ создан без основания
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", МетаданныеДокумента) 
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("Организация") Тогда
			...
			Иначе
				ДанныеЗаполнения.Организация = ОсновнаяОрганизация;
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьПеремещениеМеждуМОД_ИСМПНаОснованииАвизоМПЗИсходящее(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО АвизоМПЗИсходящееТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|ГДЕ
...
Процедура ЗаполнитьТоварыДокумента(ДокументОбъект, ДанныеЗаполнения)
...
Процедура ЗаполнитьПеремещениеМеждуМОД_ИСМПНаОснованииПеремещениеТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Документ.ПеремещениеТоваров.Товары КАК ПеремещениеТоваровТовары
	|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ПеремещениеТоваровТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура	
	|ГДЕ
	|	ПеремещениеТоваровТовары.Ссылка = &ДокументОснование
	|	И &ТекстУсловияЗапросаПиво
	|;
...
Функция ТекстУсловияЗапросаПиво()
	...
	Возврат "(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЕГАИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) 
