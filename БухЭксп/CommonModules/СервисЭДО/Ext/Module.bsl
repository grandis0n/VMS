...
Функция ОтправитьРегистрационныйПакет1СЭДО(ДанныеПакета, КонтекстДиагностики = Неопределено,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПередОтправкойРегистрационногоПакета1СЭДО(ПараметрыВыполненияОперации);
	КонецЕсли;
...
Функция НоваяТаблицаПриглашений() Экспорт
	...
	ТаблицаПриглашений.Колонки.Добавить("Получатель", Метаданные.ОпределяемыеТипы.УчастникЭДО.Тип);
	ТаблицаПриглашений.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ИНН", Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("КПП", Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("АдресЭП", Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ТекстПриглашения",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ВнешнийКод",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ИдентификаторОрганизации",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ИдентификаторКонтрагента",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ОператорЭДО",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ИмяФайлаСкана",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ДанныеФайлаСкана", Новый ОписаниеТипов("ДвоичныеДанные"));
	ТаблицаПриглашений.Колонки.Добавить("СоздаватьНастройкиЭДО", Новый ОписаниеТипов("Булево"));
	ТаблицаПриглашений.Колонки.Добавить("EmailОрганизации",  Новый ОписаниеТипов("Строка"));
	ТаблицаПриглашений.Колонки.Добавить("ВидУчетнойЗаписи", Новый ОписаниеТипов("ПеречислениеСсылка.ВидУчетнойЗаписиЭДО"));
...
Функция ПараметрыСервиса1СЭДО() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриОпределенииПараметровСервиса1СЭДО(ПараметрыСервиса);
	КонецЕсли;
...
// Это код ошибки повторной отправки.
// 
// Параметры:
//  КодОшибки - Число - Код ошибки
// 
// Возвращаемое значение:
//  Булево - Это код ошибки повторной отправки
//
...
Функция НоваяТаблицаКонтейнеров() Экспорт
	...
	ТаблицаКонтейнеров.Колонки.Добавить("ИДПакета", Новый ОписаниеТипов("Строка"));
	ТаблицаКонтейнеров.Колонки.Добавить("ИДДокументооборота", Новый ОписаниеТипов("Строка"));
	ТаблицаКонтейнеров.Колонки.Добавить("КодТранзакции", Новый ОписаниеТипов("Строка"));
	ТаблицаКонтейнеров.Колонки.Добавить("КодРегламента", Новый ОписаниеТипов("Строка"));
	ТаблицаКонтейнеров.Колонки.Добавить("ИДДокумента", 	Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(80, ДопустимаяДлина.Переменная)));
	ТаблицаКонтейнеров.Колонки.Добавить("ДвоичныеДанные");
	ТаблицаКонтейнеров.Колонки.Добавить("Приоритет", Новый ОписаниеТипов("Число"));
	ТаблицаКонтейнеров.Колонки.Добавить("ВидСервисаЭДО", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыСервисовЭДО"));
	ТаблицаКонтейнеров.Колонки.Добавить("АдресРесурсаКонтрагента", Новый ОписаниеТипов("Строка"));
	ТаблицаКонтейнеров.Колонки.Добавить("ИдФайл", Новый ОписаниеТипов("Строка"));
...
Процедура ОбработатьОшибкуСервиса(ПараметрыОперации, КонтекстДиагностики, ОшибкаОператора,
	...
	Если ЭтоОшибкаРегистрацииУОператораСИзменениемФормыВеденияДеятельности(ОшибкаОператора) Тогда
		ОбработатьОшибкуРегистрацииУОператораСИзменениемФормыВеденияДеятельности(КонтекстДиагностики, ВидОперации,
			ОшибкаОператора, ЗаписыватьОшибкуВЖурналРегистрации);
		Возврат;
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаРегистрацииУОператораСИзменениемФормыВеденияДеятельности(ОшибкаОператора)
	
	Если ОшибкаОператора.Код = 6001
		И СтрНайти(ОшибкаОператора.Описание, НСтр("ru = 'изменение типа запрещено'")) > 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
//  ОшибкаОператора - См. НоваяОшибкаОператора
//  ЗаписыватьОшибкуВЖурналРегистрации - Булево
Процедура ОбработатьОшибкуРегистрацииУОператораСИзменениемФормыВеденияДеятельности(КонтекстДиагностики, ВидОперации,
	ОшибкаОператора, ЗаписыватьОшибкуВЖурналРегистрации)
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.ВидОшибкиНеизвестнаяОшибка();
	
	КраткоеПредставление = НСтр("ru = 'Запрещено использовать идентификатор ЭДО, принадлежащий организации с иной формой ведения деятельности (ИП, ФЛ).'");
	
	ТекстОшибки = ПредставлениеОшибкиОператора(ОшибкаОператора);
...
Процедура ПрочитатьКонтакт(Контакт, ТаблицаКонтактов, ИдентификаторУчетнойЗаписи)
	...
	Если Контакт.Свойства().Получить("UserType") <> Неопределено Тогда
		ТипПользователя = ?(Контакт.UserType = "FL", "ФЛ", "");
		Строка.ТипПользователя = ТипПользователя;
	КонецЕсли;
...
Функция СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатВыполненияОперации, ДатаПоследнегоЗапроса, ВидСервисаЭДО = Неопределено)
	...
	ТаблицаЭД = Результат.ЭлектронныеДокументы;
	...
	Если ЗначениеЗаполнено(СписокЭД_XML) Тогда
		...
		ТаблицаЭД.Сортировать("Приоритет");
...
Функция ДанныеЗапросаОтправкиПриглашений(Приглашения)
	...
	Попытка
		...
		Для Каждого ТекСтрока Из Приглашения Цикл
			...
			Если ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(ТекСтрока.Получатель).ЭтоФизЛицо Тогда
				ТекСтрока.ВидУчетнойЗаписи = Перечисления.ВидУчетнойЗаписиЭДО.ФизическоеЛицо;
			КонецЕсли;
			
			ТегUserType = Перечисления.ВидУчетнойЗаписиЭДО.ТегUserTypeДляКарточки(ТекСтрока.ВидУчетнойЗаписи);
			Если ЗначениеЗаполнено(ТегUserType) Тогда
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Контакт, "UserType", ТегUserType, , Ошибки);
			КонецЕсли;
...
//      ** Комментарий - Строка
//      ** СогласенНаОбменБезПриглашений - Булево
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	АктуальныйФормат = АктуальнаяВерсияФорматаРегистрацииПоОператоруЭДО(ПараметрыРегистрации.ОператорЭДО);
	
	Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.03") < 0 Тогда
	...
	Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.04") >= 0 Тогда
		РеквизитыПакета.Вставить("Комментарий",				 ПараметрыРегистрации.Комментарий);
		РеквизитыПакета.Вставить("ОбменБезПриглашений", Ложь);
	КонецЕсли;
	РеквизитыПакета.Вставить("ВидУчетнойЗаписи", ПараметрыРегистрации.ВидУчетнойЗаписи);
	...
	ЗначенияРеквизитов.Вставить("ВерсФорм", АктуальныйФормат.Версия);
	...
	Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.04") >= 0 Тогда
		...
		Если Не ПараметрыРегистрации.НеИспользоватьПриглашения = Неопределено Тогда
			ЗначенияРеквизитов.Вставить("УчастЭДО.ОбменБезПриглашений", ПараметрыРегистрации.НеИспользоватьПриглашения);
			РеквизитыПакета.ОбменБезПриглашений = ПараметрыРегистрации.НеИспользоватьПриглашения;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыРегистрации.Комментарий) Тогда
			ЗначенияРеквизитов.Вставить("УчастЭДО.Комментарий", ПараметрыРегистрации.Комментарий);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыРегистрации.КодыТиповДокументовКОбмену) Тогда
			
			ТаблицаКодов = Новый ТаблицаЗначений;
			ТаблицаКодов.Колонки.Добавить("Код", ОбщегоНазначения.ОписаниеТипаСтрока(4));
			
			Для Каждого КодТипаДокумента Из ПараметрыРегистрации.КодыТиповДокументовКОбмену Цикл
				
				СтрокаТаблицы = ТаблицаКодов.Добавить();
				СтрокаТаблицы.Код = КодТипаДокумента;
				
			КонецЦикла;
		
			ЭлектронноеВзаимодействие.ЗагрузитьТаблицуВДерево(ДеревоРегистрационнойИнформации, ТаблицаКодов,
				"УчастЭДО.ПоддерживаемыеКодыДокументов");
		
		КонецЕсли;
		
	КонецЕсли;
	
	ТипУчастникаЭДО = ИнтеграцияЭДОПовтИсп.ТипУчастникаЭДО(ПараметрыРегистрации.Организация);
	Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.03") >= 0
			И ТипУчастникаЭДО.ЭтоФизЛицо Тогда
		
		ТипУчастника = "ФЛ";
		
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ФЛ.ИННФЛ",
			ИННОрганизации);
		
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ФЛ.ФИО.Фамилия",  ФамилияБезПодчеркивания);
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ФЛ.ФИО.Имя",      ИмяБезПодчеркивания);
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ФЛ.ФИО.Отчество", ОтчествоБезПодчеркивания);
	...
	Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.03") >= 0
			И ТипУчастникаЭДО.ЭтоФизЛицо Тогда
		ЭтоПодписаниеСертификатомРуководителя = Ложь;
	Иначе
...
Функция СформироватьРегистрационнуюИнформациюCML(ДеревоДанных)
	...
	Ошибки = Неопределено; // служебная переменная для хранения списка возникших ошибок
	ИдентификаторОператора = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "ОперЭДО.ИдОперЭДО");
	АктуальныйФормат = АктуальнаяВерсияФорматаРегистрацииПоОператоруЭДО(ИдентификаторОператора);
	
	ПутьКОписанию = СтрШаблон("{%1}.Файл", АктуальныйФормат.ПространствоИмен);
	Попытка
		...
		КонецЕсли;
		Если СервисЭДОКлиентСервер.СравнитьВерсииФорматовРегистрации(АктуальныйФормат.Версия, "5.04") >= 0 Тогда
			Комментарий = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "УчастЭДО.Комментарий", Ложь);
			Если ЗначениеЗаполнено(Комментарий) Тогда
				ЗначенияСвойствXDTO.Вставить("Комментарий", "УчастЭДО.Комментарий");
			КонецЕсли;
		
			ОбменБезПриглашений = 
				ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "УчастЭДО.ОбменБезПриглашений", Ложь);
			Если Не ОбменБезПриглашений = Неопределено Тогда
				ЗначенияСвойствXDTO.Вставить("ОбменБезПриглашений", "УчастЭДО.ОбменБезПриглашений");
			КонецЕсли;
		КонецЕсли;
		ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
		ПараметрыЗаполнения.НеобязательныеСвойства = "ИдУчастЭДО, Комментарий, ОбменБезПриглашений";
		...
		Если ЗначениеЗаполнено(ТипУчастника) Тогда
			...
			Если ИнтеграцияЭДОВызовСервера.ПоддержанаРаботаСФизическимиЛицами()
					И ТипУчастника = "ФЛ" Тогда
				
				СвИП = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(ПутьКОписанию + ".Документ.УчастЭДО.ФЛ");
				ЗначенияСвойствXDTO = Новый Соответствие;
				ЗначенияСвойствXDTO.Вставить("ИННФЛ", "УчастЭДО.ТипУчастника.ФЛ.ИННФЛ");
				ЗначенияСвойствXDTO.Вставить("ДатаПодклЭДО", "УчастЭДО.ТипУчастника.ФЛ.ДатаПодклЭДО");
				ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
				...
				ФИО = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(ПутьКОписанию + ".Документ.УчастЭДО.ФЛ.ФИО");
				ЗначенияСвойствXDTO = Новый Соответствие;
				ЗначенияСвойствXDTO.Вставить("Фамилия", "УчастЭДО.ТипУчастника.ФЛ.ФИО.Фамилия");
				ЗначенияСвойствXDTO.Вставить("Имя", "УчастЭДО.ТипУчастника.ФЛ.ФИО.Имя");
				ЗначенияСвойствXDTO.Вставить("Отчество", "УчастЭДО.ТипУчастника.ФЛ.ФИО.Отчество");
				ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
				...
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(УчастникЭДО, "ФЛ", СвИП, , Ошибки);
		...
		СтрокаТаблицыКодов = ДеревоДанных.Строки.Найти("УчастЭДО.ПоддерживаемыеКодыДокументов", "ПолныйПуть", Истина);
		Если Не СтрокаТаблицыКодов = Неопределено И ЗначениеЗаполнено(СтрокаТаблицыКодов.Значение) Тогда
			
			КодыТипа = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(ПутьКОписанию + 
				".Документ.УчастЭДО.ПоддерживаемыеКодыДокументов");
			
			Для Каждого СтрокаКода Из СтрокаТаблицыКодов.Строки Цикл
				КодыТипа.Код.Добавить(СтрокаКода.Строки[0].Значение);
			КонецЦикла;
			
			УчастникЭДО.ПоддерживаемыеКодыДокументов = КодыТипа;
		...
		ДвоичныеДанныеДляРегистрации = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(Файл, Ложь);
		Возврат РегистрационныйПакетСИсправленнымПространствомИмен(
			ДвоичныеДанныеДляРегистрации, АктуальныйФормат.ПространствоИмен);
...
// Регистрационный пакет с исправленным пространством имен.
// 
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные
//  ТекущееПространствоИмен - Строка
// 
// Возвращаемое значение:
//  ДвоичныеДанные - Регистрационный пакет с исправленным пространством имен
Функция РегистрационныйПакетСИсправленнымПространствомИмен(ДвоичныеДанные, ТекущееПространствоИмен)
	
	ПространствоИмен = "http://www.1c.ru/edo/registration";
	
	СтрокаДанных = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанные, "windows-1251");
	СтрокаДанных = СтрЗаменить(СтрокаДанных, ТекущееПространствоИмен, ПространствоИмен);
	Возврат ПолучитьДвоичныеДанныеИзСтроки(СтрокаДанных, "windows-1251");
...
Процедура СоздатьФайлыОписанияПакета(РеквизитыПакета)
	...
	// Комментарий
	Если РеквизитыПакета.Свойство("Комментарий")
		И СервисНастроекЭДОПовтИсп.ОператорПоддерживаетВерсиюФорматаРегистрации(РеквизитыПакета.ОператорЭДОИд, "5.04") Тогда
		ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
		ЭДКарточки.ЗаписатьАтрибут("Name", "Комментарий");
		ЭДКарточки.ЗаписатьАтрибут("Value", РеквизитыПакета.Комментарий);
		ЭДКарточки.ЗаписатьКонецЭлемента();
	КонецЕсли;
	
	// Согласен на работу без приглашений
	Если РеквизитыПакета.Свойство("ОбменБезПриглашений")
		И СервисНастроекЭДОПовтИсп.ОператорПоддерживаетВерсиюФорматаРегистрации(РеквизитыПакета.ОператорЭДОИд, "5.04") Тогда
		ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
		ЭДКарточки.ЗаписатьАтрибут("Name", "ОбменБезПриглашений");
		ЭДКарточки.ЗаписатьАтрибут("Value", XMLСтрока(РеквизитыПакета.ОбменБезПриглашений));
		ЭДКарточки.ЗаписатьКонецЭлемента();
	КонецЕсли;
	
	ТегUserType = Перечисления.ВидУчетнойЗаписиЭДО.ТегUserTypeДляКарточки(РеквизитыПакета.ВидУчетнойЗаписи);
	Если ЗначениеЗаполнено(ТегUserType) Тогда
		ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
		ЭДКарточки.ЗаписатьАтрибут("Name", "UserType");
		ЭДКарточки.ЗаписатьАтрибут("Value", ТегUserType);
		ЭДКарточки.ЗаписатьКонецЭлемента();
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриЗаполненииПараметровКарточкиРегистрации(ЭДКарточки);
	КонецЕсли;
...
// Возвращает структуру, описывающую формат пакета регистрационных данных
//
// Возвращаемое значение:
//  Структура:
//   * Версия - Строка
//   * ИмяМакета - Строка - Имя макета дерева заполнения формата
//   * ПространствоИмен - Строка
Функция ОписаниеФорматаРегистрационногоПакета()
	
	Формат = Новый Структура();
	Формат.Вставить("Версия", "");
	Формат.Вставить("ИмяМакета", "");
...
// Возвращает актуальный формат пакета регистрации для указанного оператора ЭДО.
//
// Параметры:
//   ИдентификаторОператора - Строка - Идентификатор оператора ЭДО
//
// Возвращаемое значение:
//   Структура - см. ОписаниеФорматаРегистрационногоПакета
Функция АктуальнаяВерсияФорматаРегистрацииПоОператоруЭДО(ИдентификаторОператора)
	
	Версия = СервисНастроекЭДОПовтИсп.ВерсияПоддерживаемогоРегистрационногоПакетаОператора(ИдентификаторОператора);
	
	АктуальныйФормат = Неопределено;
	
	Если Версия = "5.02" Тогда
		// Формат регистрации 5.02
		АктуальныйФормат = ОписаниеФорматаРегистрационногоПакета();
		АктуальныйФормат.Версия = "5.02";
		АктуальныйФормат.ИмяМакета = "ОбменСКонтрагентами.РегистрационнаяИнформация";
		АктуальныйФормат.ПространствоИмен = "http://www.1c.ru/edo/registration";
		
	ИначеЕсли Версия = "5.03" Тогда
		
		// Формат регистрации 5.03
		АктуальныйФормат = ОписаниеФорматаРегистрационногоПакета();
		АктуальныйФормат.Версия = "5.03";
		АктуальныйФормат.ИмяМакета = "ОбменСКонтрагентами.РегистрационнаяИнформация_5_03";
		АктуальныйФормат.ПространствоИмен = "http://www.1c.ru/edo/registration_5_03";
		
	ИначеЕсли Версия = "5.04" Тогда
		
		// Формат регистрации 5.04
		АктуальныйФормат = ОписаниеФорматаРегистрационногоПакета();
		АктуальныйФормат.Версия = "5.04";
		АктуальныйФормат.ИмяМакета = "ОбменСКонтрагентами.РегистрационнаяИнформация_5_04";
		АктуальныйФормат.ПространствоИмен = "http://www.1c.ru/edo/registration_5_04";
...
//  АдресРесурса - Строка
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики 
//  ВидСервисаЭДО - Неопределено, ПеречислениеСсылка.ВидыСервисовЭДО - если ВидСервисаЭДО = Неопределено - это документы ЭДО
