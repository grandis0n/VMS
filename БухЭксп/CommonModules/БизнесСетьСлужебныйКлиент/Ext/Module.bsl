...
// Подготовить организацию к использованию.
// 
// Параметры:
//  Организация   - ОпределяемыйТип.Организация - Проверяемая / подключаемая организация
//  ФормаВладелец - ФормаКлиентскогоПриложения  - Активная форма из которой требуется вызвать подключение
//  Оповещение    - ОписаниеОповещения          - Обработчик завершения подключения. Параметры:
//   # ОрганизацияПодключена   - Булево       - Признак готовности организации к запросам
//   # ДополнительныеПараметры - Произвольный - произвольные данные, переданные в описании оповещения. 
Процедура ПодготовитьОрганизациюКИспользованию(
	Знач Организация, 
	Знач ФормаВладелец = Неопределено, 
	Знач Оповещение = Неопределено) Экспорт
	
	Если БизнесСетьКлиент.ОрганизацияНеПодключенаТребуетсяПовторноеПодключение(Организация) Тогда
		ДополнительныеПараметры = НовыеПараметрыПодготовитьОрганизациюКИспользованию();
		...
		ВнутреннееОповещение = Новый ОписаниеОповещения(
			"ПослеПодключенияОрганизации", ЭтотОбъект, ДополнительныеПараметры);
		ОткрытьФормуПодключенияОрганизации(Организация, ФормаВладелец, ВнутреннееОповещение);
	Иначе
...
//               - Неопределено
Функция НовыеПараметрыПодготовитьОрганизациюКИспользованию()
...
//  ДополнительныеПараметры - См. НовыеПараметрыПодготовитьОрганизациюКИспользованию
Процедура ПослеПодключенияОрганизации(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	
	ОрганизацияПодключена = Не БизнесСетьКлиент.ОрганизацияНеПодключенаТребуетсяПовторноеПодключение(
		ДополнительныеПараметры.Организация);
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Оповещение, ОрганизацияПодключена);
