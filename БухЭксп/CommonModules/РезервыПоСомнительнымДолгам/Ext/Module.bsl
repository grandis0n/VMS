...
Функция ДанныеДляНачисленияРезерва(ОтчетнаяДата, Организации, МенеджерВременныхТаблиц, ПолучатьСостояниеРезерва = Ложь, ПолучатьСостояниеРезерваНачалоГода = Ложь)
    ...
    // Получение дочерних счетов ОбеспеченияОбязательствИПлатежейПолученные  
	Запрос = Новый Запрос;
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВсеСчета.Ссылка КАК Счет
	|ИЗ
	|	ПланСчетов.Хозрасчетный КАК ВсеСчета
	|ГДЕ
	|	ВсеСчета.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОбеспеченияОбязательствИПлатежейПолученные))";
	
	СчетаОбеспеченийИОбязательствПолученные = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Счет");
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто2 КАК Справочник.ДоговорыКонтрагентов), ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)) КАК Договор,
	|	ХозрасчетныйОстатки.СуммаОстатокДт КАК СуммаОбеспечения
	|ПОМЕСТИТЬ СуммыОбеспечения
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(
	|		&ОтчетнаяДата,
	|		Счет В (&СчетаОбеспечения),
	|		&ВидыСубконтоОбеспечений,
	|		Организация В (&Организации)
	...
	|	ЕСТЬNULL(ДоговорыКонтрагентов.ВидОбеспеченияОбязательствИПлатежей, 0) КАК ВидОбеспечения,
	|	ЕСТЬNULL(СуммыОбеспечения.СуммаОбеспечения, 0) КАК СуммаОбеспечения
	|////////////////////////////////////////////////////////////////////////////////
	|ИЗ
	|	ДанныеРегистраХозрасчетный КАК ДанныеРегистраХозрасчетный
	...
	|       ЛЕВОЕ СОЕДИНЕНИЕ СуммыОбеспечения КАК СуммыОбеспечения
	|		ПО ДанныеРегистраХозрасчетный.Контрагент = СуммыОбеспечения.Контрагент
	|			И ДанныеРегистраХозрасчетный.ДоговорКонтрагента = СуммыОбеспечения.Договор
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	Контрагент,
	|	ДоговорКонтрагента,
	...
	Запрос.УстановитьПараметр("СчетаОбеспечения", СчетаОбеспеченийИОбязательствПолученные);
	Запрос.УстановитьПараметр("ВидыСубконтоОбеспечений", МассивВидовСубконто);
	Запрос.УстановитьПараметр("ОтчетнаяДата",  ОтчетнаяДата);
	Запрос.УстановитьПараметр("Организации",  Организации);
...
Функция РассчитатьРезерв(ДанныеДляНачисленияРезерва, ОтчетнаяДата, МенеджерВременныхТаблиц)
	...
	Для каждого Запись Из РасчетРезерва Цикл
		...
		Запись.СомнительнаяЗадолженность   = Макс(Запись.Задолженность - Запись.ВстречнаяЗадолженность, 0);
		Запись.СомнительнаяЗадолженностьНУ = Макс(Запись.ЗадолженностьНУ - Запись.ВстречнаяЗадолженностьНУ, 0);
		
		// Согласно абз. 1 п. 70 Положения по ведению б/у и б/о № 34н, а также п. 1 ст. 266 НК РФ,
		// задолженность, имеющая обеспечение (залог, поручительство, банковская гарантия), не признается сомнительной.
		// При наличии любого обеспечения (СуммаОбеспечения > 0 или ЗначениеЗаполнено(Запись.ВидОбеспечения)) долг полностью исключается из расчета сомнительной
		// задолженности и резерва как в бухгалтерском, так и в налоговом учете, независимо от того,
		// перекрывает ли сумма обеспечения долг частично или полностью.
		// (см. также ФНС РФ: Письмо № ЕД-4-3/9433 от 15.06.2011)
		Если Запись.СуммаОбеспечения > 0 ИЛИ ЗначениеЗаполнено(Запись.ВидОбеспечения) Тогда
			Запись.КоэффициентРезервирования = 0;
			Запись.СомнительнаяЗадолженность   = 0;
			Запись.СомнительнаяЗадолженностьНУ = 0;	
		КонецЕсли;

		Запись.Резерв   = Запись.СомнительнаяЗадолженность * Запись.КоэффициентРезервирования;
		Запись.РезервНУ = Запись.СомнительнаяЗадолженностьНУ * Запись.КоэффициентРезервирования;
...
Функция НовыйРасчетРезерваПоСомнительнымДолгам()
	...
	РасчетРезерва.Колонки.Добавить("СомнительнаяЗадолженность", ТипСумма);
	РасчетРезерва.Колонки.Добавить("СомнительнаяЗадолженностьНУ", ТипСумма);	
	РасчетРезерва.Колонки.Добавить("ВидОбеспечения", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОбеспеченийОбязательствИПлатежей"));
	РасчетРезерва.Колонки.Добавить("СуммаОбеспечения", ТипСумма);
