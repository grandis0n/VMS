...
Процедура ПолучитьСоответствиеНаименованийОбъектовМДИРеквизитов(СоответствиеРеквизитовОбъекта) Экспорт
	...
	СоответствиеРеквизитовОбъекта.Вставить("ЮрФизЛицоОрганизации"                           , "ЮридическоеФизическоеЛицо");
...
Процедура ПроверитьГотовностьИсточников(ДокументыМассив, БезЭлектроннойПодписи = Ложь) Экспорт
	...
	Если МассивСчетовФактур.Количество() > 0 Тогда
		...
		Запрос.УстановитьПараметр("ДокументыМассив", ДокументыМассив);
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СчетФактураВыданный.Ссылка КАК Ссылка,
		|	СчетФактураВыданный.Представление КАК Представление,
		|	СчетФактураВыданный.СчетФактураНеВыставляется КАК СчетФактураНеВыставляется,
		...
		|ПОМЕСТИТЬ ВТПеревыставленныеСФ
		|ИЗ
		|	Документ.ОтчетКомитентуОПродажах.Поставщики КАК ОтчетКомитентуОПродажахПоставщики
		|ГДЕ
		|	ОтчетКомитентуОПродажахПоставщики.Ссылка В(&ДокументыМассив)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТСчетаФактуры.Ссылка КАК Ссылка,
		|	ВТСчетаФактуры.Представление КАК Представление,
		|	ВЫБОР
		|		КОГДА ВТСчетаФактуры.СчетФактураНеВыставляется
		|			ТОГДА ""Не выставляется""
		|	КОНЕЦ КАК Флаг
		|ИЗ
		|	ВТСчетаФактуры КАК ВТСчетаФактуры
		|ГДЕ
		|	ВТСчетаФактуры.СчетФактураНеВыставляется
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТСчетаФактуры КАК ВТСчетаФактуры
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(ВТСчетаФактуры.ДокументОснование) = ТИП(Документ.ОтчетКомитентуОПродажах)
		|	И ВТСчетаФактуры.ДокументОснование В(&ДокументыМассив)
		|	И НЕ ВТСчетаФактуры.Ссылка В
		|				(ВЫБРАТЬ
		|					ВТПеревыставленныеСФ.СчетФактура КАК СчетФактура
		|				ИЗ
		|					ВТПеревыставленныеСФ КАК ВТПеревыставленныеСФ)";
		...
		// Если пользователь выделил сразу Отчет комитенту и Счет-фактуру на комиссионное вознаграждение в Текущих делах ЭДО и нажал Сформировать, то
		// формируем УПД 1 на основании Отчета комитенту и игнорируем Счет-фактуру. Перед этим проверяем, что Счет-фактура не является перевыставленным комиссионером.
	...
	ДокументыМассив = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ДокументыМассив, МассивНаУдаление);
