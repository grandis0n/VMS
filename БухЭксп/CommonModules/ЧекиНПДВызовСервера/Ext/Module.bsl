...
// Проверяет счет на оплату, указанный в документе для формирования чека НПД.
//
// Параметры:
//  СчетНаОплату - ДокументСсылка.СчетНаОплатуПокупателю - счет на оплату, на основании которого формируется чек
//
// Возвращаемое значение:
//  Строка - описание ошибок заполнения счета.
//
Функция ПроверитьСчетНаОплатуДляФормированияЧекаНПД(СчетНаОплату) Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если Не ЗначениеЗаполнено(Выборка.СуммаДокумента) Тогда
			Ошибки.Добавить(НСтр("ru='сумма'"));
		КонецЕсли;
		ВыборкаТовары = Выборка.Товары.Выбрать();
		Если Не ВыборкаТовары.Следующий()
			Или Не (ЗначениеЗаполнено(ВыборкаТовары.Номенклатура) Или ЗначениеЗаполнено(ВыборкаТовары.Содержание)) Тогда
			Ошибки.Добавить(НСтр("ru='номенклатура'"));
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Ошибки) Тогда
		Возврат СтрШаблон(НСтр("ru='Не заполнена %1 в Счете на оплату'"),
			СтрСоединить(Ошибки, НСтр("ru=' и '")));
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ЗапуститьОбновлениеСведенийОСформированныхЧекахНПД(ПараметрыОбновления) Экспорт
	
	Если ПараметрыОбновления.УникальныйИдентификатор = Неопределено Тогда
		ПараметрыОбновления.УникальныйИдентификатор = Новый УникальныйИдентификатор;
	КонецЕсли;
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(ПараметрыОбновления.УникальныйИдентификатор);
