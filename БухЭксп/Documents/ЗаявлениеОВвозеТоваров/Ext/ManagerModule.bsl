...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РезервированиеАкцизаПоДОПП");
	Методы.Вставить("УбратьНачислениеЕНСПоЗаявлениюОВвозе");
...
//
// Параметры:
//   ДокументСсылка - ДокументСсылка.ЗаявлениеОВвозеТоваров - документ
//   ВидОперации - ПеречисленеСсылка.ВидыОперацийВвозТоваровИзЕАЭС - вид операции документа
...
Функция ПолучитьПредставлениеДокумента(ДокументСсылка, ВидОперации) Экспорт
	
	ТекстПредставлениеКраткое  = "";
	ТекстПредставление         = "";
	Если ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП Тогда
		ТекстОсновноеПредставление = НСтр("ru = 'Документ о предстоящей поставке товаров'");
	Иначе
		ТекстОсновноеПредставление = НСтр("ru = 'Заявление о ввозе товаров'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДокументСсылка) Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Номер, Дата");
		ТекстПредставлениеКраткое = СтрШаблон(НСтр("ru='%1 от %2'"),
			РеквизитыДокумента.Номер, Формат(РеквизитыДокумента.Дата, "ДЛФ=Д"));
		ТекстПредставление = СтрШаблон(НСтр("ru='%1 %2'"), ТекстОсновноеПредставление, ТекстПредставлениеКраткое);
	Иначе
		ТекстПредставление = СтрШаблон(НСтр("ru='%1 (создание)'"), ТекстОсновноеПредставление);
	КонецЕсли;
...
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура УбратьНачислениеЕНСПоЗаявлениюОВвозе(Параметры) Экспорт
	
	КодыВидаАвтоТранспорта = Новый Массив();
	КодыВидаАвтоТранспорта.Добавить("30");
	КодыВидаАвтоТранспорта.Добавить("31");
	КодыВидаАвтоТранспорта.Добавить("32");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("КодыВидаАвтоТранспорта", КодыВидаАвтоТранспорта);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаявленияСАкцизами.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗаявленияСАкцизами
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ЗаявленияСАкцизами
	|ГДЕ
	|	ЗаявленияСАкцизами.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе)
	|	И ЗаявленияСАкцизами.ДатаПринятияНаУчет >= ДАТАВРЕМЯ(2026, 7, 1)
	|	И ЗаявленияСАкцизами.ПодакцизныйТовар
	|	И НЕ ЗаявленияСАкцизами.МаркируемаяПродукция
	|	И ЗаявленияСАкцизами.КБК = """"
	|	И ЗаявленияСАкцизами.КодВидаПодакцизногоТовара = ЗНАЧЕНИЕ(Справочник.КодыВидовПодакцизныхТоваров.ПустаяСсылка)
	|	И ЗаявленияСАкцизами.СуммаАкциза > 0
	|	И ЗаявленияСАкцизами.Ссылка.Проведен
	|	И НЕ ЗаявленияСАкцизами.Ссылка.РучнаяКорректировка
	|	И НЕ ЗаявленияСАкцизами.Ссылка.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаявленияИсключения.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗаявленияИсключения
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ЗаявленияИсключения
	|ГДЕ
	|	ЗаявленияИсключения.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе)
	|	И ЗаявленияИсключения.ДатаПринятияНаУчет >= ДАТАВРЕМЯ(2026, 7, 1)
	|	И НЕ ЗаявленияИсключения.КодВидаТранспорта В (&КодыВидаАвтоТранспорта)
	|	И ЗаявленияИсключения.Ссылка.Проведен
	|	И НЕ ЗаявленияИсключения.Ссылка.РучнаяКорректировка
	|	И НЕ ЗаявленияИсключения.Ссылка.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаявленияИсключения.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаявленияБезАкцизов.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗаявленияБезАкцизов
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ЗаявленияБезАкцизов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаявленияИсключения КАК ЗаявленияИсключения
	|		ПО (ЗаявленияИсключения.Ссылка = ЗаявленияБезАкцизов.Ссылка)
	|ГДЕ
	|	ЗаявленияБезАкцизов.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе)
	|	И ЗаявленияИсключения.Ссылка ЕСТЬ NULL
	|	И ЗаявленияБезАкцизов.ДатаПринятияНаУчет >= ДАТАВРЕМЯ(2026, 7, 1)
	|	И НЕ ЗаявленияБезАкцизов.ПодакцизныйТовар
	|	И ЗаявленияБезАкцизов.СуммаНДС > 0
	|	И ЗаявленияБезАкцизов.Ссылка.Проведен
	|	И НЕ ЗаявленияБезАкцизов.Ссылка.РучнаяКорректировка
	|	И НЕ ЗаявленияБезАкцизов.Ссылка.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаявленияБезАкцизов.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ХозрасчетныйОбороты.Регистратор КАК ЗаявлениеОВвозе
	|ПОМЕСТИТЬ НачислениеНаЕНС
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет), , , КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате), ) КАК ХозрасчетныйОбороты
	|ГДЕ
	...
	|ПОМЕСТИТЬ ЗаявленияКОбработке
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЗаявленияБезАкцизов.Ссылка КАК Ссылка
	|	ИЗ
	|		ЗаявленияБезАкцизов КАК ЗаявленияБезАкцизов
	|			ЛЕВОЕ СОЕДИНЕНИЕ НачислениеНаЕНС КАК НачислениеНаЕНС
	|			ПО (НачислениеНаЕНС.ЗаявлениеОВвозе = ЗаявленияБезАкцизов.Ссылка)
	|	ГДЕ
	|		НЕ НачислениеНаЕНС.ЗаявлениеОВвозе ЕСТЬ NULL
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ЗаявленияСАкцизами.Ссылка
	|	ИЗ
	|		ЗаявленияСАкцизами КАК ЗаявленияСАкцизами) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ЗаявленияКОбработке.Ссылка КАК Ссылка
	|ИЗ
	|	ЗаявленияКОбработке КАК ЗаявленияКОбработке
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗаявленияКОбработке.Ссылка.Дата УБЫВ";
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеОбъекта.ЕстьУчетПодакцизныхТоваров = Истина;
		
		Для каждого СтрокаТаблицы Из ДокументОбъект.Товары Цикл
			
			Если Не СтрокаТаблицы.ПодакцизныйТовар Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаТаблицы.КБК) И ЗначениеЗаполнено(СтрокаТаблицы.КодВидаПодакцизногоТовара) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СведенияОНоменклатуре.КодВидаПодакцизногоТовара) Тогда
				СтрокаТаблицы.КодВидаПодакцизногоТовара = СведенияОНоменклатуре.КодВидаПодакцизногоТовара;
				СтрокаТаблицы.КБК                       = КБКПоКодуВидаПодакцизногоТовара(СведенияОНоменклатуре.КодВидаПодакцизногоТовара);
				СтрокаТаблицы.МаркируемаяПродукция      = СведенияОНоменклатуре.МаркируемаяПродукция;
			Иначе
				// Сбросим признак подакцизного товара чтобы не зациклить обработчик в случае некорректных данных.
		...
		Попытка
			// Запись обработанного объекта (перепроведение).
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.УбратьНачислениеЕНСПоЗаявлениюОВвозе() не удалось обработать документ ""Ввоз товаров из ЕАЭС"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ЗаявлениеОВвозеТоваров, Выборка.Ссылка, ТекстСообщения);
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.УбратьНачислениеЕНСПоЗаявлениюОВвозе() не удалось обработать документ ""Ввоз товаров из ЕАЭС"": в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ЗаявлениеОВвозеТоваров,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ЗаявлениеОВвозеТоваров.УбратьНачислениеЕНСПоЗаявлениюОВвозе() обработала очередную порцию документов ""Ввоз товаров из ЕАЭС"": %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
...
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура РезервированиеАкцизаПоДОПП(Параметры) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДОППТовары.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ДОППСАкцизами
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ДОППТовары
	|ГДЕ
	|	ДОППТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП)
	|	И ДОППТовары.Ссылка.ДатаВвоза >= ДАТАВРЕМЯ(2026, 7, 1)
	|	И ДОППТовары.ПодакцизныйТовар
	|	И НЕ ДОППТовары.МаркируемаяПродукция
	|	И ДОППТовары.ОбеспечительныйПлатеж > 0
	|	И ДОППТовары.ОбеспечительныйПлатеж > ДОППТовары.СуммаНДС
	|	И ДОППТовары.Ссылка.Проведен
	|	И НЕ ДОППТовары.Ссылка.РучнаяКорректировка
	|	И НЕ ДОППТовары.Ссылка.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ХозрасчетныйОбороты.Субконто1 КАК ДОПП
	|ПОМЕСТИТЬ РезервированиеАкциза
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(
	|			,
	|			,
	|			Регистратор,
	|			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате),
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СФПолученные),
	|			Субконто1 В
	|				(ВЫБРАТЬ
	|					ДОППСАкцизами.Ссылка
	|				ИЗ
	|					ДОППСАкцизами),
	|			,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ДОППСАкцизами.Ссылка КАК Ссылка
	|ИЗ
	|	ДОППСАкцизами КАК ДОППСАкцизами
	|		ЛЕВОЕ СОЕДИНЕНИЕ РезервированиеАкциза КАК РезервированиеАкциза
	|		ПО (РезервированиеАкциза.ДОПП = ДОППСАкцизами.Ссылка)
	|ГДЕ
	|	РезервированиеАкциза.ДОПП ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДОППСАкцизами.Ссылка.Дата УБЫВ";
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.РезервированиеАкцизаПоДОПП() не удалось обработать документ ""Ввоз товаров из ЕАЭС"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ЗаявлениеОВвозеТоваров, Выборка.Ссылка, ТекстСообщения);
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.РезервированиеАкцизаПоДОПП() не удалось обработать документ ""Ввоз товаров из ЕАЭС"": в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ЗаявлениеОВвозеТоваров,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ЗаявлениеОВвозеТоваров.РезервированиеАкцизаПоДОПП() обработала очередную порцию документов ""Ввоз товаров из ЕАЭС"": %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	// С даты начала ведения простого учета ЕНП дату принятия товаров на учет определяем по дате партии.
	Запрос.УстановитьПараметр("ВедетсяПростойУчетЕНП", Реквизиты.Период >= ЕдиныйНалоговыйСчет.НачалоПростогоУчета());
	...
	Запрос.УстановитьПараметр("СписокКодов", УчетАкцизов.КодыВидаПодакцизныхТоваровРаздел3());
	...
	Запрос.Текст = ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
		...
		+ ТекстЗапросаСозданиеНалогов(НомераТаблиц, ПараметрыПроведения, Реквизиты);
...
Процедура СформироватьПроводкуРезервированиеОбеспечительногоПлатежа(ТаблицаРезервированиеОбеспечительногоПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ТаблицаРезервированиеОбеспечительногоПлатежа.Свернуть("ДОПП", "Акциз,НДС");
	СуммаНДС   = Окр(ТаблицаРезервированиеОбеспечительногоПлатежа.Итог("НДС"));
	СуммаАкциз = Окр(ТаблицаРезервированиеОбеспечительногоПлатежа.Итог("Акциз"));
	
	Если Реквизиты.СуммаДокумента <> СуммаНДС + СуммаАкциз Тогда
		СуммаАкциз = Реквизиты.СуммаДокумента - СуммаНДС;
	КонецЕсли;
	...
	Если СуммаАкциз > 0 Тогда
		...
		Проводка.СчетДт = ПланыСчетов.Хозрасчетный.АкцизТаможенныйСоюзКУплате; // 68.43
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
			"Контрагенты", Реквизиты.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
			"Договоры", Реквизиты.ДоговорКонтрагента);
		...
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			Для каждого СтрокаТоваров Из НайденныеСтрокиТовары Цикл
				...
				Если БезАкцизов Тогда
					...
					КонецЕсли;
				ИначеЕсли СтрокаТоваров.МаркируемаяПродукция Тогда
					// По подакцизным товарам, по которым акцизы взимаются иным уполномоченным органом, сумма акцизов, уплаченная в бюджет государства-члена, 
					// указывается в графе 19. При этом в графах 14, 16 и 17 проставляется прочерк.
					МногострочнаяЧасть.Параметры.П000010000114 = "--";
					МногострочнаяЧасть.Параметры.П000010000116 = "--";
					МногострочнаяЧасть.Параметры.П000010000117 = "--";
				КонецЕсли;
...
Функция ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	|	Товары.ДатаСчетаФактуры КАК ДатаСчетаФактуры,
	|	Товары.НалоговаяБазаНДС - Товары.СуммаАкциза КАК ЦенаТовараВалРубль,
	|	ВЫБОР
...
Процедура ЗаполнитьАналитикуАкцизов(СтрокаТабличнойЧасти) Экспорт
...
Функция ВидНалогаПоКБК(КБК) Экспорт
	...
	СоответствиеВидовНалогов = Новый Соответствие;
	
	СоответствиеВидовНалогов.Вставить("18210402011011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СпиртПищевой);
	СоответствиеВидовНалогов.Вставить("18210402012011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СпиртПищевойДистилляты);
	СоответствиеВидовНалогов.Вставить("18210402013011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СпиртНепищевой);
	СоответствиеВидовНалогов.Вставить("18210402020011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СпиртосодержащаяПродукция);
	СоответствиеВидовНалогов.Вставить("18210402021011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_ВиноградноеСусло);
	СоответствиеВидовНалогов.Вставить("18210402030011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Табак);
	СоответствиеВидовНалогов.Вставить("18210402040011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Бензин);
	СоответствиеВидовНалогов.Вставить("18210402060011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Автомобили);
	СоответствиеВидовНалогов.Вставить("18210402070011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Дизель);
	СоответствиеВидовНалогов.Вставить("18210402080011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_МоторныеМасла);
	СоответствиеВидовНалогов.Вставить("18210402090011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Вина);
	СоответствиеВидовНалогов.Вставить("18210402091011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_ВинаИгристые);
	СоответствиеВидовНалогов.Вставить("18210402100011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Пиво);
	СоответствиеВидовНалогов.Вставить("18210402110011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_АлкогольСвыше9);
	СоответствиеВидовНалогов.Вставить("18210402120011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_Сидр);
	СоответствиеВидовНалогов.Вставить("18210402130011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_АлкогольДо9);
	СоответствиеВидовНалогов.Вставить("18210402140011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_БензинПрямогонный);
	СоответствиеВидовНалогов.Вставить("18210402170011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СредниеДистилляты);
	СоответствиеВидовНалогов.Вставить("18210402190011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_НикотиносодержащиеЖидкости);
	СоответствиеВидовНалогов.Вставить("18210402200011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_ТабакНагреваемый);
	СоответствиеВидовНалогов.Вставить("18210402210011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_СахоросодержащиеНапитки);
	СоответствиеВидовНалогов.Вставить("18210402220011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_ФармацевтическаяСубстанция);
	СоответствиеВидовНалогов.Вставить("18210402230011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_НикотиновоеСырье);
	СоответствиеВидовНалогов.Вставить("18210402240011000110", Перечисления.ВидыНалогов.АкцизыЕАЭС_БестабачнаяНикотиноваяСмесь);
	
	ВидНалога = СоответствиеВидовНалогов.Получить(СокрЛП(КБК));
...
Процедура СоздатьНалогиПоКБК(ТаблицаСозданиеНалогов) Экспорт
	
	Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.НДС_ВвозимыеТовары);
	
	Для каждого СтрокаТаблицы Из ТаблицаСозданиеНалогов Цикл
		ВидНалога = ВидНалогаПоКБК(СтрокаТаблицы.КБК);
...
Функция ТекстЗапросаТаблицаТовары()
	...
	|	ЗаявлениеТовары.КодВидаТранспорта КАК КодВидаТранспорта
	|ИЗ
...
Функция ТекстЗапросаТаблицаНачисленияНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР 
	|		КОГДА ТаблицаТовары.КодВидаТранспорта В (&КодыВидаАвтоТранспорта) 
	|			ТОГДА ИСТИНА 
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ КАК ПрименяетсяДОПП
	|ИЗ
...
Функция ТекстЗапросаАкцизы(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|		КОНЕЦ, МЕСЯЦ) КАК ДатаПринятияНаУчет,
	|	ЗаявлениеОВвозеТоваровТовары.МаркируемаяПродукция КАК МаркируемаяПродукция,
	|	ЗаявлениеОВвозеТоваровТовары.Количество КАК Количество,
	|	ВЫБОР
	|		КОГДА ЗаявлениеОВвозеТоваровТовары.Номенклатура.КодВидаПодакцизногоТовара.Код В (&СписокКодов)
	|				И НЕ ЗаявлениеОВвозеТоваровТовары.МаркируемаяПродукция
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДекларацийПоПодакцизнымТоварам.ДекларацияКосвенныеНалогиТС)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВидДекларации,
	|	ЗаявлениеОВвозеТоваровТовары.Номенклатура.КодВидаПодакцизногоТовара КАК КодВидаПодакцизногоТовара,
	|	ВЫБОР
	|		КОГДА ЗаявлениеОВвозеТоваровТовары.КодВидаТранспорта В (&КодыВидаАвтоТранспорта)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПрименяетсяДОПП
	|ИЗ
...
Функция ТекстЗапросаРезервированиеОбеспечительногоПлатежа(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СуммыПоДОПП.Ссылка КАК ДОПП,
	|	ВЫБОР 
	|		КОГДА СуммыПоДОПП.ОбеспечительныйПлатеж = 0 ТОГДА 
	|			0 
	|		ИНАЧЕ СуммыПоДОПП.ОбеспечительныйПлатеж - СуммыПоДОПП.СуммаНДС 
	|	КОНЕЦ КАК Акциз,
	|	ВЫБОР 
	|		КОГДА СуммыПоДОПП.ОбеспечительныйПлатеж = 0 ТОГДА 
	|			0 
	|		ИНАЧЕ СуммыПоДОПП.СуммаНДС 
	|	КОНЕЦ КАК НДС
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК СуммыПоДОПП
	|ГДЕ
	|	СуммыПоДОПП.Ссылка = &Ссылка";
	
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
КонецФункции

Функция ТекстЗапросаСозданиеНалогов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	НомераТаблиц.Вставить("ТаблицаСозданиеНалогов", НомераТаблиц.Количество());
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВвозИзЕАЭСТовары.КБК КАК КБК
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ВвозИзЕАЭСТовары
	|ГДЕ
	|	ВвозИзЕАЭСТовары.Ссылка = &Ссылка
	|	И ВвозИзЕАЭСТовары.КБК <> """"";
