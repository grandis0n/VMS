...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УказываетсяКБК = ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП
		Или Не ЗначениеЗаполнено(ДатаПринятияНаУчет)
		Или УчетНДС.НачислениеКосвенныхНалоговНаЕНСРегламентнымДокументом(ДобавитьМесяц(ДатаПринятияНаУчет, 1));
...
Процедура КБКНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КБК) Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", СокрЛП(КБК)));
	Иначе
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", КБКНДСПриВвозеТоваров));
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыборКБКЗавершение", ЭтотОбъект);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаДОПП.Видимость = Форма.ВидАкциза <> 0 И Форма.УказываетсяКБК;
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если Не ЗначениеЗаполнено(КБК) И УказываетсяКБК Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
	...
	Если Не ЗначениеЗаполнено(КодВидаПодакцизногоТовара) И УказываетсяКБК Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
...
&НаКлиенте
Процедура ВыборКБКЗавершение(РезультатВыбора, ДопПараметры) Экспорт
	
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КБК = РезультатВыбора.Код;
