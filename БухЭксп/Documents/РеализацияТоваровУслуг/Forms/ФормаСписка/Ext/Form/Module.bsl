...
Процедура АдаптироватьМКСписокФормы()
	...
	Если ЭлектронноеВзаимодействиеБП.НастроенОбменЭДО() Тогда
		Если Элементы.Найти("СостояниеЭДО") <> Неопределено Тогда
			Элементы.СостояниеЭДО.Вид = ВидПоляФормы.ПолеНадписи;
			Элементы.СостояниеЭДО.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
			Элементы.СостояниеЭДО.ЦветТекста = ЦветаСтиля.ЦветТекстаСтрокаСпискаМК;
		КонецЕсли;
