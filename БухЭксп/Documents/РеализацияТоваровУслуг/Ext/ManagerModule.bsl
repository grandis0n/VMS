...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	// расчеты по оплате через платежного агента (комиссионера)
	...
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "СчетУчетаРасчетовСКомиссионеромПоАвансам", "АвансыПокупателя");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ТребуетсяУчетАвансов");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ПродажаТоваровСоСкладаКомиссионера");
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц, ЕстьПередачаТоваровВСоставеРабот = Ложь) Экспорт
	...
	|	Реквизиты.ПередачаДляСобственныхНуждИСМП КАК ПередачаДляСобственныхНуждИСМП
	|ИЗ
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьПередачаДляСобственныхНуждИСМП");
...
Функция СоздатьДокументИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("Дата") Тогда
		...
		Если ДанныеФайла.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonВыкуп()
			Или ДанныеФайла.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
...
Функция СоздатьПередачуТовараНаРеализацию(ДанныеСтруктуры, ПротоколСоздания) Экспорт
	
	Организация = ДанныеСтруктуры.ДанныеДокумента.Получатель.Ссылка;
	Контрагент = ДанныеСтруктуры.Контрагент;
	Склад = ДанныеСтруктуры.ДанныеДокумента.Склад;
	НомерДокумента = ДанныеСтруктуры.ДанныеДокумента.Номер;
	ДатаДокумента = ДанныеСтруктуры.ДанныеДокумента.Дата;
	ТаблицаТоваров = ДанныеСтруктуры.ДанныеДокумента.ТаблицаТоваров;
	Если ДанныеСтруктуры.ДанныеДокумента.Свойство("Ссылка") Тогда
	...
	ДанныеЗаполнения.Вставить("ВидОперации", Перечисления.ВидыОперацийРеализацияТоваров.ПередачаТоваров);
...
Функция ТекстЗапросаДанныеОГрузе() Экспорт

	Текст = 
	"ВЫБРАТЬ
	|	СУММА(СправочникНоменклатура.Вес * РеализацияТоваровУслугТовары.Количество) КАК Вес,
	|	СУММА(СправочникНоменклатура.Объем * РеализацияТоваровУслугТовары.Количество) КАК Объем,
	|	СправочникНоменклатура.НасыпнойНаливной КАК НасыпнойНаливной,
	|	СправочникНоменклатура.ТипГруза КАК ГрузСсылка,
	|	РеализацияТоваровУслугТовары.Сумма КАК СтоимостьГруза
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	СправочникНоменклатура.НасыпнойНаливной,
	|	СправочникНоменклатура.ТипГруза,
	|	РеализацияТоваровУслугТовары.Сумма";
...
Функция ТекстЗапросаАкцизы(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА РеализацияТоваровУслугТовары.Номенклатура.КодВидаПодакцизногоТовара.ВидТовара В (ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.Спирт), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СпиртосодержащаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.АлкогольнаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СахаросодержашиеНапитки))
	|				И РеализацияТоваровУслугТовары.Ссылка.Контрагент.СтранаРегистрации.Код = ""643""
	...
	|	РеализацияТоваровУслугТовары.Номенклатура.КодВидаПодакцизногоТовара КАК КодВидаПодакцизногоТовара,
	|	РеализацияТоваровУслугТовары.Ссылка.Контрагент.СтранаРегистрации.Код КАК СтранаРегистрации,
	...
	|	РеализацияТоваровУслугТовары.Номенклатура,
	|	РеализацияТоваровУслугТовары.Номенклатура.КодВидаПодакцизногоТовара,
	|	ВЫБОР
	|		КОГДА РеализацияТоваровУслугТовары.Номенклатура.КодВидаПодакцизногоТовара.ВидТовара В (ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.Спирт), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СпиртосодержащаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.АлкогольнаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СахаросодержашиеНапитки))
	|				И РеализацияТоваровУслугТовары.Ссылка.Контрагент.СтранаРегистрации.Код = ""643""
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДекларацийПоПодакцизнымТоварам.ДекларацияАкцизыАлко)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	РеализацияТоваровУслугТовары.Ссылка.Контрагент.СтранаРегистрации.Код";
...
Процедура ЗаполнитьПередачаДляСобственныхНуждИСМП(Параметры) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СтатусыКонтрагентовИСМП.Контрагент КАК Контрагент
	|ПОМЕСТИТЬ ВТ_Контрагенты
	|ИЗ
	|	РегистрСведений.СтатусыКонтрагентовИСМП КАК СтатусыКонтрагентовИСМП
	|
	|СГРУППИРОВАТЬ ПО
	|	СтатусыКонтрагентовИСМП.Контрагент
	|
	|ИМЕЮЩИЕ
	|	МИНИМУМ(СтатусыКонтрагентовИСМП.СтатусУчастника) = 2
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		ПО РеализацияТоваровУслуг.Контрагент = ВТ_Контрагенты.Контрагент
	|ГДЕ
	|	НЕ РеализацияТоваровУслуг.ПередачаДляСобственныхНуждИСМП
	|	И НЕ ВТ_Контрагенты.Контрагент ЕСТЬ NULL
	|	И &КурсорСсылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДокументСсылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Параметры.Вставить("КурсорСсылка", Выборка.ДокументСсылка);
			
			ДокументОбъект.ПередачаДляСобственныхНуждИСМП = Истина;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документ.РеализацияТоваровУслуг.ЗаполнитьПередачаДляСобственныхНуждИСМП() не удалось обработать документ ""Реализация товаров и услуг"" по причине:
				|%1'"""),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документ.РеализацияТоваровУслуг.ЗаполнитьПередачаДляСобственныхНуждИСМП не удалось обработать документ ""Реализация товаров и услуг"": в %1 из %2 возникли ошибки'"""),
			ПроблемныхОбъектов,
			ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(
			...
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ЗаполнитьПередачаДляСобственныхНуждИСМП обработала очередную порцию документов ""Реализация товаров и услуг"": %1 документов'"""), 
				ОбъектовОбработано));
	КонецЕсли;
