...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УчетАкцизов.СформироватьДвиженияОперацииСПодакцизнымиТоварами(
		ПараметрыПроведения.ТаблицаАкцизы,
		ПараметрыПроведения.ТаблицаРеквизитыАкцизы,
		Движения);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если Не Патент.Пустая() И Не УчетПСН.ПатентДоступенДляВыбора(Патент, Организация, Дата) Тогда
		ДеятельностьНаПатентеПоУмолчанию = УчетПСН.ДеятельностьНаПатентеПоУмолчанию(Организация, Дата);
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДеятельностьНаПатентеПоУмолчанию);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	УстановитьСтатусКонтрагентаИСМП();
...
Процедура УстановитьСтатусКонтрагентаИСМП()
	...
	Если ПередачаДляСобственныхНуждИСМП Тогда
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Контрагент      = Контрагент;
		НоваяЗапись.СтатусУчастника = 2;
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументуОснованию(МассивОснований)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		...
		ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, 
			"АдресДоставки, АдресДоставкиЗначение, СтруктурнаяЕдиница, СпособДоставки, СпособДоставки.Контрагент, Дата");
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеОснования, "СпособДоставки, АдресДоставки, АдресДоставкиЗначение");
		
		РеализацияТоваровУслугФормы.ПроверитьИзменитьАдресДоставки(ЭтотОбъект, ДанныеОснования.Дата);
