...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ФормаБезОбработки") Тогда
		...
		Иначе
			...
			ДобавитьПолеДляЕгаис();
			
			Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда 
				Объект.ИдентификаторОформителя = Неопределено;
...
&НаСервере
Процедура ДобавитьПолеДляЕгаис()
	
	ИдентификаторИнформацииЕгаис = "ЕГАИС";
	ИндексИнформацииЕгаис = 0;
	ИнформацииЕгаис = Новый Структура;
	
	ДопИнф = ОбменСГИСЭПДКлиентСервер.МассивСтрокТаблицыИзСтруктурыФормы(ЭтотОбъект, "ТитулОформлениеДополнительнаяИнформация");   
	
	Индекс = 0;
	Для Каждого ИнфПоле Из ДопИнф Цикл
		Индекс = Индекс + 1;
		Если ИнфПоле.Свойство("ИдентификаторИнформации") 
			И ВРЕГ(ИнфПоле.ИдентификаторИнформации) = ИдентификаторИнформацииЕгаис Тогда
			ИндексИнформацииЕгаис = Индекс;
			ИнформацииЕгаис = ИнфПоле;
			Прервать;
		КонецЕсли;
	КонецЦикла; 
	
	Если ИндексИнформацииЕгаис <= 0 Тогда
		ИндексИнформацииЕгаис = 1;
	КонецЕсли;
			
	КонтейнерОтображаемойВерсии = Неопределено; 
	
	Префикс = "ТитулОформлениеДополнительнаяИнформация__" + ИндексИнформацииЕгаис + "__";
	
	Если Не ИнформацииЕгаис.Свойство("ИдентификаторСтрокиРодителя") Тогда
			
		ИдентификаторСтрокиИнфПолей = "";		
		
		ИнфПоля = ОбменСГИСЭПДКлиентСервер.МассивСтрокТаблицыИзСтруктурыФормы(ЭтотОбъект, "ТитулОформлениеИнформационныеПоля");
		
		Если ИнфПоля.Количество() > 0 Тогда 
			
			ИдентификаторСтрокиИнфПолей = ИнфПоля[0].ИдентификаторСтроки
		...
		Иначе
			...
			ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
				"ТитулОформлениеИнформационныеПоля__1__ИдентификаторСтроки", 
				ИдентификаторСтрокиИнфПолей, 
				Ложь, КонтейнерОтображаемойВерсии); 
		КонецЕсли;
		
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			Префикс +  "ИдентификаторСтрокиРодителя", 
			ИдентификаторСтрокиИнфПолей, 
			Ложь, КонтейнерОтображаемойВерсии);
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
		Префикс + "ИдентификаторИнформации", 
		ИдентификаторИнформацииЕгаис, 
		Ложь, КонтейнерОтображаемойВерсии);
	...
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
		Префикс + "Квалификаторы", 
		20, 
		Ложь, КонтейнерОтображаемойВерсии);
		
	ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
		Префикс + "Заголовок", 
		"Идентификатор заявки ЕГАИС", 
...
Процедура ЗаполнитьУчастников(ЭтоНастройкаПередОтправкой = Ложь)
	...
	Если Объект.ЭтоВходящий = Ложь Тогда
		...
		КлючНастроекОтправкиМед.ВсегдаСоздаватьАвтоматическоеПриглашение = Истина;
		...
		Если ЗначениеЗаполнено(Объект.СсылкаТитулОформлениеТехконтроль) Тогда
			...
			КлючНастроекОтправкиТК.ВсегдаСоздаватьАвтоматическоеПриглашение = Истина;
		...
		Если ЗначениеЗаполнено(Объект.СсылкаТитулОформлениеПоказанияОдометра) Тогда
			...
			КлючНастроекОтправкиОД.ВсегдаСоздаватьАвтоматическоеПриглашение = Истина;
