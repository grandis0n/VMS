...
Процедура УстановитьПериодСверки(Документ, ДатаНачала, ДатаОкончания) Экспорт
	...
	Если Не ЗначениеЗаполнено(Документ.Контрагент) Или Не ЗначениеЗаполнено(Документ.Организация) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДанныеПредставителяОрганизации(Шапка)
	...
	Если ДанныеПредставителяОрганизации.Свойство("ФИОПолные") И ЗначениеЗаполнено(ДанныеПредставителяОрганизации.ФИОПолные) Тогда
		ФИОРодительныйПадеж = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ПолучитьСклоненияСтроки(ДанныеПредставителяОрганизации.ФИОПолные, , "ПД=Родительный")[0]);
	КонецЕсли;
	ДанныеПредставителяОрганизации.ФИОРодительныйПадеж = ФИОРодительныйПадеж;
