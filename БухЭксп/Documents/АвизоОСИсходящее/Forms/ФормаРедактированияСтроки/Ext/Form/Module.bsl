...
Процедура ОсновноеСредствоПриИзмененииНаСервере()
	...
	ТаблицаСписокОС = Новый ТаблицаЗначений;
	ТаблицаСписокОС.Колонки.Добавить("ОсновноеСредство", Новый ОписаниеТипов("СправочникСсылка.ОсновныеСредства"));
	ТаблицаСписокОС.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число"));
	
	СтрокаОС = ТаблицаСписокОС.Добавить();
	СтрокаОС.ОсновноеСредство = ОсновноеСредство;
	
	Объект = Новый Структура("Ссылка, Дата, Организация", Ссылка, Дата, Организация);
	
	РезультатЗапроса = Документы.АвизоОСИсходящее.ПолучитьДанныеОС(Объект, ТаблицаСписокОС);
    Если НЕ РезультатЗапроса.Пустой() Тогда
		
		ВыборкаОС = РезультатЗапроса.Выбрать();
		Если ВыборкаОС.Следующий() Тогда		
			ЗаполнитьЗначенияСвойств(ЭтаФорма, ВыборкаОС);
		КонецЕсли;
