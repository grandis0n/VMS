...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц, ЕстьПередачаТоваровВСоставеРабот = Ложь) Экспорт
	...
	|	ЕСТЬNULL(РеализацияТоваровУслуг.ПередачаДляСобственныхНуждИСМП, ЛОЖЬ) КАК ПередачаДляСобственныхНуждИСМП,
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|			ИЛИ КорректировкаРеализации.ДатаИсходногоДокумента > &УсловиеПоДате)
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	//Если ставка налога увеличилась, исправление нельзя переносить на текущий период
	ОтражатьВНалоговомУчете  = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Реквизиты.Организация, Реквизиты.Период);
	ПрошлыйПериод            = Год(ДанныеДокументаРеализацииСсылка.Дата) < Год(Реквизиты.Период);
	Если ПрошлыйПериод И ОтражатьВНалоговомУчете Тогда
		СтавкаНалогаУвеличилась = НалоговыйУчет.НельзяПеренестиНаТекущийПериод(Реквизиты.Организация, ДанныеДокументаРеализацииСсылка.Дата, Реквизиты.Период, 
			Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение);
	Иначе
		СтавкаНалогаУвеличилась  = Ложь;
	КонецЕсли;
	Запрос.УстановитьПараметр("СтавкаНалогаУвеличилась", СтавкаНалогаУвеличилась);
...
Функция ТекстЗапросаСписаниеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	&СтавкаНалогаУвеличилась КАК СтавкаНалогаУвеличилась,
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА &ЭтоОтгрузкаБезПереходаПраваСобственности И НЕ Реквизиты.ПризнаватьЗачитыватьАванс ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.СпособыЗачетаАвансов.НеЗачитывать)
...
Функция ТекстЗапросаРублевыеСуммыДокументовВВалюте(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВалютаВзаиморасчетов <> Реквизиты.ВалютаРеглУчета Тогда
		...
		Если Реквизиты.ЕстьТовары Тогда
			...
			|	ТаблицаТовары.СуммаРубДоИзменения * (-1) КАК СуммаБУ,
			|	ТаблицаТовары.СуммаНДСРубДоИзменения * (-1) КАК СуммаНДСРуб,
			|	ТаблицаТовары.СуммаБезНДСРубДоИзменения * (-1) КАК СуммаБезНДСРуб,
			|	0,
			|	0,
			|	0,
			|	0
		...
		Если Реквизиты.ЕстьУслуги Тогда
			...
			|	"""" Как ИдентификаторСтроки,
			|	"""" Как ПрослеживаемыйКомплект,
			|	ТаблицаУслуги.СуммаРубДоИзменения * (-1) КАК СуммаБУ,
			|	ТаблицаУслуги.СуммаНДСРубДоИзменения * (-1) КАК СуммаНДСРуб,
			|	ТаблицаУслуги.СуммаБезНДСРубДоИзменения * (-1) КАК СуммаБезНДСРуб,
			|	0,
			|	0,
			|	0,
			|	0
		...
		Если Реквизиты.ЕстьАгентскиеУслуги Тогда
			...
			|	"""" Как ИдентификаторСтроки,
			|	"""" Как ПрослеживаемыйКомплект,
			|	ТаблицаАгентскиеУслуги.СуммаРубДоИзменения * (-1) КАК СуммаБУ,
			|	ТаблицаАгентскиеУслуги.СуммаНДСРубДоИзменения * (-1) КАК СуммаНДСРуб,
			|	ТаблицаАгентскиеУслуги.СуммаБезНДСРубДоИзменения * (-1) КАК СуммаБезНДСРуб,
			|	0,
			|	0,
			|	0,
			|	0
...
Функция СоздатьКорректировкуПередачиТовараНаРеализацию(ДанныеСтруктуры, СоздатьНовыйДокумент, ПротоколСоздания) Экспорт
	
	Организация = ДанныеСтруктуры.ДанныеДокумента.Получатель.Ссылка;
	Контрагент = ДанныеСтруктуры.Контрагент;
	ДатаДокумента = ДанныеСтруктуры.ДанныеДокумента.Дата;
	ТаблицаТоваров = ДанныеСтруктуры.ДанныеДокумента.ТаблицаТоваров;
	ДокументСсылка = ДанныеСтруктуры.ДанныеДокумента.Ссылка;
	...
	Иначе
		НовыйДокумент = ДанныеСтруктуры.ДанныеДокумента.Ссылка.ПолучитьОбъект();
		...
		ДокументОснованиеСсылка = ДокументСсылка.ДокументРеализации;
	КонецЕсли;
	
	ДанныеЗаполнения = Новый Структура("ДокументОснование, ВидОперации",
		ДокументОснованиеСсылка, Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение);
	НовыйДокумент.Заполнить(ДанныеЗаполнения);
	
	Для Каждого СтрокаТаблицы Из НовыйДокумент.Товары Цикл
		СтрокаТаблицы.Количество = 0;
	КонецЦикла;
	
	ОбработанныеСтроки = Новый Массив;
	СтрокиКДобавлению = НовыйДокумент.Товары.ВыгрузитьКолонки("Номенклатура, Количество");
	
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		Номенклатура = СопоставлениеНоменклатуры.Получить(СтрокаТовара.ИД);
		
		КоличествоОстаток = СтрокаТовара.Количество;
		
		НайденныеСтроки = НовыйДокумент.Товары.НайтиСтроки(Новый Структура("Номенклатура", Номенклатура));
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если ОбработанныеСтроки.Найти(НайденнаяСтрока.ИдентификаторСтроки) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			КоличествоВСтроку = Мин(НайденнаяСтрока.КоличествоДоИзменения - НайденнаяСтрока.Количество, КоличествоОстаток);
			Если КоличествоВСтроку > 0 Тогда
				НайденнаяСтрока.Количество = НайденнаяСтрока.Количество + КоличествоВСтроку;
				КоличествоОстаток = КоличествоОстаток - КоличествоВСтроку;
			КонецЕсли;
			Если НайденнаяСтрока.Количество >= НайденнаяСтрока.КоличествоДоИзменения Тогда
				ОбработанныеСтроки.Добавить(НайденнаяСтрока.ИдентификаторСтроки);
			КонецЕсли;
		КонецЦикла;
		
		Если КоличествоОстаток > 0 Тогда
			Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
				НайденныеСтроки[0].Количество = НайденныеСтроки[0].Количество + КоличествоОстаток;
				ОбработанныеСтроки.Добавить(НайденныеСтроки[0].ИдентификаторСтроки);
			Иначе
	...
	ДанныеОбъекта.Вставить("ИсправлениеКорректировки", НовыйДокумент.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки
		И ТипЗнч(НовыйДокумент.ИсправляемыйДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации"));
	ДанныеОбъекта.Вставить("ВедетсяУчетПрослеживаемыхТоваров", ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
		И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(НовыйДокумент.Дата));
	ДанныеОбъекта.Вставить("Реализация", Истина);
	ДанныеОбъекта.Вставить("ЭтоКомиссия", Истина);
	ДанныеОбъекта.РеализацияВЕАЭС               = Ложь;
	ДанныеОбъекта.ВедетсяУчетНДСПоФЗ150         = УчетНДС.ВедетсяУчетНДСПоФЗ150(НовыйДокумент.Дата);
	ДанныеОбъекта.ВедетсяУчетНДСПоФЗ335         = УчетНДС.ВедетсяУчетНДСПоФЗ335(НовыйДокумент.Дата);
	ДанныеОбъекта.ПокупательНалоговыйАгентПоНДС = Ложь;
	ДанныеОбъекта.ЕстьУчетПодакцизныхТоваров    = Ложь;
	ДанныеОбъекта.ЭкспортПриРеализации          = Ложь;
...
Процедура ЗаполнитьСтрокиТЧЗагрузкаИзФайла(ТаблицаТовары, ИмяТаблицы, ДанныеОбъекта)
	...
	Для Каждого СтрокаТовара Из ТаблицаТовары Цикл
		...
		Если Не СтрокаТовара.ЕстьВДокументеРеализации Тогда
			...
			Если ДанныеОбъекта.ВедетсяУчетПрослеживаемыхТоваров Тогда
				...
				СтрокаТовара.ПрослеживаемыйКомплект = СведенияОНоменклатуре.ПрослеживаемыйКомплект;
				СтрокаТовара.ПрослеживаемыйТоварПоДокументуОснованию = Ложь;
			Иначе
				СтрокаТовара.ПрослеживаемыйТовар = Ложь;
				СтрокаТовара.ПрослеживаемыйКомплект = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуТабЧасти(СтрокаТовара);
...
Функция ТекстЗапросаАкцизы(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	КорректировкаРеализацииТовары.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииТовары.Номенклатура.КодВидаПодакцизногоТовара.ВидТовара В (ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.Спирт), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СпиртосодержащаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.АлкогольнаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СахаросодержашиеНапитки))
	|				И КорректировкаРеализацииТовары.Ссылка.Контрагент.СтранаРегистрации.Код = ""643""
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДекларацийПоПодакцизнымТоварам.ДекларацияАкцизыАлко)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|	КорректировкаРеализацииТовары.Номенклатура.КодВидаПодакцизногоТовара КАК КодВидаПодакцизногоТовара,
	|	NULL КАК ДатаПринятияНаУчет
	|ИЗ
	|	Документ.КорректировкаРеализации.Товары КАК КорректировкаРеализацииТовары
	|ГДЕ
	|	КорректировкаРеализацииТовары.Ссылка = &Ссылка
	...
	|	КорректировкаРеализацииТовары.Номенклатура,
	|	КорректировкаРеализацииТовары.Номенклатура.КодВидаПодакцизногоТовара,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииТовары.Номенклатура.КодВидаПодакцизногоТовара.ВидТовара В (ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.Спирт), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СпиртосодержащаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.АлкогольнаяПродукция), ЗНАЧЕНИЕ(Перечисление.ВидыПодакцизныхТоваров.СахаросодержашиеНапитки))
	|				И КорректировкаРеализацииТовары.Ссылка.Контрагент.СтранаРегистрации.Код = ""643""
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДекларацийПоПодакцизнымТоварам.ДекларацияАкцизыАлко)
