...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ПрошлыйПериод            = (Год(Реквизиты.Период) > Год(ДокументПоступленияДата));
	
	Запрос.УстановитьПараметр("ЭтоКорректировкаПрошлогоПериода", ПрошлыйПериод);
	...
	Если ПрошлыйПериод И ОтражатьВНалоговомУчете Тогда
		СтавкаНалогаУвеличилась = НалоговыйУчет.НельзяПеренестиНаТекущийПериод(Реквизиты.Организация, ДокументПоступленияДата, Реквизиты.Период, 
			Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение);
	Иначе
