...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПлательщикНПД Тогда
		СведенияОЧекеНПД = ОплатаПлатежнойКартойВызовСервера.СведенияОЧекеНПД(Ссылка);
	КонецЕсли;
...
&НаКлиенте
Процедура РасшифровкаПлатежаСчетНаОплатуНПДПриИзменении(Элемент)
	
	ОплатаПлатежнойКартойКлиент.СчетНаОплатуПриИзменении(ЭтотОбъект, Элемент);
...
Процедура УправлениеФормой(Форма)
	...
	ВидимостьУслугиНПД = ВидимостьЭлементовНПД И ДоступнаИнтеграция И Не ЭтоВозвратПокупателю;
