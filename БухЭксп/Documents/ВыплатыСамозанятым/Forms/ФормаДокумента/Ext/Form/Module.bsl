...
Процедура ВыгрузитьПродолжение()
	...
	Если ПодключенаИнтеграцияСБанком Тогда
		Если ПроверитьЗаполнениеПередОтправкой() Тогда
			ИмяНужнойКоманды = "ДиректБанкОтправка";
			Для Каждого ТекущаяКоманда Из ЭтотОбъект.Команды Цикл
				РезультатПоискаКоманды = СтрНайти(ТекущаяКоманда.Имя, ИмяНужнойКоманды, НаправлениеПоиска.СКонца);
				Если РезультатПоискаКоманды > 0 Тогда
					Подключаемый_ВыполнитьКоманду(ТекущаяКоманда);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
