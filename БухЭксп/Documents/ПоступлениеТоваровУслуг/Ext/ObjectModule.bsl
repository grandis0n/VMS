...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Документы.ПоступлениеТоваровУслуг.ЗаполнитьДоговорАгентаПоставщикаПередЗаписью(ЭтотОбъект);
	...
	Если ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия
		И ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары Тогда
		ОплатаЧерезАгентаПоставщика = Ложь;
	КонецЕсли;
	
	Если Не ОплатаЧерезАгентаПоставщика Тогда
		АгентПоставщика = Справочники.Контрагенты.ПустаяСсылка();
		ДоговорАгентаПоставщика = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
		СчетУчетаРасчетовСАгентомПоставщика = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
		СчетУчетаРасчетовСАгентомПоставщикаПоАвансам = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
	КонецЕсли;
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявлениеОВвозеТоваров) И ЗначениеЗаполнено(ДОПП) Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("ЭлектроннаяТранспортнаяНакладная") Тогда
		ДобавитьОснованиеЭТрН(ДополнительныеСвойства.ЭлектроннаяТранспортнаяНакладная);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// В форме документа агент поставщика и договор с ним, счет расчетов и счет авансов редактируются в специальной форме.
	// В случае, если они не заполнены, покажем сообщение возле соответствующей гиперссылки.
	Если ОплатаЧерезАгентаПоставщика Тогда
		Если Не ЗначениеЗаполнено(АгентПоставщика) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(,,
				НСтр("ru = 'Агент поставщика (маркетплейс)'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,
				"ПорядокУчетаРасчетов", Отказ);
		КонецЕсли;
		Если ВестиУчетПоДоговорам И Не ЗначениеЗаполнено(ДоговорАгентаПоставщика) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(,,
				НСтр("ru = 'Договор с агентом поставщика (маркетплейсом)'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,
				"ПорядокУчетаРасчетов", Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СчетУчетаРасчетовСАгентомПоставщика) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(,,
				НСтр("ru = 'Счет учета расчетов с агентом поставщика (маркетплейсом)'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,
				"ПорядокУчетаРасчетов", Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СчетУчетаРасчетовСАгентомПоставщикаПоАвансам) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(,,
				НСтр("ru = 'Счет учета расчетов по авансам с агентом поставщика (маркетплейсом)'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,
				"ПорядокУчетаРасчетов", Отказ);
		КонецЕсли;
		Если ЗначениеЗаполнено(ДоговорАгентаПоставщика)
			И ЗначениеЗаполнено(ДоговорКонтрагента)
			И ДоговорАгентаПоставщика.ВалютаВзаиморасчетов <> ДоговорКонтрагента.ВалютаВзаиморасчетов Тогда
			ТекстСообщения = НСтр("ru = 'Валюта расчетов по договору с агентом поставщика (маркетплейсом) отличается от валюты расчетов по договору с поставщиком'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,
				"ПорядокУчетаРасчетов", Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаВзаиморасчетыАгентаПоставщика = УчетВзаиморасчетов.ПодготовитьТаблицуВзаиморасчетовЗачетАвансов(ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаТаблицаДокумента,
		ПараметрыПроведения.ТаблицаЗачетАвансовАгентаПоставщика, ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаРеквизиты, Отказ);
	
	ТаблицаВзаиморасчетыИП = Документы.ПоступлениеТоваровУслуг.ПодготовитьТаблицуВзаиморасчетыИП(
		ТаблицаВзаиморасчеты, ТаблицаВзаиморасчетыАгентаПоставщика, ПараметрыПроведения.Реквизиты);
	...
	СтруктураТаблицУСН.Вставить("ТаблицаРасчетовАгентаПоставщика", ТаблицаВзаиморасчетыАгентаПоставщика);
	...
	// Зачет аванса агента поставщика
	УчетВзаиморасчетов.СформироватьДвиженияЗачетАвансов(ТаблицаВзаиморасчетыАгентаПоставщика,
		ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаРеквизиты, Движения, Отказ);
	
	// Перенос задолженности на агента поставщика
	ТаблицаВзаиморасчетовПереносЗадолженности = Документы.ПоступлениеТоваровУслуг.ПодготовитьТаблицуПереносаЗадолженности(
		ТаблицаВзаиморасчеты,
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияПоступлениеМПЗ(СтруктураТаблицМПЗ,
		ТаблицаВзаиморасчетыИП, ,ПараметрыПроведения.ИПРеквизиты, Движения, Отказ);
...
Процедура ДобавитьОснованиеЭТрН(ТранспортнаяНакладнаяСсылка)
	ДокументОбъект = ТранспортнаяНакладнаяСсылка.ПолучитьОбъект();
	Если ДокументОбъект = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОбменСГИСЭПДБП.ДобавитьДокументОснование(ДокументОбъект, Ссылка);
	
	Попытка
		ДокументОбъект.Записать();
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен с ГИС ЭПД'"),
			УровеньЖурналаРегистрации.Ошибка, 
			Метаданные.Документы.ЭлектроннаяТранспортнаяНакладная, ТранспортнаяНакладнаяСсылка, 
			НСтр("ru = 'Не удалось изменить список оснований по причине: '")+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
