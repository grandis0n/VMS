...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	// расчеты с агентом поставщика
	СчетаУчетаВДокументах.ДобавитьПравилоЗаполнения(Правила, "", "СчетУчетаРасчетовСАгентомПоставщика", "РасчетыСПоставщиком");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ТребуетсяУчетРасчетов");
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "СчетУчетаРасчетовСАгентомПоставщикаПоАвансам", "АвансыПоставщику");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ТребуетсяУчетАвансов");
...
Функция ОсобенностиУчетаРасчетов(ВидОперации, ДоговорКонтрагента) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия Тогда
		ОсобенностиДокумента.ТребуетсяУчетРасчетовСАгентомПоставщика = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьДоговорАгентаПоставщикаПередЗаписью(Объект) Экспорт

	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам")
		Или Не Объект.ОплатаЧерезАгентаПоставщика Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыДоговора.Вставить("Владелец",    Объект.АгентПоставщика);
	
	ДоговорАгентаПоставщика = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	Если Не РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента(
		ДоговорАгентаПоставщика,
		ПараметрыДоговора.Владелец,
		ПараметрыДоговора.Организация,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыДоговора.ВидДоговора)) Тогда
		
		ПараметрыСоздания = Новый Структура("ЗначенияЗаполнения", ПараметрыДоговора);
		ДоговорАгентаПоставщика = РаботаСДоговорамиКонтрагентовБПВызовСервера.СоздатьОсновнойДоговорКонтрагента(ПараметрыСоздания);
	КонецЕсли;
	
	Объект.ДоговорАгентаПоставщика = ДоговорАгентаПоставщика;
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение)
	...
	|	Реквизиты.ОплатаЧерезАгентаПоставщика КАК ОплатаЧерезАгентаПоставщика,
	|	Реквизиты.АгентПоставщика КАК АгентПоставщика,
	|	Реквизиты.ДоговорАгентаПоставщика КАК ДоговорАгентаПоставщика,
	|	Реквизиты.СчетУчетаРасчетовСАгентомПоставщика КАК СчетУчетаРасчетовСАгентомПоставщика,
	|	Реквизиты.СчетУчетаРасчетовСАгентомПоставщикаПоАвансам КАК СчетУчетаРасчетовСАгентомПоставщикаПоАвансам,
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.ВалютаДокумента КАК ВалютаДокумента,
	|	ВЫБОР
	|		КОГДА Реквизиты.ОплатаЧерезАгентаПоставщика
...
Функция ТекстЗапросаЗачетАвансовАгентаПоставщика(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ОплатаЧерезАгентаПоставщика Тогда
		ПараметрыПроведения.Вставить("ЗачетАвансовАгентаПоставщикаРеквизиты",        Неопределено);
		ПараметрыПроведения.Вставить("ЗачетАвансовАгентаПоставщикаТаблицаДокумента", Неопределено);
		ПараметрыПроведения.Вставить("ТаблицаЗачетАвансовАгентаПоставщика",          Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВременнаяТаблицаСуммВзаиморасчетовАгентаПоставщика", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ЗачетАвансовАгентаПоставщикаРеквизиты",        НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ЗачетАвансовАгентаПоставщикаТаблицаДокумента", НомераТаблиц.Количество());
	...
	|	СУММА(ТаблицаСуммВзаиморасчетов.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов,
	|	СУММА(ТаблицаСуммВзаиморасчетов.СуммаРуб) КАК СуммаРуб
	|ПОМЕСТИТЬ ТаблицаСуммВзаиморасчетовАгентаПоставщика
	|ИЗ
	|	#ТаблицаСуммВзаиморасчетов КАК ТаблицаСуммВзаиморасчетов
	|;
	|
	...
	|ВЫБРАТЬ
	|	Реквизиты.Ссылка КАК ДокументРасчетов,
	|	Реквизиты.СчетУчетаРасчетовСАгентомПоставщика КАК СчетРасчетов,
	|	Реквизиты.СчетУчетаРасчетовСАгентомПоставщикаПоАвансам КАК СчетАвансов,
	|	Реквизиты.АгентПоставщика КАК Контрагент,
	|	Реквизиты.ДоговорАгентаПоставщика КАК ДоговорКонтрагента,
	|	Реквизиты.ВидДоговора КАК ВидДоговора,
	|	Реквизиты.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ВЫБОР
	|		КОГДА Реквизиты.ЭлектронныеУслуги
	...
	|ИЗ
	|	Реквизиты КАК Реквизиты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСуммВзаиморасчетовАгентаПоставщика КАК ТаблицаСуммВзаиморасчетов
	|		ПО (ИСТИНА)"
	+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	...
	Если Реквизиты.СпособЗачетаАвансов <> Перечисления.СпособыЗачетаАвансов.ПоДокументу Тогда
		ПараметрыПроведения.Вставить("ТаблицаЗачетАвансовАгентаПоставщика", Неопределено);
	Иначе
		НомераТаблиц.Вставить("ТаблицаЗачетАвансовАгентаПоставщика", НомераТаблиц.Количество());
		ТекстЗапроса = ТекстЗапроса
		+ "ВЫБРАТЬ
		|	ПоступлениеТоваровУслугЗачетАвансов.НомерСтроки КАК НомерСтроки,
		|	Реквизиты.СчетУчетаРасчетовСАгентомПоставщикаПоАвансам КАК СчетАвансов,
...
Функция ТекстЗапросаРеквизитыПереносЗадолженностиКредиторскаяЗадолженность(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.Организация КАК Организация,
	|	Реквизиты.АгентПоставщика КАК Контрагент,
	|	Реквизиты.ДоговорАгентаПоставщика КАК ДоговорКонтрагент,
	|	Реквизиты.ВалютаДокумента КАК ВалютаДокумента,
	|	&ПрименяетсяУСН КАК УчитыватьЗадолженностьУСН,
	|	&ПрименяетсяПСН КАК УчитыватьЗадолженностьУСНПатент,
	|	""Выбытие"" КАК НаправлениеДвижения,
	...
	|	&ОплатаПринятаАгентомПоставщика КАК СодержаниеПроводки
	|ИЗ
	|	Реквизиты КАК Реквизиты";
...
Функция ТекстЗапросаРегистрацияОтложенныхРасчетовСКонтрагентами(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ОплатаЧерезАгентаПоставщика Тогда
		...
		|	Реквизиты.АгентПоставщика КАК Контрагент,
		|	Реквизиты.ДоговорАгентаПоставщика КАК ДоговорКонтрагента,
		|	Реквизиты.ДоговорАгентаПоставщика.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
		|	Реквизиты.ДоговорАгентаПоставщика.ВидДоговора КАК ВидДоговора,
		|	Реквизиты.Дата КАК Дата
		|ИЗ
		|	Реквизиты";
...
Процедура ОбработкаОтложенногоПроведения(Параметры, Отказ) Экспорт
	...
	ТаблицаВзаиморасчетовАгентаПоставщика = УчетВзаиморасчетовОтложенноеПроведение.ПодготовитьТаблицуВзаиморасчетовЗачетАвансов(
		Параметры,
		ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаТаблицаДокумента,
		ПараметрыПроведения.ТаблицаЗачетАвансовАгентаПоставщика,
		ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаРеквизиты,
		Отказ);
	...
	УчетВзаиморасчетовОтложенноеПроведение.СформироватьДвиженияЗачетАвансов(
		Параметры,
		ТаблицаВзаиморасчетовАгентаПоставщика,
		ПараметрыПроведения.ЗачетАвансовАгентаПоставщикаРеквизиты,
		Отказ);
...
Функция ПодготовитьТаблицуВзаиморасчетыИП(ТаблицаВзаиморасчеты, ТаблицаВзаиморасчетыАгентаПоставщика, Реквизиты) Экспорт
	
	Если Не ЗначениеЗаполнено(Реквизиты) Тогда
		Возврат ТаблицаВзаиморасчеты.Скопировать();
	ИначеЕсли Реквизиты[0].ОплатаЧерезАгентаПоставщика Тогда
		Возврат ТаблицаВзаиморасчетыАгентаПоставщика.Скопировать();
	Иначе
		Возврат ТаблицаВзаиморасчеты.Скопировать();
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ОплатаПринятаАгентомПоставщика", НСтр("ru = 'Оплата принята агентом поставщика'"));
	...
	Запрос.Текст = ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаЗачетАвансовАгентаПоставщика(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаРеквизитыПереносЗадолженностиКредиторскаяЗадолженность(НомераТаблиц, ПараметрыПроведения, Реквизиты);
