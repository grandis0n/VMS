...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ВидДокумента) = Ложь Тогда
		ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.ЭЭР);
	КонецЕсли;
	...
	Если РольУчастника = 3 Тогда
		Если ТипЗнч(СсылкаТитулЭкспедитораЭкспедитор) = Тип("СправочникСсылка.Организации") Тогда
			Организация = СсылкаТитулЭкспедитораЭкспедитор;
		ИначеЕсли ТипЗнч(СсылкаТитулЭкспедитораСоставитель) = Тип("СправочникСсылка.Организации") Тогда
			Организация = СсылкаТитулЭкспедитораСоставитель;
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ЗначениеЗаполнено(СсылкаТитулЭкспедитораСоставитель) Тогда
		СтруктураДляРеестра.Перевозчик = СсылкаТитулЭкспедитораСоставитель;
	Иначе
		СтруктураДляРеестра.Перевозчик = СсылкаТитулЭкспедитораЭкспедитор;
	КонецЕсли;
	
	СтруктураДляРеестра.ДатаДокументаИБ = Дата;
	СтруктураДляРеестра.НомерДокументаИБ = Номер;
	СтруктураДляРеестра.ДатаЭПД = ТитулЭкспедитораДата;
	СтруктураДляРеестра.НомерЭПД = ТитулЭкспедитораНомер;
	СтруктураДляРеестра.Организация = Организация;
	СтруктураДляРеестра.Грузоотправитель = СсылкаТитулЭкспедитораЗаказчик;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ОбменСГИСЭПДПереопределяемый.ОбработкаЗаполненияЭЭР(ЭтотОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ЭтотОбъект.ТитулЭкспедитораНомер = Неопределено;
	ЭтотОбъект.ТитулЭкспедитораДата = Неопределено;
	
	ПрефиксыТитулов = ОбменСГИСЭПДКлиентСервер.ПрефиксыТитуловДокумента("Документ.ЭлектроннаяЭкспедиторскаяРасписка");
	...
	ЭтотОбъект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1");
