...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ФормаБезОбработки") Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьДоступностьЭлементовЭЭР(ЭтотОбъект);
		...
		Если Объект.ЭтоВходящий = Ложь Тогда
			...
			Элементы.СсылкаТитулЭкспедитораГрузоотправитель.ОграничениеТипа = КонтрагентОписаниеТипов;
			...
			МассивЭлементов.Добавить(Элементы.СсылкаТитулЭкспедитораГрузоотправитель);
...
Процедура ЗаполнитьЗначенияПоУмолчаниюДляТекущегоТитула()
	...
	Если (ЗначениеЗаполнено(Объект.Ссылка) = Ложь 
		Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1")) Тогда
				
		Если ЗначениеЗаполнено(Объект.ВидДокумента) = Ложь Тогда
			Объект.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.ЭЭР"));
		КонецЕсли;
...
&НаКлиенте
Процедура СсылкаТитулЭкспедитораГрузоотправительПриИзменении(Элемент)
	ПриИзмененииДанныхНаФорме(Элемент.Имя); 
...
&НаКлиенте
Процедура ПредставлениеТитулЭкспедитораАдрПриемГрузНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыЗаполнения = Новый Структура ("АдресВСвободнойФорме", Истина);
...
&НаКлиенте
Процедура ПредставлениеТитулЭкспедитораАдрПриемГрузОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыЗаполнения = Новый Структура ("АдресВСвободнойФорме", Истина);
	ОбменСГИСЭПДКлиентСервер.ЗаполнитьРеквизитыАдреса(ЭтотОбъект, ВыбранноеЗначение,
		Элемент.Имя,, ПараметрыЗаполнения);КонецПроцедуры

&НаКлиенте
Процедура ПредставлениеТитулЭкспедитораАдрПриемГрузАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ЗаполнитьТитулЭкспедитораВладИнфр(Команда)
	ОбменСГИСЭПДКлиент.ВывестиФормуВводаДанных(ЭтотОбъект, Команда.Имя);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьТитулЭкспедитораСвЛицПриемГр(Команда)
	ОбменСГИСЭПДКлиент.ВывестиФормуВводаДанных(ЭтотОбъект, Команда.Имя);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьТитулЭкспедитораСвЛицПогрГр(Команда)
	ОбменСГИСЭПДКлиент.ВывестиФормуВводаДанных(ЭтотОбъект, Команда.Имя);
КонецПроцедуры
...
&НаКлиенте
Процедура ТитулЭкспедитораФактическаяДатаПолученияГрузаПриИзменении(Элемент)
	ПриИзмененииДанныхНаФорме(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура ТитулЭкспедитораФактическоеВремяПолученияГрузаПриИзменении(Элемент)
	ПриИзмененииДанныхНаФорме(Элемент.Имя);
КонецПроцедуры
...
Процедура ПодборПорученияЭкспедиторуЗавершение(Значение, Данные) Экспорт
	...
	Если Значение <> Неопределено Тогда  
		ЭтотОбъект.ТитулЭкспедитораУИДПорЭксп = ПолучитьУИДМинтранс(Значение);
...
&НаСервереБезКонтекста
Функция ПолучитьУИДМинтранс(ПоручениеЭкспедитору)
...
&НаКлиенте
Процедура ЗаполнитьТитулЭкспедитораГрузы(Команда)
...
Процедура ИныеПолучатели(Команда)
	...
	ОткрытьФорму("Документ.ЭлектроннаяЭкспедиторскаяРасписка.Форма.ИныеПолучатели", ПараметрыФормы, ЭтотОбъект, 
...
Процедура ЗаполнитьРеквизитыОтображаемойВерсии(ВыбранныйТитулСтрока, ОтмечатьТекущий)
	...
	ВыбранныйТитул = ОбменСГИСЭПДКлиентСервер.ТитулПоПрефиксу("Документ.ЭлектроннаяЭкспедиторскаяРасписка", ВыбранныйТитулСтрока); 
...
Процедура ЗаполнитьПредставленияАдресов(Знач Титул = Неопределено)
	...
	Если Титул = Неопределено Тогда
		ПрефиксТитула = ОбменСГИСЭПДКлиентСервер.ПолучитьПрефиксТитулаПоСтраницеОсновнойФормы(Элементы.Страницы.ТекущаяСтраница.Имя, 
			ОбменСГИСЭПДКлиентСервер.ТипДокументаПоручениеЭкспедитору());	
		Титул = ОбменСГИСЭПДКлиентСервер.ТитулПоПрефиксу("Документ.ЭлектроннаяЭкспедиторскаяРасписка", ПрефиксТитула);
	КонецЕсли;
	
	Если Титул = Неопределено Или Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭЭР_Титул1 Тогда
		ОбменСГИСЭПДКлиентСервер.ЗаполнитьПредставлениеАдреса("ПредставлениеТитулЭкспедитораАдрПриемГрузАдрес", ЭтотОбъект);	
	КонецЕсли;
