...
Процедура ПодготовитьФормуНаСервере()
	...
	ОбменСГИСЭПДБПКлиентСервер.ОбновитьТекстЭПД(ЭтотОбъект);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.Доставка.Видимость = Форма.ИспользоватьНесколькоСкладовБухгалтерскийУчет
		...
		Или Не ЗначениеЗаполнено(Объект.СпособДоставки)
		ИЛИ Форма.ИспользоватьДоставкуАвтотранспортом;
...
Процедура ДатаПриИзмененииНаСервере(ТребуетсяВызовСервера, ТребуетсяПерерасчитатьСрокОплаты)
	...
	Если ТребуетсяВызовСервера Тогда
		...
		РеализацияТоваровУслугФормы.ПроверитьИзменитьАдресДоставки(Объект, ТекущаяДатаДокумента);
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	Если Элементы.Найти("КомандыЭДО") <> Неопределено Тогда
		МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВПодменюМКПоделиться(Элементы, Элементы.КомандыЭДО);
	КонецЕсли;
...
Процедура ДоставкаНажатие(Элемент)
	...
	// при включенной опции ИспользоватьДоставкуТранспортнойКомпанией 
	// адрес показывается на форме или не показывается вообще (в зависимости от способа доставки)
	ПоказыватьАдресДоставки = 
		НЕ ИспользоватьДоставкуТранспортнойКомпанией ИЛИ НЕ ЗначениеЗаполнено(Объект.СпособДоставки);
		
	ПараметрыФормы.Вставить("ПоказыватьАдресДоставки", ПоказыватьАдресДоставки);
		
	ПараметрыФормы.Вставить("ПараметрыТранспортногоСредства",  Объект.ПараметрыТранспортногоСредства);
	ПараметрыФормы.Вставить("ПоказыватьРеквизитыЭПД",          ИспользоватьДоставкуАвтотранспортом);
...
Процедура ДекорацияЭПДНажатие(Элемент)
	Если НЕ ЗначениеЗаполнено(ЭПДСсылка) Тогда
	...
	Иначе
		ПоказатьЗначение(,ЭПДСсылка);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияОформитьЭПДНажатие(Элемент)
	Если НЕ МожноСоздатьПоручениеЭкспедитору() Тогда
		СоздатьЭлектроннуюЗаказЗаявкуНаКлиенте();
	Иначе
		ВыбратьТипДокументаЭПД(Элемент);
	КонецЕсли;
КонецПроцедуры 

&НаСервере
Функция МожноСоздатьПоручениеЭкспедитору()

	Возврат ОбменСГИСЭПДБПВызовСервера.МожноСоздатьПоручениеЭкспедитору(Объект);

КонецФункции


&НаКлиенте
Асинх Процедура ВыбратьТипДокументаЭПД(Элемент)
	ТипыДокументов = Новый СписокЗначений;
	ТипыДокументов.Добавить("ЭлектронныйЗаказЗаявка",          НСтр("ru = 'Электронный заказ (заявка)'"));
	ТипыДокументов.Добавить("ЭлектронноеПоручениеЭкспедитору", НСтр("ru = 'Электронное поручение экспедитору'"));

	РезультатВыбора = Ждать ВыбратьИзСпискаАсинх(ТипыДокументов, Элемент);
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если РезультатВыбора.Значение = "ЭлектронноеПоручениеЭкспедитору" Тогда
		СоздатьЭлектронноеПоручениеЭкспедиторуНаКлиенте();
	Иначе
		СоздатьЭлектроннуюЗаказЗаявкуНаКлиенте();
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура СоздатьЭлектроннуюЗаказЗаявкуНаКлиенте()
	Если Объект.Ссылка.Пустая() ИЛИ Модифицированность Тогда
		Результат = Ждать ЗаписатьДокумент();
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗначенияЗаполнения", ОбменСГИСЭПДБПВызовСервера.ДанныеЗаполненияЭЗЗ(Объект.Ссылка));

	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьЭлектроннуюЗаказЗаявкуЗавершение", ЭтотОбъект);

	ОткрытьФорму("Документ.ЭлектронныйЗаказЗаявка.ФормаОбъекта",ПараметрыФормы,,,,,ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте
Асинх Процедура СоздатьЭлектронноеПоручениеЭкспедиторуНаКлиенте()
	Если Объект.Ссылка.Пустая() ИЛИ Модифицированность Тогда
		Результат = Ждать ЗаписатьДокумент();
		Если НЕ Результат Тогда
			Возврат;
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗначенияЗаполнения", ОбменСГИСЭПДБПВызовСервера.ДанныеЗаполненияЭПЭ(Объект.Ссылка));

	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьЭлектронноеПоручениеЭкспедиторуЗавершение", ЭтотОбъект);

	ОткрытьФорму("Документ.ЭлектронноеПоручениеЭкспедитору.ФормаОбъекта", ПараметрыФормы, , , , ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте
Процедура СоздатьЭлектронноеПоручениеЭкспедиторуЗавершение(Значение, ДополнительныеПараметры) Экспорт
	ОбменСГИСЭПДБПКлиентСервер.ОбновитьТекстЭПД(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Асинх Функция ЗаписатьДокумент()
...
&НаКлиенте
Процедура СоздатьЭлектроннуюЗаказЗаявкуЗавершение(Значение, ДополнительныеПараметры) Экспорт
	ОбменСГИСЭПДБПКлиентСервер.ОбновитьТекстЭПД(ЭтотОбъект);
КонецПроцедуры 

&НаКлиенте
Процедура СоздатьЭлектроннуюЗаказЗаявку(Команда)
	СоздатьЭлектроннуюЗаказЗаявкуНаКлиенте();
КонецПроцедуры
