...
Процедура АдаптироватьМКСписокФормы()
	...
	Если Элементы.Найти("СостояниеЭДО") <> Неопределено
		...
		МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.СостояниеЭДО, 3);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьЧерезБизнесСеть(Команда)
	БизнесСетьСлужебныйКлиент.ЗагрузитьЧерезБизнесСеть(Неопределено, Неопределено);
КонецПроцедуры
