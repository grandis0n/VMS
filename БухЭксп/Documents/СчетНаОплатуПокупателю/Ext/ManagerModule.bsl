...
Функция ТекстЗапросаДанныеОГрузе() Экспорт
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	СУММА(СправочникНоменклатура.Вес * СчетНаОплатуПокупателюТовары.Количество) КАК Вес,
	|	СУММА(СправочникНоменклатура.Объем * СчетНаОплатуПокупателюТовары.Количество) КАК Объем,
	|	СправочникНоменклатура.ТипГруза КАК ГрузСсылка,
	|	СправочникНоменклатура.НасыпнойНаливной КАК НасыпнойНаливной,
	|	СчетНаОплатуПокупателюТовары.Сумма КАК СтоимостьГруза
	|ИЗ
	|	Документ.СчетНаОплатуПокупателю.Товары КАК СчетНаОплатуПокупателюТовары
	...
	|ГДЕ
	|	СчетНаОплатуПокупателюТовары.Ссылка = &ДокументСсылка
	|
	|СГРУППИРОВАТЬ ПО
...
Функция РеализацииПоСчетам(СчетаНаОплату, ТолькоПроведенные = Истина) Экспорт
	...
	Если НЕ ТолькоПроведенные Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Шапка.Проведен",           "Истина");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СтрокиТЧ.Ссылка.Проведен", "Истина");
	КонецЕсли;
	
	// Считаем, что если у пользователя есть доступ к счетам на оплату, то к реализациям тоже должен быть.
	// Поэтому для оптимизации включаем привилегированный режим.
...
Функция РеализацииПоСчету(СчетНаОплату, ТолькоПроведенные = Истина) Экспорт
...
Функция ТекстЗапросаУслугПродукцииНПД(ИменаТаблиц) Экспорт
	
	ИменаТаблиц.Добавить("ВТ_ОплаченныеСчета");
	ИменаТаблиц.Добавить("НоменклатураОплаченныхСчетов");
	ИменаТаблиц.Добавить("ОплаченныеСчета");
	...
	|	РеквизитыДокумента.ВалютаДокумента КАК ВалютаПлатежа,
	|	СУММА(РасшифровкаПлатежа.СуммаПлатежа) КАК СуммаОплаты,
	|	СУММА(РасшифровкаПлатежа.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов,
	|	МАКСИМУМ(РасшифровкаПлатежа.НомерТЧ) КАК НомерТЧ
	|ПОМЕСТИТЬ ВТ_ОплаченныеСчета
	|ИЗ
	|	ВТ_РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_РеквизитыДокумента КАК РеквизитыДокумента
	|		ПО (ИСТИНА)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетНаОплатуПокупателю КАК СчетНаОплатуПокупателю
	|		ПО РасшифровкаПлатежа.СчетНаОплату = СчетНаОплатуПокупателю.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО РасшифровкаПлатежа.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОплаченныеСчета.СчетНаОплату КАК СчетНаОплату,
	|	&ЧастьЗапросаДляВыбораСодержанияУслуг КАК Наименование,
	|	СУММА(ТаблицаТовары.Сумма) КАК Сумма,
	|	МАКСИМУМ(ТаблицаТовары.НомерСтроки) КАК НомерСтроки,
	|	ОплаченныеСчета.НомерТЧ КАК НомерТЧ
	|ИЗ
	|	ВТ_ОплаченныеСчета КАК ОплаченныеСчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетНаОплатуПокупателю.Товары КАК ТаблицаТовары
	|		ПО ОплаченныеСчета.СчетНаОплату = ТаблицаТовары.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ОплаченныеСчета.СчетНаОплату,
	|	&ЧастьЗапросаДляВыбораСодержанияУслуг,
	|	ОплаченныеСчета.НомерТЧ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОплаченныеСчета.СчетНаОплату КАК СчетНаОплату,
	|	ОплаченныеСчета.РасчетыВУсловныхЕдиницах
	|		И ОплаченныеСчета.ВалютаДокумента <> ОплаченныеСчета.ВалютаПлатежа КАК РасчетыВУсловныхЕдиницах,
	|	ОплаченныеСчета.ВалютаДокумента КАК ВалютаДокумента,
	|	ОплаченныеСчета.ВалютаПлатежа КАК ВалютаПлатежа,
	|	ОплаченныеСчета.СуммаОплаты КАК СуммаОплаты,
	|	ОплаченныеСчета.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов
	|ИЗ
	|	ВТ_ОплаченныеСчета КАК ОплаченныеСчета";
	
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
	ЧастьЗапросаДляВыбораСодержанияУслуг = ОбщегоНазначенияБПВызовСервера.ПолучитьЧастьЗапросаДляВыбораСодержанияУслуг(
		"ТаблицаТовары");
	Возврат СтрЗаменить(ТекстЗапроса, "&ЧастьЗапросаДляВыбораСодержанияУслуг", ЧастьЗапросаДляВыбораСодержанияУслуг);
