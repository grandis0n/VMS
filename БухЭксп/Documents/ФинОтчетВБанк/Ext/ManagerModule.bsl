...
Процедура ДобавитьОтчетПоСчету(ПараметрыЗаполнения, СтрокаНастройкиКомплекта, СоставОтчетности, ДатаНачала, ДатаОкончания)
	...
	СчетУчета = ЗаполнениеФинОтчетностиВБанки.НайтиСчетПоКоду(СтрокаНастройкиКомплекта.Счет);
	Если ЗначениеЗаполнено(СтрокаНастройкиКомплекта.Счет) И Не ЗначениеЗаполнено(СчетУчета) Тогда
		Возврат;
	КонецЕсли; 
