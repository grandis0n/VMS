...
Функция КоличествоНеустановленныхПодписей(Знач СообщениеОбмена, Знач НастройкаОбмена, Знач ВидЭД)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОбмена.МаршрутПодписания) Тогда
		Возврат ВесМаршрута;
	КонецЕсли;
...
Процедура ПолучитьСтатусДокументаПослеАутентификацииSberAPI(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ВыполненоУспешно(Результат) Тогда
		...
		ПараметрыОжидания.ВыводитьСообщения = Истина;
		Оповещение = Новый ОписаниеОповещения(
			"ПослеПолученияСтатусаДокументаSberAPI", ЭтотОбъект, ДополнительныеПараметры);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатВыполнения, Оповещение, ПараметрыОжидания);
	...
	Иначе // выполнено
		...
		Если РезультатОперации.Свойство("ТребуетсяАутентификация") Тогда
			...
			Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
				...
				ПараметрыАутентификацииПоТокену = ОбменСБанкамиSberAPIСлужебныйКлиент.ПараметрыАутентификацииНаТокене();
				ПараметрыАутентификацииПоТокену.Вставить("ОповещениеОВыполнении", ОповещениеПослеАутентификации);
				ПараметрыАутентификацииПоТокену.Вставить("ИмяВнешнегоМодуля", РеквизитыНастройкиОбмена.ИмяВнешнегоМодуля);
				ПараметрыАутентификацииПоТокену.Вставить("НастройкаОбмена", Объект.НастройкаОбмена);
			
				ОбменСБанкамиSberAPIСлужебныйКлиент.АутентифицироватьсяНаТокене(ПараметрыАутентификацииПоТокену);
