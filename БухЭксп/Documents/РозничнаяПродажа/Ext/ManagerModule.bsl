...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияЕГАИС(ДанныеШтрихкода.ВидПродукции) Тогда
...
Функция СформироватьОтчетыОРозничныхПродажах(Организация, ИдентификаторУстройства) Экспорт
	...
	ОтборДокумент = Новый Структура("Организация, Склад, СчетКасса, ДеятельностьНаПатенте");
	
	ДанныеЗаполнения = Новый Структура("Организация, Склад, СчетКасса");
	...
	Для Каждого СтрокаДокументы Из ПараметрыЗаполнения.ТаблицаДокументы Цикл
		...
		ЗаполнитьЗначенияСвойств(ОтборДокумент, СтрокаДокументы);
		
		ТоварыПоДокументу                = ПараметрыЗаполнения.ТаблицаТовары.Скопировать(ОтборДокумент);
		ВозвратыПоДокументу              = ПараметрыЗаполнения.ТаблицаВозвраты.Скопировать(ОтборДокумент);
		АгентскиеУслугиПоДокументу       = ПараметрыЗаполнения.ТаблицаАгентскиеУслуги.Скопировать(ОтборДокумент);
		ПодарочныеСертификатыПоДокументу = ПараметрыЗаполнения.ТаблицаПодарочныеСертификаты.Скопировать(ОтборДокумент);
		ОплатаПоДокументу                = ТаблицаОплаты.Скопировать(ОтборДокумент); 
		ВозвратыОплат                    = ТаблицаВозвратОплаты.Скопировать(ОтборДокумент);
		
		СчетаНаОплатуПокупателю           = ПараметрыЗаполнения.СчетаНаОплату.Скопировать(ОтборДокумент);
		ВзаиморасчетыПоДокументам         = ПараметрыЗаполнения.ТаблицаВзаиморасчеты.Скопировать(ОтборДокумент);
		...
		ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ОтборДокумент);
		ОтчетОРозничныхПродажах.Заполнить(ДанныеЗаполнения);
		
		ОтчетОРозничныхПродажах.ДокументБезНДС = СтрокаДокументы.ДокументБезНДС;
		
		ОтчетОРозничныхПродажах.ДеятельностьНаПатенте = ОтборДокумент.ДеятельностьНаПатенте 
			И УчетнаяПолитика.ПрименяетсяУСНПатент(ОтчетОРозничныхПродажах.Организация, ОтчетОРозничныхПродажах.Дата);
		
		Если ОтчетОРозничныхПродажах.ДеятельностьНаПатенте Тогда
			Если НЕ ЗначениеЗаполнено(ОтчетОРозничныхПродажах.Патент) Тогда
				ОтчетОРозничныхПродажах.Патент = 
					УчетПСН.ВидДеятельностиПоПатентуПоУмолчанию(ОтчетОРозничныхПродажах.Организация, ОтчетОРозничныхПродажах.Дата);
			КонецЕсли;
			ОтчетОРозничныхПродажах.ДеятельностьНаТорговомСборе = Ложь;
		Иначе
			ОтчетОРозничныхПродажах.Патент = Справочники.Патенты.ПустаяСсылка();
	...
	Для Каждого СтрокаПоЧеку Из ПараметрыЗаполнения.ТаблицаЧеки Цикл
		...
		ЗаполнитьЗначенияСвойств(ОтборДокумент, СтрокаПоЧеку);
		
		НайденныеСтроки = ПараметрыЗаполнения.ТаблицаДокументы.НайтиСтроки(ОтборДокумент);
