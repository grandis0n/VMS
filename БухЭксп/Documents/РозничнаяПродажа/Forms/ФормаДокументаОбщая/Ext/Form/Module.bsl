...
&НаКлиенте
Процедура РазблокироватьДляКоррекции(Команда)
...
Функция СписокВетеринарныхПрепаратов(Знач СписокТовары, ЕстьРецепт)
	...
	Для каждого СтрокаТовары Из СписокТовары Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВидПродукцииИС) И ЗначениеЗаполнено(СтрокаТовары.АкцизнаяМарка) Тогда
			...
			ВидПродукцииИС = ОписаниеНоменклатурыИС[Номенклатура].КлючВидовПродукции;
		КонецЕсли;
		
		Если Не КонструкторИСМПКлиентСервер.ВидПродукцииИспользуетВетеринарныйСертификат(ВидПродукцииИС) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если ДанныеСтрокиТаблицы.ЭтоМаркируемаяПродукция 
		...
		ПараметрыОткрытия.ВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.ВидПродукцииКлючаПоПодсистеме(
			ДанныеСтрокиТаблицы.ВидПродукцииИС, ?(ДанныеСтрокиТаблицы.ЭтоМаркируемаяПродукция, "ГИСМТ", "ЕГАИС"));
...
Процедура ЗаполнитьСтрокуЧекаМаркируемыеТовары(СтрокаПозицииЧека, РезультатРаспределения, ЭтоВерсия120)
	...
	ЭтоЧастичноеВыбытиеМерногоТовара = НЕ КонструкторИСМПОбщегоНазначенияКлиентСервер.ИспользоватьЧислительИЗнаменательПриЧастичномВыбытииНаККТ(РезультатРаспределения.ВидПродукции)
...
Функция ТекстЗапросаТаблицаТоваров(НомераТаблиц)
	...
	|   ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка) КАК ЭтоТабачнаяПродукция,
	...
	|	ПО ТаблицаЧека.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура 
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПолучитьПараметрыФискальногоУстройстваЗавершение(ПараметрыФискальногоУстройства, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ВерсияФормата", ПечатьФискальныхДокументовКлиент.НовыйВерсииФорматов());
	
	Если НЕ ПараметрыФискальногоУстройства.Результат Тогда 
		ОписаниеОшибки = ПараметрыФискальногоУстройства.ОписаниеОшибки;
	Иначе
		ДополнительныеПараметры.ВерсияФормата     = ПечатьФискальныхДокументовКлиент.ВерсииФорматовФД(
			ПараметрыФискальногоУстройства.ПараметрыККТ.ВерсияФФДФН, ПараметрыФискальногоУстройства.ПараметрыККТ.ВерсияФФДККТ);
	...
	Если (ПроверятьКодыМаркировки И ЕстьМаркируемаяПродукцияВДокументе)
		ИЛИ ПараметрыФискальногоУстройства.ИдентификаторУстройства = Неопределено 
...
Процедура ПечатьЧекаНаФискальномУстройстве(ДополнительныеПараметры)
	...
	Если ИспользуетсяРегистрацияРозничныхПродажВЕГАИС Тогда
		СтрокиТовары = Объект.Товары.НайтиСтроки(Новый Структура("ЭтоАлкогольнаяПродукция", Истина));
		Для Каждого СтрокаТовара Из СтрокиТовары Цикл
			ЕстьАлкогольнаяПродукция = ЗначениеЗаполнено(СтрокаТовара.АкцизнаяМарка);
...
Процедура НачатьОперациюНаФискальномУстройстве(ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПараметрыФискальногоУстройства = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании(Истина);
		ПараметрыФискальногоУстройства.Вставить("ПараметрыККТ" , 
			МенеджерОборудованияБПВызовСервера.ПараметрыККТ(ДополнительныеПараметры.ИдентификаторУстройства));
			
		ПолучитьПараметрыФискальногоУстройстваЗавершение(ПараметрыФискальногоУстройства, ДополнительныеПараметры);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверкаСтатусаЧекаЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	РезультатПечатиЧека = ПолучитьИзВременногоХранилища(РезультатВыполнения.АдресРезультата);
	
	ОборудованиеЧекопечатающиеОблачныеККТКлиент.ПробитиеЧека(РезультатПечатиЧека, ПараметрыПроверки);
	
	Результат  = Новый Структура;
	Результат.Вставить("Результат", Истина);
	Результат.Вставить("ИдентификаторУстройства", ДополнительныеПараметры.ИдентификаторУстройства);
	ВыходныеПараметры = Новый Массив;
	ВыходныеПараметры.Добавить("");
	ВыходныеПараметры.Добавить(РезультатПечатиЧека.Результат.receiptNumber);
	
	Результат.Вставить("ВыходныеПараметры", ВыходныеПараметры);
...
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(СтрокаТаблицы, СуммаВключаетНДС, СведенияОНоменклатуре)
	...
	Если СведенияОНоменклатуре <> Неопределено Тогда
		...
		//Встраивание БгосИС 2.0
		Если СведенияОНоменклатуре.АлкогольнаяПродукция Тогда
			СтрокаТаблицы.ЭтоАлкогольнаяПродукция = Истина;
		ИначеЕсли КонструкторИСОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(СведенияОНоменклатуре.ВидПродукцииИС) Тогда
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ВедетсяУчетВетеринарныхПрепаратов = НЕ ЭтоВозврат 
		И ИнтеграцияИСМПБП.ВедетсяУчетВидовПродукцииПоВетеринарнымСертификатам();
...
Функция ИспользуетсяМаркируемаяПродукция(ТолькоОнлайнПроверка = Ложь)
	...
	Если ТолькоОнлайнПроверка Тогда
		ФильтрВидПродукции = КонструкторИСМПОбщегоНазначенияКлиентСервер.ВидыПродукцииОбязательнойОнлайнПроверкиПередРозничнойПродажей();
	Иначе
		ФильтрВидПродукции = КонструкторИСОбщегоНазначенияКлиентСервер.УчитываемыеВидыПродукцииИСМП();
	КонецЕсли;
