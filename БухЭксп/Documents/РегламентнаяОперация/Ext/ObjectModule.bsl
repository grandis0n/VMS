...
Процедура РасчетРезервовПоСомнительнымДолгам(СтруктураШапкиДокумента, Отказ)
	...
	Для Каждого Запись Из НачислениеРезерва Цикл
		...
		Если Запись.НачисленоРанееБУ = 0 И Запись.НачисленоРанееНУ = 0
			И Запись.НачисленоБУ = 0 И Запись.НачисленоНУ = 0 И Запись.ВстречнаяЗадолженность = 0
			И Запись.СуммаОбеспечения = 0 И Не ЗначениеЗаполнено(Запись.ВидОбеспечения) Тогда
			Продолжить;
		КонецЕсли;
	...
	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		...
		Истина,
		"Организация,Контрагент,ДоговорКонтрагента,ДокументРасчетовСКонтрагентом,ДатаВозникновенияЗадолженности,СуммаЗадолженности,СрокЗадолженности,ВстречнаяЗадолженность,СуммаОбеспечения,ВидОбеспечения,СомнительнаяЗадолженность",
...
Процедура ПризнаниеРасходовНаПриобретениеОСУСН(СтруктураШапкиДокумента, Отказ)
	...
	Запрос.УстановитьПараметр("КонецПериодаОплатыВнеоборотныхАктивов",
		КонецПериодаОплатыВнеоборотныхАктивов(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.КонДата));
	...
	|	ПервоначальныеСведенияОСНалоговыйУчетУСН.ПорядокВключенияСтоимостиВСоставРасходовУСН КАК ПорядокВключенияСтоимостиВСоставРасходовУСН
	|ИЗ
	|	РегистрНакопления.ОплатыОСДляУСН.Обороты(&ДатаПереходаНаУСН, &КонецПериодаОплатыВнеоборотныхАктивов, , Организация = &Организация) КАК ОплатыОСДляУСНОбороты
...
Процедура ПризнаниеРасходовНаПриобретениеНМАУСН(СтруктураШапкиДокумента, Отказ)
	...
	|ИЗ
	|	РегистрНакопления.ОплатыНМАДляУСН.Обороты(&ДатаПереходаНаУСН, &КонецПериодаОплатыВнеоборотныхАктивов, , Организация = &Организация) КАК ОплатыНМАДляУСНОбороты
...
Функция КонецПериодаОплатыВнеоборотныхАктивов(Организация, Дата)
	Возврат Обработки.ПомощникПереходаСУСННаОСНО.ГраницаОпределенияРасходовУСН(Организация, Дата);
КонецФункции
#КонецОбласти
