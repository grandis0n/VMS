...
Процедура ОплатаВидОплатыОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДопустимыеТипыОплаты.Добавить(ПредопределенноеЗначение("Перечисление.ТипыОплат.ЦифровойРубль"));
...
Процедура ЗаполнитьСопоставленныеПозицииНаСервере(Перезаполнить)
	...
	ИначеЕсли Объект.ЗагрузкаИзОФД Тогда
		...
		НесопоставленнаяНоменклатура = НоменклатураПоДокументу.НайтиСтроки(Новый Структура("НоменклатураСопоставлена", Ложь));
		Для каждого СтрокаСопоставление Из НесопоставленнаяНоменклатура Цикл
			СтрокиДокумента = Объект[СтрокаСопоставление.ИмяТабличнойЧасти].НайтиСтроки(Новый Структура("ИдентификаторСтроки", Строка(СтрокаСопоставление.ИдентификаторСтроки)));
			ЕстьНесопоставленнаяНоменклатура = СтрокиДокумента.Количество() = 1 И СтрокиДокумента[0].Номенклатура.Пустая();
			Если ЕстьНесопоставленнаяНоменклатура Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
