...
Функция ПодготовитьТаблицуПрочихРасчетовАТТ(ТаблицаБезналичныеОплаты, ТаблицаРеквизиты) Экспорт
	...
	Для Каждого СтрокаОплаты Из ТаблицаБезналичныеОплаты Цикл
		...
		Если СтрокаОплаты.ТипОплаты = Перечисления.ТипыОплат.ПлатежнаяКарта
			Или СтрокаОплаты.ТипОплаты = Перечисления.ТипыОплат.СБП
			Или СтрокаОплаты.ТипОплаты = Перечисления.ТипыОплат.ЦифровойРубль
...
Функция ТекстЗапросаВозвратыТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьБезналичныеВозвратыОплаты Тогда
		...
		|		ИЛИ ТаблицаВозвратыОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.СБП)
		|		ИЛИ ТаблицаВозвратыОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.ЦифровойРубль)
...
Функция ТекстЗапросаВозвратыОплатыУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьВозвратыПредоплаты Тогда
		...
		|	0 КАК Графа7,
		|	СУММА(ВЫБОР
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 5 / 105
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 7 / 107
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 22 / 122
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 20 / 120
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10_110)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 10 / 110
		|			КОГДА ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)
		|					ИЛИ ТаблицаВозвраты.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
		|				ТОГДА -ТаблицаВозвраты.СуммаРуб * 18 / 118
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК НДС,
		|	СУММА(ВЫБОР
		|			КОГДА &ДеятельностьНаПатенте
...
Функция ТекстЗапросаНДСВозвраты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ИЗ
	|	ТаблицаСделки КАК ТаблицаСделки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.НДСРаздельныйУчет КАК НДСРаздельныйУчет
	|		ПО ТаблицаСделки.Сделка = НДСРаздельныйУчет.Регистратор
	|			И ТаблицаСделки.Организация = НДСРаздельныйУчет.Организация
	...
	|ИЗ
	|	ТаблицаСделки КАК ТаблицаСделки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСПоПриобретеннымЦенностям КАК НДСПоПриобретеннымЦенностям
	|		ПО ТаблицаСделки.Сделка = НДСПоПриобретеннымЦенностям.Регистратор
	|			И ТаблицаСделки.Организация = НДСПоПриобретеннымЦенностям.Организация
	|ГДЕ
	...
	|	КОНЕЦ КАК ЕстьКоличество
	|ИЗ
	|	ОтборАналитикДляВозвратаПоТекущемуДокументу КАК ОтборАналитикДляВозвратаПоТекущемуДокументу
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.НДСРаздельныйУчет.Остатки(
	|				&МоментСписания,
	|				Организация = &Организация
	|					И АналитикаУчетаЗатрат В
	|						(ВЫБРАТЬ
	|							ОтборАналитикДляВозвратаПоТекущемуДокументу.АналитикаУчетаЗатрат
	|						ИЗ
	|							ОтборАналитикДляВозвратаПоТекущемуДокументу)) КАК НДСРаздельныйУчетОстатки
	|		ПО ОтборАналитикДляВозвратаПоТекущемуДокументу.АналитикаУчетаЗатрат = НДСРаздельныйУчетОстатки.АналитикаУчетаЗатрат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНДС КАК АналитикаНДС
	|		ПО (НДСРаздельныйУчетОстатки.АналитикаУчетаНДС = АналитикаНДС.КлючАналитики)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО (НДСРаздельныйУчетОстатки.Партия = ДанныеПервичныхДокументов.Документ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументовСчФ
	|		ПО (АналитикаНДС.СчетФактура = ДанныеПервичныхДокументовСчФ.Документ)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПечатьОтчетаОПродажахККМ(МассивОбъектов, ОбъектыПечати)
	...
	|		ЕСТЬNULL(СУММА(ВЫБОР
	|					КОГДА ОтчетОРозничныхПродажах.Оплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.СБП)
	|						ТОГДА ОтчетОРозничныхПродажах.Оплата.СуммаОплаты
	|					ИНАЧЕ 0
	|				КОНЕЦ), 0) КАК СБП,
	|		ЕСТЬNULL(СУММА(ВЫБОР
	|					КОГДА ОтчетОРозничныхПродажах.Оплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.ЦифровойРубль)
	|						ТОГДА ОтчетОРозничныхПродажах.Оплата.СуммаОплаты
	|					ИНАЧЕ 0
	|				КОНЕЦ), 0) КАК ЦифровойРубль,
	|		ЕСТЬNULL(СУММА(ВЫБОР
	...
	|				КОНЕЦ), 0) КАК ПлатежныеКарты,
	|		ЕСТЬNULL(СУММА(ВЫБОР
	|					КОГДА ОтчетОРозничныхПродажах.ВозвратОплаты.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.СБП)
	|						ТОГДА ОтчетОРозничныхПродажах.ВозвратОплаты.СуммаОплаты
	|					ИНАЧЕ 0
	|				КОНЕЦ), 0) КАК СБП,
	|		ЕСТЬNULL(СУММА(ВЫБОР
	|					КОГДА ОтчетОРозничныхПродажах.ВозвратОплаты.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.ЦифровойРубль)
	|						ТОГДА ОтчетОРозничныхПродажах.ВозвратОплаты.СуммаОплаты
	|					ИНАЧЕ 0
	|				КОНЕЦ), 0) КАК ЦифровойРубль,
	|		ЕСТЬNULL(СУММА(ВЫБОР
	...
	Пока Шапка.Следующий() Цикл
		...
		Если ВыборкаОплата.Следующий() Тогда
			...
			СБП                   = ВыборкаОплата.СБП;
			ЦифровойРубльСумма    = ВыборкаОплата.ЦифровойРубль;
		Иначе
			ПлатежныеКарты        = 0;
			Кредиты               = 0;
			ПодарочныеСертификаты = 0;
			СБП                   = 0;
			ЦифровойРубльСумма         = 0;
		КонецЕсли;
		...
		Если ВыборкаОплата.Следующий() Тогда
			...
			СБП                   = СБП                   - ВыборкаОплата.СБП;
			ЦифровойРубльСумма    = ЦифровойРубльСумма         - ВыборкаОплата.ЦифровойРубль;
		КонецЕсли;
		...
		ОбластьМакета.Параметры.Итого = Шапка.СуммаДокумента - Кредиты - ПлатежныеКарты - СБП - ЦифровойРубльСумма;
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета.Параметры.Итого = ПлатежныеКарты;
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета.Параметры.Итого = СБП;
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета.Параметры.Итого = ЦифровойРубльСумма;
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета.Параметры.СуммаВыручкиПрописью = ОбщегоНазначенияБПВызовСервера.СформироватьСуммуПрописью(
			СуммаИтого - ПлатежныеКарты - ПодарочныеСертификаты - СБП - ЦифровойРубльСумма,
