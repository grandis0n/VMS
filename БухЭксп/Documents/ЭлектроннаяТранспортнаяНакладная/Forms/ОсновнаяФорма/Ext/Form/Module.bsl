	...
	Если НЕ Параметры.Свойство("ФормаБезОбработки") Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда 
				Объект.ИдентификаторГрузоотправителя = Неопределено;
				Объект.ИдентификаторПеревозчика = Неопределено;
				Объект.ИдентификаторГрузополучателя = Неопределено;
			КонецЕсли;
...
Процедура ЗаполнитьЗначенияПоУмолчаниюДляТекущегоТитула(ПриСозданииНаСервере = Ложь)
	...
	Если (ЗначениеЗаполнено(Объект.Ссылка) = Ложь 
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			"ТитулГрузоотправителяСтоимостьГруза", 0, Ложь, КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
...
Процедура СкопироватьИныхПолучателейИзТитулаГрузоотправителя()
	...
	Если ЗначениеЗаполнено(Объект.ТекущийТитул) Тогда
		...
		СчетчикЦиклаОткуда = 1;
		КлючОткуда = "ТитулГрузоотправителяИдентификаторыИныхПолучателейФайла__" + СчетчикЦиклаОткуда + "__" + "ИдентификаторИногоПолучателя";
		...
		Пока СтруктураВерсииТитула.Свойство(КлючОткуда, ЗначениеОткуда) Цикл
			...
			Если ЗначениеОткуда <> Объект.ИдентификаторГрузоотправителя
				...
				СчетчикЦиклаКуда = СчетчикЦиклаКуда + 1;
				КлючКуда = ИнформацияПоПрефиксамТитула.ВПрограмме 
								+ "ИдентификаторыИныхПолучателейФайла__" + СчетчикЦиклаКуда + "__" + "ИдентификаторИногоПолучателя";
			...
			СчетчикЦиклаОткуда = СчетчикЦиклаОткуда + 1;
			КлючОткуда = "ТитулГрузоотправителяИдентификаторыИныхПолучателейФайла__" + СчетчикЦиклаОткуда + "__" + "ИдентификаторИногоПолучателя";
...
Процедура ЗаполнитьУчастников(ЭтоНастройкаПередОтправкой = Ложь)
	...
	КлючНастроекОтправкиСледующий.ВсегдаСоздаватьАвтоматическоеПриглашение = Истина;
	...
	КлючНастроекEDI = СервисEDIСлужебныйКлиент.НовыйКлючНастроек();
	
	КлючНастроекEDI.Организация  = КлючНастроекОтправкиСледующий.Отправитель;
	КлючНастроекEDI.Контрагент   = КлючНастроекОтправкиСледующий.Получатель;
	КлючНастроекEDI.Перевозчик   = КлючНастроекОтправки.Получатель;
	КлючНастроекEDI.Договор      = КлючНастроекОтправкиСледующий.Договор;
	КлючНастроекEDI.ВидДокумента = КлючНастроекОтправкиСледующий.ВидДокумента;
	
	Если СервисEDIСлужебныйКлиент.ТребуетсяНастройкаОбмена(КлючНастроекEDI) Тогда
		СервисEDIСлужебныйКлиент.ОткрытьФормуНастройкиОбменаЧерезСервис1СEDI(
			КлючНастроекEDI,
			ЭтотОбъект,
			ОповещениеОкончаниеНастройки);
	ИначеЕсли НастройкиОтправки = Неопределено И ЭтоНастройкаПередОтправкой = Истина Тогда
		НастройкиЭДОКлиент.НастроитьОбменСКонтрагентом(
			КлючНастроекОтправки, ОповещениеОкончаниеНастройки, ПараметрыНастройкиОбменаСКонтрагентом);
	Иначе
