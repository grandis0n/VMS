...
Процедура ИзменениеСтрокиОС(ЭтоНовый = Ложь, Копирование = Ложь)
	...
	СтруктураПараметров.Вставить("ДокументСозданПомощникомПереходаНаОСНО",
		ДокументСозданПомощникомПереходаНаОСНО(Объект.Ссылка));
...
&НаСервереБезКонтекста
Функция ДокументСозданПомощникомПереходаНаОСНО(Документ)
	
	Результат = Ложь;
	Если Не Документ.Пустая() Тогда
		Результат = Обработки.ПомощникПереходаСУСННаОСНО.ДокументСозданПомощником(Документ);
	КонецЕсли;
