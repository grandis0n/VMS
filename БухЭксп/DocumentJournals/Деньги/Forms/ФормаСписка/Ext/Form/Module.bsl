...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ПоказатьБаннерЗагружено", 0.1, Истина);
...
&НаКлиенте
Процедура ОтчетСверкаБанковскойВыписки(Команда)
	
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("Организация"                      , ОтборОрганизация);
	ПараметрыОтчета.Вставить("ИспользуютсяСтандартныеНастройки" , Истина);
	ПараметрыОтчета.Вставить("СформироватьОтчет"                , Ложь);
	
	ОткрытьФорму("Отчет.СверкаБанковскойВыписки.Форма.ФормаОтчета", ПараметрыОтчета,, Новый УникальныйИдентификатор);
...
Процедура ТекстРасхожденияНажатие(Элемент)
	...
	ПараметрыОтчета = Новый Структура;
	Если СтруктураПериодовРасхождения.Свойство("БанковскийСчет") Тогда
		ПараметрыОтчета.Вставить("БанковскийСчет", СтруктураПериодовРасхождения.БанковскийСчет);
	Иначе
		ПараметрыОтчета.Вставить("БанковскийСчет", ПредопределенноеЗначение("Справочник.БанковскиеСчета.ПустаяСсылка"));
	КонецЕсли;
	ПараметрыОтчета.Вставить("Организация"                      , ОтборОрганизация);
	ПараметрыОтчета.Вставить("НачалоПериода"                    , СтруктураПериодовРасхождения.НачалоПериода);
	ПараметрыОтчета.Вставить("КонецПериода"                     , СтруктураПериодовРасхождения.КонецПериода);
	ПараметрыОтчета.Вставить("ИспользуютсяСтандартныеНастройки" , Истина);
	ПараметрыОтчета.Вставить("СформироватьОтчет"                , Истина);
...
&НаКлиенте
Процедура НадписьОбучениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	УправлениеПанельюПодсказкиКлиент.НадписьОбучениеОбработкаНавигационнойСсылки(
		"РазделДеньги",
		"",
		СтандартнаяОбработка);
...
Функция АктуальнаяДатаЗагрузкиВыписки(БанковскийСчет)
	
	Возврат Обработки.КлиентБанк.АктуальнаяДатаЗагрузкиВыписки(БанковскийСчет);
...
Процедура ЗагрузитьВыписку(Организация, БанковскийСчет)
	...
	ИначеЕсли НастройкиБанковскогоСчета.ЕстьПрямойОбменСБанками Тогда
		...
		ИсточникЗагрузки = ПредопределенноеЗначение("Перечисление.ИсточникиЗагрузкиБанковскихВыписок.ДиректБанк");
	...
	Иначе 
		ИсточникЗагрузки = ПредопределенноеЗначение("Перечисление.ИсточникиЗагрузкиБанковскихВыписок.Файл");
...
Процедура ОбработатьЗагрузкуБанковскойВыписки(РезультатЗагрузки)
	...
	ИначеЕсли РезультатЗагрузки.РезультатВыполнения = "УспешнаяЗагрузка" Тогда
		...
		АдресХранилищаРаспознанныеДанныеИзБанка = РезультатЗагрузки.АдресХранилищаРаспознанныеДанныеИзБанка;
		
		// Заполнение регистров данными из выписки и оперциями для использования в аналитике расхождений
		ПодключитьОбработчикОжидания("Подключаемый_ЗаписатьВРегистрыВыписокИОпераций", 0.1, Истина);
...
&НаСервере
Функция ЗаписатьВРегистрыВыписокИОпераций(АдресХранилища)
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("РаспознанныеДанныеИзБанка", ПолучитьИзВременногоХранилища(АдресХранилища));
	СтруктураПараметров.Вставить("ИсточникЗагрузки", ИсточникЗагрузки);
...
&НаКлиенте
Процедура ПослеЗаписиВРегистрыБанкВыписок(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		ДобавитьКБаннеруСообщениеОРасхождении(АдресРезультата);
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		СообщитьОбОшибкеДлительнойОперации(ДлительнаяОперация);
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьКБаннеруСообщениеОРасхождении(АдресРезультата)
	...
	МассивБанкСчетов = Результат.МассивБанкСчетов; 
	ДатаРасхождения  = Дата(1, 1, 1);
	...
	Элементы.ГруппаСостояниеЗагрузкиИзКлиентБанка.ЦветФона = ЦветаСтиля.БыстрыеОтборыФонГруппы;
	
	Если ЗначениеЗаполнено(ДатаРасхождения) Тогда
		ТекстЗаголовка = НСтр("ru='Обнаружены расхождения при сверке выписки на начало %1'");
		ШаблонЗаголовка = СтрШаблон(ТекстЗаголовка, Формат(ДатаРасхождения, "ДФ=dd.MM.yyyy"));
		
		Элементы.ТекстРасхождения.Видимость = Истина;
		Элементы.ТекстРасхождения.Заголовок = ШаблонЗаголовка;
		Элементы.ГруппаСостояниеЗагрузкиИзКлиентБанка.ЦветФона = ЦветаСтиля.ОшибкаПолнотекстовыйПоискФон;
		
		СтруктураПериодовРасхождения = Новый Структура;
		СтруктураПериодовРасхождения.Вставить("НачалоПериода", ДатаРасхождения);
		СтруктураПериодовРасхождения.Вставить("КонецПериода", КонецПериода);
		Если МассивБанкСчетов.Количество() = 1 Тогда
			СтруктураПериодовРасхождения.Вставить("БанковскийСчет", МассивБанкСчетов[0]);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоказатьРезультатСозданияДокументов(РезультатСозданияДокументов, ВалютаСчета)
	...
	Элементы.ТекстРасхождения.Видимость = Ложь;
...
Процедура ТекстСостояниеАвтозагрузкиОшибкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки =
		...
		Отбор.СобытиеЖурналаРегистрации = ИмяСобытияЖурналаРегистрацииДиректБанк();
...
&НаКлиенте
Процедура Подключаемый_ПоказатьБаннерЗагружено()
	
	Элементы.ТекстЗагружено.Заголовок ="";
	Элементы.ГруппаЗагружено.Видимость = Ложь;
	
	Если ЗначениеЗаполнено(ОтборБанковскийСчет) И ОтборБанковскийСчетИспользование Тогда
		Элементы.ТекстЗагружено.Заголовок ="";
		АктуальнаяДатаЗагрузки = АктуальнаяДатаЗагрузкиВыписки(ОтборБанковскийСчет);
		Если АктуальнаяДатаЗагрузки = Дата(1,1,1) Тогда
			Элементы.ГруппаЗагружено.Видимость = Ложь;
		Иначе
			ТекстДатыЗагрузки = Формат(АктуальнаяДатаЗагрузки, "ДФ=dd.MM.yyyy");
			ТекстЗаголовка = НСтр("ru='Загружено по: %1'");
			ШаблонЗаголовка = СтрШаблон(ТекстЗаголовка, ТекстДатыЗагрузки);
			Элементы.ТекстЗагружено.Заголовок = ШаблонЗаголовка;
			Элементы.ГруппаЗагружено.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ЗаписатьВРегистрыВыписокИОпераций()
	
	ДлительнаяОперация = ЗаписатьВРегистрыВыписокИОпераций(АдресХранилищаРаспознанныеДанныеИзБанка);
	...
	Иначе
		...
		Обработчик = Новый ОписаниеОповещения("ПослеЗаписиВРегистрыБанкВыписок", ЭтотОбъект);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Обработчик, НастройкиОжидания);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ИмяСобытияЖурналаРегистрацииДиректБанк()
	
	ИмяСобытияДиректБанк = ОбменСБанками.ИмяСобытияЖурналаРегистрации();
	КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	Возврат СтрШаблон(НСтр("ru = '%1.Журнал'", КодОсновногоЯзыка), ИмяСобытияДиректБанк);
