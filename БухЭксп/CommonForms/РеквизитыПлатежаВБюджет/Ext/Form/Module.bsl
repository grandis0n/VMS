...
Процедура ЗаполнитьПоУмолчанию(Знач ИмяИзмененногоРеквизита, Знач НастраиватьФорму = Истина)
	...
	Если Изменения.Свойство("КодТерритории") И ПрименяетсяОКТМО8Символов Тогда
		ПлатежиВБюджетКлиентСервер.ПривестиКодТерриторииКФорматуПриказа58н(Изменения.КодТерритории);
		Изменения.Вставить("КодТерритории8", Изменения.КодТерритории);
	КонецЕсли;
