...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИспользоватьОплатуЧерезАгентаПоставщика") Тогда
		ИспользоватьОплатуЧерезАгентаПоставщика  = Параметры.ИспользоватьОплатуЧерезАгентаПоставщика;
		ОплатаЧерезКомиссионера                  = Параметры.ОплатаЧерезАгентаПоставщика;
		Комиссионер                              = Параметры.АгентПоставщика;
		ДоговорКомиссионера                      = Параметры.ДоговорАгентаПоставщика;
		СчетУчетаРасчетовСКомиссионером          = Параметры.СчетУчетаРасчетовСАгентомПоставщика;
		СчетУчетаРасчетовСКомиссионеромПоАвансам = Параметры.СчетУчетаРасчетовСАгентомПоставщикаПоАвансам;
		
		Элементы.ОплатаЧерезКомиссионера.Заголовок = НСтр("ru = 'Оплата через агента поставщика (маркетплейс)'");
		Элементы.Комиссионер.Заголовок             = НСтр("ru = 'Агент поставщика'");
		
		Элементы.ДоговорКомиссионера.ПараметрыВыбора = Новый ФиксированныйМассив(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
				Новый ПараметрВыбора("Отбор.ВидДоговора", Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком)));
	КонецЕсли;
	
	Если ИспользоватьОплатуЧерезПлатежногоАгента Тогда
		Элементы.ОплатаЧерезКомиссионера.Подсказка =
			НСтр("ru = 'Перенести задолженность покупателя на платежного агента (маркетплейс).'");
		Элементы.ОплатаЧерезКомиссионера.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	ИначеЕсли ИспользоватьОплатуЧерезАгентаПоставщика Тогда
		Элементы.ОплатаЧерезКомиссионера.Подсказка =
			НСтр("ru = 'Перенести задолженность перед поставщиком на его представителя (агента, маркетплейс).'");
		Элементы.ОплатаЧерезКомиссионера.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	КонецЕсли;
	...
	Элементы.ГруппаКомиссионер.Видимость = ИспользоватьОплатуЧерезПлатежногоАгента
		Или ИспользоватьОплатуЧерезАгентаПоставщика;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПеренестиВДокумент Тогда
		...
		ИначеЕсли ИспользоватьОплатуЧерезАгентаПоставщика Тогда
			СтруктураВозврата.Вставить("ОплатаЧерезАгентаПоставщика", ОплатаЧерезКомиссионера);
			СтруктураВозврата.Вставить("АгентПоставщика", Комиссионер);
			СтруктураВозврата.Вставить("ДоговорАгентаПоставщика", ДоговорКомиссионера);
			СтруктураВозврата.Вставить("СчетУчетаРасчетовСАгентомПоставщика", СчетУчетаРасчетовСКомиссионером);
			СтруктураВозврата.Вставить("СчетУчетаРасчетовСАгентомПоставщикаПоАвансам", СчетУчетаРасчетовСКомиссионеромПоАвансам);
		КонецЕсли;
...
&НаКлиенте
Процедура ДоговорКомиссионераПриИзменении(Элемент)
	
	ЗаполнитьСчетаУчетаРасчетовСКомиссионером();
...
Функция ПараметрыСозданияНовогоДоговораКомиссионера()
	...
	ИначеЕсли ИспользоватьОплатуЧерезАгентаПоставщика Тогда
		ДоступныеВидыДоговоров.Добавить(Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСчетаУчетаРасчетовСКомиссионером()
	
	СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаРасчетовСКонтрагентом(
		Организация, Комиссионер, ДоговорКомиссионера);
	
	Если ИспользоватьОплатуЧерезАгентаПоставщика Тогда
		СчетУчетаРасчетовСКомиссионером = СчетаУчета.СчетРасчетов;
	ИначеЕсли ИспользоватьОплатуЧерезПлатежногоАгента Тогда
		СчетУчетаРасчетовСКомиссионером = СчетаУчета.СчетРасчетовПокупателя;
	КонецЕсли;

	Если ИспользоватьОплатуЧерезАгентаПоставщика Тогда
		СчетУчетаРасчетовСКомиссионеромПоАвансам = СчетаУчета.СчетАвансов;
	ИначеЕсли ИспользоватьОплатуЧерезПлатежногоАгента Тогда
		СчетУчетаРасчетовСКомиссионеромПоАвансам = СчетаУчета.СчетАвансовПокупателя;
	КонецЕсли;
