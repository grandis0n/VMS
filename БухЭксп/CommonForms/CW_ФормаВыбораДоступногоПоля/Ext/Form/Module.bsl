...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.СхемаКомпоновкиДанных) Тогда 
		скд = ПолучитьИзВременногоХранилища(Параметры.СхемаКомпоновкиДанных);
	Иначе 
		скд = Параметры.СхемаКомпоновкиДанных;
	КонецЕсли;
	адр = ПоместитьВоВременноеХранилище(скд, ЭтаФорма.УникальныйИдентификатор); // поместим в привязке к данной форме
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(адр)); // теперь адрес можно забыть
