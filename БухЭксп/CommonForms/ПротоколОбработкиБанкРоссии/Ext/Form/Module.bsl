...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипИсточника = Тип("СправочникСсылка.УдалитьОтправкиБанкРоссии") Тогда
		Отправка = Параметры.ИсточникСсылка;	
	Иначе //РегламентированныйОтчет или ЭлектронноеПредставление
		Отправка = УдалитьДокументооборотСБанкомРоссии.ПолучитьПоследнююОтправкуОтчета(Параметры.ИсточникСсылка);
	КонецЕсли;
