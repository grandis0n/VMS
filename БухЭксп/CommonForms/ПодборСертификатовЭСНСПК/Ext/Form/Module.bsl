...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КодыТоваровТРУ.Колонки.Добавить("ДатаНачалаДействия", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	КодыТоваровТРУ.Колонки.Добавить("ДатаЗавершенияДействия", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	КодыТоваровТРУ.Колонки.Добавить("Приоритет", Новый ОписаниеТипов("Число"));
	КодыТоваровТРУ.Колонки.Добавить("Процент", Новый ОписаниеТипов("Число"));
	КодыТоваровТРУ.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число"));
	
	Если Параметры.Свойство("ДоступныеСертификаты") Тогда
		Для Каждого Сертификат Из Параметры.ДоступныеСертификаты Цикл // Структура
			Для Каждого Товар Из Сертификат.КодыТоваровТРУ Цикл
				...
				СтрокаСертификата.ДатаНачалаДействия = Сертификат.ДатаНачалаДействия;
				СтрокаСертификата.ДатаЗавершенияДействия = Сертификат.ДатаЗавершенияДействия;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	
	Если Параметры.Свойство("ДоступныеСкидки") Тогда
		Для Каждого Скидка Из Параметры.ДоступныеСкидки Цикл // Структура
			Для Каждого Товар Из Скидка.КодыТоваровТРУ Цикл
				СтрокаСкидки = КодыТоваровТРУ.Добавить();
				СтрокаСкидки.КодТРУ = Товар;
				СтрокаСкидки.ДатаНачалаДействия = Скидка.ДатаНачалаДействия;
				СтрокаСкидки.ДатаЗавершенияДействия = Скидка.ДатаЗавершенияДействия;
				СтрокаСкидки.Приоритет = Скидка.Приоритет;
				СтрокаСкидки.Процент = Скидка.Процент;
				СтрокаСкидки.Сумма = Скидка.Сумма;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
