...
Процедура ЗагружаемыеФайлыWBДокументПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Документ) Тогда
		...
		Если СсылкиНаДокументы.Количество() > 0 Тогда
			...
			ТекущиеДанные.ДействиеСДокументом = СсылкиНаДокументы[0].ДействиеСДокументом;
			Если Не ТекущиеДанные.ЭтоПодстрока Тогда
				ТекущиеДанные.Загружать = Истина;
				ТекущиеДанные.НайденоНесколькоДокументов = Ложь;
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		ТекущиеДанные.ДействиеСДокументом = "";
		Если Не ТекущиеДанные.ЭтоПодстрока Тогда
			ТекущиеДанные.Загружать = Истина;
			ТекущиеДанные.НайденоНесколькоДокументов = Ложь;
		КонецЕсли;
...
&НаКлиенте
Процедура ЗагружаемыеФайлыWBДокументАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ТекущиеДанные = Элементы.ЗагружаемыеФайлыWB.ТекущиеДанные;
	ПараметрыПодбораДокумента = ПараметрыПодбораДокумента(ТекущиеДанные.ВидДокумента, ТекущиеДанные.Документ);
	Элемент.ОграничениеТипа = ПараметрыПодбораДокумента.Тип;
	...
	СсылкиНаДокументы = ДокументыИБ.НайтиСтроки(Новый Структура("КлючСтроки", ТекущиеДанные.КлючСтроки));
	Для Каждого ДокументИБ ИЗ СсылкиНаДокументы Цикл
		Список.Добавить(ДокументИБ.Документ,
			СтрШаблон(НСтр("ru = 'Заменить: %1'"),
				?(ЗначениеЗаполнено(ДокументИБ.ПредставлениеДокумента), ДокументИБ.ПредставлениеДокумента, ДокументИБ.Документ)));
		ОтборПоДокументам.Добавить(ДокументИБ.Документ);
	КонецЦикла;
	...
	Иначе
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Организация", Организация));
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Контрагент", КонтрагентWB));
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.ВидОперации", ПараметрыПодбораДокумента.ВидОперации));
	КонецЕсли;
...
Процедура ЗагружаемыеФайлыWBВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ЗагружаемыеФайлыWBДокумент" Тогда
		...
		Если ЭтоРеестрПродажЮрлицамWB(ТекущиеДанные.ВидДокумента)
	...
	ИначеЕсли Поле.Имя = "ЗагружаемыеФайлыWBДействие" Тогда
		Если Не ВыполняетсяЗагрузка Тогда
			...
			ИначеЕсли СтрНайти(ТекущиеДанные.Действие, НСтр("ru = 'Изменить настройку интеграции'")) > 0 Тогда
				ОткрытьНастройкиИнтеграцииWildberries();
			Иначе
				ВыполнитьЗагрузкуФайловИнтерактивно();
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Поле.Имя = "ЗагружаемыеФайлыWBДействиеСДокументом" Тогда
		СписокВыбора = Элементы.ЗагружаемыеФайлыWBДействиеСДокументом.СписокВыбора;
		СписокВыбора.Очистить();
		СписокВыбора.Добавить(ДействиеЗаменитьДокумент());
		Если ИспользоватьКорректировочныеДокументы Тогда
			СписокВыбора.Добавить(ДействиеСоздатьКорректировку());
		КонецЕсли;
	КонецЕсли;
...
Процедура Загрузить(Команда)
	...
	Если ТребуетсяУказыватьСклад И Не ЗначениеЗаполнено(Склад) Тогда
		...
		ИначеЕсли Маркетплейс = ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.Wildberries") Тогда
			ТекстСообщения = НСтр("ru = 'Не указан склад отгрузки'");
		Иначе
			ВызватьИсключение НСтр("ru = 'Ошибка при проверке заполнения склада'");
		КонецЕсли;
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Склад");
		Возврат;
	КонецЕсли;
	
	Если Маркетплейс = ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.OZON")
		И Не ЗначениеЗаполнено(КонтрагентOzon) Тогда
		ТекстСообщения = НСтр("ru = 'Не указан контрагент Ozon'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "КонтрагентOzon");
		Возврат;
	КонецЕсли;
	
	Если Маркетплейс = ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.Wildberries")
		И Не ЗначениеЗаполнено(КонтрагентWB) Тогда
		ТекстСообщения = НСтр("ru = 'Не указан контрагент Wildberries'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "КонтрагентWB");
		Возврат;
	КонецЕсли;
	
	Если Маркетплейс = ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.Мегамаркет")
		И Не ЗначениеЗаполнено(КонтрагентМегамаркет) Тогда
		ТекстСообщения = НСтр("ru = 'Не указан контрагент Мегамаркета'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "КонтрагентМегамаркет");
		Возврат;
	КонецЕсли;
...
Функция ЗапуститьФоновыйРазборФайлов()
	...
	Для Каждого Файл Из Файлы Цикл 
		...
		НаименованиеФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			Файл.Наименование);
		РасширениеФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			Файл.Расширение);
		
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("ИмяФайла", НаименованиеФайла);
		ОписаниеФайла.Вставить("РасширениеФайла", РасширениеФайла);
		ОписаниеФайла.Вставить("ПолноеИмяФайла", НаименованиеФайла);
...
Процедура ОтобразитьРезультатЧтенияФайлов()
	...
	Если ТипЗнч(ДанныеФайлов) = Тип("Массив") Тогда
		...
		Для Каждого Файл Из ДанныеФайлов Цикл
			Если Файл.Свойство("ТекстОшибки") Тогда
				Продолжить;
			КонецЕсли;
			Если Файл.Свойство("СообщениеПользователю") Тогда
				ОбщегоНазначения.СообщитьПользователю(Файл.СообщениеПользователю);
			КонецЕсли;
	...
	ОпределитьНаличиеСкладаВДокументах();
	ПолучитьНастройки();
...
Процедура НайтиДокументыИБ(ТаблицаДанных, РеквизитыФормы)
	...
	|ПОМЕСТИТЬ ВТ_ДокументыРеализацииКоличество
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
	|ГДЕ
	|	РеализацияТоваровУслугТовары.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТ_ДокументыРеализации.Ссылка
	|			ИЗ
	|				ВТ_ДокументыРеализации)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ПОМЕСТИТЬ ВТ_ПоследниеДокументыКорректировки
	|ИЗ
	|	Документ.КорректировкаРеализации КАК КорректировкаРеализации
	...
	|			(ВЫБРАТЬ
	|				ВТ_ДокументыКорректировки.Ссылка
	|			ИЗ
	|				ВТ_ДокументыКорректировки)
	|	И НЕ КорректировкаРеализации.ДокументРеализации В
	|				(ВЫБРАТЬ
	|					ВТ_ДокументыКорректировки.Ссылка
	|				ИЗ
	|					ВТ_ДокументыКорректировки)
	...
	ТекстЗапросаПередачаНаРеализацию = "
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТ_ПоследниеДокументыКорректировки.Ссылка, РеализацияТоваровУслуг.Ссылка),
	...
	|	РеализацияТоваровУслуг.СуммаДокумента,
	|	0,
	|	СУММА(ЕСТЬNULL(ВТ_ПоследниеДокументыКорректировки.Количество, ВТ_ДокументыРеализацииКоличество.Количество)),
	|	РеализацияТоваровУслуг.НомерВходящегоДокумента,
	|	РеализацияТоваровУслуг.ДоговорКонтрагента
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДокументыРеализацииКоличество КАК ВТ_ДокументыРеализацииКоличество
	|		ПО (РеализацияТоваровУслуг.Ссылка = ВТ_ДокументыРеализацииКоличество.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПоследниеДокументыКорректировки КАК ВТ_ПоследниеДокументыКорректировки
	|		ПО (РеализацияТоваровУслуг.Ссылка = ВТ_ПоследниеДокументыКорректировки.ИсходныйДокументРеализации)
	|ГДЕ
	...
	ТекстЗапроса = ТекстЗапроса + ТекстЗапросаВыкуп + ТекстЗапросаПередачаНаРеализацию;
	
	МассивПериодов = Новый Массив;
	Для Каждого СтрокаДанных из ТаблицаДанных Цикл
		Если СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаWBУведомлениеВыкупа()
			ИЛИ СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonВыкуп()
			Или СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
	...
	Для Каждого СтрокаДанных из ТаблицаДанных Цикл
		...
		ИначеЕсли СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаАктПриемкиТовараWB() Тогда
			НачалоПериода = НачалоДня(СтрокаДанных.Дата);
		...
		Если СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzon()
			...
			Если Не ЗначениеЗаполнено(НайденныеДокументы) Тогда
				...
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonВыкуп()
			Или СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
			НайденныеДокументы = ТаблицаДокументов.НайтиСтроки(Новый Структура("НачалоПериода, ВидОперации, ДоговорКонтрагента",
				НачалоПериода, ВидОперации, ДоговорКонтрагентаOzon));
			Если Не ЗначениеЗаполнено(НайденныеДокументы) Тогда
			...
			ИначеЕсли НайденныеДокументы.Количество() > 1 Тогда
				НайденныеДокументыДопПоиск = ТаблицаДокументов.НайтиСтроки(Новый Структура("НачалоПериода, ВидОперации, ДоговорКонтрагента, НомерВходящегоДокумента",
					НачалоПериода, ВидОперации, ДоговорКонтрагентаOzon, СтрокаДанных.НомерВходящегоДокумента));
				Если ЗначениеЗаполнено(НайденныеДокументыДопПоиск) Тогда
					НайденныеДокументы = НайденныеДокументыДопПоиск;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СтрокаДанных.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаАктПриемкиТовараWB() Тогда
			НайденныеДокументы = ТаблицаДокументов.НайтиСтроки(Новый Структура("НачалоПериода, ВидОперации, НомерВходящегоДокумента",
		...
		Если НайденныеДокументы.Количество() > 0 Тогда
			...
			Для Каждого ДокументИБ из НайденныеДокументы Цикл
				...
				НоваяСтрока.ПредставлениеДокумента = ПредставлениеДокумента(ДокументИБ);
				...
				ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПередачаТоваров Тогда
					
					РазницаВКоличествеДокумента = СтрокаДанных.СуммаКоличество - ДокументИБ.СуммаКоличество;
					Если РазницаВКоличествеДокумента <> 0 Тогда
						НоваяСтрока.Представление = НоваяСтрока.Представление + Символы.ПС
							+ НСтр("ru='Данные по товарам в документе отличаются'");
						НоваяСтрока.ДействиеСДокументом = ДействиеЗаменитьДокумент();
						Если НайденныеДокументы.Количество() = 1 Тогда
							СтрокаДанных.ДействиеСДокументом = ДействиеЗаменитьДокумент();
						КонецЕсли;
					КонецЕсли;
				...
				Если Не ЗначениеЗаполнено(СтрокаДанных.Документ) Тогда
					...
					Если НайденныеДокументы.Количество() > 1 Тогда
						...
						СтрокаДанных.Загружать = Ложь;
						СтрокаДанных.НайденоНесколькоДокументов = Истина;
					Иначе
...
Процедура ЗаполнитьПодчиненныеДокументыРеестраПродажЮрлицамOzon(СтрокаРеестра, Файл, ДокументыИБ, Организация, Контрагент)
	...
	Для Каждого ПодчиненныйДокумент Из ПодчиненныеДокументы Цикл
		...
		Если СтрокаДокументаИБ <> Неопределено Тогда
			...
			ЗаполнитьПредставлениеПодчиненногоДокумента(
				НоваяПодстрока, ДокументыИБ, СтрокаДокументаИБ.Документ, НомерДокумента, Разница);
	...
	ЗаполнитьПредставлениеРодителяПодчиненныхДокументов(
		СтрокаРеестра, ПодчиненныеДокументы, КоличествоНайденныхДокументов, Разница);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеРодителяПодчиненныхДокументов(СтрокаРодителя, ПодчиненныеДокументы, КоличествоНайденныхДокументов, Разница)
	
	СтрокаРодителя.ДокументПредставление  = НСтр("ru='Создать новые документы'");
	СтрокаРодителя.ДокументПредставление1 = НСтр("ru='Создать новые документы'");
	СтрокаРодителя.ДокументПредставление2 = "";
	Если ПодчиненныеДокументы.Количество() > 0 Тогда
		
		Если КоличествоНайденныхДокументов > 0 Тогда
			СтрокаРодителя.ДокументПредставление  = НСтр("ru='Документы программы:'");
			СтрокаРодителя.ДокументПредставление1 = НСтр("ru='Документы программы:'");
			СтрокаРодителя.ДокументПредставление2 = "";
			
			РазницаКоличество = ПодчиненныеДокументы.Количество() - КоличествоНайденныхДокументов;
			Если РазницаКоличество > 0 Тогда
				ТекстШаблона = НСтр("ru='Найдено на %1 меньше чем в файле'");
				ДокументСтрокой = ПолучитьСклоненияСтрокиПоЧислу("документ", РазницаКоличество);
				СтрокаРодителя.ДокументПредставление  = СтрокаРодителя.ДокументПредставление
					+ Символы.ПС + СтрШаблон(ТекстШаблона, ДокументСтрокой[0]);
				СтрокаРодителя.ДокументПредставление1 = СтрокаРодителя.ДокументПредставление;
			КонецЕсли;
		КонецЕсли;
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаРодителя.ДокументПредставление) Тогда
			МассивПодстрок = СтрРазделить(СтрокаРодителя.ДокументПредставление, Символы.ПС, Ложь);
			Если МассивПодстрок.Количество() > 0 Тогда
				СтрокаРодителя.ДокументПредставление1 = МассивПодстрок[0];
			Иначе
				СтрокаРодителя.ДокументПредставление1 = СтрокаРодителя.ДокументПредставление;
			КонецЕсли;
			Если МассивПодстрок.Количество() > 1 Тогда
				СтрокаРодителя.ДокументПредставление2 = МассивПодстрок[1];
				Для Сч = 2 по МассивПодстрок.ВГраница() Цикл
					СтрокаРодителя.ДокументПредставление2 = СтрокаРодителя.ДокументПредставление2
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеПодчиненногоДокумента(СтрокаПодчиненногоДокумента, ДокументыИБ, ДокументСсылка, КлючСтроки, Разница)
	
	НоваяСтрокаДокументыИБ = ДокументыИБ.Добавить();
	НоваяСтрокаДокументыИБ.Документ      = ДокументСсылка;
	НоваяСтрокаДокументыИБ.КлючСтроки    = КлючСтроки;
	НоваяСтрокаДокументыИБ.Представление = СтрШаблон(НСтр("ru='Заменить %1'"), ДокументСсылка);
	...
	СтрокаПодчиненногоДокумента.Документ = ДокументСсылка;
	СтрокаПодчиненногоДокумента.ДокументПредставление  = НоваяСтрокаДокументыИБ.Представление;
	СтрокаПодчиненногоДокумента.ДокументПредставление1 = НоваяСтрокаДокументыИБ.Представление1;
	СтрокаПодчиненногоДокумента.ДокументПредставление2 = НоваяСтрокаДокументыИБ.Представление2;
...
Функция ЗапуститьФоновоеСозданиеДокументов()
	...
	Для Каждого Строка Из ЗагружаемыеФайлыWB.ПолучитьЭлементы() Цикл
		Если Строка.Загружать Тогда
			...
			Если ЗагружаемаяСтруктура.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаАктПриемкиТовараWB() Тогда
				ЗагружаемаяСтруктура.ДанныеДокумента.Вставить("Склад", Склад);
				Если Строка.ДействиеСДокументом = ДействиеСоздатьКорректировку() Тогда
					ЗагружаемаяСтруктура.Вставить("СоздатьКорректировкуРеализации", Истина);
				ИначеЕсли ТипЗнч(Строка.Документ) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					ЗагружаемаяСтруктура.Вставить("СоздатьКорректировкуРеализации", Ложь);
				КонецЕсли;
			КонецЕсли;
		...
		Если Строка.Загружать Тогда
			...
			Иначе
				Если ЗначениеЗаполнено(Строка.Документ) Тогда
					ЗагружаемаяСтруктура.ДанныеДокумента.Вставить("Ссылка", Строка.Документ);
				КонецЕсли;
				Если ЗагружаемаяСтруктура.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonВыкуп()
					Или ЗагружаемаяСтруктура.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.Склад.Видимость = Форма.ТребуетсяУказыватьСклад;
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеФайлыWBДействиеСДокументом");
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеФайлыWBДействиеСДокументом");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "ЗагружаемыеФайлыWB.ДействиеСДокументом", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеФайлыWBЗагружать");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "ЗагружаемыеФайлыWB.НайденоНесколькоДокументов", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеФайлыOzonЗагружать");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "ЗагружаемыеФайлыOzon.НайденоНесколькоДокументов", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеФайлыWBДокумент");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "ЗагружаемыеФайлыWB.Документ", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "ЗагружаемыеФайлыWB.ДокументПредставление1", ВидСравненияКомпоновкиДанных.Содержит, "выберите подходящий из списка");
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветОсобогоТекста);
...
Процедура УстановитьВидимостьЗамещаемогоДокумента(Знач Таблица, ЭлементФормы)
	...
	Для Каждого Строка Из Таблица Цикл
		Если ЗначениеЗаполнено(Строка.Документ)
			Или Строка.НайденоНесколькоДокументов Тогда
...
Процедура ПолучитьНастройки()
	...
	Если Не ТребуетсяУказыватьСклад Тогда
		Возврат;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПараметрыПодбораДокумента(ВидДокумента, Документ = Неопределено)
	...
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonВыкуп() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
		ПараметрыПодбора.ПустаяСсылка = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
	...
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBДетальный() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.ОтчетКомиссионераОПродажах");
		ПараметрыПодбора.ПустаяСсылка = Документы.ОтчетКомиссионераОПродажах.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBСписание() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.ОтчетКомиссионераОПродажах");
		ПараметрыПодбора.ПустаяСсылка = Документы.ОтчетКомиссионераОПродажах.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBОприходование() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.ОтчетКомиссионераОПродажах");
		ПараметрыПодбора.ПустаяСсылка = Документы.ОтчетКомиссионераОПродажах.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаРеестрПродажЮрлицамWB() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
		ПараметрыПодбора.ПустаяСсылка = Документы.ОтчетКомиссионераОПродажах.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаТоваровСоСкладаКомиссионера;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаWBУведомлениеВыкупа() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
		ПараметрыПодбора.ПустаяСсылка = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ВыкупТоваровКомиссионером;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаДетализацияПоВыкупленнымТоварамOzon() Тогда
		ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
		ПараметрыПодбора.ПустаяСсылка = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ВыкупТоваровКомиссионером;
	ИначеЕсли ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаАктПриемкиТовараWB() Тогда
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.КорректировкаРеализации");
			ПараметрыПодбора.ПустаяСсылка = Документы.КорректировкаРеализации.ПустаяСсылка();
		Иначе
			ПараметрыПодбора.Тип = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
			ПараметрыПодбора.ПустаяСсылка = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
		КонецЕсли;
		ПараметрыПодбора.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПередачаТоваров;
	КонецЕсли;
...
&НаСервере
Процедура ОпределитьНаличиеСкладаВДокументах()
	
	ЕстьДокументыСоСкладом = Ложь;
	Для Каждого СтрокаТаблицы Из ЗагружаемыеФайлыWB.ПолучитьЭлементы() Цикл
		Если СтрокаТаблицы.ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаАктПриемкиТовараWB() Тогда
			ЕстьДокументыСоСкладом = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	ТребуетсяУказыватьСклад = ВедетсяСкладскойУчет
		И (Маркетплейс = Перечисления.ВидыМаркетплейсов.Мегамаркет Или ЕстьДокументыСоСкладом);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ДействиеЗаменитьДокумент()
	Возврат НСтр("ru='Заменить документ'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ДействиеСоздатьКорректировку()
	Возврат НСтр("ru='Создать корректировку'");
КонецФункции

&НаСервереБезКонтекста
Функция ПредставлениеДокумента(ДокументИБ)
	Если Не ЗначениеЗаполнено(ДокументИБ.Ссылка) Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат СтрШаблон("%1 № %2%3 от %4",
		ОбщегоНазначения.ПредставлениеОбъекта(ДокументИБ.Ссылка.Метаданные()),
		ДокументИБ.Ссылка.Номер,
		?(ЗначениеЗаполнено(ДокументИБ.НомерВходящегоДокумента), " (" + ДокументИБ.НомерВходящегоДокумента + ")", ""),
		Формат(ДокументИБ.Ссылка.Дата, "ДФ=dd.MM.yyyy"));
