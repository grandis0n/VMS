...
Процедура СертификатОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		ПараметрыВызова = Новый Структура("Отпечаток", Сертификат);
		КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(МестоХраненияКлюча, ПараметрыВызова);
		КриптографияЭДКОКлиент.ПоказатьСертификат(ПараметрыВызова);
	КонецЕсли;
