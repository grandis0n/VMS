...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоЮрЛицо 
		И (НастройкиСистемыНалогообложения.ПлательщикНалогаНаПрибыль
		Или СписокНедействующихНастроек.НайтиПоЗначению("ПлательщикНалогаНаПрибыль") <> Неопределено) Тогда
		НастройкиУчетаНалогаНаПрибыльФормыКлиент.ПриОткрытии(ЭтотОбъект);
	КонецЕсли;
...
&НаКлиенте
Процедура УчитыватьИзменениеСтавкиПриИсправленииПриИзменении(Элемент)
	
	ИзменениеНастроекПримененияСт54();
	
КонецПроцедуры

&НаКлиенте
Процедура УчитыватьИзменениеСтавкиПриВозвратеПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПерейтиВРазделНалогНаПрибыль()
	
	СтруктураПоиска = Новый Структура("Идентификатор", "НалогНаПрибыль");
	МассивИдентификаторов = Список.НайтиСтроки(СтруктураПоиска);
	
	Если МассивИдентификаторов.Количество() > 0 Тогда
		Элементы.Список.ТекущаяСтрока = МассивИдентификаторов[0].ПолучитьИдентификатор();
		Элементы.СтраницыНастроек.ТекущаяСтраница = Элементы.Страница_НалогНаПрибыль;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьДокументыИсправленияПрошлыхПериодов(Команда)
	
	Если Модифицированность Тогда
		Оповещение = Новый ОписаниеОповещения("ВопросЗаписатьНастрокиИсправленияПрошлыхПериодов", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Для перехода к списку документов нужно записать настройки. Записать?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		НастройкиУчетаНалогаНаПрибыльФормыКлиент.ОткрытьСписокДокументовИсправленияПрошлыхПериодов(ЭтотОбъект);
	КонецЕсли;
...
Процедура ПрочитатьНастройкиОрганизации()
	...
	Если ЭтоЮрЛицо 
		И (НастройкиСистемыНалогообложения.ПлательщикНалогаНаПрибыль
		Или СписокНедействующихНастроек.НайтиПоЗначению("ПлательщикНалогаНаПрибыль") <> Неопределено) Тогда
		НастройкиУчетаНалогаНаПрибыльФормыКлиент.ПроверитьНастройкиПримененияСт54(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаписатьИзменения(ИмяРегистра)
	...
	ИначеЕсли ИмяРегистра <> "НастройкиУчетаСтраховыхВзносовИП" Или НастройкиВзносовИП_ФССИзменены Тогда
		...
		КонецЕсли;
		МенеджерЗаписи.Записать(Истина);
		Если ИмяРегистра = "НастройкиУчетаНалогаНаПрибыль" И ВариантНастройкиПрямыхРасходов = 0 Тогда
			// Скопируем связанные настройки из прошлого периода.
			ЗаполнятьДаннымиЗаПредыдущийГод = Истина;
...
&НаКлиенте
Процедура ВопросЗаписатьНастрокиИсправленияПрошлыхПериодов(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		ЗаписатьИзмененияИзВладельца();
		Модифицированность = Ложь;
		ПрочитатьНастройкиОрганизации();
		УстановитьДоступностьСписка();
		ПерейтиВРазделНалогНаПрибыль();
...
&НаКлиенте
Процедура ИзменениеНастроекПримененияСт54()
	
	Модифицированность = Истина;
	ДатаИзменения = НастройкиУчетаНалогаНаПрибыль.Период;
	ДатаИзмененияЧислом = Год(ДатаИзменения);
	ОтобразитьПериоды("НастройкиУчетаНалогаНаПрибыль");
	НастройкиУчетаНалогаНаПрибыльФормыКлиент.ПроверитьНастройкиПримененияСт54(ЭтотОбъект);
...
Процедура ОтобразитьБаннерНеобходимоСформироватьОстатки()
	...
	Если Не ОрганизацияВелаРаздельныйУчет Тогда
		Элементы.БаннерВключениеРУ.Видимость = НастройкиУчетаНДС.СложныйУчетНДС;
	КонецЕсли;
	Элементы.ГруппаОткрытияДокументовВводОстатков.Видимость = Ложь;
...
Процедура УстановитьПодсказкуВидаТарифаПриИзменении(Форма)
	...
	КонецЕсли;
	ЗарплатаКадрыКлиентСервер.УстановитьРасширеннуюПодсказкуЭлементуФормы(Форма, "ВидТарифа", ВидТарифаПредставление);
