...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПодключенияКОтчетность24.Видимость = ПравоДоступа("Чтение", Метаданные.РегистрыСведений.Отчетность24БРОПодключения);
...
&НаКлиенте
Процедура ПодключенияКОтчетность24Нажатие(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ (ТекущиеДанныеНаименованиеОтчета = "Уведомление по УСН" ИЛИ ТекущиеДанныеНаименованиеОтчета = "Переход на УСН")
				И ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(Ссылка, "ИмяФормы") =
					"Форма2025_1"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.по УСН")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Возражения и подтверждающие документы"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Возражения и подтверждающ") Тогда
