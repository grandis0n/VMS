...
Процедура ВыполнитьОперацию(Команда)
	...
	ПоддерживаемыеТипы = МенеджерОборудованияКлиент.ПараметрыТипыОборудования();
...
Процедура ИнкассацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Доступность = Ложь;   
	ЗаполнитьТекущегоКассира(ДополнительныеПараметры.ИдентификаторУстройства);
...
Процедура ВыполнитьОперациюВыбораУстройства(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ОчиститьСообщения();
	Доступность = Ложь;
	ЗаполнитьТекущегоКассира(РезультатВыбора.ИдентификаторУстройства);
	
	ПараметрыОперации = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыОткрытияЗакрытияСмены();
	ПараметрыОперации.Кассир    = Кассир;   
	ПараметрыОперации.КассирИНН = КассирИНН;
	
	Если ЭтоОблачнаяККТ(РезультатВыбора.ИдентификаторУстройства) Тогда
...
&НаСервере
Процедура ЗаполнитьТекущегоКассира(ПодключенноеОборудование = Неопределено)

	ТекущийКассир = МенеджерОборудованияКлиентСервер.ТекущийКассирДляФискальныхОпераций(ПодключенноеОборудование);
...
&НаСервере
Функция ЭтоОблачнаяККТ(ИдентификаторУстройства)
	
	ТипОборудования = ОбщегоНазначенияБПО.ЗначениеРеквизитаОбъекта(ИдентификаторУстройства, "ТипОборудования");
	Если ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ОблачнаяККТ") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
